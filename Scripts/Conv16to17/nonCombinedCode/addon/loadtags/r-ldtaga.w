...
/*------------------------------------------------------------------------
  ...
  File: addon\loadtags\r-ldtaga.w
...
DEFINE FRAME FRAME-A
     v-ord-list AT ROW 2.91 COL 8 NO-LABEL
     v-job-list AT ROW 2.91 COL 56 NO-LABEL
     begin_ord-no AT ROW 8.14 COL 21 COLON-ALIGNED HELP
          "Enter Beginning Order Number"
     end_ord-no AT ROW 8.14 COL 65 COLON-ALIGNED HELP
     ...
     tb_rel AT ROW 11.48 COL 23
     tb_16ths AT ROW 11.48 COL 62
     rd_print AT ROW 12.67 COL 40 NO-LABEL
     rd_order-sts AT ROW 13.86 COL 40 NO-LABEL
     rd_comps AT ROW 15.05 COL 40 NO-LABEL
     begin_form AT ROW 17.19 COL 19 COLON-ALIGNED
     begin_labels AT ROW 18.14 COL 19 COLON-ALIGNED
     begin_filename AT ROW 19.33 COL 19 COLON-ALIGNED
...
DO:
   apply "close" to this-procedure.
    {Advantzware/WinKit/winkit-panel-triggerend.i} /* added by script _nonAdm1.p on 04.07.2017 @  2:06:15 pm */
END.
...
ON CLOSE OF THIS-PROCEDURE DO:
   RUN disable_UI.
   {Advantzware/WinKit/closewindow-nonadm.i} /* added by script _nonAdm1.p on 04.07.2017 @  2:06:15 pm */
END.
...
/* security check need {methods/prgsecur.i} in definition section */
  ...
  APPLY "entry" TO v-ord-list IN FRAME {&FRAME-NAME}.

    {methods/setButton.i btn-cancel "Cancel"} /* added by script _nonAdm1Images1.p on 04.07.2017 @  2:07:10 pm */
    {methods/setButton.i btn-ok "OK"} /* added by script _nonAdm1Images1.p on 04.07.2017 @  2:07:10 pm */
    {Advantzware/WinKit/embedfinalize-nonadm.i} /* added by script _nonAdm1.p on 04.07.2017 @  2:06:15 pm */
...
------------------------------------------------------------------------------*/
  DISPLAY v-ord-list v-job-list begin_ord-no end_ord-no begin_job begin_job2 
...
DEF INPUT PARAM ip-rowid AS ROWID NO-UNDO.
    ...
    FOR EACH job-hdr
        ...
        NO-LOCK:
          ...
          w-ord.total-tags = ((job-hdr.qty / w-ord.total-unit) + .49) +  IF v-loadtag = "SSLABEL" THEN 0 ELSE 1.
    END.
    ...
    FOR EACH oe-ordl
      ...
      DO:
        ...
        DO:
          ...
          /* Add .49 to round up and add 1 for extra tag   */
          w-ord.total-tags = ((oe-ordl.qty / w-ord.total-unit) + .49) +  (IF v-loadtag = "SSLABEL" THEN 0 ELSE 1).
        END.  /* first-of */
      END.  /* not by-release */
      ...
      FOR EACH oe-rel
        ...
        /* Add .49 to round up and add 1 for extra tag   */
        ASSIGN w-ord.total-tags = ((oe-rel.qty / w-ord.total-unit) + .49) +  IF v-loadtag = "SSLABEL" THEN 0 ELSE 1.
      END.
    END.
...
/*        {sys/msg/rproc.i}
  ...
  */
      ...
      FOR EACH w-ord:
        ...
        DO:
          ...
          DO i = 1 TO (lv-how-many-tags * w-ord.mult):
             /* loadtags generation */
             IF i MOD w-ord.mult = 1 OR i = 1 THEN DO:
                ...
                REPEAT:
                  FIND FIRST loadtag WHERE loadtag.company = g_company
                           ...
                           /*AND STRING (lv-tag-no,"99999") + loadtag.tag-no = STRING(w-ord.i-no,"x(15)") */
                           NO-LOCK NO-ERROR.
                ...
                ASSIGN loadtag.company = cocode
                       loadtag.tag-no =  /*string(lv-tag-no,"99999") + STRING(w-ord.i-no,"x(15)") */
                                        STRING(caps(w-ord.i-no),"x(15)") + string(lv-tag-no,"99999") 
                     ...
                     /*loadtag.po-no = w-ord.po-no*/
                           ...
                           loadtag.pallet-count = w-ord.total-unit /*w-ord.pcs * w-ord.bundle*/
  ...
  IF v-loadtag = "SSBARONE" THEN RUN run-barone (v-out). 
  ELSE IF v-loadtag = "SSLABEL" THEN RUN run-lmw (v-out).
