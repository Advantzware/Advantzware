...
DEF INPUT PARAM ip-sheet-out AS INT NO-UNDO.
...
FOR EACH job-hdr
 ...
 IF job-hdr.frm EQ job-mat.frm OR
  ...
  FOR EACH job-mch
    ...
    DO:
       IF lv-n-on NE 0 THEN
       DO:
          /*task 10141007 job-mch.n-on for sheeter and printer were the same
            even when sheeter was 2 out*/
          IF lv-n-on EQ job-mch.n-on THEN
             ld1 = TRUNC(ld * ip-sheet-out / job-mch.n-on,0).
          ELSE
