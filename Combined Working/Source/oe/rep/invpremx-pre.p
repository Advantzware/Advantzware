...
DEFINE SHARED VARIABLE fcust            LIKE inv-head.cust-no.
DEFINE SHARED VARIABLE tcust            LIKE fcust INIT "zzzzzzzz".
DEFINE SHARED VARIABLE finv             LIKE inv-head.inv-no FORMAT ">>>>>>".
...
FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
    ...
    DO:
        ...
        DO:
            ASSIGN
                cXMLIdentity = sys-ctrl-shipto.char-fld
                cXMLFile     = '/cXMLInvoice.' + xinv-head.cust-no + '.' + REPLACE(REPLACE(STRING(NOW),":",""),"/","") + '.xml' 
                cXMLTemp     = 'XMLOutput' + cXMLFile
                clXMLOutput  = YES
                .
            FIND FIRST sys-ctrl NO-LOCK
                WHERE sys-ctrl.company EQ xinv-head.company
