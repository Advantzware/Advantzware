...
v-address-2 = "Hamilton 905-561-1611 " + CHR(183)
 ...
 END.
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            IF v-printline > 50 THEN DO:
               ...
               PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)"
                   "<FArial><=8><R+7><C1><CENTER=C81><B>65 CASCADE STREET, HAMILTON, ONTARIO L8E 3B7</B>"
                   "<=8><R+8><C1><CENTER=C81>" v-address-2.
               PAGE.
               v-printline = 0.
               {oe/rep/invhpb.i}
            END.
    ...
    ASSIGN
       ...
       v-currency-line = "All amounts are payable in "
                         + (IF cust.curr-code EQ "CAD" THEN "Canadian "
                            ELSE "U.S. ")
                         + "funds.".
    ...
    IF cust.curr-code EQ "CAD" THEN
       PUT "<=8><R+4><C17>" v-currency-line.
    ELSE
       PUT "<=8><R+4><C22>" v-currency-line.

    PUT "<=8><R+5><C40><P14><B> Thank  You!</B><P10>"
