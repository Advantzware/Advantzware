...
DEF VAR v-dock-loc LIKE shipto.dock-loc  NO-UNDO.
DEF VAR v-dock-hrs LIKE shipto.dock-hour NO-UNDO.
DEF VAR v-i-no     LIKE ITEM.i-no        NO-UNDO.
DEF VAR v-i-name   LIKE ITEM.i-name      NO-UNDO.
DEF VAR v-i-desc   LIKE ITEM.i-dscr      NO-UNDO.
...
if v-zone-p then v-zone-hdr = "Route No.:".
    ...
    {oe/rep/foreachr.i},
      ...
      ASSIGN
          v-dock-loc = shipto.dock-loc 
          v-dock-hrs = shipto.dock-hour.
      ...
      FOR EACH xoe-rell
        ...
        FOR EACH oe-rel WHERE 
            ...
            oe-rel.ord-no  EQ oe-ord.ord-no  AND
            oe-rel.i-no    EQ xoe-rell.i-no NO-LOCK BY oe-rel.rel-date 
                                                    BY oe-rel.po-no
                                                    BY oe-rel.ship-no 
                                                    BY oe-rel.qty:
             ASSIGN
      ...
      for each w-oe-rell,
        ...
        if last-of(w-oe-rell.po-no) then do:
          ...
          IF w-oe-rell.ord-no NE 0 THEN DO:
            FIND FIRST oe-ord NO-LOCK
              WHERE oe-ord.company EQ w-oe-rell.company 
                AND oe-ord.ord-no EQ w-oe-rell.ord-no NO-ERROR.
            IF AVAIL oe-ord AND TRIM(oe-ord.est-no) NE "" THEN DO:
          ...
          for each w-bin break by w-qty[2] desc by w-qty[1] desc:
             ...
             IF fgrecpt-int EQ 1 
               THEN v-bin = IF NOT ll-consol-rells 
                              THEN TRIM(w-loc) + "/" +
                                   TRIM(w-bin) + "/" + 
                                   TRIM(SUBSTR(w-tag,16,5)) 
                              ELSE (TRIM(w-bin) + "/" + FILL(" ",9)).
               ELSE v-bin = IF NOT ll-consol-rells 
                              THEN TRIM(w-loc) + "/" +
                                   TRIM(w-bin) + "/" + 
                                   TRIM(w-tag)
                              ELSE (TRIM(w-bin) + "/" + FILL(" ",9)).
            ...
            if last(w-qty[2]) then do:
              ...
              PUT {2} "<B><P12>" "Rel Qty"  at 68 
                      v-rel-qty     AT 77  "</P12></B>" SKIP.
