...
DEF SHARED VAR s-print-what-item AS CHAR NO-UNDO.
DEF SHARED VAR s-print-loc-from  AS CHAR NO-UNDO.
DEF SHARED VAR s-print-loc-to    AS CHAR NO-UNDO.
DEF SHARED VAR s-print-bin-from  AS CHAR NO-UNDO.
DEF SHARED VAR s-print-bin-to    AS CHAR NO-UNDO.
...
format header
       ...
       "Date      Number"                           at 62                          
       v-ticket-date                                at 60
       trim(string(oe-relh.release#,">>>>>>>>"))    at 71
...
/*         if last-of(oe-rell.po-no) then do: */
            ...
            IF s-print-what-item = "R" AND
               NOT CAN-FIND(FIRST bf-oe-rell
                                 WHERE bf-oe-rell.company  EQ oe-rell.company
                                   ...
                                   AND bf-oe-rell.tag      EQ fg-bin.tag)
            THEN NEXT.
          ...
          for each w-bin break by w-qty[2] desc by w-qty[1] desc:               
            ...
            ASSIGN v-reccnt = v-reccnt + IF first(w-qty[2]) THEN 1 ELSE 0.
            ...
            IF s-print-what-item = "R"
              ...
              FIND FIRST bf-oe-rell NO-LOCK
                WHERE bf-oe-rell.company  EQ oe-rell.company 
                  ...
                  AND bf-oe-rell.b-ord-no EQ oe-rell.b-ord-no
                  AND bf-oe-rell.po-no    EQ oe-rell.po-no   
                  AND bf-oe-rell.loc      EQ w-bin.w-loc      
                  AND bf-oe-rell.loc-bin  EQ w-bin.w-bin  NO-ERROR.
              IF AVAIL bf-oe-rell THEN ASSIGN w-cas = bf-oe-rell.cases.
            END.

            /* gdm - 10020901 */
            IF v-reccnt GE 6 THEN DO:
                PAGE.
                v-reccnt = 1.
            END.
            /* gdm - 10020901 */
