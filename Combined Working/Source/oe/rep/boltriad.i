...
for each report where report.term-id eq v-term,
  ...
  if last-of(report.key-03) then do:
    ...
    for each w2 break by w2.cas-cnt:
      ...
      assign
       i           = i + 1
       v-part-comp = if v-ship-qty + v-bol-qty ge v-ord-qty then "C"
	                                                        else "P".

      if first(w2.cas-cnt) and j eq 0 then
	    v-part-dscr = trim(string(int(report.key-02),">>>>>>")).

      else
      do j = j + 1 to 7:
	    v-part-dscr = if j eq 2 then itemfg.i-name
		              else
		              if j eq 3 then itemfg.part-dscr1
		              else
		              if j eq 4 then itemfg.part-dscr2
		              else
		              if j eq 5 then itemfg.part-no
		              else
		              if j eq 6 then itemfg.i-no
		              else "".
      ...
      display {1}
              ...
              v-part-dscr
	          report.key-03         when first(w2.cas-cnt)
	          w2.cases
	          w2.cas-cnt
	          v-bol-qty             when last(w2.cas-cnt)
	          v-part-comp           when last(w2.cas-cnt)
