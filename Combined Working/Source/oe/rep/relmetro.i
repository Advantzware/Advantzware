...
def TEMP-TABLE w-bin NO-UNDO
   ...
   FIELD w-relqty AS CHAR
    .

DEFINE TEMP-TABLE ttFGNote
    FIELD rec_key LIKE itemfg.rec_key.
...
DEF VAR v-ship-i AS cha FORM "x(100)" EXTENT 4 NO-UNDO.
...
/*             MESSAGE "Bin Qty: " fg-bin.qty "Case count: " fg-bin.case-count " cases/unit " fg-bin.cases-unit " units/pallet " fg-bin.units-pallet "  = " w-bin.w-pal */
/*                 VIEW-AS ALERT-BOX INFO BUTTONS OK.                                                                                                                   */
          end. /* for each fg-bin */
/*           w-pal = v-rel-qty / (oe-ordl.cas-cnt * oe-ordl.cases-unit). */
/*           {sys/inc/roundup.i w-pal}                                   */
          ...
          IF NOT ll-consol-rells OR ship-flag THEN DO:
            ...
            for each w-bin where w-pal eq 0 by w-qty[2] desc by w-qty[1] desc:
             w-pal = v-rel-qty / (oe-ordl.cas-cnt * oe-ordl.cases-unit).
              {sys/inc/roundup.i w-pal}
              leave.
            end.  
          ...
          for each w-bin break by w-qty[2] desc by w-qty[1] desc:
             ...
             /*IF trim(v-bin) eq "//Rel Qty:" then v-bin = "<b>      Rel Qty</b>     ". */
...
/*                          ELSE w-subq @ w-qty[1]  */
               ...
               IF w-relqty  = "relqty" THEN do:
                   PUT  "<R-1> <B>" "Rel Qty"  at 76 
                      v-rel-qty     AT 83  "</B>" . 
               END.
            ...
            if last(w-qty[2]) then do:
              ...
              FIND FIRST ttFGNote WHERE ttFGNote.rec_key EQ w-oe-rell.rec_key
              NO-LOCK NO-ERROR.
              IF NOT AVAIL ttFGNote THEN DO:
                  CREATE ttFGNote.
                  ttFGNote.rec_key = w-oe-rell.rec_key.
                  FOR EACH notes FIELDS(note_text) WHERE
                      notes.rec_key EQ w-oe-rell.rec_key AND
                      notes.note_type EQ "D" AND
...
/*               PUT {2} "<#10><From><C+80><LINE><||6><P11>" SKIP. */
  ...
  DO i = 1 TO 20:
     IF v-spec-text[i] NE "" THEN
     DO:
        PUT "<FArial><R" + STRING(55 - spec-note-count) + ">     " + v-spec-text[i] FORM "x(200)" SKIP.
        spec-note-count = spec-note-count - 1.
     END.
  END.
