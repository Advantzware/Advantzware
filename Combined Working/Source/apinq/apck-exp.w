...
ASSIGN cTextListToSelect = "Check#,Chk Date,Invoice#,Due Date,Gross Amount,Net Amount,Discount"

      cFieldListToSelect = "chk,chk-dt,inv,due-dt,grs-amt,net-amt,disc" .
{sys/inc/ttRptSel.i}

    ASSIGN cTextListToDefault  = "Check#,Chk Date,Invoice#,Due Date,Gross Amount,Net Amount,Discount".
...
begin_chk end_chk begin_inv end_inv begin_date end_date sl_avail ~
sl_selected Btn_Def Btn_Add Btn_Remove btn_Up btn_down tb_runExcel fi_file ~
btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_vend end_vend begin_chk end_chk ~
begin_inv end_inv begin_date end_date sl_avail sl_selected tb_excel ~
tb_runExcel fi_file 
...
DEFINE VARIABLE begin_chk AS CHARACTER FORMAT "x(10)" 
     LABEL "From Check#" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.
...
DEFINE VARIABLE end_chk AS INT FORMAT ">>>>>>>9" INITIAL "99999999" 
     LABEL "To Check#" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.
...
DEFINE FRAME rd-fgnq-exp
     begin_vend AT ROW 3.33 COL 25.6 COLON-ALIGNED HELP
          "Enter Beginning Vendor#" WIDGET-ID 142
     end_vend AT ROW 3.43 COL 67.6 COLON-ALIGNED HELP
          "Enter Ending Vendor#" WIDGET-ID 144
     begin_chk AT ROW 4.48 COL 25.6 COLON-ALIGNED HELP
          "Enter Beginning Vendor#" WIDGET-ID 146
     end_chk AT ROW 4.57 COL 67.6 COLON-ALIGNED HELP
          "Enter Ending Vendor#" WIDGET-ID 148
     begin_inv AT ROW 5.67 COL 25.6 COLON-ALIGNED HELP
          "Enter Beginning Vendor#" WIDGET-ID 150
     end_inv AT ROW 5.76 COL 67.6 COLON-ALIGNED HELP
          "Enter Ending Vendor#" WIDGET-ID 152
     begin_date AT ROW 6.86 COL 25.6 COLON-ALIGNED HELP
          "Enter Beginning Vendor#" WIDGET-ID 154
     end_date AT ROW 6.95 COL 67.6 COLON-ALIGNED HELP
          "Enter Ending Vendor#" WIDGET-ID 156
...
------------------------------------------------------------------------------*/
  DISPLAY begin_vend end_vend begin_chk end_chk begin_inv end_inv begin_date 
          end_date sl_avail sl_selected tb_excel tb_runExcel fi_file 
      WITH FRAME rd-fgnq-exp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_vend end_vend begin_chk end_chk begin_inv 
         end_inv begin_date end_date sl_avail sl_selected Btn_Def Btn_Add 
    ...
    FOR EACH tt-report NO-LOCK:
       ...
       FOR EACH ttRptSelected:
         /* IF LOOKUP(ttRptSelected.TextList, "Contact,Title,Email") <> 0    THEN do: */
              IF ttRptSelected.TextList = "Check#" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.check-no)).
              IF ttRptSelected.TextList = "Chk Date" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.check-date)).
              IF ttRptSelected.TextList = "Invoice#" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.inv-no)).
              IF ttRptSelected.TextList = "Due Date" THEN do:
                  IF tt-report.due-date NE ? THEN
                      v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.due-date)).
                  ELSE 
                      v-excel-detail-lines = v-excel-detail-lines + appendXLLine("").
              END.
              IF ttRptSelected.TextList = "Gross Amount" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.gross-amt)).
              IF ttRptSelected.TextList = "Net Amount" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.amt-disc)).
              IF ttRptSelected.TextList = "Discount" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(tt-report.amt-paid)).
...
FOR EACH ap-pay
    WHERE ap-pay.company  EQ cocode
      ...
      AND ap-pay.check-no GE INT(begin_chk)
      AND ap-pay.check-no LE int(end_chk) 
    ...
    WHERE ap-payl.c-no    EQ ap-pay.c-no
      AND ap-payl.inv-no  GE begin_inv
      AND ap-payl.inv-no  LE end_inv 
      AND ap-payl.due-date GE date(begin_date)
      AND ap-payl.due-date LE date(end_date)
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF begin_date:SCREEN-VALUE = "" AND end_date:SCREEN-VALUE = "" THEN
          RETURN TRUE.

      /* If start date is entered and check date is earlier, then omit. */
      IF begin_date:SCREEN-VALUE <> "" AND 
          pdDate < date(begin_date:SCREEN-VALUE) THEN
          RETURN FALSE.

      /* If end date is entered and check date is later, then omit. */
      IF end_date:SCREEN-VALUE <> "" AND 
          pdDate > date(end_date:SCREEN-VALUE) THEN
          RETURN FALSE.
