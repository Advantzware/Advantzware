/* ------------------------------------------------- cerep/jobottpk.p  */
/*  factory ticket  for folding , Ott Package                                 */
/* -------------------------------------------------------------------------- */
...
DEF VAR v-jobcard-int AS INT NO-UNDO.
...
FIND FIRST sys-ctrl WHERE sys-ctrl.company = cocode
                      ...
                      NO-LOCK NO-ERROR.
v-jobcard-int = IF AVAIL sys-ctrl THEN sys-ctrl.int-fld ELSE 0.
...
/* build tt-reft est-type 4 has no all reftable*/
FOR EACH job-hdr NO-LOCK
  ...
  FOR EACH reftable NO-LOCK
     ...
     CREATE tt-reft.
     BUFFER-COPY reftable TO tt-reft
     ASSIGN
      tt-reft.row-id   = ROWID(job-hdr)
      tt-reft.est-type = est.est-type.
  END.
  ELSE DO:
    FIND FIRST tt-reft
        WHERE tt-reft.reftable EQ "jc/jc-calc.p"
          AND tt-reft.company  EQ job-hdr.company
          AND tt-reft.loc      EQ ""
          AND tt-reft.code     EQ STRING(job-hdr.job,"999999999")
          AND tt-reft.val[12] = job-hdr.frm
          AND tt-reft.val[13] = job-hdr.blank-no   
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE tt-reft THEN DO:
      CREATE tt-reft.
      ASSIGN
       tt-reft.row-id   = ROWID(job-hdr)
       tt-reft.val[12]  = job-hdr.frm
       tt-reft.val[13]  = job-hdr.blank-no
       tt-reft.est-type = est.est-type.
    END.
  END.
END.
/* end of tt-reft building*/

FOR EACH tt-reft,
    FIRST job-hdr WHERE ROWID(job-hdr) EQ tt-reft.row-id EXCLUSIVE-LOCK,
    FIRST est NO-LOCK
    WHERE est.company  EQ job-hdr.company
      AND est.est-no   EQ job-hdr.est-no
...
if first-of(tt-reft.val[12]) then v-first = yes.
...
view frame head.
        ...
        PUT "<B>Customer Name:</B>" v-cust-name 
            "<B>ORD DATE:   DUE DATE:   Estimate: <C81>SAMPLE" SKIP
      ...
      for each ef
        ...
        if ef.form-no eq tt-reft.val[12] then ebloop:
        for each eb
            WHERE eb.company     EQ ef.company
              AND eb.est-no      eq ef.est-no
        ...
        /*  if last-of(eb.form-no) then do: */
            ...
            /*if v-first then*/
            v-upc-lbl = /*IF s-prt-sellprc THEN "Sell Price" ELSE "   UPC#".*/  "   CAD#" /* juse cad# print */.
            IF FIRST-OF(eb.form-no) THEN
              PUT /*"<R-1>" "Case" AT 97 SKIP */
                "<P9><B>F/B   FG Item #         Order Qty   Job Qty  PO#            Description      Style  Carton Size      Case Size       Cases  Count  #UP" /*v-upc-lbl*/ "</B>" SKIP
           ...
           find first oe-ordl
            ...
            display trim(string(eb.form-no,">>9")) + "-" +
                   ...
                   /* oe-ordl.cas-cnt when avail oe-ordl
                    eb.cas-cnt when (not avail oe-ordl) or oe-ordl.cas-cnt eq 0 @ oe-ordl.cas-cnt   #01240503 */                    
                    /*PACE(4)*/
                    v-case-size  FORM "x(15)"
                    v-case-qty   FORM ">>>>9"    /* #01240503*/
                    v-case-count FORM ">>>>>9"
                    v-up FORM ">>>9"
                   /* eb.cad-no  *//* #01240503*/
                    /*eb.upc-no 
                    string(oe-ordl.price,">>>,>>9.99<<<<")
                            WHEN AVAIL oe-ordl AND s-prt-sellprc @ eb.upc-no  */ /* #01240503*/
                    skip
                with stream-io width 175 no-labels no-box frame line-det1.

            v-itm-printed = v-itm-printed + 1.    
            v-tot-up = v-tot-up + v-up.
            find first item
          ...
          IF LAST-OF(eb.form-no) THEN DO:
             ...
             PUT "<P10>" v-fill SKIP                       /*REQ'D*/
                /* "<B>BOARD CODE            DUE DATE    SHEETS SHEET SIZE   NET SHEET  BOARD PO# VENDOR#  DIE SIZE     DIE#          STRIPPER LOCATION</B>" */
                   "<B>BOARD CODE            VENDOR      SHEETS       LBS SHEET SIZE       NET SHEET        DIE SIZE          TOT #UP DIE#          </B>" 
                 SKIP.
            /** PRINT SHEET **/
             x = 2.
             for each wrk-sheet WHERE wrk-sheet.form-no = ef.form-no
               ...
               IF ef.xgrain EQ "S" AND v-jobcard-int = 1 THEN 
                   ASSIGN v-die-size = string(ef.trim-l) + "x" + string(ef.trim-w)
                          v-net-size = string(ef.nsh-len) + "x" + string(ef.nsh-wid).
               ELSE ASSIGN v-die-size = string(ef.trim-w) + "x" + string(ef.trim-l)
                ...
                IF LAST-OF(wrk-ink.i-code) THEN DO:
                    ...
                    ELSE IF wrk-ink.i-pass = 2 THEN
                       ASSIGN v-ink2[i] = "F    " + /*string(wrk-ink.i-code,"X(15)")*/ STRING(v-i-qty[i],"->>>,>>>,>>9.99") + " " + 
                        ...
                        ASSIGN v-ink1[i] = string(wrk-ink.i-pass) + "   " +
...
/*
      ...
      if last-of(tt-reft.val[12]) then do:
         IF s-run-speed THEN
            PUT "<B>MACHINE                 MR WASTE   MR HRS  RUN SPEED   SPOIL%    SHEETS PER MACHINE   SIZE  FILM REQUIRED   PALLET  SIZE</B>"
                SKIP.
         else
            PUT "<B>MACHINE                 MR WASTE   MR HRS   RUN HOUR    SPOIL%    SHEETS PER MACHINE   SIZE  FILM REQUIRED   PALLET  SIZE</B>"
                SKIP.

         FOR EACH wrk-op WHERE wrk-op.s-num = tt-reft.val[12] BREAK by wrk-op.d-seq by wrk-op.b-num:
             v-mat-for-mach = "".
             /*IF lookup(wrk-op.dept,lv-mat-dept-list) > 0 THEN DO:                 
                FOR EACH xjob-mat WHERE xjob-mat.company eq cocode
                                       ...
                                       AND xjob-mat.frm = int(tt-reft.val[12])
             ...
             IF LAST(wrk-op.d-seq) THEN DO: /* pallet code*/
                FOR EACH xjob-mat WHERE xjob-mat.company eq cocode
                    ...
                    v-mat-for-mach = /*ITEM.i-name + fill(" ", 33 - LENGTH(ITEM.i-name))  /*"       " */ + */
                                      string(xjob-mat.wid) + "x" + STRING(xjob-mat.len) +
                                      "      " + string(xjob-mat.qty) + "         " + ITEM.i-no + "   " +
                                      string(ITEM.case-l) + "x" + STRING(ITEM.case-w) + "x" + STRING(ITEM.case-d).                   
                 END.
             END.          
             IF s-prt-mstandard THEN DO:
                IF s-run-speed THEN
                   PUT wrk-op.m-dscr   SPACE(5)
                       wrk-op.mr-waste[int(tt-reft.val[12])]   SPACE(5)
                       wrk-op.mr[int(tt-reft.val[12])]         SPACE(5)
                       wrk-op.speed[int(tt-reft.val[12])]      SPACE(5)
                       wrk-op.spoil[int(tt-reft.val[12])]      SPACE(5)
                       wrk-op.num-sh[int(tt-reft.val[12])]     SPACE(11)  /*9+9*/
               ...
               ELSE
                   PUT wrk-op.m-dscr   SPACE(5)
                       wrk-op.mr-waste[int(tt-reft.val[12])]   SPACE(5)
                       wrk-op.mr[int(tt-reft.val[12])]         SPACE(5)
                       wrk-op.run-hr[int(tt-reft.val[12])]     SPACE(5)
                       wrk-op.spoil[int(tt-reft.val[12])]      SPACE(5)
                       wrk-op.num-sh[int(tt-reft.val[12])]     SPACE(11)  /* 9 + 9 */
                       v-mat-for-mach FORM "x(40)"    /*60*/
                       SKIP.
