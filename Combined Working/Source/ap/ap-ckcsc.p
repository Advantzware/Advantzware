...
form skip(6)
     ap-chk.check-no   to 79
     skip(1)
     dol               at 10 skip
     skip(2)
     ap-chk.check-date to 61 FORM "99/99/99"
...
if v-print-mode ne "align" then do:
  for each ap-chk
    ...
    put ap-sel.inv-no             at 4  format "x(12)"
        ap-sel.dscr               at 17 format "x(19)"
    ...
    if last-of(ap-sel.vend-no) then do:
      ...
      put skip(max-per-chk - ll)
          "--------------"  to 80 skip
          "Check Date = "   at 29
          ap-chk.check-date to 49 FORM "99/99/99"
          "Check Total = "  at 54
          ctot              to 80 skip.
