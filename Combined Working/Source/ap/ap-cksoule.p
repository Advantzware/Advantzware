...
/*------------------------------------------------------------------------
    File        : ap/ap-cksoule.p
    Purpose     : Generic utility to print AP checks on pre-printed check forms.
...
DEF VAR viLn-Check      AS INT NO-UNDO INIT 25. /* Line to start printing the check. */
DEF VAR viLn-Void       AS INT NO-UNDO INIT 27. /* Line to start printing VOID. */
DEF VAR viLn-Section-2  AS INT NO-UNDO INIT 41. /* Line to start printing bottom section details. */
  ...
  check-block:
   FOR EACH ap-chk
      ...
      FIRST vend WHERE vend.company eq cocode AND 
       ...
       RUN Get-Remit-Address (OUTPUT add1, OUTPUT add2).
...
&IF DEFINED(EXCLUDE-get-check-num) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-check-num Procedure 
PROCEDURE get-check-num :
/*------------------------------------------------------------------------------
  Purpose:     Get the next check number.
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/

/*   DEFINE BUFFER buf-ap-sel FOR ap-sel.                                              */
/*   DEFINE VARIABLE vi AS INT NO-UNDO INIT 0.                                         */
/*                                                                                     */
/*                                                                                     */
/*    FOR EACH buf-ap-sel                                                              */
/*        where buf-ap-sel.company   eq cocode                                         */
/*          and buf-ap-sel.vend-no   eq ap-chk.vend-no                                 */
/*          and buf-ap-sel.man-check eq no                                             */
/*        no-lock                                                                      */
/*        break by buf-ap-sel.inv-no:                                                  */
/*                                                                                     */
/*      ASSIGN vi = vi + 1.                                                            */
/*                                                                                     */
/*      IF vi eq viLn-Detail-Max /*max-per-chk*/ and not last(buf-ap-sel.inv-no) then  */
/*        assign                                                                       */
/*         stnum = stnum + 1.                                                          */
/*    END.                                                                             */
/*                                                                                     */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
 DEF OUTPUT PARAM pcAdd1 AS CHAR NO-UNDO INIT "".
 DEF OUTPUT PARAM pcAdd2 AS CHAR NO-UNDO INIT "".
...
------------------------------------------------------------------------------*/
     ...
     /* Print a detail line. */
     PUT /*LineNum() FORMAT "99" AT 1 */
         wrk-chk.inv-no                TO 15 FORMAT "x(12)"
         /* PO num causing issues, customer doesn't use it anyway. */
/*         trim(string(wrk-chk.po-no,">>>>>>")) to 20 format "x(12)" */
        wrk-chk.inv-date              TO 30 FORMAT "99/99/99"
        wrk-chk.inv-amt               TO 43 FORMAT "->>>,>>9.99"
        wrk-chk.amt-paid              TO 55 FORMAT "->>>,>>9.99"
...
------------------------------------------------------------------------------*/
  ...
  /* Skip a line. 
     Soule preprinted checks are aligned differently on the bottom portion
     so after printing vendor info, skip 1 line at top and 2 lines at bottom
     in order to get the lines where we want them. */
/*   IF viCurrentSection = 1 THEN  */
      skipLines(1).
/*   ELSE               */
/*       skipLines(2).  */
