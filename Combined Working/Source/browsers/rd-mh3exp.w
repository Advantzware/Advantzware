...
ASSIGN cTextListToSelect = "Emp ID,Name,Machine,Start Date,Started,End Date,Shift,Ended,Type,Posted,Total"
       cFieldListToSelect = "machemp.employee,name,machtran.machine,start_date,strd,end_date,machemp.shift,ended,machemp.ratetype,machemp.posted,total"
    .

{sys/inc/ttRptSel.i}

    ASSIGN cTextListToDefault  = "Emp ID,Name,Machine,Start Date,Started,End Date,Shift,Ended,Type,Posted,Total" .
...
FOR EACH machemp WHERE machemp.posted = YES 
    ...
    AND machtran.machine LE END_mch NO-LOCK:

    v-excel-detail-lines = "".
   
    FOR EACH ttRptSelected:

        IF lookup(ttRptSelected.FieldList,"name,strd,ended,total,start_date,end_date") EQ 0 THEN do:
        ...
        ELSE do:
         CASE ttRptSelected.FieldList:                                                                                       
             ...
             END.
             WHEN "start_date" THEN do:
                 IF machemp.start_date NE ? THEN
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING(machemp.start_date)).          
                 ELSE
                     v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING("")).    
             END.
             WHEN "end_date" THEN do:
                 IF machemp.end_date NE ? THEN
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING(machemp.end_date)).          
                 ELSE
                     v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING("")).    
             END.
             WHEN "strd" THEN do:
