...
&Scoped-define FIELDS-IN-QUERY-Browser-Table machchrg.charge_code ~
Job-Code_Cat(machchrg.charge_code) @ job-code_cat ~
Job-Code_Dscr(machchrg.charge_code) @ job-code_dscr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH machchrg WHERE TRUE /* Join to company incomplete */ ~
      AND machchrg.company = mach.company ~
AND machchrg.machine = mach.m-code ~
AND ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH machchrg WHERE TRUE /* Join to company incomplete */ ~
      AND machchrg.company = mach.company ~
AND machchrg.machine = mach.m-code ~
AND ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table machchrg
&Scoped-define FIRST-TABLE-IN-QUERY-Browser-Table machchrg
...
DEFINE QUERY Browser-Table FOR 
      machchrg
    FIELDS(machchrg.charge_code
      machchrg.charge_code
      machchrg.charge_code) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      machchrg.charge_code FORMAT "X(5)":U LABEL-BGCOLOR 14
      Job-Code_Cat(machchrg.charge_code) @ job-code_cat COLUMN-LABEL "Cat" FORMAT "X(3)":U
            WIDTH 5
      Job-Code_Dscr(machchrg.charge_code) @ job-code_dscr COLUMN-LABEL "Description" FORMAT "X(40)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 75 BY 18.1
         FONT 2.
...
DEFINE FRAME F-Main
     ...
     fi_sortby AT ROW 19.33 COL 14 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     auto_find AT ROW 19.33 COL 35 COLON-ALIGNED HELP
          "Enter Auto Find Value"
     Btn_Clear_Find AT ROW 19.33 COL 62 HELP
...
"Job-Code_Dscr(machchrg.charge_code) @ job-code_dscr" "Description" "X(40)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
/*------------------------------------------------------------------------------
  ...
  Parameters:  OUTPUT company & machine
  Notes:       
------------------------------------------------------------------------------*/
