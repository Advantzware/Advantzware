...
&SCOPED-DEFINE browse2 methods/browsers/empalertBrowse3.i
...
&Scoped-define INTERNAL-TABLES empalert cust users
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH empalert WHERE NOT empalert.table_rec_key = '' NO-LOCK, ~
      FIRST cust WHERE cust.rec_key EQ empalert.table_rec_key NO-LOCK, ~
      FIRST users WHERE /*users.rec_key EQ empalert.titlcod reversed */ ~
                     users.USER_id EQ empalert.USER-ID NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH empalert WHERE NOT empalert.table_rec_key = '' NO-LOCK, ~
      FIRST cust WHERE cust.rec_key EQ empalert.table_rec_key NO-LOCK, ~
      FIRST users WHERE users.USER_id EQ empalert.USER-ID NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table empalert cust users
&Scoped-define FIRST-TABLE-IN-QUERY-Browser-Table empalert
&Scoped-define SECOND-TABLE-IN-QUERY-Browser-Table cust
&Scoped-define THIRD-TABLE-IN-QUERY-Browser-Table users
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD GetCurrBillTo B-table-Win 
FUNCTION GetCurrBillTo RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      cust.cust-no FORMAT "x(8)":U WIDTH 10 LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      cust.name FORMAT "x(30)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      users.user_id COLUMN-LABEL "User ID" FORMAT "X(8)":U WIDTH 10
            LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      users.user_name FORMAT "X(30)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DO:
  ...
  {methods/run_link.i "CONTAINER-SOURCE" "setempalertValues" "('Customer',ROWID(cust))"}
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    ...
    IF NOT AVAILABLE empalert OR
       CAN-FIND(FIRST emaildtl WHERE emaildtl.emailcod EQ emailcod.emailcod
                AND emaildtl.table_rec_key EQ empalert.rec_key) THEN NEXT.
    CREATE emaildtl.
    ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION GetCurrBillTo B-table-Win 
FUNCTION GetCurrBillTo RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
