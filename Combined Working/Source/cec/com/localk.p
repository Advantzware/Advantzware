...
DEF TEMP-TABLE tt-est-op NO-UNDO LIKE est-op FIELD row-id AS ROWID.
        ...
        WHERE est-op.company EQ tt-est-op.company
          ...
          AND CAN-FIND(FIRST b-ef
                       WHERE b-ef.company EQ est-op.company
                         ...
                         AND b-ef.n-out-d EQ xef.n-out-d
                         USE-INDEX est-qty)
...
vn-out = (IF b-ef.n-out   EQ 0 THEN 1 ELSE b-ef.n-out)   *
         ...
         (IF b-ef.n-out-d EQ 0 THEN 1 ELSE b-ef.n-out-d).

RUN cec/isitfoam.p (ROWID(b-ef), OUTPUT ll-foam).

RELEASE ef-nsh.

IF ll-foam THEN
FIND FIRST ef-nsh OF b-ef NO-LOCK NO-ERROR.

IF AVAIL ef-nsh THEN
RUN est/ef-#out.p (ROWID(b-ef), OUTPUT vn-out).
...
FOR EACH eb
  ...
  v-eb-qty = IF xest.est-type EQ 7 THEN eb.bl-qty ELSE eb.yld-qty.
  ...
  FOR EACH b-ef1
      WHERE b-ef1.company EQ b-ef.company
        ...
        AND b-ef1.n-out-d EQ b-ef.n-out-d
    ...
    RUN sys/inc/numup.p (b-ef1.company,b-ef1.est-no,b-ef1.form-no,OUTPUT v-num-up1).

    IF v-num-up EQ v-num-up1 THEN
    FOR EACH b-eb FIELDS(bl-qty yld-qty) OF b-ef1
        ...
        NO-LOCK:
      v-eb-qty = v-eb-qty + IF xest.est-type EQ 7 THEN b-eb.bl-qty ELSE b-eb.yld-qty.
    END.
  END.
  ...
  IF AVAIL ef-nsh THEN DO:
    RUN cec/foamblks.p (ROWID(xef), OUTPUT w-qty.num-bl).
    
    w-qty.num-bl = v-eb-qty / w-qty.num-bl.

    {sys/inc/roundup.i w-qty.num-bl}

    ASSIGN
     t-shtfrm[xef.form-no] = w-qty.num-bl
     v-eb-qty              = w-qty.num-bl * v-num-up * vn-out.
  END.
...
FOR EACH est-op
  ...
  /*IF ip-int EQ 3 THEN DO:
    ...
    v-spo[1] = (cumul / (1 - (est-op.op-spoil / 100))) - cumul.
    {sys/inc/roundup.i v-spo[1]}

    ASSIGN
     est-op.num-sh = est-op.num-sh + v-spo[1]
     r-spo[b-ef.form-no] = r-spo[b-ef.form-no] + v-spo[1]
     v-spo[1] = est-op.op-waste /
                IF est-op.op-sb THEN v-n-out ELSE (v-num-up * vn-out).
  ...
  ELSE DO:*/
    ...
    IF INDEX(PROGRAM-NAME(5),"est/oeselest") GT 0 AND
