...
FOR EACH xqitm OF xquo NO-LOCK BREAK BY xqitm.part-no:
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */:
      ...
      */
      PUT TRIM(lv-est-no) FORM "x(5)" AT 2
          ...
          /* gdm - 11040801 deducted 2 char from format, used to be 30 - now 28*/
           TRIM(lv-part-dscr1) AT 29 FORMAT "x(20)". 
    ...
    IF i EQ 5 /* AND numfit GE 5 */ THEN DO:
       ...
       IF NOT logSetPrinting AND s-print-2nd-dscr THEN
    ...
    IF AVAIL xqqty THEN DO:
       ...
       */
        /* used a different formula for extended amt copied quofib-xl.i 
           as per EP */
    ...
    /*  IF i EQ 3 THEN
       ...
       */
       ...
       put xqqty.qty FORMAT ">>>>>>>9" TO 57
           xqqty.rels space(5)
           xqqty.price FORM "->>,>>9.99" space(2)
           xqqty.uom SPACE(2)
           xxx FORM "->>>>,>>9.99".   
