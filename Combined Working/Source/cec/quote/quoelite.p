/* ------------------------------------------- cec/quote/quoelite.p 05/05 YSK */
/* print quotes in Elite format                                               */
/* -------------------------------------------------------------------------- */
    ...
    if first-of(report.key-01) OR s-sep-page then do:      
      ...
      IF ch-inst THEN DO:
         FOR EACH bf-report where bf-report.term-id eq v-term-id
             ...
             ln-cnt = ln-cnt + 1.
             IF v-inst2[1] <> "" THEN ln-cnt = ln-cnt + 1.
             IF v-inst2[2] <> "" THEN ln-cnt = ln-cnt + 1.
             IF v-inst2[3] <> "" THEN ln-cnt = ln-cnt + 1.
             IF v-inst2[4] <> "" THEN ln-cnt = ln-cnt + 1.
             IF v-inst2[5] <> "" THEN ln-cnt = ln-cnt + 1.
             IF v-inst2[6] <> "" THEN ln-cnt = ln-cnt + 1.
         END.
      END.
      lv-tot-pg = lv-tot-pg + TRUNC( ln-cnt / 25,0) .
      /* get total page number */
