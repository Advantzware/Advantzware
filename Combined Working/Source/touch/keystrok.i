...
&IF '{&KEYSTROKE}' EQ 'ALLMACHS' OR
    '{&KEYSTROKE}' EQ 'ALLJOBS'  &THEN
...
DO WITH FRAME {&FRAME-NAME}:
 ...
 IF i GT SL:NUM-ITEMS THEN DO:
  ...
  ASSIGN
    ...
    item = i.
END.
&ELSEIF '{&KEYSTROKE}' EQ 'PASSWORD' &THEN
h_field:SCREEN-VALUE = FILL('*',LENGTH(field_value)).
&ENDIF

IF VALID-HANDLE(h_field) THEN
h_field:SET-SELECTION(LENGTH(h_field:SCREEN-VALUE) + 1,LENGTH(h_field:SCREEN-VALUE) + 1).
