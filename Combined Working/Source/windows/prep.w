...
ON END-ERROR OF W-Win /* Preparation Codes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Preparation Codes */
DO:
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_prep-2 ).
       RUN set-position IN h_prep-2 ( 4.57 , 9.00 ) NO-ERROR.
       /* Size in UIB:  ( 17.71 , 131.00 ) */
       ...
       /* Links to SmartViewer h_prep-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_prep-2 ).
       RUN add-link IN adm-broker-hdl ( h_prep , 'Record':U , h_prep-2 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_prep-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_prep-2 ,
             h_folder , 'AFTER':U ).
