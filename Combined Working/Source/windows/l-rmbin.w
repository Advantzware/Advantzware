...
def input parameter ip-company like item.company no-undo.
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH rm-bin WHERE ~{&KEY-PHRASE} ~
 ...
 AND rm-bin.i-no = "" ~
 AND (ASI.rm-bin.loc = ip-loc or ip-loc eq "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* Query rebuild information for BROWSE BROWSE-1
 ...
 AND ASI.rm-bin.i-no = """"
 AND (ASI.rm-bin.loc = ip-loc or ip-loc eq """")"
...
"rm-bin.tag" "Tag" "x(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
