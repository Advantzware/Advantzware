...
DEFINE VARIABLE h_reftable AS HANDLE NO-UNDO.
DEFINE VARIABLE h_reftable-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Stack Patterns */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Stack Patterns */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Stack Pt|View Stack Pt' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/reftable.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_reftable ).
       RUN set-position IN h_reftable ( 4.81 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.52 , 92.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2') NO-ERROR.

       /* Links to SmartNavBrowser h_reftable. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_reftable ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/reftable.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_reftable-2 ).
       RUN set-position IN h_reftable-2 ( 4.81 , 4.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_reftable-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_reftable-2 ).
       RUN add-link IN adm-broker-hdl ( h_reftable , 'Record':U , h_reftable-2 ).
