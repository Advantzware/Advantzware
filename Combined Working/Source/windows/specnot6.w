...
DEFINE VARIABLE h_specnot6 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_specnot6-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/specnot6.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_specnot6-2 ).
       RUN set-position IN h_specnot6-2 ( 5.05 , 6.00 ) NO-ERROR.
       RUN set-size IN h_specnot6-2 ( 18.10 , 138.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_specnot6-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_specnot6-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnot6-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/specnot6.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_specnot6 ).
       RUN set-position IN h_specnot6 ( 5.05 , 9.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_specnot6. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_specnot6 ).
       RUN add-link IN adm-broker-hdl ( h_specnot6-2 , 'Record':U , h_specnot6 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnot6 ,
             h_folder , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  op-header = ip-header.
  op-notecode = IF (INDEX(ip-header, "oeitem") GT 0 
                    OR INDEX(ip-header, "ordlt") GT 0) THEN "PDC" ELSE "RDC".
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  IF INDEX(ip-header, "ordrel") GT 0 THEN
      RUN setNoteType IN h_specnot6 (INPUT "RDC").
  ELSE
      RUN setNoteType IN h_specnot6 (INPUT "PDC").
...
ELSE DO:
  ...
  IF NOT AVAIL itemfg THEN DO:
    find first est where FALSE no-lock no-error.
...
RUN get-title IN h_specnot6 /* (INPUT ipNoteRow) */.
END PROCEDURE.
