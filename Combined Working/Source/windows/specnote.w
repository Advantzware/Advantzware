...
DEFINE VARIABLE h_specnote AS HANDLE NO-UNDO.
DEFINE VARIABLE h_specnote-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/specnote.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_specnote-2 ).
       RUN set-position IN h_specnote-2 ( 5.05 , 7.00 ) NO-ERROR.
       RUN set-size IN h_specnote-2 ( 18.10 , 138.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_specnote-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_specnote-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnote-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/specnote.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_specnote ).
       RUN set-position IN h_specnote ( 6.00 , 14.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_specnote. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_specnote ).
       RUN add-link IN adm-broker-hdl ( h_specnote-2 , 'Record':U , h_specnote ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnote ,
             h_folder , 'AFTER':U ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE estimate-notes-proc W-Win 
PROCEDURE estimate-notes-proc :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-header-value W-Win 
PROCEDURE get-header-value :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
 DEF OUTPUT PARAM op-header-value AS cha .
 op-header-value = ip-header.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST est WHERE est.rec_key EQ ip-rec_key NO-LOCK NO-ERROR.
  IF AVAIL est THEN op-header = est.est-no.

  IF op-header EQ "" THEN
  FIND FIRST ef WHERE ef.rec_key EQ ip-rec_key NO-LOCK NO-ERROR.
  IF AVAIL ef THEN op-header = ef.est-no.

  IF op-header EQ "" THEN
  FIND FIRST eb WHERE eb.rec_key EQ ip-rec_key NO-LOCK NO-ERROR.
  IF AVAIL eb THEN op-header = eb.est-no.

  IF op-header EQ "" THEN
  FIND FIRST oe-ordl WHERE oe-ordl.rec_key EQ ip-rec_key NO-LOCK NO-ERROR.
