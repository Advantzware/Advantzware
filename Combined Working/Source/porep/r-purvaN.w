...
ASSIGN cTextListToDefault  = "PO #,Vendor #,Job #,Item #,Due Date,Rec Date,MSF,Vendor $," +
                           "Bought $,Diff $,MPV %,Overs %,Adder code1,Adder code2" .
   ...
   IF tb_repeat THEN do:
    ...
    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       ...
       IF NOT tb_adder AND LOOKUP(cTmpField, "addr2") <> 0 THEN NEXT.
            CASE cTmpField:               
                 WHEN "po-no" THEN cVarValue = STRING(po-ord.po-no) .
                 WHEN "vend" THEN cVarValue = string(po-ord.vend-no) .
                 WHEN "job" THEN cVarValue = STRING(v-bld-job).
                 WHEN "item" THEN cVarValue = string(po-ordl.i-no).
                 WHEN "due-dt" THEN cVarValue = IF DueDate NE ? THEN string(DueDate) ELSE "".
                 WHEN "rcd-dt" THEN cVarValue = IF receiptDate NE ? THEN string(receiptDate) ELSE "".
                 WHEN "msf" THEN cVarValue = IF v-tot-msf NE ? THEN string(v-tot-msf,">>,>>9.999") ELSE "".
                 WHEN "vend-$" THEN cVarValue = IF v-vend-cost NE ? THEN string(v-vend-cost,"->>,>>>,>>9.99") ELSE "".
                 WHEN "bught" THEN cVarValue = IF v-cost NE ? THEN string(v-cost,"->,>>>,>>9.99") ELSE "".
                 WHEN "diff" THEN cVarValue = IF diff-price NE ? THEN string(diff-price,"->>,>>>,>>9.99") ELSE "".
                 WHEN "mpv" THEN cVarValue = IF tb_mpv AND v-mpv NE ? then string(v-mpv,"->>>,>>9.99") else "".
   ...
   ELSE DO:
    ...
    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            ...
            CASE cTmpField:               
                 WHEN "po-no" THEN cVarValue = IF FIRST-OF(po-ord.vend-no) THEN STRING(po-ord.po-no) ELSE "".
                 WHEN "vend" THEN cVarValue = IF FIRST-OF(po-ord.vend-no) THEN string(po-ord.vend-no) ELSE "".
                 WHEN "job" THEN cVarValue = STRING(v-bld-job).
                 WHEN "item" THEN cVarValue = string(po-ordl.i-no).
                 WHEN "due-dt" THEN cVarValue = IF DueDate NE ? THEN string(DueDate) ELSE "".
                 WHEN "rcd-dt" THEN cVarValue = IF receiptDate NE ? THEN string(receiptDate) ELSE "".
    ...
    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            ...
            CASE cTmpField:               
                 ...
                 WHEN "rcd-dt" THEN cVarValue = "".
                 WHEN "msf" THEN cVarValue = string(v-sub-msf,"->>,>>>,>>9.999").
                 WHEN "vend-$" THEN cVarValue = string(v-sub-vend,"->>>,>>>,>>9.99").
                 WHEN "bught" THEN cVarValue = string(v-sub-bght,"->>>>>>,>>9.99").
                 WHEN "diff" THEN cVarValue = string(v-sub-diff,"->>>,>>>,>>9.99").
                 WHEN "mpv" THEN cVarValue = "".
    ...
    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            ...
            CASE cTmpField:               
                 ...
                 WHEN "due-dt" THEN cVarValue = "".
                 WHEN "rcd-dt" THEN cVarValue = "".
                 WHEN "msf" THEN cVarValue = string(v-grand-msf,"->>,>>>,>>9.999").
                 WHEN "vend-$" THEN cVarValue = string(v-grand-vend,"->>>,>>>,>>9.99").
                 WHEN "bught" THEN cVarValue = string(v-grand-bght,"->>>>>>,>>9.99").
                 WHEN "diff" THEN cVarValue = string(v-grand-diff,"->>>,>>>,>>9.99").
                 WHEN "mpv" THEN cVarValue = "".
                 WHEN "ovr" THEN cVarValue = "".                        
                 WHEN "addr" THEN cVarValue = "".
                 WHEN "addr2" THEN cVarValue = "" .
