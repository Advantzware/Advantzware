...
&Scoped-Define ENABLED-OBJECTS Btn-Save Btn-Cancel Btn-quit Btn-Price 
...
DEFINE FRAME Panel-Frame
     ...
     Btn-Cancel AT ROW 1.29 COL 12
     Btn-quit AT ROW 1.29 COL 21
     Btn-Price AT ROW 1.29 COL 31
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-Price C-WIn
ON CHOOSE OF Btn-Price IN FRAME Panel-Frame /* Avg Price */
DO:
  RUN get-link-handle IN adm-broker-hdl 
       (THIS-PROCEDURE, 'Tableio-Target':U, OUTPUT source-str).

  IF VALID-HANDLE(WIDGET-HANDLE(source-str)) THEN DO:
    RUN avg-price IN WIDGET-HANDLE(source-str).
    RUN avg-price-label.
  END.
END.
...
&Scoped-define SELF-NAME Btn-quit
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-quit C-WIn
ON CHOOSE OF Btn-quit IN FRAME Panel-Frame /* Quit */
DO:
   DO WITH FRAME Panel-Frame:
     def var source-str as cha no-undo.
     RUN get-link-handle IN adm-broker-hdl 
      ...
      run close-proc in widget-handle(source-str).  
  /*      run dispatch in widget-handle(source-str) ('destroy').  */
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE avg-price-label C-WIn 
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(WIDGET-HANDLE(source-str)) THEN DO WITH FRAME {&FRAME-NAME}:
    RUN avg-price-label IN WIDGET-HANDLE(source-str) (OUTPUT lv-label).

    IF lv-label EQ "" THEN
      Btn-Price:SENSITIVE = NO.
    ELSE
      Btn-Price:LABEL = lv-label.
  END.
...
&ENDIF
&IF LOOKUP("Btn-Price":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             Btn-Price:SENSITIVE = NO.
&ENDIF
