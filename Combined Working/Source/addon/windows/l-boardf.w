...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 item.i-no item.i-name item.cal 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      item.i-no COLUMN-FONT 2
      item.i-name COLUMN-FONT 2
      item.cal COLUMN-FONT 2
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
(item.industry = ip-industry or ip-industry = """")
and item.mat-type >= ""1"" and item.mat-type <= ""4"""
     _FldNameList[1]   > ASI.item.i-no
"i-no" ? ? "character" ? ? 2 ? ? ? no ?
     _FldNameList[2]   > ASI.item.i-name
"i-name" ? ? "character" ? ? 2 ? ? ? no ?
     _FldNameList[3]   > ASI.item.cal
"cal" ? ? "decimal" ? ? 2 ? ? ? no ?
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
