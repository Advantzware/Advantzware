...
DO WHILE CAN-FIND(FIRST tt-fg-set WHERE tt-fg-set.set-no NE tt-fg-set.part-no
                                    AND tt-fg-set.isaset AND tt-fg-set.alloc):
  FOR EACH tt-fg-set WHERE tt-fg-set.isaset AND tt-fg-set.alloc:
    FOR EACH fg-set
        WHERE fg-set.company EQ cocode
          AND fg-set.set-no  EQ tt-fg-set.part-no
          AND fg-set.set-no  NE fg-set.part-no
        NO-LOCK,

        FIRST itemfg
        WHERE itemfg.company EQ cocode
          AND itemfg.i-no    EQ fg-set.part-no
        NO-LOCK:
...
FOR EACH tt-fg-set BREAK BY tt-fg-set.part-no:
  IF FIRST-OF(tt-fg-set.part-no) THEN v-part-qty-dec = 0.

  v-part-qty-dec = v-part-qty-dec + tt-fg-set.part-qty-dec.

  IF LAST-OF(tt-fg-set.part-no) THEN tt-fg-set.part-qty-dec = v-part-qty-dec.

  ELSE DELETE tt-fg-set.
END.
