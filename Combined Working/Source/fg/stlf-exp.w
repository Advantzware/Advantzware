...
ASSIGN cTextListToSelect =  "Style No.,Description,Design #,Royalty,Glue Lap,Tuck,DK Length,DK Width,5th Panel,Lock TAB,Board,Ink," +
                            "Ink Cov %,Film,Leaf,Coating,Adhesive,Qty/SET,Machine Routing(1),Description(1),Machine Routing(2),Description(2)," +
                            "Machine Routing(3),Description(3),Machine Routing(4),Description(4),Machine Routing(5),Description(5),Machine Routing(6)," +
                            "Description(6),Machine Routing(7),Description(7),Lower Left Wid,Lower Left Len,Nesting Wid,Nesting Len,Stagger Wid," +
                            "Stagger Len,Sq Inches Wid,Sq Inches Len,Lid Len,Lid Wid,Lid Die ,Box Die"

      cFieldListToSelect =  "style,dscr,design-no,royalty,dim-gl,dim-tk,dim-dkl,dim-dkw,dim-pan5,dim-fit," +
                            "material[1],material[2],material[3],material[4],material[5],material[6],material[7]," +
                            "qty-per-set,m-code[1],m-dscr[1],m-code[2],m-dscr[2],m-code[3],m-dscr[3],m-code[4],m-dscr[4]," +
                            "m-code[5],m-dscr[5],m-code[6],m-dscr[6],m-code[7],m-dscr[7],formula[1],formula[2],formula[3],formula[4]," +
                            "formula[5],formula[6],formula[7],formula[8],formula[9],formula[10],formula[11],formula[12] "
...
btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_style end_style  ~
 sl_avail ~
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR style, ipc-field AS CHAR )  FORWARD.
...
DEFINE VARIABLE begin_style AS CHARACTER FORMAT "x(15)" 
     LABEL "From Style" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.

DEFINE VARIABLE end_style AS CHARACTER FORMAT "X(15)" INITIAL "zzzzzzzzzzzzzzzzzzz" 
     LABEL "To Style" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.

DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-stylef.csv" 
...
DEFINE FRAME rd-fgexp
     begin_style AT ROW 3.95 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Machine Number" WIDGET-ID 142
     end_style AT ROW 3.95 COL 71 COLON-ALIGNED HELP
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Export Folding Style to Excel" WIDGET-ID 100.
...
ON LEAVE OF begin_style IN FRAME rd-fgexp /* From Item # */
DO:
   assign {&self-name}.
END.
...
ON LEAVE OF end_style IN FRAME rd-fgexp /* To Item # */
DO:
     assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_style end_style sl_avail sl_selected tb_excel 
          tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF begin_style:SCREEN-VALUE = "" THEN do:
        FIND FIRST style WHERE style.company EQ cocode
        AND style.industry = "1" NO-LOCK NO-ERROR.
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR style, ipc-field AS CHAR ) :
/*------------------------------------------------------------------------------
