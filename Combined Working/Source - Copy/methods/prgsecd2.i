...
DO:
  DO num-groups = 1 TO NUM-ENTRIES(g_groups):
    ...
    IF NOT v-can-run AND CAN-DO(b-prgrms.can_run,ENTRY(num-groups,g_groups))
          THEN v-can-run = YES.
    IF NOT v-can-update AND CAN-DO(b-prgrms.can_update,ENTRY(num-groups,g_groups))
          THEN v-can-update = YES.
    IF NOT v-can-create AND CAN-DO(b-prgrms.can_create,ENTRY(num-groups,g_groups))
          THEN v-can-create = YES.
    IF NOT v-can-delete AND CAN-DO(b-prgrms.can_delete,ENTRY(num-groups,g_groups))
