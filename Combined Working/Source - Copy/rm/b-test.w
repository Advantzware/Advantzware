...
&Scoped-define FIELDS-IN-QUERY-br_table rm-rctd.barcode rm-rctd.cost ~
rm-rctd.job-no rm-rctd.i-no 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table rm-rctd.barcode ~
rm-rctd.cost rm-rctd.job-no rm-rctd.i-no 
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH rm-rctd WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
/* Query rebuild information for BROWSE br_table
     ...
     _FldNameList[1]   > JOBS.rm-rctd.barcode
"barcode" ? ? "character" ? ? ? ? ? ? yes ?
     _FldNameList[2]   > JOBS.rm-rctd.cost
"cost" ? ? "decimal" ? ? ? ? ? ? yes ?
     _FldNameList[3]   > JOBS.rm-rctd.job-no
