...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH gl-jrn WHERE ~{&KEY-PHRASE} ~
      AND gl-jrn.company = g_company ~
and not gl-jrn.posted ~
 AND gl-jrn.tr-date = 12/31/9999 ~
 AND gl-jrn.journal = 0 ~
 AND gl-jrn.period = 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main gl-jrn
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main gl-jrn
...
and not ASI.gl-jrn.posted
 AND ASI.gl-jrn.tr-date = 12/31/9999
 AND ASI.gl-jrn.journal = 0
 AND ASI.gl-jrn.period = 0"
...
------------------------------------------------------------------------------*/
  ...
  FIND FIRST bf-jrn WHERE bf-jrn.company eq g_company
                      AND bf-jrn.posted  eq NO
                          and bf-jrn.tr-date eq 12/31/9999
                          and bf-jrn.journal eq 0
                          and bf-jrn.period  eq 0 NO-LOCK NO-ERROR.
  IF NOT AVAIL bf-jrn THEN DO:
     find last bf-jrn use-index j-no no-error.
     x = 1 + (if avail bf-jrn then bf-jrn.j-no else 0).

     CREATE bf-jrn.
     ASSIGN bf-jrn.j-no    = x
                bf-jrn.posted  = no
                bf-jrn.company = g_company
                bf-jrn.journal = 0
                bf-jrn.tr-date = 12/31/9999.
  END.
