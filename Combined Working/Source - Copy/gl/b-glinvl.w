...
&SCOPED-DEFINE sortby BY ar-invl.inv-no BY ar-invl.i-no BY ar-invl.line
...
ar-invl.ord-no ar-invl.po-no ar-invl.est-no getCost() @ ld-cost ~
getCostUOM() @ ld-cost-uom 
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH ar-invl WHERE ar-invl.company = cocode ~
  AND ar-invl.inv-no = int(report.key-01) NO-LOCK, ~
      EACH ar-inv WHERE ar-inv.x-no = ar-invl.x-no NO-LOCK, ~
      EACH cust OF ar-inv NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH ar-invl WHERE ar-invl.company = cocode ~
  AND ar-invl.inv-no = int(report.key-01) NO-LOCK, ~
      EACH ar-inv WHERE ar-inv.x-no = ar-invl.x-no NO-LOCK, ~
      EACH cust OF ar-inv NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&Scoped-Define ENABLED-OBJECTS tb_open tb_paid fi_cust-no fi_date fi_actnum ~
fi_i-no fi_part-no fi_ord-no fi_po-no fi_bol-no fi_est-no btn_go btn_show ~
...
DEFINE FRAME F-Main
     fi_sort-by AT ROW 4.81 COL 63 COLON-ALIGNED
...
/* SETTINGS FOR FILL-IN fi_inv-no IN FRAME F-Main
   NO-ENABLE                                                            */
...
/* Query rebuild information for BROWSE Browser-Table
     ...
     _TblOptList       = ",,, OUTER"
     _JoinCode[1]      = "ASI.ar-invl.company = cocode
  AND ASI.ar-invl.inv-no = int(report.key-01)"
...
"getCostUOM() @ ld-cost-uom" "Cost!UOM" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
