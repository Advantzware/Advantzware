...
def var v-cntct-nam         as   CHAR FORMAT "x(22)" no-undo.
...
for each xxreport where xxreport.term-id eq v-term-id,
    ...
    assign
     ...
     v-ship-addr3   = shipto.ship-city + ", " +
                      shipto.ship-state + "  " +
                      shipto.ship-zip
    /* v-phone-num    = "(" + shipto.area-code,"(xxx)" + ")"  + shipto.phone */
     v-phone-num    = trim(string(shipto.area-code,"(xxx)") + STRING(shipto.phone,"xxx-xxxx")) 
     v-cntct-nam    = shipto.contact .
  ...
  FOR EACH oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no:
    IF ll-consol-bolls THEN DO:
      IF (oe-boll.qty-case * oe-boll.cases) NE 0 THEN
        RUN create-tt-boll (oe-boll.qty-case, oe-boll.cases, oe-boll.tot-pall).
...
PROCEDURE create-tt-boll.
  ...
  DEF INPUT PARAM ip-pall     LIKE oe-boll.tot-pall NO-UNDO.
  ...
  ASSIGN
   ...
   tt-boll.tot-pall = tt-boll.tot-pall + ip-pall
