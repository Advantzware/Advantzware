...
DEF VAR cShipNotes AS CHAR FORMAT "x(230)" NO-UNDO.
    ...
    if shipto.broker then DO:
       ...
       IF AVAIL oe-boll THEN DO:
          ...
          IF AVAIL oe-ord THEN
             ASSIGN v-comp-add1 = oe-ord.sold-addr[1]
...
/*    if can-do("COD,CIA", oe-ord.terms) then v-terms = oe-ord.terms-d. */
  ...
  if last-of(oe-bolh.bol-no) then do:
    ...
    */
    ...
    PUT "<C25><R45><#1>"
        "<R+13><C+100><IMAGE#1=" ls-full-img1. /* image */ .
  ...
  PUT "<R54><C50><#8><FROM><R+4><C+30><RECT> " 
    ...
    /*"<=8><R+3> Total Weight        :" v-tot-wt /*fORM ">>,>>9.99"*/ task# 03080603 */.

  /* Move individual ship notes to one variable. */
  ASSIGN cShipNotes = oe-bolh.ship-i[1] + oe-bolh.ship-i[2] + oe-bolh.ship-i[3] + oe-bolh.ship-i[4].

  /* Format the paragraph. */
  RUN FormatParagraph (INPUT-OUTPUT cShipNotes,
                       INPUT 40). /* paragraph width */

  PUT "<FArial><R51><C1><P12><B>     Shipping Instructions: </B> <P9> " 
    "<R52><C1>" cShipNotes AT 7 
    "<R58><C1>"
