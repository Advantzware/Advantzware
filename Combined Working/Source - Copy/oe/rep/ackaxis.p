...
{oe/rep/acknowl.i}
DEFINE VARIABLE cShipToAddress AS CHARACTER EXTENT 4  FORMAT "x(30)" NO-UNDO.
...
ELSE*/ ASSIGN ls-image1 = "images\axis.jpg".
...
DEF VAR lv-line-print AS INT INIT 46 NO-UNDO.
DEF VAR lv-line-start AS INT INIT 46 NO-UNDO.
      ...
      IF AVAIL oe-ordl THEN DO:
         ...
         IF AVAIL oe-rel THEN DO:
            ...
            IF AVAIL shipto THEN DO:
                ASSIGN
                    cShipToAddress[1] = shipto.ship-name
                    cShipToAddress[2] = shipto.ship-addr[1]
                    cShipToAddress[3] = shipto.ship-addr[2]
                    cShipToAddress[4] = shipto.ship-city + ", " 
                                        + shipto.ship-state + "  " 
                                        + shipto.ship-zip
                    .
/*                 IF cShipToAddress[3] EQ '' THEN               */
/*                     ASSIGN                                    */
/*                         cShipToAddress[3] = cShipToAddress[4] */
/*                         cShipToAddress[4] = ''                */
/*                         .                                     */
            END.
         END.
...
/*                    PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" . */
     ...
     END. /* v-prntinst */
      ...
      for each oe-ordm no-lock where oe-ordm.company eq oe-ord.company and
          ...
          if v-printline ge /*lv-line-print*/ 45  then
          do:
              RUN printPageNumLine.
/*               PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" . */
              PAGE .
...
*/
      ...
      PUT "<R53><C1><#10><P12><B> Comments </B> <P10> " 
        ...
        "<R55><C1>" v-billinst[2] 
        "<R56><C1>" v-billinst[3] 
        "<R57><C1>" v-billinst[4]   "<P10>Order Total:" AT 90 "<U>"  v-totord FORM "$>>>>,>>9.99"  "</U><P9>"               
/*         "<R59><C1>" " ______________________________________(Please sign and fax back) " */
        "<R60><C1>" SPACE(3) "<P9><B>THIS IS A CONFIRMATION OF YOUR ORDER,NOT AN INVOICE. "  SPACE(30) "                <P16> Thank You! <P10></B>" 
        "<R61><C1>" SPACE(3) "<P9> Please Note: All orders are subject to a standard 10% overrun or underrun unless we confirm "
        "<R62><C1>" SPACE(3) "<P9> differently herein. Final invoice will reflect the actual quantity shipped. If you specify "   
/*           "<C68><P10>Page "  string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" */

        "<R63><C1>" SPACE(3) "<P9> 'No overrun,' your order may run short by more than 10%. If you specify 'No underrun,' your "
        "<R64><C1>" SPACE(3) "<P9> order may run over by more than 10% and you will be billed the actual quantity. "
          .
      RUN printPageNumLine.
      assign
       v-totord        = 0
...
PROCEDURE printPageNumLine:
    PUT "<R62><C68><P10>Page " + STRING(PAGE-NUM - lv-pg-num,">>9") + " of [=@endPage" + string(oe-ord.ord-no) + "-@startPage" + string(oe-ord.ord-no) + "+1] " FORM "x(120)" .
END PROCEDURE.
