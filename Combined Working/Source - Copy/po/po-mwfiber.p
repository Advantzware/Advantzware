...
{po/po-mwfiber.i}
      ...
      for each po-ordl WHERE
       ...
       /* v-job-no = po-ordl.job-no + "-" + STRING(po-ordl.job-no2,">>").*/
        ...
        PUT po-ordl.i-name AT 25 FORM "x(25)" SPACE(1) /*v-vend-item FORM "x(15)" space(1) */
            v-adder[2]     /* aj */
            po-ordl.due-date  SPACE(1)
            trim(v-change-dscr)  
            v-tot-msf SKIP.
        v-printline = v-printline + 1.
       ...
       v-tot-sqft = v-tot-sqft + (v-qty * 1000).
        ...
        /* RUN calc-cost (recid(po-ordl),OUTPUT v-cost,OUTPUT v-setup). aj */                   

        v-cost = po-ordl.cost. /* - (ROUND(v-setup /  v-qty,4)). aj */ /* reclac cost from setup */
        ...
        put "W: " at 25 v-wid  FORMAT ">>9.9999" space(1) "L: " v-len  FORMAT ">>9.9999" SPACE (2) 
                 /*"                   "*/
              /*  "  Flute:"*/  trim(lv-flute) FORMAT "x(13)" /*"Test:" */ trim(lv-reg-no) FORMAT "x(13)"
