/* ------------------------------------------- ar/rep/invloyln.p 11200902 GDM */
...
DEF VAR ll-component AS LOG NO-UNDO.
     ...
     FOR EACH ar-invl NO-LOCK 
         ...
         FIND first oe-ordl 
           WHERE oe-ordl.company = cocode 
             ...
             /*AND oe-ordl.LINE = ar-invl.LINE*/ NO-LOCK NO-ERROR.  /* task 07231506 */
         IF AVAIL oe-ordl 
           THEN
            ASSIGN 
              ...
              ll-component = oe-ordl.is-a-component.
           ELSE 
            ASSIGN 
         ...
         IF NOT ll-component /*AND NOT ar-invl.misc*/ THEN DO:
             FIND FIRST tt-inv-line WHERE 
                  tt-inv-line.part-no EQ ar-invl.part-no
                  AND tt-inv-line.dscr1 EQ ar-invl.i-name
              ...
              ELSE DO:
                  ...
                  ASSIGN 
                      ...
                      tt-inv-line.bol-no = lv-bolno
                      tt-inv-line.dscr2 = ar-invl.i-dscr
                      tt-inv-line.qty = ar-invl.qty
                      tt-inv-line.dscr3 = ar-invl.part-dscr1
                      tt-inv-line.misc = ar-invl.misc.
              END.
/*              PUT                                              */
...
/*          END.                                                      */
     END. /* each ar-invl */
    FOR EACH tt-inv-line WHERE NOT tt-inv-line.misc:
        PUT             
    ...
    END.
     ...
     ASSIGN
       v-inv-total = v-subtot-lines + ar-inv.tax-amt + v-inv-freight.
        ...
        "<P10><|10><R59><C53><FROM><R64><C80.75><RECT>"
        "<R59><C55><#8><B>" 
