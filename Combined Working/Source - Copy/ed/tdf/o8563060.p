/***************************************************************************\
*****************************************************************************
**  Program: E:\robj8\patch\ed\tdf\o8563060.p
...
1.  Forced unit of measure from CS to EA on SEARS, per Dunia Grasso.
10.06.98 by CAH on \\ricky\rv8 Log#0000:
1.  Corrected some segment sequence numbers.
2.  Added SF n1 segment, which required break in header segment list.
3.  Corrected BOL# in REF BM, added DTM to SEARS partnet list.
4.  Corrected SCAC code assignment in TD5.
08.13.98 by CAH on \\ricky\robj8\ Log#0000:
1.  Added ws_section assignment of 0 since 856 has custom sectioning by HL.
2.  Misc updates to make consistent with o810 program.
*****************************************************************************
\***************************************************************************/
{ed/sharedv.i}
...
ASSIGN
  ...
  header_segment_list2 = "REF,016,PER,017,DTM,019,N1,022,N2,023,N3,024,N4,025"
  order_segment_list  = "HL,003,PRF,007,PID,009,TD1,012,REF,016,N1,022"
  tare_segment_list   = "HL,003,MAN,018"
  pack_segment_list   = "HL,003,PO4,008,MAN,018"
  detail_segment_list = "HL,003,LIN,004,SN1,005,PO4,008,REF,016" /* SLN,006,PRF,007, */
  trailer_segment_list ="CTT,028"
  ctt_count_segment_list = "HL".
CASE ws_partner:
WHEN "SEARS" THEN
ASSIGN
  partner_segment_list =
  "000,ST,BSN,HL,TD1,TD5,TD3,REF,DTM,N1,N3,N4,PRF,PID,TD1,MAN,LIN,SN1,SLN,CTT,SE". /* PO4, */
...
{rc/hdg-wide.i "ed/tdf/o8563060.p" "OUTBOUND ASN EDIT LIST" "(s-out)"}
{rc/hdg-noco.i}
ASSIGN
...
FOR EACH eddoc EXCLUSIVE
  ...
  ASSIGN
    ...
    /* edshtran.carrier-code /* 9810 CAH */ */
  ...
  FOR EACH edshline OF edshtran EXCLUSIVE
    ...
    DO: /* order level segments */
      ...
      DO:
        ...
        ASSIGN
          ...
          THEN edshord.cust-div ELSE edshord.cust-dept
          ref_desc                        = ""
          .
      END CASE.
      DISPLAY STREAM s-out
    ...
    DO:
      ...
      */
      ASSIGN
        /* n1 Z7 MARK-FOR STORE, SAME AS BY STORE */
        entity_id                       = "Z7"
        company_name                    = ""
        id_code_qualifier               = "92"
        id_code                         = ordering_store_number
...
if ws_partner = "SEARS" then do:
    ...
    item_assigned_id = "".  /* 10/13/98 CAH per Dunia */
end.
DISPLAY STREAM s-out
