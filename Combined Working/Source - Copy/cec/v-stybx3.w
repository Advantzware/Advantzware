...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL box-design-hdr AND box-design-hdr.box-3d-image <> "" THEN DO:
     IF box-design-hdr.box-3d-image MATCHES "*.pdf*"  THEN DO:
        ...
        DO:
           ...
           OS-COMMAND silent VALUE(trim(lv-quote) + lv-cmd + " " + trim(lv-quote) + box-design-hdr.box-3d-image + trim(lv-quote) + TRIM(lv-quote)).
        END.
     END.      
     ELSE DO:
       ASSIGN
          lv-cmd = lv-cmd + " " + chr(34) + box-design-hdr.box-3d-image:SCREEN-VALUE IN FRAME {&FRAME-NAME} + CHR(34)
          lv-image-file = chr(34) + box-design-hdr.box-3d-image:SCREEN-VALUE IN FRAME {&FRAME-NAME} + CHR(34).
       IF lv-cmd2 <> "" THEN
             OS-COMMAND SILENT START value(lv-cmd) /*value(lv-cmd2) value(lv-image-file)*/ .          
       ELSE OS-COMMAND SILENT VALUE(lv-cmd) /* VALUE(trim(lv-quote) + lv-cmd + " " + trim(lv-quote) + box-design-hdr.box-3d-image + trim(lv-quote) + TRIM(lv-quote))*/.
     END.
  END.
