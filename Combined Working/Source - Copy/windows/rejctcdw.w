...
DEFINE VARIABLE h_rejct-cd AS HANDLE NO-UNDO.
DEFINE VARIABLE h_rejct-cd-2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse Reason|View Reasons' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 3.14 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 21.43 , 148.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/rejct-cd.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_rejct-cd-2 ).
       RUN set-position IN h_rejct-cd-2 ( 5.76 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.62 , 103.00 ) */
       ...
       /* Links to SmartViewer h_rejct-cd-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_rejct-cd-2 ).
       RUN add-link IN adm-broker-hdl ( h_rejct-cd , 'Record':U , h_rejct-cd-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_rejct-cd-2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_rejct-cd-2 , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  RUN set-labels IN h_rejct-cd (INPUT ip-type, ip-code-label, ip-desc-label).
  RUN set-labels IN h_rejct-cd-2 (INPUT ip-type, ip-code-label, ip-desc-label).
END PROCEDURE.
