...
DEFINE VARIABLE h_empinou2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_empinout AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Emp|Log In/Out|View Notes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 22.19 , 11.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.76 , 34.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/empinout.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_empinout-2 ).
       RUN set-position IN h_empinout-2 ( 22.19 , 76.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 30.00 ) */
       ...
       RUN add-link IN adm-broker-hdl ( h_empinout-2 , 'empio':U , h_empinou2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_employee ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_empinou2 ,
             h_employee , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_empinou2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_empinout-2 ,
             h_p-navico , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN add-link IN adm-broker-hdl ( h_empinout , 'Record':U , h_allnote2 ).
