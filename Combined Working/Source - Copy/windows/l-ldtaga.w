...
DEF INPUT PARAM ip-casetag AS LOG NO-UNDO.
...
loadtag.ord-no loadtag.po-no loadtag.qty 
...
loadtag.is-case-tag = ip-casetag ~
 AND loadtag.qty > 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
loadtag.is-case-tag = ip-casetag ~
 AND loadtag.qty > 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 loadtag
...
"loadtag.qty" "On Hand" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
