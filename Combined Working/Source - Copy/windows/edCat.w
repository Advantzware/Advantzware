...
DEFINE VARIABLE h_edcatline AS HANDLE NO-UNDO.
DEFINE VARIABLE h_edcatline-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_edcatprice AS HANDLE NO-UNDO.
DEFINE VARIABLE h_edcatprice-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* EDI 832 Catalog */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* EDI 832 Catalog */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse Cat|View Cat|832 Lines|View Lines|832 Prices|View Prices' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_edcat ).
       RUN set-position IN h_edcat ( 4.81 , 3.60 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/edcatline.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edcatline ).
       RUN set-position IN h_edcatline ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_edcatline ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartNavBrowser h_edcatline. */
       RUN add-link IN adm-broker-hdl ( h_edcat , 'Record':U , h_edcatline ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edcatline ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/edcatline.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edcatline-2 ).
       RUN set-position IN h_edcatline-2 ( 5.52 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 17.14 , 144.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('3':U) NO-ERROR.

       /* Links to SmartViewer h_edcatline-2. */
       RUN add-link IN adm-broker-hdl ( h_edcatline , 'Record':U , h_edcatline-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edcatline-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 4 */
    WHEN 5 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/edcatprice.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edcatprice ).
       RUN set-position IN h_edcatprice ( 4.81 , 3.40 ) NO-ERROR.
       RUN set-size IN h_edcatprice ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('3':U) NO-ERROR.

       /* Links to SmartNavBrowser h_edcatprice. */
       RUN add-link IN adm-broker-hdl ( h_edcatline , 'Record':U , h_edcatprice ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edcatprice ,
             h_folder , 'AFTER':U ).
    END. /* Page 5 */
    WHEN 6 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/edcatprice.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edcatprice-2 ).
       RUN set-position IN h_edcatprice-2 ( 5.05 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 17.14 , 144.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('5':U) NO-ERROR.

       /* Links to SmartViewer h_edcatprice-2. */
       RUN add-link IN adm-broker-hdl ( h_edcatprice , 'Record':U , h_edcatprice-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edcatprice-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 6 */
