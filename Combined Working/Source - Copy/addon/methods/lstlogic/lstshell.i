...
DO WHILE current-widget NE ?:
  IF CAN-DO("save",current-widget:PRIVATE-DATA) THEN
  PUT UNFORMATTED "    y-pos = y-pos + 80." SKIP
    "    RUN SetCellFont IN h_Viper(~"Cell~",~"Parameter~",12,0,~"bold~",~"Arial~")." SKIP
    "    RUN SetCellPos IN h_Viper(~"Cell~",~"Parameter~",1,y-pos,800,0)." SKIP
    "    RUN SetCellText IN h_Viper(~"Cell~",~"Parameter~",~"" +
   (IF current-widget:LABEL NE ? THEN current-widget:LABEL ELSE current-widget:TOOLTIP) + ":~")." SKIP
    "    RUN SetCellAlign IN h_Viper(~"Cell~",~"Parameter~",2)." SKIP
    "    RUN FlushGroup IN h_Viper (~"Parameter~")." SKIP
    "    RUN SetCellFont IN h_Viper(~"Cell~",~"Parameter~",12,0,~"~",~"Arial~")." SKIP
    "    RUN SetCellPos IN h_Viper(~"Cell~",~"Parameter~",870,y-pos,700,0)." SKIP
    "    RUN SetCellText IN h_Viper(~"Cell~",~"Parameter~"," current-widget:NAME ")." SKIP
    "    RUN SetCellAlign IN h_Viper(~"Cell~",~"Parameter~",1)." SKIP
    "    RUN FlushGroup IN h_Viper (~"Parameter~")." SKIP.
  current-widget = current-widget:NEXT-SIBLING.
END.
PUT UNFORMATTED
