...
2) if internal procedures are needed, add'em to 'lstlogic/persist.p'
   ...
   The naming convention used for <ip-name> should begin with 'usergrp_'.
...
5) if addresses are desired for this listing use the following syntax:
   {methods/lstlogic/showaddr.i &db_table="usergrps" &col="5" &frame-name="f-addresses"}

6) if phones are desired for this listing use the following syntax:
   {methods/lstlogic/showphon.i &db_table="usergrps" &col="5" &frame-name="f-phones"}

---------------------------------------------------------------------------*/

DISPLAY
  usergrps.usergrps
  usergrps.users VIEW-AS TEXT.
