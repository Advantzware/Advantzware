...
ASSIGN cTextListToSelect = "Item Number,Vend No,Name,UOM,Up to Qty1,Cost1," +
...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Item Number,Vend No,Name,UOM,Up to Qty1,Cost1," +
                           "Up to Qty2,Cost2,Up to Qty3,Cost3"  .
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
sl_selected Btn_Add Btn_Remove btn_Up btn_down lv-ornt lv-font-no ~
...
begin_mat-type end_mat-type tb_blank sl_avail sl_selected lv-ornt ~
lv-font-no lines-per-page rd-dest lv-font-name td-show-parm tb_excel ~
...
DEFINE FRAME FRAME-A
     begin_rm-no AT ROW 2.62 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Item Number"
     end_rm-no AT ROW 2.62 COL 69 COLON-ALIGNED HELP
          "Enter Ending Item number"
     begin_vend AT ROW 3.81 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Vendor"
     end_vend AT ROW 3.81 COL 69 COLON-ALIGNED HELP
          "Enter Ending Vendor"
     begin_mat-type AT ROW 5 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Material Type"
     end_mat-type AT ROW 5 COL 69 COLON-ALIGNED HELP
          "Enter Ending Material Type"
     tb_blank AT ROW 6.29 COL 37
     sl_avail AT ROW 8.43 COL 4.2 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 8.43 COL 40.2 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 8.43 COL 59.6 NO-LABEL WIDGET-ID 28
     ...
     tb_excel AT ROW 18.86 COL 31
     tb_runExcel AT ROW 18.86 COL 77 RIGHT-ALIGNED
     v-excel-file AT ROW 19.81 COL 44 COLON-ALIGNED
     btn-ok AT ROW 22.24 COL 18
     btn-cancel AT ROW 22.24 COL 57
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 7.52 COL 5.2 WIDGET-ID 38
...
------------------------------------------------------------------------------*/
  DISPLAY begin_rm-no end_rm-no begin_vend end_vend begin_mat-type end_mat-type 
          tb_blank sl_avail sl_selected lv-ornt lv-font-no lines-per-page 
  ...
  ENABLE RECT-6 RECT-7 begin_rm-no end_rm-no begin_vend end_vend begin_mat-type 
         ...
         btn_Up btn_down lv-ornt lv-font-no lines-per-page rd-dest td-show-parm 
...
DEF VAR yy AS CHAR.
DEF VAR v-get-val AS DECIMAL EXTENT 20 .
DEF VAR v-cost-val AS DECIMAL EXTENT 20 .
DEF VAR v-get-char AS CHAR EXTENT 20 .
...
SESSION:SET-WAIT-STATE ("general").
  ...
  IF tb_excel THEN DO:
      OUTPUT STREAM st-excel TO VALUE(TRIM(v-excel-file)).
            ...
            do i = 1 to 9:
                ...
                DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                   cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                        CASE cTmpField:             
                             WHEN "i-no"  THEN cVarValue =  IF j EQ  1 THEN string(item.i-no) ELSE "" .
                             WHEN "vend"  THEN cVarValue =  IF j EQ  1 THEN string("* Blank*") ELSE "" . 
                             WHEN "name"  THEN cVarValue = "" .
                             WHEN "uom"   THEN cVarValue = IF j EQ  1 THEN string(item.pur-uom) ELSE "" .
                             WHEN "qty1"  THEN do:
                                  IF j = 1 THEN
              ...
              ASSIGN v-vend-no   = IF AVAIL vend THEN vend.vend-no ELSE e-item-vend.vend-no
                ...
                DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                   cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                        CASE cTmpField:             
                             WHEN "i-no"  THEN cVarValue =  IF j EQ  1 THEN string(item.i-no) ELSE "" .
                             WHEN "vend"  THEN cVarValue =  IF j EQ  1 THEN string(v-vend-no) ELSE "" . 
                             WHEN "name"  THEN cVarValue = IF j EQ  1 THEN string(v-vend-name,"x(30)") ELSE "" .
                             WHEN "uom"   THEN cVarValue = IF j EQ  1 THEN string(item.pur-uom) ELSE "" .
                            WHEN "qty1"  THEN do:
                                  IF j = 1 THEN
                                  cVarValue = string(v-get-val[1],">>>>>>>>9.9")  .
