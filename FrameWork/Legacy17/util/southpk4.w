...
DEFINE FRAME FRAME-A
     ...
     "This process is used to import the GL Account Master file (glmstr)" VIEW-AS TEXT
          SIZE 74 BY .71 AT ROW 2.19 COL 10
          FONT 6
     "from a comma separated ASCII file to Advantzware (account)" VIEW-AS TEXT
          SIZE 75 BY .62 AT ROW 3.14 COL 10
          FONT 6
...
ON END-ERROR OF C-Win /* Load GL Acct#s */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Load GL Acct#s */
DO:
...
/* ------------------------------------------------ util/southpk4.p 03/02 JLF */
/* Import GL Account for Southpak                                             */
/* -------------------------------------------------------------------------- */                       
...
def var v-err as int.
  ...
  repeat:
    ...
    if avail account                         or
      ...
      create account.
      assign
       account.company = cocode
       account.actnum  = substr(entry(3,v-file),5,4) + "-" +
                         substr(entry(3,v-file),11,2)
       account.dscr    = substr(entry(4,v-file),1,45)
       account.type    = if substr(entry(3,v-file),5,4) ge "5000" then "E" else
                         if substr(entry(3,v-file),5,4) ge "4000" then "R" else
