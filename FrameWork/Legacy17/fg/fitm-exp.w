...
ASSIGN cTextListToSelect = "Item#,Name,DESC,Est.DESC,Item Code,Taxable,Mat'l Type,Mat'1 Dscr,Cost Type,Cost Dscr,Category,Category Dscr,QTY Usage PTD,Qty Usage YTD," +
                           "Qty Usage Last YR,Usage Cost PTD,Usage Cost YTD,Total Cost Last YR,Ink Type,Press Type,Min Lbs/Job,SI/Lb,Wgt/100 "
      ...
      cFieldListToSelect = "i-no,i-name,i-dscr,est-dscr,i-code,tax-rcpt,mat-type,mat-dscr,cost-type,costtype-descr,procat,procat-dscr,q-ptd," +
                           "q-ytd,q-lyr,u-ptd,u-ytd,u-lyr,ink-type,press-type,min-lbs,yield,weight-100 " .
...
ON LEAVE OF begin_rm-no IN FRAME rd-fgexp /* From Item # */
DO:
   assign {&self-name}.
END.
...
ON LEAVE OF end_rm-no IN FRAME rd-fgexp /* To Item # */
DO:
     assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_rm-no end_rm-no sl_avail sl_selected tb_excel 
          tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_rm-no end_rm-no  sl_avail Btn_Add 
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF begin_rm-no:SCREEN-VALUE EQ "" THEN do:
        FIND FIRST ITEM WHERE ITEM.company EQ cocode
             AND item.industry = "1" NO-LOCK NO-ERROR.
        begin_rm-no:SCREEN-VALUE = ITEM.i-no.
        FIND LAST ITEM WHERE ITEM.company EQ cocode 
             AND item.industry = "1" NO-LOCK NO-ERROR.
    ...
    IF lcSearch NE "" THEN DO:
        ...
        IF lcsearchby EQ "2" THEN do:
            FIND FIRST ITEM WHERE ITEM.company EQ cocode
                AND ITEM.i-name BEGINS lcSearch 
                AND item.industry = "1" NO-LOCK NO-ERROR.
            IF AVAIL ITEM THEN
                begin_rm-no:SCREEN-VALUE = ITEM.i-no.
            FIND LAST ITEM WHERE ITEM.company EQ cocode
        ...
        IF lcsearchby EQ "3" THEN do:
            DEF VAR k AS INT INIT 1 NO-UNDO.
            k = 1.
            FOR EACH ITEM WHERE ITEM.company EQ cocode
                AND ITEM.mat-type BEGINS lcSearch 
                AND item.industry = "1" NO-LOCK BY ITEM.mat-type
                                                BY ITEM.i-name:
                IF k = 1 THEN
                    begin_rm-no:SCREEN-VALUE = ITEM.i-no.
                 ASSIGN
                     ...
                     k = k + 1.
            END.               
        END. /* IF lcsearchby EQ "3" */
        IF lcsearchby EQ "4" THEN do:
            DEF VAR t AS INT INIT 1 NO-UNDO.
            t = 1.
            FOR EACH ITEM WHERE ITEM.company EQ cocode
                AND ITEM.procat BEGINS lcSearch 
                AND item.industry = "1" NO-LOCK BY ITEM.procat
                                                BY ITEM.i-name:
