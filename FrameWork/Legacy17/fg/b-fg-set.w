...
&SCOPED-DEFINE dataGridInclude dataGrid\fg\b-fg-set.i
&SCOPED-DEFINE noGetCellColumns
...
itemfg.cust-no itemfg.i-dscr itemfg.style itemfg.procat itemfg.i-code ~
itemfg.est-no itemfg.cad-no itemfg.spc-no 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH itemfg WHERE ~{&KEY-PHRASE} ~
      AND itemfg.company = g_company and itemfg.isaset NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
DEFINE FRAME F-Main
     ...
     auto_find AT ROW 19.33 COL 110 COLON-ALIGNED HELP
...
/* Query rebuild information for BROWSE Browser-Table
     ...
     _TblOptList       = "USED"
     _Where[1]         = "itemfg.company = g_company and ASI.itemfg.isaset"
     _FldNameList[1]   = ASI.itemfg.i-no
     _FldNameList[2]   = ASI.itemfg.i-name
     _FldNameList[3]   = ASI.itemfg.cust-no
     _FldNameList[4]   = ASI.itemfg.i-dscr
...
"itemfg.cad-no" "CAD #" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[10]   > ASI.itemfg.spc-no
"itemfg.spc-no" "Quality/SPC #" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
