...
ASSIGN cTextListToSelect = "Job#,Customer,Machine,#Layout,JobAve#Up,Issued,Sheeted," +
...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Job#,Customer,Machine,#Layout,JobAve#Up,Issued,Sheeted," +
                                               "Press Qty,DC Qty,Plan'd Waste,Qty  Produced,Qty Received,Qty Ordered".
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME FRAME-A
     ...
     sl_avail AT ROW 10.76 COL 4.6 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 10.76 COL 40.6 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 10.76 COL 60 NO-LABEL WIDGET-ID 28
     ...
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 10.05 COL 60 WIDGET-ID 44
     "Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.24 COL 5
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 16.52 COL 3
     ...
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 9.91 COL 6 WIDGET-ID 38
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-6 RECT-7 rd_jstat begin_job-no begin_job-no2 end_job-no 
         ...
         tb_runExcel fi_file btn-ok btn-cancel sl_avail Btn_Def sl_selected
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "mach"         THEN cVarValue = STRING(bjob-mch.m-code)  .
                         WHEN "fg-job"   THEN cVarValue = string(v-sheet,'->9') .
                         WHEN "avg-up"       THEN cVarValue = string(v-blank,'->9.999') .
                         WHEN "isu-sht"      THEN cVarValue = string(v-sheets[1],'->,>>>,>>9').
                         WHEN "prs-sht" THEN cVarValue = string(v-sheets[2],'->>,>>>,>>9') .                   
                         WHEN "ctng-prs"     THEN cVarValue = string(v-sheets[3],'->>>,>>>,>>9').
                         WHEN "ctng-isu"     THEN cVarValue = string(v-sheets[4],'->,>>>,>>>,>>9') .
                         WHEN "pln-wst"      THEN cVarValue = string(v-blanks[1],'->>>,>>>,>>9') .
                         WHEN "crt-pro"      THEN cVarValue = string(v-blanks[2],'->>>,>>>,>>9').
                         WHEN "fg-rece"      THEN cVarValue = string(v-blanks[3],'->,>>>,>>9').
                         WHEN "cust-ord"       THEN cVarValue = string(v-blanks[4],'->,>>>,>>9'). 
