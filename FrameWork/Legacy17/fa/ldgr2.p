/* fa/ldgr2.p - Asset Ledger TAX - 2 Depreciation */
...
{fa/ldgr2.y}                /* YOUR PARAMETERS INCLUDE FILE */
...
FOR EACH FA-MAST WHERE fa-mast.fa-entity eq fa-control.fa-entity
  ...
  {pt/newpage main-loop string(fa-mast.asset-code)}
    ...
    if fa-mast.asset-status = "R":U then
       acc-dep = fa-mast.acc-dep-tax2 - fa-mast.cy-dep-tax-2.
    else acc-dep = fa-mast.acc-dep-tax2.
...
if show-detail = yes then do:
  FIND FIRST fa-entry OF fa-mast WHERE fa-entry.method = "TAX2":U AND
  ...
  if show-detail = yes then do:
    ...
    FOR EACH FA-ENTRY OF FA-MAST WHERE FA-ENTRY.METHOD = "TAX2":U
...
if show-detail = no then do:
   ...
   do loop = beg-prd to end-prd :
      ...
      if fa-mast.tax2-period[loop] >= 0 then debit# = fa-mast.tax2-period[loop].
      else credit# = fa-mast.tax2-period[loop].
      tot-debit = tot-debit + debit#.
      tot-credit = tot-credit + credit#.
      net-value = net-value - (debit# - credit#).

      if fa-mast.tax2-period[loop] ne 0 then do:
      DISPLAY
