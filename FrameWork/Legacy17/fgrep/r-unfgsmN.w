...
ASSIGN cTextListToSelect = "FG Item#,Description,Ordered,Shipped,BalTo Ship,"
                         + "Issued BOL,On Hand,Available,Open PO,Net Variance" 
       cFieldListToSelect = "ino,dscr,ord,ship,balto-shp,"
                          + "isu-bol,oh,aval,opn-po,net-var"
       cFieldLength = "15,30,10,10,10," + "10,10,10,10,12"
       cFieldType = "c,c,i,i,i," + "i,i,i,i,i" 
    .

{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "FG Item#,Description,Ordered,Shipped,BalTo Ship,"
                         + "Issued BOL,On Hand,Available,Open PO,Net Variance"  .
...
end_i-no begin_whse end_whse sl_avail sl_selected Btn_Def Btn_Add ~
...
begin_whse end_whse sl_avail sl_selected lv-ornt lines-per-page rd-dest ~
...
DEFINE FRAME FRAME-A
     lbl_ord-stat AT ROW 1.91 COL 26 COLON-ALIGNED NO-LABEL
     rd_ord-stat AT ROW 1.91 COL 43 NO-LABEL
     begin_ord-date AT ROW 2.95 COL 26 COLON-ALIGNED
     end_ord-date AT ROW 2.95 COL 69 COLON-ALIGNED HELP
          "Enter Ending Order Date"
     tb_cust-list AT ROW 4.1 COL 29.2 WIDGET-ID 6
     btnCustList AT ROW 4.14 COL 61.2 WIDGET-ID 8
     begin_cust AT ROW 5.24 COL 26 COLON-ALIGNED HELP
          "Enter Beginning Customer Number"
     end_cust AT ROW 5.24 COL 69 COLON-ALIGNED HELP
          "Enter Ending Customer Number"
     begin_i-no AT ROW 6.19 COL 26 COLON-ALIGNED HELP
          "Enter Beginning Order Number"
     end_i-no AT ROW 6.19 COL 69 COLON-ALIGNED HELP
          "Enter Ending Item Number"
     begin_whse AT ROW 7.14 COL 26 COLON-ALIGNED HELP
          "Enter Beginning Warehouse"
     end_whse AT ROW 7.14 COL 69 COLON-ALIGNED HELP
          "Enter Ending Warehouse"
     sl_avail AT ROW 9.38 COL 4.2 NO-LABEL WIDGET-ID 26
     sl_selected AT ROW 9.38 COL 59.2 NO-LABEL WIDGET-ID 28
     Btn_Def AT ROW 9.48 COL 40.8 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     Btn_Add AT ROW 10.62 COL 40.8 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 11.81 COL 40.8 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 13 COL 40.8 WIDGET-ID 40
     btn_down AT ROW 14.19 COL 40.8 WIDGET-ID 42
     lv-ornt AT ROW 16.52 COL 31 NO-LABEL
     lines-per-page AT ROW 16.52 COL 84 COLON-ALIGNED
     rd-dest AT ROW 17.24 COL 5 NO-LABEL
     lv-font-no AT ROW 19.62 COL 34 COLON-ALIGNED
     lv-font-name AT ROW 20.57 COL 28 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 21.91 COL 30
     tb_excel AT ROW 22.71 COL 71 RIGHT-ALIGNED
     tb_runExcel AT ROW 22.71 COL 93 RIGHT-ALIGNED
     fi_file AT ROW 23.62 COL 49 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 25.57 COL 24
     btn-cancel AT ROW 25.57 COL 57
     "Available Columns" VIEW-AS TEXT
          SIZE 20 BY .95 AT ROW 8.43 COL 5 WIDGET-ID 38
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 8.57 COL 59.4 WIDGET-ID 44
...
------------------------------------------------------------------------------*/
  DISPLAY lbl_ord-stat rd_ord-stat begin_ord-date end_ord-date tb_cust-list 
          begin_cust end_cust begin_i-no end_i-no begin_whse end_whse sl_avail 
  ...
  ENABLE RECT-17 RECT-6 rd_ord-stat begin_ord-date end_ord-date tb_cust-list 
         ...
         end_whse sl_avail sl_selected Btn_Def Btn_Add Btn_Remove btn_Up 
    ...
    /*    display oe-ordl.i-no        column-label "!Item"
     ...
     DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            CASE cTmpField:    
                WHEN "ino"        THEN cVarValue = oe-ordl.i-no  .                 
                WHEN "dscr"       THEN cvarValue = itemfg.i-name  .                
                WHEN "ord"        THEN cVarValue = STRING(v-ono[1],"->>>>>>>>9") . 
                WHEN "ship"       THEN cVarValue = STRING(v-shp[1],"->>>>>>>>9").  
                WHEN "balto-shp"  THEN cVarValue = STRING(v-bal,"->>>>>>>>9") .    
                WHEN "isu-bol"    THEN cVarValue = STRING(v-bol[1],"->>>>>>>>9"). 
                WHEN "oh"         THEN cvarValue = STRING(v-onh[1],"->>>>>>>>9") . 
                WHEN "aval"       THEN cVarValue = STRING(v-una,"->>>>>>>>9") .    
                WHEN "opn-po"     THEN cVarValue = STRING(v-opo[1],"->>>>>>>>9").  
                WHEN "net-var"    THEN cvarValue = STRING(v-var,"->>>>>>>>>>9").     
     ...
     DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            CASE cTmpField:    
                ...
                WHEN "ord"        THEN cVarValue = STRING(v-ono[2],"->>>>>>>>9") . 
                WHEN "ship"       THEN cVarValue = STRING(v-shp[2],"->>>>>>>>9").  
                WHEN "balto-shp"  THEN cVarValue = STRING(v-bal,"->>>>>>>>9") .    
                WHEN "isu-bol"    THEN cVarValue = STRING(v-bol[2],"->>>>>>>>9"). 
                WHEN "oh"         THEN cvarValue = STRING(v-onh[2],"->>>>>>>>9") . 
                WHEN "aval"       THEN cVarValue = STRING(v-una,"->>>>>>>>9") .    
                WHEN "opn-po"     THEN cVarValue = STRING(v-opo[2],"->>>>>>>>9").  
                WHEN "net-var"    THEN cvarValue = STRING(v-var,"->>>>>>>>>>9").     
            ...
            cExcelDisplay = cExcelDisplay + quoter(cExcelVarValue) + ",". 
    END.
    PUT UNFORMATTED "    tt-report Totals:" substring(cDisplay,22,300) SKIP.
