/* check for Harwell laser copied from ap-ckasi.p */
...
/*                                       to 23 format "x(10)"        */
/*         wrk-chk.inv-date              to 32 format "99/99/99"     */
/*         wrk-chk.inv-amt               to 44 format "->>>,>>9.99"  */
/*         wrk-chk.amt-paid              to 56 format "->>>,>>9.99"  */
/*         wrk-chk.disc-amt              to 67 format "->>,>>9.99"   */
/*         wrk-chk.line-amt              to 79 format "->>>,>>9.99". */
    put wrk-chk.inv-no                AT 1 format "x(12)"
        trim(string(wrk-chk.po-no,">>>>>>"))
                                      AT 14 format "x(10)"
        wrk-chk.inv-date              AT 25 format "99/99/99"
        wrk-chk.inv-amt               AT 34 format "->>>,>>9.99"
        wrk-chk.amt-paid              AT 46 format "->>>,>>9.99"
        wrk-chk.disc-amt              AT 58 format "->>,>>9.99"
        wrk-chk.line-amt              AT 69 format "->>>,>>9.99".
    assign
     ll      = ll + 1
     cgrossl = 0
     ll-void = NO   
