...
ASSIGN cTextListToSelect = "Vendor#,Name,Memo#,Memo Date,Amount,Invoice#,Due Date,Balance Due,Credit Amount," +
                                               "Debit Amount,Account Number,Account Description" 

      cFieldListToSelect = "vend,name,memo,mmo-dt,amt,inv,due-dt,bal-due,crdt-amt,debit-amt,actno,actdscr" .
{sys/inc/ttRptSel.i}

    ASSIGN cTextListToDefault  = "Vendor#,Name,Memo#,Memo Date,Amount,Invoice#,Due Date,Balance Due,Credit Amount," +
                                               "Debit Amount,Account Number,Account Description" .
...
DEFINE FRAME rd-fgnq-exp
     begin_vend AT ROW 3.86 COL 25.6 COLON-ALIGNED HELP
          "Enter Beginning Vendor#" WIDGET-ID 142
     end_vend AT ROW 3.95 COL 67.6 COLON-ALIGNED HELP
...
ON LEAVE OF begin_vend IN FRAME rd-fgnq-exp /* From Invoice# */
DO:
   assign {&self-name}.
END.
...
ON LEAVE OF end_vend IN FRAME rd-fgnq-exp /* To Invoice# */
DO:
     assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_vend end_vend sl_avail sl_selected tb_excel tb_runExcel fi_file 
      WITH FRAME rd-fgnq-exp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_vend end_vend sl_avail sl_selected Btn_Def 
...
li-pallets = 0 .
    ...
    FOR EACH ap-pay WHERE ap-pay.company EQ cocode 
        ...
        AND ap-pay.vend-no LE end_vend NO-LOCK,
        EACH ap-payl OF ap-pay WHERE TRUE NO-LOCK,
      EACH vend OF ap-pay NO-LOCK:
       ...
       FOR EACH ttRptSelected:
         /* IF LOOKUP(ttRptSelected.TextList, "Contact,Title,Email") <> 0    THEN do: */
              ...
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-pay.vend-no)).
              IF ttRptSelected.TextList = "Name" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(vend.name)).
              IF ttRptSelected.TextList = "Memo#" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-pay.check-no)).
              IF ttRptSelected.TextList = "Memo Date" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-pay.check-date)).
              IF ttRptSelected.TextList = "Amount" THEN do:
                  ...
                  DEF VAR ld-amt AS DEC NO-UNDO.
                    ...
                    /*  END.*/
                      ...
                      v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ld-amt)).
              END.
              IF ttRptSelected.TextList = "Invoice#" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-payl.inv-no)).
              IF ttRptSelected.TextList = "Due Date" THEN do:
                  IF ap-payl.due-date NE ? THEN
                      v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-payl.due-date)).
                  ELSE
                      v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string("")).
              END.
              IF ttRptSelected.TextList = "Balance Due" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-payl.amt-due)).
              IF ttRptSelected.TextList = "Credit Amount" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-payl.amt-paid)).
              IF ttRptSelected.TextList = "Debit Amount" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-payl.amt-disc)).
              IF ttRptSelected.TextList = "Account Number" THEN
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-payl.actnum)).
              IF ttRptSelected.TextList = "Account Description" THEN do:
