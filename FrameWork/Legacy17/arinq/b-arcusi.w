...
&SCOPED-DEFINE dataGridInclude dataGrid\arinq\b-arcusi.i
...
ASI.ar-inv.due    GT 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH ar-inv OF cust WHERE ~{&KEY-PHRASE} ~
      AND ar-inv.posted EQ YES AND ~
ASI.ar-inv.due    GT 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table ar-inv
...
RUN set-attribute-list (
    'Keys-Accepted = ,
     Keys-Supplied = "check-no,company,x-no,Carrier"':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ar-inv.inv-date FORMAT "99/99/9999":U WIDTH 17
      ar-inv.inv-no COLUMN-LABEL "Invoice#" FORMAT ">>>>>>>>":U
            WIDTH 14
      ar-inv.paid + ar-inv.due @ ld-inv-amt COLUMN-LABEL "Invoice Amt" FORMAT "->>,>>>,>>9.99":U
            WIDTH 20
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 91 BY 12.62
         BGCOLOR 8 FONT 2
...
ASSIGN 
       ar-inv.inv-date:AUTO-RESIZE IN BROWSE br_table = TRUE
       ar-inv.inv-no:AUTO-RESIZE IN BROWSE br_table = TRUE
       ar-inv.due:AUTO-RESIZE IN BROWSE br_table = TRUE.
...
/* Query rebuild information for BROWSE br_table
     _TblList          = "ASI.ar-inv OF ASI.cust"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _Where[1]         = "ASI.ar-inv.posted EQ YES AND
ASI.ar-inv.due    GT 0"
     _FldNameList[1]   > ASI.ar-inv.inv-date
"ar-inv.inv-date" ? ? "date" ? ? ? ? ? ? no ? no no "17" yes yes no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > ASI.ar-inv.inv-no
"ar-inv.inv-no" "Invoice#" ">>>>>>>>" "integer" ? ? ? ? ? ? no ? no no "14" yes yes no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > "_<CALC>"
"ar-inv.paid + ar-inv.due @ ld-inv-amt" "Invoice Amt" "->>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no "20" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > ASI.ar-inv.due
"ar-inv.due" "Balance Due" ? "decimal" ? ? ? ? ? ? no ? no no "20" yes yes no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > "_<CALC>"
"TODAY - ar-inv.inv-date @ li-days-old" "Days Old" "->>,>>>" ? ? ? ? ? ? ? no ? no no "12" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
ON ROW-ENTRY OF br_table IN FRAME F-Main /* Open Invoices */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON ROW-LEAVE OF br_table IN FRAME F-Main /* Open Invoices */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br_table B-table-Win
ON VALUE-CHANGED OF br_table IN FRAME F-Main /* Open Invoices */
DO:
