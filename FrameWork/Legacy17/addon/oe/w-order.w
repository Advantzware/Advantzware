...
DEFINE VARIABLE h_b-order AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-oesales AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-order AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ordl AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-ordmisc AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse  Order|View Order|Items|Misc|Sales Analy.' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'oe/b-order.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-order ).
       RUN set-position IN h_b-order ( 4.57 , 3.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'oe/v-order.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-order ).
       RUN set-position IN h_v-order ( 4.57 , 4.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-order. */
       RUN add-link IN adm-broker-hdl ( h_b-order , 'Record':U , h_v-order ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_v-order ).
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ordl ).
       RUN set-position IN h_v-ordl ( 5.05 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 17.14 , 144.00 ) */
    ...
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'oe/v-ordmisc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_v-ordmisc ).
       /* Position in UIB:  ( 6.71 , 16.00 ) */
       /* Size in UIB:  ( 2.05 , 11.60 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('3') NO-ERROR.

       /* Links to  h_v-ordmisc. */
       RUN add-link IN adm-broker-hdl ( h_v-ordl , 'State':U , h_v-ordmisc ).

    END. /* Page 4 */

    WHEN 5 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'oe/v-oesales.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_v-oesales ).
       /* Position in UIB:  ( 8.62 , 33.00 ) */
       /* Size in UIB:  ( 2.05 , 11.60 ) */
