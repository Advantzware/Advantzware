...
&Scoped-define BUTTON-INCLUDE FORMS
...
DEFINE BUTTON Btn_First 
     LABEL "FIRST FORM" 
     SIZE 20 BY 2.38 TOOLTIP "FIRST FORM".

DEFINE BUTTON Btn_Last 
     LABEL "LAST FORM" 
     SIZE 20 BY 2.38 TOOLTIP "LAST FORM".
...
ON CHOOSE OF Btn_First IN FRAME F-Main /* FIRST FORM */
DO:
  button_item = 1.
...
ON CHOOSE OF Btn_Last IN FRAME F-Main /* LAST FORM */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Get_Forms s-object 
PROCEDURE Get_Forms :
/*------------------------------------------------------------------------------
  Purpose:     populate selection list with form numbers
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
              ...
              IF FIRST-OF(job-mch.blank-no) THEN DO:
                 ...
                 THEN DO:
                    IF INDEX(itemlist,STRING(job-mch.frm)) = 0 THEN 
                       itemlist = IF itemlist = '' THEN 'FORM: ' + STRING(job-mch.frm)
