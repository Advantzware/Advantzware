...
DEFINE VARIABLE h_rfqpart AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_rfqtool-2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Part#|Brws Cust|View RFQ|RFQ Item|Size|Item Spec|Materials|Printing|Shipping' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/rfqpart.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_rfqpart ).
       RUN set-position IN h_rfqpart ( 4.57 , 4.00 ) NO-ERROR.
       RUN set-size IN h_rfqpart ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('3') NO-ERROR.

       /* Links to SmartNavBrowser h_rfqpart. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_rfqpart ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/rfqtool.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_rfqtool-2 ).
       RUN set-position IN h_rfqtool-2 ( 4.57 , 4.00 ) NO-ERROR.
       RUN set-size IN h_rfqtool-2 ( 19.52 , 145.00 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_rfqpart , 'Record':U , h_rfqtool ).
    ...
    WHEN 4 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_rfqtoest ).
       RUN set-position IN h_rfqtoest ( 21.71 , 95.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_rfqpart , 'Record':U , h_rfq-3 ).

       /* Links to SmartBrowser h_rfqitem. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav-2 , 'TableIO':U , h_rfqitem ).
       RUN add-link IN adm-broker-hdl ( h_rfqpart , 'Record':U , h_rfqitem ).
    ...
    WHEN 5 THEN DO:
       ...
       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('6,4') NO-ERROR.
    ...
    WHEN 6 THEN DO:
       ...
       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1,4') NO-ERROR.

       /* Links to SmartViewer h_rfq. */
       RUN add-link IN adm-broker-hdl ( h_rfqpart , 'Record':U , h_rfq ).
    ...
    WHEN 8 THEN DO:
       ...
       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('4,5') NO-ERROR.
    ...
    WHEN 9 THEN DO:
       ...
       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('5,4') NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  RUN get-attribute IN THIS-PROCEDURE ('Current-Page':U).
  if integer(return-value) > 4 and not ll-enable-trx then do:
     ll-enable-trx = yes.
