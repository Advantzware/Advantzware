...
&Scoped-define PROGNAME machtra_.
...
DEFINE QUERY machtran-query FOR machtran.
...
DEFINE VARIABLE show-employees AS LOGICAL FORMAT "yes/no" LABEL "Show Employee Transactions" NO-UNDO.
...
PROCEDURE List-Logic:
  CASE list-order:
    WHEN 1 THEN
    OPEN QUERY machtran-query FOR EACH machtran NO-LOCK WHERE
  ...
  END CASE.
  GET FIRST machtran-query.
  DO WHILE AVAILABLE(machtran)
      WITH FRAME {&FRAME-NAME} NO-BOX WIDTH 132 STREAM-IO DOWN:
    {methods/lstlogic/custom/machtra_.i}
    DOWN.
    GET NEXT machtran-query.
  END.
...
PROCEDURE Show-Selections:
  ...
  DO:
    ...
    RUN SetCellFont IN h_Viper("Cell","Parameter",12,0,"bold","Arial").
    RUN SetCellPos IN h_Viper("Cell","Parameter",1,y-pos,800,0).
    RUN SetCellText IN h_Viper("Cell","Parameter","Show Employee Transactions:").
    RUN SetCellAlign IN h_Viper("Cell","Parameter",2).
    RUN FlushGroup IN h_Viper ("Parameter").
    RUN SetCellFont IN h_Viper("Cell","Parameter",12,0,"","Arial").
    RUN SetCellPos IN h_Viper("Cell","Parameter",870,y-pos,700,0).
    RUN SetCellText IN h_Viper("Cell","Parameter",show-employees).
    RUN SetCellAlign IN h_Viper("Cell","Parameter",1).
    RUN FlushGroup IN h_Viper ("Parameter").
  END.
  ELSE
