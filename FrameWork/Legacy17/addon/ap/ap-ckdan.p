/* -------------------------------------------------- ap/ap-ckdan.p 01/99 JLF */
/* AP Check Print for Danbury                                                 */
/* -------------------------------------------------------------------------- */
...
form skip(5)
     ap-chk.check-no   to 61
     ap-chk.check-date to 70
     vend.vend-no      to 79
     skip(2)
     dol               at 2
...
if v-print-mode ne "ALIGN" then do:  /* production mode */
  for each ap-chk
    ...
    put ap-sel.dscr                     at 2    format "x(7)"
        ap-sel.inv-no                   at 10   format "x(9)"
        ap-inv.inv-date                 to 27
        ap-inv.net                      to 42   format "->>>,>>9.99"
        ap-inv.paid                     to 56   format "->>>,>>9.99"
