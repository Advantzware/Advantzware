    ...
    {cecrep/jobprem.i}
      ...
      for each w-ef WHERE (w-ef.frm = job-hdr.frm OR est.est-type <> 8),
       ...
       /*===== for xprint */
        ...
        IF cust.over-pct > 0 THEN
            v-job-qty = v-job-qty / (1 + (cust.over-pct / 100)).
        v-ord-qty = (if avail xoe-ordl then xoe-ordl.qty
                     else v-job-qty) * 
        ...
        DISP "<=#2> CUSTOMER INFORMATION" SKIP
            ...
            "<=#3><R-1>        " lv-ord-qty * ( 1 + round((lv-over-run) / 100,2)) FORM ">>>,>>9"
                       "         "     lv-ord-qty * ( 1 - round((lv-under-run) / 100,2)) FORM ">>>,>>9"
     ...
     PUT UNFORMATTED
        ...
        IF CAN-DO("TriState,RFC,Boxtech,Brick,Corrugat,ASI,Xprint,Pacific,TriLake,CSC-GA",v-format) THEN do:
          ...
          IF v-vend-no NE "" THEN DO:
           ...
           IF v-po-no NE 0 AND LOOKUP("CSC-GA",v-format) = 0 THEN
      ...
      /* for xprint */
        ...
        FIND FIRST xoe-rel WHERE xoe-rel.company = cocode
            AND xoe-rel.ord-no = xoe-ordl.ord-no 
            AND xoe-rel.LINE = xoe-ordl.LINE
            AND xoe-rel.i-no   = xoe-ordl.i-no NO-LOCK NO-ERROR.
      ...
      IF s-prt-set-header AND last-of(job.job-no2) AND est.est-type = 6 THEN DO: /* print set header */
        ...
        IF i > 1 THEN DO:
           ...
           v-set-qty = if avail xeb and xeb.est-type eq 6 THEN
                       ...
                       ELSE 0
           v-ord-qty = (if avail xoe-ordl then xoe-ordl.qty else v-job-qty) *
                       if est.form-qty le 1 then 1 else v-pqty
