...
DEF INPUT PARAM ip-cust-no LIKE eb.cust-no NO-UNDO.
    ...
    */
    CASE rd-sort:
        WHEN 1 THEN DO:
          ...
          OPEN QUERY {&browse-name} FOR EACH ASI.eb WHERE ASI.eb.company = ip-company 
        ...
        WHEN 2 THEN DO:
              ...
              OPEN QUERY {&browse-name} FOR EACH ASI.eb WHERE ASI.eb.company = ip-company 
                                        and eb.cust-no = ip-cust-no 
                                        AND trim(eb.part-no) BEGINS lv-search  NO-LOCK.
