...
DEFINE VARIABLE ip-ord-no AS INT NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_cstattch-2 ).
       RUN set-position IN h_cstattch-2 ( 4.81 , 3.60 ) NO-ERROR.
       RUN set-size IN h_cstattch-2 ( 18.10 , 145.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/cstattch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_cstattch ).
       RUN set-position IN h_cstattch ( 6.95 , 6.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_cstattch. */
       RUN add-link IN adm-broker-hdl ( h_cstattch-2 , 'Record':U , h_cstattch ).
       RUN add-link IN adm-broker-hdl ( h_p-attach , 'TableIO':U , h_cstattch ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_cstattch ,
             h_folder , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE OUTPUT PARAMETER op-ord-no AS CHAR NO-UNDO.

  op-rec_key = ip-rec_key.

  IF ip-ord-no NE 0 THEN
     op-ord-no = STRING(ip-ord-no).
