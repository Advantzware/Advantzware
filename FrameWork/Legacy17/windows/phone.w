...
/* MESSAGE 'Message For: DENNIS'       skip */
/*         'windows\phone.w'           skip */
/*         'Section: Definitions'      skip */
/*         'ip-rec_key:'   ip-rec_key  skip */
/*         's-rec_key:'    s-rec_key        */
...
DEFINE VARIABLE h_phone-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Phone Information */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Phone Information */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_options ).
       RUN set-position IN h_options ( 1.00 , 59.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       /* Links to SmartNavBrowser h_phone. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_phone ).
       RUN add-link IN adm-broker-hdl ( h_phone , 'Record':U , THIS-PROCEDURE ).
    ...
    WHEN 2 THEN DO:
       ...
       /* Links to SmartViewer h_phone-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_phone-2 ).
       RUN add-link IN adm-broker-hdl ( h_phone , 'Record':U , h_phone-2 ).
...
/*     IF VALID-HANDLE (h_phone-2)   THEN RUN EMailNotify        IN  h_phone-2 (YES). */
    ...
    RUN set-position      IN h_p-navico (  5.00 , 3.60 )  NO-ERROR.
    RUN set-position      IN h_p-updsav ( 18.14 , 3.60 )  NO-ERROR.
    RUN local-view        IN h_emailcod                   NO-ERROR.
  END.

  ELSE DO:
    RUN set-position      IN h_phone-2  ( 6.62 , 34.60 ) NO-ERROR.
    RUN set-position      IN h_p-navico (  5.00 ,34.60 ) NO-ERROR.
    RUN set-position      IN h_p-updsav ( 18.14 ,34.60 ) NO-ERROR.
    RUN local-hide        IN h_emailcod                  NO-ERROR.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setAddStatus W-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE INPUT PARAMETER iplAddStatus AS CHARACTER NO-UNDO.

  RUN setAddStatus IN h_emailcod (INPUT iplAddStatus).
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION ('RefreshBrowse':U IN h_phone).
