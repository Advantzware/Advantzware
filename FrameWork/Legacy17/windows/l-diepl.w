...
&scoped-define fld-name-1 prep.code
&scoped-define fld-name-2 prep.dscr
&scoped-define SORTBY-1 BY prep.code
&scoped-define SORTBY-2 BY prep.dscr
...
&Scoped-define INTERNAL-TABLES prep
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH prep WHERE ~{&KEY-PHRASE} ~
      AND prep.company = ip-company and ~
(index(ip-type,prep.mat-type) > 0 or ip-type = "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY BROWSE-1 FOR EACH prep WHERE ~{&KEY-PHRASE} ~
      AND prep.company = ip-company and ~
(index(ip-type,prep.mat-type) > 0 or ip-type = "") NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 prep
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 prep
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _Where[1]         = "ASI.prep.company = ip-company and
(index(ip-type,prep.mat-type) > 0 or ip-type = """")"
...
DO:
  op-char-val = prep.code + "," +
                 prep.dscr:screen-value in browse {&browse-name} + "," +
                 string(prep.cost) + "," +
                 string(prep.ml) + "," +
                 prep.simon + "," +
                 string(prep.mkup) + "," + 
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  {custom/lookpos.i &lookup-file = "prep" &lookup-field = "code"}
