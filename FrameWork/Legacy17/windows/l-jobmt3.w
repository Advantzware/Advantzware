...
DO:
  ...
  IF {&browse-name}:NUM-SELECTED-ROWS LT 1 THEN DO:
    MESSAGE 
        "Please select at least one RM Item from the browser or " SKIP
        "cancel to exit."
  ...
  END.
  ELSE 
  IF {&browse-name}:NUM-SELECTED-ROWS GT 1 THEN DO:
  ...
  END.
   ...
   DO li = 1 TO {&browse-name}:NUM-SELECTED-ROWS:
    {&browse-name}:FETCH-SELECTED-ROW (li) NO-ERROR.    
      ASSIGN op-char-val = op-char-val + "," + STRING(RECID(job-mat)).
      ...
      IF li EQ 1 THEN
        ...
        CREATE w-rm-rctd.
        ASSIGN
         w-rm-rctd.i-no          = lv-rm-i-no
         w-rm-rctd.job-no        = job-mat.job-no
         w-rm-rctd.job-no2       = job-mat.job-no2 
         w-rm-rctd.s-num         = job-mat.frm
         w-rm-rctd.b-num         = job-mat.blank-no
         w-rm-rctd.job-mat-rowid = ROWID(job-mat).
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  IF NOT CAN-FIND(FIRST w-rm-rctd) THEN DO:
