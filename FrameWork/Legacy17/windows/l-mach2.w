...
DO:
   ...
   DO i = 1 TO BROWSE {&browse-name}:NUM-SELECTED-ROWS:
      v-log = BROWSE {&browse-name}:FETCH-SELECTED-ROW(i).
      op-char-val = op-char-val + mach.m-code + ",".
   END.
   IF SUBSTRING(op-char-val,LENGTH(op-char-val),1) = "," THEN
      op-char-val = SUBSTRING(op-char-val,1,LENGTH(op-char-val) - 1).
