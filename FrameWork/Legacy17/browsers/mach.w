...
&SCOPED-DEFINE dataGridInclude dataGrid\browsers\mach.i
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH mach WHERE ~{&KEY-PHRASE} ~
    ...
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH mach WHERE ~{&KEY-PHRASE} ~
      AND mach.company = g_company NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      ...
      string(mach.dept[1] + mach.dept[2] + mach.dept[3] + mach.dept[4]) @ mach.dept[1] FORMAT "x(13)":U
      mach.m-dscr FORMAT "x(20)":U
      ...
      mach.p-type FORMAT "x":U
      mach.run-spoil FORMAT ">>9.99":U
...
"string(mach.dept[1] + mach.dept[2] + mach.dept[3] + mach.dept[4]) @ mach.dept[1]" ? "x(13)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   = ASI.mach.m-dscr
...
"mach.sch-m-code" "Sched Mach" ? "character" ? ? ? ? ? ? no ? no no "14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[10]   = ASI.mach.p-type
     _FldNameList[11]   = ASI.mach.run-spoil
     _FldNameList[12]   = ASI.mach.mr-waste
