...
&SCOPED-DEFINE dataGridInclude dataGrid\browsers\emailcod3.i
...
------------------------------------------------------------------------------*/
    ...
    THEN DO:
      ...
      CREATE reftable.
      ASSIGN reftable.rec_key   = STRING (vrEmpAlert)
             reftable.CODE      = emailcod.emailcod.

      FIND FIRST b-EmpAlert WHERE
           RECID (b-EmpAlert) = vrEmpAlert
           NO-LOCK NO-ERROR.

      IF AVAIL b-EmpAlert THEN
      DO:
         IF NOT can-find(FIRST emaildtl WHERE
            emaildtl.emailcod = emailcod.emailcod AND
            emaildtl.table_rec_key = b-EmpAlert.rec_key) THEN
            DO:
               CREATE emaildtl.
               ASSIGN emaildtl.emailcod = emailcod.emailcod
                      emaildtl.table_rec_key = b-EmpAlert.rec_key.
               RELEASE emaildtl.
            END.
    ...
    ELSE DO:
      ...
      DO:
         FIND FIRST emaildtl WHERE
              ...
              emaildtl.table_rec_key EQ b-EmpAlert.rec_key
...
DO:
  ...
  FIND FIRST empalert NO-LOCK
       WHERE RECID (empalert) = vrEmpAlert NO-ERROR.
END.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF VAR v-sen AS LOG NO-UNDO.

  {&OPEN-QUERY-Browser-Table}

  ASSIGN
     v-sen = NOT vrEmpAlert EQ ?
     BROWSE {&browse-name}:SENSITIVE = v-sen
     browse-order:SENSITIVE IN FRAME {&FRAME-NAME}  = v-sen
     auto_find:SENSITIVE IN FRAME {&FRAME-NAME} = v-sen
     btn_clear_find:SENSITIVE IN FRAME {&FRAME-NAME} = v-sen
     btnSelectAll:SENSITIVE IN FRAME {&FRAME-NAME} = v-sen
     btnClear:SENSITIVE IN FRAME {&FRAME-NAME} = v-sen.
