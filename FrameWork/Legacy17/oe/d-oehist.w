...
DEFINE VARIABLE h_b-oehist AS HANDLE NO-UNDO.
...
DEFINE FRAME D-Dialog
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Sales Analysis Inquiry for Customer:".
...
FIND oe-ordl WHERE ROWID(oe-ordl) EQ io-rowid NO-LOCK NO-ERROR.
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_b-oehist ).
       RUN set-position IN h_b-oehist ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-oehist ( 17.86 , 155.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-oehist. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_b-oehist ).
       RUN add-link IN adm-broker-hdl ( h_b-oehist , 'row-id':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-oehist ,
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN
   op-ord-no  = oe-ordl.ord-no
   op-item-no = oe-ordl.i-no.
