...
&SCOPED-DEFINE dataGridInclude dataGrid\oe\b-credit.i
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      (ar-inv.paid + ar-inv.due) @ ld-inv-amt COLUMN-LABEL "Invoice Amount"
            COLUMN-FONT 0
      ar-inv.due FORMAT "->>,>>>,>>9.99":U COLUMN-FONT 0
      (today - ar-inv.inv-date) @ li-days-old COLUMN-LABEL "Days Old"
            COLUMN-FONT 0
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 109 BY 7.62
         FONT 0.
...
/* Query rebuild information for BROWSE br_table
     ...
     _OrdList          = "ASI.ar-inv.inv-date|yes"
     _Where[1]         = "ASI.ar-inv.due ne 0"
     _FldNameList[1]   > ASI.ar-inv.inv-date
"ar-inv.inv-date" ? ? "date" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > ASI.ar-inv.inv-no
"ar-inv.inv-no" "Invoice Number" ? "integer" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > "_<CALC>"
"(ar-inv.paid + ar-inv.due) @ ld-inv-amt" "Invoice Amount" ? ? ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   > ASI.ar-inv.due
"ar-inv.due" ? ? "decimal" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > "_<CALC>"
"(today - ar-inv.inv-date) @ li-days-old" "Days Old" ? ? ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
