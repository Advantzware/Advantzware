...
for each report where report.term-id eq v-term-id NO-LOCK,
    ...
    if LAST-OF(report.key-06) then do:
       ...
       PUT UNFORMATTED
           "Customer Name:                     " cust.NAME SKIP(1)
           "Customer Order Number:       "  oe-boll.po-no SKIP(1)
           "Customer Part Number:          " (IF AVAIL oe-ordl THEN oe-ordl.part-no ELSE "") SKIP(1)
           "Customer Part Description:     " v-part-desc SKIP(1)
           "Drawing Number:                    " v-cad SKIP(1)
           "Customer Specification/Drawing No./" SKIP
           "Revision No:"                        SKIP(1)
           "Vendor Order Number:            " (IF AVAIL oe-ordl THEN STRING(oe-ordl.ord-no)
                                                  ELSE "") SKIP(1)
           "Vendor Manufacturing Batch Number:" SKIP(1)
           "Quantity Shipped:                "  STRING(v-bol-qty,"ZZ,ZZZ,ZZ9") SKIP(1)
           "Manufactured Date:                 " (IF v-manuf-date NE 12/31/2999 THEN
                                                     STRING(v-manuf-date,"99/99/9999")
                                                  ELSE "") SKIP(1)
           "Release Date:                          " (IF v-rel-date NE 12/31/2999 THEN
