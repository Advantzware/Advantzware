...
def var v-ship-qty as int format "9999999" no-undo.
...
def var v-disc-pct     like inv-line.disc              no-undo.
def var v-tot-freight  as dec format "->>>>>>>>>9.99"  no-undo.
def var v-tot-disc     as dec format "->>>>>>>>>9.99"  no-undo.
def var v-tot-disc-qty as dec format "->>>>>>>>>9.99"  no-undo.
def var v-uom          like inv-line.pr-uom            no-undo.
def var v-nt-tot       as dec format "->>>>>>>>>9.99" no-undo.
def var v-text as char format "x(60)" extent 2.

form header
    v-text[1]   skip
    v-text[2]
with page-bottom STREAM-IO frame text-lines.
...
form header
     ...
     skip(6)
     inv-head.cust-name    at 12                                  
     v-shipto-name         at 51 skip(1)
     inv-head.addr[1]      at 12
     v-shipto-addr[1]      at 51 
     inv-head.addr[2]      at 12
     v-shipto-addr[2]      at 51
     v-addr3               at 12
     v-sold-addr3          at 51
     skip(3)                                                        
...
form inv-misc.charge at 15 format "x(5)"
     inv-misc.dscr   at 22
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      if avail oe-bolh then do:
        assign
            v-text[1] = oe-bolh.ship-i[1]
            v-text[2] = oe-bolh.ship-i[2].
      ...
      DO TRANSACTION:
        ...
        for each inv-line no-lock where inv-line.r-no eq inv-head.r-no:
          ...
          assign
           ...
           case v-uom:
                   when "EA"  or when "L" then
                            v-t-price    = v-ship-qty * v-price.
                   when "C" then
                             v-t-price   = v-ship-qty * (v-price / 100).
                   when "M" then
                              v-t-price  = v-ship-qty * (v-price / 1000).
                   when "CS" then do:
                               if avail oe-ordl and oe-ordl.cas-cnt > 0 then
                                  v-t-price  = v-ship-qty * (v-price / oe-ordl.cas-cnt). 
                               else
                                   v-t-price    = v-ship-qty * v-price.
                               end. /* when "CS" then do: */
           end case.
           ...
           v-tax-total = v-tax-total +
                             if inv-line.tax then v-t-price else 0.

           assign
            v-tot-disc-qty = v-tot-disc-qty + 
                            if inv-line.disc ne 0 then
                              round(v-t-price - round((v-t-price * (1 - (inv-line.disc / 100))),2),2)
                            else 0                           
                            /*   (v-t-price * (inv-line.disc / 100))   */
            v-tot-freight  = inv-head.t-inv-freight.      
  
           display
                  v-ship-qty to 9 format "->>>>9"
                  v-i-dscr   at 15 format "x(30)"
                  v-price    to 68 format ">>>>9.99"
                  v-uom      at 69 format "x(3)" 
                  v-t-price  to 80 format "->>>>9.99"
              with frame f-1 no-attr-space no-labels no-box
                   no-underline down stream-io width 80.
        ...
        for each inv-misc
          ...
          display inv-misc.charge inv-misc.dscr inv-misc.amt with frame detailm.
          down.
        end. /* each inv-misc */
        ...
        /* T O T A L S */
        assign v-tot-disc = v-tot-disc-qty. 
                
        put skip(26 - v-printline) 
            "Less "                             at 42
            v-tot-disc                          to 80 skip                      
            "Freight "                          at 42
            space(11)            
            v-tot-freight to 80.
        ...
        assign v-nt-tot = inv-head.t-inv-rev - v-tax-total - 
                          inv-head.t-inv-tax -
        ...
        put skip                                       
            "NonTaxable SubTotal" at 42
             v-nt-tot             to 80 format "->>>>>>>>>9.99" skip
            "Taxable SubTotal   " at 42
            v-tax-total           to 80 format "->>>>>>>>>9.99" skip
            "Tax                " at 42
            inv-head.t-inv-tax    to 80 format "->>>>>>>>>9.99" skip
            "Total              " at 42
            inv-head.t-inv-rev    to 80 format "->>>>>>>>>9.99" skip.
