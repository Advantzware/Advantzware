/* ----------------------------------------------- oe/rep/ackdee.p            */
/* copied FROM oe/rep/ackfibrex.p rtc 11/17/2008 Task# 10220804               */
...
DEF VAR lv-line-print AS INT INIT 36 NO-UNDO.
DEF VAR ln-cnt AS INT NO-UNDO.
DEF VAR lv-is-fgnote AS LOG NO-UNDO.

DEF VAR v-cad        LIKE eb.cad-no    NO-UNDO. 
DEF VAR v-color      LIKE eb.i-coldscr NO-UNDO.
DEF VAR v-material   LIKE ef.brd-dscr  NO-UNDO.
...
/*         END.                                                    */
        ...
        /* print CAD#, Colours, Materila from quote */
         
        FIND FIRST est WHERE est.company = oe-ord.company
        ...
        IF NOT AVAIL eb AND oe-ordl.est-no <> "" THEN
          IF AVAIL est THEN 
             FIND FIRST eb WHERE eb.company EQ est.company
                             AND eb.est-no  EQ est.est-no
                             AND eb.form-no NE 0 NO-LOCK NO-ERROR.
        ...
        RUN new-page-proc.
        PUT "CAD#: " + v-cad           FORMAT "X(40)" AT 31 SKIP.
        
        RUN new-page-proc.
        PUT "Color: " + v-color        FORMAT "X(40)" AT 31 SKIP.
                
        RUN new-page-proc.
        PUT "Material: " + v-material  FORMAT "X(40)" AT 31 SKIP.
     ...
     END.
      ...
      IF v-tg-whs-mths = TRUE THEN
         PUT

         "<FArial><P8><R56><C3>Inventory not shipped within " v-rs-whs-mths FORMAT "X(1)" " months of the first acknowledged delivery date will"
         "<FArial><P8><R57><C3>be billed and held for up to 3 months free of charge, and then must be shipped.".
/*                                                                                                                                                                   */
/*             "<FArial><P8><R55><C3>Inventory not shipped within " v-rs-whs-mths FORMAT "X(1)" " months of the first acknowledged delivery date will be billed and" */
/*             "<FArial><P8><R56><C3>held for an additional 3 months free of charge. Warehouse fees of $8.90 per pallet per month will"                              */
/*             "<FArial><P8><R57><C3>be charged for all inventory left in the warehouse after 9 months from acknowledged delivery date.".                            */
      PUT
        ...
        "<R65><C48>QF-91 Sales Order Acknowledgement Rev. 3, 1/08" FORMAT "X(1000)".
...
/*MESSAGE "tested" +  STRING(v-printline) + STRING(lv-line-print) VIEW-AS ALERT-BOX ERROR .*/
   if v-printline ge lv-line-print then
   do:
      PAGE.
      {oe/rep/ackdee.i}
