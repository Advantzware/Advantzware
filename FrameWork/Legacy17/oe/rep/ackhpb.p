...
ASSIGN ls-image1 = "images\hpb.jpg".
 ...
 END.
  ...
  FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
      ...
      */
      ...
      v-currency-line = "All amounts are in "
      ...
      for each oe-ordl
       ...
       if v-printline ge lv-line-print then
        ...
        IF v-pdscr2 <> "" THEN do:
            PUT v-pdscr2 FORM "x(30)" AT 31  SKIP.
            v-printline = v-printline + 1.
        END.
        /*
        if v-printline ge lv-line-print then
...
/*  
  ...
  */
     IF v-prntinst THEN DO:
      ...
      PUT "<R48><C2>" v-currency-line
        "<R50><C1><#10><P12><B> Comments </B> <P10> " 
        "<R51><C1>" v-billinst[1] 
        "<R52><C1>" v-billinst[2] 
        "<R53><C1>" v-billinst[3] 
        "<R54><C1>" v-billinst[4]   "<P10>Order Total:" AT 71 "<U>"  v-totord FORM "$->>>>,>>9.99"  "</U><P9>".

      
      PUT "<R56><C1>" " ______________________________________(Please sign and fax back) ".

      PUT 
        "<R58><C1>" SPACE(32) "<P9><B>THIS IS A CONFIRMATION OF YOUR ORDER,NOT AN INVOICE.       <P16>Thank You! <P10></B>" 
        "<R60><C65>Page "  string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)"
        "<FArial><=8><R61><C1><CENTER=C81><B>65 CASCADE STREET, HAMILTON, ONTARIO L8E 3B7</B>"
        "<=8><R62><C1><CENTER=C81>" v-address-2.
