...
FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
   FIRST xinv-head WHERE RECID(xinv-head) EQ report.rec-id NO-LOCK
    ...
    PUT "<R57><C1><#7><FROM><C+80><LINE>"
        ...
        "<=7><C31>" v-inv-freight FORMAT ">>,>>9.99" v-net FORMAT "->,>>>,>>9.99" " " tmp1 "    " tmp2 " " (IF v-net2 <> inv-head.t-inv-rev THEN v-net2 ELSE inv-head.t-inv-rev) FORMAT "->>>>,>>9.99"
