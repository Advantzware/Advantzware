...
DEF VAR v-salesman-name LIKE sman.sname NO-UNDO.
DEF VAR v-inv-misc-line-cnt AS INT NO-UNDO.
DEF VAR v-number-of-skips AS INT NO-UNDO.
...
ASSIGN ls-image1 = "images\packrite.jpg"
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      DO TRANSACTION:
            ...
            DO:
                ...
                FIND FIRST oe-boll NO-LOCK  
                    WHERE oe-boll.company = inv-line.company
                      /*AND oe-boll.bol-no = inv-head.bol-no*/
                      AND oe-boll.b-no = inv-line.b-no
                      AND oe-boll.i-no = inv-line.i-no NO-ERROR.
                IF AVAIL oe-boll THEN ASSIGN v-rel-po-no = oe-boll.po-no.
                                 
               PUT 
                  SPACE(4) v-ordered-qty  FORMAT "->>>>>9"     SPACE(5)
                  v-ship-qty              FORMAT "->>>>>9"     SPACE(1)
                  v-inv-qty               FORMAT "->>>>>9"     SPACE(1)
                  v-rel-po-no             FORMAT "X(8)"        SPACE(1)
                  v-i-no                  FORMAT "x(21)"       SPACE(1)
                  inv-line.e-num          FORMAT ">>9"         SPACE(1)
                  v-price                 FORMAT ">>>>9.9999"  SPACE(3)
        ...
        /* count how many inv-misc there are */
        FOR EACH b-inv-misc NO-LOCK WHERE b-inv-misc.company = inv-head.company 
                                      AND b-inv-misc.r-no    = inv-head.r-no 
                                      AND b-inv-misc.bill = "Y":
            v-inv-misc-line-cnt = v-inv-misc-line-cnt + 1.
        END. /* each b-inv-misc */

        ASSIGN
         v-inv-misc-line-cnt = v-inv-misc-line-cnt + 5.

        IF v-number-of-skips > v-inv-misc-line-cnt THEN DO:
         PAGE.
            v-printline = 1.
            {oe/rep/invpkrt1.i}
            PUT SKIP.
        ...
        v-number-of-skips = (50 - v-printline) - v-inv-misc-line-cnt.
        ...
        for each inv-misc no-lock where inv-misc.company = inv-head.company and
          ...
          DO:
            PUT 
               SKIP(v-number-of-skips)
               "** Miscellaneous Items **" at 44 skip(1).
            ASSIGN v-printline = v-printline + 2.
          END.

          PUT
             inv-misc.charge AT 44 inv-misc.dscr inv-misc.amt SKIP.
    ...
    PUT "<R56><C60><#8><FROM><R+5><C+20><RECT> " 
    ...
    PUT "<FArial><R57><C25><P12> IF IT'S MICROFLUTE WE MAKE IT!<P9> " SKIP        
        "<FArial><R59><C25><P12> THANK YOU FOR YOUR BUSINESS. <P9>" SKIP.

    PUT "<FArial><R56><C1><P12><B> Comments </B> <P9> " SKIP.
        /* gdm -  12030801
         "  Our GST # R102421401." SKIP
         "  Our GST # R102421402." SKIP
         "  Our GST # R102421403." SKIP.
        */ 
