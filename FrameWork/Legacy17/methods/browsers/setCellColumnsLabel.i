...
PROCEDURE setCellColumns:
  ...
  IF SEARCH(cellColumnDat) NE ? THEN DO:
     ...
     DO i = 1 TO j:
        ...
        DO k = 1 TO j:
           IF userColumn[i] EQ cellColumn[k]:LABEL THEN
              LEAVE.
        END.
        ...
        IF userColumn[i] NE cellColumn[i]:LABEL THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 TO {&BROWSE-NAME}:NUM-COLUMNS IN FRAME {&FRAME-NAME}:
    IF cellColumn[i]:LABEL EQ {&BROWSE-NAME}:GET-BROWSE-COLUMN(i):LABEL AND
    ...
    IF saveChanges THEN DO:
      ...
      DO j = 1 TO {&BROWSE-NAME}:NUM-COLUMNS IN FRAME {&FRAME-NAME}:
        EXPORT {&BROWSE-NAME}:GET-BROWSE-COLUMN(j):LABEL {&BROWSE-NAME}:GET-BROWSE-COLUMN(j):WIDTH-PIXELS.
      END. /* do j */
      OUTPUT CLOSE.
    END. /* if savechanges */
    LEAVE.
