  ...
  for  each cust
    ...
    for each tt-report2
      ...
      if rd_sort BEGINS "S" and first-of(tt-report2.key-01) then do:
        ...
        ASSIGN
        v-slsmn-hdr2 = tt-report2.key-01
        v-slsmn-hdr = if avail sman then sman.sname else "Not on file".
     ...
     DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            CASE cTmpField: 
                WHEN "rep"      THEN cVarValue = STRING(tt-report2.key-01,"x(3)")  .                 
                WHEN "name"  THEN cvarValue = STRING(v-slsmn-hdr,"x(25)") .
                WHEN "cust"     THEN cVarValue = IF AVAIL cust THEN cust.name ELSE ""  .                 
                WHEN "inv-no"   THEN cvarValue = string(w-data.inv-no) .                                        
                WHEN "ino"      THEN cVarValue = w-data.i-no   .                                        
                WHEN "cat"      THEN cVarValue = IF AVAIL itemfg THEN itemfg.procat ELSE ""    .          
                WHEN "qty-shp"  THEN cVarValue = STRING(v-qty[2],"->>>,>>>,>>>")     .              
                WHEN "frt"      THEN cVarValue = STRING(v-frt[2],"->>>,>>>,>>9.99")  .              
                WHEN "sal-amt"  THEN cvarValue = STRING(v-amt[2],"->>>,>>>,>>9.99<<") .             
                WHEN "ful-cst"  THEN cVarValue = IF v-cost2 THEN STRING(v-brdc,"->>>,>>9.99") ELSE ""  .
                WHEN "pro"      THEN cVarValue = IF v-cost2 THEN STRING(v-marg,"->>>,>>9.99") ELSE ""  .
        ...
        /*  display "    DATE TOTALS FOR " + substr(tt-report2.key-02,5,2) + "/" +
          ...
          PUT SKIP str-line SKIP .
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
              cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                   CASE cTmpField: 
                       ...
                       WHEN "frt"      THEN cVarValue = STRING(v-frt[3],"->>>,>>>,>>9.99")  .             
           ...
           PUT UNFORMATTED "    DATE TOTALS FOR " + substr(tt-report2.key-02,5,2) + "/" +
                                           substr(tt-report2.key-02,7,2) + "/" +
                                           substr(tt-report2.key-02,1,4) + substring(cDisplay,31,350) SKIP.
            IF tb_excel THEN DO:
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
              cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                   CASE cTmpField: 
                       ...
                       WHEN "frt"      THEN cVarValue = STRING(v-frt[4],"->>>,>>>,>>9.99")  .             
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
              cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                   CASE cTmpField: 
                       ...
                       WHEN "ino"      THEN cVarValue = "" .
                       WHEN "cat"      THEN cVarValue = "" .
                       WHEN "qty-shp"  THEN cVarValue = STRING(v-qty[5],"->>>,>>>,>>>")   .               
                       WHEN "frt"      THEN cVarValue = STRING(v-frt[5],"->>>,>>>,>>9.99")  .             
