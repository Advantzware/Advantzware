    ...
    FOR EACH ar-cashl
       WHERE ar-cashl.company EQ cocode
        ...
        IF FIRST-OF(ar-cashl.check-no) THEN DO:
           ...
           IF v-s-man EQ "" AND AVAIL cust THEN
              v-s-man  = cust.sman.
        ...
        ELSE
           v-debamt = (ar-cashl.amt-paid - ar-cashl.amt-disc).

        PUT SPACE(1)
            STRING(ar-cashl.inv-no) FORMAT "x(9)"       SPACE(1)
            v-po-no                 FORMAT "x(11)"      SPACE(1)
            v-ord-no                FORMAT "x(8)"       SPACE(1)
            lv-desc                 FORMAT "x(50)"      SPACE(2)
            IF v-creamt NE 0 THEN v-creamt ELSE  v-debamt FORMAT "->>>,>>9.99" 
            SKIP(1).
