...
&Scoped-define QUERY-STRING-Query-Main FOR EACH est WHERE est.company = job.company ~
  ...
  AND ef.eqty = est-qty.eqty NO-LOCK, ~
    ...
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH est WHERE est.company = job.company ~
  AND est.est-no = job.est-no NO-LOCK, ~
      EACH est-qty WHERE est-qty.company = est.company ~
  AND est-qty.est-no = est.est-no NO-LOCK, ~
      EACH ef WHERE ef.company = est-qty.company ~
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "ASI.est WHERE ASI.job <external> ...,ASI.est-qty WHERE ASI.est ...,ASI.ef WHERE ASI.est-qty ... ...,ASI.eb OF ASI.ef"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ",,,"
     _JoinCode[1]      = "ASI.est.company = ASI.job.company
