/* custom/gethost.p 
Calling Program :
    DEF VAR v-hostname AS cha FORM "x(30)" NO-UNDO.
    DEF VAR v-hostip AS cha FORM "x(30)" NO-UNDO.

    RUN custom/gethost.p (OUTPUT v-hostname, OUTPUT v-hostip).
    MESSAGE v-hostname v-hostip VIEW-AS ALERT-BOX.
...
*/
DEF OUTPUT PARAM op-tcpname AS cha NO-UNDO.
DEF OUTPUT PARAM op-tcpip AS cha NO-UNDO.
...
&SCOPED-DEFINE WSADATA_DESCRIPTION 5 /* 
CHAR(WSADESCRIPTION_LEN + 1)
*/
&SCOPED-DEFINE WSADATA_SYSTEM_STATUS 262 /* CHAR(WSASYS_STATUS_LEN 
+ 1)
*/
...
DEF VAR my_tcpname AS CHAR NO-UNDO.
DEF VAR my_tcpipad AS CHAR NO-UNDO.

RUN i-GetTCPInfo(OUTPUT my_tcpname, OUTPUT my_tcpipad).
/*MESSAGE my_tcpname my_tcpipad VIEW-AS ALERT-BOX. */

ASSIGN op-tcpname = my_tcpname
       op-tcpip = my_tcpipad.
