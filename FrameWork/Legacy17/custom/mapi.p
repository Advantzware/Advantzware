...
{custom/windows.i}
/* save current Propath (?) 
DEFINE VARIABLE mpCurDir AS MEMPTR NO-UNDO.
DEF VAR iResult AS INT NO-UNDO.
run GetCurrentDirectoryA (255,mpCurDir, output iResult).
*/

RUN mapi ("Yoosun@Advantzware.com","yoosun_kim@earthlink.net","Mail test","Mail Body",
          "c:\tmp\test.txt").

/* Reset Propath from saved 
run SetCurrentDirectoryA (mpCurDir, output iResult).
*/
...
RUN MAPISendMail IN hpApi
 (INPUT 0,
  ...
  INPUT 0, /*11,  1 = MAPI_LOGON_UI + 2 = MAPI_NEW_SESSION + 8 = MAPI_DIALOG */
  INPUT 0,     
  OUTPUT ResultInt). 
...
PROCEDURE GetCurrentDirectoryA EXTERNAL "kernel32.dll":
   DEFINE INPUT PARAMETER in-hwnd  AS LONG.
   DEFINE INPUT PARAMETER in-state  AS memptr.
   DEFINE RETURN PARAMETER out-check  AS LONG.
END.
