...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE plcParameter    AS LONGCHAR NO-UNDO EXTENT .
  DEFINE VARIABLE plcNamedQuery   AS LONGCHAR NO-UNDO EXTENT .
  ...
  EXTENT (plcParameter) = EXTENT (pcEntity) .
  EXTENT (plcNamedQuery) = EXTENT (pcEntity) .
  ...
  RUN support/proSIretrieve.p
          (pcEntity,
           ...
           INPUT-OUTPUT DATASET-HANDLE hContextDataset).
...
------------------------------------------------------------------------------*/
  ...
  /* we don't loop as we know the adm2 does not submit more than one */
    RUN support/proSIsubmit.p (
        pcEntity[1],
        INPUT-OUTPUT DATASET-HANDLE phDataset[1],
        INPUT-OUTPUT pcContext[1]) NO-ERROR .
...
PROCEDURE stopEntity:
  ...
  RUN stopEntity IN hService (SUBSTRING (phdataset:name,2)).
END PROCEDURE.
...
END PROCEDURE. /* procedure markRows */
