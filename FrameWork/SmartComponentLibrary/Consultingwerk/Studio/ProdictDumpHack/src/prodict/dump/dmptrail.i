...
/*------------------ begin Trailer-INFO ------------------*/
  ...
  PUT {&stream} UNFORMATTED ".":U SKIP.
  
  i64 = SEEK({&seek-stream}).
  
  PUT {&stream} UNFORMATTED "PSC":U SKIP.
  
  {&entries}
  
  PUT {&stream} UNFORMATTED "cpstream=":U 
    ( if user_env[5] = "<internal defaults apply>":U
       then "UNDEFINED":U
       else user_env[5] 
     ) SKIP.
  ...
  ELSE
      PUT {&stream} UNFORMATTED STRING(i64,"9999999999":U) SKIP.
