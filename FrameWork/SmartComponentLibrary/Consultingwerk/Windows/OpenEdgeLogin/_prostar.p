...
IF OPSYS = "VMS":U THEN DO:
    VMS SILENT "@DLC:prostart.com":U.
END.
/* display hello and version for BTOS */
IF OPSYS = "BTOS":U THEN DO:
    DEFINE VARIABLE filename AS CHARACTER.
    filename = SEARCH("hello":U).
    IF filename <> ? THEN
        BTOS OS-COPY VALUE(filename).
    filename = SEARCH("version":U).
    IF filename <> ? THEN
        BTOS OS-COPY VALUE(filename).
    PAUSE 3.
END.
...
IF NUM-DBS > 1 AND savedb = "DICTDB":U THEN DO:
    ...
    BELL.
    MESSAGE
        "You cannot use security if you have more than one database"{&TRAN} SKIP
        "connected and the logical name of one database is DICTDB."{&TRAN}
