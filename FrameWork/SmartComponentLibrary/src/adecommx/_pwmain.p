...
DEFINE OUTPUT PARAMETER poEditorForm AS Consultingwerk.Studio.ProcedureEditor.ProcedureEditorForm NO-UNDO . 
...
DO ON STOP UNDO, LEAVE:
  ...
  */
    ...
    */
    ...
    ELSE DO:
        ...
        ELSE DO:
          ...
          IF NUM-ENTRIES(Proc_Name, CHR(3)) = 4 THEN
            ASSIGN Web_File   = TRUE
                   ...
                   RelPath    = (IF RelPath = ? OR RelPath = "":U THEN Proc_Name ELSE RelPath).
        END.
        ...
        /* Mike Fechner, Consultingwerk Ltd. 18.12.2009
           Launch create window procedure, will embed window before realisation */
        
        RUN adecommx/_pwcreat.p ( INPUT  p_Parent_ID /* Parent ID   */ ,
                                  INPUT  {&PW_Name}  /* Window Name */ ,
                                  INPUT  {&PW_Title_Leader} + 
                                         (IF Web_File THEN RelPath ELSE Proc_Name) /* Title */ +
                                         (IF NOT Web_File THEN "":U ELSE (CHR(3) + 
  ...
  /* Mike Fechner, Consultingwerk Ltd. 19.12.2009
     Truncate MDI Container Tab */    
     
  IF VALID-HANDLE (h_ed) AND  
     VALID-OBJECT (Consultingwerk.Framework.FrameworkSettings:MdiContainer) AND 
     TYPE-OF (Consultingwerk.Framework.FrameworkSettings:MdiContainer, 
              Consultingwerk.Framework.IFormWithUltraTabbedMdiManager) THEN DO:
     
    CAST (Consultingwerk.Framework.FrameworkSettings:MdiContainer, 
          Consultingwerk.Framework.IFormWithUltraTabbedMdiManager):SetMdiTabTextForForm (poEditorForm, 
                                                                                         ENTRY (NUM-ENTRIES(h_ed:NAME, "\"), h_ed:NAME, "\")) .
    CAST (Consultingwerk.Framework.FrameworkSettings:MdiContainer, 
          Consultingwerk.Framework.IFormWithUltraTabbedMdiManager):SetMdiTabToolTipForForm (poEditorForm, 
                                                                                            h_ed:NAME) .
   END.
  /** ******
