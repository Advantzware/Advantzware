...
DEFINE VARIABLE cRibbonButtonHandles    AS CHARACTER                                   NO-UNDO .
DEFINE VARIABLE iButton                 AS INTEGER                                     NO-UNDO .
DEFINE VARIABLE hButton                 AS HANDLE                                      NO-UNDO .
DEFINE VARIABLE cRectangles             AS CHARACTER                                   NO-UNDO .
DEFINE VARIABLE iRectangle              AS INTEGER                                     NO-UNDO .
...
IF VALID-OBJECT (oForm) AND NOT oForm:Finalized THEN DO:
    ...
    IF cRibbonButtonHandles > "":U THEN DO:
        ...
        Consultingwerk.Util.UltraToolbarsHelper:BuildRibbonGroupFromList (oForm:ToolbarsManager,
                                                                         oRibbonTab,
                                                                         THIS-PROCEDURE:FILE-NAME,
                                                                         {&WINDOW-NAME}:TITLE,
                                                                         FALSE, FALSE,
                                                                         cRibbonButtonHandles) .

        ASSIGN cRectangles = Consultingwerk.Util.WidgetHelper:FindRectanglesSurroundingButtonsOnly(FRAME {&FRAME-NAME}:HANDLE) .

        DO iButton = 1 TO NUM-ENTRIES (cRibbonButtonHandles):
            ASSIGN hButton = WIDGET-HANDLE (ENTRY (iButton, cRibbonButtonHandles))
                   hButton:X = 1
                   hButton:Y = 1
                   hButton:WIDTH-PIXELS = 1
                   hButton:HEIGHT-PIXELS = 1 .
        END.

        DO iRectangle = 1 TO NUM-ENTRIES (cRectangles):
            ASSIGN hRectangle = WIDGET-HANDLE (ENTRY (iRectangle, cRectangles)) .

            ASSIGN hRectangle:HIDDEN = TRUE .
        END.
    END.
