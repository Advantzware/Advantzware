...
if search("c:\asigui\build\buildON.txt") ne ? then do:
    message
        "A build is currently running.  This function is not available until the build completes."
        view-as alert-box.
    quit.
end.
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR cGitPropath as char no-undo.
    ...
    ELSE DO:
        ...
        ASSIGN
            ...
            preProPath = cMapDir + "\" + cEnvDir + "\" + cbEnvironment + "," +
                         ...
                         cTop + cEnvResourceDir + "\Addon" + "," +
                         cMapDir + "\" + cAdminDir + "\" + cEnvAdmin + ",".
        IF cbMode:SCREEN-VALUE EQ "GitHub" THEN ASSIGN
            preProPath = cMapDir + "\Repositories\Advantzware\Source," + 
                         cMapDir + "\Repositories\Advantzware\Resources," +
                         cMapDir + "\Repositories\Advantzware\Source\Addon," +
                         preProPath.
        PROPATH = preProPath + origPropath.
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF cbMode:VISIBLE 
    AND cModeItem EQ "GitHub" THEN DO:
        os-command silent value ("c:\asigui\build\getBranchList.bat").
        assign 
            cbEnvironment:list-items = "Devel"
            cbDatabase:list-items = "TESTDEVELd"
            cbEnvironment:screen-value = "Devel"
            cbDatabase:screen-value = "TESTDEVELd".
        RUN dCheckoutBranch.w.
        APPLY 'value-changed' TO cbEnvironment.
