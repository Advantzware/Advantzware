...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fLogDir wWin
ON VALUE-CHANGED OF fLogDir IN FRAME fMain /* Config File Directory */
DO:
    DEF VAR cCmdStr AS CHAR.
    DEF VAR cCopyFrom AS CHAR.
    DEF VAR cCopyTo AS CHAR.
    ASSIGN 
        cFileName = self:screen-value
        cLogDir = substring(cFileName, 1,R-INDEX(cFileName,"\"))
        cThisConfig = substring(cFileName,R-INDEX(cFileName,"\") + 1).
        fLogDir:SCREEN-VALUE = cLogDir.
    DO iCtr = 1 TO NUM-ENTRIES(cLogDir,"\"):
        IF INDEX(ENTRY(iCtr,cLogDir,"\")," ") <> 0 THEN DO:
            REPLACE(ENTRY(iCtr,cLogDir,"\")," ","").
            ASSIGN 
                ENTRY(iCtr,cLogDir,"\") = TRIM(SUBSTRING(ENTRY(iCtr,cLogDir,"\"),1,6)," ") + "~~1"
                fLogDir:SCREEN-VALUE = cLogDir.
    ...
    IF SEARCH(cFileName) <> ? THEN RUN ipUseExisting.
    ASSIGN
        cCopyFrom = REPLACE(fBaseCodeLocn, "Legacy", "Resources")
        cCopyTo = REPLACE(fObjCodeLocn, "Legacy", "Resources").
    APPLY 'choose' TO bProcess.
    OS-COPY VALUE(cCopyFrom) VALUE(cCopyTo).
    APPLY 'choose' TO bQuit.
END.
...
/* ***************************  Main Block  *************************** */
    ASSIGN
        fLogDir:SCREEN-VALUE = SESSION:PARAM.
    APPLY 'value-changed' TO fLogDir.
...
------------------------------------------------------------------------------*/
    ...
    REPEAT:
        ...
        IF SUBSTRING(cFullName,LENGTH(cFullName),1) <> "." AND INDEX(cAttrList,"D") <> 0 THEN DO:
        ...
        THEN DO:
            ...
            */
            ...
            ELSE
            COMPILE value(replace(cFullName,cBaseFromLocn,"")) SAVE into value(cBaseToLocn) NO-ERROR. 
...
------------------------------------------------------------------------------*/
  ...
  DO iCtr3 = 1 TO NUM-ENTRIES(cBaseList):
    ...
    ELSE IF INDEX(cBaseFromLocn,".") <> 0 THEN DO: 
        ...
        COMPILE VALUE(cBaseFromLocn) SAVE INTO VALUE(cBaseToLocn) NO-ERROR.
