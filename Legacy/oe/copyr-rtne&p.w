      ...
      if rm-rctd.rita-code eq "I" then do:  /** ISSUES **/
        if avail job and job.job-no ne "" then do:
          ...
          if item.mat-type eq "B" then
            {rm/rm-addcr.i R xrm-rctd xrm-rctd b-}
            xrm-rctd.rita-code = "ADDER".
          end.
        end.
...
form rm-rctd.rct-date                   label "DATE"
    ...
    for each rm-rctd
      ...
      v-int = 1.
      FOR EACH b-rm-rctd no-lock BY b-rm-rctd.r-no DESC:
        LEAVE.
      END.
      if avail b-rm-rctd then v-int = b-rm-rctd.r-no + 1.
      find last rm-rcpth use-index r-no no-lock no-error.
      if avail rm-rcpth and rm-rcpth.r-no ge v-int then v-int = rm-rcpth.r-no + 1.
