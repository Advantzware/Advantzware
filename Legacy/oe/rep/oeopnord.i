/* ---------------------------------- oe/rep/oeopnord.i from oehots.i ------- */
/* Open Order Report                                                          */
...
/* -------------------------------------------------------------------------- */
   ...
   ASSIGN 
      v-line  = "<C1><FROM><C107><LINE><||3></B>"
      v-line2 = IF rd_lComments THEN "Comments</B>" ELSE "Style Test</B>".
  
   FORM HEADER
      "<FCourier New><B><C5><P26>Hots <P15>(O-Z-7)<P8>" skip(1)
   ...
   ASSIGN 
      ...
      v-sort       = IF rd_sort EQ "Customer#"     THEN "C"  ELSE
                     IF rd_sort EQ "Release Date"  THEN "R"  ELSE "A"
