/* ------------------------------------------------------- oe/rep/invptreefgl3.p */
/* PRINT INVOICE   Xprint form for Peachtree with FG Lot                */
/* Modifications: br 01/11/2011 Task 12081002 - Cloned from invcfgl3.p -
                                                modified  - add print logic     
*/
/* -------------------------------------------------------------------------- */
...
DEFINE VARIABLE vc-part-no AS CHARACTER FORMAT "x(15)" INITIAL "" NO-UNDO.
DEFINE VARIABLE vc-item-no AS CHARACTER FORMAT "x(15)" INITIAL "" NO-UNDO.
...
FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
   FIRST xinv-head WHERE RECID(xinv-head) EQ report.rec-id NO-LOCK
    ...
    DO TRANSACTION:
      ...
      {oe/rep/invptreefgl3.i}  /* xprint form */
...
/*         v-po-no = inv-line.lot-no. */
        ...
        */
        ...
        ASSIGN v-ord-no = inv-line.ord-no
        ...
        /*bpv - seems unnecessary since these are stacked in the second column - removing*/
        /* br task 12081002 */
/*         CASE svi-print-item:                                  */
/*             WHEN 1 THEN ASSIGN vc-item-no = inv-line.i-no     */
/*                                vc-part-no = "".               */
/*             WHEN 2 THEN ASSIGN vc-item-no = ""                */
/*                                vc-part-no = inv-line.part-no. */
/*             OTHERWISE ASSIGN vc-item-no = inv-line.i-no       */
/*                              vc-part-no = inv-line.part-no    */
/*                                                               */
/*         END CASE.                                             */
        /*bpv - 12111202 - removing logic for radio button*/
        ASSIGN vc-item-no = inv-line.i-no
               vc-part-no = inv-line.part-no.
        PUT SPACE(1)
            v-po-no 
            vc-part-no  
    ...
    PUT "<R57><C1><#7><FROM><C+80><LINE>"
        ...
        "<=7><C21>" v-inv-freight FORMAT ">>,>>9.99" "<C30.5>" inv-head.t-inv-tax FORMAT "->>,>>9.99" v-net FORMAT "->,>>>,>>9.99" " " tmp1 "    " tmp2 " " (/*IF v-net2 <> inv-head.t-inv-rev THEN (v-net2 + vld-misc-total) ELSE*/ inv-head.t-inv-rev) FORMAT "->>>>,>>9.99"
