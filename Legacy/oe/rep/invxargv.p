...
ASSIGN ls-image1 = "images\elitelogo.jpg"
       ls-image2 = "images\elitelogo.jpg".
...
for each report where report.term-id eq v-term-id no-lock,
      ...
      IF NOT v-reprint OR xinv-head.inv-no EQ 0 THEN RUN oe/get-inv#.p (ROWID(xinv-head)).
      ...
      DO TRANSACTION:
        ...
        if inv-head.fob-code begins "ORIG" THEN assign v-fob = "Origin".
        ...
        for each xinv-line no-lock where xinv-line.r-no = inv-head.r-no
         ...
         ln-cnt = ln-cnt + 2. 
         IF xinv-line.part-dscr1 ne "" OR xinv-line.part-no <> "" then ln-cnt = ln-cnt + 1.
         IF xinv-line.part-dscr2 NE "" THEN ln-cnt = ln-cnt + 1.
        end. /* each xinv-line */
        for each inv-misc no-lock where inv-misc.company = inv-head.company and
          ...
          if first(inv-misc.ord-no) THEN ln-cnt = ln-cnt + 2.
          ln-cnt = ln-cnt + 1.
        END.
        ...
        /* display heder info 
         view frame invhead-comp.  /* Print headers */
                */
        lv-tot-pg = IF ln-cnt MOD 21 = 0 THEN TRUNC( ln-cnt / 21,0)
                    ELSE lv-tot-pg + TRUNC( ln-cnt / 21,0) .  /* 41->61 24 detail lines */
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
           ...
           */
            assign v-inv-qty = inv-line.qty /* inv-line.inv-qty : Invoied qty */
                   v-ship-qty = inv-line.ship-qty
                   v-bo-qty  = IF AVAIL oe-ordl THEN inv-line.qty - oe-ordl.ship-qty
                               ELSE if ( inv-line.qty - inv-line.ship-qty ) < 0
                               then 0 else inv-line.qty - inv-line.ship-qty
            ...
            do v = 1 to 2:
              ...
              if v-part-info ne "" OR (inv-line.part-no <> "" AND v = 1) then do:
    ...
    PUT "<R58><C58><#8><FROM><R+5><C+22><RECT> " 
        ...
        "<=8><R+3> " /*PST        :" inv-head.t-inv-tax FORM "->>,>>9.99"*/
                    /*v-bot-lab[2] */
        /*"<=8><R+4><C-55>Thank you for your order.*/ "<=8><R+4> Invoice Total:" inv-head.t-inv-rev FORM "->>,>>9.99" .
