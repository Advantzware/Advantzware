...
DEF SHARED VAR v-ship-inst AS LOG NO-UNDO.
def buffer b-itemfg     for itemfg.
DEF BUFFER bf-ttboll FOR tt-boll.
DEF BUFFER bf-w2 FOR w2.
DEF VAR v-tot-case-qty AS INT NO-UNDO.
DEF VAR v-doc-hrs AS CHAR FORMAT "x(10)" NO-UNDO.
DEF VAR v-doc-no AS CHAR FORMAT "x(10)" NO-UNDO.
DEF VAR vuser-id AS CHAR NO-UNDO.

form w2.job-po                          format "x(15)"
     w2.bin                       at 16 format "x(8)"
     w2.i-no                      at 24 format "x(15)"
     w2.dscr                    at 39 format "x(24)"
...
form v-job-po                         format "x(15)"
     w2.bin                       at 16 format "x(8)"
     oe-ordl.i-no                     at 24 format "x(15)"
     v-part-dscr                    at 39 format "x(24)"
     w2.relqty                  AT 63 FORMAT ">>>>>>>>" 
...
for each xxreport where xxreport.term-id eq v-term-id,
    ...
    FIND FIRST oe-relh WHERE oe-relh.company = oe-bolh.company
                              AND oe-relh.release# EQ oe-bolh.release# NO-LOCK NO-ERROR.
    IF AVAIL oe-relh THEN
        ASSIGN vuser-id = oe-relh.USER-ID .
        ELSE vuser-id = "" .
  ...
  PUT "<FArial><R53><C50><#8><FROM><R+4><C+30><RECT> " 
    ...
    "<R50><C1><b>   Dock Hrs: </b>" v-doc-hrs
    "<R50><C13><b>   Dock #: </b>" v-doc-no.
   IF v-ship-inst = YES THEN do: 
   ...
   END.
   PUT "<FArial><R57><C1><P9>"
    "__________________________________________________________________________________________________________________" 
    "<R58><C1>" "<B>  Signature of Receipt </B>" 
    "<R59><C1>" "Customer ________________________________________                       Carrier _______________________________________" AT 20 
    "<R61><C1>" "Date ____________________________________________                       Date _________________________________________" AT 20    
    "<R63><C1>"  " <C49> Verified By ____________________________________ " AT 20
    .
