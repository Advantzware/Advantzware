     ...
     /*   find first oe-relh where oe-relh.company = oe-bolh.company and
      ...
      DO TRANSACTION:
           ...
           do i = 1 to 3:
             if xinv-line.sman[i] ne "" then do:
               ...
               FIND FIRST sman WHERE sman.company = xinv-line.company
                              AND sman.sman = xinv-line.sman[i] NO-LOCK NO-ERROR.
        ...
        for each inv-misc no-lock where inv-misc.company = inv-head.company and
            ...
            ln-cnt = ln-cnt + 1.
            do i = 1 to 3:
             IF inv-misc.s-man[i] ne "" then do:
               create w-sman.
               assign w-sman.sman = inv-misc.s-man[i].
               FIND FIRST sman WHERE sman.company = inv-head.company
                              AND sman.sman = inv-misc.s-man[i] NO-LOCK NO-ERROR.
               w-sman.sname = IF AVAIL sman THEN sman.sname ELSE "".
             end.
           end.
        END.
        ...
        if v-prntinst then do:
          ...
          DO i = 1 TO 4:
            /* IF v-inst[i] <> "" THEN ln-cnt = ln-cnt + 1.*/
              IF inv-head.bill-i[1] <> "" THEN ln-cnt = ln-cnt + 1.
          END.
        end.
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            */
            ...
            assign v-inv-qty = inv-line.qty
                   v-ship-qty = inv-line.inv-qty /*inv-line.ship-qty*/
...
/*
            do v = 1 to 2:
              ...
              if v-part-info ne "" OR inv-line.part-no <> "" then 
...
*/
        ...
        v-first = YES.
        for each inv-misc OF inv-head no-lock where /*inv-misc.company = inv-head.company and
          inv-misc.r-no = inv-head.r-no and */
...
*/         IF v-printline > 51 THEN DO:
        ...
        /* {custom/notesprt.i inv-head v-inst 4}*/
         DO i = 1 TO 4:
              IF inv-head.bill-i[i] /*v-inst[i]*/  <> "" THEN DO:                
                 IF v-printline > 51 THEN DO:
                    PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" .
                    PAGE.
                    v-printline = v-printline - 51.
                    {oe/rep/invxabox.i}
                 END.
                 PUT inv-head.bill-i[i] /*v-inst[i]*/ SKIP.
                 v-printline = v-printline + 1.
              END.
           END.
        end.
