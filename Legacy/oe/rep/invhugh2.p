...
DEF VAR v-date-shipa AS CHAR NO-UNDO.
DEFINE VARIABLE v-misc-ln AS CHARACTER   NO-UNDO.
DEFINE VARIABLE v-item-ln LIKE oe-ordl.e-num     NO-UNDO.
...
DEF VAR v-bol-no AS CHAR NO-UNDO.
      ...
      DO TRANSACTION:
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            ASSIGN v-bol-no = string(oe-boll.bol-no).
          ...
          v-beeler-lines = 0.
            ...
            IF AVAIL oe-ordl THEN
              ASSIGN 
                  v-bo-qty = IF (inv-line.qty - inv-line.ship-qty - oe-ordl.t-ship-qty) < 0 THEN 0 
                             ELSE (inv-line.qty - inv-line.ship-qty - oe-ordl.t-ship-qty)
                  v-ordered-qty = oe-ordl.qty 
                  v-cust-po = oe-ordl.po-no
                  v-item-ln = oe-ordl.e-num.
            ELSE
               ASSIGN 
                  v-bo-qty = IF ( inv-line.qty - inv-line.ship-qty ) < 0 THEN 0 
            ...
            /* gdm - 08050902 */
            IF AVAIL inv-line THEN ASSIGN v-cust-po = inv-line.po-no.            
            ...
            DO:
               ...
               do v = 1 to 2:
                 ...
                 IF v-part-info NE "" OR v-cust-po <> "" THEN DO:
                   IF v = 1 AND v-cust-po <> "" THEN
                     put space(9) v-bol-no SPACE(8) v-cust-po format "x(15)" SPACE(1) v-part-info SKIP.
                   ELSE
        ...
        for each inv-misc no-lock where inv-misc.company = inv-head.company and
         ...
         /* put inv-misc.charge AT 10 inv-misc.dscr inv-misc.amt v-misc-ln AT 66 SKIP
            /* gdm - 08050903 */
              SPACE(29) inv-misc.po-no SKIP .*/  /* task 07221402 */
         PUT "<C1>" "      0       0"  "<C35>" inv-misc.charge "<C56>" inv-misc.amt "<C71>" inv-misc.amt SKIP
              "<C35>" inv-misc.dscr  v-misc-ln  SKIP
            /* gdm - 08050903 */
             "<C35>" inv-misc.po-no SKIP  .
