...
def VAR po-no as CHAR format "x(12)" no-undo.
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      DO TRANSACTION:
         ...
         do i = 1 to 3:
          if xinv-line.sman[i] ne "" then do:
            ...
            assign w-sman.sman = xinv-line.sman[i]
                   w-sman.sname = xinv-line.sname[i].
          end.
         end.
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
          ...
          do v-count = 1 to 2:
            v-part-info = if v-count eq 1 then inv-line.part-dscr1
