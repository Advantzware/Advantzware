...
DEF VAR v-ans-no AS INT INIT 0 NO-UNDO.
...
for each xxreport where xxreport.term-id eq v-term-id,
  ...
  if first-of(oe-bolh.bol-no) then do:
   ...
   IF AVAIL sys-ctrl AND NOT v-printed THEN DO:
    find first sys-ctrl-shipto where sys-ctrl-shipto.company eq cocode
                      and sys-ctrl-shipto.cust-vend-no   eq oe-bolh.cust-no EXCLUSIVE-LOCK no-error.
    IF AVAIL sys-ctrl-shipto THEN
        ASSIGN v-ans-no = sys-ctrl-shipto.dec-fld .

    ASSIGN oe-bolh.spare-int-5 = v-ans-no .
   END.
...
PUT "<FBook Antiqua><R52><C1><P12><B>     Shipping Instructions: </B> <P9> " SKIP(1)
 ...
 IF AVAIL sys-ctrl-shipto AND NOT v-printed THEN do:
     ASSIGN v-ans-no = v-ans-no + 1 
     sys-ctrl-shipto.dec-fld = v-ans-no .
 END.
