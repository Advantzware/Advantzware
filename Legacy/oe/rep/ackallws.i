/* ------------------------------------------- oe/rep/ackallws.i GDM 04200907*/
...
PUT "<=1><R+10><P10>"  SKIP(1)
    "Bill To:" AT 6           SKIP /*SPACE(57) "Sold To:"  */ 
    SPACE(5) oe-ord.cust-name SKIP /*( IF oe-ord.sold-name = "" THEN oe-ord.cust-name ELSE oe-ord.sold-name) AT 71*/  
    SPACE(5) oe-ord.addr[1]   SKIP /*(IF oe-ord.sold-addr[1] = "" THEN oe-ord.addr[1] ELSE oe-ord.sold-addr[1]) AT 71*/ 
    SPACE(5) oe-ord.addr[2]   SKIP /*(IF oe-ord.sold-addr[2] = "" THEN oe-ord.addr[2] ELSE oe-ord.sold-addr[2]) AT 71 */ 
    SPACE(5) v-addr3          SKIP /*v-sold-addr3 AT 71*/  
    .
...
/*     "<R19><C72><FROM><R23><C72><LINE>"         SKIP */
    .
PUT "<=4><R+1> Ready Date         FOB                       Ship Via               Terms                  Sales Person         " SKIP
    "<=4><R+3> " 
    oe-ord.due-date FORMAT "99/99/9999" SPACE(4)
    cust.fob-code   FORMAT "x(14)"      SPACE(2)
    v-shipvia       FORMAT "x(30)"      SPACE(9)
    oe-ord.terms-d  FORMAT "x(15)"      SPACE(3) 
    v-salesman      FORMAT "x(20)" 
    SKIP.
...
PUT "<=5>    Customer" "<C40>" "Order#"
    "<R+1><C1>   Part#/PO#     Status        Description               Quote#     Ordered      Unit Price  UOM    Ext. Price" SKIP(1).
