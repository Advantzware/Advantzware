/* ---------------------------------------------- oe/rep/expconts.p   */
/* Export INVOICE   for Container Services          */
/* -------------------------------------------------------------------------- */
...
for each tt-report NO-LOCK where tt-report.term-id eq "",
        ...
        FIRST cust WHERE cust.company = inv-head.company
                     ...
                     AND NOT CAN-FIND(FIRST sys-ctrl-shipto WHERE
      ...
      PUT v-client-num  /* client number 1-4 */
         ...
         for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
             ...
             FIND FIRST oe-ordl WHERE oe-ordl.company = inv-line.company
             ...
             PUT v-client-num
                 ...
                 inv-head.bol-no FORM ">>>>>>>9"
                 inv-line.t-price FORM "->>>>>>>9.99"
                 inv-line.po-no FORM "x(15)"
                 v-po-line FORM ">>9"
                 SKIP.
         END.
         v-misc-amt = 0.
