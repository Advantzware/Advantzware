...
def var v-phone-num         as   char format "x(14)" no-undo.
...
DEF VAR v-frt-class-dscr1 AS cha NO-UNDO.
DEF VAR v-frt-class-dscr2 AS cha NO-UNDO.
...
for each xxreport where xxreport.term-id eq v-term-id,
    ...
    assign
     ...
     v-ship-addr3   = shipto.ship-city + ", " +
                      shipto.ship-state + "  " +
                      shipto.ship-zip
     v-phone-num    = "(" + shipto.area-code + ")" + " " + string(shipto.phone,"xxx-xxxx").
    ...
    FOR EACH oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no NO-LOCK,
      ...
      v-frt-class = IF AVAIL itemfg THEN itemfg.frt-class ELSE "".
      LEAVE.
    end.
  ...
  if last-of(oe-bolh.bol-no) then do:
     ...
     /* end of dup loop */
      ...
      ASSIGN v-frt-class-dscr1 = ""
             v-frt-class-dscr2 = "".
             
      IF v-frt-class = "" OR v-frt-class = "1" THEN
            ASSIGN v-frt-class-dscr1 = "Corrugated Boxes"
                   v-frt-class-dscr2 = "D29275 Sub-277.5".
