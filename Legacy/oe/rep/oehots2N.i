...
IF v-first THEN 
   /*IF tb_exp-po THEN
      PUT UNFORMATTED
         ...
         (IF rd_lComments THEN "Comment" ELSE "Style Test") */
       str-tit4 "</B>" SKIP
         v-line SKIP.
   ELSE */
      PUT UNFORMATTED
...
IF AVAIL itemfg THEN DO:
   ...
   IF AVAIL job-hdr THEN
      ...
      lc-style-xl = "" .
         ...
         IF lc-test-xl EQ "" AND itemfg.est-no NE "" THEN
            FOR EACH eb fields(test style) WHERE 
                eb.company  EQ cocode AND
   ...
   END.
    ...
    BUFFER bw-ord:FIND-BY-ROWID(ROWID(w-ord), NO-LOCK) .
       ...
       DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
         ...
         ELSE DO:       
            CASE cTmpField:                               
                 WHEN "v-ord-no-text" THEN cVarValue = string(v-ord-no-text).
                 WHEN "lc-style-xl" THEN cVarValue = STRING(lc-style-xl).
                 WHEN "lc-test-xl" THEN cVarValue = STRING(lc-test-xl).
                 WHEN "v-job-qty" THEN  cVarValue = STRING(w-ord.job-qty,">>>>>>9") .
                 WHEN "lv-vend-no" THEN cVarValue = string(lv-vend-no).
                 WHEN "lv-board-po-no" THEN cVarValue = STRING(lv-board-po-no,">>>>>>9").
                 WHEN "ld-qty-rec" THEN cVarValue = STRING(ld-qty-rec,"->>>>>9").
                 WHEN "lv-routing" THEN cVarValue = SUBstring(lv-routing,1,15).
