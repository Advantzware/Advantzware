...
DEF VAR v-shpfrm AS CHAR NO-UNDO.
...
DEF VAR v-cases-tot AS DEC NO-UNDO.
DEF VAR v-qtycas-tot AS DEC NO-UNDO.
DEF VAR v-partial-tot AS DEC NO-UNDO.
DEF VAR v-total-count-tot AS DEC NO-UNDO.
DEF VAR v-weight-tot AS DEC NO-UNDO.
DEF VAR inrowcount2 AS INT NO-UNDO .
...
ASSIGN chFile = CurrDir + "\Template\bol-pu.xlt" no-error.
...
for each xxreport where xxreport.term-id eq v-term-id,
    ...
    IF AVAIL oe-bolh THEN
        ASSIGN v-terms = oe-bolh.trailer .
    ELSE
        ASSIGN v-terms = "" .
    
    IF AVAIL oe-bolh THEN DO:
        FIND FIRST oe-relh WHERE oe-relh.company EQ cocode
          AND oe-relh.release# EQ oe-bolh.release# NO-LOCK NO-ERROR .

      IF AVAIL oe-relh THEN
          ASSIGN v-rel-date = string(oe-relh.rel-date) .
    END.
    ...
    FOR EACH oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no NO-LOCK,
      ...
      /*  v-shipcontact = oe-ord.contact*/
        v-ord-date = oe-boll.bol-date /*oe-ord.ord-date*/
        v-shpfrm = oe-boll.loc .
 ...
 ASSIGN v-cases-tot = 0
  ...
  for each oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no:
      ...
      FOR EACH itemfg where itemfg.company eq oe-boll.company
                      and itemfg.i-no    eq oe-boll.i-no NO-LOCK:
            IF itemfg.frt-class NE "" THEN do:
                ASSIGN v-shipcontact =  itemfg.frt-class  .
                LEAVE .
            END.
        END. /*FOR EACH itemfg*/
  ...
  if last-of(oe-bolh.bol-no) then do:
    ...
    /*  end of getting total page per po */
     ...
     {oe/rep/bolpry22-xl.i}
     {oe/rep/bolpryt2-xl.i}
    
     ASSIGN inrowcount = inrowcount + 2 . 
    /* Ship Notes */  
    ASSIGN v-cell = "R" + string(inrowcount) + "C19".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = oe-bolh.ship-i[1] .  

    ASSIGN inrowcount = inrowcount + 1 .
    ASSIGN v-cell = "R" + string(inrowcount) + "C19".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = oe-bolh.ship-i[2] .  
    
    ASSIGN inrowcount = inrowcount + 1 .
    ASSIGN v-cell = "R" + string(inrowcount) + "C19".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = oe-bolh.ship-i[3] . 

    ASSIGN inrowcount = inrowcount + 1 .
    ASSIGN v-cell = "R" + string(inrowcount) + "C19".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = oe-bolh.ship-i[4] . 

     RUN util/GetBottomRow.p (INPUT 14, INPUT 30, INPUT 3, INPUT inrowcount, OUTPUT inrowcount).
     
     ASSIGN v-cell = STRING(inrowcount) + ":350". 
    ...
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = v-cases-tot.  
        
    /* Unit Count */  
    ASSIGN v-cell = "R" + string(inrowcount) + "C40".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = v-qtycas-tot .  
          
    /* ODD Unit */  
    ASSIGN v-cell = "R" + string(inrowcount) + "C42".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = v-partial-tot.  
          
    /* Total Count */  
    ASSIGN v-cell = "R" + string(inrowcount) + "C45".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = v-total-count-tot.  
          
    /* Item Weight */  
    ASSIGN v-cell = "R" + string(inrowcount) + "C50".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = v-weight-tot. 
  ...
  /*   ASSIGN v-cell = STRING(inrowcount) + ":350". 
     ...
     DO:
       ...
       NO-RETURN-VALUE chWorkbook:ExportAsFixedFormat(0, v-dir + "bol.pdf").
       /*chExcelApplication:ActiveWindow:SelectedSheets:PrintOut().*/
       chWorkbook:Close(no) no-error.   
       ...
       OS-RENAME VALUE(v-dir + "asi.pdf") VALUE(v-dir +  "bol.pdf").
       
       ASSIGN LvCtr = LvCtr + 1.
       CREATE tt-filelist.
       ASSIGN tt-FileCtr  = LvCtr
              tt-FileName = v-dir +  "bol.pdf".
     END.
