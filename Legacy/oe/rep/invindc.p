...
DEF VAR v-cases-ship LIKE oe-boll.cases NO-UNDO.
DEF VAR v-cases-order LIKE oe-boll.cases NO-UNDO.
...
DEF VAR ll-firstp AS LOG NO-UNDO.
      ...
      DO TRANSACTION:
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
          ...
          FOR EACH oe-boll NO-LOCK
            ...
            if oe-boll.partial ne 0 THEN assign v-part-line = "1" + " @ " + string(oe-boll.partial).
            else assign v-part-line = "".
            v-cases-ship = v-cases-ship + oe-boll.cases.
            /*v-cases-order = oe-boll.qty-case.*/
            do i = 1 to 5:
            ...
            FOR EACH oe-rell WHERE oe-rell.company  EQ oe-boll.company
              ...
              v-cases-order = v-cases-order + oe-rell.cases.
            ...
            assign v-inv-qty = v-rel-qty /* inv-line.inv-qty : Invoied qty */
                   ...
                   /*v-cases-order = inv-line.qty / v-cases-order*/.
            ...
            PUT space(1) v-inv-qty format "->>,>>9" SPACE(1)
                ...
                inv-line.ord-no FORM ">>>>>>9" SPACE(1)
...
======== */
  ...
  */
    ...
    PUT "<R58><C58><#8><FROM><R+4><C+22><RECT> " 
        "<=8> Sub Total    :" v-subtot-lines FORM "->>,>>9.99"
        "<=8><R+1> Freight      :" v-inv-freight
 /*       "<=8><R+2> Sales Tax    :" /* v-bot-lab[1] */
                           inv-head.t-inv-tax FORM "->>,>>9.99"  */
        "<=8><R+2> " /*PST        :" inv-head.t-inv-tax FORM "->>,>>9.99"*/
                    /*v-bot-lab[2] */
        "<=8><R+3> Invoice Total:" inv-head.t-inv-rev FORM "->>,>>9.99" .

  /*  PUT "<R58><C1><#9><FROM><R+3><C+30><RECT> " 
        "<=9><C2> Remit To: Elite Packaging"
        "<=9><R+1><C10> Caller Service 105100"
        "<=9><R+2><C10> Tucker, GA 30085-5100".
  */
