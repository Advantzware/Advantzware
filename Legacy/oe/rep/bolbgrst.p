/* ------------------------------------------------------------------------- *
*    Program Name  : oe/rep/bolbgrst.p                                       *
*     Author       :                                                         *
*     Purpose      : BOL FORM for BadgerShipto                               *
*     Date         : 09/15/14                                                *
*     Modify By    :                                                         *
*                                                                            *       
* -------------------------------------------------------------------------- */
...
def var v-frt-terms as char format "x(14)" no-undo.
...
DO:
   ...
   DO:
      ASSIGN
         v-ship-i2[1] = shipto.notes[1]
         v-ship-i2[2] = shipto.notes[2].
...
for each xxreport where xxreport.term-id eq v-term-id,
    ...
    FIND FIRST oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no NO-LOCK NO-ERROR.
       IF AVAIL oe-boll THEN DO:
          ...
          IF AVAIL oe-ord THEN do:
             ASSIGN v-comp-name   = oe-ord.sold-name
    ...
    FOR EACH oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no NO-LOCK,
      ...
      DO:
         ...
         DO:
            v-class-msg = "***CLASS " + itemfg.frt-class + " " + itemfg.frt-class-dscr + "***".
            RELEASE itemfg.
         END.
      END.
      ...
      assign v-terms = oe-ord.terms-d
             v-frt-terms = if oe-bolh.frt-pay eq "P" then "Prepaid"
                           else if oe-bolh.frt-pay eq "B" then "Prepaid & Add"
  ...
  IF v-ship-i2[1] <> "" THEN
     PUT "<FArial><R47><C3.3><P12><B>" v-ship-i2[1]
         "<FArial><R48><C3.3><P12><B>" v-ship-i2[2]
         "<FArial><R49><C3.3><P12><B>" v-class-msg.
  ...
  PUT "<FArial><R64><C1><P8>FORM -0043 9/01/09".
