 ...
 */
 ...
 END.
  ...
  FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
      ...
      ASSIGN   lv-prt-sts = IF ip-prt-revised THEN "<FGCOLOR=RED>REVISED<FGCOLOR=BLACK>" 
...
=======*/
      for each oe-ordl
        ...
        put v-line FORM ">>>9" SPACE(3)
                ...
                oe-ordl.i-name SPACE(3)
                oe-ordl.qty FORM "->,>>>,>>9" SPACE(1)
      ...
      */
        ...
        IF v-prntinst THEN DO:
          find first itemfg {sys/look/itemfgrlW.i}
           ...
           IF AVAIL itemfg THEN DO:
               ...
               RUN get-notes (itemfg.rec_key,spec-list,1).
       ...
       IF v-print-pen-notes THEN
      RUN get-notes (oe-ordl.rec_key,v-dept-codes,2).
...
PROCEDURE print-rels:
    ...
    DO WITH FRAME sched-rel DOWN:
          ...
          put lcnt AT 10 space(1) (IF oe-rel.link-no EQ 0 THEN oe-rel.tot-qty  ELSE oe-rel.qty ) FORM "->,>>>,>>9" /*FORM "->,>>>,>>9.99<<<<"*/  space(5) ld-date  SKIP.
    ...
    ASSIGN v-text = "".    
       ...
       FOR EACH notes NO-LOCK 
         WHERE notes.rec_key EQ ip-rec-key
           AND LOOKUP(notes.note_code, TRIM(ip-codes)) <> 0:
         IF notes.note_text <> "" THEN do:
          ASSIGN v-text = v-text + " " + 
                          "Time/Date" + "<c20>  " + "User" +  CHR(10).
         ASSIGN v-text = v-text + " " + 
                          STRING(notes.note_time,"hh:mm am") + "-" + STRING(notes.note_date) + "<c20>   " + STRING(notes.USER_id) + CHR(10).
         IF ip-skip = 1 AND SUBSTRING(notes.note_text,1,6) = "change"  THEN
             ASSIGN v-text = v-text + " " + "<FGCOLOR=RED>" + 
                          TRIM(notes.note_text) + "<FGCOLOR=BLACK>" +  CHR(10).
         ELSE  ASSIGN v-text = v-text + " " + 
