...
/*         RUN cXMLOutput (clXMLOutput,'/Extrinsic','','Row').                               */
    ...
    PUT "<R58><C54><#8><FROM><R+5><C+26><RECT> " 
        ...
        "<=8><R+1> Carga              :" v-inv-freight
        "<=8><R+2> Impuesto de venta  :" inv-head.t-inv-tax FORM "->>,>>9.99"
        "<=8><R+3>" "" 
        "<=8><R+4> Total de la factura:" inv-head.t-inv-rev FORM "->>,>>9.99" .

    PUT "<FArial><R58><C1><P12><B> Gracias. </B> <P9> " SKIP.
    PUT "<FArial><R60><C1><P12> Todas las monedas mostradas en " cCurCode FORMAT "x(3)" ". <P9> " SKIP.
