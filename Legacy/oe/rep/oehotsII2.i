...
IF v-first THEN 
    PUT UNFORMATTED
        ...
        "Begin Date:" AT 30 
        STRING(begin_date) FORM "x(5)" 
        "  End date:" STRING(end_date) FORM "x(5)" 
        "Page: " AT 145 PAGE-NUM FORM ">>9"
        SKIP
        "Qty on Hnd Customer Name                  Item Code       Description                 Cat   PO #            Order #        Sales     Rel Qty Rel Date   Rel #"
        "</B>"
        SKIP.
         ...
         FOR EACH tt-fg-set
            ...
            IF LAST-OF(tt-fg-set.QtyPerSet) THEN
               FOR EACH po-ordl WHERE 
                    ...
                    AND po-ordl.s-num     EQ INTEGER(tt-fg-set.part-qty)
