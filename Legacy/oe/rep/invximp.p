/* ---------------------------------------------- oe/rep/invximp.p  */
/* PRINT INVOICE   Xprint for Imperail             */
/* -------------------------------------------------------------------------- */
...
ASSIGN ls-image1 = "images\imperial.jpg"
       ls-image2 = "".
        ...
        for each xinv-line no-lock where xinv-line.r-no = inv-head.r-no
         ...
         FOR EACH oe-bolh NO-LOCK WHERE oe-bolh.b-no = xinv-line.b-no /*AND
           ...
           FOR EACH oe-boll NO-LOCK WHERE oe-boll.company = oe-bolh.company AND
                ...
                /* it's a # of pkg not total pallets
...
*/         IF v-printline > 53 THEN DO:
        ...
        if v-prntinst then do:
         ...
         DO i = 1 TO 4:
           ...
           END.
           */
           IF v-printline > 46 AND inv-head.bill-i[1] <> "" THEN DO:
              PAGE.
              v-printline = 0.
              {oe/rep/invximp.i}
           END.
           PUT "<R54><C1>" inv-head.bill-i[1] FORM "x(60)" 
               "<R55><C1>" inv-head.bill-i[2] FORM "x(60)" 
               "<R56><C1>" inv-head.bill-i[3] FORM "x(60)" 
               "<R57><C1>" inv-head.bill-i[4] FORM "x(60)" .
        end.
        /*
        IF v-printline > 50 THEN DO:
                    PAGE.
  ...
  */
    ...
    PUT "<R58><C58><#8><FROM><R+5><C+22><RECT> " 
        "<=8><C3><P12>Thank You For Your Business <P10><=8> Sub Total    :" v-subtot-lines FORM "$->>,>>9.99"
        "<=8><R+1> Freight      :" v-inv-freight FORM "$->>,>>9.99"
        "<=8><R+2> " v-bot-lab[1] 
        "<=8><R+3> " /*PST        :" inv-head.t-inv-tax FORM "->>,>>9.99"*/
                    v-bot-lab[2]
        "<=8><R+4><B> Invoice Total:" inv-head.t-inv-rev FORM "$->>,>>9.99" "<B>".
