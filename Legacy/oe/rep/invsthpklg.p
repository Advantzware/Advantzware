/* ---------------------------------------------- oe/rep/invsthpklg.p */
/* PRINT INVOICE   Xprint form for South Pak logistics             */
/* -------------------------------------------------------------------------- */
...
for each report where report.term-id eq v-term-id no-lock,
    ...
    END.
      ...
      FIND FIRST b-cust WHERE
           b-cust.company = xinv-head.company AND
           b-cust.cust-no = xinv-head.cust-no  
           NO-LOCK NO-ERROR.

      IF ip-multi-faxout AND FIRST-OF(report.key-01) AND AVAIL b-cust THEN DO:
         OUTPUT CLOSE.
         OUTPUT STREAM st-fax CLOSE.
         OUTPUT TO value("c:\temp\fax\fx" + b-cust.cust-no + ".xpr") PAGE-SIZE value(ip-lines-per-page).
         OUTPUT STREAM st-fax TO value("c:\temp\fax\fx" + b-cust.cust-no + ".txt").
         PUT STREAM st-fax UNFORMATTED "FAX#:" trim(b-cust.fax-prefix) b-cust.fax SKIP.
         PUT CONTROL "<PRINT=NO>".       
         PUT UNFORMATTED "<EXPORT=c:\temp\fax\fx" trim(b-cust.cust-no) ".tif,BW>" .
      END.
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no
          ...
          FOR EACH oe-boll
            ...
            PUT space(1) v-inv-qty format "->>>>>9" SPACE(1)
                inv-line.ship-qty  format "->>>>>9" SPACE(1)
