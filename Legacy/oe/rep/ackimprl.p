...
{oe/rep/ackimprl.i}
      ...
      for each oe-ordl
        ...
        put v-line FORM ">>>9" SPACE(2)
            ...
            oe-ordl.t-price FORM "$>>>>,>>9.99"  SKIP
            .
       ...
       if oe-ordl.i-no ne oe-ordl.part-no or
        ...
        if oe-ordl.part-dscr2 ne "" then do:
          put oe-ordl.part-dscr2 at 23  skip.
          v-printline = v-printline + 1.
        end.
        if v-printline ge lv-line-print then
        do:
      ...
      */
      ...
      PUT "<FArial><R55><C1><#10><P12><B> Comments </B> <P8> " 
        ...
        "<=10><R-2>" SPACE(32) "<P9><B>THIS IS A CONFIRMATION OF YOUR ORDER, NOT AN INVOICE.</B><C62>Total Order Value " v-totord FORM "$>>>>,>>9.99"    .
...
PROCEDURE print-rels:
    ...
    DO WITH FRAME sched-rel DOWN:
          ...
          put lcnt AT 10 (IF oe-rel.link-no EQ 0 THEN oe-rel.tot-qty ELSE oe-rel.qty ) FORM "->,>>>,>>9" space(5) ld-date  SKIP.
