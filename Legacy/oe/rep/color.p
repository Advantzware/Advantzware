...
{oe/rep/invoice.i}
{custom/notesdef.i}
DEF VAR v-inst AS cha FORM "x(78)" EXTENT 4 NO-UNDO.
...
form header
  ...
  /*inv-head.t-inv-weight*/ 0 to 44 format ">>>>9.99"
      ...
      DO TRANSACTION:
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
                        ...
                        and oe-boll.i-no = inv-line.i-no use-index b-no:
            ...
            if v-printline GE 28 THEN do:
              put skip(29 + 3 + v-beeler-lines - v-printline)
                  "* CONTINUED *" at 68.
        ...
        for each inv-misc of inv-head no-lock
            where inv-misc.bill eq "Y"
            break by ord-no:

          assign
           v-beeler-lines = IF FIRST(inv-misc.ord-no) THEN 2 ELSE 0
           v-printline    = v-printline + 2 + v-beeler-lines.

          if v-printline GE 28 then do:
            put skip(29 + 2 + v-beeler-lines - v-printline)
                "* CONTINUED *" at 68 .
        ...
        if v-prntinst then do:
          lv-text = "".     
          FOR EACH notes WHERE notes.rec_key EQ inv-head.rec_key NO-LOCK:
          ...
          FOR EACH tt-formtext:
            li = li + 1.
            IF li GE 4 THEN DELETE tt-formtext.
          END.
          IF li GE 4 THEN li = 4.
          v-printline = v-printline + li.

          IF v-printline GE 28 then do:
            PUT SKIP(28 + li - v-printline)
                "* CONTINUED *" AT 68.
            v-printline = li.
            PAGE.
          END.
          ...
          FOR EACH tt-formtext:
            li = li + 1.
            IF li EQ 1 THEN PUT "            " AT 5 tt-text FORM "x(65)" SKIP.
