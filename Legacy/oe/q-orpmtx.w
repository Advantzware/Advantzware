...
def temp-table tt-prmtx like oe-prmtx.
...
&Scoped-define INTERNAL-TABLES tt-prmtx
...
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY {&SELF-NAME} FOR each tt-prmtx     ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main tt-prmtx
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main tt-prmtx
...
/* Query rebuild information for QUERY Query-Main
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR each tt-prmtx
    ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE find-prmtx q-tables 
PROCEDURE find-prmtx :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
def var v-i-item as cha no-undo.
def var cocode as cha no-undo.
def var char-hdl as cha no-undo.
def var lh-prmtx as handle no-undo.

message "find 1" view-as alert-box.

assign cocode = oe-ordl.company
       v-i-item = oe-ordl.i-no.
       
find oe-ord of oe-ordl no-lock no-error.
find cust where cust.company = oe-ord.company and
                cust.cust-no = oe-ord.cust-no no-lock no-error.
...
find first oe-prmtx  {oe/oe-prmtx.w}
  ...
  if avail oe-prmtx then do:
     create tt-prmtx.
     buffer-copy oe-prmtx to tt-prmtx.
message "find avail " view-as alert-box.

     run get-link-handle in adm-broker-hdl(this-procedure,"container-source",output char-hdl).
     run add-prmtx-link in widget-handle(char-hdl) (output lh-prmtx).
     run dispatch in lh-prmtx ('row-avalable').
     
  end.
  message "find end " avail oe-prmtx view-as alert-box.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tt-prmtx"}
