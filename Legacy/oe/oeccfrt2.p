/***************************************************************************\
*****************************************************************************
**  Program: oeccfrt2.p  copied from oeccfrt.p and added for pallet & MSF calc
**       By:
** Descript:
**
*****************************************************************************
...
DEF INPUT PARAM ws_pallet AS dec NO-UNDO.
DEF INPUT PARAM ws_msf AS DEC NO-UNDO.
...
DO:
  ...
  IF AVAIL carr-mtx THEN DO:
    case carrier.chg-method:
        WHEN "W" THEN DO:
           DO i = 1 TO 10:
           if carr-mtx.weight[i] gt 0 AND carr-mtx.weight[i] GE ws_weight then leave.
           END.
           if i ge 1 and i le 10 then ws_cwt = carr-mtx.rate[i].
           IF ROUND(ws_weight / 100 * ws_cwt, 2) <=
              carr-mtx.min-rate AND ws_amt <= carr-mtx.min-rate 
           THEN ASSIGN ws_amt = carr-mtx.min-rate ws_minimum = true.
           ELSE IF ws_weight NE 0 AND ws_cwt NE 0 THEN
                ASSIGN ws_amt = ROUND(ws_weight / 100 * ws_cwt, 2).
           else assign ws_amt = 0.
           assign ws_recid = recid(carr-mtx).
        END.
        WHEN "P" THEN DO:
           DO i = 1 TO 10:
           if carr-mtx.weight[i] gt 0 AND carr-mtx.weight[i] GE ws_pallet then leave.
           END.
           if i ge 1 and i le 10 then ws_cwt = carr-mtx.rate[i].
           ws_amt = ws_pallet * ws_cwt. 
           IF ws_amt < carr-mtx.min-rate THEN
                   ASSIGN ws_amt = carr-mtx.min-rate
                          ws_minimum = YES.
           assign ws_recid = recid(carr-mtx).           

        END.
        WHEN "M" THEN DO:   /* ??? not done */
           DO i = 1 TO 10:
           if carr-mtx.weight[i] gt 0 AND carr-mtx.weight[i] GE ws_msf then leave.
           END.
           if i ge 1 and i le 10 then ws_cwt = carr-mtx.rate[i].
           ws_amt = ws_msf * ws_cwt.
           IF ws_amt < carr-mtx.min-rate THEN
                   ASSIGN ws_amt = carr-mtx.min-rate
                          ws_minimum = YES.
