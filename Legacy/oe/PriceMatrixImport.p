...
DEF TEMP-TABLE tt-oe-prmtx LIKE oe-prmtx
...
DEF VAR cExcelFile AS CHAR NO-UNDO.
...
FOR EACH tt-oe-prmtx:
    ...
    FIND FIRST bf-custype 
        ...
        NO-LOCK NO-ERROR.
    IF NOT AVAILABLE(bf-custype) THEN DO:
        tt-oe-prmtx.valid = FALSE.
    ...
    IF tt-oe-prmtx.valid = TRUE THEN DO:
        ...
        IF NOT AVAIL bf-oe-prmtx THEN DO:
            PUT STREAM log-file UNFORMATTED "Valid record - Adding " 
        ...
        ELSE DO:
            PUT STREAM log-file UNFORMATTED "Valid record - Updating (" 
                " EffDate: " STRING(dtEffDate)  
                " Cust:" TRIM(tt-oe-prmtx.cust-no)
                " CustType:" trim(tt-oe-prmtx.custype) 
                " ProdCat:" trim(tt-oe-prmtx.procat) 
