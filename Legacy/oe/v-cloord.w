...
oe-ordl.price oe-ordl.pr-uom oe-ordl.cost oe-ordl.disc oe-ordl.t-price ~
oe-ordl.po-no oe-ordl.po-no-po oe-ordl.vend-no oe-ordl.req-code ~
oe-ordl.req-date oe-ordl.prom-code oe-ordl.prom-date oe-ordl.job-no ~
oe-ordl.job-no2 oe-ordl.tax oe-ordl.s-man[1] oe-ordl.s-pct[1] ~
oe-ordl.s-comm[1] oe-ordl.s-man[2] oe-ordl.s-pct[2] oe-ordl.s-comm[2] ~
oe-ordl.s-man[3] oe-ordl.s-pct[3] oe-ordl.s-comm[3] oe-ordl.cas-cnt ~
oe-ordl.partial oe-ordl.cases-unit 
...
DEFINE FRAME F-Main
     oe-ordl.est-no AT ROW 1.24 COL 17 COLON-ALIGNED FORMAT "x(8)"
          VIEW-AS FILL-IN 
          SIZE 16 BY 1
     oe-ordl.qty AT ROW 2.24 COL 17 COLON-ALIGNED
     ...
     oe-ordl.t-price AT ROW 5.05 COL 76 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 18 BY 1
     oe-ordl.po-no AT ROW 8.38 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 20 BY 1
     oe-ordl.po-no-po AT ROW 9.33 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 14 BY 1
     oe-ordl.vend-no AT ROW 9.33 COL 51 COLON-ALIGNED
          LABEL "Board Vendor"
          VIEW-AS FILL-IN 
          SIZE 11.6 BY 1
     oe-ordl.req-code AT ROW 8.14 COL 76 COLON-ALIGNED
          LABEL "Priority"
          VIEW-AS FILL-IN 
          SIZE 6 BY 1
     oe-ordl.req-date AT ROW 8.14 COL 107 COLON-ALIGNED
          LABEL "Due Date"
          VIEW-AS FILL-IN 
          SIZE 18 BY 1
     oe-ordl.prom-code AT ROW 9.33 COL 76 COLON-ALIGNED
          LABEL "Priority"
          VIEW-AS FILL-IN 
          SIZE 6 BY 1
     oe-ordl.prom-date AT ROW 9.33 COL 107 COLON-ALIGNED
...
DEFINE FRAME F-Main
     ...
     oe-ordl.cas-cnt AT ROW 4.1 COL 114 COLON-ALIGNED
          LABEL "Qty/Case" FORMAT ">>>,>>>"
          VIEW-AS FILL-IN 
          SIZE 13 BY 1
     oe-ordl.partial AT ROW 5.05 COL 114 COLON-ALIGNED
          LABEL "Partial" FORMAT "->>>,>>>,>>9"
          VIEW-AS FILL-IN 
          SIZE 13 BY 1
     oe-ordl.cases-unit AT ROW 6 COL 114 COLON-ALIGNED
          LABEL "Cases/Pallet" FORMAT ">>>>"
          VIEW-AS FILL-IN 
          SIZE 13 BY 1
     "Comm. %" VIEW-AS TEXT
     ...
     "SalesRep" VIEW-AS TEXT
          ...
          FGCOLOR 9 
     "" VIEW-AS TEXT
          SIZE 4 BY 1 AT ROW 6.95 COL 116 WIDGET-ID 4
...
/* SETTINGS FOR FILL-IN oe-ordl.cases-unit IN FRAME F-Main
   NO-ENABLE EXP-LABEL EXP-FORMAT                                       */
...
/* SETTINGS FOR FILL-IN oe-ordl.part-no IN FRAME F-Main
   NO-ENABLE EXP-LABEL EXP-FORMAT                                       */
/* SETTINGS FOR FILL-IN oe-ordl.partial IN FRAME F-Main
   NO-ENABLE EXP-LABEL EXP-FORMAT                                       */
/* SETTINGS FOR FILL-IN oe-ordl.po-no IN FRAME F-Main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN oe-ordl.po-no-po IN FRAME F-Main
   NO-ENABLE                                                            */
...
ON LEAVE OF oe-ordl.cas-cnt IN FRAME F-Main /* Qty/Case */
DO:
    if int(oe-ordl.cas-cnt:screen-value) > int(oe-ordl.qty:screen-value) and
       int(oe-ordl.qty:screen-value) GT 0 then do:
...
for each oe-ordl where oe-ordl.company eq cocode
    ...
    for each oe-relh  where oe-relh.company eq cocode
        ...
        each oe-rell  where oe-rell.company eq cocode
...
------------------------------------------------------------------------------*/
  ...
  if not avail xoe-rel then do:
     ...
     do:
         ...
         run oe/d-shipid.w (input oe-ordl.cust-no, input-output v-ship-id)  .
  ...
  /* ===== update existing records
           ...
           for each oe-rel
               where oe-rel.company eq oe-ordl.company
                 ...
                 and oe-rel.i-no    eq oe-ordl.i-no
                 and oe-rel.line    eq oe-ordl.line:
             if oe-rel.tot-qty eq oe-ordl.qty then next.
             assign
              v-pct-chg      = oe-ordl.qty / oe-rel.tot-qty.
...
do with frame {&frame-name}:
   ...
   if oe-ordl.est-type = 4 or oe-ordl.est-type = 8 then disable oe-ordl.qty .   
end.
