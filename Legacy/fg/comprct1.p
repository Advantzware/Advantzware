  ...
  IF li-max-qty GE fg-rctd.t-qty THEN DO:
    ...
    IF itemfg.alloc EQ ?  /*assembled w/part receipts*/                                                                   AND
       CAN-FIND(FIRST fg-rcpts
                WHERE fg-rcpts.company EQ fg-rctd.company
                  AND fg-rcpts.linker  EQ "fg-rctd: " + STRING(fg-rctd.r-no,"9999999999")) THEN DO:
        ...
        FOR EACH fg-rcpts
          ...
          IF AVAIL b-itemfg THEN DO:
             ...
             ELSE           
               IF b-itemfg.pur-man EQ YES THEN llDelRecs = TRUE.
          END.
          IF llDelRecs THEN DO:

              CREATE tt-del-fg-rctd.
              ASSIGN tt-del-fg-rctd.fg-rctd-row = ROWID(b-fg-rctd).

              CREATE tt-del-fg-rcpts.
              ASSIGN tt-del-fg-rcpts.fg-rcpts-row = ROWID(fg-rcpts).
    ...
    ELSE DO:             
      ...
      FOR EACH tt-fg-set,
            ...
            IF AVAIL b-fg-rctd THEN DO:
                ...
                FOR EACH fg-rcpts
                    ...
                    IF AVAIL fg-bin THEN DO: 
                        CREATE tt-bin.
                        ASSIGN tt-bin.tt-bin-row = ROWID(fg-bin).
                        
                    END. /* avail bin */
                    ELSE DO:
                        CREATE tt-bin.
                        ASSIGN tt-bin.tt-bin-row = ROWID(bf-fg-rctd).
                    END.
        ...
        IF v-set-qty GT 0 OR (v-set-qty LT 0 AND fg-rctd.t-qty LT 0) THEN DO:
            ...
            IF AVAIL use-job AND use-job.val[1] EQ 1 THEN DO:      
