...
DEFINE INPUT PARAMETER ipcLocFrom AS CHARACTER NO-UNDO.
DEFINE INPUT PARAMETER ipcLocTo   AS CHARACTER NO-UNDO.
...
ASSIGN 
    cTextListToSelect  = "Item #,Location,Loc Name,Reorder Level,Order Minimum,Order Maximum,Lead Time (Days)" 
    cFieldListToSelect = "i-no,loc,loc-name,reorder-level,ord-min,ord-max,lead-time"  .
{sys/inc/ttRptSel.i}
...
begin_loc end_loc sl_avail Btn_Add sl_selected Btn_Remove btn_Up btn_down ~
tb_runExcel fi_file btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_i-no end_i-no begin_loc end_loc ~
...
DEFINE VARIABLE begin_loc  AS CHARACTER FORMAT "x(8)" 
...
DEFINE VARIABLE end_loc    AS CHARACTER FORMAT "X(8)" INITIAL "zzzzzzzz" 
    LABEL "To Warehouse" 
    VIEW-AS FILL-IN 
    SIZE 21 BY 1.

DEFINE VARIABLE fi_file    AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-ReOrdLevel.csv" 
...
DEFINE FRAME rd-fgexp
    ...
    begin_loc AT ROW 3.86 COL 28 COLON-ALIGNED HELP
    "Enter Beginning warehouse" WIDGET-ID 150
    end_loc AT ROW 3.86 COL 71 COLON-ALIGNED HELP
    "Enter Ending warehouse" WIDGET-ID 156
    sl_avail AT ROW 8.38 COL 9 NO-LABELS WIDGET-ID 26
    Btn_Add AT ROW 8.38 COL 44 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 130
    sl_selected AT ROW 8.38 COL 64 NO-LABELS WIDGET-ID 28
    Btn_Remove AT ROW 9.57 COL 44 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 134
    btn_Up AT ROW 10.76 COL 44 WIDGET-ID 136
    btn_down AT ROW 11.95 COL 44 WIDGET-ID 132
...
ASSIGN 
    begin_loc:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    end_i-no:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    end_loc:PRIVATE-DATA IN FRAME rd-fgexp = "parm".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_loc rd-fgexp
ON LEAVE OF begin_loc IN FRAME rd-fgexp /* From Warehouse */
    DO:
        ASSIGN {&self-name}.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_loc rd-fgexp
ON LEAVE OF end_loc IN FRAME rd-fgexp /* To Warehouse */
    DO:
        ASSIGN {&self-name}.
    END.
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_i-no end_i-no begin_loc end_loc sl_avail sl_selected tb_excel 
        tb_runExcel fi_file 
        WITH FRAME rd-fgexp.
    ENABLE RECT-6 RECT-7 RECT-8 begin_i-no end_i-no begin_loc end_loc sl_avail 
...
PROCEDURE run-report :
    ...
    FOR EACH itemfg-loc NO-LOCK
        ...
        AND itemfg-loc.i-no    GE begin_i-no
        AND itemfg-loc.i-no    LE end_i-no
        AND itemfg-loc.loc     GE begin_loc
        AND itemfg-loc.loc     LE end_loc,
        FIRST loc NO-LOCK
        ...
        FOR EACH ttRptSelected:
            CASE ttRptSelected.FieldList:                                                              
                WHEN "i-no" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(itemfg-loc.i-no).   
                WHEN "loc" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(itemfg-loc.loc). 
                WHEN "loc-name" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(loc.dscr). 
                WHEN "reorder-level" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING(itemfg-loc.ord-level)). 
                WHEN "ord-min" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING(itemfg-loc.ord-min)).
                WHEN "ord-max" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING(itemfg-loc.ord-max)).   
                WHEN "lead-time" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(STRING(itemfg-loc.lead-days)). 
