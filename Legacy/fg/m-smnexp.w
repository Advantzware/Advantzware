...
DEFINE INPUT PARAMETER lcSmanFrom   AS CHAR NO-UNDO.
DEFINE INPUT PARAMETER lcSmanTo AS CHAR NO-UNDO.
...
ASSIGN cTextListToSelect = "Rep#,Name,CustType,CustType Desc,Category,Category Desc,Basis,Basis Name,Margin%,Comm% "

      cFieldListToSelect = "sman,sname,custype,typedsc,procat,procatDscr,commbasis,basisDscr,netpct,comm "
       .
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 RECT-8 begin_slsmn end_slsmn ~
sl_avail Btn_Add sl_selected Btn_Remove btn_Up btn_down tb_runExcel fi_file ~
btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_slsmn end_slsmn  ~
 sl_avail ~
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR smanmtrx, ipc-field AS CHAR )  FORWARD.
...
DEFINE VARIABLE begin_slsmn AS CHARACTER FORMAT "x(3)" 
     LABEL "From Sales Rep" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.

DEFINE VARIABLE end_slsmn AS CHARACTER FORMAT "X(3)" INITIAL "zzz" 
     LABEL "To Sales Rep" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.

DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-smanmtrx.csv" 
...
DEFINE FRAME rd-fgexp
     begin_slsmn AT ROW 3.95 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Sales Rep Number" WIDGET-ID 142
     end_slsmn AT ROW 3.95 COL 71 COLON-ALIGNED HELP
          "Enter Ending Sales Rep Number" WIDGET-ID 144
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Export Sales Rep to Excel" WIDGET-ID 100.
...
ASSIGN 
       begin_slsmn:PRIVATE-DATA IN FRAME rd-fgexp     = 
                "parm".
  
ASSIGN 
       end_slsmn:PRIVATE-DATA IN FRAME rd-fgexp     = 
                "parm".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_slsmn rd-fgexp
ON LEAVE OF begin_slsmn IN FRAME rd-fgexp /* From Item # */
DO:
   assign {&self-name}.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_slsmn rd-fgexp
ON LEAVE OF end_slsmn IN FRAME rd-fgexp /* To Item # */
DO:
     assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_slsmn end_slsmn sl_avail sl_selected tb_excel 
          tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_slsmn end_slsmn  sl_avail Btn_Add 
...
FOR EACH b-smanmtrx WHERE b-smanmtrx.company = cocode
        AND b-smanmtrx.sman GE begin_slsmn
        AND b-smanmtrx.sman LE end_slsmn
        NO-LOCK:
    ...
    FOR EACH ttRptSelected:
        v-excel-detail-lines = v-excel-detail-lines + 
            appendXLLine(getValue-itemfg(BUFFER b-smanmtrx,ttRptSelected.FieldList)).
...
/*             WHEN "itemfg.sman" THEN                                                                */
/*                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(itemfg.sman).           */
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF lcSmanFrom <> "" THEN
    ASSIGN 
        begin_slsmn:SCREEN-VALUE = assignParam(lcSmanFrom,NO)
        end_slsmn:SCREEN-VALUE   = assignParam(lcSmanTo,NO)
        .
END.
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR smanmtrx, ipc-field AS CHAR ) :
/*------------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/
    ...
    CASE ipc-field :
        WHEN "procatDscr"  THEN DO:
            FIND FIRST fgcat WHERE fgcat.company  EQ ipb-itemfg.company
                AND fgcat.procat EQ ipb-itemfg.procat NO-LOCK NO-ERROR.
            IF AVAIL fgcat THEN
                lc-return = fgcat.dscr.
            ELSE
                lc-return = "".
            
        END.
        WHEN "typedsc"  THEN DO:
            FIND FIRST custype WHERE custype.company  EQ ipb-itemfg.company
                AND custype.custype EQ ipb-itemfg.custype NO-LOCK NO-ERROR.
        ...
        END.
        WHEN "sname"  THEN DO:
            FIND FIRST sman WHERE sman.company  EQ ipb-itemfg.company
        ...
        END.
        WHEN "basisDscr"  THEN DO:
            CASE ipb-itemfg.commbasis :
                WHEN "G" THEN
                    lc-return = "Gross Profit".
                WHEN "S" THEN
                    lc-return = "Selling Price".
                OTHERWISE
                    lc-return = "Margin".
            END CASE.
        END.
