    ...
    IF INDEX("RATCE",fg-rcpth.rita-code) NE 0 THEN ASSIGN
        {1}fg-bin.qty = fg-rdtlh.qty + IF fg-rcpth.rita-code EQ "C" THEN 0 ELSE {1}fg-bin.qty.
    ELSE ASSIGN
    ...
    ELSE DO:
        FIND FIRST {2}fg-rctd NO-LOCK WHERE 
            {2}fg-rctd.r-no EQ fg-rcpth.r-no 
            USE-INDEX fg-rctd 
            NO-ERROR.
        IF AVAIL {2}fg-rctd 
        AND {2}fg-rctd.partial NE 0 THEN ASSIGN
            {1}fg-bin.partial-count = {1}fg-bin.partial-count +
                                      ({2}fg-rctd.partial *
                                       IF INDEX("TS",fg-rcpth.rita-code) GT 0 THEN -1 ELSE 1).
        ELSE IF fg-rdtlh.qty-case NE 0 THEN ASSIGN
            {1}fg-bin.partial-count = {1}fg-bin.partial-count +
