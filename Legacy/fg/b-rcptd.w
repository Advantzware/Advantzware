            ...
            CASE FOCUS:NAME:
                WHEN "po-no" THEN 
                    DO:
                        ...
                        DO:
                            ASSIGN 
                                fg-rctd.po-no:SCREEN-VALUE IN BROWSE {&browse-name}   = ENTRY(1,char-val)
                                ...
                                fg-rctd.po-line:SCREEN-VALUE IN BROWSE {&browse-name} = ENTRY(6,char-val)
                                .
                            ...
                            RUN pGetCostsFromPO(cocode, INTEGER(fg-rctd.po-no:SCREEN-VALUE), INTEGER(fg-rctd.po-line:SCREEN-VALUE), DECIMAL(fg-rctd.t-qty:SCREEN-VALUE IN BROWSE {&browse-name}),
                            ...
                            ASSIGN                                                                             
                                ...
                                fg-rctd.std-cost:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(dCostPerUOM)
                                fg-rctd.frt-cost:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(dCostExtendedFreight)
                                fg-rctd.ext-cost:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(dCostExtended).
...
/*                            fg-rctd.std-cost:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(lv-cost).                                                                     */
                ...
                WHEN "i-no" THEN 
                    DO:
                        IF fg-rctd.po-no:SCREEN-VALUE IN BROWSE {&browse-name} <> "" THEN 
                        DO:
                        ...
                        ELSE IF fg-rctd.job-no:SCREEN-VALUE <> "" THEN 
                            ...
                            DO:
                                ...
                                DO:
                                    ...
                                    ASSIGN 
                                        fg-rctd.i-no:SCREEN-VALUE     = itemfg.i-no
...
ON ENTRY OF fg-rctd.rct-date IN BROWSE Browser-Table /* Receipt!Date */
    DO:
        IF LASTKEY NE -1 AND ll-set-parts THEN 
        DO:
...
ON LEAVE OF fg-rctd.po-no IN BROWSE Browser-Table /* PO# */
    DO:
        ...
        DO:
            ...
            IF {&self-name}:SCREEN-VALUE IN BROWSE {&browse-name} NE "" THEN 
            DO:
                FIND FIRST po-ordl
                    ...
                    AND po-ordl.line      EQ INT(fg-rctd.po-line:SCREEN-VALUE IN BROWSE {&browse-name})
                    NO-LOCK NO-ERROR.
...
ON ENTRY OF fg-rctd.job-no IN BROWSE Browser-Table /* Job# */
    DO:
        ...
        IF fg-rctd.po-no:SCREEN-VALUE IN BROWSE {&browse-name} NE "" THEN 
        DO:
...
ON LEAVE OF fg-rctd.i-no IN BROWSE Browser-Table /* Item */
    DO:
        ...
        IF INT(fg-rctd.po-no:SCREEN-VALUE IN BROWSE {&browse-name}) NE 0 AND
            fg-rctd.i-no:SCREEN-VALUE IN BROWSE {&browse-name} NE ""      THEN 
        DO:
    ...
    DO:
        ...
        DO:
            ...
            IF /*ABS(DECIMAL(fg-rctd.t-qty:SCREEN-VALUE IN BROWSE {&browse-name})) GE bfFgBin.qty */ 
                ABS(dTotalQty) + ABS(bfFgBin.qty) LE ABS(dMaxCompQty) THEN 
            DO:
        ...
        ELSE 
        DO:
            IF ABS(dTotalQty) + ABS(bfFgBin.qty) LE ABS(dMaxCompQty) THEN 
            DO:
...
PROCEDURE display-po :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN
            ...
            fg-rctd.cost-uom:SCREEN-VALUE IN BROWSE {&browse-name} = po-ordl.pr-uom
            fg-rctd.po-line:SCREEN-VALUE IN BROWSE {&browse-name}  = STRING(po-ordl.line)
            lv-cost                                                = po-ordl.cost * (IF po-ordl.disc NE 0 THEN (1 - (po-ordl.disc / 100)) ELSE 1).
...
PROCEDURE get-matrix :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        /*            AND po-ordl.line = fg-rctd.po-line                                                                                     */
        ...
        /*                AND po-ordl.line  = INT(fg-rctd.po-line:SCREEN-VALUE IN BROWSE {&browse-name})                                     */
        /*                AND po-ordl.item-type = NO                                                                                         */
        /*                NO-LOCK NO-ERROR.                                                                                                  */
        /*        IF NOT AVAILABLE po-ordl THEN                                                                                              */
        /*            FIND FIRST po-ordl WHERE po-ordl.company = cocode                                                                      */
        ...
        /*                                                                                                                                   */
        /*                                                                                                                                   */
        /*        IF ip-first-disp  AND AVAILABLE fg-rctd AND fg-rctd.i-no:SCREEN-VALUE IN BROWSE {&browse-name} <> "" THEN                  */
        /*        DO: /* for row-display */                                                                                                  */
        ...
        /*                                NO-LOCK NO-ERROR.                                                                                  */
        /*                            IF AVAILABLE sys-ctrl AND sys-ctrl.log-fld THEN v-job-qty = job-hdr.qty                          .     */
        /*                            ELSE                                                                                                   */
        /*                            DO:                                                                                                    */
        ...
        /*                                    NO-ERROR.                                                                                      */
        /*                                                                                                                                   */
        /*                                v-job-qty = (job-hdr.qty * (1 + ((IF AVAILABLE oe-ordl THEN oe-ordl.over-pct ELSE                  */
        ...
        /*                                                                                                                                   */
        /*                            IF v-rec-qty GT v-job-qty AND NOT lv-overrun-checked THEN                                              */
        /*                            DO:                                                                                                    */
        ...
        /*        IF NOT lvlTotalCostCalculated AND AVAIL(po-ordl) AND v-ord-qty <= lv-adjusted-ea AND po-ordl.setup > 0 THEN                */
        /*        DO:                                                                                                                        */
        /*                                                                                                                                   */
            ...
            DO:
                ...
                ELSE
                    ...
                    DO:
                        ...
                        IF /*itemfg.total-std-cost EQ 0 AND*/ itemfg.isaset THEN 
                        DO:
...
PROCEDURE local-assign-record :
    ...
    IF NOT AVAIL fg-rctd AND lrMissingRow NE ? THEN 
    DO:
        FIND fg-rctd 
        ...
        IF fg-rctd.job-no:SCREEN-VALUE IN BROWSE {&browse-name} NE "" THEN
            FOR EACH job-hdr NO-LOCK
                ...
                IF LAST(job-hdr.frm)                                                  OR
                    job-hdr.i-no EQ fg-rctd.i-no:SCREEN-VALUE IN BROWSE {&browse-name} THEN 
                DO:
                    ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pGetCostsFromPO B-table-Win 
    ...
    DO:
        ...
        DO WHILE AVAIL fg-rctd AND CAN-FIND(FIRST w-rowid):
            ...
            DO:
                ...
                IF fg-rctd.i-no = cSelectionItem THEN 
                DO:
        ...
        FOR EACH w-rowid:
            ...
            IF ABS(iSelectedQty) GE dMaxQty THEN 
            DO:
            ...
            ASSIGN 
                bfFgRctd.created-by   = USERID("nosweat")
            ...
            /* As in local-assign logic */
            IF glFGPOTag# AND bfFgRctd.tag EQ "" THEN 
            DO:
...
PROCEDURE show-bin-info :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ASSIGN
            ...
            fg-rctd.partial:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(INT(fg-rctd.t-qty:SCREEN-VALUE IN BROWSE {&browse-name}) -
...
PROCEDURE show-freight :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN
            ...
            fg-rctd.ext-cost:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(DEC(fg-rctd.ext-cost:SCREEN-VALUE IN BROWSE {&browse-name}) - ld) .
...
PROCEDURE valid-blank-qty :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        IF dec(fg-rctd.t-qty:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN 
        DO:
...
PROCEDURE valid-cases :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF INTE(ip-focus:SCREEN-VALUE)
            GE 0 AND fg-rctd.tag:SCREEN-VALUE IN BROWSE {&browse-name} GT "" THEN 
        DO:
...
PROCEDURE valid-job-no :
    ...
    IF fg-rctd.job-no:SCREEN-VALUE IN BROWSE {&browse-name} GT "" THEN 
    DO WITH FRAME {&frame-name}:
        fg-rctd.job-no:SCREEN-VALUE IN BROWSE {&browse-name} =
            FILL(" ",6 - LENGTH(TRIM(fg-rctd.job-no:SCREEN-VALUE IN BROWSE {&browse-name}))) +
            TRIM(fg-rctd.job-no:SCREEN-VALUE IN BROWSE {&browse-name}).
...
PROCEDURE valid-uom :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN
            fg-rctd.cost-uom:SCREEN-VALUE IN BROWSE {&browse-name} = CAPS(fg-rctd.cost-uom:SCREEN-VALUE IN BROWSE {&browse-name})
...
PROCEDURE validate-record :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ASSIGN
            fg-rctd.t-qty:SCREEN-VALUE IN BROWSE {&browse-name} = STRING((INT(fg-rctd.cases:SCREEN-VALUE IN BROWSE {&browse-name}) *
        ...
        DO:    
            ...
            IF li-max-qty LT DEC(fg-rctd.t-qty:SCREEN-VALUE IN BROWSE {&browse-name}) THEN 
            DO:
                ll = NO.
                ...
                IF ll THEN  
                    ASSIGN
                        ...
                        fg-rctd.cases:SCREEN-VALUE IN BROWSE {&browse-name}   = STRING(TRUNC((li-max-qty - DEC(fg-rctd.partial:SCREEN-VALUE IN BROWSE {&browse-name})) /
                             DEC(fg-rctd.qty-case:SCREEN-VALUE IN BROWSE {&browse-name}),0))
                        fg-rctd.partial:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(li-max-qty - (DEC(fg-rctd.cases:SCREEN-VALUE IN BROWSE {&browse-name}) *
