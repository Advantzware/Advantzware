...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR mach, ipc-field AS CHAR )  FORWARD.
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\mach.csv" 
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF begin_mach:SCREEN-VALUE EQ "" THEN do:
        DEF VAR m AS INT INIT 1 NO-UNDO.
            m = 1.
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-mach FOR mach, ipc-field AS CHAR ) :
/*------------------------------------------------------------------------------
