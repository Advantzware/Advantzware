...
PROCEDURE pBusinessLogic:
    ...
    FIND FIRST ap-ctrl NO-LOCK
         WHERE ap-ctrl.company EQ cCompany
         NO-ERROR.
    ...
    FIND FIRST bank NO-LOCK
         WHERE bank.company EQ cCompany
...
PROCEDURE pClearAP:
    FOR EACH ap-inv
        WHERE ap-inv.company EQ cCompany
...
PROCEDURE pCreateManualCheck:
    ...
    IF CAN-FIND(FIRST ap-pay
                WHERE ap-pay.company    EQ cCompany
...
PROCEDURE pEditReport:
    DEFINE INPUT PARAMETER rmpostgl AS LOGICAL NO-UNDO.
    ...
    FOR EACH xap-inv NO-LOCK
        WHERE xap-inv.company  EQ cCompany
        ...
        IF AVAILABLE ap-inv THEN DO:
            ...
            FOR EACH ap-invl NO-LOCK
                ...
                FIND FIRST fg-rcpts NO-LOCK
                    WHERE fg-rcpts.company   EQ cCompany
    ...
    FOR EACH tt-report,
        ...
        FOR EACH ap-invl NO-LOCK
            ...
            IF AVAILABLE po-ordl AND po-ordl.item-type AND ap-invl.amt NE 0 AND rmpostgl THEN DO:
                ...
                FIND FIRST costtype
                     WHERE costtype.company   EQ cCompany
...
PROCEDURE pPostGL:
    DEFINE INPUT PARAMETER apautocheck-log AS LOGICAL NO-UNDO.
                ...
                IF AVAILABLE po-ordl THEN DO:
                    ...
                    IF NOT po-ordl.item-type AND lFGPostGL THEN DO:
                        ...
                        FIND FIRST prodl NO-LOCK
                             WHERE prodl.company EQ cCompany
                               AND prodl.procat  EQ itemfg.procat
                               AND CAN-FIND(FIRST prod
                                            WHERE prod.company EQ cCompany
