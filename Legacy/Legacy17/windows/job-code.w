...
&SCOPED-DEFINE h_Browse01 h_job-code
...
DEFINE VARIABLE h_job-code AS HANDLE NO-UNDO.
DEFINE VARIABLE h_job-code-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 21.67 , 92.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/job-code.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_job-code ).
       RUN set-position IN h_job-code ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_job-code ( 19.52 , 88.00 ) NO-ERROR.
       ...
       /* Links to SmartNavBrowser h_job-code. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_job-code ).
       RUN add-link IN adm-broker-hdl ( h_job-code , 'Record':U , THIS-PROCEDURE ).
       RUN add-link IN adm-broker-hdl ( h_job-code , 'export-xl':U , h_export ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_job-code ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/job-code.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_job-code-2 ).
       RUN set-position IN h_job-code-2 ( 6.00 , 9.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_job-code-2. */
       RUN add-link IN adm-broker-hdl ( h_job-code , 'Record':U , h_job-code-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_job-code-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_job-code-2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updsav ,
             h_job-code-2 , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "job-code"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "job-code"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "job-code"}
