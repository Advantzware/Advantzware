...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Build FG Bin Master File"
         HEIGHT             = 11.86
         WIDTH              = 90.2
         MAX-HEIGHT         = 19.76
         MAX-WIDTH          = 98.2
...
ON END-ERROR OF C-Win /* Build FG Bin Master File */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Build FG Bin Master File */
DO:
...
status default "Please wait sorting bins...".

SESSION:SET-WAIT-STATE("General").

for each fg-bin
    where fg-bin.company eq cocode
    no-lock use-index co-ino
  ...
  if first-of(fg-bin.loc-bin) then do:
    ...
    if not avail b-fg-bin then do:
      status default "Creating FG Bin for " + 
                     trim(fg-bin.loc) + trim(fg-bin.loc-bin).

      create b-fg-bin.
      assign
       b-fg-bin.company = cocode
       b-fg-bin.i-no    = ""
       b-fg-bin.loc     = fg-bin.loc
       b-fg-bin.loc-bin = fg-bin.loc-bin.
    end.
  end.
end.
