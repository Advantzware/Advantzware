...
*/
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      if avail oe-bolh AND xinv-head.bol-no NE 0 then do:
     /*   find first oe-relh where oe-relh.company = oe-bolh.company and
      ...
      DO TRANSACTION:
        ...
        for each xinv-line no-lock where xinv-line.r-no = inv-head.r-no break by xinv-line.i-no:
           ...
           ln-cnt = ln-cnt + 3.
           IF xinv-line.part-no <> "" OR xinv-line.part-dscr1 <> "" THEN ln-cnt = ln-cnt + 1.
        ...
        if v-prntinst then do:
         /* {custom/notesprt.i inv-head v-inst 4}*/
          DO i = 1 TO 4:
             v-inst[i] = inv-head.bill-i[i].
             IF v-inst[i] <> "" THEN ln-cnt = ln-cnt + 1.
          END.
        end.
/*         lv-tot-pg = IF ln-cnt MOD 22 = 0 THEN TRUNC( ln-cnt / 22,0)                           */
/*                     ELSE lv-tot-pg + TRUNC( ln-cnt / 22,0) .  /* 23->47 25 po detail lines */ */
/*                                                                                               */
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            IF AVAIL itemfg THEN do:
                FIND FIRST notes WHERE notes.note_code EQ "FSC"
                    AND notes.rec_key = itemfg.rec_key NO-LOCK NO-ERROR.
                IF AVAIL notes THEN
                    ASSIGN v-spec-dscr = notes.note_text.
            END.            /*task# 11151307*/
         ...
         for each w-tax :
            ...
            */
            ...
            IF v-printline > 50 THEN DO:
               PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + 
                   " of [=@endPage" + string(inv-head.inv-no) + "-@startPage" + string(inv-head.inv-no) + "+1] " FORM "x(120)" .
               PAGE.
               v-printline = 0.
               {oe/rep/invfrank.i}
            END.
...
*/         IF v-printline > 53 THEN DO:
        ...
        /* {custom/notesprt.i inv-head v-inst 4}*/
         DO i = 1 TO 4:
             v-inst[i] = inv-head.bill-i[i].
              IF v-inst[i] <> "" THEN DO:                
                 IF v-printline > 50 THEN DO:
                    PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of [=@endPage" + string(inv-head.inv-no) + "-@startPage" + string(inv-head.inv-no) + "+1] " FORM "x(120)" .
                    PAGE.
                    v-printline = 0.
                    {oe/rep/invfrank.i}
                 END.
    ...
    PUT "<R54><C60><#8><FROM><R+5><C+20><RECT> " 
        ...
        "<C65>Page "  string(PAGE-NUM - lv-pg-num,">>9") + 
        " of [=@endPage" + string(inv-head.inv-no) + "-@startPage" + string(inv-head.inv-no) + "+1] " FORM "x(120)" .
