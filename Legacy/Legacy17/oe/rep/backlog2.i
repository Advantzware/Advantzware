...
if avail oe-ordl then do:
  ...
  IF rd_date = "Ship" THEN
     w-ord.rel-date = oe-ord.last-date.
...
if avail job then do:
  ...
  for each job-hdr
    ...
    assign
     ...
     w-ord.ord-no    = string(job-hdr.ord-no).
  end.
end.
...
if avail itemfg then do:
  ...
  assign
   ...
   w-ord.sman      = cust.sman
   w-ord.stat      = "ITEM" .
  /* w-ord.qty-due   = IF itemfg.q-onh <> 0 THEN itemfg.q-onh ELSE itemfg.q-ono.*/  /* Task 04261307 */
   
  /* Task 04261307 */
  FIND FIRST oe-ordl WHERE oe-ordl.company = cocode 
                    AND oe-ordl.ord-no = int(tt-report.key-03)
                    AND oe-ordl.i-no = itemfg.i-no NO-LOCK NO-ERROR.
  IF AVAIL oe-ordl THEN
      w-ord.ord-no    = tt-report.key-03 .
   if avail oe-ordl then do:
       ASSIGN
         li-inv-qty = 0
         li-ship-qty = 0 .
