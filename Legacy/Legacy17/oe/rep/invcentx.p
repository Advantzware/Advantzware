...
DO:
   IF cocode = "002" THEN
      ls-image1 = "images\pyxisale.jpg".
   ELSE
      ls-image1 = "images\cbxsale.jpg".
    ...
    for each report where report.term-id eq v-term-id no-lock,
     ...
     /*   find first oe-relh where oe-relh.company = oe-bolh.company and
      ...
      DO TRANSACTION:
        ...
        for each xinv-line no-lock where xinv-line.r-no = inv-head.r-no break by xinv-line.i-no:
           
           if xinv-line.sman[1] ne "" then do:
             create w-sman.
             assign w-sman.sman = xinv-line.sman[1].
             FIND FIRST sman WHERE sman.company = xinv-line.company
                            AND sman.sman = xinv-line.sman[1] NO-LOCK NO-ERROR.
              ...
              if xinv-line.est-no ne "" THEN do:
                 ...
                 IF xinv-line.form-no = 0 AND xinv-line.est-type = 2 THEN DO:
                    ...
                    FOR EACH eb NO-LOCK WHERE eb.company = xinv-line.company AND
                        ...
                        ***/
                    END. /* each eb */
                 END. /* xinv-line.form-no = 0 AND xinv-line.est-type = 2 do */
           ...
           IF xinv-line.part-dscr2 <> "" THEN ln-cnt = ln-cnt + 1. 
        end. /* each xinv-line */
        for each inv-misc no-lock where inv-misc.company = inv-head.company and
            ...
            IF inv-misc.s-man[1] ne "" then do:
              create w-sman.
              assign w-sman.sman = inv-misc.s-man[1].
              FIND FIRST sman WHERE sman.company = inv-head.company
                             AND sman.sman = inv-misc.s-man[1] NO-LOCK NO-ERROR.
              w-sman.sname = IF AVAIL sman THEN sman.sname ELSE "".
        ...
        lv-tot-pg = IF ln-cnt MOD 24 = 0 THEN TRUNC( ln-cnt / 24,0)
                    ELSE lv-tot-pg + TRUNC( ln-cnt / 24,0)  /* 29->52 24 detail lines */
                        /** Build Salesman Id String **/
        
        v-slsname = "".
        FIND FIRST w-sman NO-LOCK NO-ERROR.

        IF AVAIL w-sman THEN
        DO:
          v-slsname = w-sman.sNAME.
          delete w-sman.
        end.
        ...
        ASSIGN lv-prt-sts = IF NOT inv-head.printed THEN "ORIGINAL" ELSE "REVISED"
               lv-prt-date = TODAY
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            IF inv-line.pr-uom EQ "CS" AND inv-line.cas-cnt NE 0 THEN
              v-ship-qty = v-ship-qty / inv-line.cas-cnt.

            {sys/inc/roundup.i v-ship-qty}

            v-printline = v-printline + 2.

            IF v-printline > 51 THEN DO:
               PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" .
               PAGE.
               v-printline = v-printline - 51. 
               {oe/rep/invcentx.i}
            END.
        ...
        for each inv-misc OF inv-head no-lock WHERE
          inv-misc.bill = "Y" with frame detailm:
          IF v-first THEN
          do:
            IF v-printline > 51 THEN DO:
               PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" .
          ...
          put inv-misc.charge AT 10 inv-misc.dscr FORM "x(30)" inv-misc.amt  SKIP.
          ASSIGN
        ...
        if v-prntinst then do:
            IF inv-head.multi-invoice THEN DO:
                i = 0.
                FOR EACH bf-inv-head WHERE bf-inv-head.company = inv-head.company
                    AND bf-inv-head.inv-no = inv-head.inv-no
                    AND bf-inv-head.cust-no = inv-head.cust-no 
                    AND bf-inv-head.stat NE "H" 
                    AND NOT bf-inv-head.multi-invoice NO-LOCK:
                    {custom/notesprt.i bf-inv-head v-inst 4}
/*                     FIND FIRST notes WHERE notes.rec_key EQ bf-inv-head.rec_key NO-LOCK NO-ERROR. */
/*                     IF AVAIL notes AND notes.note_text NE "" THEN DO:                             */
/*                         i = i + 1.                                                                */
/*                         IF i LE 4 THEN v-inst[i] = notes.note_text.                               */
/*                     END.                                                                          */
                END.
            END.
            ELSE DO:
    ...
    PUT "<R54><C60><#8><FROM><R+5><C+20><RECT> " 
        ...
        "<=8><R+4><C2><FROM><R+2><C+3><RECT><||3>"
        "<=8><R+5><C6>" lv-prt-date " " lv-prt-time " " lv-prt-sts "<C40><P14><B> Thank  You!</B><P10>"
