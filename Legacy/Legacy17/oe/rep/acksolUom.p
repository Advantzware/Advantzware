...
DEF VAR lv-line-print AS INT INIT 48 NO-UNDO. /*# of lines in body*/
...
/*         end.    
        ...
        PUT oe-ordl.po-no AT 8
            v-color  /*v-pdscr1*/ FORM "x(30)" AT 23
            oe-ordl.price FORM "$->>>,>>9.99<<" AT 80 SPACE(1)
            oe-ordl.pr-uom AT 95  
...
/*             end.                                                                                                    */
            IF AVAIL shipto THEN DO:
                RUN addLines(1).
                put oe-rel.po-no AT 8 shipto.ship-name AT 23 FORM "x(28)" /*lcnt AT 55 FORM ">>9" SPACE(1)*/  ld-date AT 59 space(1) (IF oe-rel.link-no EQ 0 THEN oe-rel.tot-qty ELSE oe-rel.qty) FORM "->>>>>>>>9"
                    SKIP .
...
/*               end.                                                                                                   */
              RUN addLines(1).
              put oe-rel.po-no AT 8  ld-date AT 59 (IF oe-rel.link-no EQ 0 THEN oe-rel.tot-qty ELSE oe-rel.qty) FORM "->>>>>>>>9"     SKIP .
          END.
          PUT SKIP(1).
/*           v-printline = v-printline + 1. */
          RUN addLines(1).
