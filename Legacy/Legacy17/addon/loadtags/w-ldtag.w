...
DEFINE VARIABLE h_b-ldtag AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Loadtag Maintenance (Finished Goods)"
         HEIGHT             = 23.81
         WIDTH              = 150
         MAX-HEIGHT         = 24
         MAX-WIDTH          = 150
...
ON END-ERROR OF W-Win /* Loadtag Maintenance (Finished Goods) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Loadtag Maintenance (Finished Goods) */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brwse Loadtag|View Loadtag' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_v-ldtag ).
       RUN set-position IN h_v-ldtag ( 5.52 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 15.24 , 138.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-nav2 ).
       RUN set-position IN h_v-nav2 ( 21.95 , 6.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 22.19 , 67.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav ( 2.14 , 56.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/loadtags/vp-print.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vp-print ).
       RUN set-position IN h_vp-print ( 22.19 , 123.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 17.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-ldtag. */
       RUN add-link IN adm-broker-hdl ( h_b-ldtag , 'Record':U , h_v-ldtag ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_v-ldtag ).
       RUN add-link IN adm-broker-hdl ( h_vp-print , 'loadtag':U , h_v-ldtag ).

       /* Links to SmartViewer h_v-nav2. */
       RUN add-link IN adm-broker-hdl ( h_b-ldtag , 'nav-itm':U , h_v-nav2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ldtag ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-nav2 ,
             h_v-ldtag , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updsav ,
             h_v-nav2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vp-print ,
             h_p-updsav , 'AFTER':U ).
    END. /* Page 2 */
