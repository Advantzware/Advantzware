...
DEFINE VARIABLE h_rm-rdtl2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_rm-rdtl2-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_rm-trans AS HANDLE NO-UNDO.
DEFINE VARIABLE h_rm-trans-2 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "WAREHOUSE TRANSACTION TRANSFERS"
         HEIGHT             = 24
         WIDTH              = 150
         MAX-HEIGHT         = 24
         MAX-WIDTH          = 150
...
ON END-ERROR OF W-Win /* WAREHOUSE TRANSACTION TRANSFERS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* WAREHOUSE TRANSACTION TRANSFERS */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Transfer|View Transfer' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/rm-trans.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_rm-trans-2 ).
       RUN set-position IN h_rm-trans-2 ( 4.81 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 130.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/rm-rdtl2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_rm-rdtl2 ).
       RUN set-position IN h_rm-rdtl2 ( 9.10 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.05 , 83.40 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 16.24 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updsav-2 ).
       RUN set-position IN h_p-updsav-2 ( 16.24 , 47.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav-2 ( 2.14 , 56.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/rm-rdtl2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_rm-rdtl2-2 ).
       RUN set-position IN h_rm-rdtl2-2 ( 18.38 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.76 , 102.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1') NO-ERROR.

       /* Links to SmartViewer h_rm-trans-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_rm-trans-2 ).
       RUN add-link IN adm-broker-hdl ( h_rm-trans , 'Record':U , h_rm-trans-2 ).

       /* Links to SmartNavBrowser h_rm-rdtl2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico-2 , 'Navigation':U , h_rm-rdtl2 ).
       RUN add-link IN adm-broker-hdl ( h_rm-trans , 'Record':U , h_rm-rdtl2 ).

       /* Links to SmartViewer h_rm-rdtl2-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav-2 , 'TableIO':U , h_rm-rdtl2-2 ).
       RUN add-link IN adm-broker-hdl ( h_rm-rdtl2 , 'Record':U , h_rm-rdtl2-2 ).
