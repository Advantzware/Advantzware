...
&SCOPED-DEFINE dataGridInclude dataGrid\rm\b-pstlst.i
...
&Scoped-define FIELDS-IN-QUERY-Browser-Table rm-rctd.rct-date ~
rm-rctd.post-date rm-rctd.po-no rm-rctd.i-no rm-rctd.i-name rm-rctd.s-num ~
rm-rctd.job-no rm-rctd.job-no2 rm-rctd.loc rm-rctd.loc-bin rm-rctd.tag ~
rm-rctd.qty rm-rctd.pur-uom rm-rctd.cost rm-rctd.cost-uom 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define FIELD-PAIRS-IN-QUERY-Browser-Table
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH rm-rctd ~
      WHERE rm-rctd.rita-code = "P" NO-LOCK.
...
/* Query rebuild information for BROWSE Browser-Table
     ...
     _Options          = "NO-LOCK"
     _TblOptList       = "USED,"
     _Where[1]         = "rm-rctd.rita-code = ""P"""
     _FldNameList[1]   = JOBS.rm-rctd.rct-date
     _FldNameList[2]   = JOBS.rm-rctd.post-date
     _FldNameList[3]   = JOBS.rm-rctd.po-no
     _FldNameList[4]   > JOBS.rm-rctd.i-no
"rm-rctd.i-no" "Item#" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[5]   > JOBS.rm-rctd.i-name
"rm-rctd.i-name" "Name/Description" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[6]   = JOBS.rm-rctd.s-num
     _FldNameList[7]   = JOBS.rm-rctd.job-no
     _FldNameList[8]   = JOBS.rm-rctd.job-no2
     _FldNameList[9]   > JOBS.rm-rctd.loc
"rm-rctd.loc" "Whse" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[10]   > JOBS.rm-rctd.loc-bin
"rm-rctd.loc-bin" "Bin" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[11]   > JOBS.rm-rctd.tag
"rm-rctd.tag" "Tag" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[12]   = JOBS.rm-rctd.qty
     _FldNameList[13]   = JOBS.rm-rctd.pur-uom
     _FldNameList[14]   = JOBS.rm-rctd.cost
     _FldNameList[15]   = JOBS.rm-rctd.cost-uom
