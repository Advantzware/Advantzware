...
/*--------------------------------------------------------------------------
    ...
    Syntax      : {methods/template/viewer2.i}
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE vcShipRecKey  AS CHARACTER  NO-UNDO.

  MESSAGE 'Would you like to import from Customer contacts?'
    VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO
    UPDATE vlImport AS LOGICAL.
  ...
  IF vlImport THEN DO:
    ...
    IF VALID-HANDLE(WIDGET-HANDLE(char-hdl)) THEN
      vcShipRecKey = DYNAMIC-FUNCTION ('GetCurrShipTo':U IN WIDGET-HANDLE (char-hdl)).
    
    RUN windows\contacts.w (vcShipRecKey).

    IF VALID-HANDLE(WIDGET-HANDLE(char-hdl)) THEN
      DYNAMIC-FUNCTION ('RefreshBrowser':U IN WIDGET-HANDLE (char-hdl)).
