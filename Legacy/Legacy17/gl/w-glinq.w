...
DEFINE VARIABLE h_b-glinq AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_b-glinq , 'move-columns':U , h_movecol ).

    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gl/b-glrun.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-glrun ).
       ...
       RUN add-link IN adm-broker-hdl ( h_b-glinq , 'Run-no':U , h_b-glrun ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-glrun ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_w-glinvl ).
       /* Position in AB:  ( 4.67 , 3.20 ) */
       /* Size in UIB:  ( 1.86 , 10.80 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartWindow h_w-glinvl. */
       RUN add-link IN adm-broker-hdl ( h_b-glrun , 'Record':U , h_w-glinvl ).
