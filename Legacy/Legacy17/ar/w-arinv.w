...
&SCOPED-DEFINE h_Browse01 h_b-arinv
...
DEFINE VARIABLE h_b-arinv AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-arinv AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Customer Invoices */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Customer Invoices */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Invoices|View Invoices' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ar/b-arinv.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-arinv ).
       RUN set-position IN h_b-arinv ( 4.62 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-arinv ( 19.91 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_b-arinv. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-arinv ).
       RUN add-link IN adm-broker-hdl ( h_b-arinv , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-arinv ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ar/v-arinv.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_v-arinv ).
       RUN set-position IN h_v-arinv ( 4.57 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.52 , 130.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-arinv ).
       RUN set-position IN h_p-arinv ( 4.81 , 136.00 ) NO-ERROR.
       RUN set-size IN h_p-arinv ( 8.81 , 12.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_b-arinvl ).
       RUN set-position IN h_b-arinvl ( 14.81 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 22.67 , 10.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-cashl ).
       RUN set-position IN h_p-cashl ( 22.67 , 69.00 ) NO-ERROR.
       RUN set-size IN h_p-cashl ( 1.43 , 76.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-arinv. */
       RUN add-link IN adm-broker-hdl ( h_b-arinv , 'Record':U , h_v-arinv ).
       RUN add-link IN adm-broker-hdl ( h_b-arinvl , 'invhead':U , h_v-arinv ).
       RUN add-link IN adm-broker-hdl ( h_p-arinv , 'TableIO':U , h_v-arinv ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-inv':U , h_v-arinv ).
       ...
       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-arinv ,
             h_folder , 'AFTER':U ).
