...
tb_runExcel tb_excel fi_file btn-ok btn-cancel 
    ...
    for each report
      ...
      DO:
         ...
         PUT STREAM excel UNFORMATTED
            ...
            '"' po-ord.vend-no                                   '",'
            '"' report.key-05                                    '",'
            '"' (IF AVAIL itemfg THEN itemfg.i-name
                 ELSE IF AVAIL ITEM THEN item.i-name ELSE "")    '",'
            '"' po-ordl.vend-i-no                                '",'
            '"' (IF AVAIL itemfg THEN itemfg.prod-notes ELSE "") '",'
            '"' STRING(v-cons-qty,"->>,>>>,>>9.9<<<<<")          '",'.
      END.
      ...
      for each rm-rcpth
        ...
        DO:
          ...
          excel-skip = YES.
          RUN excel-1-proc(INPUT rm-rcpth.trans-date,
                           INPUT v-cons-qty, INPUT v-amount, INPUT v-uom).
        END.
      end.
      ...
      for each fg-rcpth
        ...
        DO:
           ...
           RUN excel-1-proc(INPUT fg-rcpth.trans-date,
                            INPUT fg-rdtlh.qty,
                            INPUT v-amount,
                            INPUT v-uom).
        END.
