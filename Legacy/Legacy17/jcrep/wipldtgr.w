...
DEFINE INPUT PARAMETER ipi-run-qty AS INT NO-UNDO.
DEFINE INPUT PARAMETER ipc-machine AS CHAR NO-UNDO.
...
DEFINE VARIABLE vl-run-from-mach AS LOGICAL NO-UNDO.
...
DEFINE FRAME FRAME-A
     ...
     fi_toRMTag AT ROW 3.86 COL 21 COLON-ALIGNED HELP
     ...
     begin_job AT ROW 16.81 COL 21.8 COLON-ALIGNED HELP
          "Enter Job Number"
     begin_job2 AT ROW 16.71 COL 38 COLON-ALIGNED HELP
          "Enter Job Number"
...
DO:
  ...
  IF NUM-ENTRIES(char-val) > 1 THEN
     DO WITH FRAME {&FRAME-NAME}:
        ASSIGN begin_job:SCREEN-VALUE  = trim(ENTRY(1,char-val))
...
DO:
    ...
    IF fi_rmtagno:SCREEN-VALUE = "" AND
       ...
       ed_singletags:SCREEN-VALUE = "" THEN DO:
                  ...
                  FOR EACH tt-rmtags:
                      ...
                      ELSE
                          ASSIGN tt-rmtags.origshtqty = ( tt-rmtags.lfqty * 12 
                                 ...
                                 tt-rmtags.availshtqty = tt-rmtags.origshtqty.
                  END.
                  IF vl-InputParameters OR ipi-run-qty GT 0 THEN
...
DO:
   ...
   DO WITH FRAME {&FRAME-NAME}:
      ...
      DO:
         ...
         RELEASE ITEM.
      END.
      ELSE IF NOT vl-run-from-mach  THEN
...
ON HELP OF fi_rmtagno IN FRAME FRAME-A /* From RM Tag# */
DO:
  DEFINE VARIABLE opCharVal AS CHARACTER NO-UNDO.
  DEFINE VARIABLE opRecID AS RECID NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi_rmtagno C-Win
ON LEAVE OF fi_rmtagno IN FRAME FRAME-A /* From RM Tag# */
DO:
  DEF VAR op-error AS LOG NO-UNDO.
  ...
  IF fi_rmtagno:SCREEN-VALUE = "" AND
     ...
     ed_singletags:SCREEN-VALUE = "" THEN DO:
      ASSIGN fi_sheets-pallet:SCREEN-VALUE = ""
             ...
             fi_UOM-label:SCREEN-VALUE = "".
      IF NOT vl-run-from-mach THEN
        ASSIGN begin_job_form:SCREEN-VALUE = "0"
               END_job_form:SCREEN-VALUE = "0"
               fi_exp-sheets-roll:SCREEN-VALUE = "".
  ...
  END.
  IF SELF:SCREEN-VALUE GT "" THEN
    ASSIGN vl-run-from-mach   = NO
           vl-InputParameters = NO. 

  IF SELF:MODIFIED THEN DO:
      /* This enables form number if tags have been entered */
      /*RUN normal-enable. */
      RUN validRMLoadtag(OUTPUT op-error).
      IF op-error THEN RETURN NO-APPLY.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi_rmtagno C-Win
ON VALUE-CHANGED OF fi_rmtagno IN FRAME FRAME-A /* From RM Tag# */
DO:
    DEF VAR op-error AS LOG NO-UNDO.
    DEF VAR vicount AS INT INIT 0 NO-UNDO.
    if self:screen-value gt "" THEN DO:
      ASSIGN vl-run-from-mach   = NO
             vl-InputParameters = NO.
      APPLY 'value-changed' TO tg_wip-reported.
    END.
...
DO:
    ...
    THEN DO:
        ...
        ASSIGN tt-rmtags.rmtagno = SELF:SCREEN-VALUE.
    END.
    IF fi_toRMTag:SCREEN-VALUE <> "" THEN 
        ASSIGN vl-run-from-mach   = NO
               vl-InputParameters = NO.
...
/* security check need {methods/prgsecur.i} in definition section */
  ...
  ASSIGN vl-InputParameters = IF INDEX(PROGRAM-NAME(2),"b-issued") > 0
                                OR INDEX(PROGRAM-NAME(2),"machtran") > 0 THEN TRUE
                              ELSE FALSE.
  ASSIGN vl-run-from-mach = IF INDEX(PROGRAM-NAME(2),"machtran") > 0 THEN TRUE
                              ELSE FALSE.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    IF vl-InputParameters THEN DO:
        FOR EACH wiptag NO-LOCK WHERE wiptag.company = g_company AND
                                      ...
                                      (IF ipc-i-no GT "" THEN wiptag.rm-i-no = ipc-i-no ELSE TRUE) AND
...
------------------------------------------------------------------------------*/
  ...
  IF vl-run-from-mach THEN DO:
    ...
    ENABLE ed_singletags fi_toRMTag reprintTag fi_rmtagno 
           tg_wip-reported fi_pallet_height fi_exp-sheets-roll
           WITH FRAME FRAME-A IN WINDOW C-Win.

    tg_wip-reported:SCREEN-VALUE = "yes".
    ASSIGN fi_exp-sheets-roll:SCREEN-VALUE =  string(ipi-run-qty).
    ASSIGN
           begin_mach:SCREEN-VALUE = ipc-machine.
  END.
    ASSIGN fi_rmtagno:SCREEN-VALUE = ipc-RMTagNo
           ...
           fi_exp-sheets-roll:SCREEN-VALUE = STRING(ipi-run-qty).

    /* wfk - when running from machine, this blanks out form-no */
    IF NOT vl-run-from-mach THEN
...
------------------------------------------------------------------------------*/
  ...
  DO:
     ...
     FOR EACH job-mat WHERE
         ...
         (IF begin_rm-i-no = "" THEN TRUE ELSE job-mat.rm-i-no = begin_rm-i-no) /* btr 02151105 */
         NO-LOCK,
         FIRST ITEM FIELDS(i-no i-name cons-uom loc loc-bin) WHERE
               ITEM.company EQ cocode AND
               ITEM.i-no EQ job-mat.rm-i-no AND
               ITEM.i-no GE begin_rm-i-no AND
               (IF end_rm-i-no = "" THEN TRUE ELSE ITEM.i-no LE end_rm-i-no) AND
               ITEM.mat-type EQ 'B'
               NO-LOCK,
         FIRST job-hdr FIELDS(cust-no i-no due-date est-no) WHERE
               job-hdr.company EQ job-mat.company AND
               ...
               IF (v-sheets-tag LE 0 OR
                  v-total-wip-tags LE 0) AND
                  (fi_rmtagno GT "" OR fi_toRMtag GT "") THEN
                  LEAVE.
   ...
   DO WITH FRAME {&FRAME-NAME}:
      ...
      IF begin_job NE "" THEN DO:
        ...
        IF AVAIL job THEN DO:
           ...
           {sys\inc\roundup.i v-exp-sheets}
           IF NOT (vl-run-from-mach AND ipi-run-qty GT 0) THEN
             fi_exp-sheets-roll:SCREEN-VALUE = STRING(v-exp-sheets).
        END. /* if avail job */
         ...
         IF AVAIL job THEN DO:
            IF ipc-machine GT "" THEN
                FIND FIRST job-mch WHERE
                     ...
                     job-mch.job-no2 = job.job-no2 AND
                     job-mch.m-code  = ipc-machine
                     use-index line-idx
                     NO-LOCK NO-ERROR.
