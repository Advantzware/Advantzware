/* ------------------------------------------- cec/quote/quolylng.p 10/02 YSK */
/* print quotes in Loylang and LoylangBSF - Xprint format                                               */
/* -------------------------------------------------------------------------- */
...
DEF VAR trim-size-d AS cha NO-UNDO.
...
DEF VAR lc-prod-cat LIKE eb.procat NO-UNDO. /*for determining whether to display BSF*/
DEF VAR li-case-cnt LIKE eb.cas-cnt INIT 1 NO-UNDO. /*for case UOM conversion to BSF*/
...
IF AVAIL sys-ctrl AND sys-ctrl.char-fld = "LoylangBSF" THEN lv-print-bsf = YES.
    ELSE lv-print-bsf = NO.  /* controls the switch between Loylang and LoylangBSF*/
...
DO:
    FIND FIRST sys-ctrl-shipto NO-LOCK
        ...
        AND sys-ctrl-shipto.cust-vend-no = xquo.cust-no NO-ERROR.
...
find first est where est.company = xquo.company
  ...
  if avail cust then
     ASSIGN v-over-under = trim(string(cust.over-pct,">>9.9<%")) + "-" +
...
if (not ch-multi) then do:
  ...
  ASSIGN
    ...
    DO:       
        PUT "<P10><C1>All tooling (Print Dies, Cutting Dies, or Prepress) is additional unless specified." SKIP
           "<C1>Pricing is subject to review of final artwork files." SKIP.
       v-printline = v-printline + 1.
    END.
    ELSE IF s-note-mode EQ "Fold" THEN
    DO:       
        PUT "<P10><C1>All tooling included unless otherwise specified.  Prepress is additional." SKIP
           "<C1>Pricing is subject to review of final artwork files." SKIP.
       v-printline = v-printline + 1.
    END.
    ...
    DO:       
        PUT "<P10><C1>For Corrugated items, all tooling (Print Dies, Cutting Dies, or Prepress) is additional unless" SKIP
           ...
           "<C1>specified.  Pricing is subject to review of final artwork files." SKIP.
       v-printline = v-printline + 1.
    END.

    /*PUT "<P10><R60><C1><#6>Quote valid for 30 Days" SKIP.*/

    PUT "<P10><C1><#6>Quote valid for 30 Days" SKIP.
 ...
 /*     IF s-note-mode EQ "Corr" THEN
      DO:
         PUT "<P10><C1>All tooling (Print Dies, Cutting Dies, or Prepress) is additional unless specified.  Standard" SKIP
             ...
             "<C1>files.  Prices are based on 'run and ship' unless otherwise specified." SKIP.
         v-printline = v-printline + 1.
      END.
      ELSE IF s-note-mode EQ "Fold" THEN
      DO:
         PUT "<P10><C1>All tooling included unless otherwise specified.  Prepress is additional.  Standard orders" SKIP
             ...
             "<C1>Prices are based on 'run and ship' unless otherwise specified." SKIP.
         v-printline = v-printline + 1.
      END.
      ELSE IF s-note-mode EQ "Corr/Fold" THEN
      DO:
         PUT "<P10><C1>For Corrugated items, all tooling (Print Dies, Cutting Dies, or Prepress) is additional unless" SKIP
             ...
             "<C1>review of final artwork files.  Prices are based on 'run and ship' unless otherwise specified." SKIP.
