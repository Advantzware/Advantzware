...
&Scoped-Define ENABLED-OBJECTS btn-save btn-unitcalc Btn-Cancel 
...
DEFINE FRAME Panel-Frame
     ...
     btn-unitcalc AT ROW 1.48 COL 17
     Btn-Cancel AT ROW 1.48 COL 32
...
ON CHOOSE OF btn-save IN FRAME Panel-Frame /* Override Unit */
DO:
...
&ENDIF
  DO:
     ...
     DO WITH FRAME Panel-Frame:
        IF Btn-Save:LABEL = '&Override Unit' THEN 
        DO:
           assign btn-unitcalc:sensitive = no    
        ...
        DO: /* Save */
           RUN notify ('update-record':U).
           {methods/setButton.i Btn-Save "Update"} /* added by script _admTransPanels.p */
           assign btn-unitcalc:sensitive = YES.
        END.                              
     END.
     ELSE 
...
DO:
  DO WITH FRAME Panel-Frame:
     ...
     disable btn-save btn-unitcalc with frame {&frame-name}.
...
------------------------------------------------------------------------------*/
  if btn-save:sensitive in frame {&frame-name}
  then btn-save:sensitive in frame {&frame-name} = no.
  else btn-save:sensitive in frame {&frame-name} = yes.
