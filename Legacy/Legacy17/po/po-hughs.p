/* -------------------------------------------------------------------------- */ 
/* po/po-xprnt.p                                                              */
/* Purchase Order Print Program for POPRINT = Hughes (copy of Xprint)                      */
/*                                                                            */
/* -------------------------------------------------------------------------- */
...
ASSIGN ls-image1 = "images\hughesdc.jpg"
       ls-image2 = ""
       ...
       /* v-job-no = po-ordl.job-no + "-" + STRING(po-ordl.job-no2,">>").*/
        ...
        ELSE do:
            ...
            IF AVAIL itemfg THEN DO:
                ...
                IF AVAIL eb THEN DO:
                    ASSIGN
                        lv-flute = "  Flute: " + eb.flute
                        lv-reg-no = "Test: " + eb.test.
                    RELEASE eb.
                END.
                ELSE
                    ASSIGN 
        ...
        IF v-wid GT 0 THEN DO:
          PUT "W: " AT 25 FNmetric(v-wid, v-wid2, "W") FORMAT "x(6)" SPACE(2).
          IF v-len GT 0 THEN
          PUT "L: "       FNmetric(v-len, v-len2, "L") FORMAT "x(10)" SPACE(2).
          IF lv-dep GT 0 THEN
          PUT "D: "       FNmetric(lv-dep, lv-dep2, "D") FORMAT "x(6)" SPACE(9).
        END.
        IF lv-flute NE "" THEN
