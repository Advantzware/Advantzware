...
/*------------------------------------------------------------------------------
  Purpose:     import RFID file, create receipt record, post
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  REPEAT:
    ...
    /* expands the scope, not sure why ... so don't do this
    errStatus = OS-ERROR.
    IF errStatus NE 0 THEN DO:
      RUN monitorActivity ('ERROR: Renaming ' + monitorFile,YES,'').
      NEXT.
    END. */
    ...
    ASSIGN
      ...
      SUBSTR(labelLine,44) = 'To'
      SUBSTR(labelLine,53) = 'Load'
      SUBSTR(labelLine,62) = 'Date'
      SUBSTR(labelLine,73) = 'Time'
      SUBSTR(labelLine,82) = 'Status'
      .
    ...
    REPEAT:
      ...
      ASSIGN
        ...
        SUBSTR(dataLine,44) = rfidData.toLocation
        SUBSTR(dataLine,53) = rfidData.loadNumber
        SUBSTR(dataLine,62) = STRING(rfidData.transDate,'99/99/9999')
        SUBSTR(dataLine,73) = rfidData.transTime
        .

      FIND FIRST rfidtag NO-LOCK
           WHERE rfidtag.company EQ g_company
             AND rfidtag.rfidtag EQ rfidData.rfidTag NO-ERROR.
      IF AVAILABLE rfidtag THEN DO:
        FIND FIRST loadtag NO-LOCK
             WHERE loadtag.company EQ rfidtag.company
               AND loadtag.item-type EQ rfidtag.item-type
               AND loadtag.tag-no EQ rfidtag.tag-no NO-ERROR.
        IF AVAILABLE loadtag THEN DO:
          ...
          IF AVAILABLE itemfg THEN DO:
            IF rfidData.transType EQ 'Move' THEN DO:
              FIND FIRST fg-bin NO-LOCK
                   WHERE fg-bin.company EQ loadtag.company
                     ...
                     AND fg-bin.qty GT 0 NO-ERROR.
              IF NOT AVAILABLE fg-bin THEN DO:
                SUBSTR(dataLine,82) = 'ERROR: FG Bin Record Does Not Exist'.
           ...
           /* ELSE RELEASE fg-bin. */
            ...
            END. /* do transaction */
          END. /* avail itemfg */
          ELSE SUBSTR(dataLine,82) = 'ERROR: FG Item Record Does Not Exist'.
        END. /* avail loadtag */
        ELSE SUBSTR(dataLine,82) = 'ERROR: LoadTag Record Does Not Exist'.
      END. /* avail rfidtag */
      ELSE SUBSTR(dataLine,82) = 'ERROR: RFIDTag Record Does Not Exist'.
      RUN monitorActivity (dataLine,NO,'').
    END. /* repeat */
