...
DEFINE VARIABLE h_b-chkrec AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-chkrec AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Check Reconciliation */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Check Reconciliation */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ap/b-chkrec.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-chkrec ).
       RUN set-position IN h_b-chkrec ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-chkrec ( 19.52 , 100.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2') NO-ERROR.

       /* Links to SmartNavBrowser h_b-chkrec. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-chkrec ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ap/v-chkrec.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-chkrec ).
       RUN set-position IN h_v-chkrec ( 5.52 , 7.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.62 , 141.00 ) */
       ...
       /* Links to SmartViewer h_v-chkrec. */
       RUN add-link IN adm-broker-hdl ( h_b-chkrec , 'Record':U , h_v-chkrec ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_v-chkrec ).
