/* ap-ckada.p  Check print for Adapt-A-Pak Laser */
...
def TEMP-TABLE wrk-chk NO-UNDO
...
form 
     ...
     "Check No: " AT 25 ap-chk.check-no
   ...
   for each ap-sel
    ...
    if ll eq max-per-chk or last(ap-sel.inv-no) then do:
      ...
      if last(ap-sel.inv-no) then do: 
        ...
        IF NOT last(ap-chk.vend-no) THEN
           put skip(14 - lv-line-cnt ).
      ...
      ELSE IF ll-void AND NOT last(ap-chk.vend-no) THEN
           PUT SKIP(17 - lv-line-cnt) /* no total frame b3 */.
