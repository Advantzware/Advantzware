...
DEFINE VARIABLE h_prgrms AS HANDLE NO-UNDO.
DEFINE VARIABLE h_prgrms-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_prgrms-3 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Program Master */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Program Master */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_options ).
       RUN set-position IN h_options ( 1.00 , 9.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'smartobj/prgrms.w':U ,
             INPUT  FRAME OPTIONS-FRAME:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_prgrms-3 ).
       RUN set-position IN h_prgrms-3 ( 1.00 , 65.00 ) NO-ERROR.
       ...
       RUN set-size IN h_folder ( 19.29 , 104.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/prgrms.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_prgrms ).
       RUN set-position IN h_prgrms ( 4.81 , 1.00 ) NO-ERROR.
       RUN set-size IN h_prgrms ( 17.14 , 106.20 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_prgrms. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_prgrms ).
       RUN add-link IN adm-broker-hdl ( h_prgrms , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_prgrms ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_prgrms-2 ).
       RUN set-position IN h_prgrms-2 ( 4.81 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 19.81 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 19.81 , 48.00 ) NO-ERROR.
