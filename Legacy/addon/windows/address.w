...
{methods/defines/address.i &NEW="NEW"}
...
DEFINE VARIABLE h_address-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     headervalue AT ROW 14.81 COL 2 NO-LABEL
...
ON END-ERROR OF W-Win /* Address Information */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Address Information */
DO:
...
&Scoped-define FRAME-NAME OPTIONS-FRAME
&Scoped-define SELF-NAME Btn_Address
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn_Address W-Win
ON CHOOSE OF Btn_Address IN FRAME OPTIONS-FRAME
DO:
  RUN Get_Procedure IN Persistent-Handle ('phone.',OUTPUT run-proc,no).
  IF run-proc NE "" THEN
  {methods/smartrun.i (address.rec_key,"{methods/headers/address.i}")}
END.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_smartmsg ).
       RUN set-position IN h_smartmsg ( 1.00 , 45.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_exit ).
       RUN set-position IN h_exit ( 1.00 , 114.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.81 , 7.80 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Browse Addr|View Address' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 3.14 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.43 , 121.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/address.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_address ).
       RUN set-position IN h_address ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_address ( 9.29 , 117.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2') NO-ERROR.

       /* Links to SmartNavBrowser h_address. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_address ).
       RUN add-link IN adm-broker-hdl ( h_address , 'Record':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/address.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_address-2 ).
       RUN set-position IN h_address-2 ( 4.57 , 13.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.38 , 100.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 12.19 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 12.19 , 65.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav ( 2.14 , 56.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1') NO-ERROR.

       /* Links to SmartViewer h_address-2. */
       RUN add-link IN adm-broker-hdl ( h_address , 'Record':U , h_address-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_address-2 ).
