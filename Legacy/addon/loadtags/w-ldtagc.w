...
DEFINE VARIABLE h_b-ldtagc AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Case Label Maintenance */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Case Label Maintenance */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brwse Loadtag|View Case Label' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/loadtags/b-ldtagc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ldtagc ).
       RUN set-position IN h_b-ldtagc ( 4.81 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-ldtagc ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_b-ldtagc. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-ldtagc ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-ldtagc ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/loadtags/v-ldtagi.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ldtagi ).
       RUN set-position IN h_v-ldtagi ( 5.76 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 15.24 , 145.00 ) */
       ...
       /* Links to SmartViewer h_v-ldtagi. */
       RUN add-link IN adm-broker-hdl ( h_b-ldtagc , 'Record':U , h_v-ldtagi ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_v-ldtagi ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-ldtagi ,
             h_folder , 'AFTER':U ).
