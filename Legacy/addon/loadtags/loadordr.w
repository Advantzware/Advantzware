...
DEFINE TEMP-TABLE ttbl NO-UNDO
  ...
  FIELD qty-case LIKE loadtag.qty-case
  FIELD case-bundle LIKE loadtag.case-bundle
  FIELD qty LIKE loadtag.qty
...
&Scoped-Define SORTBY-PHRASE BY oe-ordl.i-no
...
&Scoped-define INTERNAL-TABLES oe-ord oe-ordl ttbl ttbl-roll

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE Browser-Table                                 */
&Scoped-define FIELDS-IN-QUERY-Browser-Table oe-ord.ord-no oe-ord.ord-date ~
oe-ord.cust-no oe-ord.cust-name oe-ordl.i-no oe-ordl.i-name oe-ordl.job-no ~
oe-ordl.job-no2 oe-ordl.qty 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH oe-ord WHERE ~{&KEY-PHRASE} ~
      AND oe-ord.company = g_company NO-LOCK, ~
...
&Scoped-define FIELDS-IN-QUERY-ttbl-browse ttbl.ord-no ttbl.i-no ttbl.i-name ttbl.job-no ttbl.job-no2 ttbl.qty ttbl.loc ttbl.bin /* ttbl.s-wid ttbl.s-len */ ttbl.qty-case ttbl.case-bundle ttbl.no-of-tags ttbl.count ttbl.partial   
&Scoped-define ENABLED-FIELDS-IN-QUERY-ttbl-browse ttbl.loc ttbl.bin ttbl.qty-case ttbl.case-bundle  ttbl.no-of-tags  ttbl.count  ttbl.partial   
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 101 BY 7.86
         FONT 2.
...
DEFINE FRAME F-Main
     ...
     Btn_Add AT ROW 1.24 COL 103
     Btn_Select-PO AT ROW 2.43 COL 103 HELP
          "Unselect Records"
     Btn_Unselect-PO AT ROW 3.62 COL 103 HELP
          "Unselect Records"
     Btn_Select-Tag AT ROW 4.81 COL 103 HELP
          "Unselect Records"
     Btn_Delete AT ROW 6 COL 103
...
/* Query rebuild information for BROWSE Browser-Table
     ...
     _FldNameList[3]   = ASI.oe-ord.cust-no
     _FldNameList[4]   = ASI.oe-ord.cust-name
     _FldNameList[5]   = ASI.oe-ordl.i-no
     _FldNameList[6]   = ASI.oe-ordl.i-name
     _FldNameList[7]   = ASI.oe-ordl.job-no
     _FldNameList[8]   = ASI.oe-ordl.job-no2
     _FldNameList[9]   = ASI.oe-ordl.qty
...
DO:
  ...
  DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS:
    ...
    IF CAN-FIND(first ttbl WHERE ttbl.ord-no = oe-ord.ord-no
                       AND ttbl.i-no = oe-ordl.i-no                       
                   /*    AND ttbl.s-wid = itemfg.s-wid
                       AND ttbl.s-len = itemfg.s-len*/ ) THEN
    NEXT.
    CREATE ttbl.
    ASSIGN      
      ttbl.job-no = oe-ordl.job-no
      ttbl.job-no2 = oe-ordl.job-no2
      ttbl.ord-no = oe-ord.ord-no
      ttbl.i-no = oe-ordl.i-no
      ttbl.i-name = oe-ordl.i-name
      /*ttbl.loc = oe-ordl.loc
      ttbl.bin = oe-ordl.loc-bin
      ttbl.s-wid = itemfg.s-wid
      ttbl.s-len = itemfg.s-len*/
      ttbl.qty = oe-ordl.qty
      ttbl.qty-case = oe-ordl.cas-cnt
      ttbl.case-bundle = 1
    ...
    IF oe-ordl.est-no <> "" THEN DO:
           FIND FIRST eb WHERE eb.company = oe-ordl.company
                           ...
                           AND eb.form-no = oe-ordl.form-no
                           AND eb.blank-no = oe-ordl.blank-no
                           NO-LOCK NO-ERROR.
           IF AVAIL eb THEN ASSIGN ttbl.qty-case = eb.cas-cnt
                                   ttbl.case-bundle = eb.cas-pal
                                   ttbl.count = ttbl.qty-case * ttbl.case-bundle
                                   ttbl.no-of-tags = (ROUND(oe-ordl.qty / ttbl.COUNT,0) /* + .49*/ ) + 1 
                                   .
    END.
    ...
    find first itemfg where itemfg.company eq oe-ordl.company
                    ...
                    no-lock no-error.
    IF AVAIL itemfg THEN ASSIGN ttbl.loc = itemfg.def-loc
...
ON CHOOSE OF Btn_Delete IN FRAME F-Main /* Delete Selected Lines from Tag */
DO:
...
DO:
  ...
  DEF VAR lv-partial-created AS LOG NO-UNDO.
...
/*****************/
  ...
  FOR EACH ttbl EXCLUSIVE-LOCK:
    ...
    do i = 1 to (ttbl.no-of-tags /*+ (if ttbl.partial gt 0 then 1 else 0)*/ ):
      ...
      ELSE assign fg-rdtl.qty = ttbl.qty
                  fg-rctd.qty = ttbl.qty.
      ...
      IF NOT lv-partial-created AND i eq ttbl.no-of-tags  then
      do x = 1 to v-num-tags:
 ...
 */ 
  /*OS-COMMAND NO-WAIT VALUE("c:\progra~1\bar-on~1\labels.exe c:\ba\taglabel.lab " ).*/
  DOS  VALUE("c:\progra~1\bar-on~1\labels.exe c:\ba\taglabel.lab " ).
