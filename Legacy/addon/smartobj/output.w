...
&Scoped-Define ENABLED-OBJECTS Btn_Quick_Print RECT-4 RECT-2 RECT-1 RECT-3 ~
output-option lines-per-page 
&Scoped-Define DISPLAYED-OBJECTS output-option spool spool-date spool-time ~
spool-ampm lines-per-page orientation 
...
DEFINE VARIABLE lines-per-page AS INTEGER FORMAT ">>9":U INITIAL 33 
...
DEFINE VARIABLE orientation AS CHARACTER INITIAL "Portrait" 
...
DEFINE VARIABLE output-option AS INTEGER INITIAL 2 
...
DEFINE FRAME F-Main
     ...
     lines-per-page AT ROW 4.1 COL 49 COLON-ALIGNED HELP
          "Enter Lines Per Page"
     orientation AT ROW 5.76 COL 30 HELP
          "Select Viper Page Orientation" NO-LABEL
...
/* SETTINGS FOR RADIO-SET orientation IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       orientation:PRIVATE-DATA IN FRAME F-Main     = 
                "export".
...
&Scoped-define SELF-NAME orientation
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL orientation s-object
ON VALUE-CHANGED OF orientation IN FRAME F-Main
DO:
  lines-per-page:SCREEN-VALUE = IF SELF:SCREEN-VALUE = 'Portrait' THEN '46'
                                ELSE '34'.
END.
...
DO:
  ...
  IF {&SELF-NAME} = 4 THEN
  DO:
    ENABLE orientation WITH FRAME {&FRAME-NAME}.
    APPLY 'VALUE-CHANGED' TO orientation.
  END.
  ELSE
  DISABLE orientation WITH FRAME {&FRAME-NAME}.
END.
