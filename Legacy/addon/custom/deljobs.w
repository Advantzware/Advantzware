...
end_job-no2 Btn_OK Btn_Cancel RECT-18 
...
DEFINE FRAME F-Main
     ...
     Btn_OK AT ROW 13.38 COL 16
     Btn_Cancel AT ROW 13.38 COL 44
...
ON END-ERROR OF W-Win /* Purge completed jobs */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Purge completed jobs */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_job-no W-Win
ON LEAVE OF begin_job-no IN FRAME F-Main /* Beginning Job# */
DO:
  assign {&self-name}.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_job-no2 W-Win
...
ON CHOOSE OF Btn_OK IN FRAME F-Main /* Delete */
DO:
    message "Are you sure you want to delete the jobs within the " +
        "selection parameters?"
        view-as alert-box warning button yes-no update v-process AS LOG.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_job-no W-Win
ON LEAVE OF end_job-no IN FRAME F-Main /* Ending Job# */
DO:
  assign {&self-name}.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_job-no2 W-Win
ON LEAVE OF end_job-no2 IN FRAME F-Main
DO:
  assign {&self-name}.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE run-process W-Win 
...
DEF VAR tmp-job-no LIKE begin_job-no NO-UNDO.
DEF VAR tmp-job-no2 LIKE begin_job-no2 NO-UNDO.
  ...
  for each jobs
      where jobs.job ge begin_job-no
        ...
        and jobs.cadcam_status ne "Pending"
      exclusive-lock  transaction:  

      ASSIGN tmp-job-no = SUBSTRING(jobs.job,1,index(jobs.job,"-") - 1)
             tmp-job-no2 = SUBSTRING(jobs.job,index(jobs.job,"-") + 1).

      FIND FIRST job WHERE job.job-no = tmp-job-no
                       AND job.job-no2 = int(tmp-job-no2) NO-LOCK NO-ERROR.
      IF AVAIL job AND job.stat = "C" AND job.close-date <> ? THEN DO:
