...
lSSBolEmailPassed = FALSE.
  FOR EACH tt-email WHERE tt-email.release# EQ ip-release# BREAK BY tt-email.done-what:
      ...
      FIND FIRST asinos.users NO-LOCK WHERE asinos.users.user_id EQ oe-ord.user-id NO-ERROR.
      IF AVAIL asinos.users AND asinos.users.image_filename NE '' AND
         NOT CAN-DO(ls-to-list,asinos.users.image_filename) THEN 
         ...
         FOR EACH asinos.phone NO-LOCK
             ...
             IF CAN-FIND(FIRST emaildtl
                         WHERE (emaildtl.emailcod EQ 'ERELEASE'
                           ...
                           AND asinos.phone.e_mail NE ''
                           AND NOT CAN-DO(ls-to-list,asinos.phone.e_mail) THEN
