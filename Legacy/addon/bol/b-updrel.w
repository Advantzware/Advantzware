...
DEFINE TEMP-TABLE tt-del-ssrel
    FIELD ssrelbol-rowid AS ROWID.
...
&Scoped-define SELF-NAME scr-rel#
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL scr-rel# B-table-Win
ON LEAVE OF scr-rel# IN FRAME F-Main /* Release# */
DO:
   DO WITH FRAME {&FRAME-NAME}:
      IF INT(scr-rel#:SCREEN-VALUE) NE scr-rel# THEN
      DO:
         ASSIGN scr-rel#.
         RUN release#-value-changed (INPUT scr-rel#).
      END.
   END.
END.
...
/*ON 'entry':U OF tt-relbol.trailer# */
/* DO: */
/*   IF AVAIL tt-relbol THEN */
/*     DISP tt-relbol.tag# WITH BROWSE */ /* {&browse-name}.*/
/* END. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE apply-entry-rel B-table-Win 
PROCEDURE apply-entry-rel :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE display-qtys-query B-table-Win 
PROCEDURE display-qtys-query :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE INPUT PARAMETER lv-release# AS INT NO-UNDO.
  DEF INPUT PARAMETER lv-i-no AS CHAR NO-UNDO.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    IF lv-release# NE 0 AND TRIM(lv-i-no) NE "" THEN DO:
      ...
      FOR EACH b-relh FIELDS(r-no) NO-LOCK
          WHERE b-relh.company  EQ cocode
            AND b-relh.RELEASE# EQ lv-release#,
...
DEF VAR liOrderChosen       AS INT NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL tt-relbol THEN
  DO:
     FIND FIRST ssrelbol WHERE
          ROWID(ssrelbol) EQ tt-relbol.ROWID
          NO-ERROR.

     IF AVAIL ssrelbol THEN
        DELETE ssrelbol.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF ERROR-STATUS:ERROR THEN RETURN.
/*   WFK - this is nto in b-relbol so removing it */
/*   IF ssbolscan-cha GT "" THEN        */
/*     RUN validate-trailer NO-ERROR.   */
/*   IF ERROR-STATUS:ERROR THEN RETURN. */
...
------------------------------------------------------------------------------*/
 ...
 /* IF v-scan-qty LT v-rel-qty THEN
  ...
  EMPTY TEMP-TABLE tt-del-ssrel.

  /* Handles creating ssrelboll and oe-rell for added records */
  RUN create-temp-rel(INPUT v-create-backorder).
  
  /* Create tt-del-ssrel so we know what to delete after posting */
  FOR EACH tt-relbol.
      IF NOT CAN-FIND (FIRST ssrelbol 
                         WHERE ROWID(ssrelbol) EQ tt-relbol.ROWID) THEN DO:
          FIND FIRST ssrelbol WHERE ssrelbol.release# EQ tt-relbol.release#
                                AND ssrelbol.i-no     EQ tt-relbol.i-no
                                AND ssrelbol.i-name     EQ tt-relbol.i-name
                                AND ssrelbol.ord-no     EQ tt-relbol.ord-no
                                AND ssrelbol.cust-no     EQ tt-relbol.cust-no
                                AND ssrelbol.LINE     EQ tt-relbol.LINE
                NO-ERROR.
      ...
      CREATE tt-del-ssrel.
      ASSIGN ssrelbol-rowid = tt-relbol.ROWID.
  END.

  RUN post-release.

  /* Special for this program, not in b-relbol */
  FOR EACH tt-relbol,
      FIRST ssrelbol WHERE
            ROWID(ssrelbol) EQ tt-relbol.ROWID:
  ...
  FOR EACH tt-del-ssrel,
      FIRST ssrelbol WHERE
            ROWID(ssrelbol) EQ tt-del-ssrel.ssrelbol-rowid:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE release#-value-changed B-table-Win 
PROCEDURE release#-value-changed :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   ...
   FOR EACH ssrelbol WHERE
       ssrelbol.company EQ cocode AND
       ssrelbol.release# EQ ip-release#
       NO-LOCK:

       CREATE tt-relbol.
       BUFFER-COPY ssrelbol TO tt-relbol
          ASSIGN tt-relbol.ROWID = ROWID(ssrelbol).
       RELEASE tt-relbol.
   END.

   FIND FIRST tt-relbol NO-ERROR.
   IF AVAIL tt-relbol THEN
      RUN display-qtys-query(INPUT ip-release#,
                             INPUT tt-relbol.i-no).
...
------------------------------------------------------------------------------*/
  ...
  /* Only run this once per order / item since it examines all oe-boll lines */
  FOR EACH tt-boll 
    BREAK BY tt-boll.ord-no 
          BY tt-boll.i-no:
    IF LAST-OF(tt-boll.i-no) THEN DO:
      FIND FIRST oe-boll 
        WHERE oe-boll.company EQ tt-boll.company
          AND oe-boll.bol-no  EQ tt-boll.bol-no
          AND oe-boll.i-no    EQ tt-boll.i-no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE update-ssrelbol B-table-Win 
PROCEDURE update-ssrelbol :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   IF tt-relbol.ROWID NE ? THEN
   DO:
      FIND FIRST ssrelbol WHERE
           ROWID(ssrelbol) EQ tt-relbol.ROWID
           NO-ERROR.
