...
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY BROWSE-1 FOR EACH cust WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      cust.cust-no
      cust.name
      cust.addr[1] COLUMN-LABEL "Address"
      cust.addr[2] COLUMN-LABEL "Address"
      cust.city
      cust.state
      cust.zip
/* _UIB-CODE-BLOCK-END */
...
DEFINE FRAME Dialog-Frame
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Finished Good Information".
...
"addr[1]" "Address" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[4]   > ASI.cust.addr[2]
"addr[2]" "Address" ? "character" ? ? ? ? ? ? no ?
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Finished Good Information */
DO:
