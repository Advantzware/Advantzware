...
DEFINE VARIABLE h_v-postct AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* FINISHED GOODS  PHYSICAL COUNT PROCESSING */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* FINISHED GOODS  PHYSICAL COUNT PROCESSING */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/fg/b-phys.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-phys ).
       RUN set-position IN h_b-phys ( 5.05 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-phys ( 15.48 , 145.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 22.24 , 17.80 ) NO-ERROR.
       RUN set-size IN h_p-updsav ( 1.76 , 85.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'fg/pv-trans.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_pv-trans ).
       RUN set-position IN h_pv-trans ( 22.24 , 102.80 ) NO-ERROR.
       /* Size in UIB:  ( 1.76 , 17.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/fg/v-postct.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-postct ).
       RUN set-position IN h_v-postct ( 22.24 , 119.60 ) NO-ERROR.
       /* Size in UIB:  ( 1.76 , 17.00 ) */
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_pv-trans ,
             h_p-updsav , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-postct ,
             h_pv-trans , 'AFTER':U ).
    END. /* Page 1 */
