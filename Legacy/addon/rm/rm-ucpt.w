...
&SCOPED-DEFINE winViewPrgmName rm-ucpt
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "WAREHOUSE TRANSACTION RECEIPT UPDATE (RAW MATERIALS)"
         HEIGHT             = 24.19
         WIDTH              = 150
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 204.8
...
ON END-ERROR OF W-Win /* WAREHOUSE TRANSACTION RECEIPT UPDATE (RAW MATERIALS) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* WAREHOUSE TRANSACTION RECEIPT UPDATE (RAW MATERIALS) */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/rm/v-ucptd.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ucptd ).
       RUN set-position IN h_v-ucptd ( 4.81 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.43 , 116.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/rm/b-ucptd.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_b-ucptd ).
       /* Position in AB:  ( 6.48 , 4.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updcan ).
       RUN set-position IN h_p-updcan ( 22.43 , 4.00 ) NO-ERROR.
