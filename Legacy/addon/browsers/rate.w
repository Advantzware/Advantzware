...
DEFINE VARIABLE calculated-rate AS DECIMAL NO-UNDO.
...
&Scoped-define INTERNAL-TABLES rate

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE Browser-Table                                 */
&Scoped-define FIELDS-IN-QUERY-Browser-Table rate.rate_usage rate.shift ~
rate.machine rate.ratetype rate.rate rate.factortype 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH rate OF employee WHERE ~{&KEY-PHRASE} ~
      AND ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH rate OF employee WHERE ~{&KEY-PHRASE} ~
      AND ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD Employee-Rate B-table-Win 
...
DEFINE QUERY Browser-Table FOR 
      rate
    FIELDS(rate.rate_usage
      rate.shift
      rate.machine
      rate.ratetype
      rate.rate
      rate.factortype) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      rate.rate_usage COLUMN-LABEL "Rate Usage" FORMAT "Shift/Machine":U
            LABEL-BGCOLOR 14
      rate.shift FORMAT "XX":U LABEL-BGCOLOR 14
      rate.machine COLUMN-LABEL "Mach" FORMAT "x(6)":U LABEL-BGCOLOR 14
      rate.ratetype FORMAT "X(12)":U LABEL-BGCOLOR 14
      rate.rate FORMAT ">>>>9.99<<<":U LABEL-BGCOLOR 14
      rate.factortype FORMAT "X(12)":U LABEL-BGCOLOR 14
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 84 BY 16.19
         FONT 2.
...
DEFINE FRAME F-Main
     ...
     fi_sortby AT ROW 17.43 COL 21.4 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     auto_find AT ROW 17.43 COL 44 COLON-ALIGNED HELP
          "Enter Auto Find Value"
     Btn_Clear_Find AT ROW 17.43 COL 71 HELP
...
/* Query rebuild information for BROWSE Browser-Table
     _TblList          = "EMPTRACK.rate OF EMPTRACK.employee"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "USED"
     _Where[1]         = "~{&KEY-PHRASE}"
     _FldNameList[1]   > EMPTRACK.rate.rate_usage
"rate.rate_usage" "Rate Usage" ? "logical" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > EMPTRACK.rate.shift
"rate.shift" ? "XX" "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > EMPTRACK.rate.machine
"rate.machine" "Mach" ? "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > EMPTRACK.rate.ratetype
"rate.ratetype" ? ? "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > EMPTRACK.rate.rate
"rate.rate" ? ? "decimal" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > EMPTRACK.rate.factortype
"rate.factortype" ? ? "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION Employee-Rate B-table-Win 
FUNCTION Employee-Rate RETURNS DECIMAL
  (ip-company AS CHARACTER,
   ip-employee AS CHARACTER,
   ip-shift AS CHARACTER,
