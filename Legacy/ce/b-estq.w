...
&SCOPED-DEFINE sortby-log                             ~
    ...
    IF lv-sort-by EQ "eqty"  THEN STRING(est-qty.eqty,"9999999999")  ELSE ~
    ...
    IF lv-sort-by EQ "cust-%"  THEN STRING(9999999999 - eb.cust-%,"9999999999")  ELSE ~
...
eb.part-no display-qty() @ est-qty.eqty est-qty.eqty ~
...
DEFINE VARIABLE vi_dep-2 AS DECIMAL FORMAT "->>,>>9.99999":U INITIAL 9999.99999 
     VIEW-AS FILL-IN 
     SIZE 12 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE vi_len-2 AS DECIMAL FORMAT "->>,>>9.99999":U INITIAL 9999.99999 
     VIEW-AS FILL-IN 
     SIZE 12 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE VARIABLE vi_wid-2 AS DECIMAL FORMAT "->>,>>9.99999":U INITIAL 9999.99999 
     VIEW-AS FILL-IN 
     SIZE 12 BY 1
     BGCOLOR 15  NO-UNDO.
...
"display-qty-set() @ eb.cust-%" ? ? ? ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > ASI.eb.cust-%
"eb.cust-%" "Qty/Set" "->>>>>>9" "decimal" ? ? ? 14 ? ? yes ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[13]   > "_<CALC>"
"display-qty-set() @ eb.cust-%" ? ? ? ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[14]   > ASI.eb.die-no
...
DO:
  ...
  END.
  */
   IF TG_exact-match:SCREEN-VALUE = "NO" THEN DO:
...
&SCOPED-DEFINE cellColumnDat ceb-estq /* careful here */
