...
/*       IF {1}.est-type EQ 1 THEN */
      ...
      do v-li = 1 TO 4:
         ...
         if avail b-item and b-item.mat-type eq "W" 
             AND (xef.leaf-l[v-li] ne 0 and xef.leaf-w[v-li] ne 0)
             AND (xef.leaf-bnum[v-li] EQ 0 OR xef.leaf-bnum[v-li] EQ {1}.blank-no) then
            DO:
        ...
        do i = 1 to 20:
          ...
          for each est-op
            ...
            if {1}.i-ps2[i] eq j then do:
              ...
              assign
               ink.i-%    = {1}.i-%2[i]
               ink.i-qty  = ink.i-qty + (IF FIRST-OF({1}.form-no) AND v-first-pass THEN mach.col-wastelb ELSE 0) +
                            ((({1}.t-sqin - v-t-win) *
