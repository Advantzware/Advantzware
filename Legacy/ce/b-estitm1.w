...
DEFINE VARIABLE lv-repo AS CHAR INIT "ON" NO-UNDO.
...
DO:
     ...
     WHEN "part-no" THEN DO:
      ...
      WHEN "procat" THEN DO:
       ...
       END.  /* ship-id */
       WHEN "bl-qty" THEN DO:
             FIND FIRST est-qty NO-LOCK WHERE est-qty.company EQ gcompany
    ...
    IF AVAILABLE tt-eb-set THEN
      ASSIGN
       ...
       eb.part-no:SCREEN-VALUE IN BROWSE {&browse-name}    =
            TRIM(SUBSTRING(tt-eb-set.part-no,1,li-flen[1])) + "-1-1"
...
DO:
   ...
   IF NOT AVAILABLE ITEM THEN DO:
      ...
      RETURN NO-APPLY.
   END.              
   IF AVAILABLE ITEM AND ef.medium:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} EQ '' THEN DO:
       FIND FIRST item-bom NO-LOCK
           WHERE item-bom.company EQ ITEM.company
             AND item-bom.parent-i EQ ITEM.i-no
             AND item-bom.line# EQ 1
            NO-ERROR.
       IF AVAILABLE item-bom THEN
           ef.medium:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = item-bom.i-no.
   END.
   IF AVAILABLE ITEM AND ef.flute:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} EQ '' THEN DO:
       FIND FIRST item-bom NO-LOCK
           WHERE item-bom.company EQ ITEM.company
             AND item-bom.parent-i EQ ITEM.i-no
      ...
      IF choice THEN DO i = 1 TO 12:
         ...
         ELSE IF (i GT INTEGER(eb.i-col)) AND
                 (i LE (INTEGER(eb.i-col) + 
                       INTEGER(eb.i-coat)) )
         THEN DO:
              FIND alt-item WHERE RECID(alt-item) EQ save_id2 NO-LOCK NO-ERROR.
              ASSIGN bf-eb.i-ps2[i]   = counter
...
------------------------------------------------------------------------------*/  
  ...
  FOR EACH bb WHERE bb.company  EQ ""
                AND bb.est-no  EQ "" :
      DELETE bb.
  END.
  FOR EACH bf WHERE bf.company  EQ "" AND bf.est-no  EQ "" :
      DELETE bf.
  END.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lvFlute LIKE ef.flute NO-UNDO.
  DEFINE VARIABLE lvMedium LIKE ef.medium NO-UNDO.
  DEFINE VARIABLE ld-markup AS DECIMAL NO-UNDO.
  DEFINE VARIABLE ll AS LOGICAL NO-UNDO.
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  IF NOT AVAILABLE eb THEN FIND eb WHERE RECID(eb) = lv-eb-recid NO-LOCK NO-ERROR.
  IF NOT AVAILABLE ef THEN FIND ef WHERE RECID(ef) = lv-ef-recid NO-LOCK NO-ERROR.
...
/*         IF AVAILABLE reftable AND reftable.code2 = "I" THEN DO: */
  ...
  DO TRANSACTION:
    ...
    IF NOT AVAILABLE est OR NOT AVAIL ef OR NOT AVAIL eb THEN DO:
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT  PARAMETER ip-nav-type AS CHARACTER.
  DEF OUTPUT PARAMETER op-nav-type AS CHARACTER.
