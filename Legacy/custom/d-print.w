...
DEF VAR v-session-printer AS CHAR NO-UNDO.
DEF VAR v-session-port AS CHAR NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-2 cb-printer v-copies Btn_OK ~
Btn_Cancel 
...
DO:
  ...
  */
  ...
  DO i = 1 TO v-copies:
      ...
      /*
      lv-prt-cmd = '"' + cb-printer + '"'.
      OS-COPY VALUE(ip-print-file) value(lv-prt-cmd) /*VALUE(cb-printer)*/.
      */                                                                   
  ...
  ELSE DO i = 1 TO v-copies:
      ...
      OS-COMMAND SILENT VALUE("copy " + ip-print-file + " " + lv-prt-port).
   END.
/* 
MESSAGE cb-printer skip
    lv-prt-port SKIP
    OS-ERROR VIEW-AS ALERT-BOX.  
*/
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  */
  ...
  ASSIGN
     v-session-printer = SESSION:PRINTER-NAME
     v-session-port    = SESSION:PRINTER-PORT.

  IF CAN-FIND(FIRST tt-printer WHERE
              tt-printer.prt-name = v-session-printer) THEN
     ASSIGN
        cb-printer:SCREEN-VALUE = v-session-printer
        lv-port:SCREEN-VALUE = v-session-port.
  ELSE
  DO:
     FIND FIRST tt-printer WHERE
          tt-printer.prt-num = 1.

     IF AVAIL tt-printer THEN
     DO:
        ASSIGN
           cb-printer:SCREEN-VALUE = tt-printer.prt-name
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 cb-printer v-copies Btn_OK Btn_Cancel 
      WITH FRAME Dialog-Frame.
