...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(256)":U INITIAL "c:~\tmp~\r-invbal.csv" 
...
DEFINE FRAME FRAME-A
     begin_cust AT ROW 3.62 COL 27 COLON-ALIGNED HELP
          "Enter Beginning Customer Number" WIDGET-ID 12
     ...
     begin_date AT ROW 4.81 COL 27 COLON-ALIGNED HELP
          "Enter Beginning BOL Date" WIDGET-ID 14
     end_date AT ROW 4.81 COL 70 COLON-ALIGNED HELP
          "Enter Ending BOL Date" WIDGET-ID 18
     rd-dest AT ROW 11.24 COL 6 NO-LABEL
     lv-ornt AT ROW 11.48 COL 30 NO-LABEL
     lines-per-page AT ROW 11.48 COL 84 COLON-ALIGNED
     lv-font-no AT ROW 13.19 COL 35 COLON-ALIGNED
     lv-font-name AT ROW 14.19 COL 28 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 15.43 COL 30
     tb_excel AT ROW 16.38 COL 30 WIDGET-ID 2
     tb_runExcel AT ROW 16.43 COL 55.4 WIDGET-ID 4
     fi_file AT ROW 17.81 COL 28 COLON-ALIGNED HELP
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "A/R Invoice Balance Due Utility"
         HEIGHT             = 21.81
         WIDTH              = 96.8
         MAX-HEIGHT         = 21.81
         MAX-WIDTH          = 96.8
...
ON END-ERROR OF C-Win /* A/R Invoice Balance Due Utility */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* A/R Invoice Balance Due Utility */
DO:
...
DO:
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN {&Displayed-objects} begin_cust end_cust begin_date end_date.
  END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_cust end_cust begin_date end_date rd-dest lv-ornt lines-per-page 
          lv-font-no lv-font-name td-show-parm tb_excel tb_runExcel fi_file 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-6 RECT-7 begin_cust end_cust begin_date end_date rd-dest lv-ornt 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE run-util C-Win 
PROCEDURE run-util :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
   ...
   def var t-credits as DEC format ">,>>>,>>>.99" NO-UNDO.
   def var t-debits  as DEC format ">,>>>,>>>.99" NO-UNDO.
   ...
   def var v-pay-stat1 as log  format "Y/N" NO-UNDO.
   def var v-pay-stat2 as char format "x(4)" NO-UNDO.
   def var x-check-no  as   char   format "x(10)" NO-UNDO.
   DEF VAR v-gltrans-desc AS CHAR NO-UNDO.

   FORM
      tt-inv.inv-no COLUMN-LABEL "Inv #" FORMAT ">>>>>9"
      tt-inv.inv-date COLUMN-LABEL "Inv Date" FORMAT "99/99/99"
      tt-inv.cust-no COLUMN-LABEL "Cust #" FORMAT "X(8)"
      tt-inv.cust-name COLUMN-LABEL "Cust Name" FORMAT "X(30)"
      tt-inv.due COLUMN-LABEL "Invoice Due" FORMAT "->>,>>>,>>9.99"
      tt-inv.due-calc COLUMN-LABEL "Calculated Due" FORMAT "->>,>>>,>>9.99"
      SKIP
      WITH FRAME ar-inv NO-BOX down stream-io WIDTH 100.
   ...
   FOR EACH ar-inv WHERE
       ...
       ar-inv.cust-no GE begin_cust AND
       ar-inv.cust-no LE END_cust AND
       ...
       FOR EACH tt-arinq WHERE
           BREAK BY tt-arinq.seq:

           IF LAST(tt-arinq.seq) AND
              tt-arinq.balance NE ar-inv.due THEN
              DO:
                 CREATE tt-inv.
                 ASSIGN tt-inv.inv-no = ar-inv.inv-no
                        tt-inv.cust-no = ar-inv.cust-no
                        tt-inv.cust-name = cust.NAME
                        tt-inv.due     = ar-inv.due
                        tt-inv.due-calc = tt-arinq.balance
   ...
   FOR EACH tt-inv:
       ...
       IF tb_excel THEN
          PUT STREAM s-temp 
             ...
             '"' STRING(tt-inv.inv-date,"99/99/99") '",'
             '"' tt-inv.cust-no          '",'
             '"' tt-inv.cust-name        '",'
             '"' STRING(tt-inv.due)      '",'
