...
DEFINE VARIABLE h_b-purgeFolders AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'util/b-purgeFolders.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-purgeFolders ).
       RUN set-position IN h_b-purgeFolders ( 7.05 , 7.00 ) NO-ERROR.
       RUN set-size IN h_b-purgeFolders ( 24.52 , 234.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-purgeFolders ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  RUN disable-enable-buttons IN h_b-purgeFolders (INPUT "Disable").
...
------------------------------------------------------------------------------*/
  ...
  RUN pass-temp-table IN h_b-purgeFolders (INPUT TABLE tt-itemfg).
