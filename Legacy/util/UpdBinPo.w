...
&Scoped-Define ENABLED-OBJECTS begin_i-no end_i-no btn-ok btn-cancel 
...
SUBSCRIBE TO "NumDel" ANYWHERE.
  ...
  END.
    ...
    {methods/setButton.i btn-ok "OK"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:44:01 am */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE begin_i-no end_i-no btn-ok btn-cancel 
      WITH FRAME FRAME-A IN WINDOW C-Win.
      ...
      DEF VAR cCalcPoNo AS CHAR NO-UNDO.
       ...
       FOR EACH fg-rcpth 
         WHERE fg-rcpth.company   EQ cocode
           ...
           AND fg-rcpth.po-no     GT ""
           /* AND fg-rcpth.rita-code EQ "R" */
           NO-LOCK USE-INDEX tdate,
           EACH fg-rdtlh fields() WHERE
                 fg-rdtlh.r-no EQ fg-rcpth.r-no 
            ...
            IF LAST-OF(fg-rdtlh.loc-bin) THEN DO:
              /* Assign PO number */
              FIND FIRST fg-bin 
                 WHERE fg-bin.company       EQ fg-rdtlh.company
                     AND fg-bin.i-no        EQ fg-rcpth.i-no
              ...
              IF AVAIL fg-bin AND fg-bin.job-no GT "" THEN DO:               
                  NEXT.
              END.

              IF AVAIL fg-bin AND cCalcPoNo GT "" THEN
                  fg-bin.po-no = cCalcPoNo.                                               
