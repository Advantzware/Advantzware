...
&Scoped-define FIELDS-IN-QUERY-Browser-Table utilities.programName utilities.module ~
utilities.notes utilities.description utilities.hotkey utilities.securityLevel
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH utilities WHERE ~{&KEY-PHRASE} ~
      AND utilities.programName EQ "###"  NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH utilities WHERE ~{&KEY-PHRASE} ~
      AND utilities.programName EQ "###" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
fi_desc btn_go btn_show /*btn_prev btn_next */ ~
Browser-Table 
&Scoped-Define DISPLAYED-OBJECTS fi_pro-name fi_module fi_hotkey ~
fi_desc fi_sort-by FI_moveCol 
...
DEFINE QUERY Browser-Table FOR 
      utilities
    FIELDS(utilities.programName
      utilities.module
      utilities.hotkey
      utilities.description
      utilities.notes
      utilities.securityLevel ) SCROLLING.
&ANALYZE-RESUME
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      ...
      utilities.module  COLUMN-LABEL "Module" FORMAT "x(3)":U WIDTH 8.2 LABEL-BGCOLOR 14
      utilities.hotkey COLUMN-LABEL "Hot Key" FORMAT "x(3)":U LABEL-BGCOLOR 14
      utilities.DESCRIPTION COLUMN-LABEL "Description" FORMAT "x(48)":U  WIDTH 29 LABEL-BGCOLOR 14
      utilities.notes COLUMN-LABEL "Notes" FORMAT "x(200)":U WIDTH 29 LABEL-BGCOLOR 14
      utilities.securityLevel COLUMN-LABEL "Security Level"  LABEL-BGCOLOR 14
      
      ENABLE
      utilities.programName          
      utilities.module               
      utilities.hotkey                  
      utilities.description                  
      utilities.notes                 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 130 BY 13.90
         FONT 2.
...
DEFINE FRAME F-Main
     ...
     fi_module AT ROW 2.19 COL 27.6 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     fi_hotkey AT ROW 2.19 COL 43.6 COLON-ALIGNED NO-LABEL WIDGET-ID 40
     fi_desc AT ROW 2.19 COL 70.2 COLON-ALIGNED NO-LABEL WIDGET-ID 16
     
     btn_go AT ROW 3.62 COL 1.8 WIDGET-ID 4
     btn_show AT ROW 3.62 COL 15.2 WIDGET-ID 10
     fi_sort-by AT ROW 3.62 COL 54 COLON-ALIGNED NO-LABEL WIDGET-ID 12
     FI_moveCol AT ROW 3.62 COL 120 COLON-ALIGNED NO-LABEL WIDGET-ID 46
...
"utilities.module" "Module" ? "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"utilities.description" "Description" ? "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    _FldNameList[5]   > ASI.utilities.notes
"utilities.notes" ? ? "character" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[6]   > ASI.utilities.securityLevel
"utilities.securityLevel" "Security Level" ? "integer" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  IF AVAIL utilities THEN APPLY "value-changed" TO BROWSE {&browse-name}.
...
------------------------------------------------------------------------------*/
  ...
  RUN setCellColumns.
  /* Code placed here will execute AFTER standard behavior.    */
  ASSIGN utilities.programName:READ-ONLY IN BROWSE {&browse-name} = YES
         utilities.module:READ-ONLY IN BROWSE {&browse-name} = YES
         utilities.hotkey:READ-ONLY IN BROWSE {&browse-name} = YES
         utilities.description:READ-ONLY IN BROWSE {&browse-name} = YES
         utilities.notes:READ-ONLY IN BROWSE {&browse-name} = YES
         utilities.securityLevel:READ-ONLY IN BROWSE {&browse-name} = YES
         FI_moveCol = "Sort".
...
GET FIRST Browser-Table .
ASSIGN first-cust = utilities.programName .
GET LAST Browser-Table .
ASSIGN last-cust = utilities.programName .

RUN util/util-exp.w (first-cust ,last-cust).
