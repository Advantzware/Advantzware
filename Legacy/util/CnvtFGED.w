...
DEFINE VARIABLE tb_force AS LOGICAL INITIAL no 
     LABEL "Only update if DB field is No" 
     VIEW-AS TOGGLE-BOX
     SIZE 32 BY .95 NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Convert Exempt Discount to DB Field"
         HEIGHT             = 12.48
         WIDTH              = 90.2
         MAX-HEIGHT         = 19.76
         MAX-WIDTH          = 98.2
...
ON END-ERROR OF C-Win /* Convert Exempt Discount to DB Field */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Convert Exempt Discount to DB Field */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   RUN enable_UI.
  {methods/nowait.i}
    {methods/setButton.i btn-cancel "Cancel"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:43:58 am */
    {methods/setButton.i btn-process "Start"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:43:58 am */
...
DEFINE BUFFER bf-itemfg-unlock FOR itemfg.
...
DEFINE VARIABLE lExempt AS LOGICAL   NO-UNDO.
...
FOR EACH bf-company NO-LOCK,
    ...
    FOR EACH bf-reftable
        WHERE bf-reftable.reftable EQ "itemfg.exempt-disc"
          AND bf-reftable.company  EQ bf-itemfg.company
          AND bf-reftable.loc      EQ ""   
          AND bf-reftable.CODE     EQ bf-itemfg.i-no
          EXCLUSIVE-LOCK:
        lExempt = IF bf-reftable.val[1] = 1 THEN YES ELSE NO.
        IF lPurgeReftable THEN DELETE bf-reftable. 
    END.
    IF (lUpdateOnlyIfBlank AND NOT bf-itemfg.exempt-disc) OR NOT lUpdateOnlyIfBlank THEN DO:
        bf-itemfg.exempt-disc = lExempt.  
    END.
END. /* each itemfg */
