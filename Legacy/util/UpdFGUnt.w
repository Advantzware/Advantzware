...
DEFINE VARIABLE h_b-itemfgh-2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'FG Items' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 3.38 , 3.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 28.57 , 229.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'util/b-itemfgh.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-itemfgh-2 ).
       RUN set-position IN h_b-itemfgh-2 ( 5.76 , 7.00 ) NO-ERROR.
       /* Size in UIB:  ( 24.29 , 214.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-itemfgh-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
