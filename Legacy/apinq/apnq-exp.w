...
ASSIGN cTextListToSelect = "Invoice#,Vendor#,Invoice Date,Due Date,Net,Paid,Discount,Balance Due,Tax Code,Discount%," +
...
{sys/inc/ttRptSel.i}
    ...
    ASSIGN cTextListToDefault  = "Invoice#,Vendor#,Invoice Date,Due Date,Net,Paid,Discount,Balance Due,Tax Code,Discount%," +
                                               "Status,Tax,Freight,User,PO Number,Line,Account Number,Account Description,Quantity,UOM," +
                                               "Price,UOM Price,Tax2,SqFt,Amount,Total MSF,Item#,Description,Job#,Sheet#,Blank#".
...
DEF VAR v-lineno AS INT NO-UNDO.
       ...
       FOR EACH ttRptSelected:
         /* IF LOOKUP(ttRptSelected.TextList, "Contact,Title,Email") <> 0    THEN do: */
              ...
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-invl.po-no)).
               IF ttRptSelected.TextList = "Line" THEN do:
              ...
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-invl.amt-msf)).
               IF ttRptSelected.TextList = "Item#" THEN do:
                   IF AVAIL ap-invl THEN
              ...
              v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(ap-invl.dscr)).
               IF ttRptSelected.TextList = "Job#" THEN do:
                   IF AVAIL ap-invl THEN
                       FIND FIRST po-ordl WHERE po-ordl.company = cocode
                       AND po-ordl.po-no = ap-invl.po-no
                       AND po-ordl.LINE = ap-invl.LINE - (ap-invl.po-no * 1000)
                   ...
                   ELSE
                       v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string("")).
               END.
               IF ttRptSelected.TextList = "Sheet#" THEN do:
                   IF AVAIL ap-invl THEN
                       FIND FIRST po-ordl WHERE po-ordl.company = cocode
                       AND po-ordl.po-no = ap-invl.po-no
                       AND po-ordl.LINE = ap-invl.LINE - (ap-invl.po-no * 1000)
                   ...
                   ELSE
                       v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string("")).
               END.
               IF ttRptSelected.TextList = "Blank#" THEN do:
                   IF AVAIL ap-invl THEN
                       FIND FIRST po-ordl WHERE po-ordl.company = cocode
                       AND po-ordl.po-no = ap-invl.po-no
                       AND po-ordl.LINE = ap-invl.LINE - (ap-invl.po-no * 1000)
