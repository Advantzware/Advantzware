...
&SCOPED-DEFINE h_Browse01 h_b-apibal
...
DEFINE VARIABLE h_b-apibal AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'AP Invoices' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       ...
       RUN add-link IN adm-broker-hdl ( h_b-apibal , 'attachinv':U , h_attachvinv ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'apinq/b-apibal.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-apibal ).
       RUN set-position IN h_b-apibal ( 4.57 , 2.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-apibal , 'export-xl':U , h_export ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-apibal ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
