   ...
   do transaction on error undo:
     ...
     for each rm-rcpt
         where rm-rcpt.company   eq cocode
           and rm-rcpt.rita-code eq "C"
         use-index rita,

         each rm-rdtl
         where rm-rdtl.r-no eq rm-rcpt.r-no

         break by rm-rcpt.i-no

         with frame itemx:

       display rm-rcpt.trans-date when first-of(rm-rcpt.i-no)
               rm-rcpt.i-no       when first-of(rm-rcpt.i-no)
               rm-rcpt.i-name
               rm-rdtl.loc
               rm-rdtl.loc-bin
               rm-rdtl.tag
       ...
       v-cum-qty[1] = v-cum-qty[1] + rm-rdtl.qty.

       if last-of(rm-rcpt.i-no) then do:
         display v-cum-qty[1].
         put skip(1).
         assign
          v-cum-qty[2] = v-cum-qty[2] + v-cum-qty[1]
          v-cum-qty[1] = 0.
         if last(rm-rcpt.i-no) then
...
{sys/form/r-topw.f}
      ...
      form
          rm-rcpt.trans-date label "DATE"
          rm-rcpt.i-no label "ITEM"
          rm-rcpt.i-name label "DESCRIPTION"
          rm-rdtl.loc label "WHSE"
          rm-rdtl.loc-bin label "BIN"
          rm-rdtl.tag label "TAG#"
          rm-rdtl.qty format "->>>>>9.99<<" label "QUANTITY"
          v-cum-qty[1] column-label "TOTAL  !QUANTITY "
      WITH FRAME itemx no-box down STREAM-IO width 132.

      form
    rm-bin.i-no    label "ITEM"
    item.i-name    label "DESCRIPTION"
    item.cc-code   label "CYCLE CODE"
    rm-bin.loc     label "WHSE"
    rm-bin.loc-bin label "BIN"
    rm-bin.tag     label "TAG"
    rm-bin.qty     label "BIN QTY"
    item.cons-uom  label "UOM"
    rm-rdtl.qty    label "QTY COUNTED" format "->>>,>>9.999"
    ...
    for each rm-rcpt where rm-rcpt.company = cocode and
       rm-rcpt.rita-code = "C" break by rm-rcpt.i-no with frame itemx:
      find first item where item.company = rm-rcpt.company and
                   item.i-no    = rm-rcpt.i-no no-lock no-error.
      for each rm-rdtl where rm-rdtl.r-no = rm-rcpt.r-no
         break by rm-rdtl.r-no with frame itemx:
        find rm-bin where rm-bin.company = rm-rdtl.company and
                          rm-bin.loc     = rm-rdtl.loc and
                          rm-bin.loc-bin = rm-rdtl.loc-bin and
                          rm-bin.tag     = rm-rdtl.tag and
                          rm-bin.i-no    = rm-rcpt.i-no no-error.

        if not available rm-bin then do:
          create rm-bin.
          assign rm-bin.company = rm-rdtl.company
                 rm-bin.loc     = rm-rdtl.loc
                 rm-bin.loc-bin = rm-rdtl.loc-bin
                 rm-bin.tag     = rm-rdtl.tag
                 rm-bin.i-no    = rm-rcpt.i-no
                 rm-bin.qty     = 0.
        end.
        if line-counter > 56 then page.
        if v-avgcost then
          dollar-chg = (rm-rdtl.qty - rm-bin.qty) * item.avg-cost.
        else dollar-chg = (rm-rdtl.qty - rm-bin.qty) * item.last-cost.
         display
             rm-bin.i-no    when first-of(rm-rdtl.r-no)
             item.i-name    when first-of(rm-rdtl.r-no)
             item.cc-code   when first-of(rm-rdtl.r-no)
             rm-bin.loc
             rm-bin.loc-bin
             rm-bin.tag
             rm-bin.qty
             item.cons-uom
             rm-rdtl.qty    (sub-total by rm-rcpt.i-no)
             dollar-chg     (sub-total by rm-rcpt.i-no).
         down.
      end. /* each rm-rdtl */
    end. /* each rm-rcpt */
