...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF VAR v-form AS INT NO-UNDO.
  DEF VAR v-blank AS INT NO-UNDO.
  ...
  IF valid-handle(widget-handle(char-hdl)) THEN
      RUN get-form-blank IN WIDGET-HANDLE(char-hdl) (OUTPUT v-form,OUTPUT v-blank).

  IF v-form <> 0 AND v-blank <> 0 THEN
     FIND FIRST bf-eb WHERE bf-eb.company = est-qty.company
                        AND bf-eb.est-no = est-qty.est-no 
                        AND bf-eb.form-no = v-form
                        AND bf-eb.BLANK-no = v-blank NO-LOCK NO-ERROR.
  IF AVAIL bf-eb THEN DO:
     
     REPOSITION {&browse-name} TO ROWID ROWID(bf-eb).
     RUN dispatch ("row-changed").   
     
     /*RUN dispatch ('get-current':U). */
     MESSAGE AVAIL eb eb.blank-no VIEW-AS ALERT-BOX.
  END.
