...
&Scoped-Define ENABLED-OBJECTS RECT-1 btn-copy 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE enable-copy V-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  DO:
     DO num-groups = 1 TO NUM-ENTRIES(g_groups):
        ...
        group-ok = yes.
     END.
     IF NOT CAN-DO(TRIM(b-prgrms.can_update),USERID("ASI")) AND
        NOT group-ok THEN
  ...
  IF NOT ip-enable AND v-can-update THEN 
