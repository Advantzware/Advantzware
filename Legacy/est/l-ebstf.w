...
&scoped-define FLD-NAME-1 eb.stock-no
&scoped-define FLD-NAME-2 eb.part-dscr1
&scoped-define FLD-NAME-3 eb.cust-no
&scoped-define FLD-NAME-4 eb.part-no
&scoped-define FLD-NAME-5 eb.est-no 

&scoped-define DATATYPE-1 STRING
&scoped-define IDXNAME1 stock
&scoped-define IDXNAME2 pdscr
&scoped-define IDXNAME3 cust
&scoped-define IDXNAME4 part
&scoped-define IDXNAME5 est-no
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH eb WHERE ~{&KEY-PHRASE} ~
      AND eb.company = ip-company AND eb.est-no = "999999999999" ~
...
and itemfg.stat = "A" ~
and itemfg.i-no = eb.stock-no NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* Query rebuild information for BROWSE BROWSE-1
     _TblList          = "ASI.eb,ASI.ef OF ASI.eb,ASI.est OF ASI.eb,ASI.itemfg OF ASI.ef"
 ...
 AND ((eb.est-type ge 1 and eb.est-type le 4 and ip-est-type ge 1 and ip-est-type le 4) or
      (eb.est-type gt 4 and ip-est-type gt 4))
  AND rowid(eb) ne ip-rowid"
     _Where[4]         = "itemfg.company = eb.company
and itemfg.stat = ""A""
...
DO:
  ...
  DO idx = 1 TO columnCount:
    ASSIGN
        cellColumn[idx]:BGCOLOR = IF v-fg-est-no = eb.est-no THEN 4 ELSE ?.
  END. /* do idx */
