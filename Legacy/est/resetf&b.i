  ...
  FOR EACH xeb-form-ef
    ...
    DO li = 1 TO EXTENT(xeb-form-ef.leaf):
      IF xeb-form-ef.leaf-bnum[li] EQ {2} THEN xeb-form-ef.leaf-bnum[li] = {4}.
    END.
    DO li = 1 TO EXTENT(xeb-form-ef.mis-cost):
      IF xeb-form-ef.mis-bnum[li] EQ {2} THEN xeb-form-ef.mis-bnum[li] = {4}.
    END.
  END.
  ...
  FOR EACH reftable
    ...
    reftable.code = TRIM(est.est-no) + STRING({3},"/99").
  END.
