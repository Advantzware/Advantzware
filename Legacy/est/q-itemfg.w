...
&Scoped-define QUERY-STRING-Query-Main FOR EACH itemfg WHERE itemfg.company = eb.company ~
  AND (asi.itemfg.i-no = eb.stock-no ~
    or eb.stock-no = "") OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH itemfg WHERE itemfg.company = eb.company ~
  AND (asi.itemfg.i-no = eb.stock-no ~
    or eb.stock-no = "") OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "asi.itemfg WHERE asi.eb <external> ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "OUTER"
     _JoinCode[1]      = "asi.itemfg.company = asi.eb.company
  AND (asi.itemfg.i-no = asi.eb.stock-no
    or eb.stock-no = """")"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-eb-record q-tables 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
 DEF OUTPUT PARAM op-eb-recid AS RECID.
