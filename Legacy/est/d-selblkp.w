...
DEF OUTPUT PARAM op-prep AS LOG NO-UNDO .
...
&Scoped-Define ENABLED-OBJECTS BROWSE-2 tb_prep Btn_OK Btn_Select ~
Btn_Deselect Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS tb_prep 
...
DEFINE VARIABLE tb_prep AS LOGICAL INITIAL no 
     LABEL "Update Prep Code if it already exists?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME D-Dialog
     ...
     tb_prep AT ROW 13.67 COL 16.2
     Btn_OK AT ROW 15.24 COL 7
     Btn_Select AT ROW 15.24 COL 24
     Btn_Deselect AT ROW 15.24 COL 41
     Btn_Cancel AT ROW 15.24 COL 58
     SPACE(3.99) SKIP(0.71)
...
DO: 
    ASSIGN op-prep = IF tb_prep:SCREEN-VALUE = "yes" THEN TRUE ELSE FALSE .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tb_prep D-Dialog
ON VALUE-CHANGED OF tb_prep IN FRAME D-Dialog /* Update Prep Code if it already exists? */
DO:
  assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY tb_prep 
      WITH FRAME D-Dialog.
  ENABLE BROWSE-2 tb_prep Btn_OK Btn_Select Btn_Deselect Btn_Cancel 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
