...
and eb.loc = ip-loc ~
 AND (eb.est-type gt 4 and ip-est-type gt 4) ~
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  DO WITH FRAME {&FRAME-NAME}:
    CASE rd-sort:
      WHEN 5 THEN DO:
        {est/l-ebc.i 5 &USE-INDEX="use-index stock"}
      END.

      WHEN 4 THEN DO:
        {est/l-ebc.i 4 &USE-INDEX="use-index pdscr" }
      END.
   
      WHEN 3 THEN DO:
        {est/l-ebc.i 3 &USE-INDEX="use-index part"}
      END.

      WHEN 2 THEN DO:
        {est/l-ebc.i 2 &USE-INDEX="use-index cust"}
      END.

      OTHERWISE DO:
        {est/l-ebc.i 1 &USE-INDEX="use-index estcomp"}
      END.
    END CASE.
  END.
