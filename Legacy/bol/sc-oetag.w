...
&Scoped-Define ENABLED-OBJECTS Scan_itemfg BUTTON-1 Scan_order Btn_Close 
&Scoped-Define DISPLAYED-OBJECTS Scan_itemfg Scan_order 
...
DEFINE VARIABLE Scan_itemfg AS CHARACTER FORMAT "X(256)":U 
     LABEL "Scan Fg Item#" 
     VIEW-AS FILL-IN 
     SIZE 40 BY 1
     FONT 6 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     Scan_itemfg AT ROW 1.71 COL 14.2 COLON-ALIGNED
     BUTTON-1 AT ROW 1.71 COL 83.6
     Scan_Tag AT ROW 3.14 COL 14 COLON-ALIGNED
     BUTTON-2 AT ROW 3.86 COL 83.6
     Scan_order AT ROW 4.81 COL 14 COLON-ALIGNED
...
/* SETTINGS FOR FILL-IN Scan_Tag IN FRAME DEFAULT-FRAME
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
       Scan_Tag:HIDDEN IN FRAME DEFAULT-FRAME           = TRUE.
...
ON END-ERROR OF C-Win /* Scan  Tag# */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Scan  Tag# */
DO:
...
*/
  assign scan_itemfg:screen-value = "10x10x10       /0001".

  assign v-text = ""
         v-num = 1.
  do x = 1 to length(trim(scan_itemfg:screen-value)):
    if substring(scan_itemfg:screen-value,x,1) ne "/" then
      assign v-text[v-num] = v-text[v-num] + substring(scan_order:screen-value,x,1).
    else
      v-num = v-num + 1.
  end.

  find first oe-ordl where po-ordl.company eq gcompany
                       and po-ordl.po-no eq int(v-text[1])
                       and po-ordl.i-no eq v-text[2] 
                     no-lock no-error.
  if not avail oe-ordl then
  do:
    message "Item is not on Order. Please Check Order Label." view-as alert-box.
...
DO:
 /*ssign rm-rdtl.s-num = int(Scan_Form:screen-value). */
END.
...
------------------------------------------------------------------------------*/
  DISPLAY Scan_itemfg Scan_order 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE Scan_itemfg BUTTON-1 Scan_order Btn_Close 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
