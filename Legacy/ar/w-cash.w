...
&SCOPED-DEFINE h_Browse01 h_b-cash
...
DEF VAR lv-applied-all AS LOG NO-UNDO.
...
DEFINE VARIABLE h_b-cash AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-cashl AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-cash AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Cash Receipts */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Cash Receipts */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Cash|View Cash' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cash ).
       RUN set-position IN h_b-cash ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-cash ( 19.52 , 145.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'win':U , h_b-cash ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cash ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_v-cash ).
       RUN set-position IN h_v-cash ( 4.81 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.19 , 123.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-arinv ).
       RUN set-position IN h_p-arinv ( 4.81 , 127.00 ) NO-ERROR.
       RUN set-size IN h_p-arinv ( 6.19 , 20.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ar/b-cashl.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_b-cashl ).
       RUN set-position IN h_b-cashl ( 11.48 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-cashl ( 10.95 , 145.00 ) NO-ERROR.
       ...
       RUN set-size IN h_p-navico ( 1.67 , 55.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-cashl ).
       RUN set-position IN h_p-cashl ( 22.91 , 75.00 ) NO-ERROR.
       RUN set-size IN h_p-cashl ( 1.52 , 67.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-cash. */
       RUN add-link IN adm-broker-hdl ( h_b-cash , 'Record':U , h_v-cash ).
       RUN add-link IN adm-broker-hdl ( h_b-cashl , 'invhead':U , h_v-cash ).
       RUN add-link IN adm-broker-hdl ( h_p-arinv , 'TableIO':U , h_v-cash ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_v-cash ).
       RUN add-link IN adm-broker-hdl ( h_v-cash , 'arcash':U , THIS-PROCEDURE ).

       /* Links to SmartNavBrowser h_b-cashl. */
       RUN add-link IN adm-broker-hdl ( h_b-cash , 'browse':U , h_b-cashl ).
       RUN add-link IN adm-broker-hdl ( h_p-cashl , 'TableIO':U , h_b-cashl ).
       RUN add-link IN adm-broker-hdl ( h_v-cash , 'Record':U , h_b-cashl ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-line':U , h_b-cashl ).

       /* Links to SmartPanel h_p-cashl. */
       RUN add-link IN adm-broker-hdl ( h_v-cash , 'adding-line':U , h_p-cashl ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-arinv ,
             h_v-cash , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cashl ,
             h_p-arinv , 'AFTER':U ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE check-aramt W-Win 
PROCEDURE check-aramt :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
     ...
     IF NOT lv-applied-all THEN RETURN ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE refresh-detail W-Win 
...
RUN refresh-detail IN h_b-cashl.
END PROCEDURE.
