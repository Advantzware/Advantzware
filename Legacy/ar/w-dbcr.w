...
&SCOPED-DEFINE h_Browse01 h_b-dbcr
...
DEFINE VARIABLE h_b-dbcr AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-dbcr2 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-dbcr AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vp-arhld AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Credit/Debit Memo Entry & Editing"
         HEIGHT             = 23.81
         WIDTH              = 150
         MAX-HEIGHT         = 320
         MAX-WIDTH          = 320
...
ON END-ERROR OF W-Win /* Credit/Debit Memo Entry  Editing */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Credit/Debit Memo Entry  Editing */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse Memos|View Memos' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-arinv ).
       RUN set-position IN h_p-arinv ( 4.57 , 103.00 ) NO-ERROR.
       RUN set-size IN h_p-arinv ( 6.43 , 20.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_v-dbcr ).
       RUN set-position IN h_v-dbcr ( 4.81 , 6.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.95 , 90.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ar/vp-arhld.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vp-arhld ).
       RUN set-position IN h_vp-arhld ( 4.81 , 124.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 13.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ar/b-dbcr2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_b-dbcr2 ).
       RUN set-position IN h_b-dbcr2 ( 11.48 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-dbcr2 ( 10.00 , 145.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 22.19 , 7.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 1.91 , 55.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'panels/p-cashll.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Update,
                     AddFunction = One-Record':U ,
             OUTPUT h_p-cashl ).
       RUN set-position IN h_p-cashl ( 22.19 , 66.00 ) NO-ERROR.
       RUN set-size IN h_p-cashl ( 1.76 , 82.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-dbcr. */
       RUN add-link IN adm-broker-hdl ( h_b-dbcr , 'Record':U , h_v-dbcr ).
       RUN add-link IN adm-broker-hdl ( h_b-dbcr2 , 'invhead':U , h_v-dbcr ).
       RUN add-link IN adm-broker-hdl ( h_p-arinv , 'TableIO':U , h_v-dbcr ).
       RUN add-link IN adm-broker-hdl ( h_vp-arhld , 'hold':U , h_v-dbcr ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_v-dbcr ).

       /* Links to SmartNavBrowser h_b-dbcr2. */
       RUN add-link IN adm-broker-hdl ( h_b-dbcr , 'browse':U , h_b-dbcr2 ).
       RUN add-link IN adm-broker-hdl ( h_p-cashl , 'TableIO':U , h_b-dbcr2 ).
       RUN add-link IN adm-broker-hdl ( h_v-dbcr , 'Record':U , h_b-dbcr2 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-arinv ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-dbcr ,
             h_p-arinv , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vp-arhld ,
             h_v-dbcr , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dbcr2 ,
             h_vp-arhld , 'AFTER':U ).
