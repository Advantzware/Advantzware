  ...
  */
            ...
            assign v-inv-qty = ar-invl.qty
                ...
                if ar-invl.tax and avail stax then
                   v-line-tax-amt = v-line-tax-amt + ar-invl.amt.

                do i = 1 to 3:
                   if stax.tax-code[i] ne "" then do:
                      run ar/calctax2.p (stax.tax-code[i],no,ar-invl.amt,ar-invl.company,ar-invl.i-no, output v-tmp-tax).
       ...
       */     
        ...
        /* stacey - move up to process tax for each line item. */
/*         do i = 1 to 3:                                                                                               */
/*            if stax.tax-code[i] ne "" then do:                                                                        */
/*               run ar/calctax2.p (stax.tax-code[i],no,v-subtot-lines,ar-invl.company,ar-invl.i-no,output v-tmp-tax).  */
/*               v-t-tax[i] = v-t-tax[i] + v-tmp-tax.                                                                   */
/*            END.                                                                                                      */
/*         end.                                                                                                         */
