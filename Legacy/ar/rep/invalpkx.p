/* ---------------------------------------------- ar/rep/invalpkx.p   */
/* PRINT INVOICE   Xprint form for All Packaging             */
/* -------------------------------------------------------------------------- */
...
ASSIGN ls-image1 = "images\allpkg.jpg"
...
{ar/rep/invalpkx.i}
   ...
   /* gdm - 04020907 get the total tax from ar-inv */
   DO i = 1 to 3:

       IF i EQ 1 
         THEN
          v-t-tax[i] = ar-inv.tax-amt.
         ELSE v-t-tax[i] = 0.
   END.
   /* gdm - 04020907 end */
    ...
    do i = 1 to 3:
        v-bot-lab[i] = 
                      IF AVAIL stax AND stax.tax-code[i] <> "" THEN (string(CAPS(stax.tax-code[i]),"x(5)") +
                                         fill(" ",6) + ":" + string(v-t-tax[i],"->>>>>9.99"))
                      else "".                     
    end.
    ...
    PUT UNFORMATTED 
        "<R58><C3><#7><=7><R-0.5><FROM><R+4.5><C+32><RECT> " 
        "<=7><C5>Remit to: "
        "<=7><R+1><C5>All Packaging Company, Inc."
        "<=7><R+2><C5>PO Box 414287 "
        "<=7><R+3><C5>Kansas City, MO 64141".

    PUT "<R58><C60><#8><=8><R-0.5><FROM><R+6.5><C+20><RECT> " 
