/* ---------------------------------------------- ar/rep/invhpb.p  */
/* PRINT INVOICE   Xprint Standard Form  for HPB           */
/* -------------------------------------------------------------------------- */
  ...
  for each report where report.term-id eq v-term-id no-lock,
      ...
      find first stax
        ...
        for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no:
          assign v-case-line = ""
            ...
            PUT ar-invl.ord-no FORMAT ">>>>>9" SPACE(1)
                v-po-no
            ...
            do v = 1 to 2:
              ...
              if v-part-info ne "" OR ar-invl.part-no <> "" then do:
                 IF v = 1 THEN PUT SPACE(30) ar-invl.part-no v-part-info SKIP.
    ...
    PUT "<R54><C60><#8><FROM><R+5><C+20><RECT> " 
     ...
     IF cust.curr-code EQ "CAD" THEN
        PUT "<=8><R+4><C21>" v-currency-line.
     ELSE
        PUT "<=8><R+4><C26>" v-currency-line.

    PUT "<=8><R+5><C44><P14><B> Thank  You!</B><P10>"
