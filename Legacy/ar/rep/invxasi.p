...
def var v-termsdscr as char format "x(30)" NO-UNDO.
       ...
       for each ar-invl NO-LOCK where ar-invl.x-no  eq ar-inv.x-no  
        ...
        for each w-sman break by w-sman.sman:
          if first-of(w-sman.sman) then
            FIND FIRST sman                                     /*Task# 10301309*/
                      WHERE sman.company EQ cocode
                      AND sman.sman    EQ w-sman.sman
                      NO-LOCK NO-ERROR.
                  IF AVAIL sman THEN 
                      ASSIGN v-salesman = sman.sNAME .
          delete w-sman.
        end.
...
{ar/rep/invxasi.i}
        ...
        for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no
          ...
          PUT space(1) v-inv-qty format "->>>>9.9" SPACE(13)
              v-i-dscr  format "x(30)" SPACE(15)
    ...
    PUT "<R58><C60><#8><FROM><R+6><C+20><RECT> " 
        ...
        "<=8><R+3> " v-bot-lab[2]
        "<=8><R+4> " v-bot-lab[3]
        "<=8><R+5> Grand Total:" (IF v-invoice-with-order THEN ar-inv.gross 
                                 ELSE ar-inv.gross + ar-inv.tax-amt)
                                 + v-inv-freight FORM "->>,>>9.99" .
    ...
    PUT "<P9><U><B>Remit To:  PO Box 4018, Murrells Inlet, SC  29576</B></U></P9>"  .
