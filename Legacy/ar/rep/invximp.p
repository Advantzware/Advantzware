/* ---------------------------------------------- ar/rep/invximp.p   */
/* PRINT INVOICE   Xprint form for Imperial             */
/* -------------------------------------------------------------------------- */
    ...
    for each report where report.term-id eq v-term-id no-lock,
       ...
       for each ar-invl NO-LOCK
        ...
        for each ar-invl no-lock where ar-invl.x-no eq ar-inv.x-no
            ...
            IF FIRST-OF(ar-invl.b-no) THEN DO:
               FIND first oe-bolh no-lock where oe-bolh.b-no eq ar-invl.b-no NO-ERROR.
...
=========*/
        ...
        */
        IF v-printline > 46 AND ar-inv.bill-i[1] <> "" THEN DO:
              PAGE.
              v-printline = 0.
              {ar/rep/invximp.i}
        END.
        PUT "<R54><C1>" ar-inv.bill-i[1] FORM "x(60)" 
            "<R55><C1>" ar-inv.bill-i[2] FORM "x(60)" 
            "<R56><C1>" ar-inv.bill-i[3] FORM "x(60)" 
            "<R57><C1>" ar-inv.bill-i[4] FORM "x(60)" .
/*        IF v-printline > 50 THEN DO:
               PAGE.
               v-printline = 0.
               {ar/rep/invximp.i}
...
*/
   ...
   PUT "<R58><C58><#8><FROM><R+6><C+22><RECT> " 
        "<=8><C3><P12>Thank You For Your Business <P10><=8> Sub Total    :" v-subtot-lines FORM "$->>,>>9.99"
        "<=8><R+1> Freight      :" v-inv-freight FORM "$->>,>>9.99"
        "<=8><R+2> " v-bot-lab[1] 
        "<=8><R+3> " v-bot-lab[2]
        "<=8><R+4> " v-bot-lab[3]
        "<=8><R+5><B> Invoice Total:</B>" v-subtot-lines + v-t-tax[1] + v-t-tax[2] + v-t-tax[3] + v-inv-freight FORM "$->>,>>9.99" "</B>".
