/* ------------------------------------------ ar/rep/invsmkct.p 04270902 GDM */
...
ELSE lv-comp-color = "BLACK".
 ...
 END.
          ...
          assign v-case-line = ""
           ...
           END.
            ...
            END.
              ...
              assign v-bo-qty = if ( ar-invl.qty - v-ship-qty ) < 0 THEN 0
                                else ar-invl.qty - v-ship-qty.
    ...
    ASSIGN 
        lv-remit-name = "" v-remit-add1 = "" 
        v-remit-add2  = "" v-remit-add3 = ""
        v-remit-add4  = "" v-remit-add5 = "". 

    FIND FIRST bf-cust NO-LOCK
      WHERE bf-cust.company = cocode 
        AND bf-cust.cust-no = "REMITO" NO-ERROR.
    IF AVAIL bf-cust THEN
      ASSIGN 
        lv-remit-name = bf-cust.NAME  
        v-remit-add1 = bf-cust.addr[1]
        v-remit-add2 = bf-cust.addr[2]
        v-remit-add3 = bf-cust.city + ", " + bf-cust.state + "  " + bf-cust.zip.
    
    PUT "<FArial><R58><C1><#5><FROM><R+6><C+35><RECT>"
    ...
    PUT "<FLucida Console><R58><C59><#8><FROM><R+6><C+21><RECT> " 
        "<=8><R58> Sub Total" "<C69>:" v-subtot-lines     FORM "->>>,>>9.99"
        "<=8><R+1> Freight"   "<C69>:" v-inv-freight      FORM "->>>,>>9.99".

     IF TRIM(v-bot-lab[1]) NE ""
      THEN
       PUT "<=8><R+2> " TRIM(v-bot-lab[1]) "<C69>:" 
           v-bot-prc[1] FORM "->>>,>>9.99".

     IF TRIM(v-bot-lab[2]) NE ""
      THEN
       PUT "<=8><R+3> " TRIM(v-bot-lab[2]) "<C69>:" 
           v-bot-prc[2] FORM "->>>,>>9.99".

     IF TRIM(v-bot-lab[3]) NE ""
      THEN
       PUT "<=8><R+4> " TRIM(v-bot-lab[3]) "<C69>:" 
           v-bot-prc[3] FORM "->>>,>>9.99".
     PUT
        "<=8><R+5> Grand Total" "<C69>:"
