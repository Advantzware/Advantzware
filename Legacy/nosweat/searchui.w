...
/*------------------------------------------------------------------------
  ...
  Description:       Create/Update Search Browsers
...
&Scoped-Define ENABLED-OBJECTS m_search_prgm RECT-22 Btn_Test Btn_Cancel ~
Btn_OK 
&Scoped-Define DISPLAYED-OBJECTS m_search_prgm F1 
...
DEFINE VARIABLE m_search_prgm AS CHARACTER FORMAT "X(12)":U 
     LABEL "Search Browser Name" 
     VIEW-AS FILL-IN 
     SIZE 22.4 BY 1
     BGCOLOR 15  NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     m_search_prgm AT ROW 1.24 COL 28 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Create/Update Search Browsers"
         HEIGHT             = 4.52
         WIDTH              = 54.4
         MAX-HEIGHT         = 4.52
         MAX-WIDTH          = 65
...
ASSIGN
       Btn_Test:PRIVATE-DATA IN FRAME DEFAULT-FRAME     = 
                "ribbon-button".
...
ON END-ERROR OF C-Win /* Create/Update Search Browsers */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Create/Update Search Browsers */
DO:
...
DO:
  IF m_search_prgm:SCREEN-VALUE NE "" THEN
  DO:
    IF INDEX(m_search_prgm:SCREEN-VALUE,".") = 0 THEN
    m_search_prgm:SCREEN-VALUE = m_search_prgm:SCREEN-VALUE + ".".
    ASSIGN m_search_prgm.
    RUN Get_Procedure IN Persistent-Handle (INPUT "searches.",OUTPUT run-proc,no).
    IF run-proc NE "" THEN
    RUN VALUE(run-proc) (INPUT m_search_prgm:SCREEN-VALUE).
  END.
    {methods/setButton.i Btn_Cancel "Close"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:43:41 am */
...
DO:
  DEFINE VARIABLE drowid AS ROWID NO-UNDO.
...
&Scoped-define SELF-NAME m_search_prgm
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_search_prgm C-Win
ON HELP OF m_search_prgm IN FRAME DEFAULT-FRAME /* Search Browser Name */
DO:
  DEFINE VARIABLE m-lookup-var AS CHARACTER NO-UNDO.
  RUN "lookups/lkupprgm.p" (INPUT-OUTPUT m-lookup-var).
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  {methods/nowait.i}
    ...
    {methods/setButton.i Btn_Test "Test"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:43:41 am */
...
------------------------------------------------------------------------------*/
  DISPLAY m_search_prgm F1 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE m_search_prgm RECT-22 Btn_Test Btn_Cancel Btn_OK 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  {methods/setfocus.i m_search_prgm}
