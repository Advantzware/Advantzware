...
DEF INPUT PARAM ip-recalc-stds-ju1 AS LOG NO-UNDO.
DEF VAR gvlQtyFromJob AS LOG NO-UNDO INIT ?.
DEF VAR gvlNoPrompt AS LOG NO-UNDO.
...
DEF VAR type-chk AS CHAR INIT "C,D,F,G,I,L,M,P,R,T,V,W,B,1,2,3,4,5,6,7,8,9,X,Y,@" NO-UNDO.
...
DEF VAR ll-combo-req-qty AS LOG NO-UNDO.
DEF VAR ll-fgoecost AS LOG NO-UNDO.
DEF VAR ll-recalc-cost AS LOG NO-UNDO.
DEF VAR ll-oe-program AS LOG NO-UNDO.
...
DEF TEMP-TABLE tt-ordlist NO-UNDO
...
FIND FIRST sys-ctrl
    ...
    NO-LOCK NO-ERROR.
IF AVAIL sys-ctrl THEN ll-fgoecost = NOT sys-ctrl.log-fld.
...
DO:
  ...
  FOR EACH xeb where xeb.company = xest.company 
     ...
     /* choice = YES.
      ...
      END.*/
       ...
       choice = YES.
      if not avail itemfg OR xeb.stock-no EQ "" THEN DO  :
  ...
  FOR EACH job-hdr EXCLUSIVE-LOCK
    ...
    FIND FIRST tt-ordlist WHERE tt-ordlist.ord-no EQ job-hdr.ord-no
       NO-ERROR.
    IF NOT AVAIL tt-ordlist THEN DO:
      CREATE tt-ordlist.
      ASSIGN tt-ordlist.ord-no = job-hdr.ord-no.
    END.
