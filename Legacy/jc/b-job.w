...
{sys/inc/varasgn.i}
&SCOPED-DEFINE browse2 jc/j-jobhdr.i
...
&Scoped-define FIELDS-IN-QUERY-Browser-Table job.job-no job.job-no2 ~
job-hdr.i-no job.est-no job-hdr.ord-no job-hdr.cust-no job.start-date ~
job.stat 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH job WHERE ~{&KEY-PHRASE} ~
      AND job.company = g_company NO-LOCK, ~
      EACH job-hdr OF job NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH job WHERE ~{&KEY-PHRASE} ~
      AND job.company = g_company NO-LOCK, ~
      EACH job-hdr OF job NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table job job-hdr
&Scoped-define FIRST-TABLE-IN-QUERY-Browser-Table job
&Scoped-define SECOND-TABLE-IN-QUERY-Browser-Table job-hdr
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      job.job-no FORMAT "x(6)":U
      job.job-no2 COLUMN-LABEL "Run#" FORMAT ">9":U
      job-hdr.i-no FORMAT "x(15)":U
      job.est-no FORMAT "x(5)":U
      job-hdr.ord-no FORMAT ">>>>>9":U
      job-hdr.cust-no FORMAT "x(8)":U
      job.start-date FORMAT "99/99/9999":U
      job.stat FORMAT "x":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     auto_find AT ROW 19.33 COL 92 COLON-ALIGNED HELP
...
/*      MESSAGE "value changed" job.rec_key SKIP
      "1- {&TABLENAME}" SKIP
      "2- {&NORECKEY}" SKIP
      "3- {&SETVALUE}" SKIP
      "4- {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" SKIP
      "5- &FIRST-EXTERNAL-TABLE} " SKIP
      "6-" lookup("{&TABLENAME}","{&NORECKEY}"," ")  
      VIEW-AS ALERT-BOX.
