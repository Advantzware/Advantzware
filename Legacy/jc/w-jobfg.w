...
DEFINE VARIABLE h_q-jobhfg AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'View Item|Inventory|Totals|Bin/Jobs|Set parts|Colors|Vend Cost|History|Image' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_q-jobhfg , 'Record':U , h_itemfg ).

       /* Links to SmartViewer h_p-calcc. */
       RUN add-link IN adm-broker-hdl ( h_itemfg , 'calc-cost':U , h_p-calcc ).

       /* Links to SmartQuery h_q-jobhfg. */
       RUN add-link IN adm-broker-hdl ( h_p-navfg , 'Navigation':U , h_q-jobhfg ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_q-jobhfg ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_p-calcq ).
       RUN set-position IN h_p-calcq ( 17.67 , 88.00 ) NO-ERROR.
    ...
    WHEN 6 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_q-jobhfg , 'Record':U , h_b-ebfg ).
