...
FOR EACH xqitm OF xquo NO-LOCK BREAK BY xqitm.part-no:
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */ :
    ...
    IF i EQ 2 THEN DO:
      ...
      IF CallingParameter = "TRILAKE-EXCEL"  THEN
      DO:
      ...
      ASSIGN v-cell = "R" + string(LvLineCnt) + "C3".
      chExcelApplication:Goto(v-cell) NO-ERROR.
    ...
    IF i EQ 4 THEN DO:
       ...
       do :
         /* PUT xqitm.i-coldscr AT 29 FORM "x(30)". */
         ASSIGN v-cell = "R" + string(LvLineCnt) + "C2".
         chExcelApplication:Goto(v-cell) NO-ERROR.
    ...
    IF i EQ 5 /* AND numfit GE 5 */ THEN DO:
       ...
       */         
       ...
       ELSE do :
          ...
          END.
          ELSE IF callingparameter = "TRILAKE-EXCEL" THEN
          DO:
             ASSIGN LvLineCnt = LvLineCnt + 1 
                    v-cell = "R" + string(LvLineCnt) + "C3".
             chExcelApplication:Goto(v-cell) NO-ERROR.
       ...
       xxx    = IF xqqty.uom EQ "L" THEN xqqty.price    ELSE
           ...
           DO:
              IF lv-uom EQ "EA" THEN
                 ASSIGN chExcelApplication:ActiveCell:Value = "$" + TRIM(STRING(xqqty.price,">>>,>>9.999")).
              ELSE
                 ASSIGN chExcelApplication:ActiveCell:Value = "$" + TRIM(STRING(xqqty.price,">>>,>>9.99")).
           END.
           ELSE
           IF callingParameter = "TRILAKE-EXCEL" THEN
              ASSIGN chExcelApplication:ActiveCell:Value = xqqty.price .
  ...
  IF LINE-COUNTER > PAGE-SIZE - 10 THEN DO:
     ...
     /* skb - 1/25/07 -comments out for excel */
...
if (ch-multi and v-last) or (not ch-multi) then do:
  if ch-inst then do:
    ...
    for each est-inst
      ...
      /* skb - removed for excel output 
      put est-inst.dept at 7 
      est-inst.inst[1] at 11.
