/* ------------------------------------------- cec/quote/quookla.p 8/12 BPV */
/* print quotes in Oklahoma - Xprint format                                               */
/* -------------------------------------------------------------------------- */
...
DEF BUFFER bf-eb FOR eb.
    ...
    /*for printing the multicell jobcard description info*/
...
DEF VAR v-height1 AS DECIMAL FORMAT ">>>>>>9.99<<<<" NO-UNDO.
DEF VAR v-height2 AS DECIMAL FORMAT ">>>>>>9.99<<<<" NO-UNDO.
...
if (not ch-multi) then do: /*only one quote #*/
   ...
   IF (ch-multi and (v-last OR s-sep-page)) then do:
     ...
     if v-prt-box then DO:
        find first est where est.company eq xquo.company
                   ...
                   no-lock no-error.
     if avail est then do:
/*        IF li-design-count = 0 THEN DO:            */
/*            PAGE.                                  */
/*            li-design-count = li-design-count + 1. */
/*        END.                                       */
/*        ELSE li-design-count = 0.                  */
/*                                                   */
       PUT "<FCouriar New>" SKIP.
       FIND FIRST xest WHERE RECID(xest) = RECID(est) NO-LOCK NO-ERROR.
       run cec/desprnok.p (?,
