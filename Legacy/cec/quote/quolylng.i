...
FOR EACH tt-item,
  ...
  IF AVAIL eb THEN DO:
    ...
    ELSE ASSIGN ld-metric = 1
                lv-format = ">>>>>9.9<<<<".
    ASSIGN 
        ld-bsf = eb.t-sqin / 144
        lc-prod-cat = eb.procat
        li-case-cnt = eb.cas-cnt.
...
/*        ((AVAIL est AND est.est-type GT 4 and est.est-type NE 6) OR NOT AVAIL est) THEN numfit = numfit + 1. rtc */
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */:
    ...
    IF i EQ 2 THEN DO:
      ...
      IF LOOKUP(xqitm.style,"DC,DC2S,DC3S,DC4S,DC5S,DC6S,DCJOIN") > 0 THEN
          trim-size-d = " " .
      ELSE
...
/*                   (xqqty.qty * xqqty.price).                 */
        IF lv-print-bsf AND lc-prod-cat = "PADS" THEN
               DO: 
                   RUN sys/ref/convptom.p (xqqty.uom, xqqty.price, xqqty.qty, li-case-cnt, OUTPUT ld-price-per-m ).
                   v-pricea = string(ld-price-per-m / ld-bsf,"->>,>>9.99").
               END.
        ELSE
        ...
        PUT xqqty.qty           TO 67   FORMAT ">>>>>>9"
            xqqty.rels          TO 71   FORMAT ">>9"   
            v-pricea            TO 86   /*FORMAT "->>,>>9.99"*/.

        IF lv-print-bsf AND lc-prod-cat = "PADS" THEN /*added for LoylangBSF*/
            PUT "BSF" TO 92.
            ELSE IF xqqty.uom NE "L" THEN
                PUT xqqty.uom TO 92.
                ELSE
                PUT "LOT" TO 92.
