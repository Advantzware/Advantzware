...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 item.i-no item.i-name item.shrink 
...
item.mat-type = ip-type"
     _FldNameList[1]   > ASI.item.i-no
"item.i-no" ? "x(13)" "character" ? ? ? ? ? ? no ?
     _FldNameList[2]   = ASI.item.i-name
     _FldNameList[3]   = ASI.item.shrink
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Paper Information */
DO:
