...
/* -------------------------------------------------------------------------- */
   ...
   if ll-foam and (lookup(est-op.dept,"CR,RC") gt 0 OR
                   (est-op.dept EQ "DC" AND est-op.n-out GT 0)) then do:
     IF INDEX("AP",mach.p-type) GT 0 THEN v-on-f = 1.
     ELSE run sys/inc/numout.p (recid(est-op), output v-on-f).
