...
DEFINE VARIABLE ll-add-set AS LOGICAL NO-UNDO INITIAL NO .
DEFINE VARIABLE ll-add-set-part AS LOGICAL NO-UNDO.
DEFINE VARIABLE ll-add-set-part-2 AS LOGICAL NO-UNDO.
DEFINE VARIABLE li-new-estnum AS INTEGER NO-UNDO.
DEFINE VARIABLE ll-new-record AS LOGICAL NO-UNDO.
DEFINE VARIABLE ll-new-est AS LOGICAL NO-UNDO.
...
DEFINE NEW SHARED VARIABLE formule AS INTEGER EXTENT 12 NO-UNDO.
...
DEFINE VARIABLE v-l-array AS DECIMAL EXTENT 30 NO-UNDO.
DEFINE VARIABLE li-flen AS INTEGER EXTENT 2 NO-UNDO.
DEFINE VARIABLE v-board2 AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-part-no2 AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-assem-grain AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-auto-add-item AS LOGICAL NO-UNDO.
DEFINE VARIABLE v-ds AS LOGICAL VIEW-AS TOGGLE-BOX NO-UNDO.
DEF NEW SHARED VAR s-est-no AS CHARACTER NO-UNDO.  /* for fgadd2.p */
...
DEFINE VARIABLE lv-last-est-cat AS CHARACTER NO-UNDO INIT "".
...
DEFINE VARIABLE lv-hold-flute LIKE eb.flute NO-UNDO.
DEFINE VARIABLE lv-hold-test LIKE eb.test NO-UNDO.
DEFINE VARIABLE lv-repo AS CHARACTER INIT "ON" NO-UNDO.
DEFINE VARIABLE cadcamValue AS CHARACTER NO-UNDO.
DEFINE VARIABLE lv-Persistent-Handle AS HANDLE NO-UNDO.
...
DEFINE VARIABLE v-style2 AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-no-forms AS INTEGER NO-UNDO.
DEFINE VARIABLE v-in-cell-w AS DECIMAL NO-UNDO.
DEFINE VARIABLE v-end-cell-w1 AS DECIMAL NO-UNDO.
DEFINE VARIABLE v-end-cell-w2 AS DECIMAL NO-UNDO.
DEFINE VARIABLE v-qty-set-2 AS INTEGER NO-UNDO.
DEFINE VARIABLE v-wid-2 AS DECIMAL DECIMALS 4 NO-UNDO.
DEFINE VARIABLE v-height-2 AS DECIMAL DECIMALS 4 NO-UNDO.
DEFINE VARIABLE v-part-desc3 AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-part-desc4 AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-qty AS INTEGER NO-UNDO.
DEFINE VARIABLE v-stock-no2 AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-count AS INTEGER NO-UNDO.
DEFINE VARIABLE lv-save-set AS CHARACTER NO-UNDO.
DEFINE VARIABLE lv-set-widget AS HANDLE NO-UNDO.
DEFINE VARIABLE v-rowid-eb AS ROWID NO-UNDO.

DEFINE NEW SHARED TEMP-TABLE tt-eb-set NO-UNDO LIKE eb.

DEFINE TEMP-TABLE tt-eb NO-UNDO LIKE eb FIELD row-id AS ROWID INDEX row-id row-id.
DEFINE TEMP-TABLE tt-est-op NO-UNDO LIKE est-op.
DEFINE TEMP-TABLE tt-stock-no 
    FIELD eb-row-id AS ROWID
    FIELD stock-no LIKE eb.stock-no.
DEFINE TEMP-TABLE tt-e-vend 
     ...
     /*WHEN "part-no" THEN DO: 
      ...
      WHEN "style" THEN DO:
           ...
           IF char-val NE "" and ls-cur-val NE ENTRY(1,char-val) THEN DO:
              ...
              FIND style NO-LOCK WHERE style.company EQ gcompany and
                          ...
                          NO-ERROR.            
              IF AVAILABLE style AND ef.board:SCREEN-VALUE EQ "" THEN DO:
      ...
      WHEN "procat" THEN DO:
       ...
       WHEN "Board" THEN DO:
           ...
           ELSE lv-ind = "".  
           IF AVAILABLE style AND style.type = "f" THEN  DO: /* foam */
           ...
           ELSE DO:
              ...
              IF AVAILABLE ITEM AND ITEM.i-no NE lw-focus:SCREEN-VALUE THEN DO:
...
DO WITH FRAME {&FRAME-NAME}:
  IF eb.cust-no:MODIFIED IN BROWSE {&browse-name} THEN DO:
    ...
    IF LASTKEY NE -1 AND eb.ord-no NE 0 AND
  ...
  END.
   ...
   IF LASTKEY NE -1 AND 
      ...
      NOT CAN-FIND(cust WHERE cust.company EQ gcompany AND cust.cust-no EQ eb.cust-no:SCREEN-VALUE IN BROWSE {&browse-name} )
   THEN DO:
       IF eb.cust-no:SCREEN-VALUE EQ "" THEN DO:
       ...
       IF NOT CAN-FIND(cust WHERE cust.company EQ gcompany AND cust.cust-no = eb.cust-no:SCREEN-VALUE IN BROWSE {&browse-name} )
...
DO:
     ...
     IF AVAILABLE tt-eb-set THEN
       ASSIGN
        ...
        eb.part-no:SCREEN-VALUE IN BROWSE {&browse-name}    =
             TRIM(SUBSTRING(tt-eb-set.part-no,1,li-flen[1])) + "-1"
        eb.part-dscr1:SCREEN-VALUE IN BROWSE {&browse-name} = tt-eb-set.part-dscr1
        eb.stock-no:SCREEN-VALUE IN BROWSE {&browse-name}   =
             IF tt-eb-set.stock-no NE "" THEN (TRIM(SUBSTRING(tt-eb-set.stock-no,1,li-flen[2])) + "-1") ELSE ""
  ...
  DO:
     ...
     IF AVAILABLE tt-eb-set-part THEN
     DO:
        ASSIGN
         li-flen[1] = LENGTH(STRING(FILL("X",100),eb.part-no:FORMAT IN BROWSE {&browse-name}))  - 2
         li-flen[2] = LENGTH(STRING(FILL("X",100),eb.stock-no:FORMAT IN BROWSE {&browse-name})) - 2
         eb.part-no:SCREEN-VALUE IN BROWSE {&browse-name}    =
              TRIM(SUBSTRING(tt-eb-set-part.part-no,1,li-flen[1])) + "-1"
         eb.part-dscr1:SCREEN-VALUE IN BROWSE {&browse-name} = tt-eb-set-part.part-dscr1
         eb.stock-no:SCREEN-VALUE IN BROWSE {&browse-name}   =
              IF tt-eb-set-part.stock-no NE "" THEN (TRIM(SUBSTRING(tt-eb-set-part.stock-no,1,li-flen[2])) + "-1")
              ELSE ""
         ...
         v-part-no2   = TRIM(SUBSTRING(tt-eb-set-part.part-no,1,li-flen[1])) + "-2"
         v-stock-no2  = IF tt-eb-set-part.stock-no NE "" THEN (TRIM(SUBSTRING(tt-eb-set-part.stock-no,1,li-flen[2])) + "-2")
                        ELSE ""
...
/*                        NO-LOCK NO-ERROR.                          */
/*         IF AVAILABLE reftable AND reftable.code2 EQ "I" THEN DO:      */
...
DO:
  IF eb.est-type EQ 6                                               AND
     (INTEGER(eb.blank-no:SCREEN-VALUE IN BROWSE {&browse-name}) GT 1 OR
      INTEGER(eb.form-no:SCREEN-VALUE IN BROWSE {&browse-name}) GT 1)   THEN DO:
...
DO:
    ...
    IF INTEGER(est-qty.eqty:SCREEN-VALUE IN BROWSE {&browse-name} ) LE 0 THEN DO:
...
DO:
   DEFINE VARIABLE ls-board AS CHARACTER NO-UNDO.
   ...
   THEN DO:
     FIND FIRST reftable NO-LOCK WHERE reftable.reftable EQ "STYFLU"
                                    AND reftable.company EQ eb.style:SCREEN-VALUE
                                    AND (reftable.loc EQ eb.flute:SCREEN-VALUE OR lv-foam)
                                    AND reftable.code EQ "BOARD"
                                    NO-ERROR.
     IF AVAILABLE reftable AND reftable.dscr NE "" THEN DO:
        FIND FIRST ITEM NO-LOCK WHERE ITEM.company  EQ gcompany AND
                                      ITEM.i-no  EQ reftable.dscr
                                      NO-ERROR.
     ...
     END.                                         
  /*   IF NOT AVAILABLE item or (avail item and item.i-no = "" ) THEN   */
...
DO:
  ...
  do:  
     FIND FIRST reftable NO-LOCK WHERE reftable.reftable  EQ "STYFLU"
                                   AND reftable.company  EQ eb.style:SCREEN-VALUE IN BROWSE {&browse-name}
                                  AND (reftable.loc  EQ eb.flute:SCREEN-VALUE or lv-foam)
                                  AND reftable.code  EQ "BOARD"
                                  NO-ERROR.
     IF AVAILABLE reftable AND reftable.dscr NE "" THEN DO:
...
DO:
   ...
   DEFINE VAR len-num AS INTEGER NO-UNDO.
   ...
   v-dec = DECIMAL(SELF:SCREEN-VALUE) - TRUNCATE(DECIMAL(SELF:SCREEN-VALUE),0).
   ...
   IF v-cecscrn-dec THEN
   DO:
      len-num = INTEGER(SELF:SCREEN-VALUE) .
...
DO:
   ...
   DEFINE VARIABLE op-dec AS DECIMAL DECIMALS 6 NO-UNDO.
   DEFINE VARIABLE op-error AS LOGICAL NO-UNDO.
   DEFINE VAR wid-num AS INTEGER NO-UNDO.
   ...
   IF v-cecscrn-dec THEN
   DO:
      wid-num = INTEGER(SELF:SCREEN-VALUE) .
      RUN valid-64-dec(INPUT v-dec, OUTPUT op-error, OUTPUT op-dec).
      IF op-error THEN DO:
         MESSAGE "Invalid Dimension."
            VIEW-AS ALERT-BOX ERROR BUTTONS OK.
...
DO:
   ...
   DEFINE VARIABLE op-dec AS DECIMAL DECIMALS 6 NO-UNDO.
   DEFINE VARIABLE op-error AS LOGICAL NO-UNDO.
   DEFINE VAR dep-num AS INTEGER NO-UNDO.
   ...
   IF v-cecscrn-dec THEN
       DO:
       dep-num = INTEGER(SELF:SCREEN-VALUE) .
       RUN valid-64-dec(INPUT v-dec, OUTPUT op-error, OUTPUT op-dec).
       IF op-error THEN DO:
           MESSAGE "Invalid Dimension."
               VIEW-AS ALERT-BOX ERROR BUTTONS OK.
...
DO:
  IF DECIMAL(eb.i-col:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN
    eb.i-pass:SCREEN-VALUE IN BROWSE {&browse-name} = "".
  ELSE
  IF DECIMAL(eb.i-pass:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN
  ...
  IF eb.blank-no EQ 1 THEN
    ...
    ELSE
    IF DECIMAL(ef.f-col:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN
      ASSIGN
       ef.f-col:SCREEN-VALUE IN BROWSE {&browse-name}  =
...
DO:
  IF DECIMAL(eb.i-coat:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN
    eb.i-coat-p:SCREEN-VALUE IN BROWSE {&browse-name} = "".
  ELSE
  IF DECIMAL(eb.i-coat-p:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN
  ...
  IF eb.blank-no EQ 1 THEN
    ...
    ELSE
    IF DECIMAL(ef.f-coat:SCREEN-VALUE IN BROWSE {&browse-name}) EQ 0 THEN
      ASSIGN
       ef.f-coat:SCREEN-VALUE IN BROWSE {&browse-name}  =
...
DO:
  IF index("YN", eb.spare-char-2:SCREEN-VALUE IN BROWSE br-estitm) EQ 0 THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  IF ls-add-what EQ "" THEN RETURN NO-APPLY.  /* cancel */
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAILABLE xest AND AVAILABLE(eb) THEN DO:
      FIND FIRST xest NO-LOCK WHERE xest.company EQ eb.company 
  ...
  IF xest.est-type EQ 2 or xest.est-type EQ 6 THEN DO:
      FIND FIRST bf-eb NO-LOCK WHERE bf-eb.company EQ xest.company
                                 AND bf-eb.est-no  EQ xest.est-no
                                 AND bf-eb.form-no EQ 0
                               NO-ERROR.
      
      IF AVAILABLE bf-eb AND bf-eb.stock-no EQ "" THEN DO:
        ...
        FIND xeb WHERE ROWID(xeb) EQ ROWID(bf-eb) NO-LOCK.
        RUN fg/ce-addfg.p (bf-eb.stock-no).
        FIND itemfg WHERE itemfg.company EQ xest.company
  ...
  FIND FIRST tt-stock-no WHERE tt-stock-no.eb-row-id EQ ROWID(xeb)
                         NO-ERROR.

  IF NOT AVAILABLE tt-stock-no THEN DO:
      CREATE tt-stock-no.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE itemfg OR AVAILABLE b-eb THEN DO:
    ASSIGN
     ld-dim = DECIMAL(eb.len:SCREEN-VALUE IN BROWSE {&browse-name})
     eb.len:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(display-cw-dim(YES,ld-dim))
     ld-dim = DECIMAL(eb.wid:SCREEN-VALUE IN BROWSE {&browse-name})
     eb.wid:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(display-cw-dim(YES,ld-dim))
     ld-dim = DECIMAL(eb.dep:SCREEN-VALUE IN BROWSE {&browse-name})
...
------------------------------------------------------------------------------*/
 /* calc blank W,L SqIn */
   ...
   DEFINE VARIABLE v-score-char LIKE v-lscore-c EXTENT 100.
   ...
   IF AVAILABLE style THEN DO:
      IF style.type NE "F" THEN RUN calc-blank-size2. 
      ...
      ELSE DO:
         ...
         ELSE DO:
            ...
            ASSIGN  v-score-char    = ""
                    j               = 1.
            DO i = 1 TO 80:
              IF SUBSTRING(v-lscore-c,i,1) NE "" THEN DO:
                 v-score-char[j] = v-score-char[j] + SUBSTRING(v-lscore-c,i,1).
                 IF SUBSTRING(v-lscore-c,i + 1,1) EQ "" THEN
            ...
            DO i = 1 TO EXTENT(xeb.k-len-array2):
               ...
               IF v-cecscrn-dec AND v-score-char[i] NE "" THEN
                  ASSIGN
                     ...
                     v-str = LEFT-TRIM(STRING(INTEGER(v-str) / 64.0,">.999999"))
                     SUBSTRING(v-score-char[i],v-index) = v-str.

               xeb.k-len-array2[i] = DECIMAL(v-score-char[i]).
...
------------------------------------------------------------------------------*/
   ...
   FIND FIRST ITEM NO-LOCK WHERE ITEM.company EQ est.company
...
------------------------------------------------------------------------------*/
  ...
  FIND xest WHERE ROWID(xest) EQ ROWID(est).
  ...
  IF ceroute-chr NE "" THEN DO:
    ...
    IF AVAILABLE mach THEN DO:
      ...
      IF ceroute-int NE 0 AND ceroute-int LT xef.gsh-wid THEN
        ASSIGN
         xef.n-out   = TRUNCATE(ceroute-int / xef.nsh-wid,0)
...
/* *********** copied from uest3.p ***********/
      ...
      IF AVAILABLE style THEN DO:
         ...
         IF AVAILABLE ITEM THEN k = IF AVAILABLE style THEN INTEGER(style.material[3]) ELSE 0.
         ...
         IF AVAILABLE style AND style.material[6] NE "" THEN
            FIND FIRST alt-item NO-LOCK WHERE
                 ...
                 NO-ERROR.
      END.
      IF NOT AVAILABLE ITEM OR NOT AVAILABLE alt-item OR (k = 0) THEN DO:
         FIND FIRST ce-ctrl NO-LOCK WHERE ce-ctrl.company  EQ eb.company AND
                                         ce-ctrl.loc  EQ eb.loc
                                         NO-ERROR.
         IF k  EQ 0 THEN k = ce-ctrl.def-inkcov.
         IF NOT AVAILABLE ITEM THEN DO:

            FIND FIRST ITEM WHERE ITEM.company  EQ eb.company AND
                       ITEM.i-no  EQ ce-ctrl.def-ink NO-LOCK NO-ERROR.
         END.
      ...
      IF choice THEN DO i = 1 to 10:
         ...
         THEN DO:
              ...
              ASSIGN bf-eb.i-ps[i]   = counter
                     bf-eb.i-code[i] = IF AVAILABLE alt-item THEN alt-item.i-no ELSE ""
                     bf-eb.i-dscr[i] = IF AVAILABLE alt-item THEN alt-item.i-name ELSE ""
                     bf-eb.i-%[i]    = 100.
         END.
...
------------------------------------------------------------------------------*/
  ...
  IF ll-set THEN DO:
    ...
    FIND FIRST fg-set WHERE fg-set.company EQ eb.company
                        AND fg-set.set-no EQ bf-eb.stock-no
  ...
  ELSE
  IF NOT AVAILABLE bf-eb OR bf-eb.part-no EQ "" THEN xest.est-type = 5.
...
------------------------------------------------------------------------------*/
  ...
  FIND bf-new-eb WHERE RECID(bf-new-eb) = IF RECID(eb) NE ? THEN RECID(eb) ELSE lv-eb-RECID .
  FIND bf-new-ef WHERE RECID(bf-new-ef) = IF RECID(eb) NE ? THEN RECID(ef) ELSE lv-ef-RECID.
...
------------------------------------------------------------------------------*/
   ...
   IF eb.stock-no NE "" AND
      ...
      FOR EACH bf-e-itemfg-vend NO-LOCK WHERE
          ...
          DO TRANSACTION:
            ...
            FIND FIRST bf-e-itemfg NO-LOCK WHERE
...
/*                            NO-LOCK NO-ERROR.                                                                                                         */
/*   i = IF AVAILABLE est-prep THEN est-prep.line + 1 ELSE 1.                                                                                               */
/*                                                                                                                                                      */
 ...
 /* from crt-new-est procedure */
   ...
   /*  IF AVAILABLE itemfg AND eb.part-no = "" THEN
     ...
     END.
      ...
      IF AVAILABLE shipto AND shipto.spare-char-1 NE "" THEN DO:
          ...
          FIND sman NO-LOCK WHERE sman.company = gcompany
  ...
  /* ink/pack copied from cec/estitm1.i */
     IF eb.stock-no = "" THEN DO:
        FIND FIRST ce-ctrl NO-LOCK WHERE ce-ctrl.company EQ gcompany AND 
                                         ce-ctrl.loc EQ gloc
                                         NO-ERROR.
        ASSIGN eb.cas-no = ce-ctrl.def-case
               eb.tr-no = ce-ctrl.def-pal.      
     END.
     FIND FIRST cust NO-LOCK WHERE cust.company EQ gcompany AND 
                                   cust.cust-no EQ eb.cust-no
                                   NO-ERROR.
     ASSIGN
     eb.cas-no = IF AVAILABLE cust AND cust.case-bundle NE "" THEN cust.case-bundle ELSE eb.cas-no
     eb.tr-no = IF AVAILABLE cust AND cust.pallet NE "" THEN cust.pallet ELSE eb.tr-no.      
     ...
     IF AVAILABLE ITEM THEN ASSIGN eb.cas-cnt = (ITEM.box-case)
                              ...
                              eb.cas-wt = (ITEM.avg-w)         
                              .
     FIND FIRST ITEM NO-LOCK WHERE ITEM.company = eb.company AND
                                  ITEM.i-no = eb.tr-no
                                 NO-ERROR.
     IF AVAILABLE ITEM THEN ASSIGN eb.tr-len = (ITEM.case-l)
                               eb.tr-wid = (ITEM.case-w)
                               eb.tr-dep = (ITEM.case-d)
                               .
     DEFINE VARIABLE lv-cas-pal AS DECIMAL NO-UNDO.
     DEFINE VARIABLE lv-tr-cnt AS INTEGER NO-UNDO.
     DEFINE VARIABLE lv-numstack AS INTEGER NO-UNDO.
     DEFINE VARIABLE lv-stackcode AS CHARACTER NO-UNDO.
 ...
 /* from crt-new-est procedure */
  ...
  FOR EACH tt-artios NO-LOCK BREAK BY tt-artios.form-num BY tt-artios.blank-num:
     ...
     IF LOOKUP(eb.style,"5PF,5PFN,COW,FOL,FTB,FTT,HSC,RSC,RSCFOL") GT 0 THEN
        ASSIGN
           eb.len = IF tt-artios.len GT 0 THEN tt-artios.len ELSE 0
           eb.wid = IF tt-artios.wid GT 0 THEN tt-artios.wid ELSE 0
           eb.dep = IF tt-artios.dep GT 0 THEN tt-artios.dep ELSE 0.
           
     ELSE
        eb.dep = 0.

     IF eb.len EQ 0 AND tt-artios.t-len NE 0 THEN eb.len = tt-artios.t-len * (tt-artios.ratio / 100)  / tt-artios.CompNumUp.           
     IF eb.wid EQ 0 AND tt-artios.t-wid NE 0 THEN eb.wid = tt-artios.t-wid * tt-artios.ratio / 100.   
     IF eb.tr-cnt EQ 0 THEN eb.tr-cnt = eb.cas-cnt * eb.cas-pal.
     ...
     IF AVAILABLE cust THEN DO: 
        ...
        IF AVAILABLE shipto THEN ASSIGN eb.ship-id = shipto.ship-id.

     END.

     FIND FIRST ITEM NO-LOCK WHERE
          ITEM.company EQ gcompany AND
          ITEM.i-no  EQ tt-artios.board
          NO-ERROR.
...
/*                  VIEW-AS ALERT-BOX ERROR.                                                */
     ...
     IF iArtiosCount GT 1 THEN DO:
        FIND bf-eb WHERE bf-eb.company EQ est.company
                          AND bf-eb.est-no EQ est.est-no
 ...
 /* from crt-new-est procedure */
  ...
  FOR EACH tt-artios NO-LOCK BREAK BY tt-artios.form-num BY tt-artios.blank-num:
     ...
     */
     ...
     v-cust-no = IF tt-artios.cust-no NE "" THEN tt-artios.cust-no
                 ELSE tt-artios.custname.
...
DEFINE VARIABLE i AS INTEGER NO-UNDO.
DEFINE VARIABLE xeb-rowid AS ROWID NO-UNDO.
...
DO:
   IF LOOKUP(SUBSTRING(v-graphic-char,LENGTH(v-graphic-char)),"\,/") EQ 0 THEN
...
------------------------------------------------------------------------------*/
  ...
  IF lv-repo EQ "ON" AND AVAILABLE eb THEN DO:
      ...
      IF AVAILABLE eb AND eb.est-type EQ 8 THEN
...
FIND LAST buf-eb NO-LOCK WHERE buf-eb.company EQ gcompany 
                          AND buf-eb.est-type EQ 6 /*> 4*/  NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT ll-is-add-from-tool THEN DO:
    RUN est/d-addfol.w (INPUT YES, OUTPUT ls-add-what).
     ...
     IF ls-add-what EQ "" THEN RETURN NO-APPLY.  /* cancel from dialog box */
  END.

  IF CAN-DO("form,blank",ls-add-what) THEN DO:
    {custom/checkuse.i}

    IF AVAILABLE eb AND eb.est-type EQ 6 THEN DO:
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE is-first-record AS LOGICAL INITIAL NO NO-UNDO.
  DEFINE BUFFER bf-eb FOR eb.  
  DEFINE BUFFER bf-ef FOR ef.
  DEFINE BUFFER bf-est FOR est.
  DEFINE BUFFER bf-est-qty FOR est-qty.
  ...
  DEFINE VARIABLE ll-2pc AS LOGICAL NO-UNDO.
  DEFINE VARIABLE lv-box-des AS CHARACTER INITIAL "S" NO-UNDO.
  DEFINE VARIABLE v-dec AS DECIMAL NO-UNDO.
  DEFINE VARIABLE v-dec2 AS DECIMAL NO-UNDO.
  DEFINE VARIABLE v-count AS INTEGER NO-UNDO.
  DEFINE VARIABLE v-w-array AS DECIMAL EXTENT 30 NO-UNDO.  
  ...
  IF TRIM(est.est-no) NE TRIM(eb.est-no) THEN DO:
    ...
    REPEAT:
       ...
       DO:
          IF ce-ctrl.e-num LT INTEGER(est.est-no) THEN
             ce-ctrl.e-num = INTEGER(est.est-no).
  ...
  IF lv-hld-eqty NE est-qty.eqty THEN DO:
    FIND FIRST bf-est-qty
        ...
        NO-ERROR.
    IF AVAILABLE bf-est-qty THEN bf-est-qty.eqty = lv-hld-eqty.
  END.
  ...
  IF ll-add-set-part AND ll-add-set-part-2 EQ NO AND AVAILABLE tt-eb-set-part THEN
  ...
  END.
  ELSE
     FOR FIRST box-design-hdr NO-LOCK WHERE
         box-design-hdr.design-no  EQ 0 and
         box-design-hdr.company  EQ eb.company AND
         box-design-hdr.est-no  EQ eb.est-no AND
         box-design-hdr.form-no  EQ eb.form-no AND
         box-design-hdr.blank-no  EQ eb.blank-no :
  ...
  IF cestyle-log                    AND
    ...
    END.
    lv-cad-path = IF AVAILABLE sys-ctrl THEN sys-ctrl.char-fld ELSE "".
    IF lv-cad-path NE "" THEN DO:
      IF lv-cad-ext EQ "" THEN lv-cad-ext = ".jpg".
      IF SEARCH(lv-cad-path + eb.cad-no + lv-cad-ext) EQ ? THEN lv-cad-ext = "".
    END.
    IF SEARCH(lv-cad-path + eb.cad-no + lv-cad-ext) NE ? THEN DO:
        FIND FIRST box-design-hdr WHERE box-design-hdr.design-no EQ 0 AND  
                                     box-design-hdr.company   EQ eb.company 
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE est AND est.est-type NE 5 AND
...
------------------------------------------------------------------------------*/
  DEFINE VARIABLE char-hdl AS CHARACTER NO-UNDO.
  DEFINE VARIABLE ll-dumb AS LOGICAL NO-UNDO.
  DEFINE VARIABLE lv-copylist AS CHARACTER NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER bqty FOR est-qty.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
     ...
     FIND style NO-LOCK WHERE style.company = gcompany AND 
  ...
  DO WITH FRAME {&FRAME-NAME}:
     ...
     IF AVAILABLE est AND est.est-type EQ 8 THEN
       ASSIGN
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    FOR EACH b-eb NO-LOCK
        ...
        BY b-eb.form-no BY b-eb.blank-no:

        LEAVE.
    END.
    
  /*  IF AVAILABLE eb  AND adm-brs-in-update  AND NOT adm-new-record  /*update mode*/
    ...
    IF AVAILABLE eb AND AVAILABLE b-eb AND ROWID(b-eb) EQ ROWID(eb) THEN
       APPLY "ENTRY" TO eb.cust-no IN BROWSE {&browse-name}.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE eb AND ecbrowse-chr = "Partitions" THEN
  DO:
     ASSIGN
        eb.wid:LABEL IN BROWSE {&browse-name} = "Height".
        eb.dep:LABEL IN BROWSE {&browse-name} = "Slot".
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE li AS INTEGER NO-UNDO.
  DEFINE VARIABLE lv-ef-rowid AS ROWID NO-UNDO.
  DEFINE BUFFER bf-e-itemfg-vend FOR e-itemfg-vend.
  DEFINE VARIABLE lv-new-record AS LOGICAL NO-UNDO.
  DEFINE VARIABLE lActive AS LOGICAL NO-UNDO.
  DEFINE VARIABLE old-cat-no LIKE eb.procat NO-UNDO.
...
/*         IF AVAILABLE reftable AND reftable.code2 = "I" THEN DO:           */
    ...
    END.
     ...
     IF INTEGER(est-qty.eqty:SCREEN-VALUE IN BROWSE {&browse-name}) <= 0 THEN DO:
        MESSAGE "Quantity must be entered. " VIEW-AS ALERT-BOX ERROR.
     ...
     IF ef.board:SCREEN-VALUE IN BROWSE {&browse-name} NE "" AND 
     ...
     IF DECIMAL(eb.wid:SCREEN-VALUE) - TRUNCATE(DECIMAL(eb.wid:SCREEN-VALUE),0) >= v-16-or-32 
     THEN DO:
          MESSAGE "Cannot have more than " v-16-or-32 - 0.01 " as decimal, field is (inches.16ths/32nd's) "
             ...
             RETURN NO-APPLY.
     END.
     IF DECIMAL(eb.len:SCREEN-VALUE) - TRUNCATE(DECIMAL(eb.len:SCREEN-VALUE),0) >= v-16-or-32 
     THEN DO:
            MESSAGE "Cannot have more than " v-16-or-32 - 0.01 " as decimal, field is (inches.16ths/32nd's) "
                   VIEW-AS ALERT-BOX ERROR.
             APPLY "ENTRY" to eb.len.
             RETURN NO-APPLY.
     END.
     IF DECIMAL(eb.dep:SCREEN-VALUE) - TRUNCATE(DECIMAL(eb.dep:SCREEN-VALUE),0) >= v-16-or-32
     THEN DO:
          MESSAGE "Cannot have more than " v-16-or-32 - 0.01 " as decimal, field is (inches.16ths/32nd's) "
                   VIEW-AS ALERT-BOX ERROR.
  ...
  DO TRANSACTION:
    ...
    FIND CURRENT ef EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
    FIND CURRENT eb EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
    IF NOT AVAILABLE est OR NOT AVAILABLE est-qty OR
       NOT AVAIL ef  OR NOT AVAILABLE eb      THEN DO:
  ...
  IF old-cat-no NE eb.procat THEN DO:
       ...
       IF AVAILABLE itemfg AND itemfg.est-no EQ eb.est-no THEN DO:
           FOR EACH oe-prmtx
               WHERE oe-prmtx.company            EQ itemfg.company
               AND oe-prmtx.i-no               BEGINS itemfg.i-no
               AND SUBSTRING(oe-prmtx.i-no,1,100) EQ itemfg.i-no
               EXCLUSIVE-LOCK:
               
               ASSIGN
                   SUBSTRING(oe-prmtx.i-no,1,100) = STRING(itemfg.i-no,"X(100)")
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE ls-delete AS CHARACTER NO-UNDO.
  DEFINE VARIABLE li-delete AS INTEGER NO-UNDO.
  ...
  IF AVAILABLE eb THEN RUN est/d-masdel.w (OUTPUT ls-delete).
  ...
  IF li-delete GT 0 THEN DO:
    ...
    FOR EACH tt-eb:
      ...
      IF AVAILABLE eb AND eb.est-no EQ tt-eb.est-no            AND
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE VARIABLE lv-mat-types AS CHARACTER INITIAL "B" NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE ll AS LOGICAL NO-UNDO.
  DEFINE VARIABLE v-set-header AS LOGICAL NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lv-i-no LIKE eb.stock-no NO-UNDO.
  DEFINE VARIABLE lv-num-created AS INTEGER NO-UNDO.
  DEFINE VARIABLE lvr-eb AS ROWID NO-UNDO.
  DEFINE BUFFER bf-est FOR est.
  DEFINE VARIABLE l-est-type AS INTEGER NO-UNDO.
  ...
  IF AVAILABLE eb THEN DO:
      FIND FIRST xest NO-LOCK WHERE xest.company EQ eb.company
                              ...
                              NO-ERROR.
  END.
  IF NOT AVAILABLE xeb AND AVAILABLE(eb) THEN
    FIND xeb WHERE ROWID(xeb) EQ ROWID(eb) NO-LOCK NO-ERROR.
  ...
  ELSE DO WHILE AVAILABLE(eb) /* AND eb.stock-no = ""*/:

      FIND bf-est NO-LOCK WHERE bf-est.company EQ eb.company
      ...
      FIND xeb WHERE ROWID(xeb) EQ ROWID(eb) EXCLUSIVE-LOCK.
      FIND xef WHERE ROWID(xef) EQ ROWID(ef) NO-LOCK.
      FIND FIRST xest NO-LOCK WHERE xest.company EQ xeb.company
                                AND xest.est-no  EQ xeb.est-no
                                NO-ERROR.
      ...
      IF AVAILABLE tt-stock-no THEN DO:
      ...
      IF v-est-fg1 NE "Manual" THEN DO:
        ...
        IF AVAILABLE itemfg THEN
          ASSIGN
           lv-i-no                         = itemfg.i-no.
      END.
      FIND FIRST xef NO-LOCK WHERE xef.company EQ xeb.company
                               AND xef.est-no  EQ xef.est-no
                               NO-ERROR.
      FIND FIRST xest NO-LOCK WHERE xest.company EQ xeb.company
                               AND xest.est-no  EQ xeb.est-no
                               NO-ERROR.
  ...
  IF AVAILABLE bf-est THEN DO:
      ...
      FOR EACH xeb WHERE xeb.company  EQ eb.company 
                     AND xeb.est-no   EQ eb.est-no
                     AND xeb.stock-no EQ ""
                   EXCLUSIVE-LOCK.
          FIND FIRST tt-stock-no WHERE tt-stock-no.eb-row-id = ROWID(xeb)
                                 NO-ERROR.      
          IF AVAILABLE tt-stock-no THEN
            xeb.stock-no = tt-stock-no.stock-no.
          
          FIND FIRST itemfg NO-LOCK WHERE itemfg.company EQ xeb.company
...
lv-foam = IF AVAILABLE style AND style.type EQ "F" THEN 
          YES ELSE NO.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER ip-string AS CHARACTER NO-UNDO.

DEFINE VARIABLE lv-tmp AS CHARACTER NO-UNDO.
DEFINE VARIABLE lv-tmp-val AS CHARACTER NO-UNDO.
DEFINE VARIABLE i AS INTEGER NO-UNDO.
DEFINE VARIABLE v-count AS INTEGER NO-UNDO.
...
DO i = 1 TO LENGTH(ip-string):
   ...
   IF lv-tmp-val NE " " THEN
      lv-tmp = lv-tmp + lv-tmp-val.
   ELSE IF lv-tmp NE "" THEN
      ASSIGN
         v-count = v-count + 1
         v-l-array[v-count] = DECIMAL(lv-tmp)
         lv-tmp = "".
END.
...
------------------------------------------------------------------------------*/
DEFINE BUFFER bf-eb FOR eb.
DEFINE INPUT PARAMETER v-eb-rowid AS ROWID NO-UNDO.
DEFINE INPUT PARAMETER v-board LIKE ef.board NO-UNDO.
FIND bf-eb WHERE ROWID(bf-eb) EQ v-eb-rowid NO-LOCK.
...
IF (est.est-type NE 4 AND est.est-type NE 8) THEN DO:
    ...
    FIND FIRST sys-ctrl NO-LOCK WHERE sys-ctrl.company EQ cocode
                                  ...
                                  NO-ERROR.
    IF AVAILABLE sys-ctrl AND sys-ctrl.char-fld = "FreightClass" THEN DO:
      FIND FIRST ITEM NO-LOCK WHERE ITEM.company EQ bf-eb.company
                                AND ITEM.i-no EQ v-board
                                NO-ERROR. 
      IF AVAILABLE(ITEM) AND item.mat-type = "B" AND ITEM.spare-char-1 GT "" THEN DO:
        FIND FIRST carr-mtx NO-LOCK WHERE carr-mtx.company  EQ bf-eb.company
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE b-eb THEN DO TRANSACTION:
     FIND FIRST b-est
...
------------------------------------------------------------------------------*/
  ...
  DEFINE BUFFER bf-eb FOR eb.
  DEFINE BUFFER bf-ef FOR ef.
  DEFINE BUFFER bf-eb-header FOR eb.

  DEFINE VARIABLE i AS INTEGER NO-UNDO.
  DEFINE VARIABLE lv-type LIKE est.est-type NO-UNDO.
  DEFINE VARIABLE lv-old-type LIKE lv-type NO-UNDO.
  DEFINE VARIABLE lv-yld-qty LIKE eb.yld-qty INITIAL 999999999 NO-UNDO.
  DEFINE VARIABLE ld AS DECIMAL NO-UNDO.
  DEFINE VARIABLE v-assem-partition AS LOGICAL NO-UNDO.
  ...
  ELSE
  IF AVAILABLE bf-ord AND i LE 1 AND eb.yld-qty LE 1 THEN DO:
  ...
  IF est.est-type EQ 6 THEN DO:
     IF lv-old-type GE 7 THEN DO:
        FOR EACH bf-eb FIELDS(yld-qty) NO-LOCK
            WHERE bf-eb.company EQ est.company
              AND bf-eb.est-no  EQ est.est-no
              AND bf-eb.form-no NE 0 :
     ...
     DO:
        ...
        FOR EACH bf-eb FIELDS(style) NO-LOCK WHERE
            bf-eb.company EQ est.company AND
            bf-eb.est-no  EQ est.est-no AND
            bf-eb.form-no NE 0 ,
            FIRST style FIELDS(TYPE) NO-LOCK WHERE
     ...
     IF v-assem-partition THEN
        FIND FIRST bf-eb-header NO-LOCK WHERE 
     ...
     IF NOT v-assem-partition OR (NOT AVAILABLE bf-eb-header) THEN DO:
...
------------------------------------------------------------------------------*/
   ...
   DEFINE OUTPUT PARAMETER op-dec AS DECIMAL DECIMALS 6 NO-UNDO.
    ...
    FIND FIRST tt-64-dec NO-LOCK WHERE 
      SUBSTRING(STRING(tt-64-dec.DEC),1,3) EQ SUBSTRING(STRING(ip-dec),1,3) NO-ERROR.
...
------------------------------------------------------------------------------*/
   ...
   IF AVAILABLE bf-eb OR eb.rec_key EQ "" THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VARIABLE lv-est-no LIKE est.est-no NO-UNDO.
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  DEFINE VARIABLE lv-qty LIKE est-qty.eqty NO-UNDO.

  DEFINE BUFFER b-eb FOR eb.


  IF AVAILABLE est-qty AND AVAILABLE eb THEN DO:
    lv-qty = est-qty.eqty.

    FIND b-eb WHERE ROWID(b-eb) EQ ROWID(eb) NO-LOCK NO-ERROR.
    IF AVAILABLE b-eb AND b-eb.est-type EQ 8 THEN lv-qty = b-eb.bl-qty.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF ip-is-corr-style AND ip-dim NE 0 AND v-cecscrn-dec EQ NO THEN 
