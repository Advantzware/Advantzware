...
IF NOT ll-is-copy-record AND NOT ll-copied-from-eb THEN 
DO:
    IF (ll-new-record OR is-first-record)              THEN 
    DO:
        IF eb.stock-no = "" THEN 
        DO:
        ...
        FIND FIRST cust WHERE cust.company = gcompany AND
            ...
            NO-LOCK NO-ERROR.
        ASSIGN
            eb.cas-no = IF AVAILABLE cust AND cust.case-bundle <> "" THEN cust.case-bundle ELSE eb.cas-no
            eb.tr-no  = IF AVAILABLE cust AND cust.pallet <> "" THEN cust.pallet ELSE eb.tr-no.      
        ...
        DO:
            ...
            IF AVAILABLE ITEM AND index("G,S,T",ITEM.mat-type) > 0 AND ITEM.i-no <> "No Joint"
        ...
        IF NOT AVAILABLE cust THEN FIND cust WHERE cust.company = eb.company AND
                cust.cust-no = eb.cust-no
                NO-LOCK NO-ERROR.
        ...
        DO:  /* adder*/ 
            ...
            END.
        END.
        IF style.material[5] NE "" THEN 
        DO:  /* leaf label */
            FIND FIRST item  WHERE item.company EQ cocode
                AND item.i-no    EQ style.material[5]
                NO-LOCK NO-ERROR.
            IF AVAILABLE item THEN /*leaf-block:
        ...
        DEFINE VARIABLE lv-stackcode AS cha NO-UNDO.
