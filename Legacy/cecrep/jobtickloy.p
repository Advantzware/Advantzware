...
DEF VAR v-cnt1 AS INT NO-UNDO.
DEF VAR v-cnt2 AS INT NO-UNDO INIT 1.
DEF VAR v-cnt3 AS INT NO-UNDO.

/* gdm - 12040902 */
DEF VAR v-len-score2 AS cha EXTENT 30 NO-UNDO.
  ...
  if avail xeb then do:
    ...
    if avail xstyle /*and xstyle.type ne "d"*/ then do:
      ...
      if avail box-design-hdr then do:
        ...
        /* gdm - 12040902 */
        ASSIGN v-len-score = trim(v-len-score)
               v-score-type = "".

        IF v-len-score <> "" THEN DO:
           ASSIGN v-tmp-score = " ".
           DO v-icnt  = 1 TO LENGTH(v-len-score):

             IF (SUBSTRING(v-len-score,v-icnt,1) = "" AND
                 SUBSTRING(v-len-score,v-icnt  - 1,1) <> "") or
                 v-icnt = LENGTH(v-len-score) THEN
                 ASSIGN v-jcnt  = v-jcnt  + 1
                        v-tmp-stype = xeb.k-wid-scr-type2[v-jcnt]
                        v-len-score2[v-jcnt] = v-tmp-stype
                        v-tmp-score = "".

             ELSE v-tmp-score = v-tmp-score + " " .
           END.

           DO v-icnt = 1 TO 13:
              IF v-len-score2[v-icnt] <> "" THEN
                 v-score-type = v-score-type + v-len-score2[v-icnt] + ",".
              ELSE
                 v-score-type = v-score-type + " ".
           END.
        END.
        
        ASSIGN v-len-score  = TRIM(v-len-score)
               v-len-score  = REPLACE(v-len-score,' ',',')
               v-score-type = REPLACE(v-score-type,' ',',')
               v-tmp-stype = "".

        DO v-icnt = 1 TO NUM-ENTRIES(v-len-score):          

          IF ENTRY(v-icnt,v-len-score) NE "" THEN
             ASSIGN v-tmp-stype = v-tmp-stype + " " + 
                                  ENTRY(v-icnt,v-len-score) + " ".
          
          IF ENTRY(v-icnt,v-score-type) NE "" THEN 
             ASSIGN v-tmp-stype = v-tmp-stype +  
                                  ENTRY(v-icnt,v-score-type).
        END.
        ASSIGN v-len-score  = TRIM(v-tmp-stype).
        /* gdm - 12040902 end */
...
if v-standards then do:
  ...
  for each job-mat
    ...
    if first(job-mat.i-no) then do:
      ...
      /* gdm - USE NET SHEET - 12040902 */
      if v-form-len eq 0 THEN v-form-len  = job-mat.len.
      if v-form-wid eq 0 THEN v-form-wid  = job-mat.wid.
...
for each job-mch
  ...
  if v-1st-dc AND job-mch.dept NE "CR" /* and
