...
DEF var v-dept-note AS cha FORM "x(52)" EXTENT 6 NO-UNDO.
...
{cecrep/jobprem.i}
      ...
      for each w-ef WHERE (w-ef.frm = job-hdr.frm OR est.est-type <> 8),
       ...
       if v-format eq "RFC" or v-format eq "Boxtech" then
        ...
        DISPLAY "<=7><C30>Department Notes" AT 2   "Shipping Info: <P10>" AT 92 SKIP
                "Ship To #:" AT 90
                ...
                v-dept-note[1] AT 35 v-shp[1] AT 90 SKIP
                v-dept-note[2] AT 35 v-shp[2] AT 90 SKIP
                v-dept-note[3] AT 35 v-shp[3] AT 90 SKIP
                v-dept-note[4] AT 35 v-shp[4] AT 90 SKIP
                v-dept-note[5] AT 35 "Item PO #:" AT 90 xoe-ordl.po-no when avail xoe-ordl
                v-dept-note[6] AT 35 SKIP
