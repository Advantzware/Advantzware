/* ----------------------------------------------  */
/*  cecrep/joboklma.p  factory ticket  for United <= Xprint landscape */
/* -------------------------------------------------------------------------- */
...
DEF VAR v-board-qty-1 AS INT NO-UNDO.
DEF VAR v-board-qty-2 AS INT NO-UNDO.
...
{cecrep/jobprem.i}
    ...
    ASSIGN v-perf = YES
           ...
           v-reversed = (xef.xgrain = "B" OR xef.xgrain = "S")
           v-num-tags = 1 
           .
...
===*/    
      ...
      IF AVAILABLE w-ef THEN DO:
        ...
        RUN cecrep/jobuntd2.p (RECID(job-hdr),v-format,cust.terms).
