...
{cecrep/jobblur.i "new shared"}

{sys/inc/VAR.i SHARED}
{cec/msfcalc.i}
{custom/notesdef.i}
DEF VAR v-inst2 AS cha EXTENT 8 NO-UNDO.    
DEF VAR v-dept-inst AS cha FORM "x(80)" EXTENT 8 NO-UNDO.
    ...
    for each job-hdr
      ...
      for each w-ef:
        ...
        v-due-date = "".
          ...
          DO:
             v-due-date = string(b-oe-rel.rel-date) .
             RELEASE b-oe-rel.
          END.
        ...
        DISP "<=#1><B>CUSTOMER" AT 2 "SHIP TO:" AT 35
             ...
             "DATE" AT 61 "REL DATE " AT 73 "TOTAL SQ FT" AT 83 SKIP
              ...
              "MATERIAL:" AT 49 trim(v-form-dscr) FORM "X(30)" SKIP
              xeb.part-dscr2 WHEN AVAIL xeb AT 5 FORM "x(25)"
              "ADDER:" AT 49 v-adders FORM "x(33)"
              SKIP
              itemfg.part-dscr2 WHEN AVAIL itemfg AT 5
              "SCORING:" AT 49  v-len-score  format "x(32)"
              SKIP
              ...
              "ID   :" AT 2 xeb.part-no FORM "x(30)" "PRINTING:"  AT 49 xeb.i-coldscr when avail xeb
               SKIP
              "SCORING:" AT 2 lv-len-score2 "DIE#:" AT 49         xeb.die-no when avail xeb
               ...
               "BALE:" AT 35 trim(string(xeb.tr-cnt)) when avail xeb 
               "ITEM SQ FT:" AT 55 trim(string(v-blk-sqft)) 
               SKIP(1)
        ...
        {custom/notespr2.i job v-inst2 8 "notes.rec_key = job.rec_key and
                            (notes.note_form_no = w-ef.frm OR notes.note_form_no = 0)" }
        DO i = 1 TO 8:
           v-dept-inst[i] = v-inst2[i].
        END.
        /*IF v-ship <> "" THEN v-dept-inst[8] = v-ship.  /* shipto notes */ */
        
        display "SPECIAL INSTRUCTIONS:" AT 1
                v-dept-inst[1] FORM "x(82)" AT 2
                v-dept-inst[2] format "x(82)" AT 2 
                v-dept-inst[3] format "x(82)" AT 2   
                v-dept-inst[4] format "x(82)" AT 2 
                v-dept-inst[5] format "x(82)" AT 2
                v-dept-inst[6] format "x(82)" AT 2
                v-dept-inst[7] format "x(82)" AT 2
                v-dept-inst[8] format "x(82)" AT 2
                "</B>"
                SKIP
