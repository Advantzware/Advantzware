     ...
     /*   if v-format eq "Brick" or v-format eq "Corrugat" OR  v-format eq "ASI"
      ...
      /*  if v-format eq "RFC" or v-format eq "Boxtech" then
       ...
       /*===== for xprint */
        ...
        PUT "<#2><R+5><C+26><RECT#2><|3>"
            "<#3><R-5><C+26><RECT#3><|3>"
            "<#4><R+5><C+26><RECT#4><|3>" SKIP.
        /* 3 boxes for board */
        PUT "B".
        /* 3 Boxes for customer */
        ...
        DISP "<=#2> CUSTOMER INFORMATION" SKIP
            ...
            trim(string( (if avail xoe-ordl then xoe-ordl.qty
                                            ...
                                            format "x(7)"
            "<=#3><R-2> Cust Part #:" lv-part-no 
            "<=#3><R-1> Overrun:" 
             trim(string(lv-over-run,">>9%")) format "x(4)" SPACE(1) 
             trim(string(lv-ord-qty * ( 1 + round((lv-over-run) / 100,2)),">>>,>>9")) format "x(7)"
            "Underrun:"
            trim(string(lv-under-run,">>9%")) format "x(4)" SPACE(1)
            trim(string(lv-ord-qty * ( 1 - round((lv-under-run) / 100,2)),">>>,>>9")) format "x(7)"
     ...
     /*   IF CAN-DO("TriState,RFC,Boxtech,Brick,Corrugat,ASI,Xprint,Pacific,TriLakes",v-format) THEN do: */
      ...
      /* for xprint */
        display "<=#5><R+5>" v-qty-or-sup
                     "<=#6><R-1> Color Desc:"
                     "<=#6><R-1><C+7> " xeb.i-coldscr  when avail xeb FORMAT "x(30)"
                     "<=#7><R+5> D/C Style:"                             
      ...
      /*  if v-format eq "Brick" OR v-format = "ASI" then 
        ...
        FOR EACH notes WHERE notes.rec_key = itemfg.rec_key
            ...
            AND CAN-DO(spec-list,notes.note_code) NO-LOCK.
            ...
            DO i = 1 TO LENGTH(notes.note_text) :        
               ...
               IF k < 7 THEN v-inst2[k] = v-inst2[k] + IF SUBSTRING(notes.note_text,i,1) <> CHR(10) THEN SUBSTRING(notes.note_text,i,1) 
                                  ELSE "" .              
        ...
        display "<=#10><R-1>" /* -1*/
             ...
             /*   "      Ship To #:"
                ...
                "O"                       at 1
                v-dept-inst[1] format "x(65)" AT 3
                v-shpnote[1] format "x(50)" AT 70
                /*chr(124) format "x"       at 87
                chr(124) format "x"       at 90
                v-shp[1] */
                "T"                       at 1
                v-dept-inst[2] format "x(65)" AT 3
                v-shpnote[2] format "x(50)" AT 70
            /*    chr(124) format "x"       at 87
                chr(124) format "x"       at 90
                v-shp[2] */
                "E"                       at 1
                v-dept-inst[3] format "x(65)" AT 3
                v-shpnote[3] format "x(50)" AT 70
              /*  chr(124) format "x"       at 87
                chr(124) format "x"       at 90
                v-shp[3] */
                "S"                       at 1
                v-dept-inst[4] format "x(65)" AT 3
                v-shpnote[4] format "x(50)" AT 70
              /* chr(124) format "x"       at 87
                chr(124) format "x"       at 90
                v-shp[4] */
                v-dept-inst[5] format "x(65)" AT 3
            /*    chr(124) format "x"       at 87
                chr(124) format "x"       at 90 
                "Item PO #:"
                xoe-ordl.po-no when avail xoe-ordl */
                v-dept-inst[6] format "x(65)" AT 3
                skip(1)
