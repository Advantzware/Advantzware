    ...
    for each job-hdr
      ...
      for each w-ef WHERE (w-ef.frm = job-hdr.frm OR est.est-type <> 8),
       ...
       if v-format eq "RFC" or v-format eq "Boxtech" then
        ...
        v-form-sqft = round(if v-corr then (nsh-len * nsh-wid * .007)
                                       else (nsh-len * nsh-wid / 144),3).
        ...
        IF AVAIL xeb THEN
           FIND FIRST b-itemfg WHERE
                b-itemfg.company = xeb.company AND
                b-itemfg.i-no = xeb.stock-no
                NO-LOCK NO-ERROR.

        IF AVAIL b-itemfg THEN
           lv-fg-name = b-itemfg.i-name.
        ELSE
           lv-fg-name = "".
        ...
        DISP  "<B><p12>" v-cus[1] 
              ...
              "Cust Part#:" AT 40 xeb.part-no FORM "x(25)"
              "Status:" AT 2 lv-status  SUBSTRING(lv-part-name,23,30) AT 57               
              "<P8>"SKIP
              "<U>Board:</U>" AT 15 "<P10>"  " LF:" AT 40 v-lf "<P8>" " <U>Printing:</U>" AT 70 " <U>Die Cutting, Slit, & Saw</U>" AT 130 "<P10>"SKIP
              ...
              " " +
              "L:" + trim(string({sys/inc/k16v.i xef.nsh-len},">,>>9.99"))  WHEN FIRST-OF(w-ef.frm) AND NOT v-see-1st-blank format "x(22)"  @ v-w-l AT 2
              "SEE 1st BLANK" WHEN v-see-1st-blank @ v-w-l   
              "MSF:"  + trim(string(v-dc-qty * v-form-sqft / 1000,">>>9.9<")) format "x(11)" WHEN FIRST-OF(w-ef.frm) AND NOT v-see-1st-blank              "Ink 1:" AT 39 v-ink-1
              "Gross Size:" AT 80  
        ...
        run cecrep/jobdel2.p (recid(job-hdr),v-format,cust.terms).        
        i = 0.
        for each w-m:
          i = i + 1.
        end.
          ...
          IF i > 1 THEN DO:
             ...
             FOR EACH xeb NO-LOCK 
                 ...
                 IF xeb.yld-qty LT 0 THEN
                   PUT -1 / xeb.yld-qty FORMAT ">>>>>>9.9<<<<<<" AT 67.
                 ELSE
                   PUT xeb.yld-qty FORMAT ">>>>>>9.9<<<<<<" AT 67.
