/* --------------------------------------------- cec/rep/jobtick8.p 04/97 BPV */
/* factory ticket - created for Soule to return set-header orderline from set component*/
/* -------------------------------------------------------------------------- */
...
/* Get order for set header given a set component */
IF job-hdr.ord-no NE 0 AND NOT AVAIL xoe-ordl THEN
FIND FIRST xfg-set
    WHERE xfg-set.company EQ cocode
      AND xfg-set.part-no EQ job-hdr.i-no
    NO-ERROR.
IF AVAIL xfg-set THEN DO:
    find first xoe-ordl
        where xoe-ordl.company eq cocode
          and xoe-ordl.ord-no  eq job-hdr.ord-no
          and xoe-ordl.i-no    eq xfg-set.set-no
        no-error.
    RELEASE xfg-set.
END.
...
if avail xoe-ordl then do:
  ...
  /*only if last copy printed set printed*/
  IF v-loc-loop = v-loc-copies THEN
...
IF production AND
   job.cs-trans-date NE ? AND
  /*only if last copy printed set printed*/
  v-loc-loop = v-loc-copies THEN DO:
  li = 0.
  DO WHILE li LT 1000:
    li = li + 1.
...
ELSE DO:
  li = 0.
  IF NOT job-hdr.ftick-prnt AND
      /*only if last copy printed set printed*/
     v-loc-loop = v-loc-copies THEN DO WHILE li LT 1000:
    li = li + 1.
...
ASSIGN
   ...
   v-due-date  = if avail xoe-ordl and
                    CAN-DO("Triad,Brick,ASI,Xprint,RFC,TriLakes,LoyLang",v-format) then
                   xoe-ordl.req-code + " " +
    ...
    li = 0.
IF CAN-DO("Soule",v-format) THEN
   v-due-date = string(job.due-date,"99/99/99").

IF CAN-DO("ARTIOS,Peachtree",v-format) THEN
   v-due-date = (IF AVAIL xoe-ordl THEN
                 TRIM(xoe-ordl.req-code) ELSE "ON") + " " + v-due-date.
/* gdm - 11160905 */
IF v-format EQ "Trilakes2" THEN DO:
  FIND FIRST bf-oe-ord NO-LOCK
    WHERE bf-oe-ord.company EQ xoe-ordl.company 
      AND bf-oe-ord.ord-no EQ xoe-ordl.ord-no NO-ERROR.
  IF AVAIL bf-oe-ord  
    THEN ASSIGN v-due-date = TRIM(bf-oe-ord.due-code) + " " +
                             IF bf-oe-ord.prod-date EQ ? 
                               THEN ""
                               ELSE STRING(bf-oe-ord.prod-date,"99/99/99").
    ELSE ASSIGN v-due-date = "".
END.
