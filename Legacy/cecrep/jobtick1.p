...
if avail xoe-ordl then do:
  ...
  FIND FIRST xoe-rel
      ...
      NO-LOCK NO-ERROR.
  ELSE IF v-format EQ "Spectrum" OR v-format EQ "Allwest" THEN
  DO: 
     FOR EACH xoe-rel
         WHERE xoe-rel.company EQ xoe-ordl.company
           AND xoe-rel.ord-no  EQ xoe-ordl.ord-no
           ...
           IF  v-format EQ "Spectrum" AND LOOKUP(lv-stat,"C,Z") = 0 THEN
           DO: 
              lv-rowid = ROWID(xoe-rel).
              LEAVE.
           END.

           IF  v-format EQ "Allwest" AND LOOKUP(lv-stat,"S,L,I") GT 0 THEN       /*Task# 01221411*/
           DO: 
              lv-rowid = ROWID(xoe-rel).
              LEAVE.
           END. 
...
/*       AND bf-oe-ord.ord-no EQ xoe-ordl.ord-no NO-ERROR. */
  IF AVAIL xoe-ord AND AVAIL xoe-ordl AND xoe-ordl.spare-int-2 > 0 
    THEN ASSIGN v-due-date = TRIM(xoe-ord.due-code) + " " +
                             STRING(DATE(xoe-ordl.spare-int-2),"99/99/99").
