/* ----------------------------------------------  */
/*  cecrep/jobtickt2.p  Corrugated factory ticket  for Sutherland */
/* -------------------------------------------------------------------------- */
      ...
      for each w-ef:
       ...
       /* v-qty-or-sup = v-qty-or-sup + fill("_",38 - length(v-qty-or-sup)).*/
        ...
        DISP  v-cus[1] AT 2 "PO #:" AT 40 xoe-ord.po-no WHEN AVAIL xoe-ord 
              ...
              SKIP
              "Board:<B>" AT 2 v-form-code FORM "x(20)" "</B><C32.6>2:"  v-ink-2 "6:" v-ink-6 "<C66.8>Net   Size:" /*AT 80*/
...
*/
      ...
      */
        ...
        RUN cecrep/jobsuth2.p (RECID(job-hdr),v-format,cust.terms).
...
ASSIGN v-spec-note = ""
        ...
        FOR EACH notes WHERE notes.rec_key = itemfg.rec_key NO-LOCK.
            lv-text = notes.note_title + ":" + notes.note_text.
            DO i = 1 TO LENGTH(lv-text) :        
               ...
               k = v-tmp-lines + lv-got-return.
               IF k < 9 THEN v-spec-note[k] = v-spec-note[k] + IF SUBSTRING(lv-text,i,1) <> CHR(10) THEN SUBSTRING(lv-text,i,1) 
                                  ELSE "" .              
           
               IF SUBSTRING(lv-text,i,1) = CHR(10) OR SUBSTRING(lv-text,i,1) = CHR(13)                 
               THEN do:
                  lv-got-return = lv-got-return + 1.
                  j = i.
               END.         
