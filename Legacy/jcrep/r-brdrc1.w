...
&Scoped-Define ENABLED-OBJECTS rd_jstat begin_inv-date end_inv-date ~
...
&Scoped-Define DISPLAYED-OBJECTS lbl_jstat rd_jstat begin_inv-date ~
end_inv-date begin_job-no begin_job-no2 end_job-no end_job-no2 rd-dest ~
...
DEFINE FRAME FRAME-A
     lbl_jstat AT ROW 3.14 COL 32 COLON-ALIGNED NO-LABEL
     rd_jstat AT ROW 3.14 COL 47 NO-LABEL
     begin_inv-date AT ROW 4.81 COL 27 COLON-ALIGNED
     end_inv-date AT ROW 4.81 COL 69 COLON-ALIGNED HELP
          "Enter Ending Due Date"
     begin_job-no AT ROW 6 COL 27 COLON-ALIGNED HELP
...
DO:
  ...
  case rd-dest:
       ...
       END.
       when 5 then do:
           {custom/asimail.i &begin_cust=begin_job-no
                             &END_cust=END_job-no
...
------------------------------------------------------------------------------*/
  DISPLAY lbl_jstat rd_jstat begin_inv-date end_inv-date begin_job-no 
          begin_job-no2 end_job-no end_job-no2 rd-dest lv-ornt lines-per-page 
          lv-font-no lv-font-name td-show-parm 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE rd_jstat begin_inv-date end_inv-date begin_job-no begin_job-no2 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE run-tt-report C-Win 
...
def var v-mat-qty as   dec format "->>>,>>>,>>9" extent 3.
def var v-fg-qty  like v-mat-qty.
def var v-diff    like v-mat-qty                 extent 1.
...
END.
    ...
    for each tt-report where tt-report.term-id eq "",
      ...
      v-mat-qty[1] = round(v-mat-qty[1] * v-pct * v-on,0).
      ...
      assign
       v-mat-qty[2] = v-mat-qty[2] + v-mat-qty[1]
      ...
      if last-of(tt-report.key-01) then do:
        ...
        assign
         v-mat-qty[3] = v-mat-qty[3] + v-mat-qty[2]
      ...
      if last(tt-report.key-01) then do:
        ...
        v-diff[1] = v-mat-qty[3] - v-fg-qty[3].

        clear frame det no-pause.

        display ""              @ tt-report.key-01
                "Grand Totals"  @ cust.name
                v-mat-qty[3]    @ v-mat-qty[1]
      ...
      if last-of(tt-report.key-01) then delete tt-report.
    end.
