...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "MACH CODE,JOB #,FG DESCRIPTION,EST M/R,ACT M/R,%VAR M/R,EST RUN," +
                                              "ACT RUN,%VAR RUN,EST BRD,ACT BRD,%VAR MAT" .
...
tb_show-dl tb_show-fo tb_show-vo rd-dest lv-ornt lines-per-page lv-font-no ~
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-mchcst.csv" 
...
DEFINE FRAME FRAME-A
     lbl_jstat AT ROW 3.14 COL 26 COLON-ALIGNED NO-LABEL
     rd_jstat AT ROW 3.14 COL 41 NO-LABEL
     begin_mach AT ROW 4.81 COL 24 COLON-ALIGNED HELP
          "Enter Beginning Machine"
     end_mach AT ROW 4.81 COL 67 COLON-ALIGNED HELP
     ...
     tb_show-dl AT ROW 8.38 COL 34
     tb_show-fo AT ROW 9.33 COL 34
     tb_show-vo AT ROW 10.29 COL 34
     sl_avail AT ROW 12.52 COL 4 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 12.52 COL 40 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 12.52 COL 59.4 NO-LABEL WIDGET-ID 28
     Btn_Add AT ROW 13.52 COL 40 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 14.52 COL 40 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 15.57 COL 40 WIDGET-ID 40
     btn_down AT ROW 16.57 COL 40 WIDGET-ID 42
     rd-dest AT ROW 19 COL 6 NO-LABEL
     lv-ornt AT ROW 19.24 COL 31 NO-LABEL
     lines-per-page AT ROW 19.24 COL 84 COLON-ALIGNED
     lv-font-no AT ROW 20.67 COL 34 COLON-ALIGNED
     lv-font-name AT ROW 21.62 COL 28 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 23.05 COL 7
     tb_excel AT ROW 23.05 COL 69 RIGHT-ALIGNED
     tb_runExcel AT ROW 23.05 COL 90 RIGHT-ALIGNED
     fi_file AT ROW 23.76 COL 44 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 25.91 COL 27
     btn-cancel AT ROW 25.91 COL 58
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 11.81 COL 4.8 WIDGET-ID 38
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 18.05 COL 4
     "Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.24 COL 5
          BGCOLOR 2 
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 11.81 COL 59.4 WIDGET-ID 44
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Machine Cost By Job Report"
         HEIGHT             = 27.24
         WIDTH              = 95.6
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 204.8
...
ON END-ERROR OF C-Win /* Machine Cost By Job Report */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Machine Cost By Job Report */
DO:
...
DO:
   apply "close" to this-procedure.
    {Advantzware/WinKit/winkit-panel-triggerend.i} /* added by script _nonAdm1.p on 03.28.2017 @ 10:42:55 am */
END.
...
ON CLOSE OF THIS-PROCEDURE DO:
   RUN disable_UI.
   {Advantzware/WinKit/closewindow-nonadm.i} /* added by script _nonAdm1.p on 03.28.2017 @ 10:42:55 am */
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
 ...
 /* security check need {methods/prgsecur.i} in definition section */
  ...
  END.
    ...
    {methods/setButton.i btn-cancel "Cancel"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:43:39 am */
    {methods/setButton.i btn-ok "OK"} /* added by script _nonAdm1Images.p on 03.28.2017 @ 10:43:39 am */
    {Advantzware/WinKit/embedfinalize-nonadm.i} /* added by script _nonAdm1.p on 03.28.2017 @ 10:42:55 am */
...
------------------------------------------------------------------------------*/
  DISPLAY lbl_jstat rd_jstat begin_mach end_mach begin_job-no begin_job-no2 
          end_job-no end_job-no2 begin_date end_date tb_show-dl tb_show-fo 
          tb_show-vo rd-dest lv-ornt lines-per-page lv-font-no lv-font-name 
          td-show-parm tb_excel tb_runExcel fi_file sl_avail sl_selected
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-6 RECT-7 rd_jstat begin_mach end_mach begin_job-no begin_job-no2 
         end_job-no end_job-no2 begin_date end_date tb_show-dl tb_show-fo 
         tb_show-vo rd-dest lv-ornt lines-per-page lv-font-no td-show-parm 
         tb_excel tb_runExcel fi_file btn-ok btn-cancel sl_avail
         Btn_Def sl_selected Btn_Add Btn_Remove btn_Up btn_down
      WITH FRAME FRAME-A IN WINDOW C-Win.
...
def var show-direct-labor as log no-undo.
def var show-fixed-overhead as log no-undo.
def var show-variable-overhead as log no-undo.
def var v-mr-act-dl as dec format '->>>>>9.99' extent 3 no-undo.
def var v-mr-act-fo like v-mr-act-dl no-undo.
def var v-mr-act-vo like v-mr-act-dl no-undo.
def var v-mr-est-dl like v-mr-act-dl no-undo.
def var v-mr-est-fo like v-mr-act-dl no-undo.
def var v-mr-est-vo like v-mr-act-dl no-undo.
def var v-run-act-dl like v-mr-act-dl no-undo.
def var v-run-act-fo like v-mr-act-dl no-undo.
def var v-run-act-vo like v-mr-act-dl no-undo.
def var v-run-est-dl like v-mr-act-dl no-undo.
def var v-run-est-fo like v-mr-act-dl no-undo.
def var v-run-est-vo like v-mr-act-dl no-undo.
...
def var v-vtyp  as   char format "!"    init "M".

def var v-up-hs  like eb.num-up.
def var v-up     like eb.num-up.
def var v-out    like est-op.n-out.
def var v-on     like eb.num-up.
def var v-est-fg like job-mch.run-qty.
def var v-speed  as   dec.
def var v-run-hr as   dec.
def var v-tot-fg like mch-act.qty extent 2.
def var v-run-fg like mch-act.qty extent 2.
def var v-prd-fg like mch-act.qty extent 3.
def var v-tot-rm like v-prd-fg.
def var v-qty    as   dec.
def var v-tqty   like v-qty.
def var v-cost   like job-mat.std-cost.
...
def var v-mr-est  as   dec format "->>>>>9.99" extent 3.
def var v-mr-act  like v-mr-est.
def var v-run-est like v-mr-est.
def var v-run-act like v-mr-est.
def var v-mat-est like v-mr-est.
def var v-mat-act like v-mr-est.
def var v-est-hr  as   dec.
...
assign
 ...
 {sys/inc/ctrtext.i str-tit2 112}
  ...
  v-fdate                = begin_date
  v-tdate                = END_date
  show-direct-labor      = tb_show-dl
  show-fixed-overhead    = tb_show-fo
  show-variable-overhead = tb_show-vo.
...
DEF VAR cslist AS cha NO-UNDO.
 FOR EACH ttRptSelected BY ttRptSelected.DisplayOrder:
   ...
   ASSIGN str-tit4 = str-tit4 + 
        ...
        IF LOOKUP(ttRptSelected.TextList, "EST M/R,ACT M/R,%VAR M/R,EST RUN,ACT RUN,%VAR RUN,EST BRD,ACT BRD,%VAR MAT") <> 0    THEN
         ASSIGN
