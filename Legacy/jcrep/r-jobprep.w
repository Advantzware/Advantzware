...
DEFINE VARIABLE rsGroupBy AS CHARACTER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Customer", "Vendor",
"Prep Code", "Job#"
...
DEFINE FRAME FRAME-A
     ...
     rsGroupBy AT ROW 7.19 COL 31 NO-LABEL WIDGET-ID 14
     ...
     "Group by:" VIEW-AS TEXT
          SIZE 11 BY .62 AT ROW 7.43 COL 15 WIDGET-ID 22
...
ON LEAVE OF from_date IN FRAME FRAME-A /* Beginning Date */
DO:
  assign {&self-name}.
END.
...
ON LEAVE OF to_date IN FRAME FRAME-A /* Ending Date */
DO:
  assign {&self-name}.
END.
...
DEF VAR cCustomerName AS cha FORM "x(30)" NO-UNDO.
DEF VAR cPrepDscr AS cha FORM "x(30)" NO-UNDO.

IF rsGroupBy BEGINS "Customer" THEN
   lcHeader = "Customer Code  Customer Name      Due Date  Job#   Prep Code  Code Description   Qty   Cost    Extended".
ELSE 
   lcHeader = "Prep Code  Code Description   Customer Code  Customer Name      Due Date  Job#   Qty   Cost    Extended".
...
for each job-hdr
     ...
     EACH ASI.job-prep WHERE job-prep.company eq job-hdr.company and
                    job-prep.job eq job-hdr.job and
                    job-prep.job-no eq job-hdr.job-no and
                    job-prep.job-no2 eq job-hdr.job-no2 AND
                    job-prep.CODE GE begin_prep AND
                    job-prep.CODE LE END_prep NO-LOCK
...
for each job-hdr
    ...
    IF LAST-OF(job-hdr.job-no) THEN DO:
       ...
       display 
            ...
            ldExtCost / liQty @ ldStdCost
            ldExtCost
            with frame detPrep STREAM-IO width 132 no-box no-labels down.
    END.
