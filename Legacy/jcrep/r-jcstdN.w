...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Job #,Item ID,Item Name,Category,Cust Part #,Style,Job Qty,Estimate,Form,Blank,Order,Customer,Customer Name," + 
                           "Calc Tim,Calc By,Std Prep Material,Std Misc Material,Std Prep Labor,Std Misc Labor,Std Direct Material,Std Direct Labor," +
                           "Std Var Overhead,Std Fixed Overhead,Std Total Factory,Std Commission,Std Freight,Std Full Cost,Std Gross Profit,Std Gross Margin," +
                           "Std Net Profit,Std Net Margin,Std Sell Price,Sell Price,Booked/Std Price".
...
DEFINE FRAME FRAME-A
     ...
     lbl_sort AT ROW 7.81 COL 27.4 COLON-ALIGNED NO-LABEL WIDGET-ID 74
     rd_sort AT ROW 7.81 COL 39.4 NO-LABEL WIDGET-ID 76
...
DEFINE VARIABLE v-sort          as   char               format "!" init "J" no-undo.
...
FOR EACH costHeader NO-LOCK 
    ...
    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
       cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            CASE cTmpField:             
                 WHEN "job"       THEN cVarValue = STRING(costHeader.jobNo + "-" + STRING(costHeader.jobNo2,"99")).
                 WHEN "item"      THEN cVarValue = STRING(costHeader.fgItemID,"X(15)").
                 WHEN "item-name"    THEN cVarValue = IF AVAIL itemfg THEN SUBSTRING(itemfg.i-name,1,25) ELSE "".
                 WHEN "category"    THEN cVarValue = IF AVAIL itemfg THEN itemfg.procat ELSE eb.procat.
                 WHEN "cust-part"    THEN cVarValue = IF AVAILABLE itemfg THEN itemfg.part-no ELSE eb.part-no.
                 WHEN "style" THEN cVarValue = IF AVAILABLE itemfg THEN itemfg.style ELSE eb.style .
                 WHEN "job-qty"      THEN cVarValue = IF dQtyInM NE 0 THEN STRING((dQtyInM * 1000),"->>>>,>>>,>>9") ELSE "".
                 WHEN "est" THEN cVarValue = STRING(costHeader.estimateNo,"x(8)").
                 WHEN "form"     THEN cVarValue = string(costHeader.formNo,">>>9")  .
                 WHEN "blank"      THEN cVarValue = string(costHeader.blankNo,">>>>9") .
                 WHEN "order"   THEN cVarValue = IF AVAILABLE job-hdr THEN string(job-hdr.ord-no,">>>>>>9") ELSE "".
                 ...
                 WHEN "calc-tm"  THEN cVarValue = string(costHeader.calculationTime)   .
                 WHEN "calc-by"    THEN cVarValue = STRING(costHeader.calculatedBy,"x(8)").
                 WHEN "std-prp-mat"    THEN cVarValue = STRING((costHeader.stdCostPrepMaterial / dQtyInM),"->>>>,>>>,>>>,>>9").
                 WHEN "std-mis-mat"       THEN cVarValue = STRING((costHeader.stdCostMiscMaterial / dQtyInM),"->>>>,>>>,>>>,>>9").
                 WHEN "std-prp-lab"      THEN cVarValue = STRING((costHeader.stdCostPrepLabor / dQtyInM),"->,>>>,>>>,>>9").
                 WHEN "std-misc-lab"    THEN cVarValue = STRING((costHeader.stdCostMiscLabor / dQtyInM),"->,>>>,>>>,>>9").
                 WHEN "std-dir-mat"    THEN cVarValue = STRING((costHeader.stdCostDirectMaterial / dQtyInM),"->>,>>>,>>>,>>9.99").
                 WHEN "std-dir-lab"    THEN cVarValue = STRING((costHeader.stdCostDirectLabor / dQtyInM),"->>>>,>>>,>>9.99").
                 WHEN "std-var-over" THEN cVarValue = STRING((costHeader.stdCostVariableOverhead / dQtyInM),"->>>>,>>>,>>9.99").
                 WHEN "std-fix-over"      THEN cVarValue = STRING((costHeader.stdCostFixedOverhead / dQtyInM),"->>,>>>,>>>,>>9.99").
                 WHEN "std-tot-fac" THEN cVarValue = STRING((costHeader.stdCostTotalFactory / dQtyInM),"->,>>>,>>>,>>9.99").
                 WHEN "std-comm"     THEN cVarValue = STRING((costHeader.stdCostCommission / dQtyInM),"->>,>>>,>>9.99").
                 WHEN "std-frt"      THEN cVarValue = STRING((costHeader.stdCostFreight / dQtyInM),"->>>,>>9.99").
                 WHEN "std-ful-cst"   THEN cVarValue = STRING((costHeader.stdCostFull / dQtyInM),"->,>>>,>>9.99").
                 WHEN "std-gross-prft"     THEN cVarValue = STRING((costHeader.stdProfitGross / dQtyInM),"->>>>,>>>,>>9.99").
                 WHEN "std-gross-mar"  THEN cVarValue = IF costHeader.stdSellPrice GT 0 THEN STRING(((costHeader.stdProfitGross / costHeader.stdSellPrice) * 100),"->>,>>>,>>>,>>9%") ELSE "".
                 WHEN "std-net-prft"  THEN cVarValue = STRING((costHeader.stdProfitNet / dQtyInM),"->>,>>>,>>9.99").
                 WHEN "std-net-mar"    THEN cVarValue = IF costHeader.stdSellPrice GT 0 THEN STRING(((costHeader.stdProfitNet / costHeader.stdSellPrice) * 100),"->>>>,>>>,>>9%") ELSE "".
    ...
    ASSIGN cDisplay = ""
           ...
           DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               ...
               CASE cTmpField:             
                 ...
                 WHEN "category"           THEN cVarValue = "".
                 WHEN "cust-part"          THEN cVarValue = "".
                 WHEN "style"              THEN cVarValue = "".
                 WHEN "job-qty"            THEN cVarValue = IF dQtyInM NE 0 THEN STRING(cToTJobQty,"->>>>,>>>,>>9") ELSE "".
                 WHEN "est"                THEN cVarValue = "".
                 WHEN "form"               THEN cVarValue = "".
                 WHEN "blank"              THEN cVarValue = "".
                 WHEN "order"              THEN cVarValue = "".
                 WHEN "cust"               THEN cVarValue = "".
                 WHEN "cust-name"          THEN cVarValue = "".
                 WHEN "calc-tm"            THEN cVarValue = "".
                 WHEN "calc-by"            THEN cVarValue = "".
                 WHEN "std-prp-mat"        THEN cVarValue = "".
                 WHEN "std-mis-mat"        THEN cVarValue = "".
                 WHEN "std-prp-lab"        THEN cVarValue = "".
                 WHEN "std-misc-lab"       THEN cVarValue = "".
                 WHEN "std-dir-mat"        THEN cVarValue = "".
                 WHEN "std-dir-lab"        THEN cVarValue = "".
                 WHEN "std-var-over"       THEN cVarValue = "".
                 WHEN "std-fix-over"       THEN cVarValue = "".
                 WHEN "std-tot-fac"        THEN cVarValue = STRING(cToTStdFac,"->,>>>,>>>,>>9.99").
                 WHEN "std-comm"           THEN cVarValue = STRING(cToTStdComm,"->>,>>>,>>9.99").
                 WHEN "std-frt"            THEN cVarValue = STRING(cToTStdFrt,"->>>,>>9.99").
                 WHEN "std-ful-cst"        THEN cVarValue = "".
                 WHEN "std-gross-prft"     THEN cVarValue = "".
                 WHEN "std-gross-mar"      THEN cVarValue = "".
                 WHEN "std-net-prft"       THEN cVarValue = "".
                 WHEN "std-net-mar"        THEN cVarValue = "".
