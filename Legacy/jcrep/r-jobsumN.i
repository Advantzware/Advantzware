          ...
          if v-over-pct = ? or v-over-pct lt 0 then v-over-pct = 0.    
           ...
           ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):   
                ...
                IF LOOKUP(ENTRY(i,cSelectedList), v-header-1) = 0    THEN NEXT .
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "fg-item"    THEN cVarValue = string(work-item.i-no,"x(15)") .
                         WHEN "fgitem-desc"   THEN cVarValue = string(itemfg.i-name,"x(30)").
                         WHEN "sel-price"   THEN cVarValue = STRING(work-item.price,"->>>>>9.99").
                         WHEN "order"  THEN cVarValue = STRING(v-qty-ord,">>,>>>,>>9") .
...
/*           v-a-spo-p = (v-act-spo / (v-act-qty * v-num-up) /* v-rm-qty[1]*/) * 100. */
      ...
      put /*fill("-",90) format "x(90)" at 48*/ fg-str-line skip.
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):    
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "fgitem-desc"   THEN cVarValue = "".
                         WHEN "sel-price"   THEN cVarValue = STRING(v-avg-prc,"->>>>>9.99").
                         WHEN "order"  THEN cVarValue = STRING(v-t-ord,">>,>>>,>>9") .
 ...
 ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):  
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "machine"    THEN cVarValue = string(work-mch.m-code,"x(7)") .
                         WHEN "mach-desc"   THEN cVarValue = string(mach.m-dscr,"x(16)").
                         WHEN "mr"   THEN cVarValue =  "M".
                         WHEN "qty-prod"   THEN cVarValue = "".
                         WHEN "est-hours"   THEN cVarValue = IF work-mch.est-mr-hr <> ? AND work-mch.est-mr-hr <> 0 THEN STRING(round(work-mch.est-mr-hr,2),">>>9.99-")  ELSE "".
                         WHEN "est-speed"  THEN cVarValue = "".
                         WHEN "est-cost"   THEN cVarValue = IF work-mch.est-mr-cost <> ? AND work-mch.est-mr-cost <> 0 THEN STRING(work-mch.est-mr-cost,">>>>,>>9-") ELSE "" .   
                         WHEN "act-hours"  THEN cVarValue = IF work-mch.mr-hr <> ? AND work-mch.mr-hr <> 0 THEN  STRING(work-mch.mr-hr,">>>>9.9-")  ELSE "". 
        ...
        IF v-lab                                                  AND
           ...
           RUN mach-data("Direct Labor",0,ROUND(work-mch.est-mr-cost1,0),0,ROUND(work-mch.mr-cost1,0),0,0).
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):  
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "mr"   THEN cVarValue =  "R".
                         WHEN "qty-prod"   THEN cVarValue = IF work-mch.run-qty <> ? AND work-mch.run-qty <> 0 THEN STRING(work-mch.run-qty,">>>>>>>9-")  ELSE "".
                         WHEN "est-hours"  THEN cVarValue = IF work-mch.est-run-hr <> ? AND work-mch.est-run-hr <> 0 THEN STRING(round(work-mch.est-run-hr,2),">>>9.99-") ELSE "" .
        ...
        IF v-lab                                                    AND
          ...
          RUN mach-data("Direct Labor",0,ROUND(work-mch.est-run-cost1,0),0,ROUND(work-mch.run-cost1,0),0,0).
         /* DISPLAY "Direct Labor"                                       AT 8
        ...
        if last(work-mch.d-seq) then do:
          ...
          PUT mach-str-line SKIP .
          RUN mach-data("TOTAL CONVERSION (DIRECT) :",v-t-est-hrs,v-t-est-cost,v-t-act-hrs,v-t-act-cost,v-cst-var,v-prod-p).
     ...
     if v-mch then put skip(2).  
      ...
      if can-find(FIRST work-mat) THEN do:
         put skip(1)
             /*"                                     <-----ESTIMATED------>"
      ...
      for each work-prep BREAK BY work-prep.work-ml by work-prep.code:
       ...
       /* display work-prep.code
        ...
        ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):    
                IF LOOKUP(ENTRY(i,cSelectedList), v-header-4) = 0    THEN NEXT .
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "misc-vari-per"  THEN cVarValue = STRING(v-prod-p,">>>,>>>9.9-") .
    ...
    IF v-tot-mchg THEN do: /* task 04291401 */
        IF LAST-OF(work-prep.work-ml) AND work-prep.work-ml = "no" THEN DO:
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):    
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "misc-code"    THEN cVarValue = "" .
                         WHEN "misc-desc"   THEN cVarValue = "" .
                         WHEN "misc-est-cost"   THEN cVarValue = STRING(v-sub-est-cost,">>,>>>,>>9-").
                         WHEN "misc-act-cost"  THEN cVarValue = STRING(v-sub-act-cost,">>,>>>,>>9-") .
                         WHEN "misc-vari"   THEN cVarValue = STRING(v-sub-cst-var,">>,>>>,>>9-") .
                         WHEN "misc-vari-per"  THEN cVarValue = STRING(v-sub-prod-p,">>>,>>>9.9-") .
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):    
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "misc-code"    THEN cVarValue = "" .
                         WHEN "misc-desc"   THEN cVarValue = "" .
                         WHEN "misc-est-cost"   THEN cVarValue = STRING(v-t-est-cost,">>,>>>,>>9-").
                         WHEN "misc-act-cost"  THEN cVarValue = STRING(v-t-act-cost,">>,>>>,>>9-") .
                         WHEN "misc-vari"   THEN cVarValue = STRING(v-cst-var,">>,>>>,>>9-") .
                         WHEN "misc-vari-per"  THEN cVarValue = STRING(v-prod-p,">>>,>>>9.9-") .
