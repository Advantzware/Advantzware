...
DEFINE VARIABLE lv-fax-image          AS cha       NO-UNDO.
DEFINE VARIABLE vcDefaultForm         AS CHARACTER NO-UNDO.
DEFINE VARIABLE vmDefaultForm         AS CHARACTER NO-UNDO.
DEFINE VARIABLE viDefaultLinesPerPage AS INTEGER   NO-UNDO.
DEFINE VARIABLE lv-termPath           AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-fmt-int             AS INTEGER   INIT 0 NO-UNDO.
DEFINE VARIABLE v-ack-master          AS LOG       INIT NO NO-UNDO .
DEFINE VARIABLE lv-attach-push        AS cha       NO-UNDO.
...
DEFINE NEW SHARED VARIABLE s-print-revised        AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE LvOutputSelection      AS CHARACTER NO-UNDO.
DEFINE NEW SHARED VARIABLE v-rs-whs-mths          AS CHARACTER NO-UNDO.
DEFINE NEW SHARED VARIABLE v-tg-whs-mths          AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE v-dept-codes           AS CHARACTER NO-UNDO. 
DEFINE NEW SHARED VARIABLE v-print-components     AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE v-print-po             AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE v-UntCnt               AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE v-Shpnot               AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE v-print-due            AS LOG       NO-UNDO.
DEFINE NEW SHARED VARIABLE v-print-tot            AS LOG       NO-UNDO.

DEFINE BUFFER b-oe-ord  FOR oe-ord.
DEFINE BUFFER b1-oe-ord FOR oe-ord.
DEFINE BUFFER b2-oe-ord FOR oe-ord.
...
DEFINE FRAME FRAME-A
    rd_ack-ordmst AT ROW 1.24 COL 34.4 NO-LABELS WIDGET-ID 38
    ...
    RS_whs-mths AT ROW 8.38 COL 31 NO-LABELS WIDGET-ID 2
    ...
    rd-dest AT ROW 16.91 COL 3.4 NO-LABELS
    lv-ornt AT ROW 17.19 COL 30 NO-LABELS
    lines-per-page AT ROW 17.19 COL 82 COLON-ALIGNED
    lv-font-no AT ROW 18.38 COL 32 COLON-ALIGNED
    lv-font-name AT ROW 19.33 COL 26 COLON-ALIGNED NO-LABELS
    sel-attch AT ROW 20.52 COL 61.4 NO-LABELS WIDGET-ID 14
...
ASSIGN 
    tb_act-rel:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_billnotes:HIDDEN IN FRAME FRAME-A       = TRUE
    tb_billnotes:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_hide_sell:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    ...
    tb_itempo:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_itm-tot:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_prt-bom:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_reprint:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_sch-rel:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_ship-to:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_shpnot:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_terms:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_untcnt:PRIVATE-DATA IN FRAME FRAME-A = "parm".
                ...
                DO:
                    IF CAN-FIND(FIRST b-oe-ord
                        ...
                        FOR EACH b-oe-ord FIELDS(company cust-no ord-no) WHERE
                            ...
                            IF FIRST-OF(b-oe-ord.cust-no) THEN 
                            DO:
...
ON HELP OF dept-code IN FRAME FRAME-A /* Dept Code */
    DO:
        ...
        DO i-cnt = 1 TO NUM-ENTRIES(spec-code):
            ...
            CREATE tt-specCd.
            ASSIGN 
                tt-char-val = TRIM(ENTRY(i-cnt,spec-code)).
        END.
...
ON LEAVE OF end_ord-no IN FRAME FRAME-A /* Ending Order# */
    DO:
        ASSIGN {&self-name}.
        IF begin_ord-no = END_ord-no THEN 
        DO:
            FIND FIRST oe-ord WHERE oe-ord.company = g_company
                AND oe-ord.ord-no = begin_ord-no NO-LOCK NO-ERROR.
            IF AVAILABLE oe-ord THEN ASSIGN begin_cust-no:SCREEN-VALUE = oe-ord.cust-no
...
ON VALUE-CHANGED OF rd_ack-ordmst IN FRAME FRAME-A
    DO:
        ...
        DO:  
            ...
            IF LOOKUP(v-print-fmt,"Soule,SouleUOM,ContSvc") = 0 THEN 
            DO:
            ...
            END.
            IF LOOKUP(v-print-fmt,"PremierX") <> 0 THEN 
            DO:
                IF tb_sch-rel:SCREEN-VALUE EQ "NO" AND
                    tb_act-rel:SCREEN-VALUE EQ "NO" THEN
                    ASSIGN
        ...
        DO:
            IF CAN-FIND(FIRST b-oe-ord
                ...
                FOR EACH b1-oe-ord FIELDS(company cust-no ord-no) WHERE
                    ...
                    IF FIRST-OF(b1-oe-ord.cust-no) THEN 
                    DO:
...
PROCEDURE buildToList:
    ...
    IF opToList EQ '' OR opToList = ? THEN 
    DO: 
...
PROCEDURE GenerateEmail:
    ...
    DEFINE INPUT PARAMETER icOrdNo AS INTEGER NO-UNDO.
    ...
    DEFINE VARIABLE ls-to-list2    AS cha       NO-UNDO.
  
    ASSIGN  
        vcSubject  = "ACKNOWLEDGEMENT:" + string(icOrdNo) + '   ' + STRING (TODAY, '99/99/9999') + STRING (TIME, 'HH:MM:SS AM')
...
PROCEDURE GenerateReport :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        DEFINE INPUT PARAMETER ip-cust-to AS CHARACTER NO-UNDO.
        DEFINE VARIABLE vcErrorMsg AS CHARACTER NO-UNDO.
        DEFINE VARIABLE gb         AS CHARACTER NO-UNDO .
...
PROCEDURE Get-cust-attch :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE VARIABLE v-attch-list AS CHARACTER NO-UNDO.
    DO WITH FRAME {&frame-name}:
    
        IF TG_eml-push-att:SCREEN-VALUE = "YES" THEN 
        DO:
...
PROCEDURE run-report :
    ...
    DO:
        CASE rd-dest:
            ...
            WHEN 5 THEN 
                DO:
                    IF TG_preview THEN PUT "<PREVIEW>". 
                    ELSE PUT "<PRINT=NO>".
...
PROCEDURE SetGlobalVariables :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER ip-ord-no AS INTEGER NO-UNDO.
...
PROCEDURE SetOEAckForm :
    ...
    CASE icPrintFormat:
        WHEN "WesInd" THEN 
            ASSIGN 
                v-program      = "oe/rep/wiackn.p" 
                is-xprint-form = NO 
                lines-per-page = 60.
        WHEN "HOP" THEN 
            ASSIGN 
                tb_ship-to     = NO 
                v-program      = "oe/rep/acknhop.p" 
                is-xprint-form = NO 
                lines-per-page = 45.
        WHEN "Brick" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackbrick.p" 
                is-xprint-form = NO 
                lines-per-page = 60.  
        WHEN "Gulf" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackgulf.p" 
                is-xprint-form = NO 
                lines-per-page = 55.
        WHEN "Pacific" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackpacif.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Hopx" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackhopx.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Carded" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackcared.p" 
                is-xprint-form = YES 
                lines-per-page = 68.  /* Task 09181303  */
        WHEN "XPrint" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackxprnt.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Badger" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackbager.p" 
                is-xprint-form = YES 
                lines-per-page = 67. /* task 04021401 */
        WHEN "Hughes" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackhughes.p" 
                is-xprint-form = YES 
                lines-per-page = 65. 
        WHEN "Accord" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackacord.p" 
                is-xprint-form = YES 
                lines-per-page = 72.
        WHEN "Imperial" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackimprl.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Ruddx" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackruddx.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Shelby" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackshlby.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "PremierX" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackxprem.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "ACPI" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackacpi.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "PremierCX" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackcxprem.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Century" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackcentx.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "APC" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackxapc.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Perform" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackprfrm.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Unipak" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackunipk.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Axis" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackaxis.p" 
                is-xprint-form = YES 
                lines-per-page = 68.
        WHEN "Soule" THEN 
            ASSIGN 
                v-program      = "oe/rep/acksoule.p" 
                is-xprint-form = YES 
                lines-per-page = 69. /*Soule */
        WHEN "SouleUOM" THEN 
            ASSIGN 
                v-program      = "oe/rep/acksolUom.p" 
                is-xprint-form = YES 
                lines-per-page = 65. /*SouleUOM */ /*12031306*/
        WHEN "Oracle" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackoracl.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "OTTPkg" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackottpk.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Frankstn" OR 
        WHEN "MirPkg" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackfrank.p" 
                is-xprint-form = YES
                lines-per-page = 65.
        WHEN "3CPack" THEN 
            ASSIGN 
                v-program      = "oe/rep/ack3cpak.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "HPB" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackhpb.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "PPI" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackppi.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Southpak" THEN 
            ASSIGN 
                v-program      = "oe/rep/acksthpk.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Indiana"  THEN 
            ASSIGN 
                v-program      = "oe/rep/ackindc.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Fibrex" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackfibrex.p" 
                is-xprint-form = YES 
                lines-per-page = 69.
        WHEN "Albert" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackalbert.p" 
                is-xprint-form = NO.
        WHEN "ContSvc" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackcontsvc.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Triad" THEN 
            ASSIGN 
                v-program      = "oe/rep/acktriad.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Packrite" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackpkrit.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Dee" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackdee.p" 
                is-xprint-form = YES 
                lines-per-page = 69.
        WHEN "Allwest" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackallws.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Simkins" THEN 
            ASSIGN 
                v-program      = "oe/rep/acksimkn.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        OTHERWISE 
        ASSIGN 
...
PROCEDURE SetShipToToggle :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF LOOKUP(v-print-fmt,"ContSvc,Soule,SouleUOM") = 0 THEN 
        DO:
