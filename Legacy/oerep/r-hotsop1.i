...
/*       IF v-stat NE "A" THEN DO: */
      DO:
       ...
       IF TRIM(oe-ordl.job-no) NE "" THEN
        ...
        IF (AVAIL job AND NOT job.opened) OR NOT AVAIL job THEN NEXT.
      ...
      IF rd_due-date EQ "Release" THEN
      FOR EACH oe-rel
      ...
      /*  RUN oe/rel-stat.p (ROWID(oe-rel), OUTPUT lv-stat). lv-due-date is not working task 09130505*/
