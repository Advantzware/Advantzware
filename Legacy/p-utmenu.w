...
DO:
  ...
  /* Added by WinKit Migration tool 07.02.2016 21:11:22 */
...
------------------------------------------------------------------------------*/
  ...
  if userid('nosweat') <> "ASI" then run set-buttons('disable-all').
  else RUN set-buttons (adm-panel-state).
...
&ENDIF
  ...
  if userid('nosweat') <> "ASI" then disable all with frame {&frame-name}.
END. /* DO WITH FRAME */
