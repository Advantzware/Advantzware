...
&Scoped-define FIELDS-IN-QUERY-Browser-Table rm-bin.loc-bin 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH rm-bin OF loc WHERE ~{&KEY-PHRASE} ~
      AND ~{&KEY-PHRASE} AND ~
rm-bin.i-no = '' NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH rm-bin OF loc WHERE ~{&KEY-PHRASE} ~
      AND ~{&KEY-PHRASE} AND ~
rm-bin.i-no = '' NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
DEFINE QUERY Browser-Table FOR 
      rm-bin
    FIELDS(rm-bin.loc-bin) SCROLLING.
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE Browser-Table
     _TblList          = "ASI.rm-bin OF ASI.loc"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "USED"
     _Where[1]         = "~{&KEY-PHRASE} AND
rm-bin.i-no = ''"
     _FldNameList[1]   = ASI.rm-bin.loc-bin
