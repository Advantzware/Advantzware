...
&Scoped-define INTERNAL-TABLES notes rec_key prgmxref
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH notes WHERE ~{&KEY-PHRASE} ~
      AND notes.note_text CONTAINS word_search OR word_search = '' NO-LOCK, ~
      EACH rec_key OF notes NO-LOCK, ~
...
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH notes WHERE ~{&KEY-PHRASE} ~
      ...
      EACH prgmxref OF rec_key NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table notes rec_key prgmxref
...
"STRING(notes.note_time,'HH:MM:SS am') @ notetime" "Note Time" "X(11)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[4]   = ASI.rec_key.table_name
     _FldNameList[5]   = ASI.prgmxref.prgmname
