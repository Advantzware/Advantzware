/* fa/ldgr.p - Asset Ledger Book Depreciation */
...
{fa/ldgr.y}                /* YOUR PARAMETERS INCLUDE FILE */
...
FOR EACH FA-MAST WHERE fa-mast.fa-entity eq fa-control.fa-entity
  ...
  {pt/newpage main-loop string(fa-mast.asset-code)}
    ...
    if fa-mast.asset-status = "R":U then
       acc-dep = fa-mast.acc-dep-book - fa-mast.cy-dep-book.
    else acc-dep = fa-mast.acc-dep-book.
...
if show-detail = yes then do:
  FIND FIRST fa-entry OF fa-mast WHERE fa-entry.method = "book":U AND
  ...
  if show-detail = yes then do:
    ...
    FOR EACH FA-ENTRY OF FA-MAST WHERE FA-ENTRY.METHOD = "BOOK":U
...
if show-detail = no then do:
   ...
   do loop = beg-prd to end-prd :
      ...
      if fa-mast.period[loop] >= 0 then debit# = fa-mast.period[loop].
      else credit# = fa-mast.period[loop].
      tot-debit = tot-debit + debit#.
      tot-credit = tot-credit + credit#.
      net-value = net-value - (debit# - credit#).

      if fa-mast.period[loop] ne 0 then do:
      DISPLAY
