...
DEFINE FRAME FRAME-A
     ...
     edProcessStat AT ROW 15.05 COL 9 NO-LABEL WIDGET-ID 22
...
DO:
  ...
  FOR EACH edmast NO-LOCK
   ...
   /* MESSAGE "TEST  each edmast" edmast.partner 
     skip
     "setid_list" setid_list skip
     "ws_test-prod" ws_test-prod skip     
    view-as alert-box . */
...
/*      DISPLAY next_program WITH FRAME f-current. */
      ...
      ELSE
      DO:
        ws_status:screen-value = "Program Not Found".
        IF top-debug THEN
        RUN rc/debugmsg.p
          ( ws_status + ':
        ' + next_program).
