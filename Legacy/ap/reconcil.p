...
FOR EACH gl-jrn FIELDS(j-no company cleared journal tr-date) NO-LOCK
    ...
    USE-INDEX reconciled,
    EACH gl-jrnl FIELDS(j-no tr-amt actnum dscr) NO-LOCK WHERE
...
/* BV - added break by for gl-jrnl.actnum because transfer entries where showing as 0
   04191311 #DUPECODE - ap/reconcilrpt.p aprep/r-recrep.w*/
...
FOR EACH ar-mcash NO-LOCK
    ...
    WHERE ar-mcash-ref.reftable EQ "ar-mcash"
      AND ar-mcash-ref.company  EQ ar-mcash.company
      AND ar-mcash-ref.code     EQ ar-mcash.rec_key
      AND ar-mcash-ref.loc      EQ STRING(ar-mcash.m-no,">>>>>>9")  
      AND ar-mcash-ref.val[1]   EQ 0,

    FIRST ar-mcash-ref2 NO-LOCK
    WHERE ar-mcash-ref2.rec_key  EQ ar-mcash.rec_key
      AND ar-mcash-ref2.reftable EQ "ar-mcash-ref"
      AND ar-mcash-ref2.company  EQ "ar-mcash"
      AND ar-mcash-ref2.val[1]   EQ 0
    USE-INDEX rec_key,
  ...
  ASSIGN
   ...
   tt-cleared = ar-mcash-ref2.val[2] EQ 1.
END.
