...
/*------------------------------------------------------------------------
    File        : ap/ap-ckmyork.p
    Purpose     : Generic utility to print AP checks.

    Syntax      :

    Description : Print format: STUB/STUB/CHECK
...
DEF VAR vPrintLineNumbers    AS LOG NO-UNDO INIT NO. /* Indicates to print line numbers */
      ...
      FIRST vend WHERE vend.company eq cocode AND 
       ...
       FOR EACH ap-sel WHERE 
         ...
         IF maxDetailsPrinted() OR LAST(ap-sel.inv-no) THEN DO:
             ...
             /* If section 2, then print check or VOID and process bottom section. */
/*              IF viCurrentSection = 2  THEN DO: */
