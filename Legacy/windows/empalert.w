...
if s-rec_key ne ?   and
   s-rec_key ne ''  then
  ip-rec_key = s-rec_key.

DEFINE NEW SHARED VARIABLE vrEmpAlert  AS RECID NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD GetCurrShipTo W-Win 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD RefreshBrowser W-Win 
FUNCTION RefreshBrowser RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.
...
DEFINE VARIABLE h_emailcod3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_empalert AS HANDLE NO-UNDO.
DEFINE VARIABLE h_empalert-2 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Employee Alert Information"
         HEIGHT             = 20.91
         WIDTH              = 126
         MAX-HEIGHT         = 20.91
         MAX-WIDTH          = 132.8
...
ON END-ERROR OF W-Win /* Employee Alert Information */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Employee Alert Information */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_exit ).
       RUN set-position IN h_exit ( 1.00 , 115.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse Emp Al|View Emp Aler' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/empalert.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_empalert ).
       RUN set-position IN h_empalert ( 4.81 , 3.00 ) NO-ERROR.
       RUN set-size IN h_empalert ( 13.76 , 117.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_empalert. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_empalert ).
       RUN add-link IN adm-broker-hdl ( h_empalert , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_empalert ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/emailcod3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_emailcod3 ).
       RUN set-position IN h_emailcod3 ( 5.05 , 67.00 ) NO-ERROR.
       RUN set-size IN h_emailcod3 ( 14.76 , 57.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/empalert.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_empalert-2 ).
       RUN set-position IN h_empalert-2 ( 6.62 , 3.60 ) NO-ERROR.
       /* Size in UIB:  ( 1.67 , 62.00 ) */
       ...
       /* Links to  h_empalert-2. */
       RUN add-link IN adm-broker-hdl ( h_empalert , 'Record':U , h_empalert-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_empalert-2 ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE AdvancedNotice W-Win 
...
------------------------------------------------------------------------------*/
  ...
  RUN RefreshData IN h_emailcod3.
...
-------------------------------------------------------------*/
  ...
  RUN SetEmailNotify IN h_empalert-2.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION GetCurrShipTo W-Win 
FUNCTION GetCurrShipTo RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION RefreshBrowser W-Win 
FUNCTION RefreshBrowser RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  DYNAMIC-FUNCTION ('RefreshBrowse':U IN h_empalert).
