...
&SCOPED-DEFINE h_Browse01 h_holiday
...
DEFINE VARIABLE h_holiday AS HANDLE NO-UNDO.
DEFINE VARIABLE h_holiday-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Holidays */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Holidays */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_options ).
       RUN set-position IN h_options ( 1.00 , 62.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_smartmsg ).
       RUN set-position IN h_smartmsg ( 3.14 , 92.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/holiday.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_holiday-2 ).
       RUN set-position IN h_holiday-2 ( 5.05 , 9.00 ) NO-ERROR.
       RUN set-size IN h_holiday-2 ( 19.52 , 92.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_holiday-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_holiday-2 ).
       RUN add-link IN adm-broker-hdl ( h_holiday-2 , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_holiday-2 ,
             FRAME message-frame:HANDLE , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/holiday.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_holiday ).
       RUN set-position IN h_holiday ( 6.24 , 16.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.14 , 89.00 ) */
       ...
       /* Links to SmartViewer h_holiday. */
       RUN add-link IN adm-broker-hdl ( h_holiday-2 , 'Record':U , h_holiday ).
       RUN add-link IN adm-broker-hdl ( h_p-updncp , 'TableIO':U , h_holiday ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_holiday ,
             FRAME message-frame:HANDLE , 'AFTER':U ).
