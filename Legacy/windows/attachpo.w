  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_attachpo ).
       RUN set-position IN h_attachpo ( 4.81 , 3.00 ) NO-ERROR.
       RUN set-size IN h_attachpo ( 18.10 , 145.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/attachpo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_attachpo-2 ).
       RUN set-position IN h_attachpo-2 ( 6.48 , 5.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'panels/p-attachpo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Update,
                     AddFunction = One-Record':U ,
             OUTPUT h_p-attachpo ).
       RUN set-position IN h_p-attachpo ( 16.24 , 63.00 ) NO-ERROR.
       RUN set-size IN h_p-attachpo ( 1.76 , 82.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_attachpo-2. */
       RUN add-link IN adm-broker-hdl ( h_attachpo , 'Record':U , h_attachpo-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-attachpo , 'TableIO':U , h_attachpo-2 ).
