...
def output parameter op-list-po as CHAR no-undo. 
/*def VAR op-list-po as CHAR no-undo.*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    DO li = 1 TO {&browse-name}:NUM-SELECTED-ROWS:
      ...
      IF AVAIL po-ordl THEN DO:
        CREATE tt-selected.
        ASSIGN tt-rowid = ROWID(po-ordl)
               tt-import-zero = YES
               tt-item  = po-ordl.i-no .
        IF li >= 2 THEN
        op-list-po = op-list-po +  string(ROWID(po-ordl)) + "," .
      END.
    END. 
  END.
