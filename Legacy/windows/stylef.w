  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse Style|View Style|Box Design|Box 3D Image' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/stylef.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_stylef ).
       RUN set-position IN h_stylef ( 4.81 , 6.00 ) NO-ERROR.
       RUN set-size IN h_stylef ( 19.52 , 87.00 ) NO-ERROR.
       ...
       /* Links to SmartNavBrowser h_stylef. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_stylef ).
       RUN add-link IN adm-broker-hdl ( h_stylef , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_stylef ,
             h_folder , 'AFTER':U ).
       RUN add-link IN adm-broker-hdl ( h_stylef , 'export-xl':U , h_export ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN add-link IN adm-broker-hdl ( h_stylef , 'Record':U , h_stylef-2 ).
