...
&Scoped-define INTERNAL-TABLES oe-relh oe-rell loadtag fg-bin
...
oe-relh.release# = int(ip-release#) NO-LOCK, ~
      EACH oe-rell WHERE  ~
 oe-rell.company EQ oe-relh.company ~
        AND oe-rell.r-no    EQ oe-relh.r-no USE-INDEX r-no NO-LOCK, ~
      EACH loadtag WHERE TRUE /* Join to oe-rell incomplete */ ~
      AND loadtag.company = ip-company ~
and loadtag.item-type = ip-itemtype ~
and loadtag.i-no = oe-rell.i-no ~
...
*/ NO-LOCK, ~
      EACH fg-bin WHERE TRUE /* Join to loadtag incomplete */ ~
      AND fg-bin.company = loadtag.company ~
...
and fg-bin.qty > 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 oe-relh oe-rell loadtag fg-bin
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 oe-relh
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 oe-rell
&Scoped-define THIRD-TABLE-IN-QUERY-BROWSE-1 loadtag
&Scoped-define FOURTH-TABLE-IN-QUERY-BROWSE-1 fg-bin
...
/* Query rebuild information for BROWSE BROWSE-1
     _TblList          = "ASI.oe-relh,ASI.oe-rell WHERE ASI.oe-relh ...,ASI.loadtag WHERE ASI.oe-rell ...,ASI.fg-bin WHERE ASI.loadtag ..."
...
oe-relh.release# = int(ip-release#)"
 ...
 oe-rell.company EQ oe-relh.company
     ...
     _Where[3]         = "ASI.loadtag.company = ip-company
and loadtag.item-type = ip-itemtype
and loadtag.i-no = oe-rell.i-no
/*and (loadtag.ord-no = oe-rell.ord-no or
     loadtag.tag-no = oe-rell.tag)*/
...
and (loadtag.loc-bin = oe-rell.loc-bin or oe-rell.loc-bin = """")
*/"
     _Where[4]         = "fg-bin.company = loadtag.company
and fg-bin.i-no = loadtag.i-no
and fg-bin.loc = loadtag.loc
and fg-bin.loc-bin = loadtag.loc-bin
and fg-bin.tag = loadtag.tag-no
