...
def input parameter ip-active like vend.active no-undo.
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH vend WHERE ~{&KEY-PHRASE} ~
      AND vend.company EQ ip-company ~
AND (ASI.vend.active EQ ip-active OR ip-active eq "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 139 BY 12.38
         BGCOLOR 8 FONT 0.
...
AND (ASI.vend.active EQ ip-active OR ip-active eq """")"
     _FldNameList[1]   = ASI.vend.vend-no
     _FldNameList[2]   = ASI.vend.name
     _FldNameList[3]   > ASI.vend.add1
"vend.add1" "Address" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Vendor Information */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  &SCOPED-DEFINE key-phrase vend.company EQ ip-company AND (vend.active EQ ip-active OR ip-active EQ "")
