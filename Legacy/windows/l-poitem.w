...
def input parameter ip-cur-val1 as cha no-undo.
...
po-ordl.stat <> "C" and ~
po-ordl.po-no = integer(ip-cur-val1) NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
DEFINE FRAME Dialog-Frame
     ...
     bt-ok AT ROW 14.1 COL 87
     bt-cancel AT ROW 14.1 COL 100
     RECT-1 AT ROW 12.43 COL 2
     "Sort By:" VIEW-AS TEXT
          SIZE 9 BY 1 AT ROW 12.67 COL 4
     SPACE(98.39) SKIP(1.70)
...
po-ordl.deleted = no and
po-ordl.stat <> ""C"" and
po-ordl.po-no = integer(ip-cur-val1)"
     _FldNameList[1]   > ASI.po-ordl.i-no
"po-ordl.i-no" ? ? "character" ? ? ? ? ? ? no ? no no "23" yes no no "U" "" ""
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  DO WITH FRAME {&frame-name}:  
    IF NUM-ENTRIES(ip-cur-val1) LE 1 THEN
      ASSIGN
       rd-sort              = 1
    ...
    ELSE
      ASSIGN
       ...
       ip-cur-val1          = ENTRY(1,ip-cur-val1)
       ip-cur-val           = ENTRY(2,ip-cur-val1).
  END.
  ...
  IF rd-sort EQ 1 THEN RUN lookpos1.
                  ELSE RUN lookpos3.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE lookpos1 Dialog-Frame 
PROCEDURE lookpos1 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  &SCOPED-DEFINE key-phrase po-ordl.company = ip-company and po-ordl.deleted = no AND po-ordl.stat <> "C" AND po-ordl.po-no = integer(ip-cur-val1)
   
  {custom/lookpos.i &lookup-file = "po-ordl" &lookup-field = "i-no"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE lookpos3 Dialog-Frame 
PROCEDURE lookpos3 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  {custom/lookpos.i &lookup-file = "po-ordl" &field-type = "string" &lookup-field = "s-num"}
