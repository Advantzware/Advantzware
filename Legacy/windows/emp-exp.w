...
DEFINE INPUT PARAMETER ipcEmpFrom AS CHAR NO-UNDO.
DEFINE INPUT PARAMETER ipcEmpTo   AS CHAR NO-UNDO.
...
ASSIGN cTextListToSelect = "Emp.ID,First Name,Last Name,Social Security,Start Date,Start Time,End Date,End Time,Total Time,Machine,Mach Description,Shift" 
      cFieldListToSelect = "empid,first-n,last-n,soc,start-date,start-time,end-date,end-time,tot,mach,mach-desc,shift"  .

{sys/inc/ttRptSel.i}

    ASSIGN cTextListToDefault  = "Emp.ID,First Name,Last Name,Social Security,Start Date,Start Time,End Date,End Time,Total Time,Machine,Mach Description,Shift"   .
...
Btn_Def sl_avail sl_selected Btn_Add Btn_Remove btn_Up btn_down tb_runExcel ~
...
FUNCTION getValue-estf RETURNS CHARACTER
  ( BUFFER ipb-estf FOR employee, ipc-field AS CHAR , ipc-rowid AS ROWID )  FORWARD.
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-emp.csv" 
...
DEFINE FRAME rd-fgexp
     begin_emp AT ROW 3.95 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Employee Number" WIDGET-ID 142
     end_emp AT ROW 3.95 COL 71 COLON-ALIGNED HELP
     ...
     Btn_Remove AT ROW 14.48 COL 44 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 134
     btn_Up AT ROW 15.62 COL 44 WIDGET-ID 136
     btn_down AT ROW 16.81 COL 44 WIDGET-ID 132
...
ASSIGN 
       begin_emp:PRIVATE-DATA IN FRAME rd-fgexp     = 
                "parm".

ASSIGN 
       end_emp:PRIVATE-DATA IN FRAME rd-fgexp     = 
                "parm".
...
------------------------------------------------------------------------------*/
  DISPLAY begin_emp end_emp sl_avail sl_selected tb_excel tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_emp end_emp Btn_Def sl_avail sl_selected 
...
DEF VAR v-check AS LOG INIT NO NO-UNDO .
     ...
     FOR EACH emplogin of employee NO-LOCK:
         ...
         FOR EACH ttRptSelected:
             v-excel-detail-lines = v-excel-detail-lines + 
                 appendXLLine(getValue-estf(BUFFER employee, ttRptSelected.FieldList, ROWID(emplogin) )).
        END.
        PUT STREAM excel UNFORMATTED v-excel-detail-lines SKIP.
    END.
    IF v-check = YES THEN DO:
         v-excel-detail-lines = "".
        FOR EACH ttRptSelected:
             v-excel-detail-lines = v-excel-detail-lines + 
                 appendXLLine(getValue-estf(BUFFER employee, ttRptSelected.FieldList, ROWID(employee) )).
        END.
        PUT STREAM excel UNFORMATTED SKIP v-excel-detail-lines SKIP.
    END.
END.
...
FUNCTION getValue-estf RETURNS CHARACTER
  ( BUFFER ipb-estf FOR employee, ipc-field AS CHAR , ipc-rowid AS ROWID ) :
/*------------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST emplogin WHERE ROWID(emplogin) EQ ipc-rowid NO-LOCK NO-ERROR . 
    
    CASE ipc-field :
        WHEN "empid"  THEN DO:
                lc-return = ipb-estf.employee.
        END.
        WHEN "first-n"  THEN DO:
        ...
        END.
        WHEN "start-date"  THEN DO:
            IF AVAIL emplogin THEN
                lc-return = IF emplogin.start_date <> ? THEN  string(emplogin.start_date) ELSE  "".
             ELSE lc-return = "" .
        END.
        ...
        END.
         WHEN "end-date"  THEN DO:
             IF AVAIL emplogin THEN
                 lc-return = IF emplogin.end_date <> ? THEN string(emplogin.end_date) ELSE "".
             ELSE lc-return = "" .
        END.
        ...
        END.  
         WHEN "mach"  THEN DO:
             IF AVAIL emplogin THEN
                 lc-return = string(emplogin.machine) .
             ELSE lc-return = "" .
        END.
         WHEN "mach-desc"  THEN DO:
             IF AVAIL emplogin THEN
                 FIND FIRST mach WHERE mach.company = cocode
                           AND mach.m-code = emplogin.machine NO-LOCK NO-ERROR.
                 IF AVAIL mach THEN
                     lc-return = string(mach.m-dscr)  .
                 ELSE lc-return = "" .
        END.
         WHEN "shift"  THEN DO:
             IF AVAIL emplogin THEN
                 lc-return = string(emplogin.shift)  .
             ELSE lc-return = "" .
        END.
         WHEN "tot"  THEN DO:
