...
oe-ord.ord-date oe-ord.sold-name oe-ordl.i-no oe-ordl.i-name 
...
AND LOOKUP(oe-ord.stat,lv-ord-ok) GT 0  NO-LOCK, ~
      ...
      WHERE oe-ordl.i-no EQ ip-i-no OR ip-i-no EQ "" ~
 NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
AND LOOKUP(oe-ord.stat,lv-ord-ok) GT 0 "
     _Where[2]         = "oe-ordl.i-no EQ ip-i-no OR ip-i-no EQ """"
"
     _FldNameList[1]   = ASI.oe-ord.ord-no
     _FldNameList[2]   > ASI.oe-ord.est-no
"oe-ord.est-no" "Estimate#" "x(8)" "character" ? ? ? ? ? ? no ? no no "14" yes no no "U" "" ""
     ...
     _FldNameList[10]   = ASI.oe-ordl.i-name
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
