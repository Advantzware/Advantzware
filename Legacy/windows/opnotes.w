...
DEFINE VARIABLE h_opnotes AS HANDLE NO-UNDO.
DEFINE VARIABLE h_opnotes-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Machine (Operation) Notes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Machine (Operation) Notes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/opnotes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_opnotes ).
       RUN set-position IN h_opnotes ( 4.76 , 8.00 ) NO-ERROR.
       RUN set-size IN h_opnotes ( 18.10 , 138.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_opnotes. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_opnotes ).
       RUN add-link IN adm-broker-hdl ( h_opnotes , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_opnotes ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/opnotes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_opnotes-2 ).
       RUN set-position IN h_opnotes-2 ( 5.76 , 14.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_opnotes-2. */
       RUN add-link IN adm-broker-hdl ( h_opnotes , 'Record':U , h_opnotes-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_opnotes-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_opnotes-2 ,
             h_folder , 'AFTER':U ).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-ip-header_value W-Win 
PROCEDURE get-ip-header_value :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE OUTPUT PARAMETER op-header_value AS CHARACTER NO-UNDO.

  op-header_value = ip-header.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE OUTPUT PARAMETER op-mach AS CHARACTER NO-UNDO.
