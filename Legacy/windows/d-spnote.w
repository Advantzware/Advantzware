...
&IF DEFINED(UIB_IS_RUNNING) = 0 &THEN
    DEFINE INPUT PARAM ip-i-no AS cha NO-UNDO.
&else
    DEFINE VAR ip-i-no AS cha NO-UNDO.    
&ENDIF
...
DEFINE FRAME D-Dialog
     Btn_OK AT ROW 15.05 COL 48
     SPACE(62.19) SKIP(0.80)
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Spec Notes */
DO:  
...
find first sys-ctrl where sys-ctrl.company eq g_company
                      and sys-ctrl.name    eq "OEPOPUP" no-lock no-error.
if not avail sys-ctrl then do:
        create sys-ctrl.
        ASSIGN sys-ctrl.company  = g_company
            sys-ctrl.name     = "OEPOPUP"
            sys-ctrl.char-fld = "CS"
            sys-ctrl.descrip  = "Popup Notes during Order Entry"
            sys-ctrl.log-fld = NO.        
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       /* Links to SmartViewer h_notes. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_notes ).
