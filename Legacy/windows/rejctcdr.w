...
&SCOPED-DEFINE h_Browse01 h_rejct-cd
...
ON END-ERROR OF W-Win /* PO Rejection Codes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* PO Rejection Codes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  DEFINE VAR RUN-PROC AS CHAR INIT "windows/rejctcdw.w".
  DEF VAR is-running AS LOG.
  DEF VAR pHandle AS HANDLE NO-UNDO.
/* RUN windows/rejctcdw.w (INPUT "R", "Release Date Changed ID", "Description", */
/*                          INPUT "Release Date Change Codes").                 */
/* smartrun.i */
...
DO:
  ...
  DO: /* took persistent out, per joe 6.3.2005 (res), put back 7.1.2005 */
    phandle = ?.
    RUN VALUE(run-proc) 
        PERSISTENT SET phandle (INPUT "R", "Release Date Changed ID", "Description", 
                         INPUT "Release Date Change Codes", INPUT THIS-PROCEDURE)  NO-ERROR. /* YSK to stop if there's error*/
...
------------------------------------------------------------------------------*/
  ...
  /* CURRENT-WINDOW:TITLE = ip-win-title. */
