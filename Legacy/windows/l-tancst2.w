...
lookup(STRING(eb.est-type),"1,4,5,8") gt 0 ~
 NO-LOCK, ~
      FIRST est OF eb  ~
...
"eb.est-no" ? "x(8)" "character" ? ? ? ? ? ? no ? no no "14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > ASI.eb.cust-no
"eb.cust-no" "Customer#" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
WHEN 1 THEN DO:
  ...
  OPEN QUERY {&browse-name} FOR EACH ASI.eb WHERE TRUE 
                            ...
                            AND lookup(STRING(eb.est-type),"1,4,5,8") gt 0
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR v-est-type AS CHAR NO-UNDO.
  ...
  FOR EACH eb
    ...
    IF LAST-OF(eb.est-no) THEN DO:
       IF eb.est-type EQ 1 OR eb.est-type EQ 5 THEN ll-tandem = YES.
