...
DEF VAR lv-sys-ctrl-rec-key   AS char NO-UNDO.
DEF VAR v-att AS LOG NO-UNDO.

{methods\defines\phone.i &new="NEW"}

&SCOPED-DEFINE winReSize
&SCOPED-DEFINE h_Browse01 h_sys-ctrl
...
DEFINE VARIABLE h_sys-ctrl-3 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Control|View Control|Browse Forms|View Form' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 18.14 , 7.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 18.14 , 89.00 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_sys-form ).
       RUN set-position IN h_sys-form ( 6.71 , 4.00 ) NO-ERROR.
       RUN set-size IN h_sys-form ( 17.86 , 99.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1,4':U) NO-ERROR.

       /* Links to SmartViewer h_sys-ctrl-3. */
       RUN add-link IN adm-broker-hdl ( h_sys-ctrl , 'Record':U , h_sys-ctrl-3 ).

       /* Links to SmartNavBrowser h_sys-form. */
       RUN add-link IN adm-broker-hdl ( h_p-navico-2 , 'Navigation':U , h_sys-form ).
       RUN add-link IN adm-broker-hdl ( h_sys-ctrl , 'Record':U , h_sys-form ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_sys-ctrl-3 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_sys-form ,
             h_sys-ctrl-3 , 'AFTER':U ).
    END. /* Page 3 */
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewerid/sys-ctrl.w':U ,
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-updsav-2 ).
       RUN set-position IN h_p-updsav-2 ( 18.62 , 72.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-navico-2 ).
       RUN set-position IN h_p-navico-2 ( 18.86 , 5.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF viCurrPage = 4 AND NOT vlSelected THEN DO:
    RUN Select-Page (3).
    DYNAMIC-FUNCTION ('GetCurrentRecord':U IN h_sys-form).
    vlSelected = YES.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  assign lv-sys-ctrl-rec-key = sys-ctrl.rec_key when avail sys-ctrl no-error.

  v-att = CAN-FIND(FIRST asi.attach WHERE
          attach.company = sys-ctrl.company and
          attach.rec_key = lv-sys-ctrl-rec-key AND
          ATTACH.est-no EQ "").
