...
&SCOPED-DEFINE h_Browse01 h_stax
...
DEFINE VARIABLE h_stax AS HANDLE NO-UNDO.
DEFINE VARIABLE h_stax-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Sales Tax Codes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Sales Tax Codes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/stax.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_stax ).
       RUN set-position IN h_stax ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_stax ( 19.52 , 143.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_stax. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_stax ).
       RUN add-link IN adm-broker-hdl ( h_stax , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_stax ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/stax.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_stax-2 ).
       RUN set-position IN h_stax-2 ( 5.29 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.52 , 122.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 14.76 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 14.76 , 80.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav ( 2.14 , 56.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_stax-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_stax-2 ).
       RUN add-link IN adm-broker-hdl ( h_stax , 'Record':U , h_stax-2 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_stax-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_stax-2 ,
             h_folder , 'AFTER':U ).
