...
&scoped-define SORTBY-1 BY item.i-name
&scoped-define SORTBY-2 BY item.i-no
&scoped-define fld-name-1 item.i-name
...
and (item.mat-type = ip-mat-type  ~
or item.mat-type = "V") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
DEFINE FRAME Dialog-Frame
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Raw Material Information (Ink/Coating)".
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Raw Material Information (Ink/Coating) */
DO:
...
DO:
   op-char-val = item.i-name:SCREEN-VALUE in browse {&browse-name} + "," +
                 item.i-no:screen-value in browse {&browse-name} 
                 .
