...
def input parameter ip-i-no like item.loc no-undo. 
...
&scoped-define SORTBY-2 BY fg-bin.loc-bin
...
&Scoped-define INTERNAL-TABLES ar-invl oe-boll

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE BROWSE-1                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 oe-boll.job-no oe-boll.job-no2 ~
oe-boll.loc oe-boll.loc-bin oe-boll.tag ar-invl.cust-no oe-boll.qty 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1 
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH ar-invl WHERE ~{&KEY-PHRASE} ~
      AND ar-invl.company = ip-company and ~
ar-invl.inv-no = ip-inv-val NO-LOCK, ~
    EACH oe-boll WHERE oe-boll.company = ar-invl.company AND oe-boll.b-no = ar-invl.b-no ~
...
ar-invl.inv-no = ip-inv-val NO-LOCK , ~
    EACH oe-boll WHERE oe-boll.company = ar-invl.company AND oe-boll.b-no = ar-invl.b-no ~
      AND oe-boll.i-no EQ ar-invl.i-no  NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 ar-invl oe-boll
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 ar-invl
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 est oe-boll
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      ...
      ar-invl.cust-no COLUMN-LABEL "Customer#" FORMAT "x(8)":U WIDTH 12
      oe-boll.qty FORMAT "->>>,>>9.9<<<<<":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _Where[1]         = "ASI.ar-invl.company = ip-company and
ar-invl.cust-no = ip-cust-no and
ar-invl.inv-no = ip-inv-no"
     _FldNameList[1]   = ASI.ar-invl.ord-no
     _FldNameList[2]   = ASI.ar-invl.i-no
     _FldNameList[3]   = ASI.ar-invl.i-name
     _FldNameList[4]   > ASI.ar-invl.ship-qty
"ship-qty" "Shipped Qty" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[5]   > ASI.ar-invl.po-no
"po-no" "PO Number" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Invoice Item Information */
DO:
...
DO:
    op-char-val = STRING(ROWID(oe-boll)).
                 .
...
/*    op-char-val = oe-boll.tag + "," + oe-boll.loc + "," + oe-boll.loc-bin + "," + */
/*                  oe-boll.job-no + "," + STRING(oe-boll.job-no2)                  */
/*                  .                                                               */
   do counter = 1 to BROWSE BROWSE-1:NUM-SELECTED-ROWS:
        BROWSE BROWSE-1:FETCH-SELECTED-ROW(counter).
        op-char-val = op-char-val + STRING(ROWID(oe-boll)) + ",".

 
   END.
   op-char-val = TRIM(op-char-val,",").
...
*/  
  ...
  IF ip-selection EQ "Single" THEN
    BROWSE {&browse-name}:MULTIPLE = FALSE.
  RUN enable_UI.
