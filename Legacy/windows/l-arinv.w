...
&scoped-define fld-name-2 ar-inv.ord-no
&scoped-define SORTBY-1 BY ar-inv.inv-no
&scoped-define SORTBY-2 BY ar-inv.ord-no
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH ar-inv WHERE ~{&KEY-PHRASE} ~
      AND ar-inv.company = ip-company and ~
(ar-inv.cust-no = ip-cust-no or ip-cust-no = "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 93 BY 11.19
         BGCOLOR 8 FONT 0.
...
"inv-no" "Invoice#" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
DO:
   op-char-val = string(ar-inv.inv-no) .
