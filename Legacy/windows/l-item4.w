...
DO:
    ...
    IF {&browse-name}:NUM-SELECTED-ROWS GT 0 THEN 
        ...
        op-char-val = op-char-val + item.i-no + "," +
                      item.i-name + "," .
        
    END.  /* SELECTED */


    IF TRIM(op-char-val) EQ "" OR 
       TRIM(op-char-val) EQ "," THEN DO:
    ...
    IF op-char-val NE "" AND SUBSTR(op-char-val,LENGTH(op-char-val),1) EQ "," THEN
       ASSIGN op-char-val = SUBSTR(op-char-val,1,LENGTH(op-char-val) - 1).
...
DO:
    ...
    /* gdm - 05120908 - A NEW FILTER IF NOTHING SELECTED */
    IF TRIM(op-char-val) EQ "" OR 
       TRIM(op-char-val) EQ ","
      THEN DO:

        MESSAGE 
            "You have not selected any item from the browser." SKIP
            "Please select at least one item, or press the cancel"
            " button to exit"            
    ...
    IF SUBSTR(op-char-val,LENGTH(op-char-val),1) EQ "," 
      THEN 
        ASSIGN op-char-val = SUBSTR(op-char-val,1,LENGTH(op-char-val) - 1).
