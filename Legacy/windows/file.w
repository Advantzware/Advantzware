...
&SCOPED-DEFINE h_Browse01 h_file
...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES file
&Scoped-define FIRST-EXTERNAL-TABLE file


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR file.
...
DEFINE VARIABLE h_file-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_file-seq AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Reference File */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Reference File */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse File|View File|Re-Sequence' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/file.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_file ).
       RUN set-position IN h_file ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_file ( 19.52 , 145.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 15.52 , 14.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 15.52 , 75.00 ) NO-ERROR.
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/file-seq.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_file-seq ).
       RUN set-position IN h_file-seq ( 8.62 , 54.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.38 , 36.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_file-seq ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "file"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "file"}
...
DEFINE VARIABLE lAccess          AS LOGICAL NO-UNDO.
  ...
  IF il-cur-page = 2 THEN DO:
     ...
     RUN getSecurity IN hProcedureHandle("windows/file.w", USERID(LDBNAME(1)), "",
                                        OUTPUT lAccess).
     IF lAccess THEN
