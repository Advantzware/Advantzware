...
&SCOPED-DEFINE h_Browse01 h_snotes

/* Parameters Definitions ---                                           */

/* Local Variable Definitions ---                                       */

&Scoped-define IAMWHAT snotes
...
DEFINE VARIABLE h_snotes AS HANDLE NO-UNDO.
DEFINE VARIABLE h_snotes-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Notes Search */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Notes Search */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_options ).
       RUN set-position IN h_options ( 1.00 , 93.20 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/snotes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_snotes ).
       RUN set-position IN h_snotes ( 4.81 , 2.00 ) NO-ERROR.
       RUN set-size IN h_snotes ( 19.67 , 147.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_snotes. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_snotes ).
       RUN add-link IN adm-broker-hdl ( h_snotes , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_snotes ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/snotes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_snotes-2 ).
       RUN set-position IN h_snotes-2 ( 6.00 , 15.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 21.00 , 56.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 2.14 , 38.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_snotes-2. */
       RUN add-link IN adm-broker-hdl ( h_snotes , 'Record':U , h_snotes-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_snotes-2 ,
             h_folder , 'AFTER':U ).
