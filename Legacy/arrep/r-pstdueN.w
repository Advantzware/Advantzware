...
DEF NEW SHARED BUFFER b-itemfg FOR itemfg .
DEF NEW SHARED VAR cTextListToDefault AS cha NO-UNDO.
DEF NEW SHARED VAR str-line AS cha FORM "x(300)" NO-UNDO.

ASSIGN cTextListToSelect = "Customer,Name,Contact,SalesRep,Terms,Address1,Address2,City," +
                           "State,Zip,Credit Limit,Phone,Fax,Check/Memo,DaysOld,Type,Invoice#," +
                           "Inv Date,InvoiceAmt,Past Due Amt"
...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Customer,Name,Contact,SalesRep,Terms,Address1,Address2,City," +
                           "State,Zip,Credit Limit,Phone,Fax,Check/Memo,DaysOld,Type,Invoice#," +
                           "Inv Date,InvoiceAmt,Past Due Amt"  .
...
rd_date tb_detailed rd_sort rd_sort2 sl_avail Btn_Def sl_selected Btn_Add ~
...
lbl_sort rd_sort lbl_sort2 rd_sort2 sl_avail sl_selected rd-dest lv-ornt ~
...
DEFINE FRAME FRAME-A
     ...
     sl_avail AT ROW 11.38 COL 3.8 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 11.38 COL 39.8 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 11.38 COL 59.2 NO-LABEL WIDGET-ID 28
     Btn_Add AT ROW 12.38 COL 39.8 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 13.38 COL 39.8 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 14.43 COL 39.8 WIDGET-ID 40
     btn_down AT ROW 15.43 COL 39.8 WIDGET-ID 42
     rd-dest AT ROW 17.81 COL 7 NO-LABEL
     ...
     tb_excel AT ROW 23.67 COL 31
     tb_runExcel AT ROW 23.67 COL 73 RIGHT-ALIGNED
     fi_file AT ROW 24.62 COL 29 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 26.29 COL 22
...
------------------------------------------------------------------------------*/
  DISPLAY tb_cust-list begin_cust-no end_cust-no begin_slsmn end_slsmn 
          ...
          lbl_sort2 rd_sort2 sl_avail sl_selected rd-dest lv-ornt lines-per-page 
  ...
  ENABLE RECT-6 RECT-7 tb_cust-list btnCustList begin_cust-no end_cust-no 
         ...
         rd_sort rd_sort2 sl_avail Btn_Def sl_selected Btn_Add Btn_Remove 
...
/*page.*/
  ...
  if v-sort2 begins "Due" then
    if v-sort eq "Name" then
      run ar/rep/pastdue1N.p. 
    else
    if v-sort eq "#Number" then
      run ar/rep/pastdue2N.p.
    else
      run ar/rep/pastdue7N.p.
  else
  if v-sort2 begins "InvD" then
    if v-sort eq "Name" then
      run ar/rep/pastdue3N.p.
    else
    if v-sort eq "#Number" then
      run ar/rep/pastdue4N.p.
    else
      run ar/rep/pastdue8N.p.
  else
    if v-sort eq "Name" then
      run ar/rep/pastdue5N.p.
    else
    if v-sort eq "#Number" then
      run ar/rep/pastdue6N.p.
    else
      run ar/rep/pastdue9N.p. 
 ...
 /* display fill("_",80) format "x(80)"
   ...
   PUT SKIP str-line SKIP.
        ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField: 
                         ...
                         WHEN "curr"      THEN cVarValue = string(grand-t[1],"->,>>>,>>>,>>9.99")  .         
           ...
           PUT UNFORMATTED  "       GRAND TOTAL:" substring(cDisplay,24,300) SKIP.
           IF v-export THEN DO:
               PUT STREAM s-temp UNFORMATTED  
