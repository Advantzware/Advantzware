...
&Scoped-Define ENABLED-OBJECTS RECT-1 td_imported
&Scoped-define DISPLAYED-TABLES cust
&Scoped-define FIRST-DISPLAYED-TABLE cust
&Scoped-Define DISPLAYED-OBJECTS RECT-1 td_imported
...
DEFINE RECTANGLE RECT-1
     ...
     SIZE 22.2 BY 2.14.
 DEFINE VARIABLE td_imported AS LOGICAL 
          VIEW-AS TOGGLE-BOX
          SIZE 20 BY 1
          BGCOLOR 15 FONT 4 .
...
DEFINE FRAME F-Main
     td_imported AT ROW 1.71 COL 3.2
...
&Scoped-define SELF-NAME td_imported
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL td_imported V-table-Win
ON VALUE-CHANGED OF td_imported IN FRAME F-Main /* Created Externally? */
DO:
    FIND CURRENT cust EXCLUSIVE-LOCK NO-ERROR .
    
    ASSIGN cust.imported = IF  td_imported:SCREEN-VALUE EQ "Yes" THEN TRUE ELSE FALSE .
...
DO WITH FRAME {&FRAME-NAME}:
      td_imported:LABEL = "Contract Pricing" .
       ENABLE td_imported .
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      IF AVAIL cust  THEN
          td_imported:SCREEN-VALUE = IF cust.imported EQ TRUE THEN "Yes" ELSE "No" .
  END.
