...
DEFINE VARIABLE lButtonSec AS LOGICAL NO-UNDO .

RUN sys/ref/nk1look.p (INPUT g_company, "CEUpdateCAD", "L" /* Logical */, NO /* check by cust */, 
                         INPUT YES /* use cust not vendor */, "" /* cust */, "" /* ship-to*/,
                         OUTPUT cRtnChar, OUTPUT lRecFound).
  IF lRecFound THEN
      lButtonSec = logical(cRtnChar) NO-ERROR. 
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
&Scoped-Define ENABLED-OBJECTS btn-cad 
...
ASSIGN 
       btn-cad:PRIVATE-DATA IN FRAME Panel-Frame     = 
                "panel-image".
...
&Scoped-define SELF-NAME btn-cad
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-cad C-WIn
ON CHOOSE OF btn-cad IN FRAME Panel-Frame /* Die */
DO:
   ...
   IF valid-handle(widget-handle(char-hdl))
      THEN RUN cad-image-update IN WIDGET-HANDLE(char-hdl).
