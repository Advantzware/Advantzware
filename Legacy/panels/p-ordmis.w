...
&Scoped-Define ENABLED-OBJECTS Btn-Save Btn-Reset Btn-Add Btn-Copy Btn-Inv ~
...
DEFINE FRAME Panel-Frame
     ...
     Btn-Inv AT ROW 1.29 COL 38
     Btn-Delete AT ROW 1.29 COL 47
     Btn-Cancel AT ROW 1.29 COL 56
...
&Scoped-define SELF-NAME Btn-Inv
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL Btn-Inv C-WIn
ON CHOOSE OF Btn-Inv IN FRAME Panel-Frame /* Invoice */
DO:
  IF NOT v-can-create THEN RETURN NO-APPLY.

  RUN get-link-handle IN adm-broker-hdl (THIS-PROCEDURE, "tableio-target",OUTPUT lv-char-hdl).
  RUN create-invoice IN WIDGET-HANDLE(lv-char-hdl).
  {Advantzware/WinKit/winkit-panel-triggerend.i}
END.
...
&ENDIF
&IF LOOKUP("Btn-Inv":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             Btn-Inv:SENSITIVE = NO.
&ENDIF
...
&ENDIF
  ...
  DO WITH FRAME {&FRAME-NAME}:
    IF NOT v-can-create THEN Btn-Inv:SENSITIVE = NO.
  END.
