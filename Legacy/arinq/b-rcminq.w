...
DEF VAR ld-return-amt AS DEC NO-UNDO.

&SCOPED-DEFINE key-phrase ar-cash.company EQ cocode AND ar-cash.posted EQ YES AND ar-cash.check-no GE 90000000 AND ar-cash.check-no LE 99999999

&SCOPED-DEFINE for-each1                     ~
    FOR EACH ar-cash                         ~
        WHERE {&key-phrase}                  ~
          AND ar-cash.cust-no BEGINS fi_cust
...
&SCOPED-DEFINE sortby BY ar-cash.check-no
...
&Scoped-define INTERNAL-TABLES ar-cash cust
...
ar-cash.check-no ar-cash.check-date display-amt() @ ld-amt 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table ar-cash.cust-no ~
cust.name ar-cash.check-no ar-cash.check-date 
&Scoped-define ENABLED-TABLES-IN-QUERY-Browser-Table ar-cash cust
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH ar-cash WHERE ~{&KEY-PHRASE} ~
      AND ar-cash.check-no gt 999999999 NO-LOCK, ~
      EACH cust OF ar-cash NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH ar-cash WHERE ~{&KEY-PHRASE} ~
      AND ar-cash.check-no gt 999999999 NO-LOCK, ~
      EACH cust OF ar-cash NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table ar-cash cust
...
&Scoped-Define ENABLED-OBJECTS RECT-1 fi_fchk fi_cust btn_go btn_show ~
Browser-Table 
&Scoped-Define DISPLAYED-OBJECTS fi_fchk fi_cust fi_name fi_sort-by 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD display-amt B-table-Win 
FUNCTION display-amt RETURNS DECIMAL
...
DEFINE FRAME F-Main
     ...
     fi_cust AT ROW 1.48 COL 60 COLON-ALIGNED
...
/* Query rebuild information for BROWSE Browser-Table
     _TblList          = "asi.ar-cash,asi.cust OF asi.ar-cash"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ","
     _Where[1]         = "ar-cash.check-no gt 999999999"
...
"display-amt() @ ld-amt" "Amount" "->>>,>>>,>>9.99" ? ? ? ? ? ? ? no ? no no "20" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  {&for-each1}
      ...
      {&for-each2}
      BREAK BY ar-cash.c-no DESC:
    IF FIRST-OF(ar-cash.c-no) THEN li = li + 1.
    lv-c-no = ar-cash.c-no.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION display-amt B-table-Win 
FUNCTION display-amt RETURNS DECIMAL
...
------------------------------------------------------------------------------*/  
  ...
  IF ar-cash.check-amt EQ 0 THEN DO:
    ld-amt = 0.
    FOR EACH ar-cashl OF ar-cash NO-LOCK:
      ld-amt = ld-amt - ar-cashl.amt-paid + ar-cashl.amt-disc.
    END.
    ld-amt = ar-cash.check-amt - ld-amt.
  END.

  ELSE ld-amt = ar-cash.check-amt.
