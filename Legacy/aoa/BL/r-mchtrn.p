...
FOR EACH machtran NO-LOCK
    ...
    ASSIGN
        ...
        ttMachineTransactions.xxSort         = IF cSort EQ "Start Date / Time" THEN
                                          ...
                                          ELSE STRING(machtran.machine,"x(6)")
                                             + STRING(machtran.start_date,"99/99/9999")
                                             + STRING(machtran.start_time,"99999")
        ttMachineTransactions.loginDateTime  = TRIM(STRING(STRING(machtran.start_date) + " " + STRING(machtran.login_time,"hh:mm am")))
        ttMachineTransactions.logoutDateTime = TRIM(STRING(STRING(machtran.end_date) + " " + STRING(machtran.LOGOUT_time,"hh:mm am")))                                
        .
    ...
    FOR EACH machemp NO-LOCK
        ...
        ASSIGN
            ...
            ttMachineEmployeeTransactions.loginDateTime  = TRIM(STRING(STRING(machemp.start_date) + " " + STRING(machemp.login_time,"hh:mm am")))
            ttMachineEmployeeTransactions.logoutDateTime = TRIM(STRING(STRING(machemp.end_date) + " " + STRING(machemp.LOGOUT_time,"hh:mm am")))   
            .
