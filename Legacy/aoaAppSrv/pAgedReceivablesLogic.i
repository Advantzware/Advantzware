...
DEFINE VARIABLE v-current-trend-days AS INTEGER NO-UNDO  FORMAT "->>9".
...
&SCOPED-DEFINE for-each-arcsh                            ~
    FOR EACH ar-cash                                     ~
      ...
      ELSE DO:                                           ~
         ...
         FIND FIRST gltrans NO-LOCK WHERE         ~
      ...
      IF v-check-date NE ? AND v-check-date GT dtAsofDate THEN ~
         NEXT.

&SCOPED-DEFINE valid-factored                                       ~
    IF NOT lFactoredFGItem AND                                      ~
...
FOR EACH reftable NO-LOCK WHERE reftable.reftable EQ "FACTORED"        
  ...
  IF NOT AVAILABLE tt-factored THEN DO:
    FOR EACH ar-invl NO-LOCK WHERE ar-invl.company EQ ipcCompany
...
END. /* END FOR EACH reftable */
  ...
  FOR EACH company NO-LOCK WHERE
      company.company GE cStartCompanyNo AND
      company.company LE cEndCompanyNo ,    
      EACH cust 
      ...
      AND ((cust.curr-code GE cStartCurrency    AND
            cust.curr-code LE cEndCurrency)       OR
           (cust.curr-code EQ ""          AND
            company.curr-code GE cStartCurrency AND
            company.curr-code LE cEndCurrency)):
      ...
      ASSIGN
          tt-cust.curr-code = IF cust.curr-code EQ "" THEN company.curr-code
                              ELSE cust.curr-code
          tt-cust.sorter    = IF cSort1 EQ "#Number" THEN cust.cust-no ELSE IF cSort1 EQ "Name" THEN cust.NAME ELSE cust.sman 
          tt-cust.row-id    = ROWID(cust).

      IF tt-cust.curr-code NE company.curr-code THEN ll-mult-curr = YES.
      END. /* end IF ll-valid-cust */
  END. /* end  FOR EACH company */
  ...
  FOR EACH tt-cust,
    ...
    IF lPaidInvoice OR dtAsofDate NE TODAY THEN
    {&for-each-arinv}:
      {&valid-factored}

      CREATE tt-inv.
      ASSIGN
       tt-inv.sorter = IF cSort2 EQ "DueDate" THEN INT(ar-inv.due-date) ELSE IF cSort2 EQ "InvDate" THEN int(ar-inv.inv-date) ELSE ar-inv.inv-no 
...
/* Inserted because AR stores gross wrong */
      ...
      ASSIGN 
       ag     = amt
       d      = dtAsofDate - ar-inv.due-date  /*{&date}*/
       ni     = ni + 1
     ...
     IF ag NE 0                                                     OR 
        (lPaidInvoice AND 
         ar-inv.inv-date GE dtStartInvoiceDate AND ar-inv.inv-date LE dtEndInvoiceDate) THEN DO:
       IF v-first-cust THEN DO:
         ...
         FIND FIRST terms NO-LOCK WHERE terms.company EQ cust.company AND 
                                         terms.t-code EQ cust.terms NO-ERROR.

         /* If input trend days entered, THEN DO the trend days calculation. */
         IF  idayForRecentTrend > 0 THEN
             RUN pGetTrendDays (INPUT idayForRecentTrend,
                                OUTPUT v-current-trend-days).

         IF cDetailSummTotal = 1 THEN DO:
         END. /* if cDetailSummTotal = 1 */
        
         v-first-cust = NO.
       END. /* end  IF v-first-cust */

       IF d GE iPeriodDay3 THEN DO:
         v-int = 4.
       END. /* end IF d */
       ELSE
       IF d GE iPeriodDay2 THEN DO:
         v-int = 3.
       END. /* end IF d */
       ELSE
       IF d GE iPeriodDay1 THEN DO:
         v-int = 2.
       END. /* end IF d */
       ELSE DO:
         v-int = 1.
      ...
      IF cDetailSummTotal = 1 THEN DO:
          ...
          ASSIGN
              ...
              ttAgedReceivables.terms          =  terms.dscr   
         ...
         ASSIGN ttAgedReceivables.collNote       =  lv-text.
      END.  /* if cDetailSummTotal = 1 THEN */
          ...
          IF cDetailSummTotal = 1 THEN DO:
            ...
            ASSIGN
                ...
                .
                 ...
                 FOR EACH notes NO-LOCK WHERE notes.rec_key EQ ar-inv.rec_key
                                    AND notes.note_type EQ "I" :
                    lv-text = lv-text + " " + TRIM(notes.note_text) + CHR(10).
                 END.
                 ttAgedReceivables.invoiceNote    =  lv-text .
    ...
    {&for-each-arcsh}
      ...
      IF d GE iPeriodDay3 THEN
        unapp[4] = unapp[4] + v-cr-db-amt - v-disc-amt.
      ELSE
      IF d GE iPeriodDay2 AND d LT iPeriodDay3 THEN
        unapp[3] = unapp[3] + v-cr-db-amt - v-disc-amt.
      ELSE
      IF d ge iPeriodDay1 AND d LT iPeriodDay2 THEN 
        unapp[2] = unapp[2] + v-cr-db-amt - v-disc-amt.
      ELSE 
      IF d LT iPeriodDay1 THEN 
    ...
    {&for-each-arcsh}
      ...
      ELSE DO:
         ...
         IF CAN-FIND(FIRST reftable WHERE
            ...
            gltrans.company EQ cust.company AND
            gltrans.jrnl EQ "CASHRVD" AND
            gltrans.tr-dscr EQ v-tr-dscr) THEN DO:
              ASSIGN
                 v-type = "VD"
                 v-neg-text = "VOID".
              RELEASE reftable.
         ...
         ELSE v-check-date = ar-cash.check-date.
            ...
            IF cDetailSummTotal = 1 THEN DO:
                ...
                ASSIGN
                    ...
                    ttAgedReceivables.daysOld        =  0                                                                                     
                    ttAgedReceivables.vType          =  v-type                                                                                
                    ttAgedReceivables.invoiceNo      =  INTEGER(v-neg-text)                                                                   
                    ttAgedReceivables.invoiceDate    =  v-check-date                                                                          
                    ...
                    ttAgedReceivables.collNote       =  ""
                    . 
            END. /*IF cDetailSummTotal = 1 THEN DO */
      ...
      ELSE DO:
       ...
       IF cDetailSummTotal = 1 THEN DO:
           PUT UNFORMATTED "ttAgedReceivables" SKIP.
           CREATE ttAgedReceivables .
           ASSIGN
               ttAgedReceivables.custNo         =  cust.cust-no
               ttAgedReceivables.custName       =  cust.NAME
