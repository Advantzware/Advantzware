...
FOR EACH po-ordl
  ...
  IF ld-rec NE ld-iss THEN DO:
    ...
    FOR EACH rm-rcpth
      ...
      FOR EACH rm-rctd NO-LOCK BY rm-rctd.r-no DESCENDING:
        li = rm-rctd.r-no.
        LEAVE.
      END.
      FIND LAST b-rm-rh USE-INDEX r-no NO-LOCK NO-ERROR.
      IF AVAIL b-rm-rh AND b-rm-rh.r-no GT li THEN li = b-rm-rh.r-no.
