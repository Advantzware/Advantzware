...
DEF INPUT PARAMETER ip-check-logical AS CHAR NO-UNDO.
DEF OUTPUT PARAMETER op-validated AS LOG INIT NO NO-UNDO.

/* Local Variable Definitions ---                                       */
DEFINE VARIABLE vlNotNeeded AS LOGICAL     NO-UNDO.
{custom/globdefs.i}
...
/* If flag is not set to validate the password, just return */

IF ip-check-logical GT "" THEN DO:
    FIND FIRST sys-ctrl 
        WHERE sys-ctrl.company EQ cocode
          AND sys-ctrl.NAME    EQ ip-check-logical
        NO-LOCK NO-ERROR.

    IF AVAIL sys-ctrl AND sys-ctrl.log-fld = NO THEN DO:
        ASSIGN op-validated = FALSE
               vlNotNeeded  = TRUE.
    END.
END.
...
ELSE DO:
  MESSAGE "No System Control record exists for " + ip-type
