...
&SCOPED-DEFINE viewerHandle h_resourceDetail
...
DEFINE VARIABLE resourceDetail AS CHARACTER NO-UNDO.
...
DEFINE VARIABLE h_resourcedetail AS HANDLE NO-UNDO.
...
/* SETTINGS FOR FRAME detailFrame
                                                                        */
ASSIGN 
       FRAME detailFrame:HEIGHT           = 28.57
       FRAME detailFrame:WIDTH            = 160.
...
ON END-ERROR OF W-Win /* Resource Jobs - Scheduler */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Resource Jobs - Scheduler */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-RESIZED OF W-Win /* Resource Jobs - Scheduler */
DO:
  RUN winReSize.
END.
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
           ...
           &ENDIF
             ...
             INPUT  '':U ,
             OUTPUT h_resourcedetail ).
       RUN set-position IN h_resourcedetail ( 1.00 , 1.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  resourceDetail = findProgram('{&viewers}/',ID,'/resourceDetail.w').
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE reopenBrowse W-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN reopenBrowse IN {&viewerHandle}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE resource W-Win 
PROCEDURE resource :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  RUN select-page (0).
  RUN passResource IN {&viewerHandle} (ipResource,ipDescription,ipHandle).
