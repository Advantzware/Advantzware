...
end_ship-to-zip begin_slsmn end_slsmn begin_inv-date end_inv-date ~
tb_detailed fi_sort tb_fin-chg rd-dest lv-ornt lines-per-page lv-font-no ~
...
end_slsmn begin_inv-date end_inv-date tb_detailed tb_cust-total ~
tb_shipto-total fi_sort tb_fin-chg rd-dest lv-ornt lines-per-page ~
...
DEFINE FRAME FRAME-A
     ...
     begin_cust-no AT ROW 3 COL 29 COLON-ALIGNED HELP
          "Enter Beginning Customer Number"
     end_cust-no AT ROW 3 COL 72 COLON-ALIGNED HELP
          "Enter Ending Customer Number"
     begin_ship-to AT ROW 3.95 COL 29 COLON-ALIGNED HELP
          "Enter Beginning Ship-To#"
     end_ship-to AT ROW 3.95 COL 72 COLON-ALIGNED HELP
          "Enter Ending Ship-To#"
     begin_ship-to-zip AT ROW 4.91 COL 29 COLON-ALIGNED HELP
          "Enter Beginning Ship-To Zip Code"
     end_ship-to-zip AT ROW 4.91 COL 72 COLON-ALIGNED HELP
          "Enter Ending Ship-To Zip Code"
     begin_slsmn AT ROW 5.86 COL 29 COLON-ALIGNED HELP
          "Enter Beginning Sales Rep Number"
     end_slsmn AT ROW 5.86 COL 72 COLON-ALIGNED HELP
          "Enter Ending Sales Rep Number"
     begin_inv-date AT ROW 6.81 COL 29 COLON-ALIGNED
     end_inv-date AT ROW 6.81 COL 72 COLON-ALIGNED HELP
          "Enter Ending Due Date"
     tb_detailed AT ROW 8.24 COL 31 RIGHT-ALIGNED
     tb_cust-total AT ROW 8.24 COL 60 RIGHT-ALIGNED WIDGET-ID 2
     tb_shipto-total AT ROW 8.24 COL 86 RIGHT-ALIGNED WIDGET-ID 4
     fi_sort AT ROW 9.43 COL 29 COLON-ALIGNED HELP
          "Choose up to three categories to sort by."
     tb_fin-chg AT ROW 11.57 COL 33
     rd-dest AT ROW 14.48 COL 5 NO-LABEL
     lv-ornt AT ROW 14.48 COL 31 NO-LABEL
     lines-per-page AT ROW 14.48 COL 84 COLON-ALIGNED
     lv-font-no AT ROW 17.1 COL 35 COLON-ALIGNED
     lv-font-name AT ROW 18.05 COL 29 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 19.29 COL 31
     tb_excel AT ROW 20.19 COL 50.6 RIGHT-ALIGNED
     tb_runExcel AT ROW 20.19 COL 71.6 RIGHT-ALIGNED
     fi_file AT ROW 21 COL 28.6 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 22.91 COL 17
     btn-cancel AT ROW 22.91 COL 55
     "~"C~"ustomer" VIEW-AS TEXT
          SIZE 12 BY .67 AT ROW 9.19 COL 42
          FGCOLOR 9 
     "~"O~"rder" VIEW-AS TEXT
          SIZE 12 BY .67 AT ROW 9.91 COL 70
          FGCOLOR 9 
     "ship-to ~"Z~"ip" VIEW-AS TEXT
          SIZE 14 BY .67 AT ROW 9.91 COL 56
          FGCOLOR 9 
     "sales~"R~"ep" VIEW-AS TEXT
          SIZE 13 BY .67 AT ROW 10.62 COL 42
          FGCOLOR 9 
     ...
     "invoice ~"D~"ate" VIEW-AS TEXT
          SIZE 16 BY .67 AT ROW 9.19 COL 70
          FGCOLOR 9 
     "~"S~"hipto" VIEW-AS TEXT
          SIZE 12 BY .67 AT ROW 9.91 COL 42
          FGCOLOR 9 
     "~"I~"nvoice#" VIEW-AS TEXT
          SIZE 11 BY .67 AT ROW 9.19 COL 56
          FGCOLOR 9 
...
ON VALUE-CHANGED OF tb_detailed IN FRAME FRAME-A /* Show Invoice Lines? */
DO:
  assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY tb_cust-list begin_cust-no end_cust-no begin_ship-to end_ship-to 
          ...
          end_inv-date tb_detailed tb_cust-total tb_shipto-total fi_sort 
  ...
  ENABLE RECT-6 RECT-7 tb_cust-list btnCustList begin_cust-no end_cust-no 
         ...
         begin_slsmn end_slsmn begin_inv-date end_inv-date tb_detailed fi_sort 
    ...
    if v-det then do with frame detail:
      ...
      IF tb_excel THEN
         PUT STREAM excel UNFORMATTED
             '"' REPLACE(REPLACE(cust.cust-no,",",""),'"',"") '",'
             '"' REPLACE(REPLACE(cust.NAME,",",""),'"',"")    '",'
             '"' tt-report.key-05  '",'
             '"' tt-report.key-08  '",'
             '"' tt-report.key-10  '",'
             '"' w-data.inv-no     '",'
             '"' STRING(v-date)    '",'
             '"' STRING(v-ord,">>>>>>") '",'
             '"' REPLACE(REPLACE(w-data.i-no,",",""),'"',"") '",'
             '"' IF AVAIL itemfg THEN REPLACE(REPLACE(itemfg.i-name,",",""),'"',"")
                 ELSE REPLACE(REPLACE(w-data.i-no,",",""),'"',"") '",'
             '"' IF AVAIL ar-invl THEN REPLACE(REPLACE(ar-invl.part-dscr1,",",""),'"',"")
                 ELSE "" '",'
             '"' IF AVAIL ar-invl THEN REPLACE(REPLACE(ar-invl.part-no,",",""),'"',"")
                 ELSE ""    '",'
             '"' STRING(v-pric,"->>>>>>9.99<<") '",'
             '"' v-uom                          '",'
             '"' STRING(v-qty[1],"->>>>>>>>") '",'
