...
ASSIGN cTextListToSelect = "Name,City,State,Rep,Budget Year,Sales1,Delta1,%-OF Sales Differ1," +
...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Name,City,State,Rep,Budget Year,Sales1,Delta1,%-OF Sales Differ1," +
                           "Sales2,Delta2,%-OF Sales Differ2,Sales3,Delta3,%-OF Sales Differ3," +
                           "Sales4,Delta4,%-OF Sales Differ4"  .
...
end_slsmn tb_zer-col sl_avail sl_selected rd-dest lv-ornt lines-per-page ~
...
DEFINE FRAME FRAME-A
     ...
     lbl_sort AT ROW 7.57 COL 20 COLON-ALIGNED NO-LABEL
     rd_print AT ROW 7.57 COL 32 NO-LABEL
     tb_cust-list AT ROW 8.76 COL 32.4 WIDGET-ID 6
     btnCustList AT ROW 8.81 COL 64.4 WIDGET-ID 8
     ...
     sl_avail AT ROW 14.1 COL 4 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 14.1 COL 40 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 14.1 COL 59.4 NO-LABEL WIDGET-ID 28
     Btn_Add AT ROW 15.1 COL 40 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 16.1 COL 40 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 17.14 COL 40 WIDGET-ID 40
     btn_down AT ROW 18.14 COL 40 WIDGET-ID 42
     ...
     lv-font-name AT ROW 24.1 COL 28 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 25.33 COL 30
     tb_excel AT ROW 26.52 COL 50 RIGHT-ALIGNED
     tb_runExcel AT ROW 26.52 COL 71 RIGHT-ALIGNED
     fi_file AT ROW 27.33 COL 28 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 29.14 COL 19
     btn-cancel AT ROW 29.14 COL 57
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 13.38 COL 59.4 WIDGET-ID 44
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 13.38 COL 4.8 WIDGET-ID 38
     ...
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 19.76 COL 5
...
DO:
   apply "close" to this-procedure.
    {Advantzware/WinKit/winkit-panel-triggerend.i} /* added by script _nonAdm1.p on 03.28.2017 @ 10:43:14 am */
END.
...
ON CLOSE OF THIS-PROCEDURE DO:
   RUN disable_UI.
   {Advantzware/WinKit/closewindow-nonadm.i} /* added by script _nonAdm1.p on 03.28.2017 @ 10:43:14 am */
END.
...
/*  IF access-close THEN DO:
  ...
  ELSE
   ...
   END.
    ...
    {Advantzware/WinKit/embedfinalize-nonadm.i} /* added by script _nonAdm1.p on 03.28.2017 @ 10:43:14 am */
...
------------------------------------------------------------------------------*/
  DISPLAY begin_date1 end_date1 begin_date2 end_date2 begin_date3 end_date3 
          ...
          tb_zer-col sl_avail sl_selected rd-dest lv-ornt lines-per-page 
          lv-font-no lv-font-name td-show-parm tb_excel tb_runExcel fi_file 
        ...
        /*  PUT tt-report2.cust-name          FORMAT "x(25)" SPACE(1)
         ...
         ASSIGN cDisplay = ""
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "name"     THEN cVarValue = string(tt-report2.cust-name) . 
                         WHEN "city"     THEN cVarValue = v-city.                        
                         WHEN "stat"     THEN cVarValue = v-state.                       
                         WHEN "rep"      THEN cVarValue = tt-report2.sman  .
                         WHEN "bud-yr"   THEN cVarValue = STRING(tt-report2.curr-year,"$->,>>>,>>>,>>9.99") .
                         WHEN "sals1"    THEN cVarValue = STRING(tt-report2.Sales1,"$->,>>>,>>>,>>9.99")    .
                         WHEN "delt1"    THEN cVarValue = STRING(tt-report2.Delta1,"$->,>>>,>>>,>>9.99")    .
                         WHEN "diff1"    THEN cVarValue = IF tt-report2.per1 = 0 OR tt-report2.per1 = ? THEN "" ELSE STRING(tt-report2.per1,"->,>>>,>>>,>>9.99%") .
                         WHEN "sals2"    THEN cVarValue = STRING(tt-report2.Sales2,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt2"    THEN cVarValue = STRING(tt-report2.Delta2,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff2"    THEN cVarValue = IF tt-report2.per2 = 0 OR tt-report2.per2 = ? THEN "" ELSE STRING(tt-report2.per2,"->,>>>,>>>,>>9.99%") .
                         WHEN "sals3"    THEN cVarValue = STRING(tt-report2.Sales3,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt3"    THEN cVarValue = STRING(tt-report2.Delta3,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff3"    THEN cVarValue = IF tt-report2.per3 = 0 OR tt-report2.per3 = ? THEN "" ELSE STRING(tt-report2.per3,"->,>>>,>>>,>>9.99%") .
                         WHEN "sals4"    THEN cVarValue = STRING(tt-report2.Sales4,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt4"    THEN cVarValue = STRING(tt-report2.Delta4,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff4"    THEN cVarValue = IF tt-report2.per4 = 0 OR tt-report2.per4 = ? THEN "" ELSE STRING(tt-report2.per4,"->,>>>,>>>,>>9.99%") .
          ...
          IF LAST-OF(tt-report2.sman) THEN do:
              ...
              ASSIGN cDisplay = ""
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "bud-yr"   THEN cVarValue = STRING(sal-tot[1],"$->,>>>,>>>,>>9.99") .
                         WHEN "sals1"    THEN cVarValue = STRING(sal-tot[2],"$->,>>>,>>>,>>9.99")    .
                         WHEN "delt1"    THEN cVarValue = STRING(sal-del[1],"$->,>>>,>>>,>>9.99")    .
                         WHEN "diff1"    THEN cVarValue = IF sal-per[1] = 0 OR sal-per[1] = ? THEN "" ELSE STRING(sal-per[1],"->,>>>,>>>,>>9.99%") .
                         WHEN "sals2"    THEN cVarValue = STRING(sal-tot[3],"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt2"    THEN cVarValue = STRING(sal-del[2],"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff2"    THEN cVarValue = IF sal-per[2] = 0 OR sal-per[2] = ? THEN "" ELSE STRING(sal-per[2],"->,>>>,>>>,>>9.99%") .
                         WHEN "sals3"    THEN cVarValue = STRING(sal-tot[4],"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt3"    THEN cVarValue = STRING(sal-del[3],"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff3"    THEN cVarValue = IF sal-per[3] = 0 OR sal-per[3] = ? THEN "" ELSE STRING(sal-per[3],"->,>>>,>>>,>>9.99%") .
                         WHEN "sals4"    THEN cVarValue = STRING(sal-tot[5],"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt4"    THEN cVarValue = STRING(sal-del[4],"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff4"    THEN cVarValue = IF sal-per[4] = 0 OR sal-per[4] = ? THEN "" ELSE STRING(sal-per[4],"->,>>>,>>>,>>9.99%") .
            ...
            PUT UNFORMATTED "TOTAL Sales  " substring(cDisplay,14,350) SKIP.
            IF tb_excel THEN DO:
...
format header
 ...
 FOR EACH ttRptSelected BY ttRptSelected.DisplayOrder:
     ...
     IF LOOKUP(ttRptSelected.TextList, "Budget Year") <> 0    THEN do:
            ASSIGN ttRptSelected.TextList = ttRptSelected.TextList + " " + string(v-get-year) .
     END.
     IF LOOKUP(ttRptSelected.TextList, "Sales1") <> 0    THEN do:
            ASSIGN ttRptSelected.TextList = ttRptSelected.TextList + " " + string(v-label[1]) .
     END.
     IF LOOKUP(ttRptSelected.TextList, "Sales2") <> 0    THEN do:
            ASSIGN ttRptSelected.TextList = ttRptSelected.TextList + " " + string(v-label[2]) .
     END.
     IF LOOKUP(ttRptSelected.TextList, "Sales3") <> 0    THEN do:
            ASSIGN ttRptSelected.TextList = ttRptSelected.TextList + " " + string(v-label[3]) .
     END.
     IF LOOKUP(ttRptSelected.TextList, "Sales4") <> 0    THEN do:
            ASSIGN ttRptSelected.TextList = ttRptSelected.TextList + " " + string(v-label[4]) .
     END. 
          ...
          ASSIGN cDisplay = ""
               ...
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "sals1"    THEN cVarValue = STRING(tt-report2.Sales1,"$->,>>>,>>>,>>9.99")    .
                         WHEN "delt1"    THEN cVarValue = STRING(tt-report2.Delta1,"$->,>>>,>>>,>>9.99")    .
                         WHEN "diff1"    THEN cVarValue = IF tt-report2.per1 = 0 THEN "" ELSE STRING(tt-report2.per1,"->,>>>,>>>,>>9.99%") .
                         WHEN "sals2"    THEN cVarValue = STRING(tt-report2.Sales2,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt2"    THEN cVarValue = STRING(tt-report2.Delta2,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff2"    THEN cVarValue = IF tt-report2.per2 = 0 THEN "" ELSE STRING(tt-report2.per2,"->,>>>,>>>,>>9.99%") .
                         WHEN "sals3"    THEN cVarValue = STRING(tt-report2.Sales3,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt3"    THEN cVarValue = STRING(tt-report2.Delta3,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff3"    THEN cVarValue = IF tt-report2.per3 = 0 THEN "" ELSE STRING(tt-report2.per3,"->,>>>,>>>,>>9.99%") .
                         WHEN "sals4"    THEN cVarValue = STRING(tt-report2.Sales4,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "delt4"    THEN cVarValue = STRING(tt-report2.Delta4,"$->,>>>,>>>,>>9.99")    .                              
                         WHEN "diff4"    THEN cVarValue = IF tt-report2.per4 = 0 THEN "" ELSE STRING(tt-report2.per4,"->,>>>,>>>,>>9.99%") .
