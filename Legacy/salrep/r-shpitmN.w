...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "PRODUCT,DESCRIPTION,SHIP TO,NAME,CITY,ST,INV DATE,QTY SHIP,"
                           + "UNIT PRICE,TOTAL SALE" .
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-shpitm.csv" 
...
DEFINE FRAME FRAME-A
     ...
     begin_ship AT ROW 3.86 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Ship-to#"
     end_ship AT ROW 3.86 COL 69 COLON-ALIGNED HELP
          "Enter Ending Zip Code"
     begin_i-no AT ROW 4.81 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Item Number"
     end_i-no AT ROW 4.81 COL 69 COLON-ALIGNED HELP
          "Enter Ending Customer Number"
     begin_inv-date AT ROW 5.76 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Invoice Date"
     end_inv-date AT ROW 5.76 COL 69 COLON-ALIGNED HELP
     ...
     rd-dest AT ROW 15.52 COL 7 NO-LABEL
     ...
     tb_excel AT ROW 21.38 COL 50.2 RIGHT-ALIGNED
     tb_runExcel AT ROW 21.38 COL 71.2 RIGHT-ALIGNED
     fi_file AT ROW 22.19 COL 28.2 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 24.33 COL 19
...
def var fcust as   char init "".
def var tcust like fcust init "zzzzzzzz".
def var fship as   char init "".
...
def var v-name like shipto.ship-name.
def var v-city like shipto.ship-city.
def var v-stat like shipto.ship-state.
def var v-pric as   dec format ">>>>>>.99<<<<".
def var v-astr as   char format "x" extent 2.
       ...
       ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "prod"    THEN cVarValue = string(ar-invl.i-no,"x(15)") .
                         WHEN "desc"   THEN cVarValue = IF ar-invl.i-dscr <> "" THEN string(ar-invl.i-dscr,"x(30)") ELSE IF AVAIL itemfg THEN STRING(itemfg.i-name,"x(30)") ELSE "" .
                         WHEN "shipto"   THEN cVarValue = STRING(tt-report.key-03,"x(15)").
                         WHEN "name"  THEN cVarValue = STRING(v-name,"x(20)") .
                         WHEN "city"   THEN cVarValue = STRING(v-city,"x(15)") .
                         WHEN "st"  THEN cVarValue = STRING(v-stat,"x(2)") .
                         WHEN "inv-date"   THEN cVarValue = STRING(bf-ar-inv.inv-date,"99/99/99") .
                         WHEN "qty-ship"  THEN cVarValue = STRING(ar-invl.inv-qty,"->>,>>>,>>9") .
                         WHEN "unit-pr"   THEN cVarValue = STRING(v-pric,">>>>>>9.99<<<") .
                         WHEN "tot-sal"  THEN cVarValue = STRING(ar-invl.amt,"->>>,>>>,>>9.99") .
    ...
    if last-of(tt-report.key-01) then do:
      ...
      down with frame itemx.*/
       ...
       ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "tot-sal"  THEN cVarValue = STRING(v-amt[1],"->>>,>>>,>>9.99") .
            ...
            END.
            PUT str-line SKIP .
            PUT UNFORMATTED  "        ** PRODUCT TOTAL **"  substring(cDisplay,28,350) SKIP.
            IF tb_excel THEN DO:
                 PUT STREAM excel UNFORMATTED  '** PRODUCT TOTAL ** ,'
    ...
    if last(tt-report.key-01) then do:
      ...
      down with frame itemx.*/
        ...
        ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "unit-pr"   THEN cVarValue = "" .
                         WHEN "tot-sal"  THEN cVarValue = STRING(v-amt[2],"->>>,>>>,>>9.99") .
            ...
            END.
            PUT str-line SKIP .
            PUT UNFORMATTED  "          ** GRAND TOTAL **"  substring(cDisplay,28,350) SKIP.
            IF tb_excel THEN DO:
                 PUT STREAM excel UNFORMATTED  '** GRAND TOTAL ** ,'
