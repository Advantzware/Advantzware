...
DEF VAR POFarmOutScores-log AS LOG NO-UNDO.
/* Indicates to always create scores records for FG items, not just board */
/* Nk1look.p placed here to ensure NK1 record is created, can be used to replace the code below */
RUN sys/ref/nk1look.p (INPUT cocode, "POFarmOutScores", "L" /* Logical */, NO /* check by cust */, 
                       INPUT YES /* use cust not vendor */, "" /* cust */, "" /* ship-to*/,
                       OUTPUT lcNk1Value, OUTPUT llRecFound).
IF llRecFound THEN
POFarmOutScores-log = LOGICAL(lcNk1Value) NO-ERROR.
