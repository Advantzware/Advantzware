...
/* ***************************  Definitions  ************************** */
DEF VAR gvlDebug AS LOG NO-UNDO.
gvldebug = NO.
DEF STREAM sDebug.
...
IF gvcFilIdSource EQ "JOB" THEN
  RUN ttItemfgFromJob (INPUT gvrJob, INPUT gvcFilIdSource).
...
DEF BUFFER bf-w-job-mat FOR w-job-mat.
...
FIND bf-ordl      WHERE ROWID(bf-ordl)      EQ iprOeOrdl NO-LOCK NO-ERROR.  
  ...
  bf-po-ord.type = "D".
  RELEASE itemfg.
  IF AVAIL bf-ordl AND NOT bf-w-job-mat.this-is-a-rm THEN
  ...
  IF gvcShipChoice EQ "C" THEN DO WHILE TRUE:
...
------------------------------------------------------------------------------*/
  ...
  IF nk1-oeautopo-char EQ "AutoRM" AND v-autopo-sec THEN DO:
    ...
    FIND FIRST b-item
        WHERE b-item.company EQ cocode
          ...
          AND b-item.i-no    NE ""
        NO-LOCK NO-ERROR.
    IF AVAIL b-item THEN bf-po-ordl.i-no = v-new-i-no.
  ...
  FIND bf-po-ordl WHERE ROWID(bf-po-ordl) EQ iprPoOrdl NO-LOCK NO-ERROR.
    ...
    IF v-len               EQ 0                       AND
      ...
      (bf-po-ordl.pr-qty-uom NE bf-po-ordl.cons-uom OR
       bf-po-ordl.pr-uom     NE bf-po-ordl.cons-uom)        THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  IF oeautofg-log AND v-autofg-sec AND AVAIL oe-ord AND AVAIL oe-ordl THEN
  ...
  IF AVAIL bf-job AND ((nk1-oeautopo-char NE "Manual" AND v-autopo-sec AND v-po-best EQ NO) OR v-po-best) THEN
    RUN wJobFromJobMat (INPUT v-po-best, INPUT ROWID(bf-job), INPUT gvrOeOrdl).
  
  IF AVAIL bf-job 
       AND oeautoprep-log AND v-autoprep-sec 
       AND AVAIL bf-ordl AND v-po-best EQ NO THEN 
    DO:
...
IF AVAIL tt-ei THEN DO:
    ...
    IF AVAIL report THEN DO:
      ...
      IF AVAIL vend THEN DO:   
        ...
        IF AVAIL bf-ordl AND iplFirstFrm THEN
          FOR EACH b-oe-ordl
...
------------------------------------------------------------------------------*/
  ...
  DEF BUFFER bf2-ordl FOR oe-ordl.
  DEF BUFFER bf-job-hdr FOR job-hdr.
  DEF BUFFER bf2-w-job-mat FOR w-job-mat.
  ...
  IF bf-po-ordl.item-type THEN DO:
    ...
    IF v-po-qty OR bf-w-job-mat.n-up EQ 0 OR
      INDEX("BP",bf-item.mat-type) LE 0 THEN  DO:       
    ...
    ELSE DO:
      ...
      END.
      ELSE  IF AVAIL est AND (est.est-type EQ 4 OR est.est-type EQ 8) THEN DO:
        ld-line-qty = 0.
        ...
        ELSE IF AVAIL  bf-w-job-mat THEN DO:
      ...
      IF bf-po-ordl.pr-qty-uom EQ "EA" THEN DO:
...
FIND b-item WHERE ROWID(b-item) EQ iprItem NO-LOCK NO-ERROR.
  ...
  bf-po-ordl.cust-no = IF AVAIL bf-ord THEN bf-ord.cust-no
                    ELSE
...
------------------------------------------------------------------------------*/
  ...
  FIND bf-w-job-mat WHERE ROWID(bf-w-job-mat) EQ iprWJobMat NO-LOCK NO-ERROR.
  FIND bf-po-ordl WHERE ROWID(bf-po-ordl) EQ iprPoOrdl EXCLUSIVE-LOCK NO-ERROR.  
  IF NOT AVAIL bf-w-job-mat OR NOT AVAIL bf-po-ordl THEN
    RETURN ERROR.
  ...
  IF bf-po-ordl.pr-qty-uom EQ "BF" THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL bf-po-ordl THEN
    ...
    IF NOT bf-po-ord.received AND (v-hld-line-qty EQ 0 OR v-hld-cost EQ 0) THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  outers:
  REPEAT iCnt = 1 TO 2:
    IF iCnt NE 1 THEN DO:
      ASSIGN oplNextOuters = TRUE.
      LEAVE.
    END.
...
FIND bf-ordl WHERE ROWID(bf-ordl) EQ iprOeOrdl NO-LOCK NO-ERROR.
  ...
  IF NOT AVAIL po-ordl AND nk1-oeautopo-char EQ "AutoRM" AND v-autopo-sec AND bf-w-job-mat.this-is-a-rm THEN DO:
    FIND FIRST po-ordl
        WHERE po-ordl.company EQ cocode
          AND po-ordl.job-no  EQ bf-w-job-mat.job-no
          AND po-ordl.job-no2 EQ bf-w-job-mat.job-no2
...
DEFINE INPUT  PARAMETER iprWJobMat AS ROWID       NO-UNDO.
DEF BUFFER bf-w-job-mat FOR w-job-mat.
FIND bf-w-job-mat WHERE ROWID(bf-w-job-mat) EQ iprWJobMat
   NO-LOCK NO-ERROR.
FIND itemfg WHERE ROWID(itemfg) EQ iprItemfg NO-LOCK NO-ERROR.
IF NOT AVAIL itemfg THEN
  RETURN ERROR.
    ...
    FOR EACH e-itemfg
        ...
        IF bf-w-job-mat.est-no NE "" THEN DO:
           FOR EACH e-itemfg-vend NO-LOCK
             ...
             IF LAST(e-itemfg-vend.eqty)            OR
                e-itemfg-vend.eqty GE bf-w-job-mat.qty THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  DEF BUFFER bf-w-job-mat FOR w-job-mat.
  FIND bf-w-job-mat WHERE ROWID(bf-w-job-mat) EQ iprJobMat
    NO-LOCK NO-ERROR.
  IF gvlDebug THEN             
...
FIND bf-oe-ordl WHERE ROWID(bf-oe-ordl) EQ iprOeOrdl NO-LOCK NO-ERROR.
  ...
  FIND FIRST po-ord
      WHERE po-ord.company   EQ cocode
        ...
        AND b-orderpo.val[1] NE 0
      NO-LOCK NO-ERROR.
  IF AVAIL po-ord THEN gvcVendNo = po-ord.vend-no.  
...
FIND bf-w-job-mat WHERE ROWID(bf-w-job-mat) EQ iprWJobMat NO-ERROR.
  ...
  IF AVAIL tt-ei THEN DO:
    ...
    IF bf-w-job-mat.qty-uom EQ "BF" THEN DO:
...
END.
  ...
  IF bf-po-ordl.item-type THEN DO:
    IF AVAIL bf-tt-ei THEN DO:
      ...
      IF AVAIL bf-ordl AND bf-po-ordl.item-type THEN DO:
        ...
        IF nk1-oeautopo-char EQ "AutoRM" AND v-autopo-sec AND NOT v-new-avail THEN DO:         
          fil_id = RECID(b-item).
...
------------------------------------------------------------------------------*/
  ...
  FIND job-mat WHERE RECID(job-mat) EQ bf-w-job-mat.w-recid NO-LOCK NO-ERROR.
  ...
  FIND po-ordl WHERE ROWID(po-ordl) EQ ROWID(bf-po-ordl) EXCLUSIVE-LOCK.
  FIND po-ord WHERE ROWID(po-ord) EQ ROWID(bf-po-ord) EXCLUSIVE-LOCK.
...
FIND po-ord WHERE ROWID(po-ord) EQ iprPoOrd NO-LOCK NO-ERROR.
  ...
  IF NOT gvlChoice THEN DO:
    /****************************************/
    /** Chose not to update the existing PO */
    /****************************************/
    gvlChoice = NO.
  
    IF gvcVendNo EQ "" THEN DO:
      IF AVAIL tt-eiv THEN
        FIND FIRST po-ord
      ...
      ELSE DO: /* not avail tt-eiv */
        ...
        IF AVAIL vend THEN DO:
          ...
          IF AVAIL bf-ordl AND iplFirstOfFrm THEN
          FOR EACH b-oe-ordl
              WHERE b-oe-ordl.company EQ bf-ordl.company
                AND b-oe-ordl.ord-no  EQ bf-ordl.ord-no
                AND b-oe-ordl.job-no  EQ bf-ordl.job-no
...
FOR EACH w-job-mat    
 ...
 ASSIGN 
  ...
  IF NOT w-job-mat.this-is-a-rm THEN DO:
  ...
  IF gvcVendNo EQ "" OR ll-canceled THEN DO:
    IF gvlDebug THEN             
  ...
  FIND po-ord WHERE ROWID(po-ord) EQ gvrPoOrd NO-LOCK NO-ERROR.
  FIND oe-ord WHERE ROWID(oe-ord) EQ gvrOeOrd NO-LOCK NO-ERROR.
  ...
  IF NOT gvlChoice AND AVAIL(po-ord) THEN DO:
    IF gvlDebug THEN             
  ...
  FIND vend WHERE ROWID(vend) EQ gvrVend NO-LOCK NO-ERROR.
  ...
  FIND itemfg WHERE ROWID(itemfg) EQ gvrItemfg NO-LOCK NO-ERROR.
...
FIND b-orderpo WHERE ROWID(b-orderpo) EQ iprB-orderpo NO-LOCK NO-ERROR.
  ...
  IF AVAIL po-ord AND NOT po-ord.opened THEN DO: 
    oplNextOuters = TRUE.
  END. /* If PO was found but was not opened */
  ELSE DO:

  
    gvlChoice = AVAIL po-ord.
  
    IF gvlChoice AND po-ord.opened AND v-autopo-sec THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  FIND vend WHERE ROWID(vend) EQ iprVend NO-LOCK NO-ERROR.
  FIND bf-ord WHERE ROWID(bf-ord) EQ iprOeOrd NO-LOCK NO-ERROR.
  FIND bf-job WHERE ROWID(bf-job) EQ iprJob NO-LOCK NO-ERROR.

  /* vend.disc-days eq lead time */
  IF AVAIL vend THEN gvdPoDate = TODAY.
  ELSE gvdPoDate = IF AVAIL bf-ord THEN bf-ord.ord-date ELSE bf-job.start-date.

  IF AVAIL vend THEN gvdDueDate = TODAY + vend.disc-days.
  ELSE gvdDueDate = IF AVAIL bf-ord THEN bf-ord.ord-date ELSE bf-job.start-date.
...
FIND bf-po-ordl WHERE ROWID(bf-po-ordl) EQ iprPoOrdl EXCLUSIVE-LOCK NO-ERROR.
FIND bf-po-ord  WHERE ROWID(bf-po-ord)  EQ iprPoOrd  NO-LOCK NO-ERROR.
FIND bf-itemfg  WHERE ROWID(bf-itemfg)  EQ iprItemfg NO-LOCK NO-ERROR.
FIND bf-w-job-mat WHERE ROWID(bf-w-job-mat) EQ iprWJobMat NO-LOCK NO-ERROR.
FIND bf-ordl WHERE ROWID(bf-ordl) EQ iprOeOrdl NO-LOCK NO-ERROR.
FIND bf-job WHERE ROWID(bf-job) EQ iprJob NO-LOCK NO-ERROR.
...
END.
    ...
    IF AVAIL bf-itemfg THEN DO:
     ...
     FIND FIRST xest
         ...
         NO-ERROR.
     IF AVAIL(xest) THEN
       FIND FIRST xeb WHERE xeb.company = bf-po-ordl.company
  ...
  /*      END.                                                          
      ...
      bf-po-ordl.ord-no = IF AVAIL bf-ordl THEN bf-ordl.ord-no ELSE 0.
      FIND oe-ordl WHERE RECID(oe-ordl) EQ lv-recid EXCLUSIVE-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  FIND bf-po-ord WHERE ROWID(bf-po-ord) EQ iprPoOrd EXCLUSIVE-LOCK NO-ERROR.
  FIND bf-ordl WHERE ROWID(bf-ordl) EQ iprOeOrdl EXCLUSIVE-LOCK NO-ERROR.
  FIND bf-w-job-mat WHERE ROWID(bf-w-job-mat) EQ iprWJobMat NO-LOCK NO-ERROR.
  FIND b-orderpo WHERE ROWID(b-orderpo) EQ iprB-orderpo EXCLUSIVE-LOCK NO-ERROR.

  FIND CURRENT b-orderpo NO-ERROR.
  IF NOT AVAIL b-orderpo THEN DO:
  ...
  IF AVAIL bf-ordl        AND bf-po-ord.type EQ "D"    AND
     bf-po-ord.cust-no NE "" AND bf-po-ord.frt-pay NE "P" THEN
  DO:
     FIND FIRST oe-rel OF bf-ordl NO-LOCK NO-ERROR.
     IF AVAIL oe-rel THEN bf-po-ord.carrier = oe-rel.carrier.
  END.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL job AND ipcFilIdSource = "JOB" THEN DO:
     FOR EACH job-hdr NO-LOCK
         WHERE job-hdr.company EQ job.company
           AND job-hdr.job     EQ job.job
           AND job-hdr.job-no  EQ job.job-no
...
FIND bf-po-ord WHERE ROWID(bf-po-ord) EQ iprPoOrd EXCLUSIVE-LOCK NO-ERROR.
IF NOT AVAIL bf-po-ord THEN DO:
...
------------------------------------------------------------------------------*/
   DEF INPUT PARAMETER iprJob AS ROWID NO-UNDO.
   DEF BUFFER bf-job FOR job.
...
DEFINE INPUT  PARAMETER ipcIno AS CHARACTER   NO-UNDO.
DEF BUFFER bfOeOrd FOR oe-ord.


FIND bfOeOrd WHERE ROWID(bfOeOrd) EQ iprOrd NO-LOCK NO-ERROR.

IF NOT AVAIL bfOeOrd THEN
  RETURN.
...
FOR EACH oe-Ordm 
   ...
   FIRST est-prep 
      ...
      NO-LOCK ,
    FIRST prep FIELDS(i-no number-up) 
