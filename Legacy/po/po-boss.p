      ...
      for each po-ordl WHERE
       ...
       /* ELSE
        ...
        PUT /*int(po-ordl.over-pct) FORM ">>9" AT 7 " / "
            ...
            po-ordl.i-name AT 20 FORM "x(30)" SPACE(1)
            /*v-adder[2]*/ SPACE(27)
            v-change-dscr SKIP.
        ...
        if po-ordl.dscr[1] ne "" /*OR v-adder[3] <> ""*/ then do:
           put po-ordl.dscr[1] format "x(30)"  at 20 " "             
               /*v-adder[3] */
               skip.
        ...
        if po-ordl.dscr[2] ne "" /*OR v-adder[4] <> ""*/ then do:
          put po-ordl.dscr[2] format "x(30)" at 20              
             /* " " v-adder[4]*/ skip.
        ...
        IF v-wid GT 0 THEN DO:
          PUT "W: " AT 20 FNmetric(v-wid, v-wid2, "W") FORMAT "x(8)" SPACE(1).
          IF v-len GT 0 THEN
        ...
        {po/poprints.i}
         ...
         END.
         IF s-group-notes AND v-printline + li > 49 THEN DO:
           PAGE.
           v-printline = 0.
           {po/po-boss.i}
         END.
...
FOR EACH notes WHERE notes.rec_key = po-ord.rec_key NO-LOCK:
  ...
  PUT /*"<R53><C1>" /*v-inst[1] */ "jdksfsfh"
      "<R54><C1>" v-inst[2]
      "<R55><C1>" v-inst[3]
      "<R56><C1>" v-inst[4] "dskjfhksjdfhksdj " */
