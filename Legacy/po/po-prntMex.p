/* --------------------------------------------- po/po-prntMex.p */
/* Purchase Order Print Program for S-8-POPRINT = POPrint-Mex                        */
/* -------------------------------------------------------------------------- */
...
DEF VAR v-cut-po LIKE oe-ord.po-no no-undo.
...
ASSIGN ls-image1 = "images\premierpo.jpg"
       ls-image2 = ""
...
DEF VAR v-vend-ino AS cha NO-UNDO.
...
DEF VAR lv-cust-part AS CHAR FORM "x(15)" NO-UNDO.
DEF VAR lv-i-name AS cha FORM "x(30)" NO-UNDO.
      ...
      for each po-ordl 
       ...
       :
        ...
        FIND item where item.company eq po-ordl.company
                  ...
                  no-lock no-error.
        ASSIGN
            v-vend-item = (IF AVAIL ITEM /*AND ITEM.vend-no = po-ord.vend)*/ THEN ITEM.vend-no ELSE "")
                          +
                          (IF AVAIL ITEM /*AND ITEM.vend2-no = po-ord.vend)*/ THEN (" " + ITEM.vend2-no) ELSE "")

            v-vend-ino = (IF AVAIL ITEM /*AND ITEM.vend-no = po-ord.vend)*/ THEN ITEM.vend-item ELSE "")            
                         +                                                                                        
                         (IF AVAIL ITEM /*AND ITEM.vend2-no = po-ord.vend)*/ THEN (" " + item.vend2-item) ELSE "")  .

        
        IF v-vend-item EQ "" THEN do:           /*Task# 10041302*/
            FIND itemfg where itemfg.company eq po-ordl.company
                and itemfg.i-no  eq po-ordl.i-no
                /*and po-ordl.item-type*/ no-lock no-error.
            ASSIGN
            v-vend-item = (IF AVAIL itemfg /*AND ITEM.vend-no = po-ord.vend)*/ THEN itemfg.vend-no ELSE "")
                            +
                          (IF AVAIL itemfg /*AND ITEM.vend2-no = po-ord.vend)*/ THEN (" " + itemfg.vend2-no) ELSE "")
            v-vend-ino  = (IF AVAIL itemfg /*AND ITEM.vend-no = po-ord.vend)*/ THEN itemfg.vend-item ELSE "")            
                          +                                                                                        
                          (IF AVAIL itemfg /*AND ITEM.vend2-no = po-ord.vend)*/ THEN (" " + itemfg.vend2-item) ELSE "")  .
        END.
...
***/
        ...
        IF AVAIL itemfg THEN
         ...
         FIND FIRST cust-part WHERE cust-part.company = itemfg.company 
            ...
            IF lv-cust-part = "" AND AVAIL itemfg THEN
                 ASSIGN lv-cust-part = itemfg.part-no.


           ASSIGN lv-i-name = po-ordl.i-name .
           IF lv-cust-part = "" THEN DO:
      ...
      /*  IF TRIM(v-lstloc) NE "" THEN              /*Task# 10041302*/
          PUT "WHS/BIN" AT 7.  */            
        /* gdm - 11040905 end */
        ...
        lv-add-line = NO.
        /* gdm - 11040905 */
      /*  IF TRIM(v-lstloc) NE "" THEN DO:      /*Task# 10041302*/
            PUT v-lstloc AT 7 FORM "x(15)".
            lv-add-line = YES.
        END. */
         if lv-i-name ne ""  then do:
        ...
        IF po-ordl.vend-i-no NE "" THEN do:
            put "<C21>" po-ordl.vend-i-no  FORM "x(30)" 
                skip.
            lv-add-line = NO.
            v-line-number = v-line-number + 1.
            v-printline = v-printline + 1.
        ...
        FIND FIRST oe-ord WHERE oe-ord.company EQ po-ordl.company               /*Task# 10041302*/
                AND oe-ord.ord-no EQ po-ordl.ord-no NO-LOCK NO-ERROR.
...
PUT "<FArial><R58><C1><P12><B> Términos y Condiciones </B> <P9> " SKIP
