...
DEFINE TEMP-TABLE tt_sch NO-UNDO 
  FIELD m-code AS cha
  FIELD Start-date AS date
  FIELD job-list AS cha FORM "x(50)"
  INDEX udx IS UNIQUE PRIMARY m-code start-date.
 
DEFINE TEMP-TABLE tt_Start-date NO-UNDO
  FIELD start-seq AS cha
  FIELD Start-date AS DATE
  FIELD start-time AS INT
  INDEX udx IS PRIMARY Start-date.
...
---------------------------------------------------------------------*/
  ...
  i = 0.
  FOR EACH job-mch NO-LOCK BREAK BY job-mch.m-code BY job-mch.start-date BY start-time:
     i = i + 1.
     FIND tt_sch WHERE tt_sch.m-code = job-mch.m-code
                   AND tt_sch.start-date = job-mch.start-date NO-ERROR.
     IF NOT AVAILABLE(tt_sch) THEN
     DO:
       CREATE tt_Sch.
       ASSIGN tt_Sch.m-code = job-mch.m-code
              tt_Sch.start-date = job-mch.start-date.
     END.
     ASSIGN tt_sch.job-list = tt_sch.job-list + job-mch.job-no.
 
     /* We need a list of the sales rep columns to create. Use a temp-table
        to make sure you have them all. */
     IF NOT CAN-FIND(tt_Start-date WHERE tt_start-date.Start-date = job-mch.start-date)
     THEN DO /*j = 1 TO 24: */ :
       CREATE tt_Start-date.
       ASSIGN tt_start-date.start-seq = string(i)
              tt_Start-date.Start-date = job-mch.start-date
              tt_start-date.start-time = job-mch.start-time.
     END.
     IF i > 200 THEN LEAVE.
  END.
  ...
  hXTab:ADD-LIKE-FIELD("m-code","job-mch.m-code").

  /* Now add a field for each sales rep */
  FOR EACH tt_Start-date BREAK BY tt_start-date.start-date:
    hXTab:ADD-NEW-FIELD(tt_start-date.start-seq,"cha",0,"x(10)","","Start Date").
  END.

  /* Index the stuff on the state field */
  /*  hXTab:ADD-NEW-INDEX("udx",true,true).
    hXTab:ADD-INDEX-FIELD("udx","m-code").
  */
  ...
  DO iCount = 2 TO hTTBuff:NUM-FIELDS:
    ...
    FIND tt_Start-date NO-LOCK
      WHERE tt_start-date.start-seq = hField:NAME NO-ERROR.
    IF AVAILABLE(tt_start-date) THEN
      hField:LABEL = string(tt_start-date.start-date,"99/99/9999").
  ...
  FOR EACH tt_sch ,
      FIRST tt_start-date WHERE tt_start-date.start-date = tt_sch.start-date
      BREAK BY tt_Sch.m-code BY tt_Sch.start-date:
 
    /* If this is the first of the state, create a record in the
       Cross tab table for the state */
    IF FIRST-OF(tt_Sch.m-code) THEN
    DO:
      hTTBuff:BUFFER-CREATE().
      hField = hTTBuff:BUFFER-FIELD("m-code").
      hField:BUFFER-VALUE = tt_sch.m-code.
    END.
   
    /* Get the field in the crosstab table that is this sales reps field */
    hField = hTTBuff:BUFFER-FIELD(tt_start-date.start-seq).
    /* Set the amount */
    hField:BUFFER-VALUE = tt_sch.job-list.
 
    /* If this is the last of the state, release the record */
    IF LAST-OF(tt_Sch.m-code) THEN
    DO:
