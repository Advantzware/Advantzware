...
DEFINE VARIABLE h_b-jobsch AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-mach AS HANDLE NO-UNDO.
DEFINE VARIABLE h_exit AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_options2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_pv-jobsch AS HANDLE NO-UNDO.
DEFINE VARIABLE h_smartmsg AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vp-color AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Scheduling Process */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Scheduling Process */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_options2 ).
       RUN set-position IN h_options2 ( 1.00 , 85.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.81 , 55.80 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'jcsch/b-mach.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-mach ).
       RUN set-position IN h_b-mach ( 5.05 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-mach ( 15.95 , 13.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'jcsch/b-jobsch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_b-jobsch ).
       RUN set-position IN h_b-jobsch ( 5.05 , 19.00 ) NO-ERROR.
       RUN set-size IN h_b-jobsch ( 15.71 , 130.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'jcsch/pv-jobsch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Edge-Pixels = 2,
                     SmartPanelType = Update,
                     AddFunction = One-Record':U ,
             OUTPUT h_pv-jobsch ).
       RUN set-position IN h_pv-jobsch ( 21.00 , 19.00 ) NO-ERROR.
       RUN set-size IN h_pv-jobsch ( 1.76 , 129.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'jcsch/vp-color.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vp-color ).
       RUN set-position IN h_vp-color ( 22.91 , 10.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.43 , 132.00 ) */

       /* Links to SmartBrowser h_b-jobsch. */
