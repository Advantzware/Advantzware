/* ------------------------------------------------- cerep/jobmwest.p  */
/*  factory ticket  for folding, Frankston                                  */
/* -------------------------------------------------------------------------- */
...
v-fill3 = "<C2><FROM><C105><LINE><||3><R-1>".
format header
       "JOB NUMBER:<B>" v-job-no space(0) "-" space(0) v-job-no2 format "99" "</B>"
       "<C43><B><P12>F A C T O R Y   T I C K E T</B><P10>"  "<C77>JOB START DATE:" /*at 123*/ v-start-date skip
       v-fill
    with no-box frame head no-labels stream-io width 155 PAGE-TOP.
...
for each job-hdr NO-LOCK
      ...
      if v-first then do:
        ...
        ELSE PUT "JOB NUMBER:<B>" v-job-no space(0) "-" space(0) v-job-no2 format "99" "</B>"
               "<C43><B><P12>F A C T O R Y   T I C K E T</B><P10>"  "JOB START DATE:" at 123 v-start-date skip
               v-fill SKIP.
...
IF NOT LAST(tt-reftable.val[12]) 
 ...
 PUT "<P10>" v-fill SKIP                                  
     "<B>F# BOARD CODE            VENDOR      SHEETS       LBS SHEET SIZE    NET SHEET     DIE SIZE          DIE#       STRIPPER LOCATION</B>" 
      SKIP.
 FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
                             AND bf-jobhdr.job-no = job-hdr.job-no
                             AND bf-jobhdr.job-no2 = job-hdr.job-no2
          ...
          IF LAST-OF(eb.form-no) THEN DO:
            ...
            /** PRINT SHEET **/
             ...
             for each wrk-sheet WHERE wrk-sheet.form-no = tt-reftable.val[12] break by wrk-sheet.form-no:                 
               ...
               display wrk-sheet.form-no wrk-sheet.brd-dscr
                    /*v-po-duedate*/
        ...
        /** PRINT INK **/
        PUT "<B>F#  PASS             LBS INK NAME            ITEMS      F#  PASS             LBS INK NAME            ITEMS       PLATE #</B>"
            SKIP.
      FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
                                       AND bf-jobhdr.job-no = job-hdr.job-no
                                       AND bf-jobhdr.job-no2 = job-hdr.job-no2
...
=============*/
             for each wrk-ink WHERE wrk-ink.form-no = eb.form-no
                ...
                IF first-OF(wrk-ink.i-code) /*OR v-ink-use-per-blank > 1 */ THEN DO: 
                   ...
                   IF wrk-ink.i-pass = 1 THEN
                       ASSIGN v-ink1[i] = (IF v-pass-count = 1 THEN "F    " ELSE "B    ") +
                                          /*string(wrk-ink.i-code,"X(15)")*/ STRING(v-i-qty,"->>>,>>>,>>9.99") + " " + 
                                          string(wrk-ink.i-dscr,"x(20)") + " " + trim(v-alloc) /*v-item[i]*/
                                          /*+ (IF i = 1 THEN "  " + eb.plate-no ELSE "") */
                              i = i + 1         . 
                    ELSE IF wrk-ink.i-pass = 2 THEN
                       ASSIGN v-ink2[i] = (IF v-pass-count = 1 THEN "F    " ELSE "B    ") +
                                          /*string(wrk-ink.i-code,"X(15)")*/ STRING(v-i-qty,"->>>,>>>,>>9.99") + " " + 
                                          string(wrk-ink.i-dscr,"x(20)") + " " + trim(v-alloc) /*v-item[i]*/
                                          /*+ (IF i = 1 THEN "  " + eb.plate-no ELSE "") */
                              i = i + 1.           
             ...
             DO j = 1 TO 8:
                ...
                IF v-ink1[j] <> "" THEN do:
                   IF v-skip THEN do:
                       PUT  wrk-ink.form-no "  " v-ink1[j] FORM "x(52)" .
                       IF j = 2 THEN do:
                           PUT eb.plate-no AT 110. /*106*/
                           v-plate-printed = YES. 
                       END.
                       PUT SKIP.
                   END.
                   ELSE PUT wrk-ink.FORM-no "  " v-ink1[j] FORM "x(52)" .                                                             
                   v-skip = NOT v-skip.             
                END.
             END.
             /*IF NOT v-plate-printed THEN PUT eb.plate-no AT 106 SKIP.  */
             ...
             DO j = 1 TO 8:
                ...
                END.                
             END.
             IF v-ink1[1] <> "" OR v-ink2[i] <> "" THEN put v-fill at 1 skip.
          END. /* last-of(eb.form-no) */
        end. /* each eb */
      end. /* each ef */
    END. /*first-of(tt-reftable.val[12]*/
    END. /*tt-reftable*/
      END. /* last-of(bf-jobhdr.frm) */
          ...
          IF s-run-speed THEN
            PUT "<B>F# MACHINE                 MR WASTE   MR HRS  RUN SPEED   SPOIL%    SHEETS PER MACHINE   SIZE  TOTAL REQUIRED  PALLET</B>"
                SKIP.
         else
            PUT "<B>F# MACHINE                 MR WASTE   MR HRS   RUN HOUR    SPOIL%    SHEETS PER MACHINE   SIZE  TOTAL REQUIRED  PALLET</B>"
                SKIP.
...
FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
             ...
             IF s-prt-mstandard THEN DO:
                IF s-run-speed THEN
                   PUT wrk-op.s-num "  " wrk-op.m-dscr   SPACE(5)
                       ...
                       wrk-op.spoil[wrk-op.s-num]      SPACE(5)
                       wrk-op.num-sh[wrk-op.s-num]     SPACE(11)  /*9+9*/
    ...
    END. /*tt-reftable*/
      ...
      /*  PUT v-fill AT 1 SKIP.*/
        ...
        /*{custom/notespr4.i job v-inst2 20 "notes.rec_key = job.rec_key and notes.note_form_no = job-hdr.frm and notes.note_code <> '' AND LOOKUP(notes.note_code,v-exc-depts) EQ 0"}
        DO i = 1 TO 20:
