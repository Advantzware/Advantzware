/* cerep/jobfibre.p   Xprint FC Factory  Ticket for Fibre */
/*  factory ticket  for folding , Fibre */
/* -------------------------------------------------------------------------- */
...
form header
     skip(1)
     /*07/22/02 Job Ticket QF-130"*/
    "6/05 Job Ticket QF-130 Rev.A"    to 150
    with no-box no-attr-space frame bott page-bottom stream-io width 160.
*/
...
v-fill = /*fill("=",128)*/ "<||3><C1><FROM><C137><LINE><||3>".
v-fill2 = FILL("-",160).
...
for each job-hdr NO-LOCK
   ...
   find first oe-ordl where oe-ordl.company eq job-hdr.company
    ...
    v-managed-order = IF v-managed-order = "" AND AVAIL reftable AND reftable.val[1] EQ 1 THEN "MANAGED   WAREHOUSE   ORDER"
...
for each job-hdr NO-LOCK
    ...
    if v-first then do:   
     ...
     FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
        ...
        IF FIRST(bf-jobhdr.frm) THEN
           PUT "  <P10><B>F/B   FG ITEM #        ORDER QTY    FORM QTY  PO#       DESCRIPTION      STYLE  CARTON SIZE                COUNT CASE SIZE             CAD#    UP   COLORS" /*v-upc-lbl*/ "</B>" SKIP.
      ...
      /*    if est.est-type eq 4 and eb.stock-no ne job-hdr.i-no then next ebloop. */
        ...
        /*  if last-of(eb.form-no) then do: */
            ...
            display " " trim(string(eb.form-no,">>9")) + "-" +
                    ...
                    oe-ordl.po-no when avail oe-ordl FORM "x(10)"
                    /*eb.part-no*/
                    v-dsc[1] FORM "x(16)"  /*was 18*/
                    eb.style /*v-stypart */
                    v-size[1] FORM "x(26)" /* was 15*/
                    oe-ordl.cas-cnt when avail oe-ordl AND oe-ordl.i-no EQ v-stock-no  
                    eb.cas-cnt when not avail oe-ordl OR oe-ordl.i-no NE v-stock-no or oe-ordl.cas-cnt eq 0 @ oe-ordl.cas-cnt
                    /*PACE(4)*/
    ...
    END. /*tt-reftable*/
     ...
     PUT "<P10>" /*v-fill SKIP*/                       /*REQ'D*/
                 "<B>FORM BOARD CODE          DUE DATE   SHEETS             SHEET SIZE                                        DIE SIZE               DIE#        TOTAL UP</B>" 
                 SKIP.
               ...
               PUT /*display*/ wrk-sheet.form-no
                  ...
                  /*  string(wrk-sheet.sh-wid) + "x" + string(wrk-sheet.sh-len) decimal */
                   ...
                   /* string(ef.nsh-wid) + "x" + string(ef.nsh-len) FORM "x(30)"
                    ...
                    eb.die-no 
                    v-up
                    /*with stream-io width 170 no-labels no-box frame sheet. */ SKIP.
             end. /* each wrk-sheet */
          END. /* last-of(eb.form*/
          END. /* eb */
           END. /* ef */
...
/*===========         
        ...
        {custom/notespr6.i job v-inst2 32 "notes.rec_key = job.rec_key and notes.note_code <> '' AND LOOKUP(notes.note_code,v-exc-depts) EQ 0"}
        lv-do-page-skip = no.
        lv-page-line = PAGE-SIZE - LINE-COUNT.
        IF PAGE-SIZE - LINE-COUNT < 3 THEN PAGE.
        IF LINE-COUNT > PAGE-SIZE OR PAGE-SIZE - LINE-COUNT >= 16 
        ...
        ELSE DO:
            ...
            DO i = ((PAGE-SIZE - LINE-COUNT) * 2 + 1) TO 32:  /* 23-32*/
               ...
               IF lv-new-i > 16 THEN DO: 
                  /*v-dept-inst2[lv-new-i - ((PAGE-SIZE - LINE-COUNT) * 2 + 1)] = v-inst2[i].*/
                  IF lv-tmp-cnt <= 16 THEN
                     v-dept-inst2[lv-tmp-cnt] = v-inst2[i].
                  lv-tmp-cnt = lv-tmp-cnt + 1.
               END.
               ELSE v-dept-inst[lv-new-i] = IF v-inst2[i] BEGINS "<B>" THEN v-inst2[i]
                                     ELSE "   " + v-inst2[i].
        ...
        IF first(bf-jobhdr.frm) THEN PUT "  <B>FORM DEPARTMENT   INSTRUCTION NOTES  <C81>FORM DEPARTMENT   INSTRUCTION NOTES</B>" SKIP.
       /* from top to bottom and right to left*/
        IF v-have-note THEN
           PUT bf-jobhdr.frm "  "  v-dept-inst[1] FORM "x(80)"  "<C68.5>| "v-dept-inst2[1] FORM "x(80)" SKIP.
        DO i = 2 TO 16 /*cnt*/ :
           IF lv-do-page-skip AND i >= lv-PAGE-LINE and
              (v-dept-inst[i] <> "" OR v-dept-inst2[i] <> "") THEN DO:
              PAGE.
