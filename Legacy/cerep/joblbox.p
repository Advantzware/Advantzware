...
for each job-hdr NO-LOCK
      ...
      /*    if est.est-type eq 4 and eb.stock-no ne job-hdr.i-no then next ebloop. */
          ...
          /** BUILD INK WORK FILE **/
          /*if ((est.est-type eq 4 and eb.stock-no = job-hdr.i-no) OR
              est.est-type <> 4) THEN */
          for each job-mat
      ...
      if last-of(job-hdr.frm) then do:
         IF s-run-speed THEN
            PUT "<B>MACHINE                 MR WASTE   MR HRS  RUN SPEED   SPOIL%     SHEETS   MATERIAL FOR MACHINE          SIZE   TOTAL REQUIRED</B>"
                SKIP.
         else
            PUT "<B>MACHINE                 MR WASTE   MR HRS   RUN HOUR    SPOIL%     SHEETS   MATERIAL FOR MACHINE          SIZE   TOTAL REQUIRED</B>"
                SKIP.
              ...
              IF s-prt-mstandard THEN DO:
                IF s-run-speed THEN
                   PUT wrk-op.m-dscr   SPACE(5)
                       ...
                       wrk-op.spoil[job-hdr.frm]      SPACE(2)
    ...
    /*  IF v-ship <> "" THEN v-dept-inst[6] = v-ship.  /* shipto notes */ */
        PUT "<B>DEPARTMENT   INSTRUCTION NOTES</B>" SKIP
            ...
            v-dept-inst[9] FORM "x(128)" SKIP
            v-dept-inst[10] FORM "x(128)" SKIP
            v-dept-inst[11] FORM "x(128)" SKIP
            v-dept-inst[12] FORM "x(128)" SKIP
            v-dept-inst[13] FORM "x(128)" SKIP
            v-dept-inst[14] FORM "x(128)" SKIP
            v-dept-inst[15] FORM "x(128)" SKIP
            v-dept-inst[16] FORM "x(128)" SKIP
 /*           "            " v-dept-inst[17] SKIP
            "            " v-dept-inst[18] SKIP
            "            " v-dept-inst[19] SKIP
            "            " v-dept-inst[20] SKIP
