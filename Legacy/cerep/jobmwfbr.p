/* cerep/jobkeyst.p   factory ticket  for folding , Keystone  */
/* -------------------------------------------------------------------------- */
...
DEF VAR v-last-order LIKE oe-ord.pord-no NO-UNDO.
...
v-fill = /*fill("=",128)*/ "<||6><C1><FROM><C82><LINE><||6>".
v-fill78 = /*fill("=",128)*/ "<||6><C1><FROM><C78><LINE><||6>".
v-fill2 = FILL("-",80).
...
format header
       "<FGCOLOR=RED><B>  S.O<FGCOLOR=BLACK>" v-job-no FORM "X(6)" /*space(0) "-" space(0) v-job-no2 format "99"*/        
       /*"<B><P12>F A C T O R Y   T I C K E T</B><P10>" at 32  "JOB START DATE:" at 85 v-start-date skip*/
...
{cerep/jobkeyst.i EXCLUSIVE-LOCK}
    ,
    EACH tt-reftable NO-LOCK
      ...
      if v-first then do:
        assign
         ...
         v-last-order = IF AVAIL oe-ord THEN oe-ord.pord-no ELSE 0   
      ...
      /*    if est.est-type eq 4 and eb.stock-no ne job-hdr.i-no then next ebloop. */
          ...
          for each job-mat
           ...
           /*        Fibre want to displ all inks more than once
                find first wrk-ink
...
/*
            ...
            IF FIRST-OF(eb.form-no) THEN
              PUT /*"<R-1>" "Case" AT 97 SKIP */
                "  <P9><B>F/B   FG ITEM #        ORDER QTY     JOB QTY  PO#            PART#           DESCRIPTION        STYLE  CARTON SIZE     COUNT CASE SIZE             CAD#            #UP   COLORS" /*v-upc-lbl*/ "</B>" SKIP
...
*/
            ...
            if avail oe-ordl then do:
              ...
              v-ovund = string("Overrun/Underrun %:  " +
                               trim(string(oe-ord.over-pct,">>9.99")) + "/" +
                               trim(string(oe-ord.under-pct,">>9.99"))).
            end.
           ...
           v-case-size = (IF vs-len <> "" THEN trim(vs-len) + "x" ELSE "") +
            ...
            FOR EACH bf-eb WHERE bf-eb.company = eb.company 
                             ...
                             AND bf-eb.form-no = eb.form-no NO-LOCK:
...
/*
            display " " trim(string(eb.form-no,">>9")) + "-" +
                    ...
                    /*PACE(4)*/
                    v-case-size  FORM "x(21)"
                    eb.cad-no
                    v-up
                 /*   eb.upc-no 
                    oe-ordl.price WHEN AVAIL oe-ordl AND s-prt-sellprc @ eb.upc-no
                 */   
                    SPACE(3)
                    v-alloc FORM "x(15)"
                    skip
...
*/
  ...
  /*           IF v-itm-printed < 4 THEN PUT SKIP(4 - v-itm-printed).*/
             ...
             v-over-pct = IF AVAIL oe-ord THEN oe-ord.over-pct ELSE 1.
             v-prt-up = v-tot-up * ef.n-out-l.
             IF AVAIL po-ord THEN
                 FIND FIRST po-ordl OF po-ord WHERE po-ordl.i-no = wrk-sheet.i-no /*ef.board*/  NO-LOCK NO-ERROR.
             ...
             v-cust-name            v-shipto[1] AT 35         /*  "<FGCOLOR=RED>OF<FGCOLOR=BLACK>" AT 79 */ SKIP
             v-cus[2]  v-shipto[2] AT 35                         SKIP
             v-cus[3]  v-shipto[3] AT 35                        "<FGCOLOR=RED>LAST SO.<FGCOLOR=BLACK>" AT 79 v-last-order FORM ">>>>>9" SKIP
             ...
             for each wrk-sheet break by wrk-sheet.form-no:
                 ...
                 assign v-size[1] = /*trim(vs-len) + " x " +*/ TRIM(vs-wid) + " x " + trim(vs-len).
                 RUN sys/inc/dec-frac.p (ef.nsh-len,64,OUTPUT vs-len).
                 RUN sys/inc/dec-frac.p (ef.nsh-wid,64,OUTPUT vs-wid).
                 IF substring(vs-len,1,1) = "-" THEN vs-len = SUBSTRING(vs-len,2).
                 IF substring(vs-wid,1,1) = "-" THEN vs-len = SUBSTRING(vs-wid,2).
                 assign v-size[2] = /*trim(vs-len) + " x " +*/ TRIM(vs-wid) + " x " + trim(vs-len).
               ...
               display "<FGCOLOR=RED>BOARD<FGCOLOR=BLACK>" wrk-sheet.brd-dscr 
                       ...
                       "  <FGCOLOR=RED>MILL<FGCOLOR=BLACK>" AT 115 tt-keyst.tt-mill FORM "x(20)" SKIP
                       "<FGCOLOR=RED>SHEET SIZE<FGCOLOR=BLACK>" v-size[1]  FORM "X(20)"
                       "<FGCOLOR=RED>CUT TO<FGCOLOR=BLACK>" AT 60
                    /*string(ef.nsh-wid) + "x" + string(ef.nsh-len)*/ v-size[2] FORM "x(20)"
                       /*"<FGCOLOR=RED>SHEETS<FGCOLOR=BLACK>" AT 115 v-sht-qty /*wrk-sheet.gsh-qty*/ */
                       "<FGCOLOR=RED><C51>PRINT SHEETS<C58><FGCOLOR=BLACK>" /*AT 110*/ v-prt-sht SKIP
             ...
             PUT SKIP(1)
                 "<FGCOLOR=RED>SKIDS REC'D ________________________________  SHEETS REC'D ________________________________<FGCOLOR=BLACK>" 
                 SKIP
                 v-fill78 SKIP.     
...
/*
             ...
             /** PRINT INK **/
             PUT "  <B>INK PASS           INK NAME                       LBS              INK PASS           INK NAME                       LBS </B>"
                 SKIP.
*/                 
             x = 2.
             i = 1.
             ...
             DO j = 1 TO 8:
                ...
                IF v-ink2[j] <> "" THEN do:
             /*      IF v-skip THEN PUT "  " v-ink2[j] FORM "x(65)"  SKIP.
                   ELSE PUT "  " v-ink2[j] FORM "x(65)" .
                   v-skip = NOT v-skip.*/
            ...
            /* ==============*/ 
              ...
              for each wrk-ink WHERE wrk-ink.form-no = eb.form-no 
                   ...
                   IF LAST-OF(wrk-ink.i-code) THEN DO:
                      PUT wrk-ink.i-code wrk-ink.i-dscr.
                      PUT SKIP.
                   END.
              END.
              */
              PUT "<FGCOLOR=RED><=5><C41> ORIG NEGATIVES<FGCOLOR=BLACK> " tt-keyst.tt-negs FORM "x(25)" SKIP
                   "<FGCOLOR=RED><C41> PLATES<FGCOLOR=BLACK> " tt-keyst.tt-plate2 FORM "x(20)" /*eb.plate-no*/ SKIP
                   "<FGCOLOR=RED><C41> DIES<FGCOLOR=BLACK> " tt-keyst.tt-die2 FORM "x(20)" /*eb.die-no*/ SKIP
                   "<FGCOLOR=RED><C41> COPY<FGCOLOR=BLACK> " tt-keyst.tt-COPY FORM "x(20)" SKIP
                   "<FGCOLOR=RED><C41> SIZE & STYLE<FGCOLOR=BLACK> " tt-keyst.tt-size FORM "x(20)" /* v-size[1] eb.style*/ SKIP(1)
                  "<FGCOLOR=RED><C41> PLATES<FGCOLOR=BLACK> " SKIP
                  "<C45>" tt-keyst.tt-plate[1] FORM "x(20)" SKIP
                  "<C45>" tt-keyst.tt-plate[2] FORM "x(20)" SKIP
                  "<FGCOLOR=RED><C41> DIES<FGCOLOR=BLACK>" SKIP
                  "<C45>" tt-keyst.tt-die[1] FORM "x(20)" SKIP  
                  "<C45>" tt-keyst.tt-die[2] FORM "x(20)"  SKIP
                  v-fill78  SKIP.
    ...
    "<=1>" .
                 ...
                 PUT "<#6>"
                     /*"QTY:  " v-job-qty FORM ">>>>,>>9" "   NO UP " v-up   "SIZE:  " AT 44 "<C39>" v-size[1] FORM "x(35)"  SKIP*/
                     "QTY: " v-job-qty FORM ">>>>,>>9" "   PRINT " v-prt-up FORM ">9" ", CUT " v-up FORM ">9" " UP" "<C38>" "SIZE: " /*AT 47 */ "<C43>" v-size[1] FORM "x(35)"  SKIP
                     "DESC:  " v-dsc[1] FORM "x(30)"  "<C38>" "STYLE:  " /*AT 40 */  v-stypart /*bf-eb.style*/ SKIP
                     v-dsc[2] AT 7    FORM "x(30)"  "<C45>" tt-key2.tt-style[1] /* AT 46 */ FORM "x(20)" SKIP
                     v-dsc[3] AT 7    FORM "x(30)"  "<C45>" tt-key2.tt-style[2] /*AT 46*/ FORM "x(20)" SKIP
                     "PLATE#:  " bf-eb.plate-no "<C38>" "DIE#:  " /*AT 40*/  "<C43>" bf-eb.die-no SKIP
                     "CUSTOMER PART#:  " bf-eb.part-no  SKIP
                     "PO:  " v-po-no       "<C38>" "PRTD:  " bf-eb.i-coldscr SKIP
                     "DEL:  " v-del-date      SKIP
                     "SHIP TO:  " v-shipto[1] 
                     /*"PACK:" AT 40 tt-keyst.tt-tray-qty FORM ">>>>>>>>9"
                     "PER TRAY:" AT 58 tt-keyst.tt-tray-desc FORM "x(20)" SKIP*/ SKIP
                     v-shipto[2] AT 8 
                     "<C38>" "PACK:  "  bf-eb.cas-cnt "        PER CASE:  " bf-eb.cas-no  SKIP
                   /*  "AQL:"  tt-keyst.tt-aql FORM "x(20)" 
                      "SAMPLES/CASE:" AT 20 */ 
                      v-shipto[3] AT 8 "<C38>AQL:  "  tt-key2.tt-aql FORM "x(20)" SKIP
                      v-shipto[4] AT 8 "<C38>GLUING PICK-UP COUNT:  " tt-key2.tt-glue-cnt FORM "x(20)" 
                     SKIP
