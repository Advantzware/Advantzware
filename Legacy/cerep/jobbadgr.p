/* -------------------------------------------------------- cerep/jobbadgr.p  */
/*  factory ticket  for folding , Badger                                      */
/* -------------------------------------------------------------------------- */
...
for each job-hdr NO-LOCK
        ...
        /*  if last-of(eb.form-no) then do: */
          ...
          IF LAST-OF(eb.form-no) THEN DO:
            ...
            /** PRINT SHEET **/
             ...
             /** PRINT INK **/
             PUT "<B>PASS INK             INK NAME            ITEMS      PASS INK             INK NAME            ITEMS       PLATE #</B>"
                 SKIP.
                ...
                IF LAST-OF(wrk-ink.i-code) THEN DO:
                    ...
                    IF wrk-ink.i-pass = 1 THEN
                       ASSIGN v-ink1[i] = (IF v-pass-count = 1 THEN "F    " ELSE "B    ") +
                              ...
                              i = i + 1         . 
                    ELSE IF wrk-ink.i-pass = 2 THEN
                       ASSIGN v-ink2[i] = "F    " + string(wrk-ink.i-code,"X(15)") + " " + 
                                   string(wrk-ink.i-dscr,"x(20)") + " " + trim(v-alloc) /*v-item[i]*/
                              i = i + 1.           
                END.
    ...
    /*  IF v-ship <> "" THEN v-dept-inst[6] = v-ship.  /* shipto notes */ */
        ...
        IF v-dept-inst[5] <> "" THEN PUT
            v-dept-inst[5] FORM "x(128)" SKIP .
        IF v-dept-inst[6] <> "" THEN PUT
            v-dept-inst[6] FORM "x(128)" SKIP .
        IF v-dept-inst[7] <> "" THEN PUT
            v-dept-inst[7] FORM "x(128)" SKIP .
        IF v-dept-inst[8] <> "" THEN PUT
            v-dept-inst[8] FORM "x(128)" SKIP .
        IF v-dept-inst[9] <> "" THEN PUT
            v-dept-inst[9] FORM "x(128)" SKIP .
        IF v-dept-inst[10] <> "" THEN PUT
