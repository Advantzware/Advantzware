...
/* 	PRINTING NOTES	*/
        ...
        RUN PRprintfg.        
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"PR",job-hdr.frm),80).
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"PRT",job-hdr.frm),80).
         DO i = 1 TO NUM-ENTRIES(v-text,"`"):       
            PUT "<C1>" 
                ENTRY(i,v-text,"`") FORMAT "X(80)" SKIP. 
         END.          
...
/* DIE CUTTING */
        ...
        RUN PRpage (2).   
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"dc",job-hdr.frm),80).
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"cc",job-hdr.frm),80).
         DO i = 1 TO NUM-ENTRIES(v-text,"`"):     
...
/* 	WINDOWING */
        ...
        PUT v-thick 
            ...
            "<C1>Window Checklist Complete : <FROM><R+1><C+2><RECT><||3>" SKIP                     
        ...
        FOR EACH ef NO-LOCK 
                    WHERE ef.company = job-hdr.company
                      ...
                      AND ef.form-no = job-hdr.frm,          
           FIRST item FIELDS (i-no i-name) NO-LOCK
                      WHERE item.company = ef.company
                        AND item.i-no    = ef.board
                  BREAK BY item.i-no:
           IF FIRST-OF(item.i-no) THEN DO:
              FOR FIRST eb FIELDS (len wid dep) NO-LOCK
              ...
              PUT "<C1>"              
                  ...
                  eb.len                  
                  SKIP .
           END. /* IF FIRST-OF(item.i-no) THEN DO: */                            
        ...
        ASSIGN v-text = FNformat(FNnotes(vjobreckey,"WN",job-hdr.frm),80).        
        PUT " " SKIP.        
        ...
        PUT " " SKIP.
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"WD",job-hdr.frm),80).
         DO i = 1 TO NUM-ENTRIES(v-text,"`"):       
            PUT "<C1>" 
                ENTRY(i,v-text,"`") FORMAT "X(80)"  SKIP. 
         END. 
...
/* 	FINISHING	*/
        ...
        PUT v-thick  SKIP  /*RDR*/
            "<C1><B>FINISHING</B>"    SKIP          /*RDR*/      
            "<C1>Finishing Checklist Complete : <FROM><R+1><C+2><RECT><||3> "  SKIP                        
            "<C1><u>Corrugated</u>"   SKIP           /*RDR*/            
            "<C1><u>RM Item #</u> <u>Name                          </u> <u>Length </u> <u>Width  </u> <u>Depth  </u> <u>No. of Cases</u>"  SKIP.
...
*/
        ...
        PUT "<C1><u>Pallet</u>"  SKIP.              /*RDR*/ 
        PUT "<C1><u>RM Item #</u> <u>Name                          </u> <u>Length  </u> <u>Width </u>" SKIP.
...
*/ 
        RUN PRprintfg.             
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"GL",job-hdr.frm),80)
              /*  r = r + 1 */.
         ...
         ASSIGN v-text = FNformat(FNnotes(vjobreckey,"PK",job-hdr.frm),80).
         DO i = 1 TO NUM-ENTRIES(v-text,"`"):        
            PUT "<C1>" ENTRY(i,v-text,"`") FORMAT "X(80)" SKIP. 
         END.          
...
/* 	OTHER NOTES	*/
        ...
        PUT "<C1><FROM><C105><LINE><|3>"  SKIP  /*RDR*/
            "<C1><B>Other Notes</B>"  SKIP            /*RDR*/ 
            "<C1>Other Notes Checklist Complete : <FROM><R+1><C+2><RECT><||3> "  SKIP.
        ...
        DO i = 1 TO NUM-ENTRIES(spec-list).
           IF NOT CAN-DO("PR,PRT,DC,CC,WN,WD,GL,PK",ENTRY(i,spec-list)) THEN
              ASSIGN vlist = vlist + (IF vlist = "" THEN "" ELSE ",") +
                             ENTRY(i,spec-list).
        END. 
        ASSIGN v-text = FNformat(FNnotes(vjobreckey,vlist,job-hdr.frm),80).
        ...
        FOR EACH tt-fgitm BY tt-fgitm.seq.
          ...
          IF i >= 3 THEN DO:
             display v-fill skip
               ...
               "<B>FG Item #:</B>" WHEN v-fgitm[3] <> "" AT 101 v-fgitm[3] WHEN v-fgitm[3] <> "" SKIP 
               "Shipto   :" v-shipto[1]                             
        ...
        PUT " " SKIP.
        PUT UNFORMATTED "<#=100><AT=,1><FROM><AT=+.6,+2><BARCODE,TYPE=39,CHECKSUM=NONE,VALUE="
              v-fgitm[1] ">"
              "<AT=-.6,4.6><FROM><AT=+.6,+2><BARCODE,TYPE=39,CHECKSUM=NONE,VALUE="
              v-fgitm[2] ">"
              "<AT=-.6,8.2><FROM><AT=+.6,+2><BARCODE,TYPE=39,CHECKSUM=NONE,VALUE="
              v-fgitm[3] ">"  
              "<AT=,1>" v-fgitm[1] 
              "<AT=,4.6>" v-fgitm[2]
              "<AT=,8.2>" v-fgitm[3]                            
        ...
        PUT " " SKIP.
        PUT UNFORMATTED "<#=200><AT=,1><FROM><AT=+.6,+2><BARCODE,TYPE=39,CHECKSUM=NONE,VALUE=" v-fgitm[1] ">".
        IF v-fgitm[2] <> "" THEN
           PUT UNFORMATTED "<=200><AT=-.6,4.6><FROM><AT=+.6,+2><BARCODE,TYPE=39,CHECKSUM=NONE,VALUE=" v-fgitm[2] ">".  
        IF v-fgitm[3] <> "" THEN
           PUT UNFORMATTED "<=200><AT=-.6,8.2><FROM><AT=+.6,+2><BARCODE,TYPE=39,CHECKSUM=NONE,VALUE=" v-fgitm[3] ">".  
        
        PUT UNFORMATTED "<=200><AT=,1>" v-fgitm[1]. 
        IF v-fgitm[2] <> "" THEN
           PUT UNFORMATTED "<=200><AT=,4.6>" v-fgitm[2].
        IF v-fgitm[3] <> ""  THEN
           PUT UNFORMATTED "<=200><AT=,8.2>" v-fgitm[3].                                                   
