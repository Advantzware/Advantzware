...
DEF VAR v-flute AS CHAR NO-UNDO.
...
view frame head.
      ...
      for each ef
        ...
        /*  if last-of(eb.form-no) then do: */
            ...
            DO:
              PUT "<P9><B>F/B   FG Item #         Order Qty   Job Qty  PO#            Description      Style  CAD#         #UP Case Code  Count Layers  Stacks /Pallet</B>" SKIP.
              v-total-up = 0.
            END.
           ...
           find first oe-ordl
            ...
            IF eb.layer-pad <> "" THEN DO: /* Task # 10121106 */
                PUT SPACE(101) eb.layer-pad  eb.lp-up FORMAT ">>>>>9"
                    SKIP.
                v-itm-printed = v-itm-printed + 1.
            END.

            IF eb.divider <> "" THEN DO:  /* Task # 10121106 */
                PUT SPACE(101) eb.divider  eb.div-up FORMAT ">>>>>9"
                    SKIP.
                 v-itm-printed = v-itm-printed + 1.
            END.
          ...
          IF LAST-OF(eb.form-no) THEN DO:
/*              IF v-itm-printed < 4 THEN PUT SKIP(4 - v-itm-printed). */ /* Task # 10121106 */
             ...
             PUT "<P10>" v-fill SKIP                       /*REQ'D*/
                   "<B>BOARD CODE            VENDOR     SHEETS        SHEET SIZE    DIE#         Flute       #UP BOARD PO# QTY RECEIVED</B>" 
                 SKIP.
            /** PRINT SHEET **/
             x = 2.
            
             for each wrk-sheet WHERE wrk-sheet.i-no = ef.board  AND wrk-sheet.form-no = ef.form-no :
               ...
               v-dept-title = IF AVAIL notes THEN notes.note_title ELSE "".
                 IF ITEM.mat-type = "P" THEN
                     ASSIGN wrk-sheet.gsh-qty = 0 .

                 IF ef.medium <> "" OR ef.flute <> "" THEN DO:
                 IF ef.trim-pen = 0  THEN ASSIGN v-flute = "N" .
                 ELSE IF ef.trim-pen = 52  THEN ASSIGN v-flute = "F" .
                 ELSE IF ef.trim-pen = 78  THEN ASSIGN v-flute = "E" .
                 ELSE IF  ef.trim-pen = 125  THEN ASSIGN v-flute = "B" .
                 ELSE IF ef.trim-pen = 100 THEN ASSIGN v-flute = "C".
                 END.
                 ELSE v-flute = "" .
                  
               DISPLAY wrk-sheet.i-no FORM "x(20)"
             ...
             for each wrk-sheet WHERE wrk-sheet.form-no = ef.form-no AND wrk-sheet.i-no <> ef.board  :
               find first ITEM where item.company eq cocode
                                 and item.i-no    eq wrk-sheet.i-no no-lock no-error.
               /*v-lbs = wrk-sheet.gsh-qty * (wrk-sheet.sh-wid * wrk-sheet.sh-len / 144) / 1000 * ITEM.basis-w.*/
               FIND first notes WHERE notes.rec_key = job.rec_key AND
      ...
      if last-of(tt-reftable.val[12]) then do:
        ...
        FOR EACH notes WHERE notes.rec_key = job.rec_key
                    AND (notes.note_form_no = tt-reftable.val[12] OR notes.note_form_no = 0)
                    AND LOOKUP(notes.note_code,v-exc-depts) EQ 0  NO-LOCK
                    BREAK BY notes.note_code BY notes.note_form_no:
               lv-text = "".
               FOR EACH tt-formtext:
                   DELETE tt-formtext.
               END.
    ...
    /*  IF v-ship <> "" THEN v-dept-inst[6] = v-ship.  /* shipto notes */ */
        ...
        IF AVAIL ef THEN DO:
           IF PAGE-SIZE - LINE-COUNTER < 2 THEN PAGE.
           PUT v-fill skip
               "<B>                             Miscellaneous - SubContract                                  Prep</B>"
               SKIP.
       ...
       FIND first ef WHERE ef.company EQ job-hdr.company
          ...
          j = 1.
           ...
           END.
            FIND FIRST eb WHERE eb.company EQ ef.company
                ...
                AND eb.form-no = ef.form-no NO-LOCK NO-ERROR.
            IF AVAIL eb  THEN do:
                PUT v-fill skip.
                IF PAGE-SIZE - LINE-COUNTER < 8 THEN PAGE .
             
           PUT "<B>Special Materials " AT 3  "<B>PALLET BANDING INFORMATION </B> " AT 45 SKIP .
          
            PUT    v-spec-no[1]  AT 3  "<B>Total Strapping Bands:</B> " AT 42  string(eb.spare-int-1) FORMAT "x(25)"   SKIP
                   v-spec-no[2]  AT 3  "<B>Stretch Wrap?:</B> " AT 42  (IF eb.spare-int-2 EQ 1 THEN "YES" ELSE "NO") FORMAT "x(25)" SKIP
                   v-spec-no[3]  AT 3  "<B>Stack Pattern?:</B> " AT 42  eb.stack-code FORMAT "x(25)" SKIP
                   v-spec-no[4]  AT 3  "<B>Banding Tool?:</B> " AT 42   eb.spare-char-5  FORMAT "x(25)"
                   v-spec-no[5]  AT 3
                   v-spec-no[6]  AT 3
                   v-spec-no[7]  AT 3
                   v-spec-no[8]  AT 3
        ...
        IF print-box THEN
        IF AVAIL ef THEN DO:
           IF ef.cad-image NE "" THEN DO: 
               FIND FIRST eb WHERE eb.est-no = ef.est-no 
                   AND eb.company = ef.company 
                   AND eb.form-no = ef.form-no NO-LOCK NO-ERROR.
               IF AVAIL eb THEN
                   ASSIGN vdie = eb.die-no .
               ELSE 
                   vdie = "" .

                   PAGE.
                   ls-fgitem-img = ef.cad-image .
                   
                   PUT UNFORMATTED "<#12><C1><FROM><C106><R+47><RECT><||3><C80>" /*v-qa-text*/ SKIP
                       "<=12><R+1><C5>Die: "  vdie FORMAT "x(30)" " "
