...
PROCEDURE Set-g_rec_key:
  ...
  CASE prgmxref.table_name:
    WHEN "deptcode" THEN
    DO:
      FIND deptcode WHERE deptcode.rec_key = notes.rec_key NO-LOCK.
    ...
    WHEN "deptoga" THEN
    DO:
      FIND deptogaWHERE deptoga.rec_key = notes.rec_key NO-LOCK.
    ...
    WHEN "elements" THEN
    DO:
      FIND elements WHERE elements.rec_key = notes.rec_key NO-LOCK NO-ERROR.
    ...
    WHEN "recordid" THEN
    DO:
      FIND recordid WHERE recordid.rec_key = notes.rec_key NO-LOCK NO-ERROR.
      IF NOT AVAILABLE recordid THEN
      RETURN.
      FIND applctns OF recordid NO-LOCK NO-ERROR.
      IF NOT AVAILABLE applctns THEN
