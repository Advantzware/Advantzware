...
&SCOPED-DEFINE dataGridInclude dataGrid\gl\b-glrun.i
...
&Scoped-define INTERNAL-TABLES tt-glhist report
...
&Scoped-define QUERY-STRING-br_table FOR EACH tt-glhist, ~
                                   FIRST report                             WHERE report.rec_key EQ tt-glhist.rec_key                               AND report.term-id EQ v-term                               AND report.rec-id  EQ RECID(tt-glhist)                             OUTER-JOIN USE-INDEX rec_key                             BY tt-glhist.actnum
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH tt-glhist, ~
                                   FIRST report                             WHERE report.rec_key EQ tt-glhist.rec_key                               AND report.term-id EQ v-term                               AND report.rec-id  EQ RECID(tt-glhist)                             OUTER-JOIN USE-INDEX rec_key                             BY tt-glhist.actnum.
&Scoped-define TABLES-IN-QUERY-br_table tt-glhist report
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tt-glhist
&Scoped-define SECOND-TABLE-IN-QUERY-br_table report
...
OPEN QUERY {&SELF-NAME} FOR EACH tt-glhist,
                            FIRST report
                            WHERE report.rec_key EQ tt-glhist.rec_key
                              AND report.term-id EQ v-term
                              AND report.rec-id  EQ RECID(tt-glhist)
                            OUTER-JOIN USE-INDEX rec_key
                            BY tt-glhist.actnum.
     _END_FREEFORM
  ...
  FOR EACH tt-glhist WHERE tt-glhist.tr-dscr MATCHES "*Inv# *":
    ...
    IF li-inv-no NE 0                              AND
       CAN-FIND(FIRST ar-inv
                WHERE ar-inv.company EQ lv-company
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tt-glhist"}
