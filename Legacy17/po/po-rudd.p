...
form po-ordl.ord-qty        to 6    format ">>>>>9"
     ...
     po-ordl.vend-i-no      at 42
     po-ordl.cost           at 59   format ">>,>>9.999"
...
output stream last-page to value("po-rudd.txt") page-size VALUE(v-lines-per-page).
    ...
    FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
      ...
      if first(po-ord.po-no) then
        form header
             ...
             "PURCHASE ORDER"                   at 40
             po-ord.po-change-date              to 66   FORMAT "99/99/99"
             po-ord.po-no                       to 77
             skip(7)
             "ORDER FROM:"                      at 2
             "SHIP TO:"                         at 47 skip(1)
             vend.name                          at 3
             v-sname                            at 48
             vend.add1                          at 3
             v-saddr[1]                         at 48
             ...
             string(trim(vend.city)  +
                    ...
                    trim(vend.state) +
                    "  "             +
                    trim(vend.zip))             at 3   format "x(30)"
             string(trim(v-scity)  +
                    ", "           +
                    trim(v-sstate) +
                    "  "           +
                    trim(v-szip))               at 48   format "x(30)"
             skip(2)
             "QTY"                              to 5
             "TEST"                             at 8
             "ADDER"                            at 25
