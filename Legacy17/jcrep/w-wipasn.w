...
&SCOPED-DEFINE h_Browse01 h_b-wiptga
...
&Scoped-Define ENABLED-OBJECTS btPost 
...
DEFINE VARIABLE h_b-wipmch AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-wiptga AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     btPost AT ROW 5.05 COL 3 WIDGET-ID 4
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse WIP|WIP History' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'jcrep/b-wiptga.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-wiptga ).
       RUN set-position IN h_b-wiptga ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-wiptga ( 17.14 , 146.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav-2 ).
       RUN set-position IN h_p-updsav-2 ( 22.19 , 47.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav-2 ( 1.76 , 56.00 ) NO-ERROR.

       /* Links to SmartNavBrowser h_b-wiptga. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav-2 , 'TableIO':U , h_b-wiptga ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'addline':U , h_b-wiptga ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-wiptga ,
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'jcrep/b-wipmch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-wipmch ).
       RUN set-position IN h_b-wipmch ( 6.48 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-wipmch ( 17.62 , 145.00 ) NO-ERROR.

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-wipmch ,
             btPost:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 2 */
...
DEF VAR adm-current-page AS INT.
  /* Code placed here will execute PRIOR to standard behavior. */
  IF VALID-HANDLE(h_b-wiptga) THEN
      RUN get-tag-no IN h_b-wiptga (OUTPUT v-tag-no).
  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'change-page':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
  IF VALID-HANDLE(h_b-wipmch) THEN
    RUN set-tag-no IN h_b-wipmch (INPUT v-tag-no).
  IF VALID-HANDLE(h_b-wipmch) THEN
    RUN adm-open-query IN h_b-wipmch.
  RUN get-attribute IN THIS-PROCEDURE ('Current-Page':U).
  ASSIGN adm-current-page = INTEGER(RETURN-VALUE).
  IF adm-current-page = 1 THEN
      btPost:VISIBLE IN FRAME f-main = YES.
  ELSE 
      btPost:VISIBLE IN FRAME f-main = NO.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE set-tag-no W-Win 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER ip-tag-no LIKE wiptag.tag-no.
RUN set-tag-no IN h_b-wipmch (INPUT ip-tag-no).
END PROCEDURE.
