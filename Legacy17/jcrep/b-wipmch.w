...
{Advantzware\WinKit\admBrowserUsing.i} /* added by script _admBrowsers.p on 04.07.2017 @  2:08:01 pm */
...
&Scoped-define INTERNAL-TABLES wiptag-mch wiptag item mach dept

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE Browser-Table                                 */
&Scoped-define FIELDS-IN-QUERY-Browser-Table wiptag.tag-no wiptag-mch.m-code dept.dscr wiptag.wip-warehouse wiptag.wip-rm-bin wiptag.job-no wiptag.job-no2 wiptag-mch.produced-qty dept.seq   
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table   
&Scoped-define SELF-NAME Browser-Table
&Scoped-define QUERY-STRING-Browser-Table FOR EACH wiptag-mch WHERE wiptag-mch.company =      cocode     AND wiptag-mch.tag-no = v-tag-no NO-LOCK, ~
           FIRST wiptag WHERE wiptag.company = cocode                    AND wiptag.tag-no = wiptag-mch.tag-no                  NO-LOCK, ~
             EACH item WHERE TRUE /* Join to wiptag incomplete */       AND item.company = cocode  AND item.i-no = wiptag.rm-i-no NO-LOCK, ~
             EACH mach WHERE TRUE /* Join to wiptag-mch incomplete */       AND mach.company = cocode and asi.mach.m-code = wiptag-mch.m-code NO-LOCK, ~
             EACH dept WHERE TRUE /* Join to mach incomplete */       AND dept.code = mach.dept[1] NO-LOCK     ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY {&SELF-NAME} FOR EACH wiptag-mch WHERE wiptag-mch.company =      cocode     AND wiptag-mch.tag-no = v-tag-no NO-LOCK, ~
           FIRST wiptag WHERE wiptag.company = cocode                    AND wiptag.tag-no = wiptag-mch.tag-no                  NO-LOCK, ~
             EACH item WHERE TRUE /* Join to wiptag incomplete */       AND item.company = cocode  AND item.i-no = wiptag.rm-i-no NO-LOCK, ~
             EACH mach WHERE TRUE /* Join to wiptag-mch incomplete */       AND mach.company = cocode and asi.mach.m-code = wiptag-mch.m-code NO-LOCK, ~
             EACH dept WHERE TRUE /* Join to mach incomplete */       AND dept.code = mach.dept[1] NO-LOCK     ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table wiptag-mch wiptag item mach ~
dept
&Scoped-define FIRST-TABLE-IN-QUERY-Browser-Table wiptag-mch
&Scoped-define SECOND-TABLE-IN-QUERY-Browser-Table wiptag
&Scoped-define THIRD-TABLE-IN-QUERY-Browser-Table item
&Scoped-define FOURTH-TABLE-IN-QUERY-Browser-Table mach
&Scoped-define FIFTH-TABLE-IN-QUERY-Browser-Table dept
...
DEFINE VARIABLE fi_sortby AS CHARACTER FORMAT "X(256)":U INITIAL "Machine Dept" 
     LABEL "Sorted By" 
     VIEW-AS FILL-IN 
     SIZE 22 BY 1
...
/* Query rebuild information for BROWSE Browser-Table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH wiptag-mch WHERE wiptag-mch.company =
     cocode
    AND wiptag-mch.tag-no = v-tag-no NO-LOCK,
    FIRST wiptag WHERE wiptag.company = cocode
                   AND wiptag.tag-no = wiptag-mch.tag-no
                 NO-LOCK,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE set-tag-no B-table-Win 
PROCEDURE set-tag-no :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER ip-tag-no LIKE wiptag.tag-no NO-UNDO.
v-tag-no = ip-tag-no.
RUN adm-open-query.
RUN adm-initialize.
END PROCEDURE.
