...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "GL Acct#,Description,Customer,Cust Name,Inv#,PO#,Customer Lot#,Item#,Item Desc,Run#,Date,Amount" .
...
DEFINE FRAME FRAME-A
     ...
     begin_date AT ROW 4.71 COL 19 COLON-ALIGNED HELP
          "Enter Beginning Vendor Number"
     end_date AT ROW 4.71 COL 63 COLON-ALIGNED HELP
          "Enter Ending Vendor Number"
     begin_acct AT ROW 6.14 COL 19 COLON-ALIGNED HELP
          "Enter Beginning GL Account Number"
     end_acct AT ROW 6.14 COL 63 COLON-ALIGNED HELP
     ...
     btn_down AT ROW 12.91 COL 40.2 WIDGET-ID 42
     rd-dest AT ROW 15.52 COL 6 NO-LABEL
     lv-ornt AT ROW 15.76 COL 30 NO-LABEL
     lines-per-page AT ROW 16 COL 83 COLON-ALIGNED
     lv-font-no AT ROW 17.43 COL 33 COLON-ALIGNED
     lv-font-name AT ROW 18.38 COL 27 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 19.33 COL 29
     tb_excel AT ROW 20.38 COL 49 RIGHT-ALIGNED
     tb_runExcel AT ROW 20.38 COL 51
     fi_file AT ROW 21.33 COL 27 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 22.91 COL 19
     btn-cancel AT ROW 22.91 COL 58
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 8.14 COL 5 WIDGET-ID 38
     ...
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 8.14 COL 59.6 WIDGET-ID 44
...
FOR EACH tt-report
 ...
 /* IF tb_excel THEN
  ...
  ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "act"                   THEN cVarValue = (IF FIRST-OF(tt-report.actnum) THEN string(tt-report.actnum) ELSE "") .
                         ...
                         WHEN "cust-name"        THEN cVarValue = IF FIRST-OF(tt-report.cust-no) AND AVAIL(cust) THEN cust.NAME ELSE "".
                         WHEN "inv"                     THEN cVarValue = STRING(tt-report.inv-no) .
                         WHEN "po"                 THEN cVarValue = STRING(tt-report.po-no) .
                         WHEN "cust-lot"            THEN cVarValue = STRING(tt-report.lot-no) .
                         WHEN "itm"                     THEN cVarValue = STRING(tt-report.i-no) .
                         WHEN "itm-dscr"            THEN cVarValue = STRING(tt-report.i-name) .
  ...
  IF LAST-OF(tt-report.cust-no) THEN DO:
    PUT SKIP(1).
 ...
 PUT SKIP str-line SKIP .
  ASSIGN cDisplay = ""
            ...
            PUT UNFORMATTED "   Acct# Totals" substring(cDisplay,16,300) SKIP.
