...
DEFINE INPUT  PARAMETER ip-note-code AS CHARACTER   NO-UNDO.
DEFINE INPUT PARAMETER ip-note-type AS CHARACTER NO-UNDO.
&ELSE
DEFINE VARIABLE ip-rec_key AS CHARACTER NO-UNDO.
DEFINE VARIABLE ip-header AS CHARACTER NO-UNDO.
DEFINE VARIABLE ip-note-code AS CHARACTER   NO-UNDO.
DEFINE VARIABLE ip-note-type AS CHARACTER NO-UNDO.
...
DEFINE VARIABLE cSaveRecKey AS CHARACTER   NO-UNDO.
...
DEFINE VARIABLE h_datenote AS HANDLE NO-UNDO.
DEFINE VARIABLE h_datenote-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Date Change Notes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Date Change Notes */
DO:
...
IF ERROR-STATUS:ERROR THEN do:
   RETURN ERROR.
END.
IF INDEX(ip-header, "oeitem") GT 0 THEN
  ip-rec_key = cSaveRecKey.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/datenote.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_datenote-2 ).
       RUN set-position IN h_datenote-2 ( 5.05 , 6.00 ) NO-ERROR.
       RUN set-size IN h_datenote-2 ( 18.10 , 138.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_datenote-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_datenote-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_datenote-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/datenote.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_datenote ).
       RUN set-position IN h_datenote ( 5.05 , 9.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_datenote. */
       RUN add-link IN adm-broker-hdl ( h_datenote-2 , 'Record':U , h_datenote ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_datenote ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_datenote ,
             h_folder , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE OUTPUT PARAMETER op-header AS CHAR NO-UNDO.
  DEFINE OUTPUT PARAMETER op-notecode AS CHARACTER   NO-UNDO.
  DEFINE OUTPUT PARAMETER op-notetype AS CHARACTER   NO-UNDO.
  /* This is called from browse. Indicates whether to find records */
  /* related to the item tab - Promise Date Change, or from the */
  /* release tab - Release Date Change */
  ip-rec_key = cSaveRecKey.
  op-rec_key = ip-rec_key.
  op-header = ip-header.
  op-notecode = ip-note-code.
  op-notetype = ip-note-type.
...
------------------------------------------------------------------------------*/
  ...
  RUN setNoteType IN h_datenote (INPUT ip-note-code, INPUT ip-note-type).
...
ELSE DO:
  FIND FIRST oe-rel WHERE oe-rel.rec_key = ip-rec_key NO-LOCK NO-ERROR.
  IF AVAIL oe-rel THEN DO:
      FIND FIRST itemfg WHERE itemfg.i-no = oe-rel.i-no NO-LOCK NO-ERROR.
  ...
  ELSE DO:
    ...
    IF AVAIL oe-ordl THEN DO:
        FIND FIRST itemfg WHERE itemfg.i-no = oe-ordl.i-no NO-LOCK NO-ERROR.
        IF AVAIL itemfg THEN
                ASSIGN
       lbl_i-no = "     FG Item#"
       fi_i-no  = itemfg.i-no
  ...
  IF NOT AVAIL itemfg THEN DO:
    IF CAN-FIND(FIRST oe-relh WHERE oe-relh.rec_key = ip-rec_key)  THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE send-title W-Win 
PROCEDURE send-title :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER ipNoteRow AS ROWID.
RUN get-title IN h_datenote /* (INPUT ipNoteRow) */.
END PROCEDURE.
