...
DEFINE VARIABLE h_notes-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/notes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_notes ).
       RUN set-position IN h_notes ( 4.81 , 7.00 ) NO-ERROR.
       RUN set-size IN h_notes ( 18.10 , 138.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2') NO-ERROR.

       /* Links to SmartNavBrowser h_notes. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_notes ).
       RUN add-link IN adm-broker-hdl ( h_notes , 'Record':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/notes.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_notes-2 ).
       RUN set-position IN h_notes-2 ( 6.24 , 16.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_notes-2. */
       RUN add-link IN adm-broker-hdl ( h_notes , 'Record':U , h_notes-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_notes-2 ).
