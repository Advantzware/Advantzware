...
&SCOPED-DEFINE h_Browse01 h_surcharg
...
DEFINE VARIABLE h_expsurchrg AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_surcharg AS HANDLE NO-UNDO.
DEFINE VARIABLE h_surcharg-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Surcharge */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Surcharge */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws SurCharg|View Surcharg' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/surcharg.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_surcharg ).
       RUN set-position IN h_surcharg ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_surcharg ( 19.52 , 114.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_surcharg. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_surcharg ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_surcharg ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/surcharg.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_surcharg-2 ).
       RUN set-position IN h_surcharg-2 ( 6.71 , 23.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.00 , 99.00 ) */
       ...
       /* Links to SmartViewer h_surcharg-2. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_surcharg-2 ).
       RUN add-link IN adm-broker-hdl ( h_surcharg , 'Record':U , h_surcharg-2 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_surcharg-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_surcharg-2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_surcharg-2 , 'AFTER':U ).
