...
def var v-codelst as cha init "B,C,D,F,P,R,W" no-undo.
def var v-dscrlst as cha init "Boxes,Clear Poly Bags,Die Cuts,Foam,Sheet Fed Partitions,Roll Fed Partitions,Wood" no-undo.
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 tt-table.code tt-table.name   
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Due Code Information */
DO:
...
DO:
 /*  op-char-val = tt-table.code:screen-value in browse {&browse-name} + "," +
                 tt-table.name:screen-value in browse {&browse-name} .
...
------------------------------------------------------------------------------*/
  ...
  do i = 1 to num-entries(v-codelst):
    create tt-table.
    assign
     tt-table.code = entry(i,v-codelst)
     tt-table.name = entry(i,v-dscrlst).
  end.
END PROCEDURE.
