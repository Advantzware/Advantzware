    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_e-item ).
       RUN set-position IN h_e-item ( 1.00 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updcan ).
       RUN set-position IN h_p-updcan ( 19.33 , 29.00 ) NO-ERROR.
       RUN set-size IN h_p-updcan ( 1.76 , 31.00 ) NO-ERROR.

       /* Links to SmartViewer h_e-item. */
       RUN add-link IN adm-broker-hdl ( h_p-updcan , 'TableIO':U , h_e-item ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_e-item ).
       RUN add-link IN adm-broker-hdl ( h_e-item , 'new-record':U , THIS-PROCEDURE ).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  /*needed for interaction with e-item.w */
