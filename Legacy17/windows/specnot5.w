...
DEFINE VARIABLE h_specnot5 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_specnot5-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/specnot5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_specnot5-2 ).
       RUN set-position IN h_specnot5-2 ( 5.05 , 6.00 ) NO-ERROR.
       RUN set-size IN h_specnot5-2 ( 18.10 , 138.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_specnot5-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_specnot5-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnot5-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/specnot5.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_specnot5 ).
       RUN set-position IN h_specnot5 ( 5.05 , 9.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_specnot5. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_specnot5 ).
       RUN add-link IN adm-broker-hdl ( h_specnot5-2 , 'Record':U , h_specnot5 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnot5 ,
             h_folder , 'AFTER':U ).
...
RUN get-title IN h_specnot5 /* (INPUT ipNoteRow) */.
END PROCEDURE.
