...
{custom/windows.i}
DEF VAR  op-printer-ports AS cha NO-UNDO.
DEF var  op-printer-names AS cha NO-UNDO.
...
DEFINE VARIABLE lpPortName    AS MEMPTR  NO-UNDO.
  ...
  /* The first call to EnumPrinters is only to 
   ...
   IF pcbNeeded=0 THEN DO:
      MESSAGE "No printers found" VIEW-AS ALERT-BOX.
      RUN DeAlloc.
      RETURN.
   END.
 ...
 /* end of third running -========*/
   ...
   /* The third call actually fills the pPrinterEnum structure */ 
   RUN EnumPrinters{&A} IN hpApi(5,  /* = PRINTER_ENUM_LOCAL */
                                 "", 
                                 2,
                                 GET-POINTER-VALUE (pPrinterEnum),
                                 GET-SIZE(pPrinterEnum), 
   ...
   END.
                  ...
                  MESSAGE op-printer-ports SKIP
                          op-printer-names VIEW-AS ALERT-BOX.

                  DO i = 1 TO NUM-ENTRIES(op-printer-ports):
                      MESSAGE i ENTRY(i,op-printer-ports)
                          " name:" ENTRY(i,op-printer-names)
                          VIEW-AS ALERT-BOX.
                  END.
