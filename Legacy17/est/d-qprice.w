...
/* Returns a quantity per order selected from history button */
DEF OUTPUT PARAMETER io-qty-list AS CHAR NO-UNDO.
DEF OUTPUT PARAMETER io-price-list AS CHAR NO-UNDO.
DEF OUTPUT PARAMETER io-uom-list AS CHAR NO-UNDO.
...
DEFINE VARIABLE h_b-qprice-2 AS HANDLE NO-UNDO.
...
DEFINE FRAME D-Dialog
     btn_select AT ROW 19.81 COL 65
     SPACE(66.59) SKIP(1.03)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Quote Item Selection for Customer:".
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Quote Item Selection for Customer: */
DO:
...
DO:
  ...
  DO li = 1 TO NUM-ENTRIES(io-rowid-list):
      ...
      IF VALID-HANDLE(ip-handle) THEN DO:
        RUN setQtyPriceFromRow IN WIDGET-HANDLE(char-hdl) (INPUT io-rowid).
/*         RUN getQtyPrice IN WIDGET-HANDLE(char-hdl) (OUTPUT historyQty,      */
/*                                                     OUTPUT historyPrice,    */
/*                                                     OUTPUT historyPrUOM,    */
/*                                                     OUTPUT setFromHistory). */
/*         RUN setQtyPrice IN ip-handle (io-rowid,                             */
/*                                       decimal(ENTRY(li, io-qty-list)),      */
/*                                       historyPrice,                         */
/*                                       historyPrUOM,                         */
/*                                       setFromHistory) NO-ERROR.             */
      END.
  END.
...
FIND FIRST cust OF itemfg NO-LOCK NO-ERROR.                      */
FIND FIRST cust WHERE ROWID(cust) = io-rowid NO-LOCK NO-ERROR.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'est/b-qprice.w':U ,
             INPUT  FRAME D-Dialog:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-qprice-2 ).
       RUN set-position IN h_b-qprice-2 ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_b-qprice-2 ( 17.86 , 169.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-qprice-2. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'Record':U , h_b-qprice-2 ).
       RUN add-link IN adm-broker-hdl ( h_b-qprice-2 , 'row-id':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-qprice-2 ,
