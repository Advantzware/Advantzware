...
ASSIGN cTextListToSelect = "GL Account,Description,Amount" 

       cFieldListToSelect = "act,dscr,amt"

       cFieldLength = "25,30,23" 
       cFieldType = "c,c,i" 
    .

{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "GL Account,Description,Amount" .
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 begin_pdate end_pdate sl_avail ~
...
btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_pdate end_pdate sl_avail sl_selected ~
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(256)":U INITIAL "c:~\tmp~\r-invsum.csv" 
...
DEFINE FRAME FRAME-A
     begin_pdate AT ROW 3 COL 23 COLON-ALIGNED HELP
          "Enter Begining Invoice Date"
     end_pdate AT ROW 3 COL 65.2 COLON-ALIGNED HELP
          "Enter Ending Invoice Date"
     sl_avail AT ROW 6.05 COL 4.4 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 6.05 COL 40.4 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 6.05 COL 59.8 NO-LABEL WIDGET-ID 28
     Btn_Add AT ROW 7.05 COL 40.4 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 8.05 COL 40.4 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 9.1 COL 40.4 WIDGET-ID 40
     btn_down AT ROW 10.14 COL 40.4 WIDGET-ID 42
     rd-dest AT ROW 13.38 COL 7 NO-LABEL
     lv-ornt AT ROW 13.38 COL 30 NO-LABEL
     lines-per-page AT ROW 13.38 COL 84 COLON-ALIGNED
     lv-font-no AT ROW 14.52 COL 34 COLON-ALIGNED
     lv-font-name AT ROW 15.71 COL 28 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 16.91 COL 30
     tb_excel AT ROW 17.91 COL 30
     tb_runExcel AT ROW 17.91 COL 56
     fi_file AT ROW 18.91 COL 28 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 21.71 COL 19
     btn-cancel AT ROW 21.71 COL 58
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 5.14 COL 5.40 WIDGET-ID 38
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 5.24 COL 60 WIDGET-ID 44
     "Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.24 COL 5
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 12.43 COL 5
...
ON END-ERROR OF C-Win /* GL Invoice Summary */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* GL Invoice Summary */
DO:
...
DO:
   apply "close" to this-procedure.
    {Advantzware/WinKit/winkit-panel-triggerend.i} /* added by script _nonAdm1.p on 04.18.2017 @ 11:36:03 am */
END.
...
ON CLOSE OF THIS-PROCEDURE DO:
   RUN disable_UI.
   {Advantzware/WinKit/closewindow-nonadm.i} /* added by script _nonAdm1.p on 04.18.2017 @ 11:36:03 am */
END.
...
/* security check need {methods/prgsecur.i} in definition section */
  ...
  DO WITH FRAME {&FRAME-NAME}:
    {methods/setButton.i btn-cancel "Cancel"} /* added by script _nonAdm1Images2.p on 04.18.2017 @ 11:37:00 am */
    {methods/setButton.i btn-ok "OK"} /* added by script _nonAdm1Images2.p on 04.18.2017 @ 11:37:00 am */
    {custom/usrprint.i}
    RUN DisplaySelectionList2.
    APPLY "entry" TO begin_pdate.
  END.

    {Advantzware/WinKit/embedfinalize-nonadm.i} /* added by script _nonAdm1.p on 04.18.2017 @ 11:36:03 am */
...
------------------------------------------------------------------------------*/
  DISPLAY begin_pdate end_pdate sl_avail sl_selected rd-dest lv-ornt 
          lines-per-page lv-font-no lv-font-name td-show-parm tb_excel 
          tb_runExcel fi_file 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-6 RECT-7 begin_pdate end_pdate sl_avail Btn_Def sl_selected 
...
for each ar-inv
   where ar-inv.company eq cocode
    AND ar-inv.inv-date GE begin_pdate
    AND ar-inv.inv-date LE end_pdate
...
/*display 
   v-ar-freight "Freight"   format "x(22)" v-frgt   skip
   v-ar-disc    "Discounts" format "x(22)" v-disc   skip
   v-ar-stax    "Sales Tax" format "x(22)" v-tax    skip
   skip(2)
...
DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
   cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
        CASE cTmpField:             
             WHEN "act"  THEN cVarValue = string(v-ar-freight,"x(25)") .
             WHEN "dscr" THEN cVarValue = STRING("Freight") .
             WHEN "amt"  THEN cVarValue = string(v-frgt,"->>>,>>>,>>>,>>>,>>9.99") .
...
DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
   cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
        CASE cTmpField:             
             WHEN "act"  THEN cVarValue = string(v-ar-disc,"x(25)") .
             WHEN "dscr" THEN cVarValue = STRING("Discounts") .
             WHEN "amt"  THEN cVarValue = string(v-disc,"->>>,>>>,>>>,>>>,>>9.99") .
...
DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
   cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
        CASE cTmpField:             
             WHEN "act"  THEN cVarValue = string(v-ar-stax,"x(25)") .
             WHEN "dscr" THEN cVarValue = STRING("Sales Tax") .
             WHEN "amt"  THEN cVarValue = string(v-tax,"->>>,>>>,>>>,>>>,>>9.99") .
...
PUT UNFORMATTED cDisplay SKIP(2).
IF tb_excel THEN DO:
     PUT STREAM s-temp UNFORMATTED  
           cExcelDisplay SKIP(2).
END.

v-total = v-frgt + v-disc + v-tax.

for each w-f by w-f.actnum with frame f2 no-labels no-box stream-io:
    {custom/statusMsg.i " 'Processing Account#  '  + string(w-f.actnum) "}
  ...
  /* IF v-export THEN
   ...
   ASSIGN cDisplay = ""
       ...
       DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
          cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
               CASE cTmpField:             
                    WHEN "act"     THEN cVarValue = string(w-f.actnum,"x(25)") .
                    WHEN "dscr" THEN cVarValue = STRING(v-acct-descr,"x(30)") .
                    WHEN "amt"     THEN cVarValue = string(w-f.amt,"->>>,>>>,>>>,>>>,>>9.99") .
