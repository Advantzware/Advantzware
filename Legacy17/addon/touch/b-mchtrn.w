...
DEF VAR lv-run-total AS INT LABEL "Total Run" FORM "->>,>>>,>>9" NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD display-run-total B-table-Win 
FUNCTION display-run-total RETURNS INTEGER
...
FUNCTION Time_String RETURNS CHARACTER
  (ip-time AS INTEGER,ip-clock-time AS LOGICAL,ip-end-date AS DATE) FORWARD.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 109 BY 7.86
         FONT 2.
...
DEFINE FRAME F-Main
     ...
     Btn_Clear_Find AT ROW 9.1 COL 96 HELP
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION display-run-total B-table-Win 
FUNCTION display-run-total RETURNS INTEGER
...
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  lv-run-total = lv-run-total + IF AVAIL machtran THEN machtran.RUN_qty ELSE 0.
  RETURN lv-run-total.   /* Function return value. */
...
FUNCTION Time_String RETURNS CHARACTER
  (ip-time AS INTEGER,ip-clock-time AS LOGICAL,ip-end-date AS DATE):
/*------------------------------------------------------------------------------
  Purpose:  return time in string format
    Notes:  
------------------------------------------------------------------------------*/
  IF ip-time = 0 AND ip-end-date eq ? THEN
  RETURN ''.
  ELSE
