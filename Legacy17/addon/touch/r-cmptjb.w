...
&Scoped-Define ENABLED-OBJECTS RECT-2 RECT-6 begin_machine end_machine ~
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-cmptjb.csv" 
...
DEFINE FRAME FRAME-A
     selected-company AT ROW 3.62 COL 37 COLON-ALIGNED
     begin_machine AT ROW 5.29 COL 37 COLON-ALIGNED HELP
          "Enter Beginning Machine"
     end_machine AT ROW 6.95 COL 37 COLON-ALIGNED HELP
     ...
     td-show-parm AT ROW 14.1 COL 48
     lv-font-name AT ROW 15.05 COL 27 COLON-ALIGNED NO-LABEL
     tb_excel AT ROW 16.48 COL 47.2
     tb_runExcel AT ROW 16.48 COL 90 RIGHT-ALIGNED
     fi_file AT ROW 17.52 COL 45 COLON-ALIGNED HELP
          "Enter File Name"
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Completed Machine Job List"
         HEIGHT             = 22.43
         WIDTH              = 95
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 204.8
...
ON END-ERROR OF C-Win /* Completed Machine Job List */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Completed Machine Job List */
DO:
...
DO:
  ...
  CASE rd-dest:
    ...
    WHEN 4 THEN DO:
      /*run output-to-fax.*/
      {custom/asifax.i &type="Completed Machine Jobs List"
    ...
    WHEN 5 THEN DO:
      IF is-xprint-form THEN DO:
        {custom/asimail.i &type="Completed Machine Jobs List"
                          &begin_cust=begin_machine
                          &end_cust=end_machine
      ...
      END.
      ELSE DO:
        {custom/asimailr.i &type="Completed Machine Jobs List"
                           &begin_cust=begin_machine
                           &end_cust=end_machine
                           &mail-subject=CURRENT-WINDOW:TITLE
                           &mail-body=CURRENT-WINDOW:TITLE
...
------------------------------------------------------------------------------*/
  DISPLAY selected-company begin_machine end_machine rd-dest lv-ornt 
          lines-per-page lv-font-no td-show-parm lv-font-name tb_excel 
          tb_runExcel fi_file 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-2 RECT-6 begin_machine end_machine rd-dest lv-ornt lines-per-page 
...
/*==== Report main body procedure ================================*/
  ...
  FOR EACH mach fields(m-code) WHERE
      ...
      NO-LOCK,
      EACH cmpltjob FIELDS(machine job_number job_sub) WHERE
           cmpltjob.company EQ selected-company AND
           cmpltjob.machine EQ mach.m-code
           NO-LOCK
    ...
    IF tb_excel THEN PUT STREAM excel UNFORMATTED
      replace(cmpltjob.machine,","," ") ","
      replace(string(cmpltjob.job_number),",","") ","
