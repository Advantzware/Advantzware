...
/* ***************************  Definitions  ************************** */
DEF INPUT PARAMETER ip-tag-no LIKE fg-rctd.tag.
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 post-date FI_beg-tag-no ~
FI_end-tag-no begin_userid end_userid tg_account tg_show-inv rd-dest ~
lv-ornt lines-per-page td-show-parm lv-font-no btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS post-date FI_beg-tag-no FI_end-tag-no ~
begin_userid end_userid tg_account tg_show-inv rd-dest lv-ornt ~
lines-per-page td-show-parm lv-font-no lv-font-name 
...
DEFINE VARIABLE FI_beg-tag-no AS CHARACTER FORMAT "X(20)":U 
     LABEL "Beginning Tag#" 
     VIEW-AS FILL-IN 
     SIZE 27 BY 1 NO-UNDO.

DEFINE VARIABLE FI_end-tag-no AS CHARACTER FORMAT "X(20)":U 
     LABEL "Ending Tag#" 
     VIEW-AS FILL-IN 
     SIZE 27 BY 1 NO-UNDO.
...
DEFINE FRAME FRAME-A
     post-date AT ROW 3.43 COL 39 COLON-ALIGNED
     FI_beg-tag-no AT ROW 5 COL 19.2 COLON-ALIGNED WIDGET-ID 2
     FI_end-tag-no AT ROW 5 COL 61 COLON-ALIGNED WIDGET-ID 4
     begin_userid AT ROW 6.29 COL 19.2 COLON-ALIGNED HELP
          "Enter the Beginning User ID"
     end_userid AT ROW 6.29 COL 61 COLON-ALIGNED HELP
          "Enter the Ending User ID"
     tg_account AT ROW 8.38 COL 39
     tg_show-inv AT ROW 9.1 COL 39 WIDGET-ID 6
     rd-dest AT ROW 11.71 COL 4 NO-LABEL
     lv-ornt AT ROW 11.95 COL 28.4 NO-LABEL
     lines-per-page AT ROW 11.95 COL 81 COLON-ALIGNED
     td-show-parm AT ROW 12.95 COL 28.4
     lv-font-no AT ROW 13.95 COL 31.8 COLON-ALIGNED
     lv-font-name AT ROW 14.91 COL 26.4 COLON-ALIGNED NO-LABEL
     btn-ok AT ROW 18.38 COL 23
     btn-cancel AT ROW 18.38 COL 58
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 10.76 COL 5
     "Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.29 COL 5.2
     "This procedure will post all selected finished goods physical count transactions" VIEW-AS TEXT
          SIZE 90.6 BY .95 AT ROW 2.29 COL 2.4
          FGCOLOR 4 FONT 6
     RECT-6 AT ROW 10.52 COL 1.4
...
/* security check need {methods/prgsecur.i} in definition section */
  ...
  {methods/nowait.i}
    {methods/setButton.i btn-cancel "Cancel"} /* added by script _nonAdm1Images1.p on 04.18.2017 @ 11:36:35 am */
    {methods/setButton.i btn-ok "OK"} /* added by script _nonAdm1Images1.p on 04.18.2017 @ 11:36:35 am */
...
------------------------------------------------------------------------------*/
  DISPLAY post-date FI_beg-tag-no FI_end-tag-no begin_userid end_userid 
          tg_account tg_show-inv rd-dest lv-ornt lines-per-page td-show-parm 
          lv-font-no lv-font-name 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-6 RECT-7 post-date FI_beg-tag-no FI_end-tag-no begin_userid 
         end_userid tg_account tg_show-inv rd-dest lv-ornt lines-per-page 
         td-show-parm lv-font-no btn-ok btn-cancel 
      WITH FRAME FRAME-A IN WINDOW C-Win.
...
form fg-rctd.rct-date     column-label "Trans.!Date"
     fg-rctd.i-no           label "Item" FORMAT "X(15)"
     ...
     fg-rctd.loc             label "Whse" FORMAT "X(5)"
     fg-rctd.loc-bin         label "Bin"
     fg-rctd.tag             label "Tag" FORM "x(8)"
     fg-rctd.t-qty           format "->>>>>>9" label "Quantity"
     v-sell-price            format "->>>,>>>,>>9.99" COLUMN-LABEL "Selling!Value"
/*      fg-rctd.pur-uom        label "UOM" */
     v-std-cost        label "Cost/M"
...
time_stamp = string(time,"hh:mmam").
 ...
 SESSION:SET-WAIT-STATE("general").  
  ...
  FOR EACH fg-rctd NO-LOCK 
    ...
    assign
     v-tot-value = v-std-cost * (fg-rctd.t-qty / 1000)
    ...
    display fg-rctd.rct-date when first-of(fg-rctd.i-no)
            ...
            fg-rctd.i-name
            itemfg.part-no
            SUBSTR(fg-rctd.tag,16,8) WHEN SUBSTR(fg-rctd.tag,1,15) EQ fg-rctd.i-no @ fg-rctd.tag
            fg-rctd.t-qty
/*             fg-rctd.pur-uom */
            fg-rctd.loc
            fg-rctd.loc-bin
...
form tt-fg-bin.rct-date FORMAT "99/99/99" column-label "Trans.!Date"
     ...
     tt-fg-bin.loc      label "Whse" FORMAT "X(5)"
     tt-fg-bin.loc-bin  label "Bin"
     tt-fg-bin.tag      label "Tag" FORM "x(8)"
     tt-fg-bin.on-hand-qty format "->>>>>>9" COLUMN-LABEL "On-hand!Quantity"
     tt-fg-bin.counted-qty format "->>>>>>9" COLUMN-LABEL "Counted!Quantity"
     tt-fg-bin.sell-price format "->>>,>>>,>>9.99" COLUMN-LABEL "Selling!Value"
/*      tt-fg-bin.pur-uom  label "UOM" */
     tt-fg-bin.std-cost label "Cost/M"
