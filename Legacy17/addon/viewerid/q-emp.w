...
&Scoped-define QUERY-STRING-Query-Main FOR EACH employee WHERE employee.employee = machemp.employee NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH employee WHERE employee.employee = machemp.employee NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main employee
&Scoped-define FIRST-TABLE-IN-QUERY-Query-Main employee
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-rowid q-tables 
PROCEDURE get-rowid :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL employee THEN op-rowid = ROWID(employee).
...
------------------------------------------------------------------------------*/
  ...
  /*  set rec_key_value for emplogin browser's note */
  run get-link-handle in adm-broker-hdl(this-procedure, "rec-key-target", output char-hdl).
  if valid-handle(widget-handle(char-hdl)) then run get-rec-key in widget-handle(char-hdl).
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/sndkycas.i "emp_type" "employee" "emp_type"}
