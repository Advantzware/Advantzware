...
rfqitem.i-col rfqitem.i-coat rfqitem.len rfqitem.wid rfqitem.dep ~
rfqitem.board rfqitem.cal rfqitem.est-no rfqitem.seq 
...
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH rfq WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      EACH rfqitem OF rfq ~
      WHERE {&item-key-phrase} OUTER-JOIN NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      ...
      rfqitem.cal
      rfqitem.est-no
      rfqitem.seq COLUMN-LABEL "Ln#" FORMAT ">>9"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"rfqitem.qty[1]" "Qty" ? "integer" ? ? ? ? ? ? no ?
     _FldNameList[8]   = rfq.rfqitem.i-col
     _FldNameList[9]   = rfq.rfqitem.i-coat
     _FldNameList[10]   = rfq.rfqitem.len
     _FldNameList[11]   = rfq.rfqitem.wid
     _FldNameList[12]   = rfq.rfqitem.dep
     _FldNameList[13]   = rfq.rfqitem.board
     _FldNameList[14]   = rfq.rfqitem.cal
     _FldNameList[15]   = rfq.rfqitem.est-no
     _FldNameList[16]   > rfq.rfqitem.seq
"rfqitem.seq" "Ln#" ">>9" "integer" ? ? ? ? ? ? no ?
...
message "rep".
    ...
    do while true:
      if available rfqitem then
         if rowid(rfqitem) = prRowIdent then leave.
