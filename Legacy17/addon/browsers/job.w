...
&SCOPED-DEFINE dataGridInclude dataGrid\addon\browsers\job.i
...
&Scoped-Define ENABLED-OBJECTS btn_showall lv-job-no lv-est-no br_table ~
CloseDate selected-option 
&Scoped-Define DISPLAYED-OBJECTS lv-job-no lv-est-no closed-jobs CloseDate ~
selected-option 
...
DEFINE FRAME F-Main
     btn_showall AT ROW 1 COL 1
     lv-job-no AT ROW 1 COL 15 COLON-ALIGNED
     lv-est-no AT ROW 1.95 COL 15 COLON-ALIGNED
...
/* BROWSE-TAB br_table lv-est-no F-Main */
ASSIGN 
...
&Scoped-define SELF-NAME btn_showall
...
&Scoped-define SELF-NAME lv-est-no
...
DO:
  ...
  IF LENGTH(lv-est-no) LT 8
      THEN lv-est-no = fill(" ",8 - LENGTH(TRIM(lv-est-no))) +
                           TRIM(lv-est-no).
         .
  FIND FIRST bf-ttbl WHERE bf-ttbl.job-no = lv-est-no no-error.
  IF AVAIL bf-ttbl THEN do:
      REPOSITION {&browse-name} TO ROWID ROWID(bf-ttbl) .
      BROWSE {&browse-name}:SELECT-FOCUSED-ROW().
  END.
  ELSE DO:     
      FIND FIRST job WHERE job.company = g_company
                       AND job.est-no = lv-est-no NO-LOCK NO-ERROR.
...
DO:
  ...
  ASSIGN v-job-no = substring(self:screen-value,1,index(SELF:SCREEN-VALUE,"-") - 1)
         ...
         .
  FIND FIRST bf-ttbl WHERE bf-ttbl.job-no = lv-job-no:SCREEN-VALUE NO-ERROR.
  IF AVAIL bf-ttbl THEN do:
      REPOSITION {&browse-name} TO ROWID ROWID(bf-ttbl) .
      BROWSE {&browse-name}:SELECT-FOCUSED-ROW().
  END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH job
      WHERE job.company = g_company          
         AND (index("CZ",job.stat) eq 0
         or job.close-date gt closedate
         or job.close-date eq ? )
      AND (job.job-no <> "" OR job.est-no <> "")
...
------------------------------------------------------------------------------*/
  ...
  RUN pDataGridInit. /* added by script _admBrowsers.p on 04.18.2017 @ 11:37:35 am */
