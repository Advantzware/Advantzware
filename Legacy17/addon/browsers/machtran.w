...
/* gdm - 11210805*/
&SCOPED-DEFINE yellowColumnsName machtran

&SCOPED-DEFINE browse2 touch\j-machtran.i
...
&Scoped-define ADM-SUPPORTED-LINKS Record-Source,Record-Target,TableIO-Target,Navigation-Target,wiptag-Target
...
Btn_Clear_Find 
&Scoped-Define DISPLAYED-OBJECTS fi_sortby fi_sort-by browse-order ~
auto_find 
...
DEFINE FRAME F-Main
     ...
     fi_sort-by AT ROW 17.91 COL 135 COLON-ALIGNED NO-LABEL WIDGET-ID 4
...
/* SETTINGS FOR FILL-IN fi_sort-by IN FRAME F-Main
   NO-ENABLE                                                            */
ASSIGN 
       fi_sort-by:HIDDEN IN FRAME F-Main           = TRUE.
...
DO:
   ...
   ASSIGN {&SELF-NAME}.
   /*
   if browse-order = 2 AND LENGTH(trim(auto_find)) < 6 then do:
...
DO:
  ...
  DO:
     ...
     {methods/run_link.i "CONTAINER-SOURCE" "MF-Message"
       "(CAN-FIND(FIRST mfvalues WHERE mfvalues.rec_key = machtran.rec_key))"}
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE disable-pages B-table-Win 
PROCEDURE disable-pages :
/*------------------------------------------------------------------------------
  Purpose:     Disable paging functionality when running from "Job Notes" button
               on Data Collection menu.
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/

  /* This is set to "2" by default so double-clicking the record
     makes the program go to page 2.  Setting to unknown causes a
     message to occur so we're setting it to "1". */
  ASSIGN {&BROWSE-NAME}:PRIVATE-DATA IN FRAME {&FRAME-NAME} = "1".
...
IF AVAIL machtran AND machtran.machine NE "" THEN
    ...
    RUN addon/browsers/rd-mtrexp.w (FromMch,ToMch).
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
     ASSIGN browse-order.
  END.
  apply "value-changed" to browse-order in frame {&frame-name}.   /* to have right sort */
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE wiptag B-table-Win 
PROCEDURE wiptag :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
DEF VAR v-result AS CHAR FORMAT "x(200)" NO-UNDO.

IF AVAIL machtran THEN
  RUN jcrep/wipldtgr.w (INPUT "", /* tag */
                       INPUT machtran.job_number,
                       INPUT machtran.job_sub,
                       INPUT "", /* i-no */
                       INPUT machtran.FORM_number, /* form */
                       INPUT machtran.BLANK_number, /* blank */
                       INPUT machtran.RUN_qty, /* qty */
                       INPUT "", /* uom */
                       INPUT machtran.RUN_qty,
                       INPUT machtran.machine,
                       OUTPUT v-result).
END PROCEDURE.
