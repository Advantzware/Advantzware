...
&Scoped-Define DISPLAYED-OBJECTS users_user_id company_name ~
loc_loc 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW MAINMENU ASSIGN
         HIDDEN             = YES
         TITLE              = "ADDON Main Menu - Advantzware version {&awversion}"
         HEIGHT             = 24.52
         WIDTH              = 122.6
         MAX-HEIGHT         = 40
         MAX-WIDTH          = 235.6
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH ttbl NO-LOCK WHERE ttbl.menu2 = menu-item:
    ...
    CREATE BUTTON button-widget IN WIDGET-POOL "dyn-buttons"
      ...
      END TRIGGERS.
    RUN Mneumonic (button-widget:HANDLE).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE INPUT PARAMETER bttn AS WIDGET-HANDLE NO-UNDO.
/*
  IF CAN-DO('File Maintenance >,Inquiries >,Reports >',bttn:LABEL) THEN
  bttn:LABEL = '&' + SUBSTRING(bttn:LABEL,1,1) + ' ' + bttn:LABEL.
  ELSE
  IF button-count LT 41 THEN
  ASSIGN
    button-count = button-count + 1
    bttn:LABEL = '&' + ENTRY(button-count,button-mneumonic) + ' ' + bttn:LABEL.
  ELSE
    ...
    DO:
      ...
      DO:
        IF NOT CAN-DO('&F File Maintenance >,&I Inquiries >,&R Reports >',
           ...
           INDEX(current-widget:LABEL,'&') NE 0 THEN
        button-count = button-count - 1.
...
/*      loc_dscr:SCREEN-VALUE = ip-loc_dscr  */
    ...
    RUN custom/gettime.p .  /* get time source*/
  END.
