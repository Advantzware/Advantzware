...
&scoped-define fld-name-1 eb.part-no
&scoped-define fld-name-2 eb.est-no
&scoped-define SORTBY-1 BY eb.part-no
&scoped-define SORTBY-2 BY eb.est-no
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 eb.est-no eb.cust-no eb.stock-no ~
...
ASi.eb.loc = ip-loc and ~
/*ASI.eb.cust-no = ip-cust-no and*/ ~
asi.eb.part-no <> "" NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      eb.est-no
      eb.cust-no COLUMN-LABEL "Customer#"
      eb.stock-no COLUMN-LABEL "FG Item#"
      eb.part-no
      eb.part-dscr1
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
"eb.cust-no" "Customer#" ? "character" ? ? ? ? ? ? no ?
     _FldNameList[3]   > ASI.eb.stock-no
"eb.stock-no" "FG Item#" ? "character" ? ? ? ? ? ? no ?
...
DO:
   op-char-val = eb.part-no:screen-value in browse {&browse-name} + "," +
                 string(eb.est-no)  
                 .
