...
ON CHOOSE OF Btn-Update IN FRAME F-Main /* Close */
DO:
...
/* ***************************  Main Block  *************************** */
  ...
  {methods/setButton.i Btn-Update "Close"} /* added by script c:\tmp\p64604__panelImages.ped */
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ENABLE btn-update.
    IF AVAIL job AND INDEX("PX",job.stat) EQ 0 THEN
      btn-update:LABEL = IF job.opened THEN "&Close" ELSE "&Reopen".
    ELSE DO:
