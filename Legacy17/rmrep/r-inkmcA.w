...
&Scoped-Define ENABLED-OBJECTS tb_fold tb_corr begin_mach-no end_mach-no ~
...
fi_file btn-ok btn-cancel RECT-6 RECT-8 
...
DEFINE FRAME FRAME-A
     ...
     lv-ornt AT ROW 9.76 COL 32 NO-LABEL
     lines-per-page AT ROW 9.76 COL 85 COLON-ALIGNED
     rd-dest AT ROW 10.52 COL 6 NO-LABEL
     lv-font-no AT ROW 11.43 COL 35 COLON-ALIGNED
     lv-font-name AT ROW 12.38 COL 29 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 13.71 COL 31
     tb_excel AT ROW 15.05 COL 68 RIGHT-ALIGNED
     tb_runExcel AT ROW 15.05 COL 92 RIGHT-ALIGNED
     fi_file AT ROW 16 COL 46 COLON-ALIGNED HELP
          "Enter File Name"
...
------------------------------------------------------------------------------*/
  DISPLAY tb_fold lbl_industry tb_corr begin_mach-no end_mach-no begin_job-no 
          begin_job-no2 end_job-no end_job-no2 begin_date end_date lv-ornt 
          lines-per-page rd-dest lv-font-no lv-font-name td-show-parm tb_excel 
          tb_runExcel fi_file 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE tb_fold tb_corr begin_mach-no end_mach-no begin_job-no begin_job-no2 
         end_job-no end_job-no2 begin_date end_date lv-ornt lines-per-page 
         rd-dest lv-font-no td-show-parm tb_excel tb_runExcel fi_file btn-ok 
         btn-cancel RECT-6 RECT-8 
      WITH FRAME FRAME-A IN WINDOW C-Win.
    ...
    for each tt-report where tt-report.term-id eq "",
      ...
      if last-of(tt-report.key-05) THEN DO:
        display trim(substr(tt-report.key-02,9,9))
                ...
                cust.name           column-label "NAME"
                                    format "x(20)"
                job-hdr.i-no        when avail job-hdr
                                    column-label "FG ITEM #"
                v-board             column-label "BOARD"
                                    format "x(10)"
                v-adder[1]          column-label "ADDER 1"
                                    format "x(10)"
                v-adder[2]          column-label "ADDER 2"
                                    format "x(10)"
                v-date              column-label "DUE DATE"
                                    format "99/99/99"
                v-cov               column-label "PCT!COV"
                                    format ">>9"
                v-qty               column-label "   QTY!SHEETS"
                                    format ">,>>>,>>9"
                job-mat.i-no        column-label "INK!CODE"
                                    format "x(10)"    
