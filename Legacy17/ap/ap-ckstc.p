/* ap-ckstc.p  Check print for ST Clair */
...
if v-print-mode ne "ALIGN" then do:         /* production mode */
  for each ap-chk
   ...
   for each ap-sel
    ...
    FIND FIRST ap-invl WHERE ap-invl.i-no = ap-inv.i-no 
           USE-INDEX i-no NO-LOCK NO-ERROR.

    create wrk-chk.
    assign
     wrk-chk.inv-no   = ap-sel.inv-no
     wrk-chk.po-no    = IF AVAIL ap-invl THEN ap-invl.po-no ELSE 0
