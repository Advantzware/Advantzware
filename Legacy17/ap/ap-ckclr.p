/* ap-ckclr.p  Check print for Color Carton Laser */
...
DEF VAR viLn-Check      AS INT NO-UNDO INIT 26. /* Line to start printing the check. */
  ...
  check-block:
   FOR EACH ap-chk
      ...
      FIRST vend WHERE vend.company eq cocode AND 
       ...
       /* Get the next check number (updates variable stnum) */
/*        RUN get-check-num. */
       ...
       FOR EACH ap-sel WHERE 
         ...
         IF maxDetailsPrinted() OR LAST(ap-sel.inv-no) THEN DO:
             ...
             IF viCurrentSection = 1  THEN DO:
                 ...
                 /* If last detail line, print totals */
                 IF LAST(ap-sel.inv-no) THEN
                     RUN Print-Totals (ap-chk.check-no).
         ...
         /* If all data is processed, then increment the check number. */
/*          IF LAST(ap-sel.inv-no) THEN     */
/*              ASSIGN stnum  = stnum + 1.  */
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEFINE INPUT PARAMETER ipCheckNo LIKE ap-chk.check-no NO-UNDO.
