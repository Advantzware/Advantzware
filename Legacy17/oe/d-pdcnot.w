...
&Scoped-define QUERY-STRING-dialog-Frame FOR EACH notes ~
      WHERE notes.rec_key = ip-note-rec-key SHARE-LOCK
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND FIRST oe-rel WHERE oe-rel.rec_key = ip-note-rec-key NO-LOCK NO-ERROR.
...
FIND FIRST oe-ordl WHERE oe-ordl.rec_key = ip-note-rec-key NO-LOCK NO-ERROR.
...
FOR EACH rejct-cd 
    ...
    cbTitle:ADD-LAST(rejct-cd.CODE + " " + rejct-cd.dscr, rejct-cd.CODE).        
END.
ASSIGN notes.note_code:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "DDC"
       notes.note_text:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE rel-due-date-init dialog-Frame
PROCEDURE rel-due-date-init:
    /*------------------------------------------------------------------------------
     Purpose:
     Notes:
    ------------------------------------------------------------------------------*/
    ...
    END.
    ASSIGN 
        notes.note_code:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "DRC"
        notes.note_text:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
    FRAME dialog-Frame:TITLE = "Release Due Date Reason Selection".
    DISABLE notes.note_code.
