...
&Scoped-define QUERY-STRING-Query-Main FOR EACH itemfg WHERE itemfg.company eq oe-ordl.company and ~
itemfg.i-no    eq oe-ordl.i-no ~
 NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY Query-Main FOR EACH itemfg WHERE itemfg.company eq oe-ordl.company and ~
itemfg.i-no    eq oe-ordl.i-no ~
 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Query-Main itemfg
...
/* Query rebuild information for QUERY Query-Main
     _TblList          = "ASI.itemfg WHERE ASI.oe-ordl ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _JoinCode[1]      = "itemfg.company eq oe-ordl.company and
itemfg.i-no    eq oe-ordl.i-no
"
...
------------------------------------------------------------------------------*/
  ...
  IF lv-navi-selected THEN DO:
      /* gdm - 09100801 
       made query rowid specific */
      OPEN QUERY {&query-name}
          /*FOR EACH  ASI.itemfg NO-LOCK WHERE itemfg.company = g_company.*/
          FOR EACH ASI.itemfg NO-LOCK WHERE ROWID(itemfg) = lv-rowid.
