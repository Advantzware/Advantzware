/*oe/rep/invpkrt.i  */
   ...
   PUT 
      ...
      "<P10><=3><R+3>" SKIP
      "<FCourier New><B>"
      "Bill To:  </B>"  inv-head.cust-name 
      "<B>Ship To:  </B>" AT 83 v-shipto-name FORMAT "X(25)" SKIP 
      SPACE(10) inv-head.addr[1]  
      v-shipto-addr[1]           AT 72    SKIP      
      SPACE(10) inv-head.addr[2]   
   ...
   PUT 
      "<FArial><P12><=#3><R-3> <B>Invoice#: " inv-head.inv-no 
      "             <=#3><R-2> <B>Invoice Date: " v-inv-date "</B><P10>" SKIP
      "<=#3><R+0><B> Customer ID         Telephone"
      "<=#3><R+2><B> Contact                 Customer Email <FCourier New></B>"  
      "<=3><R+1> " inv-head.cust-no SPACE(6) 
      cust.area-code + cust.phone   FORMAT "(999)999-9999" 
      "<=3><R+3> " cust.contact     FORMAT "X(13)" SPACE(1) 
      TRIM(cust.email)              FORMAT "X(20)". 
   ...
   PUT 
      ...
      v-fob                                  FORMAT "x(11)" SPACE(1)
      v-shipvia                              FORMAT "x(16)" SPACE(1)
      xinv-head.terms-d                      FORMAT "x(20)" space(1) 
      v-salesman-name                        FORMAT "X(14)" SPACE(1) 
      v-tot-pallets                          FORMAT ">>>,>>9" 
      xinv-head.bol-no 
      SKIP.
