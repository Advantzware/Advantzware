...
DEF VAR vld-misc-total AS DEC FORMAT "->>>,>>>,>>9.9999999999" INIT 0 NO-UNDO.
...
FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
   FIRST xinv-head WHERE RECID(xinv-head) EQ report.rec-id NO-LOCK
      ...
      FOR EACH inv-misc NO-LOCK 
        WHERE inv-misc.company = inv-head.company 
         ...
         ASSIGN v-subtot-lines = v-subtot-lines + inv-misc.amt
                v-printline = v-printline + 2
                vld-misc-total = vld-misc-total + inv-misc.amt.
    ...
    PUT "<R57><C1><#7><FROM><C+80><LINE>"
        ...
        "<=7><C31>" v-inv-freight FORMAT ">>,>>9.99" v-net FORMAT "->,>>>,>>9.99" " " tmp1 "    " tmp2 " " (IF v-net2 <> inv-head.t-inv-rev THEN (v-net2 + vld-misc-total) ELSE (inv-head.t-inv-rev + vld-misc-total)) FORMAT "->>>>,>>9.99"
