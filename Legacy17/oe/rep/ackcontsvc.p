/* -------------------------------------------- oe/rep/ackcontsvc.p 05/08 GBW */
/* ORDER ACKNOLEDGEMENT   for Container Services (from franke )                        */
...
DEF VAR ls-full-img1 AS cha FORM "x(1000)" NO-UNDO.
DEF VAR v-first AS LOG NO-UNDO.

ASSIGN ls-image1 = "images\cscphone.jpg"
...
DEF VAR v-ship-adr AS cha FORMAT "X(30)" EXTENT 4 NO-UNDO.
...
{custom/notesdef.i}
 ...
 END.
  ...
  FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
      ...
      IF oe-ord.sold-city = "" AND oe-ord.sold-state = "" AND
         oe-ord.sold-zip = "" THEN
         v-sold-addr3 = v-addr3.
...
/*              OR  (oe-rel.link-no NE 0 AND v-actrel)) */
      ...
      {oe/rep/ackcontsvc.i} /* page header stuff */

      for each oe-ordl NO-LOCK
         where oe-ordl.company eq oe-ord.company
           and oe-ordl.ord-no  eq oe-ord.ord-no:
        
        if v-printline ge lv-line-print then RUN next-page IN THIS-PROCEDURE.

        ASSIGN
           v-ext-price = oe-ordl.t-price
           v-part = oe-ordl.part-no
           v-pdscr1 = oe-ordl.part-dscr1
           v-pdscr2 = oe-ordl.part-dscr2
           v-pdscr3 = "".

        IF v-pdscr1 = "" THEN  
           ASSIGN v-pdscr1 = v-pdscr2
                  v-pdscr2 = v-pdscr3
                  v-pdscr3 = "".
        IF v-pdscr1 = "" THEN 
           ASSIGN v-pdscr1 = v-pdscr2
        ...
        put /*v-line FORM ">>>9" SPACE(3)*/
                ...
                oe-ordl.i-name FORM "x(32)" SPACE(1)
                oe-ordl.qty FORM "->,>>>,>>>,>>9" 
        ...
        PUT oe-ordl.i-no AT 2 
            v-pdscr1 FORMAT "x(30)" AT 33  SKIP.
        v-printline = v-printline + 1.
        ...
        do:
           find first itemfg {sys/look/itemfgrlW.i} AND
                itemfg.i-no eq oe-ordl.i-no no-lock no-error.
        ...
        IF v-prntinst THEN DO:
           ...
           IF AVAIL itemfg THEN DO:
              ...
              if v-printline ge lv-line-print then RUN next-page IN THIS-PROCEDURE.
           END. /* avail itemfg */
        END. /* print spec notes - v-prntinst */
      end. /* each oe-ordl */
      ...
      /* last page footer */
      PUT "<R55><C1><#10><P12><B> Notes: </B> <P10> " 
        "<R56><C1>" v-billinst[1] 
        "<R57><C1>" v-billinst[2] 
        "<R58><C1>" v-billinst[3] 
        "<R59><C1>" v-billinst[4]   "<P10>Total Order Value:" AT 65 "<U>"  v-totord FORM "$->>>>,>>9.99"  "</U><P9>".

      PUT 
        "<R61><C35><P9>Thank You For Your Order-<R63><C35>Please Check For Accuracy" SKIP
        "<R64><C30>If We May Be Of Any Further Help Please Call"
        "<R64><C65><P10>Page "  string(PAGE-NUM - lv-pg-num,">>9") + " of " + string(lv-tot-pg) FORM "x(20)" 
        .
...
RETURN. /* no main-line code past here, reserved for procedures */
   ...
   DO WITH with frame sched-rel DOWN:
      if ip-first then do:
         ...
         if ip-sch THEN do:
           ...
           put "Scheduled Releases:" at 33 SKIP .
           v-printline = v-printline + 1.
         end.
         else do:
           if v-printline ge lv-line-print then
           ...
           put "Actual Releases:" at 33 SKIP. 
           v-printline = v-printline + 1.  
         end.
      end.          
      ...
      IF AVAIL shipto THEN DO:
          IF v-shipto THEN
             put oe-rel.po-no AT 2 shipto.ship-name AT 33 FORM "x(28)" /*lcnt AT 55 FORM ">>9" SPACE(1)*/  ld-date AT 61 (if ip-sch then oe-rel.tot-qty else oe-rel.qty)  SKIP .
          ELSE
             PUT oe-rel.po-no AT 2 ld-date AT 61 (if ip-sch then oe-rel.tot-qty else oe-rel.qty)  SKIP .
          ...
          IF v-shipto AND shipto.ship-addr[1] <> "" THEN DO:
             ...
             PUT shipto.ship-addr[1] AT 33  SKIP.
             v-printline = v-printline + 1.
          ...
          IF v-shipto AND shipto.ship-addr[2] <> "" THEN DO:
              ...
              PUT shipto.ship-addr[2] AT 33 SKIP.
              v-printline = v-printline + 1.
          END.
