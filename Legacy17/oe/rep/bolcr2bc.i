...
IF FIRST-OF(tt-boll.LINE) THEN DO:
  ...
  FOR EACH bf-ttboll WHERE bf-ttboll.i-no = tt-boll.i-no
      ...
      ASSIGN w2.cas-cnt = bf-ttboll.qty-case
             ...
             .
      IF i = 1 THEN do: 
          ASSIGN w2.job-po = bf-ttboll.po-no .
          IF AVAIL oe-ordl THEN 
              ASSIGN w2.dscr = oe-ordl.i-no
              w2.qty = oe-ordl.qty.
      END.
      ELSE IF i = 2 THEN do: 
...
/*                              (trim(oe-ordl.job-no) + "-" + string(oe-ordl.job-no2,"99")) */
      ...
      END.
      else if i eq 3 AND AVAIL oe-ordl then ASSIGN w2.dscr = oe-ordl.part-dscr1.
      ELSE if i eq 4 AND AVAIL oe-ordl then ASSIGN w2.dscr = oe-ordl.part-dscr2.
...
/*            trim(string(w2.qty,"->>,>>>,>>>")) WHEN i = 1 @ w2.i-no */
           itemfg.part-no WHEN i = 1 @ w2.i-no
            w2.job-po
            w2.dscr
            w2.pallets
            w2.cases
...
*/
  ...
  do i = v-lines + 1 to 4:
    ...
    if i eq 3 AND AVAIL oe-ordl then v-part-dscr = oe-ordl.part-dscr1.
...
/*  if line-counter {2} - 1 + v-lines gt page-size {2} + 1 then page {1}.*/
  ...
  for each w2 break by w2.cases:
    ...
    else if i eq 3 AND AVAIL oe-ordl then v-part-dscr = oe-ordl.part-dscr1.

    ELSE if i eq 4 AND AVAIL oe-ordl then v-part-dscr = oe-ordl.part-dscr2.

    DISPLAY 
/*         trim(string(oe-ordl.qty,"->>,>>>,>>>")) when i eq 1 */
           itemfg.part-no                when i eq 1
                                                    @ oe-ordl.i-no
