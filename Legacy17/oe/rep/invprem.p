/* ----------------------------------------------- oe/rep/invprem.p 02/98 FWK */
/* PRINT INVOICE when sys-ctrl.char-fld eq "Premier" - O/E Module             */
/* -------------------------------------------------------------------------- */
...
form header
  skip(2)
  v-inv-date at 62
  inv-head.inv-no at 72 skip(4)
  inv-head.cust-name at 8 v-shipto-name    at 50 skip
  inv-head.addr[1]   at 8 v-shipto-addr[1] at 50 skip
  inv-head.addr[2]   at 8 v-shipto-addr[2] at 50 skip
  v-addr3            at 8 v-sold-addr3     at 50
  skip(3)
  "B.O.L. #:" AT 50 inv-head.bol-no skip
...
form
  "Sub-Total:" to 65 space(0) v-tot-lines  to 80 format "->>,>>9.99"
  "Sales Tax:" to 65 space(0) inv-head.t-inv-tax  to 80 format "->>,>>9.99"
  disp-frt to 65 space(0) inv-head.t-inv-freight TO 80 format "->,>>9.99"
  "----------" to 80
  "Total Invoice:" to 65 v-tot-rev TO 80 format "->,>>>,>>9.99"
  "==========" to 80
      ...
      DO TRANSACTION:
        ...
        for each inv-line
            where inv-line.r-no eq inv-head.r-no
              and (inv-line.ship-qty ne 0 or inv-line.inv-qty ne 0)
            no-lock:
          assign
           v-case-line = ""
           v-part-line = ""
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            do:
              put skip(28 - v-printline) "* CONTINUED *" to 80.
            ...
            assign v-inv-qty = inv-line.qty
                   ...
                   v-tot-lines = v-tot-lines + inv-line.t-price.
...
*/
        ...
        do:
          assign v-tot-rev = v-tot-lines + inv-head.t-inv-tax +
