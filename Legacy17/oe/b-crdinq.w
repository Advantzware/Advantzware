...
&SCOPED-DEFINE key-phrase oe-ord.company EQ cocode
...
DEFINE VARIABLE tb_appr AS LOGICAL INITIAL no 
     LABEL "Approved" 
     VIEW-AS TOGGLE-BOX
     SIZE 17 BY 1
     FONT 6 NO-UNDO.

DEFINE VARIABLE tb_held AS LOGICAL INITIAL yes 
     LABEL "On Hold" 
     VIEW-AS TOGGLE-BOX
     SIZE 17 BY 1
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
APPLY 'down-arrow' TO BROWSE {&browse-name}.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY tb_appr WITH FRAME {&FRAME-NAME}.
  ...
  &SCOPED-DEFINE open-query                   ~
      OPEN QUERY {&browse-name}               ~
        {&for-each11}                         ~
              AND oe-ord.ord-no GE lv-ord-no ~
            ,         ~
            {&for-each21}
