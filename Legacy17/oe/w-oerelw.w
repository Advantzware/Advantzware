...
DEFINE VARIABLE h_b-oerelw AS HANDLE NO-UNDO.
DEFINE VARIABLE h_exit AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_options-2 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_smartmsg AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v-approv-3 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Web Releases */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Web Releases */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_options-2 ).
       RUN set-position IN h_options-2 ( 1.00 , 85.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'oe/b-oerelw.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-oerelw ).
       RUN set-position IN h_b-oerelw ( 4.57 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.76 , 144.00 ) */

       /* Links to SmartNavBrowser h_b-oerelw. */
       RUN add-link IN adm-broker-hdl ( h_b-oerelw , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-oerelw ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'oe/v-approv.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-approv-3 ).
       RUN set-position IN h_v-approv-3 ( 13.14 , 97.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.91 , 21.00 ) */
       ...
       RUN add-link IN adm-broker-hdl ( h_b-oerelw , 'Record':U , h_v-oerel ).

       /* Links to SmartViewer h_v-approv-3. */
       RUN add-link IN adm-broker-hdl ( h_v-oerel , 'approve':U , h_v-approv-3 ).

       /* Links to SmartViewer h_v-navest. */
       RUN add-link IN adm-broker-hdl ( h_b-oerelw , 'nav-itm':U , h_v-navest ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-oerel ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-approv-3 ,
             h_v-oerel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-navest ,
             h_v-approv-3 , 'AFTER':U ).
    ...
    WHEN 3 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_b-oerelw , 'Record':U , h_v-relhs ).
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL oe-relh AND AVAIL oe-rell THEN
  RUN custom/setUserPrint.p (g_company,'oe-relh_.',
                             'begin_cust-no,end_cust-no,begin_relnum,end_relnum,begin_ord-no,end_ord-no,tb_printed',
