...
/* gdm - 07130906*/
DEF INPUT PARAM ip-order LIKE oe-ordl.ord-no NO-UNDO.
...
def var v-fill3 as char format "x(260)" NO-UNDO.
...
DEF VAR v-printer-sheets LIKE ef.gsh-qty NO-UNDO.
DEF VAR v-sheets LIKE ef.gsh-qty NO-UNDO.
DEF VAR v-printer-dept-seq AS INT NO-UNDO.
DEF VAR v-n-out-sheeter AS INT NO-UNDO.
DEF VAR v-top-sheets LIKE v-printer-sheets NO-UNDO.
DEF VAR v-die-sheets LIKE v-top-sheets NO-UNDO.
...
DEF BUFFER sheetsize-reftable FOR reftable.

DEF TEMP-TABLE tt-printer-sheets-form NO-UNDO
    FIELD job-no AS CHAR
    FIELD job-no2 AS INT
    FIELD form-no AS INT
    field sheets like ef.gsh-qty.
...
DEF VAR v-ink3 AS cha EXTENT 50 NO-UNDO.
...
DEF VAR v-i-qty AS DEC FORMAT ">>>,>>9.99" NO-UNDO.
...
FIND FIRST dept WHERE
     dept.CODE EQ "PR"
     NO-LOCK.

IF AVAIL dept THEN
DO:
   v-printer-dept-seq = dept.fc.
   RELEASE dept.
END.
...
for each job-hdr NO-LOCK
    ...
    IF production AND job.cs-trans-date NE ? THEN DO:
      li = 0.
      DO WHILE li LT 1000:
        li = li + 1.
    ...
    if v-first then do:   
       ...
       /* gdm - 07130906*/
       IF ip-order GT 0 THEN DO:
         FIND FIRST bf-oe-ord NO-LOCK
           WHERE bf-oe-ord.company EQ job-hdr.company
             AND bf-oe-ord.ord-no  EQ ip-order NO-ERROR.
         IF AVAIL oe-ord THEN
           FIND FIRST bf-cust 
             WHERE bf-cust.company = bf-oe-ord.company 
               AND bf-cust.cust-no = bf-oe-ord.cust-no NO-LOCK NO-ERROR.

           v-cust-name = IF AVAIL bf-oe-ord THEN bf-oe-ord.cust-name 
                           ELSE IF AVAIL bf-cust THEN bf-cust.name
       ...
       IF v-shipto[2] <> "" THEN
          PUT v-shipto[2] AT 10 SKIP.

       IF v-shipto[4] <> "" THEN
         PUT v-shipto[4] AT 10 SKIP.
     ...
     FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
        ...
        IF FIRST(bf-jobhdr.frm) THEN
           PUT "  <P10><B><C2.7>F/B   FG ITEM #        ORDER QTY    FORM QTY  PO#       ITEM NAME                  G.L. #      STYLE  CARTON SIZE                CAD#    UP   COLORS" /*v-upc-lbl*/ "</B>" SKIP.
              ...
              if not avail wrk-ink                              and
               (job-mat.blank-no  eq eb.blank-no or
                ...
                assign
                 ...
                 wrk-ink.i-pass   = 1.
              end.                                    /* task 01131504 (01/15/2015)*/
              if avail wrk-ink then wrk-ink.i-qty = /*wrk-ink.i-qty +*/ job-mat.qty /*job-mat.cost-m*/ /* task 11171411 */ .
            END. /* eb*/
          end. /* JOB-MAT */
        ...
        for each eb
          ...
          if eb.est-type eq 4 then v-fac = eb.yld-qty / v-est-qty.
            ...
            ASSIGN
               ...
               v-job-qty = 0
               v-stock-no = IF est.est-type >= 2 AND est.est-type <= 3 THEN
                               bf-jobhdr.i-no
                            ELSE eb.stock.
            ...
            /* ==============*/
            find first xoe-ordl where 
                       xoe-ordl.company eq job-hdr.company
                   and xoe-ordl.ord-no  eq job-hdr.ord-no
                   and xoe-ordl.job-no  eq job-hdr.job-no
                   and xoe-ordl.job-no2 eq job-hdr.job-no2
                   and xoe-ordl.i-no    eq v-stock-no
                   no-lock no-error.
            IF AVAIL xoe-ordl THEN
               ASSIGN v-ord-qty = IF AVAIL xoe-ordl AND xoe-ordl.i-no EQ v-stock-no THEN
                                 (IF eb.cust-% > 0 THEN (xoe-ordl.qty * eb.cust-%)
                                  ELSE xoe-ordl.qty)
                                  ELSE v-est-qty.
            ELSE 
               ASSIGN v-ord-qty = IF AVAIL oe-ordl AND oe-ordl.i-no EQ v-stock-no THEN
            ...
            v-po-no4  = IF AVAIL oe-ordl THEN oe-ordl.po-no ELSE bf-jobhdr.po-no.
    ...
    END. /*tt-reftable*/
     ...
     PUT "<P10>" /*v-fill SKIP*/                       /*REQ'D*/
         "<B><C1>FORM BOARD CODE          DUE DATE   SHEETS             SHEET SIZE                                        DIE SIZE               DIE#        TOTAL UP</B>" 
         SKIP.
     ...
     FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
        ...
        IF FIRST-OF(tt-reftable.val[12]) THEN DO:
         ...
         for each eb
            ...
            /* gdm - 12010809 */
            ASSIGN v-die-no = IF trim(eb.die-no) NE "" 
                                THEN  trim(eb.die-no)
                                ELSE v-die-no.
            IF v-die-no EQ ""
              THEN
               FIND FIRST itemfg NO-LOCK
                  WHERE itemfg.company = eb.company 
                    AND itemfg.i-no = eb.stock-no NO-ERROR.
               IF AVAIL itemfg 
                 THEN ASSIGN v-die-no = TRIM(itemfg.die-no).
          ...
          find first oe-ordl
             ...
             IF AVAIL oe-ordl THEN 
               FIND FIRST po-ord WHERE po-ord.company = oe-ordl.company
                            AND po-ord.po-no = int(oe-ordl.po-no-po)
                    NO-LOCK NO-ERROR.
            ...
            /** PRINT SHEET **/
             for each wrk-sheet WHERE wrk-sheet.form-no = tt-reftable.val[12] break by wrk-sheet.form-no:
                 ...
                 FIND FIRST sheetsize-reftable WHERE
                      sheetsize-reftable.reftable EQ "cerep/d-fibr1.w" AND
                      sheetsize-reftable.company  EQ bf-jobhdr.company AND
                      sheetsize-reftable.loc      EQ bf-jobhdr.job-no AND
                      sheetsize-reftable.code     EQ STRING(bf-jobhdr.job-no2,"9999999999") AND
                      sheetsize-reftable.val[1]   EQ tt-reftable.val[12]
                      NO-LOCK NO-ERROR.

                 IF AVAIL sheetsize-reftable THEN
                 DO:
                    IF sheetsize-reftable.val[3] <> 0 THEN RUN sys/inc/dec-frac.p (sheetsize-reftable.val[3],32,OUTPUT vs-len).
                    IF sheetsize-reftable.val[2] <> 0 THEN RUN sys/inc/dec-frac.p (sheetsize-reftable.val[2],32,OUTPUT vs-wid).
                    ASSIGN
                       v-printer-sheets = INT(sheetsize-reftable.val[4])
                       v-die-sheets = INT(sheetsize-reftable.val[5]).
                 END.
                 ELSE
                 DO:
                    ...
                    v-printer-sheets = wrk-sheet.gsh-qty.
                 END.

                 FIND FIRST tt-printer-sheets-form WHERE
                      tt-printer-sheets-form.job-no EQ bf-jobhdr.job-no AND
                      tt-printer-sheets-form.job-no2 EQ bf-jobhdr.job-no2 AND
                      tt-printer-sheets-form.form-no EQ tt-reftable.val[12]
                      NO-ERROR.

                 IF NOT AVAIL tt-printer-sheets-form THEN
                 DO:
                     CREATE tt-printer-sheets-form.
                     ASSIGN
                        tt-printer-sheets-form.job-no = bf-jobhdr.job-no
                        tt-printer-sheets-form.job-no2 = bf-jobhdr.job-no2
                        tt-printer-sheets-form.form-no = tt-reftable.val[12]
                        tt-printer-sheets-form.sheets = v-printer-sheets.
                     RELEASE tt-printer-sheets-form.
                 END.
                 ...
                 IF CAN-FIND(FIRST job-mch WHERE
                    job-mch.company EQ bf-jobhdr.company AND
                    job-mch.job     EQ bf-jobhdr.job AND
                    job-mch.job-no  EQ bf-jobhdr.job-no AND
                    job-mch.job-no2 EQ bf-jobhdr.job-no2 AND
                    job-mch.dept EQ "PR" AND
                    (job-mch.frm    EQ bf-jobhdr.frm OR
                     CAN-FIND(FIRST est WHERE
                     est.company   EQ bf-jobhdr.company AND
                     est.est-no    EQ bf-jobhdr.est-no AND
                     (est.est-type EQ 2 OR est.est-type EQ 6)))) THEN
                    v-top-sheets = v-printer-sheets.
                 ELSE
                    v-top-sheets = v-die-sheets.

                 PUT                
                    wrk-sheet.form-no
                       " " wrk-sheet.brd-dscr " "
                    v-po-duedate " " v-top-sheets SPACE(13)
                    v-sht-size format "x(49)" " "
                    v-die-size FORM "x(20)"  " "
                    v-die-no /* gdm - 12010809 eb.die-no */
                    v-up
                    SKIP.
             end. /* each wrk-sheet */
          END. /* last-of(eb.form)*/
          END. /* eb */
           END. /* ef */
          IF NOT LAST(tt-reftable.val[12]) OR tt-reftable.est-type = 4 THEN
             put v-fill3  skip.
        END. /*first-of(tt-reftable.val[12] */
        END.  /* tt-reftable*/
    ...
    IF FIRST-OF(tt-reftable.val[12]) THEN DO:     
       ...
       END.
         ...
         v-n-out-sheeter = ef.n-out.

         FOR EACH wrk-op WHERE wrk-op.s-num = tt-reftable.val[12] BREAK by wrk-op.d-seq by wrk-op.b-num:
             v-mat-for-mach = "".

             FIND FIRST tt-printer-sheets-form WHERE
                  tt-printer-sheets-form.job-no EQ job-hdr.job-no AND
                  tt-printer-sheets-form.job-no2 EQ job-hdr.job-no2 AND
                  tt-printer-sheets-form.form-no EQ tt-reftable.val[12]
                  NO-ERROR.

             IF AVAIL tt-printer-sheets-form THEN
                v-printer-sheets = tt-printer-sheets-form.sheets.

             IF lookup(wrk-op.dept,lv-mat-dept-list) > 0 THEN DO:
                 
                FOR EACH xjob-mat FIELDS(rm-i-no wid len qty) WHERE
                    xjob-mat.company eq cocode
             ...
             IF s-prt-mstandard THEN DO:
                ...
                IF wrk-op.d-seq LT v-printer-dept-seq THEN
                   v-sheets = v-printer-sheets / v-n-out-sheeter.
                ELSE IF wrk-op.dept EQ "PR" THEN
                   v-sheets = v-printer-sheets.
                ELSE
                   v-sheets = wrk-op.num-sh[wrk-op.s-num].
