...
DEF VAR v-prev-job LIKE oe-ord.po-no NO-UNDO.
...
DEF BUFFER bf-reftable-unit1 FOR reftable.
DEF BUFFER bf-reftable-unit2 FOR reftable.
...
DEF VAR v-fgname LIKE eb.part-dscr1 EXTENT 10 NO-UNDO.
DEF VAR v-fgdsc LIKE eb.part-dscr2 EXTENT 10 NO-UNDO.
DEF VAR v-batch LIKE oe-rel.lot-no EXTENT 10 NO-UNDO.
...
DEF VAR cPass AS CHAR NO-UNDO.
DEF VAR iUnit AS INT NO-UNDO.
DEFINE TEMP-TABLE tt-ink-print NO-UNDO
...
/*         v-next-unit = 0.    */
        ...
        IF NOT AVAIL eb THEN 
          FIND first eb WHERE eb.company     EQ job-hdr.company
                          ...
                          AND eb.blank-no > 0 
                        NO-LOCK NO-ERROR.
        v-bar-no = IF AVAIL eb THEN eb.spc-no ELSE "".
        IF v-bar-no = "" THEN v-bar-no = trim(job-hdr.job-no) + "-" + STRING(job-hdr.job-no2,"99").
        ...
        PUT "<R-1><#1><C91>Date/Time Generated:" SKIP
            "<B>CUSTOMER NAME:</B>" v-cust-name "<B> DUE DATE:   ESTIMATE:  OVER/UNDER:" lv-prt-date AT 119 SPACE(1) lv-prt-time SKIP
            "SHIPTO:</B>" v-shipto[1] /*v-req-date AT 49*/ v-due-date AT 49 trim(job-hdr.est-no) FORM "x(8)" AT 64 string(v-over-pct,">9") + "%/" + STRING(v-under-pct,">9") + "%" AT 75
            "<C91.7>Status" SKIP
...
/*                 end. /* JOB-MAT */                                                  */
            ...
            /*  if last-of(eb.form-no) then do: */
                ...
                IF eb.blank-no > 0 AND eb.blank-no < 11 THEN
                  ASSIGN v-fgname[eb.blank-no] = eb.part-dscr1
                         v-fgdsc[eb.blank-no] = eb.part-dscr2.
...
/*                 FOR EACH wrk-ink:           */
/*                     /*MESSAGE               */
/*                    "icode" wrk-ink.i-code   */
/* "form" wrk-ink.form-no                      */
/* "blank" wrk-ink.blank-no                    */
/* "idsc" wrk-ink.i-dscr                       */
/* "pass" wrk-ink.i-pass                       */
/* "unt" wrk-ink.i-unit  VIEW-AS ALERT-BOX. */ */
/*                                             */
/*                 END.                        */
             ...
             /* Number of sheets Single board - oe/rep/ticket1.p, multi ticket2.p */
                ...
                */
                ...
                PUT "<P10>" v-fill SKIP                 /*REQ'D*/
                 "<B>BOARD CODE            SHEETS    SHEET SIZE             DIE SIZE      BOARD PO# VENDOR#   DUE DATE   DIE#     PREVIOUS JOB#</B>" 
                 SKIP.
            /** PRINT SHEET **/
                ...
                for each wrk-sheet break by wrk-sheet.form-no:
                    IF AVAIL po-ord THEN
                      ...
                      v-po-duedate = IF AVAIL po-ordl 
                                     ...
                                     ELSE ?.
                    IF AVAIL oe-ord THEN v-prev-job = oe-ord.po-no2.
                    display 
                        wrk-sheet.brd-dscr
                        wrk-sheet.gsh-qty FORMAT ">,>>>,>>9" SPACE(3)
                        string(wrk-sheet.sh-wid) + "x" + string(wrk-sheet.sh-len) format "x(21)"
...
/*     VIEW-AS ALERT-BOX INFO BUTTONS OK. */
            IF s-prt-mstandard THEN DO:
                IF s-run-speed THEN
                   ...
                   PUT wrk-op.m-dscr   SPACE(5)
                       ...
                       wrk-op.num-sh[job-hdr.frm] FORM ">>,>>>,>>9" SPACE(3)
                       v-output  FORM ">>,>>>,>>9"
       ...
       /* PRINTING NOTES	*/
        PUT "<#14>" v-thick   /*RDR*/
                 "<C1><B>PRINTING</B><C9>P<C12>INK<C27>DESC.<C47>LBS.<C61>P<C64>INK<C78>DESC.<C98>LBS." SKIP. /* 02/02/07 rdb SKIP(1). */
       /* PRINTING INKS  */


        /*Create table with borders*/     
        PUT "<#15><FROM><R+8><C105><RECT><||3>"
                 "<=14><C53><FROM><R+9><C53><LINE><|3>"
                 "<=14><C9><FROM><R+9><C9><LINE><||3>"
                 "<=14><C11><FROM><R+9><C11><LINE><||3>"
                 "<=14><C26><FROM><R+9><C26><LINE><||3>" 
                 "<=14><C46><FROM><R+9><C46><LINE><||3>"
                 "<=14><C61><FROM><R+9><C61><LINE><||3>"
                 "<=14><C63><FROM><R+9><C63><LINE><||3>"
                 "<=14><C77><FROM><R+9><C77><LINE><||3>"
                 "<=14><C97><FROM><R+9><C97><LINE><||3>"
                 "<=14><C105><FROM><R+9><C105><LINE><||3>"
                 ...
                 "<=15><R+7><C1><FROM><C105><LINE><||3> " SKIP.
        ...
        DO j = 1 TO 4:
             IF v-ink1[j] <> "" OR v-ink10[j] <> "" THEN DO:
                 ...
                 PUT "UNIT" j FORM ">9: " /*v-num-of-inks*/
                     v-ink1[j] FORM "x(45)" 
                     "  UNIT"  j + 4 FORM ">9: " /*v-num-of-inks*/
                     v-ink10[j] FORM "x(45)" 
                       "<C105><FROM><LINE><||3>" 
                       SKIP.
             END.
        ...
        DO j = 1 TO 4:
            IF (v-ink2[j] <> "" OR v-ink20[j] <> "" ) AND 
                v-num-of-inks < 4 THEN DO:
        ...
        */
        ...
        FOR EACH tt-ink-print:
            DELETE tt-ink-print.
        END.      
        FOR EACH wrk-ink WHERE wrk-ink.form-no EQ job-hdr.frm
                BREAK BY wrk-ink.i-pass   
                   BY wrk-ink.i-code 
                  BY wrk-ink.blank-no:
            ...
            FIND FIRST tt-ink-print WHERE tt-ink-print.i-code EQ wrk-ink.i-code
                ...
                NO-LOCK NO-ERROR.
            IF AVAIL tt-ink-print THEN tt-ink-print.qty = tt-ink-print.qty + wrk-ink.i-qty.
            ELSE DO:
                CREATE tt-ink-print.
                ASSIGN
                    tt-ink-print.i-code = wrk-ink.i-code
                    tt-ink-print.i-dscr = wrk-ink.i-dscr
                    tt-ink-print.qty = wrk-ink.i-qty
                    tt-ink-print.pass = cPass
                    tt-ink-print.unit = iUnit.
...
/*              IF wrk-ink.cnt > 12 THEN DO:            */
/*                  IF AVAIL b-rt THEN                  */
/*                      j = b-rt.val[wrk-ink.cnt - 12]. */
/*                  ELSE                                */
/*                      j = j + 1.                      */
/*              END.                                    */
/*              ELSE j = reftable.val[wrk-ink.cnt].     */
        FOR EACH tt-ink-print NO-LOCK:
            j = tt-ink-print.unit.
            IF j > 8 THEN ASSIGN i = 1 j = j - 8. 
            ELSE i = 0.
            IF j > 0  THEN DO:
                PUT  "<=14><R+" j FORMAT "9" "><C" (9 + i * 52) FORMAT "99" ">"
                    tt-ink-print.pass
                    "<C" (11 + i * 52) FORMAT "99" ">"
                    tt-ink-print.i-code FORMAT "X(15)"
                    "<C" (26 + i * 52) FORMAT "99" ">"
                    tt-ink-print.i-dscr FORMAT "x(20)"
                    "<C" (46 + i * 52) FORMAT "99" ">"
                    tt-ink-print.qty FORMAT ">,>>9.9<".
            END. /* j > 0*/
        END. /*each tt-ink-print*/
        DO j = 1 TO 8:
          DO i = 0 TO 1:
              PUT "<=14><R+" j FORMAT "9" "><C" (1 + i * 52) FORMAT "99" ">UNIT" (j + (i * 8)) FORMAT ">99:  " .
       ...
       /* rdb 02/16/07  02140714 
        ASSIGN v-text = FNformat(FNdeptnotes(vjobreckey,"GL",job-hdr.frm),80)
                /* r = r + 1 */.
        */        
        ...
        */
        ...
        for each xjob-hdr where xjob-hdr.company eq cocode
            ...
            ASSIGN tt-fgitm.i-no = xjob-hdr.i-no
                  ...
                  tt-fgitm.i-name = IF AVAIL b-eb THEN b-eb.part-dscr1 ELSE xjob-hdr.i-no
                  tt-fgitm.i-dscr = IF AVAIL b-eb THEN b-eb.part-dscr2 ELSE ""
       ...
       /* label prints per item */
        ...
        FOR EACH tt-fgitm BY tt-fgitm.seq.
            ...
            IF i >= 3 THEN DO:                
               ...
               SKIP
               "Name     :" v-fgname[1]
               "Name     :" WHEN v-fgitm[2] <> "" AT 44 v-fgname[2]  WHEN v-fgitm[2] <> "" 
               "Name     :" WHEN v-fgitm[3] <> "" AT 87  v-fgname[3] WHEN v-fgitm[3] <> "" 
               SKIP
               "Descriptn:" v-fgdsc[1]
               ...
               "Qty/Pal  :" WHEN v-fgitm[2] <> "" v-cas-pal[2]  WHEN v-fgitm[2] <> "" 
               "Qty/Case :" WHEN v-fgitm[3] <> "" AT 87  v-cas-cnt[3] WHEN v-fgitm[3] <> ""                              
               "Qty/Pal  :" WHEN v-fgitm[3] <> "" v-cas-pal[3] WHEN v-fgitm[3] <> ""                               
               SKIP
               "Batch #  :" v-batch[1]
               "Batch #  :" WHEN v-fgitm[2] <> "" AT 44 v-batch[2]  WHEN v-fgitm[2] <> ""
               "Batch #  :" WHEN v-fgitm[3] <> "" AT 87  v-batch[3] WHEN v-fgitm[3] <> ""
