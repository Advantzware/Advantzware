...
&Scoped-Define ENABLED-OBJECTS begin_i-no end_i-no btn-process btn-cancel ~
RECT-17 
...
DEFINE FRAME FRAME-A
     begin_i-no AT ROW 7.19 COL 45 COLON-ALIGNED HELP
          "Enter Beginning Raw Material Item Number"
     end_i-no AT ROW 9.1 COL 45 COLON-ALIGNED HELP
          "Enter Ending Raw Material Item Number"
...
ON END-ERROR OF C-Win /* Clear Out RM Tag#'s */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Clear Out RM Tag#'s */
DO:
...
ON LEAVE OF begin_i-no IN FRAME FRAME-A /* Beginning RM Item# */
DO:
  ASSIGN {&self-name}.
END.
...
ON LEAVE OF end_i-no IN FRAME FRAME-A /* Ending RM Item# */
DO:
  ASSIGN {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE begin_i-no end_i-no btn-process btn-cancel RECT-17 
      WITH FRAME FRAME-A IN WINDOW C-Win.
...
FOR EACH rm-rcpth
    ...
    WHERE rm-rdtlh.company   EQ rm-rcpth.company
      ...
      AND (rm-rdtlh.tag      NE "" OR rm-rdtlh.tag2 NE "")
