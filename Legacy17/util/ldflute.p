...
DEF TEMP-TABLE tt-flute LIKE flute.
DEF TEMP-TABLE tt-stkf LIKE stack-flute.
...
DEFINE FRAME Dialog-Frame
     ...
     "Enter Flute Data File Path" VIEW-AS TEXT
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Load Flute */
DO:
...
DO:
    ...
    ASSIGN lv-file.
    /* save current data */
    OUTPUT TO value("c:\tmp\flute.d" + string(TIME)) .
    FOR EACH flute NO-LOCK:
            EXPORT flute.
    END.
    OUTPUT CLOSE.
    OUTPUT TO value("c:\tmp\stkflute.d" + STRING(TIME)).
    ...
    REPEAT:
        ...
        CREATE tt-flute.
        IMPORT tt-flute.
        FIND FIRST flute WHERE flute.company = tt-flute.company
                           AND flute.code = tt-flute.CODE 
                           NO-ERROR.
        IF NOT AVAIL flute THEN DO:
           CREATE flute.
        END.
        BUFFER-COPY tt-flute TO flute.
    END.
    ...
    REPEAT:
        ...
        find first stack-flute where stack-flute.company = tt-stkf.company  
                                 AND stack-flute.loc = tt-stkf.loc
                                 AND stack-flute.code = tt-stkf.code
                                 AND stack-flute.pallet = tt-stkf.pallet
                                 AND stack-flute.page-no = tt-stkf.page-no
                        no-error.
        IF NOT AVAIL stack-flute THEN CREATE stack-flute.
        BUFFER-COPY tt-stkf TO stack-flute.
        /*
        DISP tt-stkf.company tt-stkf.loc tt-stkf.CODE
            tt-stkf.pallet tt-stkf.page-no.
        */

    END.

    SESSION:SET-WAIT-STATE("").
    MESSAGE "Flute Loading is completed." VIEW-AS ALERT-BOX.
