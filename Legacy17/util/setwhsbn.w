...
&Scoped-Define ENABLED-OBJECTS tb_0-bin tb_neg-bin btn-process btn-cancel ~
RECT-17 
&Scoped-Define DISPLAYED-OBJECTS tb_0-bin tb_neg-bin 
...
DEFINE VARIABLE tb_0-bin AS LOGICAL INITIAL no 
     LABEL "Delete Zero Quantity Bins?" 
     VIEW-AS TOGGLE-BOX
     SIZE 35 BY .95 NO-UNDO.

DEFINE VARIABLE tb_neg-bin AS LOGICAL INITIAL no 
     LABEL "Delete Negative Quantity Bins?" 
     VIEW-AS TOGGLE-BOX
     SIZE 35 BY .95 NO-UNDO.
...
DEFINE FRAME FRAME-A
     tb_0-bin AT ROW 6.48 COL 29
     tb_neg-bin AT ROW 7.91 COL 29
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Set FG Blank Warehouse/Bin"
         HEIGHT             = 11.86
         WIDTH              = 90.2
         MAX-HEIGHT         = 19.76
         MAX-WIDTH          = 98.2
...
ASSIGN 
       tb_0-bin:PRIVATE-DATA IN FRAME FRAME-A     = 
                "parm".
...
ON END-ERROR OF C-Win /* Set FG Blank Warehouse/Bin */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Set FG Blank Warehouse/Bin */
DO:
...
&Scoped-define SELF-NAME tb_0-bin
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tb_0-bin C-Win
ON VALUE-CHANGED OF tb_0-bin IN FRAME FRAME-A /* Delete Zero Quantity Bins? */
DO:
  assign {&self-name}.
END.
...
ON VALUE-CHANGED OF tb_neg-bin IN FRAME FRAME-A /* Delete Negative Quantity Bins? */
DO:
  assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY tb_0-bin tb_neg-bin 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE tb_0-bin tb_neg-bin btn-process btn-cancel RECT-17 
      WITH FRAME FRAME-A IN WINDOW C-Win.
...
for each oe-boll
  ...
  for each fg-rcpth
      ...
      where fg-rdtlh.r-no     eq fg-rcpth.r-no
        ...
        and (fg-rdtlh.loc     eq "" or
             fg-rdtlh.loc-bin eq ""):

    assign
     fg-rdtlh.loc     = v-loc
     fg-rdtlh.loc-bin = v-loc-bin.
  end.
