...
ASSIGN cTextListToSelect = "Customer,Ship-To,Item,Customer PO,Order,Unit Pr,On Hand,"
...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Customer,Ship-To,Item,Customer PO,Order,Unit Pr,On Hand,"
                         + "On Order,Allocated,Available,Reord Qty,Reord Lev,Min Ord,"
                         + "1st Recpt,R/L"  .
...
DEFINE FRAME FRAME-A
     tb_cust-list AT ROW 1.52 COL 28.8 WIDGET-ID 6
     btnCustList AT ROW 1.57 COL 60.8 WIDGET-ID 8
     begin_cust AT ROW 2.62 COL 24 COLON-ALIGNED HELP
          "Enter Beginning Customer Number"
     end_cust AT ROW 2.62 COL 67 COLON-ALIGNED HELP
          "Enter Ending Customer Number"
     begin_ship AT ROW 3.57 COL 24 COLON-ALIGNED HELP
          "Enter Beginning Ship-To Number"
     end_ship AT ROW 3.57 COL 67 COLON-ALIGNED HELP
          "Enter Ending Ship-To Number"
     begin_i-no AT ROW 4.52 COL 24 COLON-ALIGNED HELP
          "Enter Beginning Order Number"
     end_i-no AT ROW 4.52 COL 67 COLON-ALIGNED HELP
          "Enter Ending Item Number"
     begin_cat AT ROW 5.48 COL 24 COLON-ALIGNED HELP
          "Enter Beginning Category"
     end_cat AT ROW 5.48 COL 67 COLON-ALIGNED HELP
          "Enter Ending Category"
     tb_inc-qoh AT ROW 6.71 COL 26
     lbl_qoh AT ROW 7.67 COL 5 NO-LABEL
     rd_qoh AT ROW 7.67 COL 47 NO-LABEL
     as-of-date AT ROW 8.86 COL 45 COLON-ALIGNED
     lbl_pur-man AT ROW 9.91 COL 29 COLON-ALIGNED NO-LABEL
     rd_pur-man AT ROW 9.91 COL 40 NO-LABEL
     lbl_lot-reo AT ROW 10.95 COL 29 COLON-ALIGNED NO-LABEL
     rd_lot-reo AT ROW 10.95 COL 40 NO-LABEL
     sl_avail AT ROW 13 COL 4 NO-LABEL WIDGET-ID 26
     sl_selected AT ROW 13 COL 59 NO-LABEL WIDGET-ID 28
     Btn_Def AT ROW 13.1 COL 40.6 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     Btn_Add AT ROW 14.24 COL 40.6 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 15.43 COL 40.6 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 16.62 COL 40.6 WIDGET-ID 40
     btn_down AT ROW 17.81 COL 40.6 WIDGET-ID 42
     rd-dest AT ROW 20.05 COL 5 NO-LABEL
     lv-ornt AT ROW 20.29 COL 31 NO-LABEL
     lines-per-page AT ROW 20.29 COL 84 COLON-ALIGNED
     lv-font-no AT ROW 21.71 COL 34 COLON-ALIGNED
     lv-font-name AT ROW 22.67 COL 28 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 23.86 COL 30
     tb_excel AT ROW 24.81 COL 71 RIGHT-ALIGNED
     tb_runExcel AT ROW 24.81 COL 93 RIGHT-ALIGNED
     fi_file AT ROW 25.71 COL 49 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 27.33 COL 19
     btn-cancel AT ROW 27.33 COL 57
     "Available Columns" VIEW-AS TEXT
          SIZE 20 BY .62 AT ROW 12.29 COL 4.6 WIDGET-ID 38
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 12.24 COL 59 WIDGET-ID 44
     "Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.24 COL 5
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 19.33 COL 2
            ...
            ASSIGN cDisplay = ""
             ...
             DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:    
                        WHEN "cust"      THEN cVarValue = oe-rel.cust-no  .                 
                        WHEN "ship-to"   THEN cvarValue = oe-rel.ship-id  .                
                        WHEN "item"      THEN cVarValue = STRING(oe-rel.i-no) . 
                        WHEN "cust-po"   THEN cVarValue = STRING(oe-rel.po-no).  
                        WHEN "ord"       THEN cVarValue = STRING(oe-rel.ord-no) .       
                        WHEN "unit-pr"   THEN cVarValue = IF decUnitPrice NE ? THEN STRING(decUnitPrice,">>>9.99<<") ELSE "". 
                        WHEN "on-h"      THEN cvarValue = STRING(itemfg.q-onh,"->>>>>>9") . 
                        WHEN "on-ord"    THEN cVarValue = STRING(itemfg.q-ono,"->>>>>>9") .        
                        WHEN "allo"      THEN cVarValue = STRING(intAllocated,"->>>>>>>9").  
                        WHEN "avail"     THEN cvarValue = STRING(intAvailable,"->>>>>>>9")    .     
                        WHEN "reor-qty"  THEN cVarValue = STRING(v-reord-qty,">>>>>>>>9"). 
                        WHEN "reor-lev"  THEN cvarValue = STRING(itemfg.ord-level,">>>>>>>>9") . 
                        WHEN "min-ord"   THEN cVarValue = STRING(itemfg.ord-min,">>>>>>9") .      
                        WHEN "1st-rec"   THEN cVarValue = STRING(chrRecDate).  
                        WHEN "r-l"       THEN cvarValue = STRING(chrPolicy)  .     
             ...
             DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:    
                        ...
                        WHEN "unit-pr"   THEN cVarValue = IF decUnitPrice NE ? THEN STRING(decUnitPrice,">>>9.99<<") ELSE "".
                        WHEN "on-h"      THEN cvarValue = STRING(itemfg.q-onh,"->>>>>>9") . 
                        WHEN "on-ord"    THEN cVarValue = STRING(chrDummy,"->>>>>>9") .        
                        WHEN "allo"      THEN cVarValue = STRING(intAllocated,"->>>>>>>9").  
                        WHEN "avail"     THEN cvarValue = STRING(intAvailable,"->>>>>>>9")    .     
                        WHEN "reor-qty"  THEN cVarValue = STRING(v-reord-qty,">>>>>>>>9"). 
                        WHEN "reor-lev"  THEN cvarValue = STRING(itemfg.ord-level,">>>>>>>>9") . 
