/* ---------------------------------------------- ar/rep/invxargv.p  */
/* PRINT INVOICE   Xprint Form for Argrov         */
/* -------------------------------------------------------------------------- */
      ...
      IF AVAIL shipto THEN 
       ...
       ASSIGN v-del-no = 0
        ...
        for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no
         ...
         ln-cnt = ln-cnt + 2. 
         IF ar-invl.part-dscr1 ne "" OR ar-invl.part-no <> "" then ln-cnt = ln-cnt + 1.
         IF ar-invl.part-dscr2 NE "" THEN ln-cnt = ln-cnt + 1.
        end. /* each ar-invl */
        ...
        lv-tot-pg = IF ln-cnt MOD 21 = 0 THEN TRUNC( ln-cnt / 21,0)
                    ELSE lv-tot-pg + TRUNC( ln-cnt / 21,0) .  /* 41->61 21 detail lines */
...
*/
       ...
       */
       ...
       /*     if avail oe-ordl then
            ...
            assign v-bo-qty = IF AVAIL oe-ordl THEN ar-invl.qty - oe-ordl.ship-qty
                              ELSE if ( ar-invl.qty - ar-invl.ship-qty ) < 0 then 0 
                              else ar-invl.qty - ar-invl.ship-qty.

            assign v-inv-qty = ar-invl.qty /* ar-invl.inv-qty : inv qty */
  ...
  */
    ...
    PUT "<R58><C58><#8><FROM><R+5><C+22><RECT> " 
        ...
        "<=8><R+3> " /*PST        :" ar-inv.t-inv-tax FORM "->>,>>9.99"*/
                    /*v-bot-lab[2] */
        /*"<=8><R+4><C-55>Thank you for your order.*/ "<=8><R+4> Invoice Total:" ar-inv.due FORM "->>,>>9.99" .
