/* ---------------------------------------------- ar/rep/expcmemo.p   */
/* Export Credit Memo   for Container Services          */
/* -------------------------------------------------------------------------- */
...
DEF VAR v-bill-i LIKE ar-inv.bill-i NO-UNDO.
{oe/cnsexprt.i}
...
for each report NO-LOCK where report.term-id eq v-term-id
    ...
    FIRST cust NO-LOCK WHERE cust.company = ar-cash.company
                         AND cust.cust-no = ar-cash.cust-no
