/* ------------------------------------------------------- ar/rep/invptreefgl3.p */

DEF INPUT PARAM ip-copy-title AS CHAR NO-UNDO.
...
FOR EACH report 
  ...
  ASSIGN v-sold-addr3 = v-shipto-city + ", " + v-shipto-state +
                        "  " + v-shipto-zip
         v-line = 1
         v-printline = 0.
  FIND FIRST stax
  ...
  ASSIGN v-bill-i = ar-inv.bill-i[1]
  ...
  for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no
    ...
    IF v-printline > 45 THEN DO:
      ...
      {ar/rep/invptreefgl3.i}  /* xprint form */
    ...
    ASSIGN v-ord-no = ar-invl.ord-no
...
/*     /* br task 12081002 */                                */
/*     CASE svi-print-item:                                  */
/*         WHEN 1 THEN ASSIGN vc-item-no = ar-invl.i-no      */
/*                            vc-part-no = "".               */
/*         WHEN 2 THEN ASSIGN vc-item-no = ""                */
/*                            vc-part-no = ar-invl.part-no.  */
/*         OTHERWISE ASSIGN vc-item-no = ar-invl.i-no        */
/*                          vc-part-no = ar-invl.part-no.    */
/*     END CASE.                                             */

/*bpv - 12111202*/
    ASSIGN
        vc-item-no = ar-invl.i-no
        vc-part-no = ar-invl.part-no.
  ...
  PUT "<R57><C1><#7><FROM><C+80><LINE>"
      ...
      "<=7><C21>" v-inv-freight FORM ">>,>>9.99" "<C30.5>" ar-inv.tax-amt FORMAT "->>,>>9.99" v-net FORMAT "->,>>>,>>9.99" " " tmp1 "    " tmp2 " " ar-inv.gross FORMAT "->>>>,>>9.99"
