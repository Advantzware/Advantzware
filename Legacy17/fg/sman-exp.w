...
ASSIGN cTextListToSelect = "Sales Rep,Name,Basis,Commission,Net%,Sales Territory,Terr Name "

      cFieldListToSelect = "sman,sname,commbasis,scomm,netpct,territory,terrdsce "
       .
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR sman, ipc-field AS CHAR )  FORWARD.
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-sman.csv" 
...
FOR EACH b-sman WHERE b-sman.company = cocode
        AND b-sman.sman GE begin_slsmn
        AND b-sman.sman LE end_slsmn
        NO-LOCK:
    ...
    FOR EACH ttRptSelected:
        v-excel-detail-lines = v-excel-detail-lines + 
            appendXLLine(getValue-itemfg(BUFFER b-sman,ttRptSelected.FieldList)).
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    IF begin_slsmn:SCREEN-VALUE EQ "" THEN do:
        FIND FIRST sman WHERE sman.company EQ cocode NO-LOCK NO-ERROR.
            begin_slsmn:SCREEN-VALUE = sman.sman.
        FIND LAST sman WHERE sman.company EQ cocode NO-LOCK NO-ERROR.
            end_slsmn:SCREEN-VALUE   = sman.sman.
    END.


    IF lcSearch NE "" THEN DO:
        IF lcsearchby EQ "1" THEN do:
            FIND FIRST sman WHERE sman.company EQ cocode
                AND sman.sman BEGINS lcSearch NO-LOCK NO-ERROR.
            IF AVAIL sman THEN
                begin_slsmn:SCREEN-VALUE = sman.sman.
            FIND LAST sman WHERE sman.company EQ cocode
                AND sman.sman BEGINS lcSearch NO-LOCK NO-ERROR.
            IF AVAIL sman THEN
                END_slsmn:SCREEN-VALUE = sman.sman.
        ...
        ELSE DO:
            FIND FIRST sman WHERE sman.company EQ cocode
                AND sman.sname BEGINS lcSearch NO-LOCK NO-ERROR.
            IF AVAIL sman THEN
                begin_slsmn:SCREEN-VALUE = sman.sman.
            FIND LAST sman WHERE sman.company EQ cocode
                AND sman.sname BEGINS lcSearch NO-LOCK NO-ERROR.
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR sman, ipc-field AS CHAR ) :
/*------------------------------------------------------------------------------
...
------------------------------------------------------------------------------*/
    ...
    CASE ipc-field :
        WHEN "terrdsce"  THEN DO:
            FIND FIRST terr WHERE terr.company  EQ ipb-itemfg.company
                AND terr.terr     EQ ipb-itemfg.territory NO-LOCK NO-ERROR.
            IF AVAIL terr THEN
                lc-return = terr.dscr.
            ELSE
                lc-return = "".
