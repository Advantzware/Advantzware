...
&Scoped-Define ENABLED-FIELDS itemfg.box-3d-image 
...
&Scoped-Define DISPLAYED-FIELDS itemfg.box-3d-image 
...
DEFINE FRAME F-Main
     itemfg.box-3d-image AT ROW 1.48 COL 22 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 88 BY 1
...
&Scoped-define SELF-NAME itemfg.box-3d-image
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL itemfg.box-3d-image V-table-Win
ON HELP OF itemfg.box-3d-image IN FRAME F-Main /* 3D Image File */
DO:
...
------------------------------------------------------------------------------*/
  ...
  /*
   ...
   ELSE */
        IF itemfg.box-3d-image <> "" then do:
     /*  box-image:auto-resize = yes. */
     ll-dummy = box-image-2:load-image(itemfg.box-3d-image) in frame {&frame-name}.
     /*assign box-image:height-pixels = box-image:height-pixels - 10
            box-image:width-pixels =  box-image:width-pixels - 10.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL itemfg AND itemfg.box-3d-image <> "" THEN DO:

     IF itemfg.box-3d-image MATCHES "*.pdf*"  THEN DO:
         RUN custom/runapdf.p (OUTPUT lv-cmd).
     END.           
     OS-COMMAND SILENT VALUE(lv-cmd + " " + trim(lv-quote) + itemfg.box-3d-image + trim(lv-quote)).
  END.
  ll-box-refreshed = NO.
