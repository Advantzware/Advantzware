...
&SCOPED-DEFINE dataGridInclude dataGrid\browsers\datenote.i
...
DEFINE VARIABLE ip-notetype AS CHARACTER   NO-UNDO.
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH notes WHERE ~{&KEY-PHRASE} ~
 ...
 AND LOOKUP(ASI.notes.note_type, ip-notetype) GT 0 ~
 AND (LOOKUP(ASI.notes.note_code, ip-notecode) GT 0 ~
      or LOOKUP("RDC", ip-notecode) GT 0 and notes.note_code = "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* Query rebuild information for BROWSE Browser-Table
 ...
 AND LOOKUP(ASI.notes.note_type, ip-notetype) GT 0
 AND (LOOKUP(ASI.notes.note_code, ip-notecode) GT 0
      or LOOKUP(""RDC"", ip-notecode) GT 0 and ASI.notes.note_code = """")"
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(h_cont) THEN DO:
    RUN get-ip-rec_key IN h_cont (OUTPUT ip-rec_key, OUTPUT ip-header, OUTPUT ip-notecode, OUTPUT ip-notetype).    
  END.
