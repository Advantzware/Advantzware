...
&Scoped-define INTERNAL-TABLES emaildtl phone vend
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH emaildtl OF emailcod  WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      ...
      FIRST vend WHERE vend.rec_key = phone.table_rec_key NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH emaildtl OF emailcod  WHERE ~{&KEY-PHRASE} NO-LOCK, ~
      ...
      FIRST vend WHERE vend.rec_key = phone.table_rec_key NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table emaildtl phone vend
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      vend.vend-no FORMAT "x(8)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 2
      vend.name FORMAT "x(30)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 2
      phone.attention FORMAT "X(35)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 2
      phone.titlcode COLUMN-LABEL "Title" FORMAT "X(8)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 2
      vend.rec_key FORMAT "X(20)":U
      phone.rec_key FORMAT "X(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS MULTIPLE SIZE 121 BY 7.14
         FONT 2.
...
/* Query rebuild information for BROWSE Browser-Table
     _TblList          = "asi.emaildtl OF asi.emailcod ,NOSWEAT.phone WHERE asi.emaildtl ...,asi.vend WHERE NOSWEAT.phone ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "USED, USED, FIRST"
     _JoinCode[2]      = "NOSWEAT.phone.rec_key = asi.emaildtl.table_rec_key"
     _Where[2]         = "~{&KEY-PHRASE}"
     _JoinCode[3]      = "asi.vend.rec_key = NOSWEAT.phone.table_rec_key"
     _FldNameList[1]   > asi.vend.vend-no
"vend.vend-no" ? ? "character" ? ? ? 2 15 ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > asi.vend.name
"vend.name" ? ? "character" ? ? ? 2 15 ? no ? no no ? yes no no "U" "" ""
...
"vend.rec_key" ? ? "character" ? ? ? ? ? ? no ? no no ? no no no "U" "" ""
