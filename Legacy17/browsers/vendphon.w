...
&SCOPED-DEFINE dataGridInclude dataGrid\browsers\vendphon.i
&SCOPED-DEFINE BRWSDEFS emailvnd
&SCOPED-DEFINE emailTable vend
&SCOPED-DEFINE browse2 methods/browsers/phoneBrowse.i
...
&Scoped-define INTERNAL-TABLES phone vend
...
phone.attention phone.titlcode vend.rec_key phone.rec_key 
&Scoped-define ENABLED-FIELDS-IN-QUERY-Browser-Table 
&Scoped-define QUERY-STRING-Browser-Table FOR EACH phone WHERE TRUE /* Join to emailcod incomplete */ NO-LOCK, ~
      FIRST vend WHERE vend.rec_key EQ phone.table_rec_key NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH phone WHERE TRUE /* Join to emailcod incomplete */ NO-LOCK, ~
      FIRST vend WHERE vend.rec_key EQ phone.table_rec_key NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table phone vend
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD ApplyValueChanged B-table-Win 
FUNCTION ApplyValueChanged RETURNS CHARACTER
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD GetCurrVendor B-table-Win 
FUNCTION GetCurrVendor RETURNS CHARACTER
  ( /* parameter-definitions */ )  FORWARD.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      vend.vend-no FORMAT "x(8)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      vend.name FORMAT "x(30)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      phone.attention FORMAT "X(35)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      phone.titlcode COLUMN-LABEL "Title" FORMAT "X(8)":U LABEL-FGCOLOR 15 LABEL-BGCOLOR 4
      vend.rec_key FORMAT "X(20)":U
...
ASSIGN 
       vend.rec_key:VISIBLE IN BROWSE Browser-Table = FALSE
       phone.rec_key:VISIBLE IN BROWSE Browser-Table = FALSE.
...
/* Query rebuild information for BROWSE Browser-Table
     _TblList          = "NOSWEAT.phone WHERE asi.emailcod <external> ...,asi.vend WHERE NOSWEAT.phone ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = "USED, FIRST USED"
     _JoinCode[2]      = "vend.rec_key EQ phone.table_rec_key"
     _FldNameList[1]   > asi.vend.vend-no
"vend.vend-no" ? ? "character" ? ? ? 4 15 ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > asi.vend.name
"vend.name" ? ? "character" ? ? ? 4 15 ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"phone.titlcode" "Title" ? "character" ? ? ? 4 15 ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > asi.vend.rec_key
"vend.rec_key" ? ? "character" ? ? ? ? ? ? no ? no no ? no no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  ...
  {methods/run_link.i "CONTAINER-SOURCE" "setPhoneValues" "('Vendor',ROWID(vend))"}
...
------------------------------------------------------------------------------*/
  ...
  DO i = 1 TO {&BROWSE-NAME}:NUM-SELECTED-ROWS IN FRAME {&FRAME-NAME}:
    ...
    IF NOT AVAILABLE phone OR
       CAN-FIND(FIRST emaildtl WHERE emaildtl.emailcod EQ emailcod.emailcod
                AND emaildtl.table_rec_key EQ phone.rec_key) THEN NEXT.
    CREATE emaildtl.
    ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION ApplyValueChanged B-table-Win 
FUNCTION ApplyValueChanged RETURNS CHARACTER
...
------------------------------------------------------------------------------*/
  ...
  APPLY 'VALUE-CHANGED':U TO Browser-Table IN FRAME {&FRAME-NAME}.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION GetCurrVendor B-table-Win 
FUNCTION GetCurrVendor RETURNS CHARACTER
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
