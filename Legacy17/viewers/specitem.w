...
DEFINE FRAME F-Main
     item-spec.code AT ROW 2.43 COL 15 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 7 BY 1
     item-spec.notes[1] AT ROW 2.43 COL 22 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 86 BY 1
     "Description" VIEW-AS TEXT
          SIZE 21 BY .62 AT ROW 1.71 COL 26
     RECT-1 AT ROW 1 COL 1
     "Spec" VIEW-AS TEXT
          SIZE 8 BY .62 AT ROW 1.71 COL 16
...
DO:
    ...
    if lastkey <> -1 and
       can-find(bf-item-spec where bf-item-spec.company = gcompany and
                                   bf-item-spec.i-no = "" and
                                   bf-item-spec.code = self:screen-value and
                                   recid(bf-item-spec) <> recid(item-spec) )
    then do:
...
------------------------------------------------------------------------------*/
 ...
 do with frame {&frame-name} :
   if item-spec.code:screen-value = "" then do:
       message "Spec Code must be entered. " view-as alert-box error.
       apply "entry" to item-spec.code.
       return no-apply.       
    end.
    if can-find(bf-item-spec where bf-item-spec.company = gcompany and
                                   bf-item-spec.i-no = "" and
                                   bf-item-spec.code = item-spec.code:screen-value and
                                   recid(bf-item-spec) <> recid(item-spec) )
    then do:
         message "Spec Code already exists." view-as alert-box error.
         apply "entry" to item-spec.code.
