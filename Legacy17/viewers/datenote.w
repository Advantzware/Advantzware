...
DEFINE VARIABLE gvcNoteType AS CHARACTER   NO-UNDO.
DEFINE VARIABLE gvcNoteCode AS CHARACTER   NO-UNDO.
{custom/globdefs.i}
...
FOR EACH rejct-cd 
    WHERE rejct-cd.TYPE    EQ (IF LOOKUP("P", gvcNoteType) GT 0 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE display-note-type V-table-Win 
PROCEDURE display-note-type :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF AVAIL notes THEN DO:
      ...
      CASE notes.note_code:
        ...
        WHEN "DDC" THEN
          spec-desc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Due Date Change". 
        WHEN "DRC" THEN
          spec-desc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Release Due Date Change". 
        OTHERWISE 
          spec-desc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Promise Date Change" .
      END CASE.
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR cHeader AS CHAR.
  DEF VAR cType AS CHAR.
  DEF VAR cCode AS CHAR.
  DEF VAR cScreenNoteCode AS CHAR.
  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'create-record':U ) .

  /* Code placed here will execute AFTER standard behavior.    */
{methods/run_link.i "CONTAINER-SOURCE" "Get-ip-rec_key" "(OUTPUT ip-rec_key, output cHeader, output cCode, output cType)"}
ASSIGN
  ...
  CASE  cScreenNoteCode:
    WHEN "PDC" THEN notes.note_type = "P".
    WHEN "DDC" THEN notes.note_type = "D".
    WHEN "DRC" THEN notes.note_type = "L".
    OTHERWISE  notes.note_type = cScreenNoteCode.
  END CASE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE setNoteType V-table-Win 
PROCEDURE setNoteType :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT  PARAMETER ipcNoteCode AS CHARACTER   NO-UNDO.
DEFINE INPUT  PARAMETER ipcNoteType AS CHARACTER   NO-UNDO.


gvcNoteType = ipcNoteType. /* E.g. 'R' */
gvcNoteCode = ipcNoteCode. /* e.g. 'RDC' */

/* PDC uses rejct code 'R', so use 'R' until record is assigned */
IF LOOKUP("PDC", ipcNoteCode) GT 0  OR LOOKUP("DDC", ipcNoteCode) GT 0 THEN
  gvcNoteType = "R".
