...
DEF TEMP-TABLE tt-rel NO-UNDO FIELD rel-date LIKE oe-rel.rel-date.
...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Due Date,Ord Date,Job #,Ord #,Customer Name,FG Item #,"
                             + "FG Item Name,Job Qty,MSF" .
...
end_due-date sl_avail sl_selected rd-dest lv-ornt lines-per-page lv-font-no ~
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-opbdue.csv" 
...
DEFINE FRAME FRAME-A
     ...
     begin_due-date AT ROW 4.33 COL 27 COLON-ALIGNED
     end_due-date AT ROW 4.33 COL 70 COLON-ALIGNED HELP
          "Enter Ending Due Date"
     sl_avail AT ROW 6.95 COL 4 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 6.95 COL 40 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     sl_selected AT ROW 6.95 COL 59.4 NO-LABEL WIDGET-ID 28
     ...
     td-show-parm AT ROW 17.48 COL 31
     tb_excel AT ROW 19.52 COL 51 RIGHT-ALIGNED
     tb_runExcel AT ROW 19.52 COL 72 RIGHT-ALIGNED
     fi_file AT ROW 20.33 COL 29 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 22.1 COL 19
     btn-cancel AT ROW 22.1 COL 57
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 6.24 COL 4.8 WIDGET-ID 38
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 6.24 COL 59.4 WIDGET-ID 44
     "Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.24 COL 5
          BGCOLOR 2 
     "Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 13 COL 4
     RECT-6 AT ROW 12.57 COL 2
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
 ...
 /* security check need {methods/prgsecur.i} in definition section */
  ...
  DO WITH FRAME {&FRAME-NAME}:
    {methods/setButton.i btn-cancel "Cancel"} /* added by script _nonAdm1Images2.p on 04.18.2017 @ 11:37:12 am */
    {methods/setButton.i btn-ok "OK"} /* added by script _nonAdm1Images2.p on 04.18.2017 @ 11:37:12 am */
    {custom/usrprint.i}
    RUN DisplaySelectionList2.
...
def var v-fcust like job-hdr.cust-no  extent 2 init ["","zzzzzzzz"].
def var v-fdate like oe-ord.due-date  extent 2 format "99/99/99" 
                                      init [today, 12/31/99].

def var v-date as date.
def var v-msf as dec.
def var v-tot-qty as int format ">>>,>>>,>>9".
def var v-tot-msf as dec format ">>>>9.9".
...
display "" with frame r-top.
    ...
    for each tt-report,
      ...
      assign
       v-msf  = if avail itemfg then itemfg.t-sqft * job-hdr.qty / 1000
                else 0
      ...
      ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "due-date"    THEN cVarValue = IF v-date NE ? THEN string(v-date,"99/99/99") ELSE "" .
                         WHEN "ord-date"   THEN cVarValue = IF AVAIL oe-ord AND oe-ord.ord-date NE ? THEN string(oe-ord.ord-date,"99/99/99") ELSE "".
                         WHEN "job"   THEN cVarValue = trim(job-hdr.job-no) + "-" + string(job-hdr.job-no2,"99") .
