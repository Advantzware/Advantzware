...
DEFINE VARIABLE is-xprint-form AS LOG       NO-UNDO INIT YES.
...
DEFINE TEMP-TABLE score NO-UNDO
    ...
    FIELD panelLabel AS CHARACTER FORMAT 'X(12)'
    FIELD panel      AS DECIMAL   FORMAT '9.99' LABEL 'Panel'
    FIELD score      AS DECIMAL   FORMAT '-9.99' EXTENT 20
...
printScore printDesign rd-dest tbAutoClose btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_style end_style printScore ~
printDesign rd-dest tbAutoClose 
...
FUNCTION fluteValue RETURNS DECIMAL
    (ipStyle AS CHARACTER,ipFluteCode AS CHARACTER,ipCode AS CHARACTER,ipIdx AS INTEGER)  FORWARD.
...
DEFINE FRAME FRAME-A
    begin_style AT ROW 2.52 COL 31 COLON-ALIGNED HELP
    "Enter Begining Style"
    end_style AT ROW 2.52 COL 66.6 COLON-ALIGNED HELP
    "Enter Ending Style"
    printScore AT ROW 3.67 COL 33 HELP
    "Select to Print Score Values"
    printDesign AT ROW 4.86 COL 33 HELP
    "Select to Print 2D Design Image"
    lines-per-page AT ROW 6.86 COL 87 COLON-ALIGNED
    lv-font-no AT ROW 6.95 COL 37 COLON-ALIGNED
    lv-ornt AT ROW 6.95 COL 46 NO-LABELS
    rd-dest AT ROW 7 COL 4.8 NO-LABELS
    lv-font-name AT ROW 7.95 COL 29 COLON-ALIGNED NO-LABELS
    tb_batch AT ROW 9.67 COL 60
    td-show-parm AT ROW 10.57 COL 29.2
    tbAutoClose AT ROW 11.86 COL 29.2 WIDGET-ID 64
    btn-ok AT ROW 12.91 COL 29
    btn-cancel AT ROW 12.91 COL 52
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 6.24 COL 4
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.48 COL 4
    RECT-7 AT ROW 1.95 COL 3
    RECT-8 AT ROW 6.67 COL 3
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "EB3" }
    ASSIGN
...
PROCEDURE buildScore :
    ...
    FOR EACH flute NO-LOCK WHERE flute.company EQ g_company:
        DO idx = 2 TO NUM-ENTRIES(scoreLabel):
            ...
            IF AVAILABLE reftable AND reftable.val[13] NE 0 THEN 
            DO:
                CREATE score.
                ASSIGN
...
PROCEDURE enable_UI :
    ...
    {&OPEN-QUERY-FRAME-A}
    GET FIRST FRAME-A.
    DISPLAY begin_style end_style printScore printDesign rd-dest tbAutoClose 
    ...
    *****************************************************************************
    ...
    FOR EACH style NO-LOCK WHERE style.company EQ g_company
        ...
        DISPLAY
            ...
            WHEN AVAIL(box-design-hdr) FORMAT 'X(55)'
        ...
        IF printDesign AND AVAILABLE box-design-hdr AND
            SEARCH(box-design-hdr.box-image) NE ? THEN 
        DO:
            imageFound = YES.          
            PUT UNFORMATTED 
                '<#1><C1><FROM><C80><R+20><RECT><||3>'  SKIP
            ...
            FOR EACH score NO-LOCK BREAK BY score.flute WITH STREAM-IO WIDTH 100 NO-BOX:
                ...
                DO:
                    DISPLAY
                        ...
                        style.dim-tk FORMAT '9.99' 
                        WHEN style.dim-tk NE 0
                        style.dim-pan5 FORMAT '9.99' 
                        WHEN style.dim-pan5 NE 0
                        style.dim-fit FORMAT '9.99' LABEL 'Lock Tab' 
                        WHEN style.dim-fit NE 0
                        WITH FRAME flute STREAM-IO WIDTH 100 SIDE-LABELS.
                    iLineCount = iLineCount + 4.   
                END.    
        
                IF iLineCount GE 70  THEN 
                DO:
                    PAGE.     
                    imageFound = NO.
                    iLineCount = 0.
                ...
                DISPLAY
                    score.panelLabel NO-LABELS
                    score.panel
                    score.score[1] LABEL '1' 
                    WHEN score.score[1] NE 0
                    score.score[2] LABEL '2' 
                    WHEN score.score[2] NE 0
                    score.score[3] LABEL '3' 
                    WHEN score.score[3] NE 0
                    score.score[4] LABEL '4' 
                    WHEN score.score[4] NE 0
                    score.score[5] LABEL '5' 
                    WHEN score.score[5] NE 0
                    score.score[6] LABEL '6' 
                    WHEN score.score[6] NE 0
                    score.score[7] LABEL '7' 
                    WHEN score.score[7] NE 0
                    score.score[8] LABEL '8' 
                    WHEN score.score[8] NE 0
                    score.score[9] LABEL '9' 
                    WHEN score.score[9] NE 0
                    score.score[10] LABEL '10' 
                    WHEN score.score[10] NE 0
                    score.score[11] LABEL '11' 
                    WHEN score.score[11] NE 0
                    score.score[12] LABEL '12' 
                    WHEN score.score[12] NE 0
                    score.score[13] LABEL '13' 
                    WHEN score.score[13] NE 0.
...
FUNCTION fluteValue RETURNS DECIMAL
    (ipStyle AS CHARACTER,ipFluteCode AS CHARACTER,ipCode AS CHARACTER,ipIdx AS INTEGER) :
    /*------------------------------------------------------------------------------
      Purpose:  
        Notes:  
    ------------------------------------------------------------------------------*/
