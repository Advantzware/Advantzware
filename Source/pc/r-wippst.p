...
end_shift begin_loc end_loc tb_tot-hrs tb_pg-brk lv-ornt lines-per-page ~
...
begin_loc end_loc tb_tot-hrs tb_pg-brk lv-ornt lines-per-page rd-dest ~
...
DEFINE FRAME FRAME-A
     ...
     end_shift AT ROW 5.52 COL 69 COLON-ALIGNED HELP
          "Enter Beginning Machine Number"
     begin_loc AT ROW 6.48 COL 26 COLON-ALIGNED HELP
          "Enter Beginning Machine Number" WIDGET-ID 8
     end_loc AT ROW 6.48 COL 69 COLON-ALIGNED HELP
          "Enter Ending Machine Number" WIDGET-ID 10
     tb_tot-hrs AT ROW 8.48 COL 31
     tb_pg-brk AT ROW 9.67 COL 65 RIGHT-ALIGNED
...
------------------------------------------------------------------------------*/
  DISPLAY begin_date end_date begin_mach end_mach begin_job-no begin_job-no2 
          ...
          tb_tot-hrs tb_pg-brk lv-ornt lines-per-page rd-dest lv-font-no 
  ...
  ENABLE RECT-6 RECT-7 begin_date end_date begin_mach end_mach begin_job-no 
         ...
         end_loc tb_tot-hrs tb_pg-brk lv-ornt lines-per-page rd-dest lv-font-no 
    ...
    for each work-gl 
        ...
        do:              
          RUN GL_SpCreateGLHist(cocode,
                               ...
                               "A",
                               TODAY,
                               (IF work-gl.job-no NE "" THEN "Job:" + STRING(work-gl.job-no) + "-" + STRING(work-gl.job-no2) ELSE ""),
                               "WIP"). 
            assign
                debits  = 0
                credits = 0.
...
/* --------------------------------------------------- pc/pc-post.p  8/94 gb  */
    ...
    FOR EACH tt-report NO-LOCK,
      ...
      and mach.m-code eq pc-prdd.m-code
        ...
        if avail est and INDEX("AP",mach.p-type) LE 0 then 
        do:
            ...
            if ((avail est-op) and est-op.op-sb)           or
                ((not avail est-op) and mach.p-type ne "B") then 
            do:
    ...
    */
    ...
    IF pc-prdd.qty > 0 OR v-runqty > 0 THEN
        FOR EACH reftable
            ...
            if avail est and INDEX("APB",mach.p-type) LE 0 then 
            do:
                run sys/inc/numup.p (est.company, est.est-no, job-mch.frm, output v-up).
    ...
    */
    ...
    IF pc-prdd.qty > 0 OR v-runqty > 0 THEN
        ...
        FOR EACH job-hdr
            ...
            if avail est and index("AB",mach.p-type) le 0 then 
            do:
                run sys/inc/numup.p (est.company, est.est-no, job-mch.frm, output v-up).
...
/* -------------------------------------------------------------------------- */
    ...
    def var cLoc AS CHARACTER extent 2 init ["","zzzzz"] no-undo. 
