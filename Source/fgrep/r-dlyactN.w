...
ASSIGN 
    cTextListToSelect  = "Cust #,Item #,Description,Beg Balance,Dly Rec/Adj,"
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Cust #,Item #,Description,Beg Balance,Dly Rec/Adj,"
                         + "Dly Shpmnts,End Balance,Day,Count1,Count2" .
...
end_cust-no begin_i-no end_i-no sl_avail sl_selected rd-dest fi_file ~
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\FGDailyActivity.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    sl_selected AT ROW 8.38 COL 59.8 NO-LABELS WIDGET-ID 28
    ...
    td-show-parm AT ROW 18.24 COL 28
    fi_file AT ROW 19.24 COL 26.2 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 19.38 COL 92.6 RIGHT-ALIGNED
    tbAutoClose AT ROW 20.52 COL 28 WIDGET-ID 58
    btn-ok AT ROW 21.48 COL 28
    btn-cancel AT ROW 21.48 COL 53
    "Available Columns" VIEW-AS TEXT
    SIZE 20 BY .95 AT ROW 7.43 COL 3 WIDGET-ID 38
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 14.76 COL 4.4
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.33 COL 4.4
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 7.57 COL 60 WIDGET-ID 44
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "IL7" }
    ASSIGN
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE vdat           AS DATE      FORMAT "99/99/9999" INIT TODAY.
    DEFINE VARIABLE fitm           LIKE itemfg.i-no NO-UNDO.
    DEFINE VARIABLE titm           LIKE fitm INIT "zzzzzzzzzzzzzzz" NO-UNDO.
    DEFINE VARIABLE fcus           LIKE itemfg.cust-no INIT "".
    DEFINE VARIABLE tcus           LIKE fcus INIT "zzzzzzzz".

    DEFINE VARIABLE v-recadj       AS INTEGER   EXTENT 2 NO-UNDO.
    DEFINE VARIABLE v-shipments    AS INTEGER   EXTENT 2 NO-UNDO.
    DEFINE VARIABLE v-qty-onh      AS INTEGER   EXTENT 4 NO-UNDO.
    DEFINE VARIABLE v-end-qty      AS INTEGER   EXTENT 2 NO-UNDO.
    DEFINE VARIABLE v-cday         AS CHARACTER INIT "" NO-UNDO.
    ...
    FOR EACH itemfg
        ...
        FOR EACH fg-rcpth
            ...
            IF LAST-OF(fg-rdtlh.tag) THEN 
            DO:
                IF fg-rdtlh.loc EQ "CUST" THEN 
                DO:
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            ...
            CASE cTmpField:    
                WHEN "cust"     THEN 
                    cVarValue = IF FIRST-OF(itemfg.cust-no) THEN itemfg.cust-no ELSE ""  .                 
                WHEN "ino"      THEN 
                    cvarValue = itemfg.i-no  .                
                WHEN "dscr"     THEN 
                    cVarValue = STRING(itemfg.i-name) . 
                WHEN "beg-bal"  THEN 
                    cVarValue = STRING(v-qty-onh[3],"->,>>>,>>9")    .
                WHEN "dly-rec"  THEN 
                    cVarValue = STRING(v-qty-onh[4],"->,>>>,>>9")  .  
                WHEN "dly-shp"  THEN 
                    cVarValue = STRING(v-shipments[2],"->,>>>,>>9")  .
                WHEN "end-bal"  THEN 
                    cvarValue = STRING(v-end-qty[2],"->,>>>,>>9") .   
                WHEN "day"      THEN 
                    cVarValue = STRING(v-cday,"x") .                  
                WHEN "cnt1"     THEN 
                    cVarValue = "___________"     .                   
                WHEN "cnt2"     THEN 
                    cvarValue = "___________"   .                     
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\FGDailyActivity.csv".    
    END.
