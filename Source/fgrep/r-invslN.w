...
ASSIGN 
    cTextListToSelect  = "SRep,Rep Name,Item,Description,Cust #,Customer Name,Cust PO#," +
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "SRep,Rep Name,Item,Description,Cust #,Cust PO#," +
                           "Cust Job#,Qty Prod.,Qty Ship,Qty On Hand," +
                           "Total Cost,Total Value".
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\FGInventoryBySales.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    begin_cust AT ROW 3.05 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Customer Number"
    end_cust AT ROW 3.05 COL 71 COLON-ALIGNED HELP
    "Enter Ending Customer Number"
    begin_cust-po AT ROW 4 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Customer PO Number"
    end_cust-po AT ROW 4 COL 71 COLON-ALIGNED HELP
    "Enter Ending Customer PO Number"
    begin_slm AT ROW 4.95 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Sales Rep Number"
    end_slm AT ROW 4.95 COL 71 COLON-ALIGNED HELP
    "Enter Ending Sales Rep Number"
    tb_inc-zer AT ROW 6.14 COL 30.8
    tb_inc-cust AT ROW 7.05 COL 73.8 RIGHT-ALIGNED
    lbl_sort AT ROW 8.19 COL 23.8 COLON-ALIGNED NO-LABELS
    rd_sort AT ROW 8.19 COL 47.8 NO-LABELS
    sl_avail AT ROW 10.29 COL 4 NO-LABELS WIDGET-ID 26
    sl_selected AT ROW 10.29 COL 60.6 NO-LABELS WIDGET-ID 28
    Btn_Def AT ROW 10.33 COL 40.8 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    Btn_Add AT ROW 11.29 COL 40.8 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 12.24 COL 40.8 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 13.19 COL 40.8 WIDGET-ID 40
    btn_down AT ROW 14.14 COL 40.8 WIDGET-ID 42
    lv-ornt AT ROW 16.19 COL 30 NO-LABELS
    lines-per-page AT ROW 16.19 COL 85 COLON-ALIGNED
    rd-dest AT ROW 16.57 COL 6.2 NO-LABELS
    lv-font-no AT ROW 17.1 COL 34 COLON-ALIGNED
    lv-font-name AT ROW 18.05 COL 28 COLON-ALIGNED NO-LABELS
    td-show-parm AT ROW 19.33 COL 28.2
    fi_file AT ROW 20.43 COL 26.2 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 20.52 COL 92.4 RIGHT-ALIGNED
    tbAutoClose AT ROW 21.95 COL 28.2 WIDGET-ID 58
    btn-ok AT ROW 22.91 COL 28.2
    btn-cancel AT ROW 22.91 COL 48.2
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 9.57 COL 4.2 WIDGET-ID 38
    " Output Destination" VIEW-AS TEXT
    SIZE 19 BY .62 AT ROW 15.71 COL 5
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21.2 BY .71 AT ROW 1.14 COL 5
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 9.62 COL 60.2 WIDGET-ID 44
    RECT-11 AT ROW 1.52 COL 4
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {methods/nowait.i}
    {sys/inc/reportsConfigNK1.i "IR5" }
    ASSIGN
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE tcust       LIKE fcust INIT "ZZZZZZZZZ".
    DEFINE VARIABLE fcustpo     AS ch        INIT " ".
    DEFINE VARIABLE tcustpo     LIKE fcustpo INIT "ZZZZZZZZZZZZZZZ".
    DEFINE VARIABLE fsman       LIKE cust.sman INIT " ".
    DEFINE VARIABLE tsman       LIKE cust.sman INIT "ZZZ".
    DEFINE VARIABLE zbal        AS LOG       FORMAT "Y/N".
    DEFINE VARIABLE v-sub-ext   LIKE v-ext.
    DEFINE VARIABLE v-cust-ext  LIKE v-ext.
    DEFINE VARIABLE tot-string  AS CHARACTER FORMAT "x(5)" INIT "Total".
    DEFINE VARIABLE v-cost      LIKE itemfg.total-std-cost.
    DEFINE VARIABLE v-qty-onh   AS INTEGER   FORMAT "->>>,>>>,>>9".
    DEFINE VARIABLE v-totcost   AS DECIMAL   FORMAT "->>>,>>9.99".
    DEFINE VARIABLE v-frst      AS LOG.
    DEFINE VARIABLE v-sel-one   AS CHARACTER FORMAT "x(30)".
    DEFINE VARIABLE v-sel-two   AS CHARACTER FORMAT "x(33)".
    DEFINE VARIABLE v-sel-three AS CHARACTER FORMAT "x(20)".
    DEFINE VARIABLE v-custown   AS LOG       FORMAT "Y/N" INIT "N".
    DEFINE VARIABLE v-cust-qty  AS INTEGER   FORMAT "->>>,>>>,>>9".
    DEFINE VARIABLE v-sales-rep AS CHARACTER NO-UNDO.
    DEFINE VARIABLE sort-opt    AS CHARACTER NO-UNDO INIT "C" FORMAT "!".
    ...
    FOR EACH ttRptSelected BY ttRptSelected.DisplayOrder:
        IF LOOKUP(ttRptSelected.TextList, "Total Cost") <> 0    THEN 
        DO:
    ...
    FOR EACH itemfg
        ...
        IF v-sales-rep GE fsman  AND v-sales-rep LE tsman THEN 
        DO:
            CREATE tt-itemfg.
            ASSIGN
    ...
    FOR EACH tt-itemfg WHERE 
                ...
                **/
                ...
                DO:
                    ...
                    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                        ...
                        CASE cTmpField:             
                            WHEN "sman"     THEN 
                                cVarValue = IF AVAILABLE sman THEN STRING(tt-itemfg.sales-rep) ELSE " " .
                            WHEN "sname"    THEN 
                                cVarValue = IF AVAILABLE sman THEN STRING(sman.sname,"x(20)") ELSE " " .
                            WHEN "itm"      THEN 
                                cVarValue = STRING(itemfg.i-no,"x(15)").
                            WHEN "dscr"     THEN 
                                cVarValue = STRING(itemfg.i-name,"x(20)") .
                            WHEN "cust"     THEN 
                                cVarValue = STRING(itemfg.cust-no,"x(8)") .
                            WHEN "cust-nam" THEN 
                                cVarValue = STRING(cust.NAME,"x(30)")  .
                            WHEN "cust-po"  THEN 
                                cVarValue = STRING(itemfg.cust-po-no,"x(9)").
                            WHEN "cust-job" THEN 
                                cVarValue = STRING(itemfg.cust-job-no,"x(9)").
                            WHEN "qty-pro"  THEN 
                                cVarValue = STRING(itemfg.q-prod-ptd,"->>,>>>,>>9")  .
                            WHEN "qty-ship" THEN 
                                cVarValue = STRING(itemfg.q-ship-ptd,"->>,>>>,>>9") .
                            WHEN "qty-onh"  THEN 
                                cVarValue = IF v-qty-onh NE ? THEN STRING(v-qty-onh,"->>,>>>,>>9") ELSE "". 
                            WHEN "ttl-cst"  THEN 
                                cVarValue = IF prt-cost AND v-totcost NE ? THEN STRING(v-totcost,"->>,>>>,>>9.99") ELSE "".
                            WHEN "ttl-val"  THEN 
        ...
        END. /* each bin */
        IF LAST-OF
            (IF sort-opt EQ "C" THEN itemfg.cust-no 
        ELSE itemfg.i-no)
        ...
        DO: /* 9508 CAH */
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField:             
                    ...
                    WHEN "ttl-cst"  THEN 
                        cVarValue = " " .
                    WHEN "ttl-val"  THEN 
                        cVarValue = STRING(v-cust-ext,"->>,>>>,>>9.99") .
            ...
            PUT UNFORMATTED 
                "   CUSTOMER Totals" SUBSTRING(cDisplay,19,300) SKIP(1).
        ...
        DO:
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField:             
                    ...
                    WHEN "ttl-cst"  THEN 
                        cVarValue = " " .
                    WHEN "ttl-val"  THEN 
                        cVarValue = STRING(v-sub-ext,"->>,>>>,>>9.99") .
            ...
            PUT UNFORMATTED 
                "   SALESREP Totals" SUBSTRING(cDisplay,19,300) SKIP(1).
            /*IF rd-dest EQ 3 THEN DO:
                 PUT STREAM excel UNFORMATTED  
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\FGInventoryBySales.csv".   
    END.
