...
/* 10 + 6 + 8 = 24 columns */
ASSIGN 
    cTextListToSelect  = "DATE,ITEM,DESCRIPTN,PO#,JOB#,VENDOR#,TRANSACTION TYPE,TAG#,RFID#,UNITS," +
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "DATE,ITEM,DESCRIPTN,PO#,VENDOR#,TRANSACTION TYPE,TAG#,UNITS," +
                           "COUNT,BIN,CUOM,TOTAL QTY,TOTAL COST,TOT SELL VAL" .
...
tb_adj tb_ret tb_count tb_total sl_avail sl_selected rd-dest fi_file ~
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\FGPostingHistory.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    tb_rec AT ROW 7.48 COL 37.2
    tb_ship AT ROW 8.19 COL 37.2
    rsShowVendor AT ROW 8.86 COL 63.2 NO-LABELS WIDGET-ID 14
    tb_tran AT ROW 8.95 COL 37.2
    tb_adj AT ROW 9.67 COL 37.2
    rsShowTag AT ROW 10.05 COL 63.2 NO-LABELS WIDGET-ID 18
    tb_ret AT ROW 10.38 COL 37.2
    tb_count AT ROW 11.1 COL 37.2
    tb_total AT ROW 11.81 COL 37.2
    Btn_Def AT ROW 13.76 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_avail AT ROW 13.81 COL 3 NO-LABELS WIDGET-ID 26
    sl_selected AT ROW 13.81 COL 63.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 14.76 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 15.76 COL 40.4 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 16.81 COL 40.4 WIDGET-ID 40
    btn_down AT ROW 17.86 COL 40.4 WIDGET-ID 42
    lv-font-no AT ROW 19.57 COL 35 COLON-ALIGNED
    lv-ornt AT ROW 19.57 COL 45 NO-LABELS
    ...
    td-show-parm AT ROW 21.95 COL 27
    fi_file AT ROW 22.95 COL 25 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 23.05 COL 92 RIGHT-ALIGNED
    tbAutoClose AT ROW 24.38 COL 27.2 WIDGET-ID 58
    btn-cancel AT ROW 25.29 COL 55
    btn-ok AT ROW 25.38 COL 27
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 13.1 COL 3 WIDGET-ID 38
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1 COL 4.2
    BGCOLOR 15 
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 18.95 COL 4.2
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 13.1 COL 61.2 WIDGET-ID 44
    "Transaction Types" VIEW-AS TEXT
    SIZE 22 BY .62 AT ROW 7.38 COL 13.2
    FONT 6
...
ASSIGN 
    begin_user:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    end_user:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    ...
    rsShowTag:PRIVATE-DATA IN FRAME FRAME-A = "rsShowTag,parm".

/* SETTINGS FOR RADIO-SET rsShowVendor IN FRAME FRAME-A
   NO-DISPLAY NO-ENABLE                                                 */
ASSIGN 
    rsShowVendor:HIDDEN IN FRAME FRAME-A       = TRUE
    rsShowVendor:PRIVATE-DATA IN FRAME FRAME-A = "rsShowVendor,parm".

ASSIGN 
    tb_adj:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_count:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_cust-list:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_rec:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    tb_ship:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_total:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_tran:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
PROCEDURE calc-case-and-tag :
    ...
    IF bf-fg-rdtlh.qty-case EQ 0 THEN 
    DO:
        FIND FIRST fg-bin
...
PROCEDURE enable_UI :
    ...
    DISPLAY from_date to_date tb_cust-list begin_cust end_cust begin_i-no end_i-no 
        ...
        tb_total sl_avail sl_selected rd-dest fi_file tb_OpenCSV tbAutoClose 
...
PROCEDURE run-report :
    ...
    */
    ...
    IF rd-dest = 3 THEN PUT STREAM excel UNFORMATTED '"' REPLACE(excelheader,',','","') '"' SKIP.
    ...
    FOR EACH tt-report WHERE tt-report.term-id EQ "" NO-LOCK,
        ...
        DO:
            ...
            DISPLAY fg-rcpth.trans-date     /*  when first-of(tt-report.key-02) */
                ...
                fg-rcpth.po-no
                po-ord.vend-no            
                WHEN AVAILABLE po-ord AND fg-rcpth.po-no <> ""                
                v-tran-type
                v-tag @ fg-rdtlh.tag
                v-cases
                v-qty-case
        ...
        END.
        ELSE IF rsShowVendor = "Job#" AND rsShowTag = "Tag#" THEN 
            DO:
                ...
                /* po-ord.vend-no            when avail po-ord AND fg-rcpth.po-no <> "" */
                    fg-rcpth.job-no + "-" + string(fg-rcpth.job-no2) 
                    WHEN fg-rcpth.job-no GT "" @ fg-rcpth.job-no
                    v-tran-type
                    v-tag @ fg-rdtlh.tag
                    v-cases
                    v-qty-case
            ...
            END.
            ELSE IF rsShowVendor = "Vendor" AND rsShowTag = "RFID#" THEN 
                DO:
                    ...
                    DISPLAY fg-rcpth.trans-date     /*  when first-of(tt-report.key-02) */
                        ...
                        WHEN AVAILABLE po-ord AND fg-rcpth.po-no <> ""                     
                        v-tran-type
                        SUBSTRING(rfidtag.rfidtag, 13)  
                        WHEN AVAIL(rfidtag) @ rfidtag.rfidtag
                        v-cases
                        v-qty-case
                        v-fg-qty            /* (sub-total by fg-rcpth.i-no) */
                        fg-rdtlh.loc-bin
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\FGPostingHistory.csv".    
    END.
