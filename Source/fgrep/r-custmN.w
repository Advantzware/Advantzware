...
/*DEF TEMP-TABLE tt-report LIKE report
    ...
    INDEX row-id row-id.
*/
DEFINE TEMP-TABLE tt-fg-bin LIKE fg-bin.
DEFINE STREAM excel.
...
ASSIGN 
    cTextListToSelect  = "Cust #,P.O. NUMBER,ITEM #,CUST PART #,DESCRIPTION,JOB NUMBER,QTY ON-HAND,PALLETS,SELL PRICE," +
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Cust #,P.O. NUMBER,ITEM #,DESCRIPTION,JOB NUMBER,QTY ON-HAND,PALLETS,SELL PRICE," +
...
fi_days-old lbl_srt rd_ascdsc sl_avail sl_selected rd-dest fi_file ~
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\FGSalesByCustomer.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    lbl_srt AT ROW 9.48 COL 3 COLON-ALIGNED NO-LABELS
    rd_ascdsc AT ROW 9.52 COL 29.6 NO-LABELS
    sl_avail AT ROW 11.71 COL 4 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 11.71 COL 40.6 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 11.71 COL 60.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 12.71 COL 40.6 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 13.71 COL 40.6 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 14.76 COL 40.6 WIDGET-ID 40
    btn_down AT ROW 15.76 COL 40.6 WIDGET-ID 42
    lv-ornt AT ROW 17.91 COL 29 NO-LABELS
    lines-per-page AT ROW 17.91 COL 82 COLON-ALIGNED
    rd-dest AT ROW 18.29 COL 6 NO-LABELS
    lv-font-no AT ROW 18.76 COL 32 COLON-ALIGNED
    lv-font-name AT ROW 19.71 COL 26 COLON-ALIGNED NO-LABELS
...
/* SETTINGS FOR FILL-IN lbl_srt IN FRAME FRAME-A
   NO-ENABLE                                                            */
ASSIGN 
    lbl_srt:PRIVATE-DATA IN FRAME FRAME-A = "rd_ascdsc".
                ...
                DO:
                    ...
                    DO:        
                        MESSAGE "CSV file have been created." SKIP(1)
                            ...
                            TITLE "" UPDATE lChoice1 AS LOGICAL.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {methods/nowait.i}
    {sys/inc/reportsConfigNK1.i "IR6" }
    ASSIGN
...
PROCEDURE calc-qoh :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER op-qty AS DECIMAL.
...
PROCEDURE first-date :
    ...
    DEFINE INPUT-OUTPUT PARAMETER io-date AS DATE NO-UNDO.

    DEFINE BUFFER b-bin   FOR fg-bin.
    ...
    IF AVAILABLE b-rcpth THEN io-date = b-rcpth.trans-date.
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE vdat        AS DATE      INIT TODAY FORMAT "99/99/9999" NO-UNDO.
    DEFINE VARIABLE fcus        LIKE itemfg.cust-no NO-UNDO.
    DEFINE VARIABLE tcus        LIKE fcus INIT "zzzzzzzz" NO-UNDO.
    DEFINE VARIABLE fpo         LIKE oe-ordl.po-no NO-UNDO.
    DEFINE VARIABLE tpo         LIKE fpo INIT "zzzzzzzzzzzzzzz" NO-UNDO.
    DEFINE VARIABLE fsls        LIKE cust.sman NO-UNDO.
    DEFINE VARIABLE tsls        LIKE fsls INIT "zzz" NO-UNDO.
    DEFINE VARIABLE vzer        AS LOG       FORMAT "Y/N" INIT NO NO-UNDO.
    DEFINE VARIABLE vwhs        LIKE vzer NO-UNDO.
    DEFINE VARIABLE vpcp        LIKE vzer NO-UNDO.
    DEFINE VARIABLE vdue        LIKE vzer FORMAT "DueDate/OrderDate" NO-UNDO.
    ...
    DEFINE VARIABLE v-ext       AS DECIMAL   EXTENT 3 NO-UNDO.
    DEFINE VARIABLE v-qty       AS DECIMAL   EXTENT 3 NO-UNDO.
    DEFINE VARIABLE v-date      AS DATE      NO-UNDO.
    DEFINE VARIABLE v-rct-date  AS DATE      NO-UNDO.
    DEFINE VARIABLE v-pallets   AS DECIMAL   NO-UNDO.
    DEFINE VARIABLE v-frst-date AS DATE      NO-UNDO.
    DEFINE VARIABLE v-ship-date AS DATE      NO-UNDO.
    DEFINE VARIABLE lv-stat     AS CHARACTER NO-UNDO.
    DEFINE VARIABLE v-date2     AS DATE.
    DEFINE VARIABLE v-date3     AS DATE.
    DEFINE VARIABLE vjob-no     AS CHARACTER NO-UNDO.
    ...
    DEFINE VARIABLE cSlsREp AS CHARACTER.
        ...
        FOR EACH fg-bin
            ...
            IF fg-bin.tag NE "" THEN
                FOR EACH fg-rdtlh NO-LOCK
                    ...
                    IF LAST-OF(fg-rdtlh.cust-no) THEN 
                    DO:
                        ASSIGN
                        ...
                        v-date = IF AVAILABLE oe-ordl AND vdue THEN oe-ordl.req-date
                        ELSE IF AVAILABLE oe-ord THEN oe-ord.ord-date 
                        ELSE ?.                 
                        ...
                        ASSIGN
                            ...
                            tt-report.key-06    = IF AVAILABLE oe-ordl THEN STRING(oe-ordl.req-date) ELSE ""
                            tt-report.key-07    = IF AVAILABLE oe-ord THEN  STRING(oe-ord.ord-date) ELSE "".
                    END.
                END.
...
form cust.cust-no column-label "CUSTOMER!   ID"
     ...
     space(0) "-" space(0) fg-bin.job-no2 column-label "" format "999"
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\FGSalesByCustomer.csv".   
    END.
