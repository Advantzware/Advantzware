...
DEFINE VARIABLE v-invalid      AS LOG       INIT NO NO-UNDO.
DEFINE VARIABLE v-print-fmt    AS CHARACTER NO-UNDO.
DEFINE VARIABLE is-xprint-form AS LOGICAL.
DEFINE VARIABLE ls-fax-file    AS CHARACTER NO-UNDO.

/* gdm - 10010905 */
DEFINE STREAM str-exl.
...
ASSIGN 
    cTextListToSelect  = "Account Number,Description,Run #,Journal,Reference,Date," +
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Account Number,Description,Run #,Journal,Reference,Date," +
                           "Debits,Credits,Balance,Jrnl#"  .
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)":U INITIAL "c:~\tmp~\LedgerSummary.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    tran-date AT ROW 2.19 COL 46.4 COLON-ALIGNED
    tran-period AT ROW 3.38 COL 46.4 COLON-ALIGNED
    begin_acct AT ROW 5 COL 19.4 COLON-ALIGNED HELP
    "Enter Beginning Account Number"
    end_acct AT ROW 5 COL 63.4 COLON-ALIGNED HELP
    "Enter Ending Account Number"
    sl_avail AT ROW 7.1 COL 3 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 7.1 COL 40 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 7.1 COL 60.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 8.1 COL 40 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 9.1 COL 40 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 10.14 COL 40 WIDGET-ID 40
    btn_down AT ROW 11.19 COL 40 WIDGET-ID 42
    rd-dest AT ROW 12.91 COL 4.4 NO-LABELS
    lv-font-no AT ROW 12.91 COL 35 COLON-ALIGNED
    lv-ornt AT ROW 12.91 COL 44 NO-LABELS
    lines-per-page AT ROW 12.91 COL 87 COLON-ALIGNED
    lv-font-name AT ROW 13.95 COL 29 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 15.05 COL 72.4 WIDGET-ID 2
    td-show-parm AT ROW 15.67 COL 28
    fi_file AT ROW 16.57 COL 26 COLON-ALIGNED HELP
    "Enter File Name" WIDGET-ID 6
    tb_OpenCSV AT ROW 16.62 COL 77.4 WIDGET-ID 4
    tbAutoClose AT ROW 17.91 COL 28 WIDGET-ID 64
    btn-ok AT ROW 18.86 COL 27.8
    btn-cancel AT ROW 18.86 COL 51
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 6.48 COL 3 WIDGET-ID 38
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 6.48 COL 60.8 WIDGET-ID 44
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "GR2" }
    ASSIGN
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE tacct          LIKE glhist.actnum LABEL "    To Account Number".
    DEFINE VARIABLE facct          LIKE glhist.actnum LABEL "  From Account Number".
    DEFINE VARIABLE op             AS CHARACTER FORMAT "!" INIT "S" LABEL "  S)ummary or D)etail?".
    DEFINE VARIABLE inc            AS LOG       INIT NO LABEL "  Print Accounts with NO Activity?".

    DEFINE VARIABLE suppress_zero  AS LOGICAL   NO-UNDO INIT TRUE
        LABEL "Suppress Zero Balances?".

    DEFINE VARIABLE tot-all        AS DECIMAL   FORMAT "->>>,>>>,>>>,>>9.99".
    DEFINE VARIABLE tot-tx         LIKE tot-all.
    DEFINE VARIABLE tot-act        LIKE tot-all.
    DEFINE VARIABLE open-amt       LIKE tot-all.
    DEFINE VARIABLE net-inc        AS DECIMAL.
    DEFINE VARIABLE tmp-amt        LIKE glhist.tr-amt FORMAT "(>>>,>>>,>>9.99)".
    DEFINE VARIABLE tmp-dscr       LIKE glhist.tr-dscr.
    DEFINE VARIABLE v-crdit        LIKE tmp-amt EXTENT 3.
    DEFINE VARIABLE v-debit        LIKE v-crdit.
    DEFINE VARIABLE v-jrnl-no      AS CHARACTER NO-UNDO.

    DEFINE VARIABLE vyear          LIKE period.yr NO-UNDO.
    DEFINE VARIABLE v-tot-bal      AS DECIMAL   NO-UNDO.

    /* gdm - 10010905 */
    DEFINE VARIABLE v-excel-hdr    AS CHARACTER.
    DEFINE VARIABLE v-runbal       LIKE open-amt NO-UNDO.
    ...
    FOR EACH account
        ...
        FOR EACH glhist NO-LOCK
            ...
            /* gdm - 10010905 */
            ASSIGN 
                v-runbal = v-runbal + v-crdit[1] + v-debit[1].
            ...
            DO:
                ...
                DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                    ...
                    CASE cTmpField:             
                        ...
                        WHEN "dscr"   THEN 
                            cVarValue = STRING(REPLACE(account.dscr, "," ," "),"x(45)") .
                        WHEN "run"    THEN 
                            cVarValue = STRING(glhist.tr-num) .
                        WHEN "jrnl"   THEN 
                            cVarValue = STRING(glhist.jrnl) .
                        WHEN "ref"    THEN 
                            cVarValue = STRING(SUBSTRING(glhist.tr-dscr,1,LENGTH(glhist.tr-dscr) - 11),"x(35)")  .
                        WHEN "date"   THEN 
                            cVarValue = STRING(glhist.tr-date,"99/99/9999").
                        WHEN "dbt"    THEN 
                            cVarValue = IF v-debit[1] NE 0 THEN STRING(v-debit[1],"->>>,>>>,>>>,>>9.99") ELSE "".
                        WHEN "crdt"   THEN 
                            cVarValue = IF v-crdit[1] NE 0 THEN STRING(v-crdit[1],"->>>,>>>,>>>,>>9.99") ELSE "".
                        WHEN "bal"    THEN 
                            cVarValue = IF v-runbal NE ? THEN STRING(v-runbal,"->>>,>>>,>>>,>>9.99") ELSE "".
                        WHEN "jrnl#"  THEN 
                            cVarValue = STRING(v-jrnl-no,"x(7)") .
                ...
                DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                    ...
                    CASE cTmpField:             
                        ...
                        WHEN "date"   THEN 
                            cVarValue = "" .
                        WHEN "dbt"    THEN 
                            cVarValue = STRING(v-debit[2],"->>>,>>>,>>>,>>9.99") .
                        WHEN "crdt"   THEN 
                            cVarValue = STRING(v-crdit[2],"->>>,>>>,>>>,>>9.99") .
                        WHEN "bal"    THEN 
                            cVarValue = IF v-runbal NE ? THEN STRING(v-tot-bal,"->>>,>>>,>>>,>>9.99") ELSE "".
                        WHEN "jrnl#"  THEN 
                            cVarValue = "" .
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField:             
                    ...
                    WHEN "date"   THEN 
                        cVarValue = "" .
                    WHEN "dbt"    THEN 
                        cVarValue = STRING(v-debit[3],"->>>,>>>,>>>,>>9.99") .
                    WHEN "crdt"   THEN 
                        cVarValue = STRING(v-crdit[3],"->>>,>>>,>>>,>>9.99") .
                    WHEN "bal"    THEN 
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\LedgerSummary.csv".   
    END.
