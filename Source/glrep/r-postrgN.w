...
ASSIGN 
    cTextListToSelect  = "Run#,Account Number,Account Description,Journal,Reference," +  
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Run#,Account Number,Account Description,Journal,Reference," +  
                               "Date,Balance" .
...
end_date tb_non-zero sl_avail sl_selected rd-dest fi_file tb_OpenCSV ~
tbAutoClose 
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)":U INITIAL "c:~\tmp~\GLPostingRegister.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    Btn_Def AT ROW 7.1 COL 40.2 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 7.1 COL 60.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 8.1 COL 40.2 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 9.1 COL 40.2 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 10.14 COL 40.2 WIDGET-ID 40
    btn_down AT ROW 11.19 COL 40.2 WIDGET-ID 42
    lv-ornt AT ROW 12.91 COL 43.4 NO-LABELS
    lines-per-page AT ROW 12.91 COL 86.8 COLON-ALIGNED
    rd-dest AT ROW 12.95 COL 4.6 NO-LABELS
    lv-font-no AT ROW 12.95 COL 33.6 COLON-ALIGNED
    lv-font-name AT ROW 14 COL 29 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 15.05 COL 72 WIDGET-ID 2
    td-show-parm AT ROW 15.71 COL 28
    fi_file AT ROW 16.81 COL 26 COLON-ALIGNED HELP
    "Enter File Name" WIDGET-ID 6
    tb_OpenCSV AT ROW 16.91 COL 77.6 WIDGET-ID 4
    tbAutoClose AT ROW 18.14 COL 28 WIDGET-ID 64
    btn-ok AT ROW 19.1 COL 28
    btn-cancel AT ROW 19.1 COL 54
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 6.48 COL 3 WIDGET-ID 38
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.24 COL 5
    " Output Destination" VIEW-AS TEXT
    SIZE 21 BY .62 AT ROW 12.33 COL 4
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 6.48 COL 60.4 WIDGET-ID 44
    RECT-6 AT ROW 12.71 COL 3
...
ASSIGN 
    tb_non-zero:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "GR6" }
    ASSIGN
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE v-exp-name     AS CHARACTER FORMAT "x(40)" INIT "c:\tmp\r-postrg.csv".
    DEFINE VARIABLE v-tr-num       AS CHARACTER. 

    /*{sys/form/r-topw.f}*/

    DEFINE VARIABLE v-s-run        LIKE glhist.tr-num FORMAT ">>>>>>" INIT 0.
    DEFINE VARIABLE v-e-run        LIKE v-s-run INIT 999999.
    DEFINE VARIABLE v-s-dat        LIKE glhist.tr-date FORMAT "99/99/9999" INIT 01/01/0001.
    DEFINE VARIABLE v-e-dat        LIKE v-s-dat INIT 12/31/9999.

    DEFINE VARIABLE tot-all        AS DECIMAL   FORMAT "(>>>,>>>,>>>,>>9.99)".
    DEFINE VARIABLE tot-tot        AS DECIMAL   FORMAT "(>>>,>>>,>>>,>>9.99)".
    DEFINE VARIABLE v-print        AS LOG       INIT NO.
    DEFINE VARIABLE tmp-dscr       LIKE glhist.tr-dscr.
    ...
    FOR EACH ttRptSelected BY ttRptSelected.DisplayOrder:
        ...
        IF LOOKUP(ttRptSelected.TextList, "Balance") <> 0    THEN
            ASSIGN
    ...
    FOR EACH tt-glhist NO-LOCK
        ...
        BY tt-glhist.lNonZero DESCENDING:
    ...
    FOR EACH tt-glhist NO-LOCK
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            ...
            CASE cTmpField:             
                WHEN "run"    THEN 
                    cVarValue = STRING(tt-glhist.tr-num,"9999999") .
                WHEN "acc"   THEN 
                    cVarValue = STRING(tt-glhist.actnum,"x(25)").
                WHEN "acc-desc"  THEN 
                    cVarValue = IF AVAILABLE account THEN  STRING(account.dscr,"x(30)") ELSE "" .
                WHEN "jour"   THEN 
                    cVarValue = STRING(tt-glhist.jrnl,"x(12)") .
                WHEN "ref"   THEN 
                    cVarValue = STRING(tt-glhist.tr-dscr,"x(30)").
                WHEN "date"   THEN 
                    cVarValue = STRING(tt-glhist.tr-date,"99/99/9999").
                WHEN "bal"   THEN 
                    cVarValue = STRING(tt-glhist.tr-amt,"->>>,>>>,>>9.99").
                WHEN "acc-stat"   THEN 
                    cVarValue = IF AVAILABLE account AND account.inactive  THEN STRING("Inactive") ELSE "Active".
                WHEN "period"   THEN 
                    cVarValue = STRING(tt-glhist.period,">>>>>9").
                WHEN "out-per"   THEN 
                    cVarValue = IF tt-glhist.period NE MONTH(tt-glhist.tr-date) THEN STRING("Yes") ELSE "".                        

            END CASE.
            
            IF cTmpField = "date" THEN cExcelVarValue = DYNAMIC-FUNCTION("sfFormat_Date",tt-glhist.tr-date) .
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\GLPostingRegister.csv".   
    END.
