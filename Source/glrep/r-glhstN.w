...
tb_exc-auto tb_detailed tb_exc-acc sl_avail sl_selected rd-dest fi_file ~
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\GLAccountHistory.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    tb_exc-auto AT ROW 5 COL 38
    tb_detailed AT ROW 6.19 COL 38
    tb_exc-acc AT ROW 7.29 COL 38 WIDGET-ID 58
    sl_avail AT ROW 9.24 COL 3 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 9.24 COL 40.4 HELP
    "Default Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 9.24 COL 59.8 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 10.24 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 11.24 COL 40.4 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 12.24 COL 40.4 WIDGET-ID 40
    btn_down AT ROW 13.24 COL 40.4 WIDGET-ID 42
    lv-ornt AT ROW 14.91 COL 43.6 NO-LABELS
    lines-per-page AT ROW 14.91 COL 87 COLON-ALIGNED
    rd-dest AT ROW 14.95 COL 4.6 NO-LABELS
    lv-font-no AT ROW 14.95 COL 33.4 COLON-ALIGNED
    lv-font-name AT ROW 15.95 COL 29 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 17.19 COL 91 RIGHT-ALIGNED
    td-show-parm AT ROW 17.67 COL 27.8
    fi_file AT ROW 18.52 COL 25.8 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 18.62 COL 92.2 RIGHT-ALIGNED
    tbAutoClose AT ROW 19.81 COL 27.8 WIDGET-ID 64
    btn-ok AT ROW 20.76 COL 27.6
    btn-cancel AT ROW 20.76 COL 53
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 14.29 COL 4
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.24 COL 4
    "Available Columns" VIEW-AS TEXT
    SIZE 22 BY .62 AT ROW 8.62 COL 3 WIDGET-ID 38
...
ASSIGN 
    tb_exc-acc:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_exc-auto:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "GR5" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    DISPLAY begin_date end_date begin_acct end_acct tb_exc-auto tb_detailed 
        tb_exc-acc sl_avail sl_selected rd-dest fi_file tb_OpenCSV 
        tbAutoClose 
...
PROCEDURE run-report :
    ...
    */    
    ...
    FOR EACH account NO-LOCK WHERE account.company EQ cocode AND
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            ...
            CASE cTmpField:               
                ...
                WHEN "actdesc" THEN 
                    cVarValue = STRING(account.dscr) .
                WHEN "ap-desc" THEN 
                    cVarValue =  "" .
                WHEN "run"     THEN 
                    cVarValue = /*STRING(v-cust-no)*/ "".
                WHEN "jour"    THEN 
                    cVarValue = /*IF AVAIL cust THEN STRING(cust.name) ELSE*/ "".
                WHEN "ref"     THEN 
        ...
        DO:
            ...
            FOR EACH glhist NO-LOCK
                ...
                tmp-dscr = "".
                ap-dscr = "".
                IF glhist.jrnl EQ "ACPAY" /*AND tb_desc*/ THEN 
                DO:
                ...
                DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                    ...
                    CASE cTmpField:               
                        ...
                        WHEN "actdesc" THEN 
                            cVarValue =  "" .
                        WHEN "ap-desc" THEN 
                            cVarValue = STRING(ap-dscr,"x(30)") .
                        WHEN "run"     THEN 
                            cVarValue = STRING(glhist.tr-num,"99999999") .
                        WHEN "jour"    THEN 
                            cVarValue = STRING(glhist.jrnl) .
                        WHEN "ref"     THEN 
                            cVarValue = STRING(tmp-dscr,"x(52)").
                        WHEN "date"    THEN 
            ...
            FOR EACH glhist NO-LOCK
                ...
                DO:
                    ...
                    FOR EACH xglhist FIELDS(tr-amt) NO-LOCK WHERE
                        ...
                        ASSIGN 
                            tmp-amt = tmp-amt + xglhist.tr-amt.
                    END.
                                        ...
                                        ELSE IF glhist.jrnl EQ "CDISB" THEN
                                            ...
                                            ELSE IF glhist.jrnl EQ "APMEM" THEN
                                                ...
                                                ELSE IF glhist.jrnl EQ "CRMEM" THEN
                                                    ...
                                                    ELSE IF glhist.jrnl EQ "ACPAY" THEN 
                                                        DO:
                                                            RUN GetTransDesc(INPUT glhist.tr-dscr,
                                                                INPUT glhist.tr-amt,
                                                                OUTPUT tmp-dscr). 
        ...
        DO:
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField:               
                    ...
                    WHEN "bal"     THEN 
                        cVarValue = STRING(tot-act + open-amt,"->>,>>>,>>>,>>9.99") + "*" .
                END CASE.
                cExcelVarValue = cVarValue.  
                cDisplay = cDisplay + cVarValue +
                    FILL(" ",INTEGER(ENTRY(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldLength)) + 1 - LENGTH(cVarValue)).             
    ...
    END.
    PUT UNFORMATTED 
        "    Total  " SUBSTRING(cDisplay,12,300) SKIP.
    IF tb_excel THEN 
    DO:
        PUT STREAM excel UNFORMATTED  
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\GLAccountHistory.csv".   
    END.
