...
&Scoped-Define ENABLED-OBJECTS btSelectAll btDelete RECT-13 RECT-15 btFilter fiUserID ~
...
DEFINE BUTTON btSelectAll 
     ...
     LABEL "Select All" 
     SIZE 7.2 BY 1.71 TOOLTIP "Select All"
     BGCOLOR 21 FGCOLOR 21 .
...
DEFINE FRAME DEFAULT-FRAME
     btSelectAll AT ROW 1.14 COL 157 WIDGET-ID 324
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         DEFAULT-BUTTON btFilter WIDGET-ID 100.
...
&Scoped-define BROWSE-NAME ttUserWindow
&Scoped-define SELF-NAME ttUserWindow
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ttUserWindow DEFAULT-FRAME
ON MOUSE-EXTEND-CLICK OF ttUserWindow IN FRAME DEFAULT-FRAME
DO:
  RUN set-select.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ttUserWindow DEFAULT-FRAME
ON MOUSE-MENU-CLICK OF ttUserWindow IN FRAME DEFAULT-FRAME
DO:
  RUN set-select.
  APPLY 'U1' TO BROWSE ttUserWindow. 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ttUserWindow DEFAULT-FRAME
ON MOUSE-MOVE-CLICK OF ttUserWindow IN FRAME DEFAULT-FRAME
DO:
  RUN set-select.
  APPLY 'U1' TO BROWSE ttUserWindow. 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ttUserWindow DEFAULT-FRAME
ON MOUSE-SELECT-CLICK OF ttUserWindow IN FRAME DEFAULT-FRAME
DO:
      RUN set-select.
...
ON CHOOSE OF btSelectAll IN FRAME DEFAULT-FRAME /* btSelectAll */
DO:
    DEF VAR lSelectAll AS LOGICAL NO-UNDO.
...
DO:
    ...
    FOR EACH UserWindow NO-LOCK 
       WHERE UserWindow.usrID BEGINS fiUserID:SCREEN-VALUE 
         AND UserWindow.programName BEGINS fiProgramName:SCREEN-VALUE
        ...
        FOR FIRST prgrms NO-LOCK 
            WHERE ENTRY(1,prgrms.prgmname,".") EQ ENTRY(2,UserWindow.programName,"/") 
              ...
              AND prgrms.mnemonic BEGINS fiHotKey:SCREEN-VALUE 
            :      
            CREATE ttUserWindow.
...
DO: 
  ...
  FOR FIRST ttUserWindow WHERE ttUserWindow.programname = selectedprogram AND
    ...
    ttUserWindow.cSelect = ttUserWindow.cSelect.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE btSelectAll btDelete RECT-13 RECT-15 btFilter fiUserID fiHotKey fiTitle cbState 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE set-select C-Win 
...
------------------------------------------------------------------------------*/
  ...
  DEF BUFFER b-ttUserWindow FOR ttUserWindow.
  
  DO WITH FRAME {&frame-name}:
    FOR EACH b-ttUserWindow:
      b-ttUserWindow.cSelect = NO.
    END.
    ...
    DO li = 1 TO {&browse-name}:NUM-SELECTED-ROWS:
      ...
      IF AVAIL ttUserWindow THEN ttUserWindow.cSelect = YES.
    END.
  END.
END PROCEDURE.
