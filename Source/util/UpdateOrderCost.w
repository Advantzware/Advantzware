...
fiBeginingOrder fiEndingOrder begin_ord-date end_ord-date rd_jobstat ~
fiDirectory btStartProcess tbOpenFile 
&Scoped-Define DISPLAYED-OBJECTS fiBeginingOrder fiEndingOrder ~
begin_ord-date end_ord-date rd_jobstat fiDirectory tbOpenFile 
...
DEFINE FRAME DEFAULT-FRAME
     btSimulatePurge AT ROW 14.33 COL 23 WIDGET-ID 10
     btExit AT ROW 1.14 COL 88.8 WIDGET-ID 24
     fiBeginingOrder AT ROW 4.52 COL 28.4 COLON-ALIGNED WIDGET-ID 6
     fiEndingOrder AT ROW 4.52 COL 71.4 COLON-ALIGNED WIDGET-ID 8
     begin_ord-date AT ROW 5.95 COL 28.4 COLON-ALIGNED WIDGET-ID 28
     end_ord-date AT ROW 5.95 COL 71.4 COLON-ALIGNED HELP
          "Enter Ending Order Date" WIDGET-ID 14
     rd_jobstat AT ROW 8.14 COL 31.4 NO-LABEL WIDGET-ID 38
     fiDirectory AT ROW 11.67 COL 4 COLON-ALIGNED NO-LABEL WIDGET-ID 18
     btStartProcess AT ROW 14.33 COL 50.2 WIDGET-ID 12
     tbOpenFile AT ROW 11.76 COL 71.4 WIDGET-ID 26
     "Records to view  will be stored in directory:" VIEW-AS TEXT
          SIZE 50.6 BY .62 AT ROW 10.76 COL 12.4 WIDGET-ID 16
          FONT 22
     "Order Status:" VIEW-AS TEXT
          SIZE 13 BY .62 AT ROW 8.19 COL 17 WIDGET-ID 42
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
ASSIGN 
       begin_ord-date:PRIVATE-DATA IN FRAME DEFAULT-FRAME     = 
                "parm".

ASSIGN 
       end_ord-date:PRIVATE-DATA IN FRAME DEFAULT-FRAME     = 
                "parm".
...
ON LEAVE OF begin_ord-date IN FRAME DEFAULT-FRAME /* Beginning Order Date */
DO:
        ASSIGN {&self-name}.
    END.
...
ON LEAVE OF end_ord-date IN FRAME DEFAULT-FRAME /* Ending Order Date */
DO:
        ASSIGN {&self-name}.
    END.
...
ON VALUE-CHANGED OF rd_jobstat IN FRAME DEFAULT-FRAME
DO:
  assign {&self-name}.
...
------------------------------------------------------------------------------*/
  DISPLAY fiBeginingOrder fiEndingOrder begin_ord-date end_ord-date rd_jobstat 
          fiDirectory tbOpenFile 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btSimulatePurge btExit RECT-15 RECT-16 fiBeginingOrder fiEndingOrder 
         begin_ord-date end_ord-date rd_jobstat fiDirectory btStartProcess 
         tbOpenFile 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
/*------------------------------------------------------------------------------
     ...
     Notes:
    ------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER iplUpdateCost AS LOGICAL NO-UNDO.
    ...
    cFileName = "\OrderCostChanges" + STRING(TIME) + ".csv".
    IF NOT iplUpdateCost THEN    
    ...
    DO WITH FRAME {&FRAME-NAME}:
        RUN util/UpdateOrderCostProc.p(
            INPUT cocode,             
            INPUT DATE(begin_ord-date:SCREEN-VALUE), 
            INPUT DATE(end_ord-date:SCREEN-VALUE), 
            INPUT INTEGER(fiBeginingOrder:SCREEN-VALUE), 
            INPUT INTEGER(fiEndingOrder:SCREEN-VALUE),
            INPUT rd_jobstat:SCREEN-VALUE,
            INPUT iplUpdateCost, 
...
FUNCTION pfSetDirectory RETURNS LOGICAL PRIVATE
    ...
    cLocation = cLocation + "\OrderCostChanges-" + STRING(YEAR(TODAY),"9999") + STRING(MONTH(TODAY),"99")+ STRING(DAY(TODAY),"99") + "-" + STRING(TIME,"99999").  
    DO WITH FRAME {&FRAME-NAME}:
