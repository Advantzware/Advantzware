...
DEFINE VARIABLE v-i-no AS CHAR NO-UNDO.
...
DO WHILE VALID-HANDLE(hProc):
    ...
    IF index(hProc:FILE-NAME, "b-ordfgi") GT 0 THEN
        LEAVE. /* found it. */
...
IF VALID-HANDLE(hProc) THEN DO:
    ...
    RUN get-current-item IN hProc (OUTPUT v-i-no).

END.
asi.itemfg.i-no:SCREEN-VALUE = v-i-no.
