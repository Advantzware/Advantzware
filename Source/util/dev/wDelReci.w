...
&Scoped-Define ENABLED-OBJECTS cbChoose btStartConversion 
&Scoped-Define DISPLAYED-OBJECTS cbChoose 
...
DEFINE BUTTON btStartConversion 
     LABEL "Start Convert from recid to rec_key" 
     SIZE 45 BY 1.14.

DEFINE VARIABLE cbChoose AS CHARACTER FORMAT "X(256)":U 
     LABEL "Choose" 
...
DEFINE FRAME F-Main
     cbChoose AT ROW 8.62 COL 55 COLON-ALIGNED WIDGET-ID 2
     btStartConversion AT ROW 14.81 COL 44 WIDGET-ID 4
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Convert recid to rec_key"
         HEIGHT             = 24
         WIDTH              = 149.8
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 204.8
...
ON END-ERROR OF W-Win /* Convert recid to rec_key */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Convert recid to rec_key */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE convertPhone W-Win 
PROCEDURE convertPhone :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH phone NO-LOCK,
  ...
  if not avail bf-reftable then do:
    message "Could not lock record for update. " skip
      "affect phone number: " phone.phone
      view-as alert-box.
      next.
  end.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE cbChoose btStartConversion 
      WITH FRAME F-Main IN WINDOW W-Win.
