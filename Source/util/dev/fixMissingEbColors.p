...
FOR EACH eb NO-LOCK WHERE 
    ...
    FOR EACH itemfg-ink
        ...
        FIND FIRST ITEM NO-LOCK WHERE 
            ...
            item.i-no       EQ ttInk.rm-i-no AND 
            item.press-type EQ mach.pr-type
            NO-ERROR.
    ...
    DO TRANSACTION:
        ...
        FOR EACH ttInk NO-LOCK 
            ...
            ELSE DO:
                ASSIGN
                    b-eb.i-ps2[iCtr - 10]     = ttInk.pass
                    b-eb.i-code2[iCtr - 10]   = ttInk.rm-i-no
                    b-eb.i-dscr2[iCtr - 10]   = ttInk.dscr
                    b-eb.i-%2[iCtr - 10]      = ttInk.cover%
                    b-eb.side[iCtr]           = IF ttInk.in-out THEN " " ELSE "F".
            END.

            /* Assign pass-level field values */
            IF LAST-OF(ttInk.pass) THEN ASSIGN
                b-eb.i-pass   = eb.i-pass + (IF iCtr GT 0 THEN 1 ELSE 0)
                b-eb.i-coat-p = eb.i-coat-p + (IF iCoatPCtr GT 0 THEN 1 ELSE 0)
                iCoatPCtr   = 0.  
        END.
