...
&Scoped-Define ENABLED-OBJECTS RECT-7 in-file-name btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS in-file-name v-status 
...
DEFINE FRAME FRAME-A
     ...
     v-status AT ROW 16.24 COL 12 COLON-ALIGNED NO-LABEL
...
ON END-ERROR OF C-Win /* Import GL Accounts */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Import GL Accounts */
DO:
...
DO:
  ...
  MESSAGE "Are you sure you want to import GL Accounts?"  SKIP
          "(All current GL accounts will be deleted.)" 
       VIEW-AS ALERT-BOX WARNING BUTTON YES-NO UPDATE ll-ans AS LOG.
  IF ll-ans THEN do:
      run run-report.
      MESSAGE "Import procedure is completed ." VIEW-AS ALERT-BOX.
  END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY in-file-name v-status 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-7 in-file-name btn-ok btn-cancel 
      WITH FRAME FRAME-A IN WINDOW C-Win.
...
DEF VAR lv-input AS cha FORM "x(300)" NO-UNDO.
    ...
    FOR EACH account WHERE account.company = cocode.
        EXPORT account.
        DELETE account.
    END.
    OUTPUT CLOSE.
    ...
    repeat:
      ...
      IF NUM-ENTRIES(lv-input) < 3 THEN NEXT.

      CREATE tt-act.
      ASSIGN tt-act.actnum = ENTRY(1,lv-input)
             tt-act.dscr = ENTRY(2,lv-input)
             tt-act.TYPE = ENTRY(3,lv-input).
    END.
    INPUT CLOSE.

    FOR EACH tt-act:
        FIND FIRST account WHERE account.company eq cocode 
                             AND account.actnum = tt-act.actnum EXCLUSIVE-LOCK NO-ERROR.
        IF NOT AVAIL account THEN CREATE account.
        ASSIGN account.company = cocode
               account.actnum = tt-act.actnum
               account.dscr = tt-act.dscr
               account.TYPE = IF lookup(tt-act.TYPE,"A,C,E,L,R,T") > 0 THEN tt-act.TYPE ELSE "A".

        v-status:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Importing for account: " + tt-act.actnum.
    END. 
