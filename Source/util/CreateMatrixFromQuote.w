...
/*------------------------------------------------------------------------
  ...
  File: util\CreateMatrixFromQuote.w
...
DEF TEMP-TABLE tt-quote NO-UNDO
...
DEFINE FRAME fMain
     ...
     "This Procedure will create price matrix" VIEW-AS TEXT
          SIZE 64 BY .95 AT ROW 1.48 COL 21
     "from the unapproved quote." VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW wWin ASSIGN
         HIDDEN             = YES
         TITLE              = "Create Price Matrix form unapproved Quote"
         HEIGHT             = 9.19
         WIDTH              = 97.4
         MAX-HEIGHT         = 28.81
         MAX-WIDTH          = 146.2
...
DO:
  ...
  INPUT FROM value(ls-proc) .
  REPEAT:
     CREATE tt-quote.
     IMPORT DELIMITER "," tt-quote NO-ERROR.     
  END.   
  INPUT CLOSE. 
...
&Scoped-define SELF-NAME ls-proc
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ls-proc Dialog-Frame
ON HELP OF ls-proc IN FRAME fMain
DO:
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tt-quote WHERE tt-quote.quoteNo NE 0:  
    FIND FIRST bf-quotehd EXCLUSIVE-LOCK
        WHERE bf-quotehd.company  EQ g_company
        AND bf-quotehd.q-no     EQ tt-quote.quoteNo 
        NO-ERROR.
    IF AVAIL bf-quotehd AND NOT bf-quotehd.approved THEN
    DO:
       ASSIGN
        bf-quotehd.approved = YES
        bf-quotehd.effectiveDate = TODAY
        bf-quotehd.expireDate = 12/31/2099.
