...
DEFINE BUFFER tt-report2 FOR tt-report.
DEFINE STREAM excel.
...
ASSIGN 
    cTextListToSelect  = "PRODUCT,DESCRIPTION,SHIP TO,NAME,CITY,ST,INV DATE,QTY SHIP,"
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "PRODUCT,DESCRIPTION,SHIP TO,NAME,CITY,ST,INV DATE,QTY SHIP,"
                           + "UNIT PRICE,TOTAL SALE" .
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\ShipmentHistoryByItem.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    tb_cust-list AT ROW 2.1 COL 30.8 WIDGET-ID 6
    btnCustList AT ROW 2.1 COL 71.4 WIDGET-ID 8
    begin_cust-no AT ROW 3.14 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Customer Number"
    end_cust-no AT ROW 3.14 COL 69.8 COLON-ALIGNED HELP
    "Enter Ending Customer Number"
    begin_ship AT ROW 4.1 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Ship-to#"
    end_ship AT ROW 4.1 COL 69.8 COLON-ALIGNED HELP
    "Enter Ending Zip Code"
    begin_i-no AT ROW 5.05 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Item Number"
    end_i-no AT ROW 5.05 COL 69.8 COLON-ALIGNED HELP
    "Enter Ending Customer Number"
    begin_inv-date AT ROW 6 COL 28.8 COLON-ALIGNED HELP
    "Enter Beginning Invoice Date"
    end_inv-date AT ROW 6 COL 69.8 COLON-ALIGNED HELP
    "Enter Ending Invoice Date"
    sl_avail AT ROW 8.14 COL 3 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 8.14 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 8.14 COL 60.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 9.14 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 10.14 COL 40.4 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 11.19 COL 40.4 WIDGET-ID 40
    btn_down AT ROW 12.19 COL 40.4 WIDGET-ID 42
    tb_excel AT ROW 13.86 COL 46 RIGHT-ALIGNED
    lv-font-no AT ROW 13.86 COL 49 COLON-ALIGNED
    lv-ornt AT ROW 13.86 COL 55 NO-LABELS
    lines-per-page AT ROW 13.86 COL 87 COLON-ALIGNED
    rd-dest AT ROW 14.05 COL 5.2 NO-LABELS
    lv-font-name AT ROW 15.05 COL 29 COLON-ALIGNED NO-LABELS
    td-show-parm AT ROW 16.62 COL 28.2
    fi_file AT ROW 17.57 COL 26.2 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 17.67 COL 92.6 RIGHT-ALIGNED
    tbAutoClose AT ROW 19.1 COL 28 WIDGET-ID 64
    btn-ok AT ROW 20 COL 27.8
    btn-cancel AT ROW 20 COL 55
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 7.43 COL 60.4 WIDGET-ID 44
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.24 COL 4.6
    BGCOLOR 15 
    "Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 13.33 COL 4.6
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 7.43 COL 3 WIDGET-ID 38
    RECT-6 AT ROW 13.71 COL 3
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
    CREATE WINDOW C-Win ASSIGN
        HIDDEN             = YES
        TITLE              = "Shipment History - By Item/Customer"
        HEIGHT             = 20.57
        WIDTH              = 95
        MAX-HEIGHT         = 33.29
        MAX-WIDTH          = 204.8
...
ON END-ERROR OF C-Win /* Shipment History - By Item/Customer */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Shipment History - By Item/Customer */
    DO:
...
PROCEDURE run-report :
    ...
    *****************************************************************************
    ...
    DEFINE VARIABLE fcust          AS CHARACTER INIT "".
    DEFINE VARIABLE tcust          LIKE fcust INIT "zzzzzzzz".
    DEFINE VARIABLE fship          AS CHARACTER INIT "".
    ...
    DEFINE VARIABLE v-name         LIKE shipto.ship-name.
    DEFINE VARIABLE v-city         LIKE shipto.ship-city.
    DEFINE VARIABLE v-stat         LIKE shipto.ship-state.
    DEFINE VARIABLE v-pric         AS DECIMAL   FORMAT ">>>>>>.99<<<<".
    DEFINE VARIABLE v-astr         AS CHARACTER FORMAT "x" EXTENT 2.
    DEFINE VARIABLE v-qty          LIKE ar-invl.inv-qty EXTENT 2.
    DEFINE VARIABLE v-amt          LIKE ar-invl.amt EXTENT 2.
    ...
    FORM ar-invl.i-no           COLUMN-LABEL "PRODUCT"
        ar-invl.i-dscr         COLUMN-LABEL "DESCRIPTION"  FORMAT "x(20)"
        tt-report.key-03          COLUMN-LABEL "SHIP TO"
        v-name                 COLUMN-LABEL "NAME"         FORMAT "x(20)"
        v-city                 COLUMN-LABEL "CITY"
        v-stat                 COLUMN-LABEL "ST"
        bf-ar-inv.inv-date       COLUMN-LABEL "INV DATE"
        ar-invl.inv-qty        COLUMN-LABEL "QTY SHIP"     FORMAT "->>>>>>>9"
    ...
    FOR EACH ttRptSelected BY ttRptSelected.DisplayOrder:
        ...
        IF LOOKUP(ttRptSelected.TextList, "QTY SHIP,TOTAL SALE") <> 0    THEN
            ASSIGN
            ...
            IF v-ship GE fship AND
                ...
                FOR EACH ar-invl
                    ...
                    ASSIGN
                        ...
                        tt-report2.key-01  = ar-invl.i-no
                        tt-report2.key-02  = tt-report.key-09
                        tt-report2.key-03  = v-ship
                        tt-report2.key-04  = STRING(YEAR(ar-inv.inv-date),"9999") +
                              string(MONTH(ar-invl.inv-date),"99")  +
                              string(DAY(ar-invl.inv-date),"99")
    ...
    FOR EACH tt-report WHERE tt-report.term-id EQ "",
        ...
        DO:
            ...
            FIND FIRST bf-ar-inv NO-LOCK
                WHERE bf-ar-inv.inv-no EQ ar-invl.inv-no NO-ERROR.

            FIND FIRST shipto
                WHERE shipto.company EQ cocode
                AND shipto.cust-no EQ tt-report.key-02
                AND shipto.ship-id EQ tt-report.key-03
                NO-LOCK NO-ERROR.
            ...
            /*display ar-invl.i-no
                    ar-invl.i-dscr
                      itemfg.i-name   when avail itemfg and ar-invl.i-dscr eq ""
                                      @ ar-invl.i-dscr
                    tt-report.key-03
                    v-name
                    v-city
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField:             
                    WHEN "prod"    THEN 
                        cVarValue = STRING(ar-invl.i-no,"x(15)") .
                    WHEN "desc"   THEN 
                        cVarValue = IF ar-invl.i-dscr <> "" THEN STRING(ar-invl.i-dscr,"x(30)") ELSE IF AVAILABLE itemfg THEN STRING(itemfg.i-name,"x(30)") ELSE "" .
                    WHEN "shipto"   THEN 
                        cVarValue = STRING(tt-report.key-03,"x(15)").
                    WHEN "name"  THEN 
                        cVarValue = STRING(v-name,"x(20)") .
                    WHEN "city"   THEN 
                    ...
                    WHEN "tot-sal"  THEN 
                        cVarValue = STRING(ar-invl.amt,"->>>,>>>,>>9.99") .
            ...
            /*IF tb_excel THEN
               PUT STREAM excel UNFORMATTED
                   '"' ar-invl.i-no                                   '",'
                   '"' IF avail itemfg and ar-invl.i-dscr eq "" THEN
                          itemfg.i-name ELSE ar-invl.i-dscr           '",'
                   '"' tt-report.key-03                               '",'
                   '"' v-name                                         '",'
                   '"' v-city                                         '",'
                   '"' v-stat                                         '",'                          
                   '"' STRING(bf-ar-inv.inv-date)                     '",'
                   '"' STRING(ar-invl.inv-qty,"->>>>>>>9")            '",'
                   '"' STRING(v-pric,">>>>>>.99<<<<")                 '",'
                   '"' STRING(ar-invl.amt,"->>>>>>>9.99")             '",'
                   SKIP.*/

            ASSIGN
                v-qty[1] = v-qty[1] + ar-invl.inv-qty
        ...
        DO:
            /*display ar-invl.i-no
                    ...
                    itemfg.i-name         when avail itemfg and ar-invl.i-dscr eq ""
                                          @ ar-invl.i-dscr
                    "** PRODUCT TOTAL **" @ v-name
                    v-qty[1]              @ ar-invl.inv-qty
                    v-amt[1]              @ ar-invl.amt
            ...
            PUT str-line SKIP .
            PUT UNFORMATTED  
                "        ** PRODUCT TOTAL **"  SUBSTRING(cDisplay,28,350) SKIP.
            IF tb_excel THEN 
            DO:
                PUT STREAM excel UNFORMATTED  
                    '** PRODUCT TOTAL ** ,'
        ...
        DO:
            /*display "** GRAND TOTAL **"   @ v-name
                    v-qty[2]              @ ar-invl.inv-qty
                    v-amt[2]              @ ar-invl.amt
            ...
            PUT str-line SKIP .
            PUT UNFORMATTED  
                "          ** GRAND TOTAL **"  SUBSTRING(cDisplay,28,350) SKIP.
            IF tb_excel THEN 
            DO:
                PUT STREAM excel UNFORMATTED  
                    '** GRAND TOTAL ** ,'
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\ShipmentHistoryByItem.csv".   
    END.
