...
ASSIGN 
    cTextListToSelect  = "Customer,Name,Labels,Period1,Period2,Period3,Period4,Period5, "  +
                                               "Period6,Period7,Period8,Period9,Period10,Period11,Period12"
    cFieldListToSelect = "cust,name,hding,p1,p2,p3,p4,p5," +               
                                     "p6,p7,p8,p9,p10,p11,p12"         
    cFieldLength       = "8,30,8,12,12,12,12,12," +
                                "12,12,12,12,12,12,12"
    cFieldType         = "c,c,c,i,i,i,i,i," + "i,i,i,i,i,i,i" 
    .

{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Customer,Name,Labels,Period1,Period2,Period3,Period4,Period5, "  +
                                               "Period6,Period7,Period8,Period9,Period10,Period11,Period12" .
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 as-of-date srt-period cust ~
tb_cust-list btnCustList begin_cust-no end_cust-no begin_slsmn end_slsmn ~
begin_fg-cat end_fg-cat tb_prt-cust sl_avail Btn_Def sl_selected Btn_Add ~
Btn_Remove btn_Up btn_down rd-dest fi_file tb_OpenCSV tbAutoClose btn-ok ~
btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS as-of-date srt-period cust tb_cust-list ~
begin_cust-no end_cust-no begin_slsmn end_slsmn begin_fg-cat end_fg-cat ~
tb_prt-cust sl_avail sl_selected rd-dest fi_file tb_OpenCSV tbAutoClose 
...
DEFINE FRAME FRAME-A
    as-of-date AT ROW 2.24 COL 30 COLON-ALIGNED HELP
    "Enter As Of Date"
    srt-period AT ROW 3.19 COL 30 COLON-ALIGNED HELP
    "Enter Periods to Sort By"
    cust AT ROW 4.24 COL 30 COLON-ALIGNED HELP
    "Customers To Print"
    tb_cust-list AT ROW 5.48 COL 32.6 WIDGET-ID 6
    btnCustList AT ROW 5.57 COL 64 WIDGET-ID 8
    begin_cust-no AT ROW 6.62 COL 30 COLON-ALIGNED HELP
    "Enter Beginning Customer Number"
    end_cust-no AT ROW 6.62 COL 73 COLON-ALIGNED HELP
    "Enter Ending Customer Number"
    begin_slsmn AT ROW 7.67 COL 30 COLON-ALIGNED HELP
    "Enter Beginning Sales Rep Number" WIDGET-ID 2
    end_slsmn AT ROW 7.67 COL 73 COLON-ALIGNED HELP
    "Enter Ending Sales Rep Number" WIDGET-ID 4
    begin_fg-cat AT ROW 8.67 COL 30 COLON-ALIGNED HELP
    "Enter Beginning Product Category"
    end_fg-cat AT ROW 8.67 COL 73 COLON-ALIGNED HELP
    "Enter Ending Product Category"
    tb_prt-cust AT ROW 9.76 COL 32
    sl_avail AT ROW 11.86 COL 3 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 11.86 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 11.86 COL 61.8 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 12.86 COL 40.4 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 13.86 COL 40.4 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 14.91 COL 40.4 WIDGET-ID 40
    btn_down AT ROW 15.91 COL 40.4 WIDGET-ID 42
    lv-font-no AT ROW 17.67 COL 35 COLON-ALIGNED
    lv-ornt AT ROW 17.67 COL 45 NO-LABELS
    lines-per-page AT ROW 17.67 COL 88 COLON-ALIGNED
    rd-dest AT ROW 17.91 COL 5 NO-LABELS
    lv-font-name AT ROW 18.86 COL 30 COLON-ALIGNED NO-LABELS
    td-show-parm AT ROW 20.14 COL 29.8
    fi_file AT ROW 21.1 COL 27.8 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 21.14 COL 93.4 RIGHT-ALIGNED
    tbAutoClose AT ROW 22.67 COL 29.8 WIDGET-ID 60
    btn-ok AT ROW 23.62 COL 30
    btn-cancel AT ROW 23.62 COL 55
    "(Enter 99 For YTD)" VIEW-AS TEXT
    SIZE 22 BY 1 AT ROW 3.62 COL 51
    FGCOLOR 9 
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 17.1 COL 4
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.24 COL 5
    BGCOLOR 15 
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 11.1 COL 3 WIDGET-ID 38
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 11.1 COL 61.2 WIDGET-ID 44
    RECT-6 AT ROW 17.57 COL 3
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
    CREATE WINDOW C-Win ASSIGN
        HIDDEN             = YES
        TITLE              = "High Sales Tracking (This Yr vs. Last)"
        HEIGHT             = 24.43
        WIDTH              = 96.2
        MAX-HEIGHT         = 33.29
        MAX-WIDTH          = 204.8
...
ON END-ERROR OF C-Win /* High Sales Tracking (This Yr vs. Last) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* High Sales Tracking (This Yr vs. Last) */
    DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    FIND FIRST period NO-LOCK
        ...
        NO-ERROR.  
    IF AVAILABLE period THEN srt-period = period.pnum.
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "HR14" }
    ASSIGN
    ...
    RUN sys/inc/CustListForm.p ( "HR14",cocode, 
        OUTPUT ou-log,
        OUTPUT ou-cust-int) .
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY as-of-date srt-period cust tb_cust-list begin_cust-no end_cust-no 
        begin_slsmn end_slsmn begin_fg-cat end_fg-cat tb_prt-cust sl_avail 
        sl_selected rd-dest fi_file tb_OpenCSV tbAutoClose 
        WITH FRAME FRAME-A IN WINDOW C-Win.
    ENABLE RECT-6 RECT-7 as-of-date srt-period cust tb_cust-list btnCustList 
        begin_cust-no end_cust-no begin_slsmn end_slsmn begin_fg-cat 
        end_fg-cat tb_prt-cust sl_avail Btn_Def sl_selected Btn_Add Btn_Remove 
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE fdate          AS DATE      EXTENT 26 INIT 12/31/9999 NO-UNDO.
    DEFINE VARIABLE tdate          AS DATE      EXTENT 26 INIT 12/31/9999 NO-UNDO.
    ...
    DEFINE VARIABLE tcust          LIKE cust.cust-no INIT "zzz" NO-UNDO.
    ...
    DEFINE VARIABLE v-amt1         AS DECIMAL.
    DEFINE VARIABLE v-slsm         LIKE ar-invl.sman EXTENT 1 NO-UNDO.
    DEFINE VARIABLE v-slsp         LIKE ar-invl.s-pct EXTENT 1 NO-UNDO.

    DEFINE VARIABLE v-amt          AS DECIMAL   EXTENT 26 NO-UNDO.
    DEFINE VARIABLE excel-amt      AS DECIMAL   EXTENT 26 NO-UNDO.
    DEFINE VARIABLE v-tot          AS DECIMAL   EXTENT 26 NO-UNDO.
    DEFINE VARIABLE v-pct          AS DECIMAL   EXTENT 13 NO-UNDO.
    DEFINE VARIABLE excel-pct      AS DECIMAL   EXTENT 13 NO-UNDO.
    DEFINE VARIABLE v-label        AS CHARACTER EXTENT 26 FORMAT "x(12)" NO-UNDO.
    ...
    DEFINE VARIABLE lv-ltype       AS CHARACTER EXTENT 6 NO-UNDO.
    DEFINE VARIABLE ll-tots        AS LOG       NO-UNDO.
    ...
    FORM HEADER
        ...
        WITH NO-BOX NO-ATTR-SPACE PAGE-TOP FRAME r-top2 STREAM-IO WIDTH 200.
    
    FORM cust.name      FORMAT "x(21)"
         lv-ltype[1]    FORMAT "x(8)"
         v-amt[14]      FORMAT "->>>,>>>,>>9"
         v-amt[15]      FORMAT "->>>,>>>,>>9"
         v-amt[16]      FORMAT "->>>,>>>,>>9"
         v-amt[17]      FORMAT "->>>,>>>,>>9"
         ...
         v-amt[13]      FORMAT "->>>,>>>,>>9"
         SKIP
         cust.cust-no   FORMAT "x(21)"
         lv-ltype[3]    FORMAT "x(8)"
         v-pct[01]      FORMAT "->>,>>>,>>9%"
         v-pct[02]      FORMAT "->>,>>>,>>9%"
         v-pct[03]      FORMAT "->>,>>>,>>9%"
         v-pct[04]      FORMAT "->>,>>>,>>9%"
    ...
    FOR EACH ttRptSelected BY ttRptSelected.DisplayOrder:
        IF ttRptSelected.TextList = "Labels" THEN
            ASSIGN ttRptSelected.TextList = "        " .
    ...
    FOR EACH period NO-LOCK
        ...
        IF i LE 0 THEN i = i + v1.

        ASSIGN
            fdate[i] = period.pst
            tdate[i] = period.pend.

        v2 = IF period.pnum GT v THEN -1 ELSE 0.

        IF v2 EQ -1 THEN 
        DO:
            RUN sys/ref/chgyear.p (INPUT-OUTPUT fdate[i], v2).
            RUN sys/ref/chgyear.p (INPUT-OUTPUT tdate[i], v2).
        END.

        v-label[i] = STRING(period.yr + v2,"9999") + " P" +
            TRIM(STRING(period.pnum,">9")).

        IF srt-period EQ period.pnum THEN v-per-2 = i + 13.
    END.

    DO i = 1 TO 13:
        IF v-label[i] NE "" THEN
        DO:
            v-label[i + 13] = "------------".

            IF rd-dest = 3 THEN
        /*  excelheader = excelheader + "This Yr " + v-label[i] + ","
                      + "Last Yr " + v-label[i] + ","
                      + "Period % " + v-label[i] + ","
                      + "This YTD " + v-label[i] + ","
                      + "Last YTD " + v-label[i] + ","
    ...
    DO i = 1 TO 13:
        ...
        RUN sys/ref/chgyear.p (INPUT-OUTPUT fdate[i], -1).
        RUN sys/ref/chgyear.p (INPUT-OUTPUT tdate[i], -1).
    END.
