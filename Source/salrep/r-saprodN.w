...
{sys/inc/ttRptSel.i}
ASSIGN cTextListToDefault  = "Catgy,Dly Sq Ft/M,Dly Amount,Dly $/MSF,"
              + "PTD Sq Ft/M,PTD Amount,PTD $/MSF,YTD Sq Ft/M,YTD Amount,YTD $/MSF"     .
...
DEFINE FRAME FRAME-A
     ...
     sl_avail AT ROW 6.81 COL 4.2 NO-LABEL WIDGET-ID 26
     sl_selected AT ROW 6.81 COL 59.2 NO-LABEL WIDGET-ID 28
     ...
     lv-ornt AT ROW 14.33 COL 29 NO-LABEL
     lines-per-page AT ROW 14.33 COL 82 COLON-ALIGNED
     lv-font-no AT ROW 16.24 COL 32 COLON-ALIGNED
     lv-font-name AT ROW 17.19 COL 26 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 18.38 COL 28
     tb_excel AT ROW 20.1 COL 48 RIGHT-ALIGNED
     tb_runExcel AT ROW 20.1 COL 69 RIGHT-ALIGNED
     fi_file AT ROW 20.91 COL 26 COLON-ALIGNED HELP
          "Enter File Name"
     btn-ok AT ROW 22.62 COL 20
     btn-cancel AT ROW 22.62 COL 57
     "Available Columns" VIEW-AS TEXT
          SIZE 20 BY .62 AT ROW 6.05 COL 5 WIDGET-ID 38
     ...
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 6.1 COL 59.4 WIDGET-ID 44
...
------------------------------------------------------------------------------*/
  DISPLAY inv-date tb_fin-chg tb_misc-sal sl_avail sl_selected rd-dest lv-ornt 
...
display "" with frame r-top.
    ...
    ASSIGN cDisplay = ""
           ...
           cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                CASE cTmpField:
                    ...
                    WHEN "dly-sf"   THEN cVarValue = STRING(v-gtot-sqft,"->>>,>>9.999").       
                    WHEN "dly-amt"  THEN cVarValue = STRING(v-gtot-amt,"->,>>>,>>9.99") .
                    WHEN "dly-msf"  THEN cVarValue = STRING(v-gtot-msf,"->,>>>,>>9.99") .     
                    WHEN "ptd-sf"   THEN cVarValue = STRING(v-gtot-ptd-sqft,"->>>,>>9.999") .
    ...
    ASSIGN cDisplay = ""
           ...
           cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                CASE cTmpField:
                    WHEN "cat"      THEN cVarValue = "" .
                    WHEN "dly-sf"   THEN cVarValue = STRING(v-mtot-sqft,"->>>,>>9.999")     .       
                    WHEN "dly-amt"  THEN cVarValue = STRING(v-mtot-amt,"->,>>>,>>9.99")     .
                    WHEN "dly-msf"  THEN cVarValue = STRING(v-mtot-msf,"->,>>>,>>9.99")     .      
                    WHEN "ptd-sf"   THEN cVarValue = STRING(v-mtot-ptd-sqft,"->>>,>>9.999") .
    ...
    ASSIGN cDisplay = ""
           ...
           cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                CASE cTmpField:
                  WHEN "cat"      THEN cVarValue = "" .
                  WHEN "dly-sf"   THEN cVarValue = ""     .       
                  WHEN "dly-amt"  THEN cVarValue =  STRING(v-grtot-amt,"->,>>>,>>9.99")    .
                  WHEN "dly-msf"  THEN cVarValue = ""     .      
                  WHEN "ptd-sf"   THEN cVarValue = "" .
                  WHEN "ptd-amt"  THEN cVarValue = STRING(v-grtot-ptd-amt,"->,>>>,>>9.99") .
                  WHEN "ptd-msf"  THEN cVarValue = "" .
                  WHEN "ytd-sf"   THEN cVarValue = "" .
                  WHEN "ytd-amt"  THEN cVarValue = STRING(v-grtot-ytd-amt,"->,>>>,>>9.99") .  
                  WHEN "ytd-msf"  THEN cVarValue = "" .
                END CASE.
        ...
        PUT UNFORMATTED "  GRAND TOTAL" substring(cDisplay,14,350) SKIP.
        IF tb_excel THEN DO:
            PUT STREAM excel UNFORMATTED  
