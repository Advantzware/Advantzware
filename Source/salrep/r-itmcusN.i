  ...
  for each tt-report
      ...
      for each ar-invl
        ...
        do i = 1 to 3:
          ...
          assign
           ...
           xtt-report.key-06  = string(ar-invl.inv-no,"99999999")
    ...
    if tt-report.key-10 eq "ar-cashl" then do:
      ...
      assign
       ...
       tt-report.key-06 = string(ar-cashl.inv-no,"99999999")
  ...
  for each tt-report,
   ...
   ASSIGN
       v-sname[1] = IF AVAIL sman THEN sman.sname ELSE "" .
   ...
   ASSIGN
       v-sname[2] = IF AVAIL sman THEN sman.sname ELSE "" .
   ...
   ASSIGN
       v-sname[3] = IF AVAIL sman THEN sman.sname ELSE "" .
    ...
    ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         WHEN "cust"    THEN cVarValue = string(v-cust,"x(8)") .
                         WHEN "cust-name"   THEN cVarValue = string(v-cnam,"x(30)").
                         WHEN "cont"   THEN cVarValue = STRING(v-cont,"x(15)").
                         WHEN "phone"  THEN cVarValue = STRING(v-phn#,"x(15)") .
                         WHEN "fax"   THEN cVarValue = STRING(v-fax#,"x(10)") .
                         WHEN "fgitem"  THEN cVarValue = STRING(v-i-no,"x(15)") .
                         ...
                         WHEN "qty-ord"  THEN cVarValue = STRING(v-qty[6],"->>,>>>,>>9.99") .
                         WHEN "qty-ship"   THEN cVarValue = STRING(v-qty[1],"->>,>>>,>>9.99") .
                         WHEN "price"  THEN cVarValue = STRING(v-pric,"->>>,>>9.99") .
                         WHEN "gp"  THEN cVarValue = IF ll-secure THEN STRING(v-prof,"->>>,>>9.99%") ELSE "" .
                         WHEN "rep1"   THEN cVarValue = IF AVAIL ar-invl THEN STRING(ar-invl.sman[1] ,"x(5)") ELSE "". 
                         WHEN "name1"   THEN cVarValue = STRING(v-sname[1],"x(25)") .
                         WHEN "rep2"   THEN cVarValue = IF AVAIL ar-invl THEN STRING(ar-invl.sman[2] ,"x(5)") ELSE "". 
                         WHEN "name2"   THEN cVarValue = STRING(v-sname[2],"x(25)") .
                         WHEN "rep3"   THEN cVarValue = IF AVAIL ar-invl THEN STRING(ar-invl.sman[3] ,"x(5)") ELSE "". 
                         WHEN "name3"   THEN cVarValue = STRING(v-sname[3],"x(25)") .
                         
                    END CASE.
                      
                    IF  cTmpField = "ship-date" THEN
            ...
            IF tb_excel THEN DO:
             ...
             IF LAST-OF(tt-report.key-02) AND v-skip-l THEN
                 PUT SKIP(1) .
   ...
   PUT str-line SKIP.
      ASSIGN cDisplay = ""
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
               cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
                    CASE cTmpField:             
                         ...
                         WHEN "qty-ord"  THEN cVarValue = STRING(v-qty[10],"->>,>>>,>>9.99") .
                         WHEN "qty-ship"   THEN cVarValue = STRING(v-qty[5],"->>,>>>,>>9.99") .
                         WHEN "price"  THEN cVarValue = "" .
                         WHEN "gp"  THEN cVarValue = IF ll-secure THEN STRING(v-prof,"->>>>>>9.99%") ELSE "" .
                         WHEN "rep1"   THEN cVarValue = "" .
                         WHEN "name1"   THEN cVarValue = "" .
                         WHEN "rep2"   THEN cVarValue = "" .
                         WHEN "name2"   THEN cVarValue = "" .
                         WHEN "rep3"   THEN cVarValue = "" .
                         WHEN "name3"   THEN cVarValue = "" .
            ...
            IF tb_excel THEN DO:
                 PUT STREAM excel UNFORMATTED SKIP(1) 'GRAND TOTALS,'
