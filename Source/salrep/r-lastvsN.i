...
IF INDEX(cSelectedList, "YTD Sales") GT 0 OR index(cSelectedList, "MTD Sales") GT 0 THEN 
DO:
        ...
        FOR EACH ar-invl
            ...
            DO i = 1 TO 3:
                ...
                IF ar-inv.inv-date GE fdate[1] AND
                    ar-inv.inv-date LE tdate[1] THEN 
                DO:            
    ...
    ELSE
        ...
        DO:
            ...
            AND */ xtt-reportYtd.key-01 = "2.1"
                ...
                USE-INDEX rec_key
                NO-ERROR.
            IF NOT AVAILABLE xtt-reportYtd THEN 
            DO:
                CREATE xtt-reportYtD.
                ASSIGN 
                    xtt-reportYtd.key-01  = "2.1"
           ...
           AND */ xtt-reportMtd.key-01 = "2.2"
                ...
                USE-INDEX rec_key
                NO-ERROR.        
            IF NOT AVAILABLE xtt-reportMtd THEN 
            DO:
                CREATE xtt-reportMtd.
                ASSIGN 
                    xtt-reportMtd.key-01  = "2.2"
...
FOR EACH xtt-report
    ...
    FOR EACH tt-report
        ...
        DO:
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                DO: 
                    CASE cTmpField:                                          
                        WHEN "v-salesRep" THEN 
                            cVarValue = tt-report.key-03.
                        WHEN "v-amt[1]" THEN 
                            cVarValue = IF v-amt[1] = 0 THEN "" ELSE STRING(v-amt[1],"->,>>>,>>>,>>>.99").
                        WHEN "v-amt[2]" THEN 
                            cVarValue = IF v-amt[2] = 0 THEN "" ELSE STRING(v-amt[2],"->,>>>,>>>,>>>.99").
                        WHEN "v-amt[3]" THEN 
                            cVarValue = IF v-amt[3] = 0 THEN "" ELSE STRING(v-amt[3],"->,>>>,>>>,>>>.99").
                        WHEN "v-amt[4]" THEN 
                            cVarValue = IF v-amt[4] = 0 THEN "" ELSE STRING(v-amt[4],"->,>>>,>>>,>>>.99").
                        WHEN "v-amt[5]" THEN 
                            cVarValue = IF v-amt[5] = 0 THEN "" ELSE STRING(v-amt[5],"->,>>>,>>>,>>>.99").
                        WHEN "v-amt[6]" THEN 
                            cVarValue = IF v-amt[6] = 0 THEN "" ELSE STRING(v-amt[6],"->,>>>,>>>,>>>.99").
                        WHEN "v-msf[1]" THEN 
                            cVarValue = IF v-msf[1] = 0 THEN "" ELSE STRING(v-msf[1],"->,>>>,>>>,>>>.99").
                        WHEN "v-msf[2]" THEN 
                            cVarValue = IF v-msf[2] = 0 THEN "" ELSE STRING(v-msf[2],"->,>>>,>>>,>>>.99").
                        WHEN "v-msf[3]" THEN 
                            cVarValue = IF v-msf[3] = 0 THEN "" ELSE STRING(v-msf[3],"->,>>>,>>>,>>>.99").
                        WHEN "v-msf[4]" THEN 
                            cVarValue = IF v-msf[4] = 0 THEN "" ELSE STRING(v-msf[4],"->,>>>,>>>,>>>.99").
                        WHEN "v-msf[5]" THEN 
                            cVarValue = IF v-msf[5] = 0 THEN "" ELSE STRING(v-msf[5],"->,>>>,>>>,>>>.99").
                        WHEN "v-msf[6]" THEN 
                            cVarValue = IF v-msf[6] = 0 THEN "" ELSE STRING(v-msf[6],"->,>>>,>>>,>>>.99").                            
                        WHEN "v-diff" THEN   
                            cVarValue = IF v-diff = 0 THEN "" ELSE STRING(v-diff,"->,>>>,>>>,>>>.99").
                        WHEN "v-diffYtd" THEN   
                            cVarValue = IF v-diffYTD = 0 THEN "" ELSE STRING(v-diffYTD,"->,>>>,>>>,>>>.99").
                        WHEN "v-diffMtd" THEN   
                            cVarValue = IF v-diffMTD = 0 THEN "" ELSE STRING(v-diffMTD,"->,>>>,>>>,>>>.99").                            
                        WHEN "v-diffMSF" THEN 
                            cVarValue = IF v-diffMSF = 0 THEN "" ELSE STRING(v-diffMSF,"->,>>>,>>>,>>>.99").
                        WHEN "v-pct" THEN    
                            cVarValue = IF v-pct = 0 THEN "" ELSE STRING(v-pct,"->,>>>,>>>,>>>.99").
                        WHEN "v-pctYtd" THEN    
                            cVarValue = IF v-pctYTD = 0 THEN "" ELSE STRING(v-pctYTD,"->,>>>,>>>,>>>.99").
                        WHEN "v-pctMtd" THEN    
                            cVarValue = IF v-pctMTD = 0 THEN "" ELSE STRING(v-pctMTD,"->,>>>,>>>,>>>.99").
                        WHEN "v-pctMSF" THEN 
                            cVarValue = IF v-pctMSF = 0 THEN "" ELSE STRING(v-pctMSF,"->,>>>,>>>,>>>.99").
                        WHEN "v-profit" THEN 
                            cVarValue = IF v-profit = 0 THEN "" ELSE STRING(v-profit,"->,>>>,>>>,>>>.99"). 
                        WHEN "v-cost" THEN 
                            cVarValue = IF v-cost = 0 THEN "" ELSE STRING(v-cost,"->,>>>,>>>,>>>.99"). 
                    END CASE.
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            ...
            DO:      
                CASE cTmpField:                                          
                    ...
                    WHEN "v-diffYtd" THEN   
                        cVarValue = IF v-diffYtd = 0 THEN "" ELSE STRING(v-diffYtd,"->,>>>,>>>,>>>.99").
                    WHEN "v-diffMSFYtd" THEN 
                        cVarValue = IF v-diffMSFYtd = 0 THEN "" ELSE STRING(v-diffMSFYtd,"->,>>>,>>>,>>>.99").
                    WHEN "v-pctYtd" THEN    
                        cVarValue = IF v-pctYtd = 0 THEN "" ELSE STRING(v-pctYtd,"->,>>>,>>>,>>>.99").
                    WHEN "v-pctMSFYtd" THEN 
                        cVarValue = IF v-pctMSFYtd = 0 THEN "" ELSE STRING(v-pctMSFYtd,"->,>>>,>>>,>>>.99").
                    ...
                    WHEN "v-diffMtd" THEN   
                        cVarValue = IF v-diffMtd = 0 THEN "" ELSE STRING(v-diffMtd,"->,>>>,>>>,>>>.99").
                    WHEN "v-diffMSF" THEN 
                        cVarValue = IF v-diffMSFMtd = 0 THEN "" ELSE STRING(v-diffMSFMtd,"->,>>>,>>>,>>>.99").
                    WHEN "v-pctMtd" THEN    
                        cVarValue = IF v-pctMtd = 0 THEN "" ELSE STRING(v-pctMtd,"->,>>>,>>>,>>>.99").
                    WHEN "v-pctMSFMtd" THEN 
                        cVarValue = IF v-pctMSFMtd = 0 THEN "" ELSE STRING(v-pctMSFMtd,"->,>>>,>>>,>>>.99").
    ...
    DO:
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            cTmpField = ENTRY(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
            CASE cTmpField:                                          
                WHEN "cust.cust-no" THEN 
                    cVarValue = "" /*tt-report.key-03*/.
                WHEN "cust.NAME" THEN 
                    cVarValue = "Grand Totals" .
                WHEN "v-salesRep" THEN 
                    cVarValue = "" .
                WHEN "v-amt[1]" THEN 
                    cVarValue = IF v-t-amt[1] = 0 THEN "" ELSE STRING(v-t-amt[1],"->,>>>,>>>,>>>.99").
                WHEN "v-amt[2]" THEN 
                    cVarValue = IF v-t-amt[2] = 0 THEN "" ELSE STRING(v-t-amt[2],"->,>>>,>>>,>>>.99").
                WHEN "v-msf[1]" THEN 
                    cVarValue = IF v-t-msf[1] = 0 THEN "" ELSE STRING(v-t-msf[1],"->,>>>,>>>,>>>.99").
                WHEN "v-msf[2]" THEN 
                    cVarValue = IF v-t-msf[2] = 0 THEN "" ELSE STRING(v-t-msf[2],"->,>>>,>>>,>>>.99").
                WHEN "v-amt[3]" THEN 
                    cVarValue = IF v-t-amt[3] = 0 THEN "" ELSE STRING(v-t-amt[3],"->,>>>,>>>,>>>.99").
                WHEN "v-amt[4]" THEN 
                    cVarValue = IF v-t-amt[4] = 0 THEN "" ELSE STRING(v-t-amt[4],"->,>>>,>>>,>>>.99").
                WHEN "v-msf[3]" THEN 
                    cVarValue = IF v-t-msf[3] = 0 THEN "" ELSE STRING(v-t-msf[3],"->,>>>,>>>,>>>.99").
                WHEN "v-msf[4]" THEN 
                    cVarValue = IF v-t-msf[4] = 0 THEN "" ELSE STRING(v-t-msf[4],"->,>>>,>>>,>>>.99").
                WHEN "v-amt[5]" THEN 
                    cVarValue = IF v-t-amt[5] = 0 THEN "" ELSE STRING(v-t-amt[5],"->,>>>,>>>,>>>.99").
                WHEN "v-amt[6]" THEN 
                    cVarValue = IF v-t-amt[6] = 0 THEN "" ELSE STRING(v-t-amt[6],"->,>>>,>>>,>>>.99").
                WHEN "v-msf[5]" THEN 
                    cVarValue = IF v-t-msf[5] = 0 THEN "" ELSE STRING(v-t-msf[5],"->,>>>,>>>,>>>.99").
                WHEN "v-msf[6]" THEN 
                    cVarValue = IF v-t-msf[6] = 0 THEN "" ELSE STRING(v-t-msf[6],"->,>>>,>>>,>>>.99").
                ...
                WHEN "v-pctMSFMTD" THEN 
                    cVarValue = IF v-pctmsfMTD = 0 THEN "" ELSE STRING(v-pctmsfMTD,"->,>>>,>>>,>>>.99").
                WHEN "v-profit" THEN 
                    cVarValue = IF v-t-amt[1] = 0 AND v-costTotal = 0  THEN "" ELSE STRING(v-t-amt[1] - v-costTotal,"->,>>>,>>>,>>>.99"). 
                WHEN "v-cost" THEN 
                    cVarValue = IF v-costTotal = 0 THEN "" ELSE STRING(v-costTotal,"->,>>>,>>>,>>>.99"). 
            END CASE.
