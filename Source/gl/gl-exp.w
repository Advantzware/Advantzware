...
ASSIGN cTextListToSelect = "Id,Date,Reverse Entry,Line,Account,Description,Amount"

      cFieldListToSelect = "journal,date,rev-entry,line,account,desc,amt" .
{sys/inc/ttRptSel.i}

    ASSIGN cTextListToDefault  = "Id,Date,Reverse Entry,Line,Account,Description,Amount" .
...
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-glexp.csv" 
...
DEFINE FRAME rd-fgexp
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "General Ledger Journal Entries to Excel" WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_date end_date sl_avail sl_selected tb_excel tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_date end_date Btn_Def sl_avail sl_selected 
    ...
    FOR EACH ttRptSelected:
      ...
      /* v-excel-detail-lines = v-excel-detail-lines + 
         ...
         CASE ttRptSelected.FieldList:                                                              
             WHEN "journal" THEN                                                                
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gl-jrn.journal)).           
             WHEN "date" THEN                                                              
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gl-jrn.tr-date)).         
             WHEN "rev-entry" THEN                                                              
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gl-jrn.reverse)).         
             WHEN "line" THEN                                                             
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gl-jrnl.LINE)).        
             WHEN "account" THEN                                                              
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gl-jrnl.actnum)).         
             WHEN "desc" THEN                                                                
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(gl-jrnl.dscr).           
             WHEN "amt" THEN                                                             
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gl-jrnl.tr-amt)).        
