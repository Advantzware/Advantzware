...
ASSIGN cTextListToSelect = "Run,Date,Account,Account Dscription,Dscription,Journal,Year,Period,Amount"

      cFieldListToSelect = "run,date,acc,acc-dscr,dscr,jrnl,yr,period,amt" .
{sys/inc/ttRptSel.i}

    ASSIGN cTextListToDefault  = "Run,Date,Account,Dscription,Journal,Year,Period,Amount" .
...
begin_acc end_acc begin_jrnl end_jrnl Btn_Def sl_avail sl_selected Btn_Add ~
Btn_Remove btn_Up btn_down tb_runExcel fi_file btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_date end_date begin_acc end_acc ~
begin_jrnl end_jrnl sl_avail sl_selected tb_excel tb_runExcel fi_file 
...
DEFINE VARIABLE begin_acc AS CHARACTER FORMAT "X(25)" 
     LABEL "From Account" 
     VIEW-AS FILL-IN 
     SIZE 17.8 BY 1.
...
DEFINE VARIABLE begin_jrnl AS CHARACTER FORMAT "X(10)" 
     LABEL "From Journal" 
     VIEW-AS FILL-IN 
     SIZE 17.8 BY 1.

DEFINE VARIABLE end_acc AS CHARACTER FORMAT "X(25)" INITIAL "zzzzzzzzzzzzzzz" 
     LABEL "To Account" 
     VIEW-AS FILL-IN 
     SIZE 17.8 BY 1.
...
DEFINE VARIABLE end_jrnl AS CHARACTER FORMAT "X(10)" INITIAL "zzzzzzzzzz" 
     LABEL "To Journal" 
     VIEW-AS FILL-IN 
     SIZE 17.8 BY 1.

DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)" INITIAL "c:~\tmp~\r-gltrans.csv" 
...
DEFINE FRAME rd-fgexp
     begin_date AT ROW 3.95 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Date" WIDGET-ID 142
     end_date AT ROW 3.95 COL 65.2 COLON-ALIGNED HELP
          "Enter Ending Date" WIDGET-ID 144
     begin_acc AT ROW 5.43 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Date" WIDGET-ID 146
     end_acc AT ROW 5.43 COL 65.2 COLON-ALIGNED HELP
          "Enter Ending Account" WIDGET-ID 148
     begin_jrnl AT ROW 7.05 COL 28 COLON-ALIGNED HELP
          "Enter Beginning Journal" WIDGET-ID 150
     end_jrnl AT ROW 7.05 COL 65.2 COLON-ALIGNED HELP
          "Enter Ending Journal" WIDGET-ID 152
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "General Ledger Transaction to Excel" WIDGET-ID 100.
...
ASSIGN 
       begin_jrnl:PRIVATE-DATA IN FRAME rd-fgexp     = 
                "parm".
...
ASSIGN 
       end_jrnl:PRIVATE-DATA IN FRAME rd-fgexp     = 
                "parm".
...
ON HELP OF FRAME rd-fgexp /* General Ledger Transaction to Excel */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rd-fgexp rd-fgexp
ON WINDOW-CLOSE OF FRAME rd-fgexp /* General Ledger Transaction to Excel */
DO:
...
ON LEAVE OF begin_acc IN FRAME rd-fgexp /* From Account */
DO:
   assign {&self-name}.
END.
...
&Scoped-define SELF-NAME begin_jrnl
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_jrnl rd-fgexp
ON LEAVE OF begin_jrnl IN FRAME rd-fgexp /* From Journal */
DO:
   assign {&self-name}.
END.
...
ON LEAVE OF end_acc IN FRAME rd-fgexp /* To Account */
DO:
     assign {&self-name}.
END.
...
&Scoped-define SELF-NAME end_jrnl
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_jrnl rd-fgexp
ON LEAVE OF end_jrnl IN FRAME rd-fgexp /* To Journal */
DO:
     assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_date end_date begin_acc end_acc begin_jrnl end_jrnl sl_avail 
          sl_selected tb_excel tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_date end_date begin_acc end_acc begin_jrnl 
         end_jrnl Btn_Def sl_avail sl_selected Btn_Add Btn_Remove btn_Up 
...
DEFINE VARIABLE cAccountDscr AS CHARACTER NO-UNDO .
...
FOR EACH gltrans NO-LOCK
    WHERE gltrans.company = cocode  
      and gltrans.jrnl GE begin_jrnl
      and gltrans.jrnl LE END_jrnl
      and gltrans.actnum GE begin_acc
      and gltrans.actnum LE END_acc
      AND gltrans.tr-date GE begin_date
      AND gltrans.tr-date LE end_date :

    v-excel-detail-lines = "".
    cAccountDscr = "".
     FIND FIRST account  NO-LOCK
        WHERE account.company EQ cocode 
          AND account.actnum EQ  gltrans.actnum NO-ERROR .
    IF AVAIL account THEN
          cAccountDscr = account.dscr .
    
    FOR EACH ttRptSelected:
      
         CASE ttRptSelected.FieldList:                                                              
             WHEN "run" THEN                                                                
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.trnum)).           
             WHEN "date" THEN                                                              
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.tr-date)).         
             WHEN "acc" THEN                                                              
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.actnum)).         
             WHEN "acc-dscr" THEN                                                             
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(cAccountDscr)).        
             WHEN "dscr" THEN                                                              
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.tr-dscr)).         
             WHEN "jrnl" THEN                                                                
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(gltrans.jrnl).           
             WHEN "yr" THEN                                                        
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.yr)).   
             WHEN "period" THEN                                                             
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.period)). 
             WHEN "amt" THEN                                                          
                 v-excel-detail-lines = v-excel-detail-lines + appendXLLine(string(gltrans.tr-amt)). 
