...
&SCOPED-DEFINE h_Browse01 h_b-gljrnr
&SCOPED-DEFINE h_Object01 h_p-navico
&SCOPED-DEFINE h_Object02 h_p-cashl
&SCOPED-DEFINE h_Object03 h_v-glload
...
DEFINE VARIABLE h_b-gljrnr AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-glload AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-glload AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "General Ledger Recurring Journal Entries"
         HEIGHT             = 24
         WIDTH              = 150
         MAX-HEIGHT         = 320
         MAX-WIDTH          = 320
...
ON END-ERROR OF W-Win /* General Ledger Recurring Journal Entries */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* General Ledger Recurring Journal Entries */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse|Detail|Load Journals' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gl/b-gljrnr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-gljrnr ).
       RUN set-position IN h_b-gljrnr ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-gljrnr ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_b-gljrnr. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-gljrnr ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'win':U , h_b-gljrnr ).
       RUN add-link IN adm-broker-hdl ( h_b-gljrnr , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-gljrnr ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_v-gljrn ).
       RUN set-position IN h_v-gljrn ( 5.52 , 7.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-gljrnr , 'Record':U , h_v-gljrn ).
       ...
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'recurring':U , h_v-gljrn ).
       RUN add-link IN adm-broker-hdl ( h_v-gljrn , 'is-balanced':U , THIS-PROCEDURE ).

       /* Links to SmartNavBrowser h_b-gljrn2. */
       RUN add-link IN adm-broker-hdl ( h_b-gljrnr , 'browse':U , h_b-gljrn2 ).
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gl/b-glload.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-glload ).
       RUN set-position IN h_b-glload ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-glload ( 17.62 , 145.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gl/v-glload.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-glload ).
       RUN set-position IN h_v-glload ( 22.19 , 53.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.38 , 44.00 ) */

       /* Links to SmartViewer h_v-glload. */
       RUN add-link IN adm-broker-hdl ( h_b-glload , 'load-recurring':U , h_v-glload ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-glload ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-glload ,
             h_b-glload , 'AFTER':U ).
    END. /* Page 3 */
