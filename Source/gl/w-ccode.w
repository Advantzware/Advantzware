...
ON END-ERROR OF W-Win /* Currency Code */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Currency Code */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Browse Curr.|View Currency' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gl/b-ccode.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-ccode ).
       RUN set-position IN h_b-ccode ( 4.81 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-ccode ( 19.52 , 116.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_b-ccode. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-ccode ).
       RUN add-link IN adm-broker-hdl ( h_b-ccode , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-ccode ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'gl/v-ccode.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-ccode ).
       ...
       /* Links to SmartViewer h_v-ccode. */
       RUN add-link IN adm-broker-hdl ( h_b-ccode , 'Record':U , h_v-ccode ).
