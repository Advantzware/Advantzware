...
DEF VAR lv-au AS cha FORM "x(21)" NO-UNDO.
...
DEF VAR lv-rel-qty AS CHAR EXTENT 20 NO-UNDO.
DEF VAR lv-rel-date AS CHAR EXTENT 20 NO-UNDO.
DEF VAR lv-score-check AS CHAR EXTENT 30 NO-UNDO.
DEF VAR lv-tmp-val AS cha NO-UNDO.

DEF VAR v-rel-qty AS INT NO-UNDO.
DEF VAR v-ordl-qty AS INT NO-UNDO.
...
DEF VAR v-plate-no AS cha FORM "X(15)" NO-UNDO.
...
DEF VAR v-shp-id AS CHAR NO-UNDO .
DEF VAR v-shp-add1 AS CHAR NO-UNDO .
DEF VAR v-shp-add2 AS CHAR NO-UNDO .
DEF VAR v-shp-add3 AS CHAR NO-UNDO .
DEF VAR v-shp-add4 AS CHAR NO-UNDO .
...
def new shared var v-type-score-len     as   char  no-undo.
def new shared var v-type-score-wid     as   char  no-undo.
def new shared var v-type-score-wid2     as   char  no-undo.
...
DEF BUFFER bf-xoe-ordl FOR oe-ordl .
...
FORM HEADER
       ...
       v-loc-bin                        at 124
*/
    with no-box frame head-bell no-labels width 170 NO-ATTR-SPACE STREAM-IO.
      ...
      for each w-ef WHERE (w-ef.frm = job-hdr.frm OR est.est-type <> 8),
        ...
        ASSIGN lv-over-run = IF AVAIL xoe-ordl THEN trim(string(xoe-ordl.over-pct,">>9.99%")) ELSE
           ...
           ASSIGN
               ...
               lv-au = lv-au + "   " +  IF itemfg.alloc THEN "U" ELSE "A"
       ...
       "<C40>Production Specification </B><P10>Part item name:" lv-part-name SKIP*/
        "<P10></PROGRESS>" SKIP "<FCourier New><C2><B>" lv-au "<C33>" lv-est-type "</B>"
       "<P12><B><C90>BELL JOB TICKET <P10>" /*AT 140*/  /*caps(SUBSTRING(v-fg,1,1)) FORM "x" AT 40*/       
       SKIP

       "<#1><C1><FROM><C105><R+45><RECT><|3>" 
       "<=1><C32><FROM><R+17><C32><LINE><|3>"
       "<=1><C66><FROM><R+17><C66><LINE><|3>"
       ...
       view frame head-bell .  /* factory header display  */  
       ...
       if v-format eq "RFC" or v-format eq "Boxtech" then
        ...
        v-plate-loc = IF AVAIL xxprep THEN xxprep.loc-bin ELSE "".
                    ...
                    DO X = 1 TO EXTENT(xeb.k-wid-array2):
                       ASSIGN
                           v-type-score-wid  = v-type-score-wid  + trim(xeb.k-wid-scr-type2[X]) .
                   END.
                   DO X = 1 TO EXTENT(xeb.k-len-array2):
                       ASSIGN
                           v-type-score-len  = v-type-score-len +  trim(xeb.k-len-scr-type2[X]).
                  END.
   
     X = 1 . 
     DO i = 1 TO LENGTH(v-len-score):

         lv-tmp-val = SUBSTRING(v-len-score,i,1).
         IF lv-tmp-val = "" THEN
             X = X + 1 .
         ELSE
         lv-score-check[X] = lv-score-check[X] + lv-tmp-val .
     END.   

      DO X = 1 TO EXTENT(xeb.k-wid-array2):
         IF X = 31 THEN LEAVE.
          ASSIGN
              v-type-score-wid2  = v-type-score-wid2  + TRIM(xeb.k-wid-scr-type2[X]) .
          
          IF  xeb.k-wid-scr-type2[X] <> "" THEN do:
              IF length(lv-score-check[X]) = 4 THEN
                  ASSIGN
                  v-type-score-wid2  = v-type-score-wid2  + "    " .
              IF length(lv-score-check[X]) = 5 THEN
                  ASSIGN
                  v-type-score-wid2  = v-type-score-wid2  + "     " .
              IF length(lv-score-check[X]) = 3 THEN
                  ASSIGN
                  v-type-score-wid2  = v-type-score-wid2  + "   " .
              IF length(lv-score-check[X]) = 6 THEN
                  ASSIGN
                  v-type-score-wid2  = v-type-score-wid2  + "      " .
          END.
          ELSE do:
              ...
              IF length(lv-score-check[X]) = 3 THEN
                  ...
                  v-type-score-wid2  = v-type-score-wid2  + "    " .
              IF length(lv-score-check[X]) = 6 THEN
               ASSIGN v-type-score-wid2  = v-type-score-wid2  + "       " .
          END.
      END.
    
      IF AVAIL xoe-ordl AND AVAIL xeb THEN
        find first bf-xoe-ordl
        where bf-xoe-ordl.company eq xoe-ordl.company
        and bf-xoe-ordl.ord-no  eq xoe-ordl.ord-no
        and bf-xoe-ordl.job-no  eq job-hdr.job-no
        and bf-xoe-ordl.job-no2 eq job-hdr.job-no2
        and bf-xoe-ordl.i-no    eq xeb.stock-no
        no-error.

       v-ordl-qty = if avail bf-xoe-ordl then bf-xoe-ordl.qty ELSE IF AVAIL xoe-ordl THEN (xoe-ordl.qty * if est.form-qty le 1 then 1
                                            else v-pqty ) ELSE  (xJobQty * if est.form-qty le 1 then 1 else v-pqty) .
        ...
        DISP  v-cus[1] AT 2 "PO #:" AT 40 xoe-ord.po-no WHEN AVAIL xoe-ord 
             ...
             /* "Job Qty:" AT 40 trim(string(xJobQty * v-pqty,">>>,>>9"))    format "x(7)" */
              "Job Qty:" AT 40 trim(string(job-hdr.qty,">>>,>>9"))    format "x(7)"  
              "Order Qty:" trim(STRING(v-ordl-qty,">>>,>>9"))
                                            format "x(7)"
                          "   "
              ...
              SKIP
              v-cus[3] AT 2 "Set Part#:" AT 40 lv-part-no
              "Joint:" AT 80 v-joint-dscr             
              SKIP
              v-cus[4] AT 2
             
              "Set Item Name: " AT 40 lv-fg-name FORM "x(26)"
              ...
              SKIP
              v-form-dscr  AT 2 FORM "x(30)" "Ink 3:" AT 39 v-ink-3 "Die   Size:" AT 80
              ...
              "Adders:" AT 2 v-adders FORM "x(29)"
              ...
              SKIP
               "Score:" AT 2 v-len-score WHEN xstyle.TYPE <> "F" format "x(130)" SKIP
               /*"D/C Style:" AT 80 SKIP*/
               " Type:  " AT 2 v-type-score-wid2 FORMAT "x(130)"  SKIP
        ...
        put skip
            "<P7>   Machine Routing                       Date        OppSU:     Start       Stop              RUN:      Hours      Start       Stop          QTY:  In        Out          <P10>" SKIP.
        i = 0.
        ...
        for each w-m by w-m.dseq:
          ...
          DO:
             display w-m.dscr AT 3
                     ...
                     fill("_",7)  format "x(7)"    to 36   when w-m.dscr ne ""
                     fill("_",7)  format "x(7)"    to 44   when w-m.dscr ne ""
                     fill("_",7)  format "x(7)"    to 52   when w-m.dscr ne ""
                     fill("_",7)  format "x(7)"    to 60   when w-m.dscr ne ""
                     space(4)
                     w-m.r-sp                              when w-m.r-sp ne 0
                     w-m.r-hr                              when w-m.r-hr ne 0
                     fill("_",7)  format "x(7)"    to 87   when w-m.dscr ne ""
                     fill("_",7)  format "x(7)"    to 96   when w-m.dscr ne ""
                     fill("_",7)  format "x(7)"    to 106    when w-m.dscr ne ""
                     fill("_",8)  format "x(8)"    to 117  when w-m.dscr ne ""
                     /*fill("_",8)  format "x(8)"    to 122  when w-m.dscr ne ""*/
        ...
        PUT SKIP(1) "<#6><C1><FROM><C105><LINE><|3>" .
            
        RUN cecrep/jobbell2.p (RECID(job-hdr),v-format,cust.terms).

        PUT "<=6><C28><FROM><R+6><C28><LINE><|3>"
            "<=6><C60><FROM><R+6><C60><LINE><|3>"
            /*"<=6><C64><FROM><R+8><C64><LINE><|3>"*/
            "<=6><C79><FROM><R+6><C79><LINE><|3>"            
            "<=6><C93><FROM><R+6><C93><LINE><|3>".
        ...
        FOR EACH notes WHERE notes.rec_key = itemfg.rec_key NO-LOCK.
         ...
         RELEASE oe-rel-whse.
          ...
          ASSIGN
              ...
              v-shpnote[4] = ''. 
         FOR EACH b-oe-rel
         WHERE b-oe-rel.company EQ xoe-ordl.company
           AND b-oe-rel.ord-no  EQ xoe-ordl.ord-no
           AND b-oe-rel.i-no    EQ xoe-ordl.i-no
           AND b-oe-rel.line    EQ xoe-ordl.LINE
           NO-LOCK BY b-oe-rel.rel-date:
             FIND FIRST oe-rell
                 WHERE oe-rell.company EQ b-oe-rel.company
                 AND oe-rell.ord-no  EQ b-oe-rel.ord-no      
                 AND oe-rell.i-no    EQ b-oe-rel.i-no
                 AND oe-rell.line    EQ b-oe-rel.line
                 AND oe-rell.link-no EQ b-oe-rel.r-no
                 NO-LOCK NO-ERROR.
             IF AVAIL oe-rell THEN
                 FIND FIRST oe-relh WHERE oe-relh.r-no EQ oe-rell.r-no NO-ERROR.
             
             ASSIGN
             lv-rel-qty[i] = string(b-oe-rel.tot-qty )
             lv-rel-date[i] = IF AVAIL oe-relh THEN string(oe-relh.rel-date) ELSE string(b-oe-rel.rel-date) .
              i = i + 1.

            IF i = 2 THEN 
                ASSIGN
                v-shp-id = b-oe-rel.ship-id 
                v-shpnote[1] = b-oe-rel.ship-i[1]
                v-shpnote[2] = b-oe-rel.ship-i[2]
                v-shpnote[3] = b-oe-rel.ship-i[3] 
                v-shpnote[4] = b-oe-rel.ship-i[4] 
                v-shp-add2  =  b-oe-rel.ship-addr[1]
                v-shp-add3  =  b-oe-rel.ship-addr[2]
                v-shp-add4  =  trim(b-oe-rel.ship-city) + ", " +
                b-oe-rel.ship-state + "  " + b-oe-rel.ship-zip 
                .
            IF v-shp-id = "" THEN
                ASSIGN v-shp-id = b-oe-rel.ship-id 
                       v-shpnote[1] = b-oe-rel.ship-i[1]
                v-shpnote[2] = b-oe-rel.ship-i[2]
                v-shpnote[3] = b-oe-rel.ship-i[3] 
                v-shpnote[4] = b-oe-rel.ship-i[4] 
        ...
        ASSIGN 
             ...
             /*  v-ship-id    = IF v-whs-ship-id NE "" THEN "LOC"
                              ...
                              ELSE IF AVAIL xeb AND TRIM(xeb.ship-id) NE "" THEN
                                   xeb.ship-id
                              ELSE "".*/
        IF v-shp-id = "" AND AVAIL xoe-ord THEN
            ASSIGN  v-shp-id = xoe-ord.sold-id . 
        ...
        FIND FIRST bf-shipto-notes NO-LOCK WHERE
                   ...
                   bf-shipto-notes.ship-id EQ v-shp-id NO-ERROR.
        IF AVAIL bf-shipto-notes THEN
            v-shp-add1  =  bf-shipto-notes.ship-name. 
           IF i = 1 AND AVAIL bf-shipto-notes THEN
        ...
        DISPLAY "<=7><C30>" AT 2   "Shipping Info: <P10>" AT 92 SKIP
                "Sched Rel:  " AT 35 lv-rel-qty[1] lv-rel-date[1] "Ship To #:" AT 90
                 v-shp-id
                ...
                lv-rel-qty[2] AT 48 lv-rel-date[2]  v-shp-add1 FORMAT "x(36)" AT 90 SKIP
                lv-rel-qty[3] AT 48 lv-rel-date[3]  v-shp-add2 FORMAT "x(36)" AT 90 SKIP
                lv-rel-qty[4] AT 48 lv-rel-date[4]  v-shp-add3 FORMAT "x(36)" AT 90 SKIP
                lv-rel-qty[5] AT 48 lv-rel-date[5]  v-shp-add4 FORMAT "x(36)" AT 90 SKIP
                "Spec/Notes                                         " AT 35 "Ship Notes #:" AT 90 /*xoe-ordl.po-no when avail xoe-ordl*/
                 v-spec-note[1] FORM "x(41)" AT 35        substring(v-shpnote[1],1,36) FORMAT "x(36)" AT 90                
                 v-spec-note[2] FORM "x(41)" AT 35        substring(v-shpnote[1],37,60) FORMAT "x(36)" AT 90   
                 v-spec-note[3] FORM "x(41)" AT 35        substring(v-shpnote[2],1,36) FORMAT "x(36)" AT 90    
                 v-spec-note[4] FORM "x(41)" AT 35        substring(v-shpnote[2],37,60) FORMAT "x(36)" AT 90     
                 v-spec-note[5] FORM "x(41)" AT 35        substring(v-shpnote[3],1,36) FORMAT "x(36)" AT 90  
                 v-spec-note[6] FORM "x(41)" AT 35       substring(v-shpnote[3],37,60) FORMAT "x(36)" AT 90  SKIP
       ...
       /* v-spec-note = "" */
        ...
        ASSIGN
           ...
           v-plate-no = IF AVAIL xeb THEN xeb.plate-no ELSE ""  .
        ...
        PUT "<#11><C1><FROM><C105><R+47><RECT><|3>"  
            ...
            "<C80><B> Plate#: </B>" v-plate-no FORMAT "X(20)" skip
            "<C1><FROM><C105><LINE><|3>".

        if print-box and avail xest then do:
            
           run cec/desprntbell.p (recid(xef),
