/* ---------------------------------------------- */
/*  factory ticket for PFS-a-pack copied from CSC                    */
/* -------------------------------------------------------------------------- */
/*  BPV 03/05/13  */
...
DEF VAR lv-line-chars AS INT INIT 46 NO-UNDO.
DEF VAR v-prev-note-rec AS RECID NO-UNDO.
DEF VAR v-prev-extent AS INT NO-UNDO.
DEF VAR v-cntct-no AS CHAR FORMAT "x(25)" NO-UNDO.
DEF NEW SHARED VAR v-bn-note AS CHAR NO-UNDO .
{jcrep/r-ticket.i "shared"}

{cecrep/jobpfs.i "new shared"}
...
DEF VAR v-inst2 AS cha FORMAT "x(46)" EXTENT 14 NO-UNDO.    
DEF VAR v-dept-inst AS cha FORM "x(46)" EXTENT 14 NO-UNDO.
DEF VAR v-note-length AS INT INIT 46 NO-UNDO.
...
DEF VAR v-no-per-bndl AS CHAR NO-UNDO .
...
do v-local-loop = 1 to v-local-copies:
    ...
    for each job-hdr
      ...
      IF AVAIL xoe-ord THEN 
           v-cntct-no = xoe-ord.contact .
      IF v-cntct-no EQ "" THEN DO:
          IF AVAIL cust THEN
              v-cntct-no = cust.contact .
      END.
        ...
        DISP "<=#1><B>CUSTOMER" AT 2 "SHIP TO:" AT 35
             ...
             "ORDER DATE" AT 60 "DUE DATE" AT 72 "TOTAL SQ FT" AT 83 SKIP
              ...
              "FG#:" AT 2  xeb.stock-no "CUST. PART#:" xeb.part-no "SHEET SIZE:</PROGRESS>" AT 60
               ...
               "MATERIAL:" AT 60 trim(v-form-dscr) FORM "X(25)" SKIP
               xeb.part-dscr2 WHEN AVAIL xeb AT 5
               "ADDER:" AT 60 v-adders FORM "x(28)"
               SKIP
               itemfg.part-dscr2 WHEN AVAIL itemfg AT 5
               "SCORING:" AT 60  v-len-score  WHEN xstyle.TYPE <> "F"  format "x(27)"
               SKIP
                itemfg.part-dscr3 WHEN AVAIL itemfg AT 5
                SKIP
               ...
               SKIP
               "FINISH BALE COUNT      TOTAL" AT 49 SKIP(2)
     ...
     /* PUT "<#6><C1><FROM><C105><LINE><|3>" .*/
        ...
        RUN cecrep/jobpfs2.p (INPUT RECID(job-hdr), INPUT w-ef.frm).
        ...
        IF AVAIL xeb THEN DO:
            IF xeb.cas-no EQ "FP" THEN
                ASSIGN v-no-per-bndl = xeb.cas-no .
            ELSE
                ASSIGN v-no-per-bndl = string(xeb.cas-cnt) .
        END.

        PUT "<#10><C1><FROM><C80><LINE><|3>  <=10>"
                "<#10><C40><FROM><R+9><C40><LINE><|3>  <=10><R-1>"  SKIP.
        display "SPECIAL INSTRUCTIONS:"  AT 1 "STACKING:" AT 48 
                v-dept-inst[1] format "x(46)" AT 2        "Pallet ID:" xeb.tr-no when avail xeb  /*     v-dept-inst[8] format "x(46)" AT 50          */
...
/*             END. */
            find box-design-hdr
                ...
                and box-design-hdr.company = xeb.company 
            ...
            IF AVAIL box-design-hdr THEN do:
                ...
                PUT unformatted "<R43><C1><#30><R+22><C+80><IMAGE#30=" FILE-INFO:FULL-PATHNAME ">" .
           END.
      
        end.
       /* ELSE /*PAGE.*/  PUT CHR(12). /*Page Break*/*/
       IF LAST(job.job-no2) THEN PUT "" .
       ELSE PUT CHR(12). 
      end.  /* for each w-ef */
    end.  /* each job */
