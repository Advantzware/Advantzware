...
format header
    ...
    with no-box frame head-fluted no-labels width 146 /*132*/  STREAM-IO.
    ...
    {cecrep/jobprem.i}
      ...
      for each w-ef WHERE (w-ef.frm = job-hdr.frm OR est.est-type <> 8),
        ...
        IF NOT FIRST(w-ef.frm) THEN
            PUT SKIP(1).
        view frame head-fluted.  /* factory header display  */
     ...
     PUT UNFORMATTED
            ...
            "<=#4><R+1> " "Item Name: <b>" (IF AVAIL xeb THEN xeb.part-dscr1 ELSE "") /*cProcat*/ FORM "x(40)"
        ...
        ELSE DO:
                PUT "<=#5><R+1><b>"
                    ...
                    trim(string(v-sht-qty * v-form-sqft / 1000,">>>9.9<"))
                                                                format "x(11)".
                 PUT "</b><=#6><R-5> Meters Planned: _________________<b>"   .                                                             
               /*PUT "</b><=#6><R-5> Ink 1:<b>"
                   w-i.i-dscr
            ...
            /* PUT     "</b><=#7><R+2> Blank Size:<b>"
             ...
             PUT     "</b><=#6><R-3> Bottom Length: _________________<b>" SKIP(2).
             /*PUT     "</b><=#6><R-3> Ink 3:<b>"
                     w-i.i-dscr
                     IF string(w-i.i-qty) ne "0" THEN string(w-i.i-qty) ELSE ""
                     IF w-i.i-dscr ne "" THEN "LBS" ELSE "".
      ...
      /* for xprint */
        ...
        PUT SKIP(1).
        run cecrep/jobtickfluted.p (recid(job-hdr),v-format,cust.terms).
        RUN stackImage.
       ...
       /* if print-box and avail xest then do:            
        ...
        IF s-prt-fgimage THEN DO:        
            ...
            PUT UNFORMATTED                  
                ...
                "<=12><R+4><C1><FROM><C81><LINE><||3>"
                "<=12><R+5><C5><#21><R+45><C+74><IMAGE#21=" ls-fgitem-img ">" SKIP. 
            PAGE.
         END.
...
PROCEDURE stackImage:
  ...
  IF v-stackcode EQ '' THEN RETURN.   
   FIND FIRST stackPattern NO-LOCK
       WHERE stackPattern.stackCode EQ SUBSTR(v-stackcode,12,1) NO-ERROR.
      
  IF AVAILABLE stackPattern AND SEARCH(stackPattern.stackImage) NE ? THEN     
  PUT UNFORMATTED  "<C60>"
    "<#71><C1><R+8><FROM><C17><R-12>"
