/* ----------------------------------------------  */
/*  cecrep/jobPkgAtl.p  Corrugated factory ticket  for Xprint landscape */
/* -------------------------------------------------------------------------- */
...
IF lRecFound THEN
    ls-fgitem-img = cRtnChar NO-ERROR.
...
FOR EACH w-ef WHERE (w-ef.frm = job-hdr.frm OR est.est-type <> 8),
    ...
    IF v-job-cust AND NOT AVAILABLE xoe-ord AND AVAILABLE cust THEN
        ASSIGN
            ...
            .
    ASSIGN 
        cBarCodeVal = TRIM(STRING(DYNAMIC-FUNCTION('sfFormat_JobFormatWithHyphen', job-hdr.job-no, job-hdr.job-no2))) + "-" + STRING(xeb.form-no,"99") + "-" + STRING(xeb.blank-no,"99") .
    ASSIGN  
        cCustName = cust.NAME 
    ...
    PUT 
        ...
        "<=BoxImageStart><C+61><R+18><#BoxImageEnd>"
        .
    ...
    DO:
        ...
        DO:
            ...
            RUN cec/desprntL20.p (RECID(xef),
                ...
                v-format ).
        END.
        ELSE IF AVAILABLE box-design-hdr THEN 
            DO:
                cImagePath = box-design-hdr.box-image.
                PUT UNFORMATTED 
                    "<=BoxImageStart><C+.5><R+.5><#BoxImage><R50><C62><IMAGE#BoxImage=" cImagePath "><=BoxImage>".
            END.
        PUT "<P8>" .
    END. 
    ...
    PUT "<=BoxImageStart><FROM><LINE#BoxImageTR><|1>"
        ...
        "<=BoardStart><R+3><RIGHT=C+5>Test: <#TestPrint>" 
        "<=BoardStart><R+3><RIGHT=C+23>Flute: <#FlutePrint>"
        "<=BoardStart><R+4><RIGHT=C+5>Size: <#SheetsSize>"
        "<=BoardStart><R+4><RIGHT=C+23>MSF: <#SheetsMSF>"
        "<=BoardStart><R+5><RIGHT=C+5>Scores: <#Scores>"
        "<=BoardStart><R+6><RIGHT=C+5>Adders:"
        "<=BoardStart><R+6><C6><#Adders1><C20><#Adders2>"
        "<=BoardStart><R+7><C6><#Adders3><C20><#Adders4>"
        "<=BoardStart><R+8><RIGHT=C+5>PO: <#VendorPO>"
        "<=BoardStart><R+8><RIGHT=C+20>Vendor: <#VendorCode>"
        ...
        "<=PrintStart><R+1><RIGHT=C+35>BMC #: <#SpcQc>"
    ...
    PUT "<FGColor=Blue><B>"
        ...
        "<=TabInOut>" IF AVAILABLE xeb AND xeb.tab-in EQ YES THEN "In" ELSE IF AVAILABLE xeb AND xeb.tab-in EQ NO THEN "Out" ELSE "" FORMAT "x(10)"
        ...
        "<=SpcQc>" IF AVAILABLE xeb THEN xeb.spc-no ELSE "" FORMAT "x(30)"
        .
...
PUT  /*"<C1><R1.2><#Start>"
    ...
    "<=Start><R39.2><#PageStart>"
    "<=PageStart><#PackingStart>"
    "<=PackingStart><C+25><#PackingTR>"
    "<=PackingStart><R+11><#PackingBL>"
    "<=PackingStart><C+25><R+11><#PackingEnd>"
    ...
    "<=PatternImageStart><R+11><#PatternImageBL>"
    "<=PatternImageStart><C+22><R+11><#PatternImageEnd>"
    ...
    "<=ShippingStart><R+11><#ShippingBL>"
    "<=ShippingStart><C108><R+11><#ShippingEnd>"
    ...
    "<=ShippingStart><R+6><C85>Carrier: <#Carrier>"
    ...
    "<=StrapsPallets>" IF AVAILABLE stackPattern THEN STRING(stackPattern.strapCount) ELSE "" FORMAT "x(6)"
    ...
    "<=Carrier>" IF AVAILABLE xoe-ord THEN xoe-ord.carrier ELSE ""  FORMAT "x(20)"
...
PUT "<||3><R39.2><C1><FROM><C108><LINE><||3>" .                                                                              
PAGE.
           
PUT UNFORMATTED 
    "<R2><C2><#1><R+47><C+108><IMAGE#ItemImage=" ls-fgitem-img ">".
PAGE.
