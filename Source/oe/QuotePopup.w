...
DEF OUTPUT PARAM opcChoice AS CHAR NO-UNDO. 
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-3 ttQuoteList.q-no ttQuoteList.qty ttQuoteList.price ttQuoteList.uom ttQuoteList.est-no ttQuoteList.quote-date ttQuoteList.rel ttQuoteList.rel-qty  ttQuoteList.part-no 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-3 gDialog _FREEFORM
  QUERY BROWSE-3 DISPLAY
      ...
      ttQuoteList.part-no FORMAT "x(15)" LABEL "Cust Part" 
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 110 BY 8.81 FIT-LAST-COLUMN.
...
DEFINE FRAME gDialog
     ...
     btn_price AT ROW 12.62 COL 3
     btn_priceqty AT ROW 12.62 COL 30.6 WIDGET-ID 2
     btn_ok AT ROW 12.62 COL 30 
     Btn_Cancel AT ROW 12.62 COL 58.6
...
FOR EACH quotehd FIELDS(q-no est-no company quo-date) NO-LOCK  WHERE
    ...
    ASSIGN 
        ...
        ttQuoteList.rwRowid    = ROWID(quoteqty)
        ttQuoteList.part-no  =  quoteitm.part-no
        .
END.
...
lcMsg = ipcMsgStart + " for Estimate: " + ipxEstNo + CHR(10) .
