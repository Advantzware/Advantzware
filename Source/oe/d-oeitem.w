...
DEFINE FRAME d-oeitem
    ...
    fiPromDtLabel AT ROW 14.57 COL 103.8 COLON-ALIGNED NO-LABELS WIDGET-ID 26
    fi_type-dscr AT ROW 7.67 COL 119.4 COLON-ALIGNED NO-LABELS
    ...
    VIEW-AS FILL-IN 
    SIZE 16.6 BY 1
    oe-ordl.job-no2 AT ROW 1.14 COL 112 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 4.4 BY 1
    ...
    "Enter Unit of Measure for Purchasing this Raw Material" NO-LABELS
...
DEFINE FRAME d-oeitem
    ...
    oe-ordl.type-code AT ROW 7.67 COL 113.6 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    ...
    VIEW-AS TOGGLE-BOX
    SIZE 18 BY .81
    oe-ordl.s-man[1] AT ROW 13.14 COL 2.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 9 BY 1
    oe-ordl.s-pct[1] AT ROW 13.14 COL 45.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 12 BY 1
    oe-ordl.s-comm[1] AT ROW 13.14 COL 60.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 9.2 BY 1
    oe-ordl.s-man[2] AT ROW 14.14 COL 2.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 9 BY 1
    oe-ordl.s-pct[2] AT ROW 14.14 COL 45.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 12 BY 1
    oe-ordl.s-comm[2] AT ROW 14.14 COL 60.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 9.2 BY 1
    oe-ordl.s-man[3] AT ROW 15.14 COL 2.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 9 BY 1
    oe-ordl.s-pct[3] AT ROW 15.14 COL 45.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 12 BY 1
    oe-ordl.s-comm[3] AT ROW 15.14 COL 60.2 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 9.2 BY 1
    fi_s-pct-lbl AT ROW 12.43 COL 45.2 COLON-ALIGNED NO-LABELS
    fi_s-comm-lbl AT ROW 12.43 COL 60.2 COLON-ALIGNED NO-LABELS
    fi_sman-lbl AT ROW 12.38 COL 1.2 COLON-ALIGNED NO-LABELS
    fi_sname-1 AT ROW 13.14 COL 13.2 COLON-ALIGNED NO-LABELS
    fi_sname-2 AT ROW 14.14 COL 13.2 COLON-ALIGNED NO-LABELS
    fi_sname-3 AT ROW 15.14 COL 13.2 COLON-ALIGNED NO-LABELS
    fi_sname-lbl AT ROW 12.38 COL 18.2 COLON-ALIGNED NO-LABELS
    ...
    VIEW-AS FILL-IN 
    SIZE 18 BY 1
    oe-ordl.prom-date AT ROW 14.52 COL 121 COLON-ALIGNED NO-LABELS
    VIEW-AS FILL-IN 
    SIZE 18 BY 1
...
DEFINE FRAME d-oeitem
    ...
    VIEW-AS FILL-IN 
    SIZE 18 BY 1
    oe-ordl.spare-char-2 AT ROW 2.19 COL 69 COLON-ALIGNED NO-LABELS WIDGET-ID 16
    VIEW-AS FILL-IN 
    SIZE 7 BY 1
            ...
            DO:
                ...
                DO:
                    ...
                    IF itemfg.cust-no NE oe-ord.cust-no AND itemfg.cust-no NE "" AND
                        AVAILABLE cust AND cust.active NE "X"                         THEN 
                    DO:
...
PROCEDURE display-est-detail :
    ...
    DO WITH FRAME {&Frame-name}:
        ...
        FIND FIRST est-qty NO-LOCK
            ...
            NO-ERROR.
        IF v-quo-price-log AND AVAILABLE est-qty AND est-qty.qty[1] NE 0 AND
    ...
    cQuoteEst = IF oe-ordl.SourceEstimateID:SCREEN-VALUE NE "" THEN oe-ordl.SourceEstimateID:SCREEN-VALUE ELSE oe-ordl.est-no:SCREEN-VALUE .
    IF AVAILABLE xest AND v-quo-price-log AND NOT ll-got-qtprice AND
        NOT CAN-FIND(FIRST tt-item-qty-price WHERE
        tt-item-qty-price.tt-selected = YES AND
        (tt-item-qty-price.part-no EQ oe-ordl.part-no:SCREEN-VALUE OR
        (tt-item-qty-price.part-no EQ oe-ordl.i-no:SCREEN-VALUE AND oe-ordl.i-no:SCREEN-VALUE NE ""))) THEN 
...
PROCEDURE final-steps :
    ...
    IF AVAILABLE oe-ordl AND (oe-ordl.est-no NE "" AND oe-ordl.job-no EQ "") THEN
...
PROCEDURE final-steps2 :
    ...
    DO TRANSACTION:
        ...
        IF oe-ord.est-no EQ ""                                       OR
            (v-qty-mod AND (NOT ll-new-record OR lv-new-tandem NE ?)) THEN 
        DO:
    ...
    ELSE IF oe-ord.type NE "T" 
            ...
            po-ord.po-no   EQ oe-ordl.po-no-po)))) THEN 
        DO:
            ASSIGN 
                lMsgResponse = TRUE.
...
PROCEDURE get-est-cost :
    ...
    DEFINE VARIABLE v-run-list AS CHARACTER NO-UNDO.
    DEFINE VARIABLE iEbCnt AS INTEGER NO-UNDO.
    IF ip-est-no NE "" AND NOT AVAILABLE xest THEN
        FIND FIRST xest WHERE xest.company EQ cocode AND
...
PROCEDURE get-price :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        IF NOT price-ent                           AND
            ...
            TRIM(oe-ordl.est-no:SCREEN-VALUE) EQ "" THEN 
        DO:
