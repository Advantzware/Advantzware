/* -------------------------- oe/rep/relhenry.i 03/22 Sachin Chahal */
/* Print OE Release/Picking tickets    for Henry Xprint             */
/* -----------------------------------------------------------------*/
...
FUNCTION fGetQty RETURNS INTEGER
  ( INPUT ipcCompany AS CHAR,INPUT ipcOrdNo AS INT,INPUT ipcItemNo AS CHAR,INPUT ipcLine AS INT )  FORWARD.
        ...
        ASSIGN
           ...
           for each fg-bin
               ...
               assign
                ...
                w-bin.w-unit-count = fGetQty(w-oe-rell.company,w-oe-rell.ord-no,w-oe-rell.i-no,w-oe-rell.line)
                w-bin.w-units  = v-pallet-count
...
FUNCTION fGetQty RETURNS INTEGER
  ( INPUT ipcCompany AS CHAR,INPUT ipcOrdNo AS INT,INPUT ipcItemNo AS CHAR,INPUT ipcLine AS INT) : 
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
    ...
    IF VALID-HANDLE(lr-rel-lib) THEN DO:
        FOR EACH oe-rel NO-LOCK
            WHERE oe-rel.company EQ ipcCompany 
              AND oe-rel.ord-no  EQ ipcOrdNo
              AND oe-rel.i-no    EQ ipcItemNo 
              AND oe-rel.line    EQ ipcLine 
