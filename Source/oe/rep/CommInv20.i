...
PUT "<=1>" SKIP.
  PUT "<C2><R2><#1><R+10><C+37><IMAGE#1=" ls-full-img1  SKIP
    ...
    SPACE(4) cust.name "<C50>" v-shipto-name  skip
    SPACE(4) cust.addr[1] "<C50>"  v-shipto-addr[1]  SKIP
    SPACE(4) cust.addr[2] "<C50>" v-shipto-addr[2]  SKIP
...
PUT "<FArial><P12><=#3><R-2> <B>Invoice#: " oe-bolh.bol-no "</B><P10>                            Page: " string(PAGE-NUM - v-page-num,">>9") SKIP
...
PUT "<FArial><=4><R+1>     Ship Date               FOB                           Ship Via                                                 Terms               S.Person       Pallets     BOL#" SKIP
     ...
     oe-ord.terms-d FORM "x(15)" space(1) v-salesman FORM "x(8)"
     v-tot-pallets FORM "->>>,>>9" SPACE(1)
     oe-bolh.bol-no FORMAT ">>>>>>9"
    SKIP.
