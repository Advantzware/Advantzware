...
if v-zone-p then v-zone-hdr = "Route No.:".
    ...
    {oe/rep/foreachr.i},
      ...
      IF AVAIL oe-ord AND oe-ord.sman[1] <> "" THEN DO:
         FIND FIRST sman WHERE sman.company EQ oe-ord.company
                           AND sman.sman    EQ oe-ord.sman[1] NO-LOCK NO-ERROR.
         cSalRepName = IF AVAIL sman THEN sman.sname ELSE oe-ord.sman[1].
      END.
             ...
             CASE vBinLineCount:
                 ...
                 WHEN 3 THEN do:
                     ...
                     IF v-bin = "" AND lOHAvail = NO THEN DO:
                        v-bin = "Job:" + string(oe-ordl.job-no) + "-" + STRING(oe-ordl.job-no2,"999").
                        IF oe-ordl.job-no = "" THEN DO:
                            FIND FIRST job-hdr WHERE job-hdr.company = oe-ordl.company
                                AND job-hdr.i-no = oe-ordl.i-no
                                AND job-hdr.opened NO-LOCK NO-ERROR.
                            IF AVAIL job-hdr THEN 
                                ASSIGN v-bin = "Job:" + string(job-hdr.job-no) + "-" + STRING(job-hdr.job-no2,"999")
...
/*                      SUBSTRING(w-bin.w-date-time,1,4)         */
           ...
           ELSE*/ /*IF vBinlineCount < 6 THEN do:
               MESSAGE " vBinlineCount " STRING(vBinlineCount) VIEW-AS ALERT-BOX ERROR .
        ...
        /*IF vBinlineCount < 6 THEN do:
            MESSAGE " vBinlineCount2222 " STRING(vBinlineCount) VIEW-AS ALERT-BOX ERROR .
           v-printline = v-printline + (7 - vBinLineCount).
           PUT SKIP(7 - vBinLineCount ).           
        END.*/
        IF v-printline > 37 THEN DO:
