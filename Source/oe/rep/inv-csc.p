/* ----------------------------------------------- oe/rep/inv-csc.p 12/97 JLF */
/*                                                                            */
/* Print invoice when sys-ctrl.char-fld eq "ContSrvc" - O/E Module            */
/*                                                                            */
/* -------------------------------------------------------------------------- */
...
def var v-inv-date as date initial TODAY NO-UNDO.
def shared var v-fr-tax as log initial NO NO-UNDO.
...
def var v as int.
def var v-rel-qty as int format "99999" no-undo.
...
DEF VAR v-surcharge-amt AS DEC NO-UNDO.
DEF VAR v-surcharge-dscr AS cha NO-UNDO.
DEF VAR v-trailer-no AS CHAR FORMAT "X(20)" NO-UNDO.
...
form header
     ...
     v-shipto-name         at 46
     inv-head.addr[1]      at 9
     v-shipto-addr[1]      at 46
     inv-head.addr[2]      at 9
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      DO TRANSACTION:
        ...
        for each xinv-line
         ...
         for each oe-bolh FIELDS(company b-no bol-date tot-pallets)
             where oe-bolh.b-no   eq xinv-line.b-no
               and oe-bolh.ord-no eq xinv-line.ord-no
             no-lock:

           for each oe-boll FIELDS(cases)
         ...
         if last-of(xinv-line.i-no) then do:
           if xinv-line.est-no ne "" then do:
             ...
             if xinv-line.form-no eq 0 and xinv-line.est-type eq 2 then do:
               for each fg-set FIELDS(part-qty)
               ...
               for each eb FIELDS(stock-no cas-cnt cas-wt)
                   where eb.company eq xinv-line.company
                     and eb.est-no  eq xinv-line.est-no
                     and eb.e-num   eq xinv-line.e-num
                     and eb.form-no ne 0
        ...
        for each inv-line no-lock where inv-line.r-no eq inv-head.r-no:
          ...
          if inv-line.po-no  ne "" then v-beeler-lines = v-beeler-lines + 1.
          if inv-line.ord-no ne 0  then v-beeler-lines = v-beeler-lines + 1.
          ...
          display v-rel-qty  to 6  format "->>>>9"
          ...
          if inv-line.po-no ne "" then do:
            display "PO #: " + string(inv-line.po-no) @ v-part-info
                with frame beeler.
            down with frame beeler. 
          end.             
         
          if inv-line.ord-no gt 0 then do:
            display "Ord#: " + string(inv-line.ord-no) @ v-part-info
                with frame beeler.
            down with frame beeler.
          end.  
        ...
        ASSIGN v-surcharge-amt = 0
               v-surcharge-dscr = "".

        for each inv-misc fields(charge dscr amt) where
            inv-misc.company eq inv-head.company AND
            inv-misc.r-no    eq inv-head.r-no AND
            inv-misc.bill    eq "Y"
            NO-LOCK,
            FIRST surcharge WHERE
                  surcharge.company = inv-head.company AND
                  surcharge.charge = inv-misc.charge AND
                  surcharge.dscr = inv-misc.dscr
                  NO-LOCK:
            IF v-surcharge-dscr = "" THEN
               v-surcharge-dscr = inv-misc.dscr.
            v-surcharge-amt = v-surcharge-amt + inv-misc.amt.                              
        END.
        for each inv-misc
            where inv-misc.company eq inv-head.company
              and inv-misc.r-no    eq inv-head.r-no
              and inv-misc.bill    eq "Y"
              /*AND NOT CAN-FIND(FIRST surcharge WHERE surcharge.company = inv-head.company
                                      AND surcharge.charge = inv-misc.charge)*/
        ...
        put skip(27 - v-printline)
            v-surcharge-dscr AT 48 FORM "x(19)" 
            v-surcharge-amt format "->>>>>>>>>9.99" skip
            "NonTaxable SubTotal"               at 48
            inv-head.t-inv-rev - v-tax-total - inv-head.t-inv-tax - v-surcharge-amt - 
