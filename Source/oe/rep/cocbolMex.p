/* ---------------------------------------------- oe/rep/cocbolMex.p */
/* Print Bol Mex COC (Certificate of Compliance)                     */
/* -------------------------------------------------------------------*/
...
for each report where report.term-id eq v-term-id NO-LOCK,
    ...
    if LAST-OF(report.key-06) then do:
       ...
       PUT UNFORMATTED
           "Nombre del cliente<C30>: " cust.NAME SKIP(1)
           "Número de pedido del proveedor<C30>: "  oe-boll.po-no SKIP(1)
           "Número de parte del cliente<C30>: " (IF AVAIL oe-ordl THEN oe-ordl.part-no ELSE "") SKIP(1)
           "Número de parte Descripción<C30>: " v-part-desc SKIP(1)
           "Número de dibujo<C30>: " v-cad SKIP(1)
           "Especificación del cliente / Nº de dibujo /" SKIP
           "Nº de revisión<C30>: "                        SKIP(1)
           "Número de pedido del proveedor<C30>: " (IF AVAIL oe-ordl THEN STRING(oe-ordl.ord-no)
                                                  ELSE "") SKIP(1)
           "Número de lote de fabricación del proveedor<C30>: " SKIP(1)
           "Cantidad enviada<C30>: "  STRING(v-bol-qty,"ZZ,ZZZ,ZZ9") SKIP(1)
           "Fecha de manufacturacion<C30>: " (IF v-manuf-date NE 12/31/2999 THEN
                                                     STRING(v-manuf-date,"99/99/9999")
                                                  ELSE "") SKIP(1)
           "Fecha de lanzamiento<C30>: " (IF v-rel-date NE 12/31/2999 THEN
