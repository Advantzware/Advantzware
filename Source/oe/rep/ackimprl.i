...
PUT "<FArial>".
     PUT "<C1><#1><R+8><C+45><IMAGE#1=" ls-full-img1  SKIP. /* pacific package */ 
...
PUT /*"<C1><#2>" /*<R+10><C+35><IMAGE#2=" ls-full-img2 SKIP  /* company image */ */
   ...
   "<FCourier New>"
   "Bill To:" SPACE(30) "Sold To: <P9>"  SKIP
   SPACE(5) oe-ord.cust-name 
    ( IF oe-ord.sold-name = "" THEN oe-ord.cust-name ELSE oe-ord.sold-name) AT 55 FORM "x(30)" skip
   SPACE(5) oe-ord.addr[1] 
    (IF oe-ord.sold-addr[1] = "" THEN oe-ord.addr[1] ELSE oe-ord.sold-addr[1]) AT 55 FORM "x(30)" SKIP
   SPACE(5) oe-ord.addr[2] 
    (IF oe-ord.sold-addr[2] = "" THEN oe-ord.addr[2] ELSE oe-ord.sold-addr[2]) AT 55 FORM "x(30)" SKIP
   SPACE(5) v-addr3  v-sold-addr3 AT 55 "<P10>" SKIP.
/*
...
PUT "<FArial><P18><=#3><R-2> <B>Acknowledgement</B> <P10>" SKIP
...
PUT "<FArial><=5><R+1> Line        Customer Part#                          Description                                       Ordered               Price        UOM        Ext. Price" SKIP(1).
PUT "<FCourier New>"          .
