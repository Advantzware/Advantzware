...
DEF VAR style AS cha FORM "x(8)" NO-UNDO.
...
{sys/form/r-top5DL2.f}
   ...
   /* form header 
     ...
     skip(1)
     with frame r-top row 1 column 1 stream-io width 300
...
/*     RUN oe/cleanrel.p (ROWID(oe-ordl)). */
  ...
  for each tt-report where tt-report.term-id eq ""
    ...
    IF AVAIL oe-rel THEN do:
        {custom/statusMsg.i " 'Processing Order#/FG #  '  + string(oe-rel.ord-no) + '/' + oe-rel.i-no "}
    END.
   ...
   FOR EACH oe-boll NO-LOCK
    ...
    FOR EACH w-ord
      ...
      IF AVAIL itemfg OR ll-show-top-only THEN DO:
        ...
        FIND FIRST bw-ord WHERE rowid(bw-ord) = ROWID(w-ord) NO-LOCK NO-ERROR.
        /*BUFFER bw-ord:FIND-BY-ROWID(ROWID(w-ord), NO-LOCK) .*/
        ASSIGN style = itemfg.style .
        ...
        IF AVAIL shipto THEN v-del-zone = shipto.dest-code.
        {oe/rep/shdrelN.i}
      END.
    END.
