...
def var v-x as char format "x(4)" init "" no-undo.
...
form v-inv-qty                 format ">>>>>>9-"
  ...
  with frame pack-detail no-attr-space
...
form skip(1)
     inv-head.t-inv-freight format "->,>>9.99" to 22
     inv-head.t-inv-tax format "->,>>9.99" to 32
     v-net format "->,>>>,>>9.99" to 46
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-n:
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            if v-printline gt v-max-lines + 1 then do:
              ...
              put skip(v-max-lines + 3 - v-printline) "* CONTINUED *" to 80.
...
/*           tmp2 = today + terms.disc-days. */
        ...
        */
        ...
        display " " skip(1) with frame blankl.
        ...
        display inv-head.t-inv-freight
                ...
                v-net @ inv-head.t-inv-rev
                inv-head.t-inv-rev when avail terms or inv-head.terms eq "CASH"
