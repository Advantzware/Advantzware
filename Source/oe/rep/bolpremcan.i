...
for each xxreport where xxreport.term-id eq v-term-id,
 ...
 IF v-footer THEN do:
     PUT "<FArial><C2><R26><#3><R+5><C+160><IMAGE#3=" cImageFooter + ">" FORMAT "x(200)"  .
 END.

  PUT "<R2><C22><b>Connaissement # / Bill of Lading #: " oe-bolh.bol-no   "</B>    Page#" string(PAGE-NUM,">9") + " of <#PAGES>" FORM "x(20)" SKIP
      "<R5><C53><#8><FROM><R+4><C+27><RECT> " 
      "<=8><R+1> Type de paquets   :" v-grand-total-cases format ">>,>>>,>>9"
      "<=8><R+3> Total Weight      :" v-tot-wt format ">>,>>>,>>9".
  
  PUT "<FArial><R4><C1><P12><B>     Shipping Instructions: <P10> "
      "<R5.5><C3.5>" oe-bolh.ship-i[1] 
      "<R6.5><C3.5>" oe-bolh.ship-i[2] 
      "<R7.5><C3.5>" oe-bolh.ship-i[3] 
      "<R8.5><C3.5>" oe-bolh.ship-i[4] "</B><P9>".
      
 PUT "<R10><C22><P10>Toutes les réclamations lors de l'expédition doivent être faites dans les 10 jours </B>"
      "<SAVE=LPI><ADJUST=LPI><P7>" 
      "<R11><C27.5><From><R+11><C27.5><LINE><||3>"
      "<R11><C45.7><From><R+11><C45.7><LINE><||3>"
  
      "<R12.8><C28><From><C45><LINE><||3>"
      "<R13><C28><From><C45><LINE><||3>"
      "<R14><C28><From><C45><LINE><||3>"
      "<R15><C28><From><C45><LINE><||3>"
      "<R17><C29><From><C45><LINE><||3>"
      "<R19><C54><From><C78><LINE><||3>"
      "<R19><C29><From><C45><LINE><||3>"
      "<R20.5><C52><From><C78><LINE><||3>"
      "<R20.8><C33><From><C45><LINE><||3><R21.8><C50><From><C78><LINE><||3>"
  
      "<R11><C1><P7>Sous réserve des conditions du connaissement applicable, <C28><P6>Si les modifications doivent être prépayées, <P7>          <C46>Si l'envoi se déplace entre deux ports par un transporteur par voie "
      "<R12><C1>cet envoi doit être livré au destinataire sans recours contre   <C28><P6>écrivez ou tamponnez ici. ""Pour être prépayé."" <P7>   <C46>d'eau, la loi exige que le connaissement indique s'il s'agit du poids du"
      "<R13><C1>l'expéditeur, l'expéditeur doit signer la déclaration suivante.                                                                  <C46>transporteur ou de l'expéditeur."
      "<R14><C1>Le transporteur ne doit pas effectuer la livraison de cette     <C28>reçu $                                     <C46>Remarque: Lorsque le taux dépend de la valeur, les expéditeurs sont tenus    "
      "<R15><C1>déclaration sans paiement du fret et de tous les autres frais <C28><P6> à appliquer en prépaiement des <P7>     <C46>d'indiquer spécifiquement par écrit la valeur convenue ou déclarée de la "
      "<R16><C1>légaux.                                                       <C28><P6> charges sur le bien décrit ci-dessus.<P7> <C46>propriété. La valeur convenue ou déclarée de la propriété est expressément   "
      "<R17><C28><P6>             agent ou transporteur <P7>                                                                      <C46>déclarée par l'expéditeur comme ne dépassant pas. ".
  IF cSignatureFile NE "" THEN 
      PUT "<C1><R18><#3><R+4><C+20><IMAGE#3=" cSignatureFile.
  PUT "<R22><C1><From><C27><LINE><||3><R22><C9>(Signature de l'expéditeur)"
      "<R17.5><C28>Per "
      "<R18.5><C46>  Expéditeur par"   
      "<R18.2><C28><From><C45><LINE><||3>"  /* per line*/
      "<R18.2><C28><P6>(seul le montant proposé.)<P7>"
      "<R19><C28><From><C43><LINE><||3>"
      "<R19> <C28>des charges"
      "<R20><C28><P7>avancé <P7><C47>  Agent"
      "<R21><C28> <P7><C47>  Per"
      "<RESTORE=LPI>".
 ...
 END.
  ...
  PUT "<R32><C53><#8><FROM><R+4><C+27><RECT> " 
      "<=8><R+1> Total Units       :" v-grand-total-cases format ">>,>>>,>>9"
      "<=8><R+3> Total Weight      :" v-tot-wt format ">>,>>>,>>9".
  
  PUT "<FArial><R31><C1><P12><B>     Shipping Instructions: <P10> "
      "<R32.5><C3.5>" oe-bolh.ship-i[1] 
      "<R33.5><C3.5>" oe-bolh.ship-i[2] 
      "<R34.5><C3.5>" oe-bolh.ship-i[3] 
      "<R35.5><C3.5>" oe-bolh.ship-i[4] "</B><P9>".
  ...
  PUT "<R37><C26><P10>ALL CLAIMS ON SHIPMENT MUST BE MADE WITHIN 10 DAYS </B>"
      "<SAVE=LPI><ADJUST=LPI><P7>" 
      "<R38><C27.5><From><R+10><C27.5><LINE><||3>"
      "<R38><C43.7><From><R+10><C43.7><LINE><||3>"
  
      "<R39.8><C28><From><C43><LINE><||3>"
      "<R40><C28><From><C43><LINE><||3>"
      "<R41><C28><From><C43><LINE><||3>"
      "<R42><C28><From><C43><LINE><||3>"
      "<R44><C29><From><C43><LINE><||3>"
      "<R45><C54><From><C78><LINE><||3>"
      "<R46><C29><From><C43><LINE><||3>"
      "<R46.5><C50><From><C78><LINE><||3>"
      "<R47.8><C33><From><C43><LINE><||3><C50><From><C78><LINE><||3>"
  
      "<R38><C1><P7>Subject To Section 7 of Conditions of applicable bill of lading,  <C28>If charges are to be prepaid write or   <C44>If the shipment moves between two ports by a carrier by water, the law requires that"
      "<R39><C1>this shipment is to be delivered to the consignee without re-   <C28>stamp here, ""To be Prepaid.""          <C44>the bill of lading shall state whether it is carrier's or shipper's weight."
      "<R40><C1>course on the consignor, the consignor shall sign the follo-                        <C44>NOTE:Where the rate is dependent on value, shippers are required to state specific-"
      "<R41><C1>wing statement.     <C28>Received $                              <C44>ally in writing the agreed or declared value of the property. The agreed or declared"
      "<R42><C1>The carrier shall not make delivery of this statement without <C28><P6>  to apply in prepayment of the charges<P7>     <C44>value of the property is hereby specifically stated by the shipper to be not exceeding."
      "<R43><C1>payment of freight and all other lawful charges.   <C28><P6> on the property described hereon.<P7>"
      "<R44><C28><P6>              Agent or Cashier <P7>  <C44>  SHIPPER PER".
  IF cSignatureFile NE "" THEN 
      PUT "<C1><R45><#3><R+4><C+20><IMAGE#3=" cSignatureFile.
  PUT "<R48><C1><From><C27><LINE><||3><R48><C9>(Signature of consignor)"
      "<R44.5><C28>Per "
      "<R45.5><C44>  AGENT"   
      "<R45.2><C28><From><C43><LINE><||3>"  /* per line*/
      "<R45.2><C28><P4>(The signature here acknowledges only the amount proposed.)<P7>"
      "<R46><C28><From><C43><LINE><||3>"
      "<R46> <C28>Charges"
      "<R47><C28><P6>advanced:$ <P7><C45>  PER"
      "<RESTORE=LPI>".
