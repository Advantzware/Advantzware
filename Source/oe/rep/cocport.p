/* ---------------------------------------------- oe/rep/cocport.p */
/* Print Portugese COC (Certificate of Compliance)                     */
/* -------------------------------------------------------------------*/
...
for each report where report.term-id eq v-term-id NO-LOCK,
    ...
    if LAST-OF(report.key-06) then do:
       ...
       PUT UNFORMATTED
           ...
           "Número de peça do cliente:                             " (IF AVAIL oe-ordl THEN oe-ordl.part-no ELSE "") SKIP(1)
           "Descrição da peça do cliente:                          " v-part-desc SKIP(1)
           "Número do desenho:                                       " v-cad SKIP(1)
           "Especificação do cliente/N° do desenho/" SKIP
           "N° da revisão:"                        SKIP(1)
           "Número do pedido do fornecedor:                   " (IF AVAIL oe-ordl THEN STRING(oe-ordl.ord-no)
                                                                 ELSE "") SKIP(1)
