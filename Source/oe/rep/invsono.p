...
def var v-invno         as char format "x(10)".
...
form header
  ...
  skip(3)
  inv-head.cust-no   at 11 v-shipto-no      at 51 skip
  inv-head.cust-name at 11 v-shipto-name    at 51 skip
  inv-head.addr[1]   at 11 v-shipto-addr[1] at 51 skip
  inv-head.addr[2]   at 11 v-shipto-addr[2] at 51 skip
...
*/
  ...
  "BOL #" TO 65 "Pallets" AT 72 skip v-date-ship
  v-shipvia at 11 format "x(25)"
  inv-head.t-inv-weight to 44 format ">>>>9.99"
  inv-head.bol-no to 65 format ">>>>>9"
      ...
      DO TRANSACTION:
        ...
        for each w-sman break by w-sman.sman:
          ...
          delete w-sman.
                ...
                v-invno = "A720" + string(inv-head.inv-no).
        ...
        for each inv-line no-lock where inv-line.r-no eq inv-head.r-no:
          ...
          for each oe-boll no-lock where oe-boll.company eq inv-line.company
            ...
            do v = 1 to 3:
              ...
              if v eq 1 then next.
              v-ord# = if v = 1 and sys-ctrl.char-fld = "Beeler" then
            ...
            display inv-line.po-no inv-line.i-no inv-line.inv-qty
                      ...
                      inv-line.ord-no inv-line.i-name
                      inv-line.ship-qty when sys-ctrl.char-fld ne "Imperial"
                      with frame detail.
...
*/
        ...
        do:  /* T O T A L S */
          ...
          release terms.
          find first terms where terms.company eq inv-head.company and
            terms.t-code eq inv-head.terms no-lock no-error.
