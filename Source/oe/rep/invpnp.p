...
form " " to 80
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      v-del-no = 0.
      /* removed 9.16.2005 per task# 09150512
      ...
      if avail oe-bolh then do:
        ...
        find first shipto where shipto.company  eq oe-bolh.company and
                   ...
                   shipto.ship-no eq oe-bolh.ship-no no-lock no-error.
      ...
      DO TRANSACTION:
        ...
        for each xinv-line no-lock where xinv-line.r-no eq inv-head.r-no
         ...
         v-date-ship = xinv-head.inv-date.
        ...
        for each wf no-lock where wf.r-no eq inv-head.r-no:  
          ...
          do v = 1 to 3:
            assign
             v-ord#      = if v eq 1 then wf.ord-no else 0
             v-part-info = if      v eq 1 then wf.part-no
                           else if v eq 2 then wf.part-dscr1
                           else                wf.part-dscr2.

            if v-part-info ne "" or v-ord# gt 0 then
          ...
          do v = 1 to 3:
            ...
            if v-part-info ne "" or v-ord# gt 0 then do:
        ...
        /*{custom/prntnote.i inv-head.rec_key}*/
        end.
