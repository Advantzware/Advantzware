...
/*PUT "<P10><R3><C66>PAGE " lv-page FORMAT ">9" " of <#PAGES>".*/
        ...
        PUT "<C1><#2>" /*<R+10><C+35><IMAGE#2=" ls-full-img2 SKIP  /* company image */ */
            ...
            SPACE(10) inv-head.cust-name "<C53>" v-shipto-name skip
            SPACE(10) inv-head.addr[1]   "<C53>" v-shipto-addr[1] SKIP
            SPACE(10) inv-head.addr[2]  "<C53>" v-shipto-addr[2] SKIP
...
PUT "<FArial><P12><=#3><R-2> <P10>" ip-copy-title FORM "x(20)" SKIP
    ...
    "<=#3><R+2>              PAGE                   " lv-page FORMAT ">9" " of [=@endPage" + TRIM(string(inv-head.inv-no,">>>>>9")) + "-@startPage" + TRIM(string(inv-head.inv-no,">>>>>9")) + "+1]" FORMAT "x(60)" "<FCourier New>"
/*     "<=#3><R+2>              PAGE                   " lv-page FORMAT ">9" " of <#PAGES>" "<FCourier New>" */
    SKIP(1)
    .
...
PUT "<FArial><=4>  SHIP DATE             FOB                      SHIP VIA                            TERMS                      SALES REP                        BOL#" SKIP
     ...
     xinv-head.terms-d FORM "x(15)" space(7) 
    /*v-salesman FORM "x(8)"*/ 
