...
/*                 if oe-boll.partial gt 0 then 1 else 0 */
  ...
  if oe-boll.qty-case ne 0 and oe-boll.cases ne 0 then do:
    find first w2 where w2.cas-cnt eq 
        (oe-boll.qty-case * oe-boll.cases + oe-boll.partial) no-error.
    if not avail w2 then create w2.
    assign
     w2.cas-cnt = oe-boll.qty-case * oe-boll.cases
     w2.cases   = w2.cases + 1
     v-tot-pkgs = v-tot-pkgs + 1.
  end.
