        ...
        PUT "<R56><C59><#8><FROM><R+8><C+23><RECT> " 
         ...
         ELSE 
           PUT "<R56><p9><C42><#8><FROM><R+9><C+41.4><RECT> " 
            ...
            "<=8><R+6> " v-bot-lab[4] 
            "<=8><R+7> " v-bot-lab[5] 
            "<=8><R+8> Grand Total:" (inv-head.t-inv-rev * dExchangeRate[1]) FORM "€->,>>>,>>9.99" (inv-head.t-inv-rev * dExchangeRate[2]) FORM "$->,>>>,>>9.99" (inv-head.t-inv-rev * dExchangeRate[3]) FORM "£->,>>>,>>9.99" "<p10>" .
    END.   
    ELSE do:
        IF cust.fax-country EQ "USA" OR cust.fax-country EQ "" THEN
        PUT "<R56><C59><#8><FROM><R+6><C+23><RECT> " 
         ...
         PUT "<R56><p9><C42><#8><FROM><R+7><C+41.4><RECT> " 
            ...
            "<=8><R+4> " v-bot-lab[2] 
            "<=8><R+5> " v-bot-lab[3] 
            "<=8><R+6> Grand Total:" (inv-head.t-inv-rev * dExchangeRate[1]) FORM "€->,>>>,>>9.99" (inv-head.t-inv-rev * dExchangeRate[2]) FORM "$->,>>>,>>9.99" (inv-head.t-inv-rev * dExchangeRate[3]) FORM "£->,>>>,>>9.99" "<p10>" .
    END.
    ASSIGN
