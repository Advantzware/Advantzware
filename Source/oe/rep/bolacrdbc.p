/* ---------------------------------------------- oe/rep/bolacrdbc 10/02 YSK *
*    Program Name  : oe/rep/bolacrdbc.p                                        *
      Author       :                                                         *
...
RUN getPrintBinsTags IN SOURCE-PROCEDURE (OUTPUT v-print-binsTags) NO-ERROR.
...
for each xxreport where xxreport.term-id eq v-term-id,
    ...
    FOR EACH oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no NO-LOCK,
      ...
      v-job-no = IF oe-boll.job-no = "" THEN "" ELSE (oe-boll.job-no + "-" + STRING(oe-boll.job-no2,">>9")).
  ...
  for each oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no:
    ...
    assign
     ...
     report.key-03   = string(oe-boll.po-no)
     report.rec-id   = recid(oe-boll)
     oe-boll.printed = yes.
  end.
  FOR EACH oe-boll WHERE oe-boll.company = oe-bolh.company AND oe-boll.bol-no = oe-bolh.bol-no NO-LOCK:
      CREATE tt-boll-bintag.
      ASSIGN 
          tt-boll-bintag.bol-no = oe-boll.bol-no
          tt-boll-bintag.i-no = oe-boll.i-no
          tt-boll-bintag.po-no = oe-boll.po-no
          tt-boll-bintag.job-no = oe-boll.job-no
          tt-boll-bintag.job-no2 = oe-boll.job-no2
          tt-boll-bintag.loc-bin = oe-boll.loc-bin
          tt-boll-bintag.tag = oe-boll.tag.
  END.
