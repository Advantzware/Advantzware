...
*/
    ...
    /*  no need to print dept notes, only spec note will be printed */
    FOR EACH notes WHERE notes.rec_key = oe-ord.rec_key NO-LOCK: 
           v-tmp-lines = LENGTH(NOTES.NOTE_TEXT) / 80.
           {SYS/INC/ROUNDUP.I v-tmp-lines}
           IF notes.note_text <> "" THEN DO i = 1 TO v-tmp-lines:
              PUT substring(NOTES.NOTE_TEXT,(1 + 80 * (i - 1)), 80) FORM "x(80)" SKIP.              
...
*/
      ...
      PUT "<FArial><R56><C1><#10><P12><B> Comments </B> <P8>  "          
        "<R57><C1>  All above items will be processed and shipped by date required above. All item quantities are subject to an over/under charge of 10%." 
        "<R58><C1>  If any products are returned a 20% Re-Stocking fee will be charged on all non custom orders." 
        "<R60><C1>  I acknowledge the above quantity, product and pricing are correct." 
