...
IF ll-display-comp THEN DO:
  ...
  IF AVAIL cust THEN
     ASSIGN v-comp-add1 = cust.addr[1]
            v-comp-add2 = "101 Prystup Drive"
            v-comp-add3 = "Livingston, AL 35470—1039"
...
if v-zone-p then v-zone-hdr = "Route No.:".
    ...
    {oe/rep/foreachr.i},
      ...
      for each w-oe-rell USE-INDEX idx,
        ...
        if last-of(w-oe-rell.ord-no) then do:
           EMPTY TEMP-TABLE w-bin.
           ...
           for each fg-bin
               ...
               IF s-print-what-item = "R" AND
                  NOT CAN-FIND(FIRST oe-rell
                                    WHERE oe-rell.company  EQ w-oe-rell.company
                                      ...
                                      AND oe-rell.tag      EQ fg-bin.tag ) THEN  DO:
                   NEXT.
               END.
               ...
               assign
                ...
                w-bin.w-unit-count = oe-ordl.cas-cnt /*fg-bin.case-count*/
                w-bin.w-units  = oe-ordl.cases-unit /*TRUNC((fg-bin.qty - fg-bin.partial-count) / fg-bin.case-count,0)*/
                w-bin.w-i-no   = fg-bin.i-no
                w-bin.w-cust-qty = oe-ordl.spare-dec-1
                w-bin.w-uom    = oe-ordl.spare-char-2
                i        = i + 1.
                w-bin.w-skids = /*oe-ordl.qty*/ w-oe-rell.qty / (w-bin.w-units * w-bin.w-unit-count) /*w-bin.w-units / fg-bin.cases-unit*/ .
          ...
          IF NOT CAN-FIND(FIRST w-bin WHERE w-bin.w-i-no EQ w-oe-rell.i-no) THEN DO:
              create w-bin.  
                   assign
                       w-bin.w-tag    = w-oe-rell.tag
                       w-bin.w-loc    = w-oe-rell.loc
                       w-bin.w-bin    = w-oe-rell.loc-bin 
                       w-bin.w-qty[1] = w-oe-rell.qty
                       w-bin.w-qty[2] = w-oe-rell.qty  
                       w-bin.w-units  =  oe-ordl.cases-unit
                       w-bin.w-unit-count = oe-ordl.cas-cnt 
                       w-bin.w-i-no   = w-oe-rell.i-no
                       w-bin.w-cust-qty = oe-ordl.spare-dec-1
                       w-bin.w-uom    = oe-ordl.spare-char-2
                       i        = i + 1.
                       w-bin.w-skids = /*oe-ordl.qty*/ w-oe-rell.qty / (w-bin.w-units * w-bin.w-unit-coun)    .
           END. 
           ...
           DO:
              ...
              FOR EACH w-bin:
                  ...
                  ASSIGN
                     ...
                     w-bin-cons.w-skids = w-bin-cons.w-skids + w-bin.w-skids.
           ...
           for each w-bin where w-bin.w-par eq ""
               ...
               by w-bin.w-qty[1] desc:
                 ...
                 ASSIGN w-bin.w-par = "RFQ:" + w-oe-rell.lot-no.
           ...
           for each w-bin
             ...
             assign
                v-bin = SUBSTRING(w-bin.w-tag,16,LENGTH(w-bin.w-tag) - 15) + "/" +
                        trim(w-bin.w-loc) + "/" +
                        trim(w-bin.w-bin).
