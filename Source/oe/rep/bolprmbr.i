...
for each xxreport where xxreport.term-id eq v-term-id,
  ...
  cSignatureFile = "".
/*   RUN oe\GetBolSign.p (cocode,ROWID(oe-bolh),NO,NO, */
/*                      OUTPUT cSignatureFile).        */
...
PUT "<R57><C26><P10>ALL CLAIMS ON SHIPMENT MUST BE MADE WITHIN 10 DAYS</B>"
    ...
    PUT
    "<R68><C1><From><C27><LINE><||3><R68><C9>(Signature of consignor)"
    "<R64.5><C28>Per "
    "<R65.5><C44>  AGENT"   
    ...
    IF AVAIL sys-ctrl-shipto THEN DO:
        CASE sys-ctrl-shipto.char-fld:
            ...
            WHEN "PremierBroker" THEN
                RUN oe/rep/cocprempkgb.p(INPUT RECID(oe-bolh)).
            OTHERWISE
