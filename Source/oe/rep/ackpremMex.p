    ...
    /* bpv 05291402 */
      ...
      IF AVAIL bf-shipto AND bf-shipto.broker then DO:
       ASSIGN
          ...
          v-comp-add4 = "Teléfono:  " + string(cust.area-code,"(999)") + string(cust.phone,"999-9999")
          v-comp-add5 = "Fax     :  " + string(cust.fax,"(999)999-9999") 
          lv-email    = "Email:  " + cust.email  .
...
*/      
      IF v-hide-price THEN
          v-totord = 0.
      PUT "<FArial><R55><C1><#10><P12><B> Comentarios </B>   <P10><C55> Valor total del pedido:" v-totord "<P8>" 
        "<R56><C1>" v-billinst[1] 
        "<R57><C1>" v-billinst[2] 
        "<R58><C1>" v-billinst[3] 
        "<R59><C1>" v-billinst[4] 
        "<R61><C1>" " ______________________________________(Please sign and fax back) " 
        "<=10><R-2><C23><P9><B>Esto es una confirmación de su pedido, no una factura.</B>" .
