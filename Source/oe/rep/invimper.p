...
FORM header
  ...
  skip(4)
  inv-head.cust-no at 8 /*11*/            v-shipto  at 51 skip
  v-cust[1]        at 8            v-ship[1] at 51 
  v-cust[2]        at 8            v-ship[2] at 51
  v-cust[3]        at 8            v-ship[3] at 51
  v-cust[4]        at 8            v-ship[4] at 51
  skip(3)
  v-date-ship
...
form header
  skip(6)
  v-invhead at 24
  v-inv-date at 72 skip
  inv-head.inv-no at 69 skip
  skip(2)
  v-fob at 4
  inv-head.terms-d at 32
  skip(2)
  inv-head.cust-no at 8 /*11*/           v-shipto  at 51 
  v-cust[1]        at 8            v-ship[1] at 51 
  v-cust[2]        at 8            v-ship[2] at 51
  v-cust[3]        at 8            v-ship[3] at 51
  v-cust[4]        at 8            v-ship[4] at 51 
...
form header
  ...
  "------------" "----------------" AT 19 "----------" TO 52 "-----" TO 64
  "------" TO 80 skip
with frame invhead-comp-exp page-top no-labels no-box no-underline stream-io width 90.
/*****************************************************************************************************/
...
form " " to 80
    ...
    if v-print-fmt eq "ILWalker" OR v-print-fmt = "Boxtech"
        then v-ord-del-hdr = "Ord#".

    if v-print-fmt eq "Beeler" then v-ord-del-hdr = "".

    if v-print-fmt eq "Colonial" then
     assign
      v-print-head = no
      end-page     = 28.
      
   else
   if v-print-fmt eq "Century"  or
      v-print-fmt eq "Imperial" then
     assign
      v-invhead = ""
      end-page  = 26.
    ...
    for each report where report.term-id eq v-term-id no-lock,
      ...
      DO TRANSACTION:
        ...
        for each xinv-line no-lock where xinv-line.r-no eq inv-head.r-no
                     break by xinv-line.b-no:
            IF FIRST-OF(xinv-line.b-no) THEN DO:
               FIND first oe-bolh no-lock where oe-bolh.b-no eq xinv-line.b-no NO-ERROR.
               v-tot-pallets = v-tot-pallets + IF AVAIL oe-bolh THEN  oe-bolh.tot-pallets ELSE 0.
            END.
        END.
        for each xinv-line no-lock where xinv-line.r-no eq inv-head.r-no
          break by xinv-line.i-no:

         if (v-print-fmt eq "ILWalker" OR v-print-fmt = "boxtech") and
            xinv-line.ord-no ne 0     AND
            v-del-no EQ 0             then v-del-no = xinv-line.ord-no.
         ...
         for each oe-bolh no-lock where oe-bolh.b-no eq xinv-line.b-no:
           ...
           assign v-date-ship = oe-bolh.bol-date.
                 /* v-tot-pallets = v-tot-pallets + oe-bolh.tot-pallets more pal than bol*/
         end. /* each oe-bolh */
        ...
        if first(report.key-01) then do:
          if v-print-head then do:
            if v-print-fmt eq "ILWalker" OR v-print-fmt = "Boxtech" then do:
              view frame ilwalker-comp.
            end.
            else do:
              if v-print-fmt eq "Express" then do:
                view frame invhead-comp-exp.  
              end.
              else
        ...
        for each inv-line no-lock where inv-line.r-no eq inv-head.r-no:
          ...
          if v-print-fmt eq "Valley" and avail oe-relh then
            wf.po-no = oe-relh.po-no.
        end.

        if v-print-fmt eq "Century" then do:
          for each wf:
            for each xwf where recid(wf) ne recid(xwf):
              if wf.i-no eq xwf.i-no and wf.po-no eq xwf.po-no then do:
                wf.inv-qty = wf.inv-qty + xwf.inv-qty.    
                wf.t-price  = wf.t-price + xwf.t-price.
                delete xwf.
              end. /*if*/
            end. /*for each xwf*/
          end. /*for each wf*/
        ...
        for each wf no-lock where wf.r-no eq inv-head.r-no:  
          ...
          if v-print-fmt ne "Century" then do:
            for each oe-boll
            ...
            if wf.qty-case ne 0 then 
              assign
               wf.cases  =  wf.inv-qty / wf.qty-case
               wf.partial  =  wf.inv-qty mod wf.qty-case.

            if wf.cases ne 0 and wf.qty-case ne 0 then
            assign v-case-line = string(wf.cases) + " @ " +
                                            string(wf.qty-case).
            else assign v-case-line = "".
            if wf.partial ne 0 then
            assign v-part-line = "1" + " @ " + string(wf.partial).
          ...
          v-beeler-lines = 0.
          if lookup(v-print-fmt,"Beeler,ILWalker,Imperial,Boxtech" +
                                      "Express,Century,Valley") gt 0 then
          do v = 1 to 3:
            if lookup(v-print-fmt,"Imperial,Express,Century,Valley") gt 0
               and v eq 1 then next.
            v-ord# = if v eq 1 and
                        lookup(v-print-fmt,"Beeler") gt 0 then
                       wf.ord-no else 0.
          ...
          if v-print-fmt eq "ILWalker" OR v-print-fmt = "Boxtech" then do:
            find first oe-ordl
                where oe-ordl.company eq wf.company
