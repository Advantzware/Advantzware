...
def var v-net-wt            as   dec format ">>,>>>,>>9" NO-UNDO.
DEF var v-gross-wt          as   dec format ">>,>>>,>>9" NO-UNDO.
def var v-tot-pcs           as   INT format ">>>>>9" NO-UNDO.
DEF var v-tot-cwt           as   dec format ">>,>>>,>>9" NO-UNDO.
DEF var v-case-weight       as   dec format ">>,>>>,>>9" NO-UNDO.
def var v-tot-plt           as   INT format ">>>>>9" NO-UNDO.
DEF VAR v-receipt-date AS DATE FORM "99/99/99" NO-UNDO.
DEF VAR v-ship-from AS CHAR NO-UNDO.
DEF VAR v-t-sqin LIKE itemfg.t-sqin NO-UNDO.
...
DEF TEMP-TABLE tt-bol-tags 
    FIELD tag AS CHAR
    FIELD rcpt-date AS DATE
    FIELD qty AS INT.
DEF TEMP-TABLE tt-bol-tagline
    FIELD LINE AS CHAR.
...
DEF VAR li-tag-count AS INT NO-UNDO.

{ce/msfcalc.i} /* needed for net-weight calc */
...
DEF VAR lv-qty-sum AS INT NO-UNDO.
/*DEF VAR lv-qty-sum2 AS INT NO-UNDO.*/
DEF VAR lv-job-printed AS LOG NO-UNDO.
...
for each xxreport where xxreport.term-id eq v-term-id,
  ...
  for each oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no:
      ...
      IF oe-boll.tag NE "" THEN DO:
          FIND FIRST fg-rdtlh WHERE fg-rdtlh.company = oe-boll.company AND
            fg-rdtlh.i-no = oe-boll.i-no AND 
            fg-rdtlh.tag = oe-boll.tag and
            fg-rdtlh.rita-code = "R" NO-LOCK NO-ERROR.
          FIND FIRST fg-rcpth WHERE fg-rcpth.r-no = fg-rdtlh.r-no NO-LOCK NO-ERROR.
          CREATE tt-bol-tags.
          tt-bol-tags.tag = oe-boll.tag.
          tt-bol-tags.qty = oe-boll.qty.
          IF AVAIL fg-rcpth THEN tt-bol-tags.rcpt-date = fg-rcpth.trans-date.
      END.
      IF v-frt-class = "" THEN do:
         ...
         IF AVAIL itemfg THEN
            FOR EACH fg-rcpth FIELDS (r-no trans-date) NO-LOCK
               ...
               ASSIGN v-receipt-date = fg-rcpth.trans-date.
               LEAVE.      
            END.
      END.
  ...
  if last-of(oe-bolh.bol-no) then do:
     ...
     ln-cnt = 0
     v-tot-cwt = 0
     v-tot-plt = /*oe-bolh.tot-pallet*/ 0.
     ...
     FOR EACH tt-boll,
        ...
        BREAK BY tt-boll.i-no
         ...
         if tt-boll.weight eq 0 then
            v-tot-wt = v-tot-wt + (tt-boll.qty / 100 * itemfg.weight-100).
         ELSE
            v-tot-wt = v-tot-wt + tt-boll.weight.

         /*** accumulate case weight to be included to gross wt calc ***/
         ASSIGN v-case-weight = 0.
         FIND FIRST job-hdr WHERE
                job-hdr.company EQ tt-boll.company AND
                job-hdr.job-no  EQ tt-boll.job-no AND
                job-hdr.job-no2 EQ tt-boll.job-no2 AND
                job-hdr.i-no    EQ tt-boll.i-no
                NO-LOCK NO-ERROR.

         IF AVAIL job-hdr THEN
            FOR EACH job-mat FIELDS(i-no) WHERE
                ...
                job-mat.blank-no EQ job-hdr.blank-no,
                FIRST ITEM FIELDS(basis-w) WHERE
                      ITEM.company EQ job-hdr.company AND
                      ITEM.i-no EQ job-mat.i-no AND
                      ITEM.mat-type EQ "C"
                      NO-LOCK:
        
                ASSIGN v-case-weight = ITEM.basis-w.
                LEAVE.     
            END.

         /* case weight */
         ASSIGN v-tot-cwt = v-tot-cwt + 
                         ((tt-boll.cases + if tt-boll.partial gt 0 then 1 else 0) * v-case-weight ).

         IF (lv-bolfmt-int = 1 AND FIRST-OF(tt-boll.ord-no)) OR
             lv-bolfmt-int <> 1 THEN DO:
     ...
     IF lv-bolfmt-int = 1 THEN
        lv-tot-pg = IF ln-cnt MOD 19 = 0 THEN TRUNC( ln-cnt / 19,0)
                    ELSE lv-tot-pg + TRUNC( ln-cnt / 19,0) .  /* 16->34 19 po detail lines */
     ELSE lv-tot-pg = IF ln-cnt MOD 17 = 0 THEN TRUNC( ln-cnt / 17,0)
                    ELSE lv-tot-pg + TRUNC( ln-cnt / 17,0) .  /* 16->34 19 po detail lines */
...
PUT "<FBook Antiqua><R43><C1><P12><B>     Shipping Instructions: </B> <P9> " SKIP(1)
    oe-bolh.ship-i[1] AT 7 SKIP
    oe-bolh.ship-i[2] AT 7 SKIP
    oe-bolh.ship-i[3] AT 7 SKIP
    oe-bolh.ship-i[4] AT 7 SKIP(1)
    "_________________________________________________________________________________________________________________________________" SKIP(1)
    ...
    "  ALL CLAIMS MUST BE MADE WITHIN 10 DAYS, OUR RESPONSIBILITY CEASES WHEN SHIPMENTS ARE TURNED OVER TO CARRIER."
    SKIP(1)
