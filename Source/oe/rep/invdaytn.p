...
DEF VAR v-tmp-tax AS DEC NO-UNDO.
DEF VAR v-line-tax-amt AS DEC NO-UNDO.
        ...
        for each inv-line no-lock where inv-line.r-no = inv-head.r-no:
          ...
          for each oe-boll no-lock where oe-boll.company = inv-line.company
            ...
            do v = 1 to 2:
              ...
              if v-part-info ne "" OR (inv-line.part-no <> "" AND v EQ 1) then v-beeler-lines = v-beeler-lines + 1.
            end.
            v-printline = v-printline + v-beeler-lines.

            IF inv-line.disc NE 0 THEN v-printline = v-printline + 1.
/*
...
*/
            ...
            assign v-inv-qty = inv-line.qty
                ...
                if inv-line.tax and avail stax THEN
                   v-line-tax-amt = v-line-tax-amt + inv-line.t-price.

                if inv-line.tax and avail stax THEN /*rounding problem*/
                do i = 1 to 3:
            ...
            ELSE if inv-line.pr-uom eq "CS" and avail itemfg then 
               v-price = inv-line.inv-qty /
                         (if inv-line.cas-cnt ne 0 THEN inv-line.cas-cnt
                          ELSE if avail itemfg and itemfg.case-count ne 0 then
                            itemfg.case-count else 1) *
                         inv-line.price.
            else do:
                 find first uom where uom.uom  eq inv-line.pr-uom
       ...
       */     
        ...
        /*do i = 1 to 3:
           if stax.tax-code[i] ne "" then do:
              run ar/calctax.p (inv-head.tax-gr, no,v-line-tax-amt, output v-tmp-tax).
              v-t-tax[i] = v-t-tax[i] + v-tmp-tax.                     
           END.
        end.
        */
