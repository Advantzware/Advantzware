...
ASSIGN cTextListToSelect = "Machine Code,RM Part Code,RM Part Description 1,RM Part Description 2,Model #,Serial #," +
                           "Date Purchased,Date Installed,Company Installed,Person Installed,Total Impressions Run," +
                           "Total Hours of Use,Expected Life (Impress),Expected Life (Hours),Expected Life (Days)  " 

      cFieldListToSelect = "m-code,rm-part-code,rm-part-desc-1,rm-part-desc-2,model-no," +
                           "serial-no,date-purchased,date-installed,company-installed,person-installed," + 
                           "total-impressions-run,total-hours-of-use,expected-impressions,expected-hours,expected-days"/*         cFieldListToSelect = "itemfg.i-no,itemfg.i-name,itemfg.part-no,itemfg.cust-no," +                 */
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR mach-part, ipc-field AS CHAR )  FORWARD.
...
DEFINE FRAME rd-fgexp
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "Export Machine Part to Excel" WIDGET-ID 100.
...
ON LEAVE OF begin_mach IN FRAME rd-fgexp /* From Item # */
DO:
   assign {&self-name}.
END.
...
ON LEAVE OF end_mach IN FRAME rd-fgexp /* To Item # */
DO:
     assign {&self-name}.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_mach end_mach sl_avail sl_selected tb_excel 
          tb_runExcel fi_file 
      WITH FRAME rd-fgexp.
...
FOR EACH b-mach-part WHERE b-mach-part.company = cocode
        AND b-mach-part.m-code GE begin_mach
        AND b-mach-part.m-code LE end_mach
        NO-LOCK:
    ...
    FOR EACH ttRptSelected:
        v-excel-detail-lines = v-excel-detail-lines + 
            appendXLLine(getValue-itemfg(BUFFER b-mach-part,ttRptSelected.FieldList)).
...
FUNCTION getValue-itemfg RETURNS CHARACTER
  ( BUFFER ipb-itemfg FOR mach-part, ipc-field AS CHAR ) :
/*------------------------------------------------------------------------------
