...
&SCOPED-DEFINE h_Browse01 h_b-fgadj
...
DEFINE VARIABLE h_b-fgadj AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "FINISHED GOODS WAREHOUSE TRANSACTION ADJUSTMENTS"
         HEIGHT             = 24
         WIDTH              = 150
         MAX-HEIGHT         = 320
         MAX-WIDTH          = 320
...
ON END-ERROR OF W-Win /* FINISHED GOODS WAREHOUSE TRANSACTION ADJUSTMENTS */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* FINISHED GOODS WAREHOUSE TRANSACTION ADJUSTMENTS */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Adjustment' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_b-fgadj ).
       RUN set-position IN h_b-fgadj ( 4.81 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-fgadj ( 18.10 , 146.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 22.91 , 42.00 ) NO-ERROR.
       ...
       /* Links to SmartNavBrowser h_b-fgadj. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_b-fgadj ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'cancel-item':U , h_b-fgadj ).
       RUN add-link IN adm-broker-hdl ( h_b-fgadj , 'Record':U , THIS-PROCEDURE ).

       /* Links to SmartPanel h_p-updsav. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'receipt':U , h_p-updsav ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-fgadj ,
             h_folder , 'AFTER':U ).
...
DEFINE VARIABLE lv-rowid AS ROWID   NO-UNDO.
 RUN util/dev/impFGAdj.p .
 IF VALID-HANDLE(h_b-fgadj) THEN
 RUN repo-query IN h_b-fgadj (lv-rowid) .
