...
/*for each fg-rctd of rm-rcth :
        ...
        */  
*/      
      find first item where item.company eq fg-rctd.company
                        and item.i-no    eq fg-rctd.i-no
                      no-error.

     
 /*     run get-matrix (output ld-cvt-qty, output ld-cvt-cost).
 */      
      find first rm-bin where rm-bin.company eq fg-rctd.company
              and rm-bin.loc     eq fg-rctd.loc
              and rm-bin.i-no    eq fg-rctd.i-no
              and rm-bin.loc-bin eq fg-rctd.loc-bin
              and rm-bin.tag     eq fg-rctd.tag
            no-error.
      if not avail rm-bin then do:
          create rm-bin.
          assign
           rm-bin.company = fg-rctd.company
           rm-bin.loc     = fg-rctd.loc
           rm-bin.loc-bin = fg-rctd.loc-bin
           rm-bin.tag     = fg-rctd.tag
           rm-bin.i-no    = fg-rctd.i-no.
        end. /* not avail rm-bin */

        if fg-rctd.rita-code eq "A" then do:        /** Adjustment**/
           if fg-rctd.cost <> 0 then do:
           {rm/rm-post.i "rm-bin.qty" "rm-bin.cost" "fg-rctd.qty" "fg-rctd.cost"}
         /*  {rm/rm-post.i "rm-bin" "ld-cvt-qty" "ld-cvt-cost"} */
           end.
           assign  rm-bin.qty     = rm-bin.qty + fg-rctd.qty
                   item.last-cost = if fg-rctd.cost <> 0 then fg-rctd.cost else item.last-cost
                   item.q-onh     = item.q-onh + fg-rctd.qty
                   item.q-avail   = item.q-onh - item.q-comm.
        end. /* R */

   find ps-rctd where recid(ps-rctd) = recid(fg-rctd).
