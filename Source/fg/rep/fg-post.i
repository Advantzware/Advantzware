  ...
  for each w-fg-rctd
    ...
    if first-of(w-fg-rctd.loc) then do:
      ...
      if first(w-fg-rctd.loc) then do:
        ...
        view STREAM after  frame r-top-after.
        IF fgpost-cha EQ "Both" THEN view STREAM after  frame after.
        view STREAM after  frame r-top1.
      ...
      if w-fg-rctd.ext-cost ne 0 AND NOT ll-wip then do:
        ...
        IF AVAIL itemfg AND v-fgpostgl NE "None"       AND
          ...
          if w-fg-rctd.ext-cost ne 0  and
            ...
            work-gl.cDesc  = work-gl.cDesc + (IF w-fg-rctd.job-no NE "" THEN "Job: " + w-fg-rctd.job-no + "-" + STRING(w-fg-rctd.job-no2) ELSE IF w-fg-rctd.po-no NE "" THEN "Po: " + w-fg-rctd.po-no + "-" + STRING(w-fg-rctd.po-line) ELSE "").
            /* Credit WIP Material */
    ...
    if w-fg-rctd.rita-code eq "A" and v-adjustgl then do:
      find first fg-bin
          where fg-bin.company eq cocode
            ...
            and fg-bin.tag     eq w-fg-rctd.tag
          no-lock no-error.
      cDescription = IF w-fg-rctd.job-no NE "" THEN "Job: " + w-fg-rctd.job-no + "-" + STRING(w-fg-rctd.job-no2) 
                     ELSE IF w-fg-rctd.po-no NE "" THEN "PO: " + w-fg-rctd.po-no + "-" + STRING(w-fg-rctd.po-line) ELSE "".
    ...
    IF rd-ItmPo EQ 1
      THEN DO:   
       ...
       display STREAM before
            ...
            v-cstprt   when FIRST-OF(w-fg-rctd.i-no) @ w-fg-rctd.i-no
            SUBSTR(w-fg-rctd.i-name,1,30) FORMAT "x(30)"  @ w-fg-rctd.i-name
            v-tran-type     
            w-fg-rctd.tag
            w-fg-rctd.cases
            w-fg-rctd.qty-case
    ...
    IF tb_excel  THEN DO:
      IF rd-ItmPo EQ 1 THEN DO:
          ...
          PUT STREAM excel UNFORMATTED
            ...
            v-time ","
            REPLACE(v-cstprt,","," ") ","
            REPLACE(w-fg-rctd.i-name,","," ") ","
            v-tran-type  format "x(1)" ","
            w-fg-rctd.tag  "," 
            w-fg-rctd.cases "," 
            w-fg-rctd.qty-case "," 
            v-fg-qty "," 
    ...
    IF rd-ItmPo EQ 1
      THEN DO:    
       ...
       display STREAM after
            ...
            v-time              when first-of(w-fg-rctd.i-no)
            v-cstprt   when FIRST-OF(w-fg-rctd.i-no) @ w-fg-rctd.i-no
            w-fg-rctd.i-name FORMAT "x(30)" @ w-fg-rctd.i-name
            v-tran-type
            w-fg-rctd.tag
            w-fg-rctd.cases
            w-fg-rctd.qty-case
   ...
   assign
    ...
    if last-of(w-fg-rctd.i-no) then do:
      if v-pr-tots2 then do:
        ...
        IF w-fg-rctd.rita-code EQ "R" THEN DO:
          put STREAM before
              "MSF->  FG: " + trim(string(v-msf[3],"->>,>>9.9<<")) +
