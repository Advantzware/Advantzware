...
&SCOPED-DEFINE h_Browse01 h_b-fg-set
&SCOPED-DEFINE h_Object01 h_p-fgset
...
DEFINE VARIABLE h_b-fg-set AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Finished Goods Sets */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Finished Goods Sets */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Items|Set parts' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'fg/b-fg-set.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-fg-set ).
       RUN set-position IN h_b-fg-set ( 5.05 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-fg-set ( 19.52 , 145.00 ) NO-ERROR.

       /* Links to SmartNavBrowser h_b-fg-set. */
       RUN add-link IN adm-broker-hdl ( h_b-fg-set , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-fg-set ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_itemfg2-2 ).
       RUN set-position IN h_itemfg2-2 ( 5.00 , 3.80 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_fg-set ).
       RUN set-position IN h_fg-set ( 8.62 , 4.00 ) NO-ERROR.
       RUN set-size IN h_fg-set ( 13.10 , 144.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-fgset ).
       RUN set-position IN h_p-fgset ( 22.19 , 32.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-fg-set , 'Record':U , h_itemfg2-2 ).

       /* Links to SmartBrowser h_fg-set. */
       RUN add-link IN adm-broker-hdl ( h_b-fg-set , 'Record':U , h_fg-set ).
       RUN add-link IN adm-broker-hdl ( h_p-fgset , 'TableIO':U , h_fg-set ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_fg-set ).
