...
&Scoped-Define ENABLED-OBJECTS RECT-6 begin_whse begin_loc-bin ~
begins_ship-no rd-dest td-post-trans Btn_OK Btn_select Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_whse begin_loc-bin begins_ship-no ~
rd-dest td-post-trans 
...
DEFINE FRAME FRAME-A
    begin_whse AT ROW 3.48 COL 25.8 COLON-ALIGNED HELP
    "Enter Beginning Warehouse" WIDGET-ID 42
    begin_loc-bin AT ROW 3.48 COL 46.8 COLON-ALIGNED HELP
    "Enter Beginning Warehouse" NO-LABELS WIDGET-ID 40
    begins_ship-no AT ROW 4.91 COL 26 COLON-ALIGNED HELP
    "Enter the From FG Item Number"
    rd-dest AT ROW 7.95 COL 10.2 NO-LABELS
    td-post-trans AT ROW 9.91 COL 38
    ...
    SIZE 41.2 BY .95 AT ROW 1.71 COL 5.8
    FONT 6
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
    CREATE WINDOW C-Win ASSIGN
        HIDDEN             = YES
        TITLE              = "Create Internal Transfer BOL"
        HEIGHT             = 16.76
        WIDTH              = 81.8
        MAX-HEIGHT         = 45.05
        MAX-WIDTH          = 256
...
ON END-ERROR OF C-Win /* Create Internal Transfer BOL */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Create Internal Transfer BOL */
    DO:
...
ON LEAVE OF begin_loc-bin IN FRAME FRAME-A
    DO:
        ASSIGN {&self-name}.
        ...
        FOR EACH fg-rctd
            ...
            AND (fg-rctd.loc EQ begin_whse )
            AND (fg-rctd.loc-bin EQ begin_loc-bin )
            USE-INDEX rita-code:       
            RUN build-tables.
        END.
    END.
...
ON LEAVE OF begin_whse IN FRAME FRAME-A /* To Location/Bin */
    DO:
        DEFINE VARIABLE lCheckError AS LOGICAL NO-UNDO.
        ASSIGN {&self-name}.
        RUN valid-loc(OUTPUT lCheckError).
...
ON CHOOSE OF Btn_select IN FRAME FRAME-A /* Select */
    DO:
        ...
        DO WITH FRAME {&FRAME-NAME}:
            ASSIGN {&DISPLAYED-OBJECTS}.
            IF begin_whse EQ "" THEN
            DO:
                MESSAGE "Please enter Location...,try help" 
                    VIEW-AS ALERT-BOX INFORMATION.
                RETURN NO-APPLY.         
            END.
            IF begin_loc-bin EQ "" THEN
            DO:
                MESSAGE "Please enter Bin...,try help" 
                    VIEW-AS ALERT-BOX INFORMATION.
                RETURN NO-APPLY.         
            END.
            iCount = 0. 
            ...
            FOR EACH w-fg-rctd 
                WHERE w-fg-rctd.IS-SELECTED EQ FALSE:
                DELETE w-fg-rctd.   
            END.      
        END.    
    END.
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_whse begin_loc-bin begins_ship-no rd-dest td-post-trans 
        WITH FRAME FRAME-A IN WINDOW C-Win.
    ENABLE RECT-6 begin_whse begin_loc-bin begins_ship-no rd-dest td-post-trans 
        Btn_OK Btn_select Btn_Cancel 
        WITH FRAME FRAME-A IN WINDOW C-Win.
...
PROCEDURE init-values :
    ...
    DO WITH FRAME {&FRAME-NAME}: 
        EMPTY TEMP-TABLE w-fg-rctd.
        IF begin_whse:screen-value NE "" AND begin_loc-bin:screen-value NE "" THEN 
        DO:  
            FOR EACH fg-rctd
                WHERE fg-rctd.company   EQ gcompany
                AND fg-rctd.rita-code EQ "T"
                AND (fg-rctd.loc EQ begin_whse:screen-value )
                AND (fg-rctd.loc-bin EQ begin_loc-bin:screen-value )
                USE-INDEX rita-code:       
                RUN build-tables.
            END.
        END.       
...
PROCEDURE print-and-post :
    ...
    FOR EACH w-fg-rctd:
        MESSAGE "kkkkkkk " VIEW-AS ALERT-BOX ERROR .
    END.
    RUN run-report.
