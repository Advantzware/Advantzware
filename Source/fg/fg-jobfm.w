...
&SCOPED-DEFINE h_Browse01 h_b-fgjobfm
...
DEFINE VARIABLE h_b-fgjobfm AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* FINISHED GOODS ISSUES */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* FINISHED GOODS ISSUES */
DO:
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_b-fgjobfm ).
       RUN set-position IN h_b-fgjobfm ( 4.81 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-fgjobfm ( 18.10 , 146.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updsav ).
       RUN set-position IN h_p-updsav ( 22.91 , 34.00 ) NO-ERROR.
       RUN set-size IN h_p-updsav ( 1.76 , 82.00 ) NO-ERROR.

       /* Links to SmartNavBrowser h_b-fgjobfm. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_b-fgjobfm ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'cancel-item':U , h_b-fgjobfm ).
       RUN add-link IN adm-broker-hdl ( h_b-fgjobfm , 'Record':U , THIS-PROCEDURE ).

       /* Links to SmartPanel h_p-updsav. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'receipt':U , h_p-updsav ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-fgjobfm ,
             h_folder , 'AFTER':U ).
