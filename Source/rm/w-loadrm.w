...
&SCOPED-DEFINE winViewPrgmName w-loadrm
...
DEFINE VARIABLE h_b-loadrm AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* RM Load Processing */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* RM Load Processing */
DO:
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = Yes':U ,
             OUTPUT h_b-loadrm ).
       RUN set-position IN h_b-loadrm ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-loadrm ( 17.14 , 139.00 ) NO-ERROR.
       ...
       /* Links to SmartNavBrowser h_b-loadrm. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_b-loadrm ).
       RUN add-link IN adm-broker-hdl ( h_v-post , 'State':U , h_b-loadrm ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'focus':U , h_b-loadrm ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-loadrm ,
             h_folder , 'AFTER':U ).
