...
DO iCntWidHand = 1 TO NUM-ENTRIES(cSmartObjList,","): 
    ...
    IF VALID-HANDLE(hTempObjHand) AND
        (LOOKUP("panel-identifier",hTempObjHand:INTERNAL-ENTRIES)            GT 0 OR
         ...
         LOOKUP( "containr-identifier",hTempObjHand:INTERNAL-ENTRIES, ",")   GT 0 OR 
         LOOKUP( "winmethods-identifier",hTempObjHand:INTERNAL-ENTRIES, ",") GT 0 OR 
...
FOR EACH toreposition
    ...
    DO:
        ASSIGN
            ...
            deColPos = 0
            .     
        IF LOOKUP(STRING(toreposition.widhand),cSmartObjList) GT 0 THEN NEXT.
        RUN get-position IN  WIDGET-HANDLE(toreposition.widhand) (OUTPUT deRowPos, OUTPUT deColPos) NO-ERROR.
        RUN set-position IN  WIDGET-HANDLE(toreposition.widhand) (deRowPos, deColPos + deDeltaWidthchange) NO-ERROR.
    END.
...
ASSIGN 
    ...
    cSmartObjList         = ""
    .  
RUN get-link-handle IN adm-broker-hdl (THIS-PROCEDURE, "PAGE" + "1" + "-TARGET", OUTPUT cSmartObjList) NO-ERROR.
// get the last position of the icon after shifting
FOR EACH toreposition
    WHERE toreposition.resizepage EQ "1" 
