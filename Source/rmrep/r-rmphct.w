...
rd_sort tb_qty-oh tb_zero-bal rd-dest fi_file tb_OpenCSV tbAutoClose ~
btn-ok btn-cancel 
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\RMInventoryByBin.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    begin_whs AT ROW 2.19 COL 33 COLON-ALIGNED HELP
    "Enter Beginng Warehouse"
    end_whs AT ROW 2.19 COL 73 COLON-ALIGNED HELP
    "Enter Endng Warehouse"
    begin_rm-no AT ROW 3.14 COL 33 COLON-ALIGNED HELP
    "Enter Beginning Item Number"
    end_rm-no AT ROW 3.14 COL 73 COLON-ALIGNED HELP
    "Enter Ending Item number"
    begin_code AT ROW 4.1 COL 33 COLON-ALIGNED HELP
    "Enter Begining Cycle Count Code"
    end_code AT ROW 4.1 COL 73 COLON-ALIGNED HELP
    "Enter Ending Cycle Count Code"
    begin_bin AT ROW 5.05 COL 33 COLON-ALIGNED HELP
    "Enter Beginning Bin"
    end_bin AT ROW 5.05 COL 73 COLON-ALIGNED HELP
    "Enter ending Bin"
    begin_procat AT ROW 6 COL 33 COLON-ALIGNED HELP
    "Enter Begining Category"
    end_procat AT ROW 6 COL 73 COLON-ALIGNED HELP
    "Enter Ending Category"
    lbl_sort AT ROW 7.48 COL 29 COLON-ALIGNED NO-LABELS
    rd_sort AT ROW 7.48 COL 41 NO-LABELS
    tb_qty-oh AT ROW 8.67 COL 31
    tb_zero-bal AT ROW 9.86 COL 31
    lv-font-no AT ROW 11.81 COL 30.6 COLON-ALIGNED
    lines-per-page AT ROW 11.91 COL 84.6 COLON-ALIGNED
    lv-ornt AT ROW 11.95 COL 40.6 NO-LABELS
    rd-dest AT ROW 12 COL 4.6 NO-LABELS
    lv-font-name AT ROW 13 COL 29.2 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 14.19 COL 91.4 RIGHT-ALIGNED
    td-show-parm AT ROW 15.05 COL 27.8
    fi_file AT ROW 16 COL 25.8 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 16.1 COL 92 RIGHT-ALIGNED
    tbAutoClose AT ROW 17.52 COL 27.8 WIDGET-ID 64
    btn-ok AT ROW 18.48 COL 27.6
    btn-cancel AT ROW 18.48 COL 54.2
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.14 COL 4
    " Output Destination" VIEW-AS TEXT
    SIZE 19 BY .62 AT ROW 11.24 COL 4
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "MC1" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    ENABLE RECT-6 RECT-7 begin_whs end_whs begin_rm-no end_rm-no begin_code 
        ...
        tb_zero-bal rd-dest fi_file tb_OpenCSV tbAutoClose btn-ok btn-cancel 
        WITH FRAME FRAME-A IN WINDOW C-Win.
...
PROCEDURE run-report :
    ...
    {sys/form/r-topw.f}
    DEFINE VARIABLE it-spaces  AS ch        FORMAT "X(21)" INITIAL "      _______________".
    DEFINE VARIABLE price      AS DECIMAL   FORMAT "->>>>9.99".
    DEFINE VARIABLE alloc      AS DECIMAL   FORMAT "->>>,>>9.99".
    DEFINE VARIABLE xwhse      LIKE rm-bin.loc.
    DEFINE VARIABLE fcode      AS ch        FORMAT "x(2)" INITIAL "".
    DEFINE VARIABLE tcode      LIKE fcode INITIAL "ZZ".
    DEFINE VARIABLE type       AS LOG       FORMAT "Y/N" INITIAL NO.
    DEFINE VARIABLE type-prt   AS ch        FORMAT "X(13)" INITIAL "             ".
    DEFINE VARIABLE v-floc     AS ch        FORMAT "x(5)" INITIAL "".
    DEFINE VARIABLE v-tloc     LIKE v-floc INITIAL "ZZZZZ".
    DEFINE VARIABLE v-cat      AS ch        FORMAT "x(5)" EXTENT 2 INITIAL ["","ZZZZZ"].
    DEFINE VARIABLE v-cust     AS ch        FORMAT "x(8)" EXTENT 2 INITIAL ["","ZZZZZZZZ"].
    DEFINE VARIABLE v-i-no     AS ch        FORMAT "x(15)" EXTENT 2 INITIAL ["","ZZZZZZZZZZZZZZZ"].
    DEFINE VARIABLE v-loc-bin  AS ch        FORMAT "x(8)" EXTENT 2 INITIAL ["", "ZZZZZZZZ"] NO-UNDO.
    DEFINE VARIABLE v-item-bin AS CHARACTER NO-UNDO.
    DEFINE VARIABLE v-prnt-onh AS LOGICAL   INITIAL "N".
    DEFINE VARIABLE v-prnt-zer AS LOG       INIT "N".
    ...
    IF v-item-bin EQ "Item" THEN
        ...
        FOR EACH item WHERE item.company =  cocode
            ...
            DISPLAY rm-bin.i-no    
                ...
                WHEN FIRST-OF(rm-bin.i-no)
                rm-bin.loc     
                WHEN FIRST-OF(rm-bin.loc)
                rm-bin.loc-bin
                rm-bin.tag FORM "x(20)"
                rm-bin.qty     
                WHEN v-prnt-onh = TRUE
                it-spaces.
            DOWN.
            IF tb_excel THEN 
                EXPORT STREAM excel DELIMITER ","
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\RMInventoryByBin.csv".   
    END.
