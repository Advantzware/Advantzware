...
DEFINE TEMP-TABLE tt-rm-bin LIKE rm-bin.
DEFINE STREAM excel.
...
end_rm-no end_procat begin_procat end_mat-type begin_mat-type rd_item ~
rd_qty tb_zero-bal tb_qty-ord tb_neg sl_selected sl_avail Btn_Def Btn_Add ~
...
end_procat begin_procat end_mat-type begin_mat-type lbl_itm-code rd_item ~
lbl_grnd-tot rd_qty tb_zero-bal tb_qty-ord tb_neg sl_selected sl_avail ~
...
DEFINE FRAME FRAME-A
     as-of-date AT ROW 1.67 COL 71.6 COLON-ALIGNED
     begin_rm-no AT ROW 2.71 COL 29.6 COLON-ALIGNED HELP
          "Enter Beginning Item Number"
     end_rm-no AT ROW 2.71 COL 71.6 COLON-ALIGNED HELP
          "Enter Ending Item Number"
     end_procat AT ROW 3.76 COL 71.6 COLON-ALIGNED HELP
          "Enter Ending Category"
     begin_procat AT ROW 3.81 COL 29.6 COLON-ALIGNED HELP
          "Enter Beginning Category"
     end_mat-type AT ROW 4.81 COL 71.6 COLON-ALIGNED HELP
          "Enter Ending Material Type"
     begin_mat-type AT ROW 4.86 COL 29.6 COLON-ALIGNED HELP
          "Enter Beginning Material Type"
     lbl_itm-code AT ROW 6.14 COL 39.6 COLON-ALIGNED NO-LABEL
     rd_item AT ROW 6.14 COL 54.6 NO-LABEL
     lbl_grnd-tot AT ROW 7.38 COL 7.6 COLON-ALIGNED NO-LABEL
     rd_qty AT ROW 7.38 COL 54.6 NO-LABEL
     tb_zero-bal AT ROW 8.52 COL 38.6
     tb_qty-ord AT ROW 9.48 COL 38.6
     tb_neg AT ROW 10.43 COL 38.6
     sl_selected AT ROW 12.38 COL 61.4 NO-LABEL WIDGET-ID 28
     sl_avail AT ROW 12.43 COL 4.2 NO-LABEL WIDGET-ID 26
     Btn_Def AT ROW 12.48 COL 41.2 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 56
     Btn_Add AT ROW 13.48 COL 41.2 HELP
          "Add Selected Table to Tables to Audit" WIDGET-ID 32
     Btn_Remove AT ROW 14.48 COL 41.2 HELP
          "Remove Selected Table from Tables to Audit" WIDGET-ID 34
     btn_Up AT ROW 15.52 COL 41.2 WIDGET-ID 40
     btn_down AT ROW 16.52 COL 41.2 WIDGET-ID 42
     lv-ornt AT ROW 18.38 COL 42.6 NO-LABEL
     lv-font-no AT ROW 18.38 COL 76.6 COLON-ALIGNED
     rd-dest AT ROW 18.62 COL 6.6 NO-LABEL
     lines-per-page AT ROW 18.67 COL 83.6 COLON-ALIGNED
     lv-font-name AT ROW 19.57 COL 29.6 COLON-ALIGNED NO-LABEL
     td-show-parm AT ROW 20.76 COL 69.6
     fi_file AT ROW 22.38 COL 28.6 COLON-ALIGNED HELP
          "Enter File Name"
     tb_OpenCSV AT ROW 22.48 COL 89 RIGHT-ALIGNED
     tbAutoClose AT ROW 24.19 COL 30.8 WIDGET-ID 16
     btn-ok AT ROW 25.19 COL 30.6
     btn-cancel AT ROW 25.24 COL 55.6
     "Available Columns" VIEW-AS TEXT
          SIZE 29 BY .62 AT ROW 11.62 COL 4 WIDGET-ID 38
     "Selected Columns(In Display Order)" VIEW-AS TEXT
          SIZE 34 BY .62 AT ROW 11.57 COL 60.8 WIDGET-ID 44
     " Selection Parameters" VIEW-AS TEXT
          SIZE 21 BY .71 AT ROW 1.14 COL 5.4
          BGCOLOR 15 
     " Output Destination" VIEW-AS TEXT
          SIZE 18 BY .62 AT ROW 17.81 COL 6.6
     RECT-6 AT ROW 18.14 COL 4.6
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "MR4" }
    ASSIGN
...
------------------------------------------------------------------------------*/
  DISPLAY as-of-date begin_rm-no end_rm-no end_procat begin_procat end_mat-type 
          begin_mat-type lbl_itm-code rd_item lbl_grnd-tot rd_qty tb_zero-bal 
          tb_qty-ord tb_neg sl_selected sl_avail rd-dest fi_file tb_OpenCSV 
          tbAutoClose 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-6 RECT-7 as-of-date begin_rm-no end_rm-no end_procat begin_procat 
         end_mat-type begin_mat-type rd_item rd_qty tb_zero-bal tb_qty-ord 
         tb_neg sl_selected sl_avail Btn_Def Btn_Add Btn_Remove btn_Up btn_down 
...
/*------------------------------------------------------------------------------
    ...
    DEFINE OUTPUT PARAMETER op-qty AS DECIMAL NO-UNDO INIT 0.
    DEFINE OUTPUT PARAMETER op-cost AS DECIMAL NO-UNDO INIT 0.
    DEFINE OUTPUT PARAMETER op-value AS DECIMAL NO-UNDO INIT 0.
...
/* ----------------------------------------------- rm/rep/rm-aitem.p 10/93 cd */
    ...
    DEFINE VARIABLE v-value        AS DECIMAL   NO-UNDO FORMAT "->,>>>,>>9.99". /*"->>>>9.99".*/
    DEFINE VARIABLE v-tot-value    AS DECIMAL   NO-UNDO FORMAT "->,>>>,>>9.99".
    DEFINE VARIABLE v-qty          AS DECIMAL   NO-UNDO FORMAT "->>>>9.99".
    DEFINE VARIABLE v-qty-avail    LIKE item.q-avail NO-UNDO.

    DEFINE VARIABLE v-fitem        LIKE item.i-no NO-UNDO INIT "".
    DEFINE VARIABLE v-titem        LIKE v-fitem NO-UNDO INIT "zzzzzzzzzz".
    DEFINE VARIABLE v-fcat         LIKE item.procat NO-UNDO INIT "".
    DEFINE VARIABLE v-tcat         LIKE v-fcat NO-UNDO INIT "zzzzz".
    DEFINE VARIABLE v-type         AS CHARACTER FORMAT "!" NO-UNDO INIT "B".
    DEFINE VARIABLE v-zbal         AS LOG       FORMAT "Y/N" NO-UNDO INIT NO.
    DEFINE VARIABLE v-fmtype       LIKE item.mat-type NO-UNDO INIT "".
    DEFINE VARIABLE v-tmtype       LIKE v-fmtype NO-UNDO INIT "z".
    /* def var v-fst-loc like item.loc NO-UNDO .  */
    DEFINE VARIABLE inc            AS LOG       FORMAT "Y/N" NO-UNDO INIT YES.
    ...
    FOR EACH ITEM NO-LOCK
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            ...
            CASE cTmpField:             
                ...
                WHEN "desc"   THEN 
                    cVarValue = STRING(item.i-dscr,"x(20)").
                WHEN "cat"   THEN 
                    cVarValue = STRING(item.procat,"x(5)").
                WHEN "uom"  THEN 
                    cVarValue = STRING(item.cons-uom,"x(3)") .
                WHEN "cost"   THEN 
                    cVarValue = STRING(item.avg-cost,"->>>>9.99") .
                WHEN "on-hand"  THEN 
                    cVarValue = /*IF item.i-code ne "E" THEN*/ STRING(lv-q-onh,"->,>>>,>>9.999") /*ELSE ""*/ .
                WHEN "on-order"   THEN 
                    cVarValue = /*IF item.i-code ne "E" THEN*/ STRING(item.q-ono,"->,>>>,>>9.999") /*ELSE ""*/ .
                WHEN "qty-all"  THEN 
                    cVarValue = /*IF item.i-code ne "E" THEN*/ STRING(item.q-comm,"->,>>>,>>9.999") /*ELSE ""*/ .

                WHEN "reord-l"  THEN 
                    cVarValue = /*IF item.i-code ne "E" THEN*/ STRING(item.ord-level,"->>>>,>>9.999") /*ELSE ""*/ .
                WHEN "qty-avail"   THEN 
                    cVarValue = /*IF item.i-code ne "E" THEN*/ STRING(v-qty-avail,"->,>>>,>>9.999") /*ELSE ""*/ .
                WHEN "value"  THEN 
