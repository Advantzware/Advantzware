...
&Scoped-Define ENABLED-OBJECTS btn-update-all btn-update  ~
btn-export 
...
DEFINE FRAME Panel-Frame
     ...
     btn-update-all AT ROW 1.24 COL 11
     btn-update AT ROW 1.24 COL 20
     btn-export AT ROW 1.24 COL 40
...
ON CHOOSE OF btn-update IN FRAME Panel-Frame /* Add */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-export C-WIn
ON CHOOSE OF btn-export IN FRAME Panel-Frame /* Delete */
DO:
   run get-link-handle in adm-broker-hdl(this-procedure, "tableio-target", output lv-char-hdl).
    run pExportRecord in widget-handle(lv-char-hdl).  
END.
...
&Scoped-define SELF-NAME btn-update-all
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-update-all C-WIn
ON CHOOSE OF btn-update-all IN FRAME Panel-Frame /* Update */
DO:

  run get-link-handle in adm-broker-hdl(this-procedure, "tableio-target", output lv-char-hdl).
  run pUpdateAllRecord in widget-handle(lv-char-hdl).
  ...
  DO WITH FRAME {&FRAME-NAME}:  
      IF panel-state = 'disable-all':U THEN DO:
          ASSIGN btn-update:SENSITIVE = NO
              btn-update-all:SENSITIVE = NO
              btn-export:SENSITIVE = NO.
      END.
      ELSE IF panel-state = 'initial':U THEN DO:
          ASSIGN btn-update:SENSITIVE = YES
              btn-update-all:SENSITIVE = YES              
              btn-export:SENSITIVE = YES.
      END.
    ...
    IF NOT v-can-update THEN ASSIGN btn-update-all:SENSITIVE = NO                                    
                                    btn-update:SENSITIVE = NO .

    
    IF v-can-create                    AND
       btn-update-all:LABEL EQ "&Update"       AND
       panel-state    NE "disable-all" AND
       panel-state    NE "add-only"    THEN btn-update-all:SENSITIVE = YES.
...
DO WITH FRAME panel-frame: 
  btn-update-all:LABEL = label-string.
END.
...
------------------------------------------------------------------------------*/
   ...
   DO WITH FRAME panel-frame:
        ...
        APPLY 'choose' TO btn-update-all.
      
        IF btn-update-all:LABEL = '&Update' THEN 
        DO:
