...
DO:
    ...
    {methods/run_link.i "TABLEIO-TARGET" "local-row-available"}    
END.
...
/*--------------------------------------------------------------------------
    ...
    IF RETURN-VALUE <> 'DESIGN':U THEN 
    DO:
        IF VALID-HANDLE (adm-broker-hdl) THEN 
        DO:
            DEFINE VARIABLE tab-target-link AS CHARACTER NO-UNDO.
...
/*------------------------------------------------------------------------------
      ...
      Parameters:  Character string that denotes which action to set the button
                   ...
                   The values are: initial - the panel is in a state where no record
                                   ...
                                   add-only - for the time that there are no records
                                              ...
                                              taken is an add.
                                   no-add - all buttons are enabled except for add
                                   update-only - all buttons are disabled except for Save/Update button
                                   copy-only - all buttons are disabled except for Copy button
      Notes:       
    ------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME Panel-Frame:
        ...
        ELSE IF panel-state = 'no-add':U THEN DO:
        ...
        ELSE IF panel-state = 'copy-only':U THEN DO:
            /* All buttons are set to insensitive, except copy. */
        ...
        END. /* panel-state = 'copy-only' */ 
        ELSE IF panel-state = 'update-only':U THEN DO:
            /* All buttons are set to insensitive, except copy. */
