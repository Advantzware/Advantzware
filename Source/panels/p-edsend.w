...
&Scoped-Define ENABLED-OBJECTS btn-resend 
...
&Scoped-define SELF-NAME btn-resend
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-resend C-WIn
ON CHOOSE OF btn-resend IN FRAME Panel-Frame
DO:
    run get-link-handle in adm-broker-hdl(this-procedure, "resend-target", output lv-char-hdl).
    run resend-doc in widget-handle(lv-char-hdl).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAM ip-resend AS LOG NO-UNDO.
  
  IF v-can-update THEN
     btn-resend:SENSITIVE IN FRAME {&FRAME-NAME} = ip-resend.
END PROCEDURE.
...
&ENDIF
  ...
  &IF LOOKUP("btn-resend":U, "{&ENABLED-OBJECTS}":U," ":U) NE 0 &THEN
             btn-resend:SENSITIVE = yes.
  &ENDIF
  ...
  DO WITH FRAME {&FRAME-NAME}:     
    btn-resend:SENSITIVE = yes.
    IF NOT v-can-update THEN ASSIGN btn-resend:SENSITIVE = NO.    
  END.
