...
DEFINE VARIABLE h_b-delt AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "WAREHOUSE TRANSACTION DELETION(FINISHED GOODS)"
         HEIGHT             = 24.19
         WIDTH              = 150
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 204.8
...
ON END-ERROR OF W-Win /* WAREHOUSE TRANSACTION DELETION(FINISHED GOODS) */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* WAREHOUSE TRANSACTION DELETION(FINISHED GOODS) */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'addon/fg/b-delt.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-delt ).
       RUN set-position IN h_b-delt ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_b-delt ( 17.38 , 145.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updbar ).
       RUN set-position IN h_p-updbar ( 22.67 , 34.00 ) NO-ERROR.
       RUN set-size IN h_p-updbar ( 1.91 , 85.00 ) NO-ERROR.

       /* Links to SmartNavBrowser h_b-delt. */
       RUN add-link IN adm-broker-hdl ( h_p-updbar , 'TableIO':U , h_b-delt ).
