...
&Scoped-Define ENABLED-OBJECTS RECT-12 btn_post 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn_post V-table-Win
ON CHOOSE OF btn_post IN FRAME F-Main /* Post */
DO:
    DEF VAR char-hdl AS cha NO-UNDO.
    DEF VAR lv-in-update AS LOG NO-UNDO.
    DEFINE VARIABLE lChoice AS LOGICAL NO-UNDO.
    DEFINE VARIABLE lNotPostData AS LOGICAL NO-UNDO.
    ...
    IF lv-in-update THEN DO:
       MESSAGE "Save or Cancel First before Post." VIEW-AS ALERT-BOX ERROR.
       RETURN NO-APPLY.
    END.
    IF VALID-HANDLE(WIDGET-HANDLE(char-hdl)) THEN       
       RUN pCheckPeriod IN WIDGET-HANDLE(char-hdl) (OUTPUT lNotPostData).    
    IF lNotPostData THEN RETURN NO-APPLY.
    lChoice = NO.
...
DEF VAR char-hdl AS CHAR NO-UNDO.
  ...
  /* Code placed here will execute AFTER standard behavior.    */
   ...
   /* Security is on all post buttons except in scan label 01191334  */
   v-source-handle = WIDGET-HANDLE(char-hdl) NO-ERROR.
   IF VALID-HANDLE(v-source-handle) THEN DO:
     IF INDEX(v-source-handle:NAME, "rcptd") EQ 0  THEN
       v-post-sec = YES.
   END.

   IF v-post-sec = NO THEN
       btn_post:SENSITIVE IN FRAME f-main = NO.
END PROCEDURE.
