/* rm/chkporun.i   Check PO qty overrun */
    ...
    if avail po-ordl then do:
        v-rec-qty = po-ordl.t-rec-qty + input rm-rctd.qty.
        ...
        for each xrm-rctd
            where xrm-rctd.company   eq cocode
              ...
              v-rec-qty = v-rec-qty + xrm-rctd.qty.
        end.
        if v-rec-qty gt po-ordl.cons-qty * (1 + (po-ord.over-pct / 100))
        then do:          
            ...
            IF ll-ans  THEN DO:
                ...
                RETURN NO-APPLY.
            END.          
        ELSE IF IF rmunderover-cha EQ "UnderRuns and OverRun" AND v-rec-qty LT po-ordl.cons-qty - (po-ordl.cons-qty * po-ord.under-pct / 100) THEN
        DO:
