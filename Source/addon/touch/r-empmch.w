    ...
    EACH machtran NO-LOCK WHERE machtran.rec_key = machemp.TABLE_rec_key 
      ...
      IF tb_excel THEN
        PUT STREAM excel UNFORMATTED
            ...
            '"' machtran.start_date                    '",'
            '"' STRING(machemp.start_time,'HH:MM am')  '",'
            '"' machtran.end_date '",'
            '"' STRING(machemp.end_time,'HH:MM am') '",'
            '"' machtran.charge_code '",'
            '"' (IF ld-mr-time > 0 THEN STRING(ld-mr-time,"HH:MM") ELSE "") '",'
            '"' (IF ld-run-time > 0 THEN STRING(ld-run-time,"HH:MM") ELSE "") '",'
            '"' STRING(machemp.total_time,'HH:MM') '",'
            '"' STRING(lv-rqty,"->>,>>>,>>9") '",'
            '"' STRING(lv-wqty,"->>>>9") '",'
           SKIP.
