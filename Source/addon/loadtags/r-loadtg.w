 ...
 v-stat         = rd_order-sts.
  ...
  FOR EACH job
      WHERE job.company EQ cocode
        AND job.job-no  GE FILL(" ", iJobLen - LENGTH(TRIM(begin_job))) + TRIM(begin_job)
        AND job.job-no  LE FILL(" ", iJobLen - LENGTH(TRIM(end_job)))   + TRIM(end_job)
  ...
  FOR EACH w-ord,
    ...
    IF rd_comps  EQ "B"                       OR
       (rd_comps EQ "A" AND NOT itemfg.alloc) OR
