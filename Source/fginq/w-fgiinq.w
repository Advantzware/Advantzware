...
&SCOPED-DEFINE h_Browse01 h_b-fgiinq
...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES fg-rcpth
&Scoped-define FIRST-EXTERNAL-TABLE fg-rcpth


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR fg-rcpth.
...
DEFINE VARIABLE h_b-fgiinq AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* FG History Inquiry */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* FG History Inquiry */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'FG History' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'fginq/b-fgiinq.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-fgiinq ).
       RUN set-position IN h_b-fgiinq ( 4.57 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 19.76 , 148.00 ) */

       /* Links to SmartObject h_movecol. */
       RUN add-link IN adm-broker-hdl ( h_b-fgiinq , 'move-columns':U , h_movecol ).

       /* Links to SmartObject h_export. */
       RUN add-link IN adm-broker-hdl ( h_b-fgiinq , 'export-xl':U , h_export ).

       /* Links to SmartNavBrowser h_b-fgiinq. */
       RUN add-link IN adm-broker-hdl ( h_b-fgiinq , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_export ,
             h_movecol , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-fgiinq ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "fg-rcpth"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "fg-rcpth"}
