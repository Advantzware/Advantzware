/*------------------------------------------------------------------------------
         pure4gltvwinfunc.p   20-AUG-2007  by slacroix   
  Purpose:  copy of WINFUNC.P with private new GLOBAL shared VARIABLE to remain 
...
FUNCTION CreateProcess RETURNS INTEGER 
   ...
   run CreateProcess{&A} in hppure4gltvApi
     ( 0,
       CommandLine,
       0,
       0,
  ...
  RUN CloseHandle in hppure4gltvApi(hThread, output ReturnValue).
...
FUNCTION GetLastError RETURNS INTEGER :
  ...
  run GetLastError in hppure4gltvApi (output dwMessageID).
  RETURN (dwMessageID). 
END FUNCTION.    
...
FUNCTION GetParent RETURNS INTEGER 
  ...
  run GetParent in hppure4gltvApi (hWnd, output hParent).
  RETURN (hParent). 
END FUNCTION.    
...
FUNCTION ShowLastError RETURNS INTEGER :
  ...
  txt = fill(" ",300).
  run FormatMessage{&A} in hppure4gltvApi (512 + 4096,  /* = FORMAT_MESSAGE_IGNORE_INSERTS  
