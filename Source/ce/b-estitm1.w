...
DEFINE VARIABLE h_copyest AS HANDLE NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-br-estitm est.est-no est.est-date eb.cust-no ~
eb.ship-id eb.part-no eb.part-dscr1 eb.stock-no eb.bl-qty eb.style ef.board ~
ef.medium ef.flute ef.cal eb.procat eb.len eb.wid eb.dep eb.cust-% eb.i-col ~
eb.i-coat eb.form-no eb.blank-no eb.num-wid eb.num-len eb.num-up eb.die-in ~
ef.f-col ef.f-pass ef.f-coat ef.f-coat-p eb.pur-man 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-estitm est.est-date eb.cust-no ~
eb.ship-id eb.part-no eb.part-dscr1 eb.stock-no eb.bl-qty eb.style ef.board ~
ef.medium ef.flute ef.cal eb.procat eb.len eb.wid eb.dep eb.cust-% eb.i-col ~
eb.i-coat eb.die-in ef.f-col ef.f-pass ef.f-coat ef.f-coat-p eb.pur-man 
&Scoped-define ENABLED-TABLES-IN-QUERY-br-estitm est eb ef
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br-estitm est
&Scoped-define SECOND-ENABLED-TABLE-IN-QUERY-br-estitm eb
&Scoped-define THIRD-ENABLED-TABLE-IN-QUERY-br-estitm ef
...
"ef.medium" "Paper 1" ? "character" ? ? ? ? ? ? yes ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[12]   > ASI.ef.flute
"ef.flute" "Paper 2" ? "character" ? ? ? ? ? ? yes ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
   ...
   ELSE IF AVAIL est AND  est.estimateTypeID = "SetSubAssembly"  THEN do:
       EMPTY TEMP-TABLE ttInputEst .
       EMPTY TEMP-TABLE tt-eb-set.
       RUN est/dAddSetEst.w(INPUT "Edit", INPUT "F", INPUT "SetSubAssembly", INPUT ROWID(eb)) .
       RUN local-open-query.
   END.
   ELSE
     ...
     WHEN "part-no" THEN DO:
      ...
      WHEN "procat" THEN DO:
       ...
       WHEN "Board" THEN DO:
           ...
           END. /* if foam */
           ELSE DO:
               RUN windows/l-board1.w (eb.company,lv-ind,ef.board:SCREEN-VALUE IN BROWSE {&browse-name}, OUTPUT lv-rowid).
...
DO:
   ...
   IF NOT AVAIL item THEN DO:
      ...
      RETURN NO-APPLY.
   END.              
   IF AVAIL ITEM AND ef.medium:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} EQ '' THEN DO:
       FIND FIRST item-bom
           WHERE item-bom.company EQ ITEM.company
             AND item-bom.parent-i EQ ITEM.i-no
             AND item-bom.line# EQ 1
           NO-LOCK NO-ERROR.
       IF AVAIL item-bom THEN
           ef.medium:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = item-bom.i-no.
   END.
   IF AVAIL ITEM AND ef.flute:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} EQ '' THEN DO:
       FIND FIRST item-bom
           WHERE item-bom.company EQ ITEM.company
             AND item-bom.parent-i EQ ITEM.i-no
             AND item-bom.line# EQ 2
           NO-LOCK NO-ERROR.
       IF AVAIL item-bom THEN
           ef.flute:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = item-bom.i-no.
   END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ef.medium br-estitm _BROWSE-COLUMN B-table-Win
ON LEAVE OF ef.medium IN BROWSE br-estitm /* Paper 1 */
DO:
  IF LASTKEY NE -1 THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ef.flute br-estitm _BROWSE-COLUMN B-table-Win
ON LEAVE OF ef.flute IN BROWSE br-estitm /* Paper 2 */
DO:
  IF LASTKEY NE -1 THEN DO:
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR lvFlute LIKE ef.flute NO-UNDO.
  DEF VAR lvMedium LIKE ef.medium NO-UNDO.
  DEF VAR ld-markup AS DEC NO-UNDO.
  DEF VAR ll AS LOG NO-UNDO.
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ASSIGN
      lvFlute = ef.flute:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}
      lvMedium = ef.medium:SCREEN-VALUE IN BROWSE {&BROWSE-NAME}.
  END.
  viEQtyPrev = eb.eqty.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL eb THEN DO:
    /* Set the search est no field so that add estimate if faster */
    v-rowid-eb  = ROWID(eb).
    RUN get-link-handle IN adm-broker-hdl (THIS-PROCEDURE,'record-source':U,OUTPUT char-hdl).
...
------------------------------------------------------------------------------*/
  ...
  IF NOT AVAIL bf-ord THEN DO:
    RUN est/d-esttyp.w (ROWID(est), INPUT-OUTPUT lv-type) NO-ERROR.
    IF lv-type EQ ? THEN RETURN.
  END.
  ...
  IF est.est-type EQ 2 THEN DO:
    ...
    IF est.estimateTypeID EQ "SetSubAssembly" then DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE valid-medium B-table-Win 
