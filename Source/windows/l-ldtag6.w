...
&Scoped-define INTERNAL-TABLES loadtag rm-bin
...
loadtag.i-name loadtag.loc loadtag.loc-bin loadtag.po-no ~
...
/*  AND rm-bin.qty > 0 */ NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 loadtag rm-bin
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 loadtag
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 rm-bin
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      ...
      loadtag.pallet-count COLUMN-LABEL "Qty/Tag" FORMAT "->,>>>,>>9":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE BROWSE-1
     _TblList          = "ASI.loadtag,ASI.rm-bin WHERE ASI.loadtag ..."
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "ASI.loadtag.company = ip-company and
loadtag.item-type = ip-itemtype"
     _JoinCode[2]      = "ASI.rm-bin.company = ASI.loadtag.company
  AND ASI.rm-bin.loc = ASI.loadtag.loc
  AND ASI.rm-bin.i-no = ASI.loadtag.i-no
  AND ASI.rm-bin.loc-bin = ASI.loadtag.loc-bin
  AND ASI.rm-bin.tag = ASI.loadtag.tag-no
/*  AND ASI.rm-bin.qty > 0 */"
...
"loadtag.tag-no" ? "X(23)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     ...
     _FldNameList[4]   = ASI.loadtag.loc
     _FldNameList[5]   > ASI.loadtag.loc-bin
...
"loadtag.po-no" "PO#" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[7]   > ASI.loadtag.pallet-count
"loadtag.pallet-count" "Qty/Tag" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
