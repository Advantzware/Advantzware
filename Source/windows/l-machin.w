...
def input param ip-ind as cha no-undo.
...
&scoped-define fld-name-1 mach.m-code
&scoped-define fld-name-2 mach.m-dscr
&scoped-define fld-name-3 mach.dept[1]
&scoped-define SORTBY-1 BY mach.m-code
&scoped-define SORTBY-2 BY mach.m-dscr
&scoped-define SORTBY-3 BY mach.dept[1]
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      mach.m-code FORMAT "x(6)":U COLUMN-FONT 0
      mach.m-dscr FORMAT "x(20)":U COLUMN-FONT 0
...
and mach.loc  = ip-loc
and mach.industry = ip-ind"
     _FldNameList[1]   > ASI.mach.m-code
"mach.m-code" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > ASI.mach.m-dscr
"mach.m-dscr" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Machine Information */
DO:
...
DO:
   op-char-val = mach.m-code:screen-value in browse {&browse-name} + "," +
                 mach.m-dscr + "," + string(mach.max-wid) + "," + string(mach.max-len)
                 + "," + mach.dept[1].
