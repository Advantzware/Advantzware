...
&scoped-define SORTBY-2 BY item.i-name BY item.i-no
&scoped-define SORTBY-3 BY item.i-code BY item.i-no
...
"item.i-code" "E/R" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Laminate */
DO:
...
DO:
   op-char-val = item.i-no:screen-value in browse {&browse-name} + "," +
                 item.cal:screen-value in browse {&browse-name} + "," +
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  &SCOPED-DEFINE key-phrase item.company = ip-company AND (item.industry = ip-industry or ip-industry = '') and item.mat-type = 'L'
