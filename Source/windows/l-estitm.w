...
&IF DEFINED(UIB_is_Running) NE 0 &THEN
  ...
  def var op-rec-val as RECID no-undo. /* string i-code + i-name */
&else 
  def input parameter ip-company like itemfg.company no-undo.
  def input parameter ip-loc like est.loc no-undo.
  DEF INPUT PARAMETER ip-est-no LIKE est.est-no NO-UNDO.
  def input parameter ip-cur-val as cha no-undo.
  def output parameter op-rec-val as RECID no-undo. /* string i-code + i-name */
&endif
...
and eb.loc = ip-loc ~
and eb.est-no = ip-est-no NO-LOCK ~
    ~{&SORTBY-PHRASE}
