...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 88 BY 11.19
...
"oe-bolh.bol-no" "BOL#" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
DEFINE VAR same-cust AS CHAR NO-UNDO.
DEFINE VAR same-shipto AS CHAR NO-UNDO.
   do counter = 1 to BROWSE BROWSE-1:NUM-SELECTED-ROWS:
      BROWSE BROWSE-1:FETCH-SELECTED-ROW(counter).

      IF counter = 1 THEN 
         ASSIGN 
         same-cust   = oe-bolh.cust-no 
