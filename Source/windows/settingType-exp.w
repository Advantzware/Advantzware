...
ASSIGN 
    cTextListToSelect  = "Name,Description,Data Type,Valid Value,Default Value,Minimum Valid Value," +
                           "Maximum Valid Value, Is Password,Has Context,Security Level,Category Tags" 
    cFieldListToSelect = "settingName,description,dataType,validValues,defaultValue,validValueMin," +
                           "validValueMax,isPassword,hasContext,securityLevel,categoryTags" .
...
begin_Data-type end_Data-type sl_avail Btn_Add sl_selected Btn_Remove ~
btn_Up btn_down tb_runExcel fi_file btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_Name end_name begin_Data-type ~
end_Data-type sl_avail sl_selected tb_excel tb_runExcel fi_file 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getValue-settingType rd-fgexp 
FUNCTION getValue-settingType RETURNS CHARACTER
    ( BUFFER ipb-settingType FOR settingType, ipc-field AS CHARACTER )  FORWARD.
...
DEFINE VARIABLE begin_Data-type AS CHARACTER FORMAT "x(50)" 
    LABEL "Data Type" 
    VIEW-AS FILL-IN 
    SIZE 20 BY 1.
...
DEFINE VARIABLE end_Data-type   AS CHARACTER FORMAT "X(50)" INITIAL "zzzzzzzzzz" 
    LABEL "To Data Type" 
    VIEW-AS FILL-IN 
    SIZE 21 BY 1.

DEFINE VARIABLE end_name        AS CHARACTER FORMAT "X(100)" INITIAL "zzzzzzzzzzzzzzzzzzzz" 
    LABEL "To Name" 
    VIEW-AS FILL-IN 
    SIZE 21 BY 1.
...
DEFINE FRAME rd-fgexp
    ...
    begin_Data-type AT ROW 5.43 COL 28 COLON-ALIGNED HELP
    "Enter Beginning Data Type" WIDGET-ID 146
    end_Data-type AT ROW 5.43 COL 71 COLON-ALIGNED HELP
    "Enter Ending Data Type" WIDGET-ID 148
    ...
    TITLE "Setting Type To Excel" WIDGET-ID 100.
...
ASSIGN 
    begin_Data-type:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    begin_Name:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    end_Data-type:PRIVATE-DATA IN FRAME rd-fgexp = "parm".
...
ON HELP OF FRAME rd-fgexp /* Setting To Excel */
    DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rd-fgexp rd-fgexp
ON WINDOW-CLOSE OF FRAME rd-fgexp /* Setting To Excel */
    DO:
...
&Scoped-define SELF-NAME begin_Data-type
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_Data-type rd-fgexp
ON LEAVE OF begin_Data-type IN FRAME rd-fgexp /* Data Type */
    DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_Data-type
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_Data-type rd-fgexp
ON LEAVE OF end_Data-type IN FRAME rd-fgexp /* To Data Type */
    DO:
        ASSIGN {&self-name}.
    END.
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_Name end_name begin_Data-type end_Data-type sl_avail sl_selected 
        tb_excel tb_runExcel fi_file 
        WITH FRAME rd-fgexp.
    ENABLE RECT-6 RECT-7 RECT-8 begin_Name end_name begin_Data-type end_Data-type 
...
PROCEDURE run-report :
    ...
    FOR EACH settingType NO-LOCK 
        WHERE settingType.settingName GE begin_name
        AND settingType.settingName LE end_name
        AND settingType.dataType GE begin_data-type
        AND settingType.dataType LE end_data-type :
    
        v-excel-detail-lines = "".    
    
        FOR EACH ttRptSelected:          
            v-excel-detail-lines = v-excel-detail-lines + 
                appendXLLine(getValue-settingType(BUFFER settingType, ttRptSelected.FieldList )).
        END.        
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getValue-settingType rd-fgexp 
FUNCTION getValue-settingType RETURNS CHARACTER
    ( BUFFER ipb-settingType FOR settingType, ipc-field AS CHARACTER ) :
    /*------------------------------------------------------------------------------
    ...
    CASE ipc-field :
        ...
        DO:
            ...
            END.
            h-field = BUFFER ipb-settingType:BUFFER-FIELD(ipc-field).
