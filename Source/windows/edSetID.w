...
&SCOPED-DEFINE h_Browse01 h_edSetID
...
/* External Tables                                                      */
&Scoped-define EXTERNAL-TABLES edSetID
&Scoped-define FIRST-EXTERNAL-TABLE edSetID


/* Need to scope the external tables to this procedure                  */
DEFINE QUERY external_tables FOR edSetID.
...
DEFINE VARIABLE h_edsetid AS HANDLE NO-UNDO.
DEFINE VARIABLE h_edsetid-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/edsetid.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edsetid ).
       RUN set-position IN h_edsetid ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_edsetid ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_edsetid. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_edsetid ).
       RUN add-link IN adm-broker-hdl ( h_edsetid , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edsetid ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/edsetid.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edsetid-2 ).
       RUN set-position IN h_edsetid-2 ( 4.81 , 4.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_edsetid-2. */
       RUN add-link IN adm-broker-hdl ( h_edsetid , 'Record':U , h_edsetid-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_edsetid-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edsetid-2 ,
             h_folder , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/row-list.i "edSetID"}

  /* Get the record ROWID's from the RECORD-SOURCE.                  */
  {src/adm/template/row-get.i}

  /* FIND each record specified by the RECORD-SOURCE.                */
  {src/adm/template/row-find.i "edSetID"}
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "edSetID"}
