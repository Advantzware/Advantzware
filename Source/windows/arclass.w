...
DEFINE VARIABLE h_b-arClass AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-arclass AS HANDLE NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* arClass */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brow AR Class|View AR Class' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:        
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/arclass.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-arClass ).
       RUN set-position IN h_b-arClass ( 4.81 , 4.00 ) NO-ERROR.
       /* Size in UIB:  ( 18.57 , 137.00 ) */           

       /* Links to SmartNavBrowser h_b-arClass. */
       RUN add-link IN adm-broker-hdl ( h_b-arClass , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-arClass ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/arclass.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-arclass ).
       RUN set-position IN h_v-arclass ( 5.29 , 24.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_v-arclass. */
       RUN add-link IN adm-broker-hdl ( h_b-arClass , 'Record':U , h_v-arclass ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_v-arclass ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-arclass ,
             h_folder , 'AFTER':U ).
