...
def input parameter ip-cur-file as cha no-undo.
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH job-hdr WHERE ~{&KEY-PHRASE} ~
      ...
      ((v-check-page AND ( (lookup(job-hdr.cust-no,custcount) <> 0 AND job-hdr.cust-no <> "") OR custcount = "")) OR NOT v-check-page) NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
"job-hdr.ord-no" ? ? "integer" ? ? ? 14 ? ? no ? no no ? yes no no "U" "" ""
     ...
     OPEN QUERY {&browse-name}
          FOR EACH ASI.job-hdr
              WHERE {&key-phrase}
                AND job-hdr.company EQ ip-company
                AND ((v-check-page AND ( (lookup(job-hdr.cust-no,custcount) <> 0 AND job-hdr.cust-no <> "") OR custcount = "")) OR NOT v-check-page)
                AND (TRIM(job-hdr.est-no) BEGINS lv-search OR (TRIM(job-hdr.est-no) MATCHES lv-search AND lv-search BEGINS "*"))
              NO-LOCK
              {&sortby-1}.
