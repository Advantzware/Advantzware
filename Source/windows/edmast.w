...
&SCOPED-DEFINE h_Browse01 h_edmast
...
DEFINE VARIABLE h_edmast AS HANDLE NO-UNDO.
DEFINE VARIABLE h_edmast-2 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "EDI Customer/Vendor Master"
         HEIGHT             = 24
         WIDTH              = 150.2
         MAX-HEIGHT         = 320
         MAX-WIDTH          = 320
...
ON END-ERROR OF W-Win /* EDI Customer/Vendor Master */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* EDI Customer/Vendor Master */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/edmast.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edmast ).
       RUN set-position IN h_edmast ( 4.81 , 4.00 ) NO-ERROR.
       RUN set-size IN h_edmast ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_edmast. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_edmast ).
       RUN add-link IN adm-broker-hdl ( h_edmast , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edmast ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/edmast.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edmast-2 ).
       RUN set-position IN h_edmast-2 ( 5.05 , 4.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_edmast-2. */
       RUN add-link IN adm-broker-hdl ( h_edmast , 'Record':U , h_edmast-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_edmast-2 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_edmast-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edmast-2 ,
             h_folder , 'AFTER':U ).
