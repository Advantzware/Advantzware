...
&Scoped-Define ENABLED-OBJECTS BROWSE-1 btOk 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      sman.sman COLUMN-LABEL "Sales Rep" FORMAT "x(3)":U
      sman.sname COLUMN-LABEL "Sales Rep Name" FORMAT "x(20)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 36 BY 11.19
         BGCOLOR 8 .
...
DEFINE FRAME Dialog-Frame
     ...
     btOk AT ROW 12.67 COL 11 WIDGET-ID 2
     SPACE(11.19) SKIP(0.09)
...
"sman.sman" "Sales Rep" ? "character" ? ? ? ? ? ? no "" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > ASI.sman.sname
"sman.sname" "Sales Rep Name" ? "character" ? ? ? ? ? ? no "" no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
DO:
   IF BROWSE BROWSE-1:NUM-SELECTED-ROWS EQ 1 THEN
   ...
   ELSE DO:   
     do counter = 1 to BROWSE BROWSE-1:NUM-SELECTED-ROWS:
        BROWSE BROWSE-1:FETCH-SELECTED-ROW(counter).
           ...
           op-char-val = op-char-val + sman.sman .
