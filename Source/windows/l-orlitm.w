...
&Scoped-define INTERNAL-TABLES oe-rell

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE BROWSE-1                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 oe-rell.i-no oe-rell.job-no ~
oe-rell.job-no2 oe-rell.loc oe-rell.loc-bin oe-rell.ord-no oe-rell.qty ~
oe-rell.tag 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1 
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH oe-rell WHERE ~{&KEY-PHRASE} ~
      AND oe-rell.company = ip-company ~
and oe-rell.ord-no = ip-ord-no NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY BROWSE-1 FOR EACH oe-rell WHERE ~{&KEY-PHRASE} ~
      AND oe-rell.company = ip-company ~
and oe-rell.ord-no = ip-ord-no NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 oe-rell
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 oe-rell
...
/* Query rebuild information for BROWSE BROWSE-1
     _TblList          = "ASI.oe-rell"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "ASI.oe-rell.company = ip-company
and oe-rell.ord-no = ip-ord-no"
     _FldNameList[1]   > ASI.oe-rell.i-no
"i-no" "Item#" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > ASI.oe-rell.job-no
"job-no" "Job#" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > ASI.oe-rell.job-no2
"job-no2" "" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   = ASI.oe-rell.loc
     _FldNameList[5]   > ASI.oe-rell.loc-bin
"loc-bin" "Bin" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[6]   = ASI.oe-rell.ord-no
     _FldNameList[7]   > ASI.oe-rell.qty
"qty" "Quantity! Released" ? "integer" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[8]   > ASI.oe-rell.tag
"tag" "Tag#" "x(15)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Order Release Information */
DO:
...
DO:
   op-char-val = string(oe-rell.i-no)                 .
   op-rec-id = recid(oe-rell).                 
