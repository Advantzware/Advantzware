...
DEFINE INPUT PARAMETER ipcEmpFrom AS CHARACTER NO-UNDO.
DEFINE INPUT PARAMETER ipcEmpTo   AS CHARACTER NO-UNDO.
...
ASSIGN 
    cTextListToSelect  = "Emp.ID,First Name,Middle Name,Last Name,Social Security," + 
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Emp.ID,First Name,Middle Name,Last Name,Social Security," + 
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 RECT-8 begin_emp end_emp ~
sl_avail Btn_Def sl_selected Btn_Add Btn_Remove btn_Up btn_down fi_file ~
tb_OpenCSV tbAutoClose btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_emp end_emp sl_avail sl_selected ~
...
FUNCTION getValue-estf RETURNS CHARACTER
    ( BUFFER ipb-estf FOR employee, ipc-field AS CHARACTER  )  FORWARD.
...
DEFINE VARIABLE begin_emp AS CHARACTER FORMAT "x(6)" 
    LABEL "From Employee" 
    VIEW-AS FILL-IN 
    SIZE 20 BY 1.

DEFINE VARIABLE end_emp   AS CHARACTER FORMAT "X(6)" INITIAL "zzzzz" 
    LABEL "To Employee" 
    VIEW-AS FILL-IN 
    SIZE 21 BY 1.

DEFINE VARIABLE fi_file   AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\EmployeeTracking.csv" 
    LABEL "Name" 
...
DEFINE FRAME rd-fgexp
    begin_emp AT ROW 3.57 COL 27 COLON-ALIGNED HELP
    "Enter Beginning Employee Number" WIDGET-ID 142
    end_emp AT ROW 3.57 COL 70 COLON-ALIGNED HELP
    "Enter Ending Employee #" WIDGET-ID 144
    sl_avail AT ROW 7.95 COL 5 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 7.95 COL 45.2 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 7.95 COL 67.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 9.1 COL 45.2 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 130
    Btn_Remove AT ROW 10.29 COL 45.2 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 134
    btn_Up AT ROW 11.48 COL 45.2 WIDGET-ID 136
    btn_down AT ROW 12.67 COL 45.2 WIDGET-ID 132
    tb_excel AT ROW 15.1 COL 36 WIDGET-ID 32
    fi_file AT ROW 15.81 COL 21.8 COLON-ALIGNED HELP
    "Enter File Name" WIDGET-ID 22
    tb_OpenCSV AT ROW 15.86 COL 87.6 RIGHT-ALIGNED WIDGET-ID 34
    tbAutoClose AT ROW 17.71 COL 31.6 WIDGET-ID 64
    btn-ok AT ROW 18.67 COL 31.2 WIDGET-ID 14
    btn-cancel AT ROW 18.67 COL 56.6 WIDGET-ID 12
    "Selected Columns" VIEW-AS TEXT
    SIZE 23.8 BY .62 AT ROW 7.24 COL 67.6 WIDGET-ID 138
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 7.24 COL 5.4 WIDGET-ID 140
    " Export Selection" VIEW-AS TEXT
    SIZE 17 BY .62 AT ROW 6.48 COL 4 WIDGET-ID 86
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.05 COL 4 WIDGET-ID 36
    RECT-6 AT ROW 6.91 COL 3 WIDGET-ID 30
    RECT-7 AT ROW 1.52 COL 3 WIDGET-ID 38
    RECT-8 AT ROW 15 COL 3 WIDGET-ID 84
    SPACE(1.99) SKIP(2.65)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
    SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
    BGCOLOR 15 
    TITLE "Export Employee Tracking And Data Collection To Excel" WIDGET-ID 100.
...
ASSIGN 
    begin_emp:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    end_emp:PRIVATE-DATA IN FRAME rd-fgexp = "parm".
...
ON HELP OF FRAME rd-fgexp /* Export Employee Tracking And Data Collection To Excel */
    DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rd-fgexp rd-fgexp
ON WINDOW-CLOSE OF FRAME rd-fgexp /* Export Employee Tracking And Data Collection To Excel */
    DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_emp rd-fgexp
ON LEAVE OF begin_emp IN FRAME rd-fgexp /* From Employee */
    DO:
        ASSIGN {&self-name}.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_emp rd-fgexp
ON LEAVE OF end_emp IN FRAME rd-fgexp /* To Employee */
    DO:
        ASSIGN {&self-name}.
    END.
...
APPLY "entry" TO begin_emp.
fi_file:SCREEN-VALUE = "c:\tmp\EmployeeTracking.csv".
END.
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_emp end_emp sl_avail sl_selected fi_file tb_OpenCSV tbAutoClose 
        WITH FRAME rd-fgexp.
    ENABLE RECT-6 RECT-7 RECT-8 begin_emp end_emp sl_avail Btn_Def sl_selected 
...
PROCEDURE run-report :
    ...
    FOR EACH employee WHERE  employee.company = gcompany 
        AND employee.employee GE begin_emp 
        AND employee.employee LE end_emp NO-LOCK :
    
        v-excel-detail-lines = "".
         
        FOR EACH ttRptSelected:
            v-excel-detail-lines = v-excel-detail-lines + 
                appendXLLine(getValue-estf(BUFFER employee, ttRptSelected.FieldList )).
        END.
...
PROCEDURE Set-Sort-Data :
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        IF ipcEmpFrom <> "" THEN
            ASSIGN 
                begin_emp:SCREEN-VALUE = assignParam(ipcEmpFrom,NO)
                end_emp:SCREEN-VALUE   = assignParam(ipcEmpTo,NO)
                .
...
FUNCTION getValue-estf RETURNS CHARACTER
    ( BUFFER ipb-estf FOR employee, ipc-field AS CHARACTER  ) :
    /*------------------------------------------------------------------------------
    ...
    DEFINE VARIABLE li-t-time AS INTEGER   NO-UNDO.

    CASE ipc-field :
        WHEN "empid"  THEN 
            DO:
                lc-return = ipb-estf.employee.
            END.
        WHEN "first-n"  THEN 
            DO:
                lc-return = ipb-estf.first_name .
            END.
        WHEN "midd-n"  THEN 
            DO:
                lc-return = ipb-estf.middle_name .
            END.
        WHEN "last-n"  THEN 
            DO:
                lc-return = ipb-estf.last_name .
            END.
        WHEN "soc"  THEN 
            DO:
                lc-return = IF ipb-estf.soc_sec <> ? THEN STRING(ipb-estf.soc_sec) ELSE "".
            END.
        WHEN "rate-u"  THEN 
            DO:
                lc-return = STRING(ipb-estf.rate_usage) .
            END.
        WHEN "emp-type"  THEN 
            DO:
                lc-return = ipb-estf.emp_type .
            END.
        WHEN "key-type"  THEN 
            DO:
                lc-return = ipb-estf.keyboard_type .
            END.
        WHEN "manag"  THEN 
            DO:
                lc-return = ipb-estf.ref_no .
            END.
        WHEN "start-date"  THEN 
            DO:
                lc-return = IF ipb-estf.start_date <> ? THEN  STRING(employee.start_date) ELSE  "".
            END.

        WHEN "gl-acc"  THEN 
            DO:
                lc-return =  STRING(ipb-estf.actnum).
            END.
        WHEN "dock-time"  THEN 
            DO:
                lc-return =  STRING(ipb-estf.dock-time) .
            END.
        WHEN "lunch"  THEN 
            DO:
                lc-return =  STRING(ipb-estf.lunch_paid) .
            END.
