...
DEFINE VARIABLE h_vinvattch AS HANDLE NO-UNDO.
DEFINE VARIABLE h_vinvattch-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Attachment */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Attachment */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/vinvattch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vinvattch ).
       RUN set-position IN h_vinvattch ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_vinvattch ( 18.10 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_vinvattch. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_vinvattch ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_vinvattch ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/vinvattch.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vinvattch-2 ).
       RUN set-position IN h_vinvattch-2 ( 6.95 , 8.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.14 , 141.20 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-attach ).
       RUN set-position IN h_p-attach ( 15.76 , 59.00 ) NO-ERROR.
       RUN set-size IN h_p-attach ( 2.24 , 82.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 16.00 , 8.00 ) NO-ERROR.
       RUN set-size IN h_p-navico ( 2.14 , 38.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_vinvattch-2. */
       RUN add-link IN adm-broker-hdl ( h_p-attach , 'TableIO':U , h_vinvattch-2 ).
       RUN add-link IN adm-broker-hdl ( h_vinvattch , 'Record':U , h_vinvattch-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_vinvattch-2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-attach ,
             h_vinvattch-2 , 'AFTER':U ).
...
------------------------------------------------------------------------------*/
  ...
  DEFINE OUTPUT PARAMETER op-inv-no AS CHAR NO-UNDO.

  op-rec_key = ip-rec_key.

  IF ip-inv-no NE "" THEN
     op-inv-no = STRING(ip-inv-no,"X(12)") + "APINV".
