...
&scoped-define fld-name-2 style.dscr
&scoped-define SORTBY-1 BY style.style
&scoped-define SORTBY-2 BY style.dscr
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 style.style style.dscr 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1 
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH style WHERE ~{&KEY-PHRASE} ~
      AND style.company = ip-company and style.style <> "" ~
and style.industry = "1" NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BROWSE-1 Dialog-Frame _STRUCTURED
  QUERY BROWSE-1 NO-LOCK DISPLAY
      ...
      style.dscr COLUMN-LABEL "Description" FORMAT "x(30)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _Where[1]         = "ASI.style.company = ip-company and style.style <> """"
and style.industry = ""1"""
     _FldNameList[1]   > ASI.style.style
"style" "Style" ? "character" ? ? ? ? ? ? no ? no no "9" yes no no "U" "" ""
     _FldNameList[2]   > ASI.style.dscr
"dscr" "Description" "x(30)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Style Information */
DO:
...
DO:
   op-char-val = style.style:screen-value in browse {&browse-name} + "," +
                 style.dscr.
