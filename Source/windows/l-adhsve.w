...
and (item.mat-type = 'G' or item.mat-type = 'T') NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
(item.industry = ip-industry or ip-industry = '')
and (item.mat-type = 'G' or item.mat-type = 'T')"
...
"i-code" "E/R" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Adhesive */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  &SCOPED-DEFINE key-phrase item.company = ip-company AND (item.industry = ip-industry or ip-industry = '') and (item.mat-type = 'G' or item.mat-type = 'T')
