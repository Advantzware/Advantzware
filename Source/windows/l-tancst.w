...
&Scoped-define INTERNAL-TABLES eb est tt-report
...
eb.cust-no begins ip-cust-no and ~
TRIM(eb.master-est-no) = "" and ~
(eb.est-type = (ip-est-type - 3) or ~
 eb.est-type = ip-est-type) NO-LOCK, ~
      FIRST est OF eb  ~
      WHERE est.e-num EQ 0  NO-LOCK, ~
      FIRST tt-report WHERE TRUE /* Join to est incomplete */ ~
      AND tt-report.term-id eq v-term and ~
tt-report.key-01 eq est.est-no NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
tt-report.key-01 eq est.est-no NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 eb est tt-report
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 eb
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 est
&Scoped-define THIRD-TABLE-IN-QUERY-BROWSE-1 tt-report
...
/* Query rebuild information for BROWSE BROWSE-1
     _TblList          = "ASI.eb,ASI.est OF ASI.eb ,tt-report WHERE ASI.est ..."
...
(eb.est-type = (ip-est-type - 3) or
 eb.est-type = ip-est-type)"
     _Where[2]         = "est.e-num EQ 0 "
     _Where[3]         = "tt-report.term-id eq v-term and
tt-report.key-01 eq est.est-no"
...
WHEN 1 THEN DO:
  ...
  OPEN QUERY {&browse-name} FOR EACH ASI.eb WHERE TRUE 
                            ...
                            FIRST tt-report WHERE tt-report.term-id EQ v-term
                                           AND tt-report.key-01 EQ est.est-no.
