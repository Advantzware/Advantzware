...
def input param ip-foam as log no-undo.
def input param ip-icode as cha no-undo.
...
and index(ip-mat-type,item.mat-type) > 0 NO-LOCK ~
    ~{&SORTBY-PHRASE}.
...
DEFINE FRAME Dialog-Frame
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "~"Raw Material Information (~" + ip-mat-type + )".
...
(item.industry = ip-industry or ip-industry = """")
and index(ip-mat-type,item.mat-type) > 0"
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* "Raw Material Information (" + ip-mat-type + ) */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  case ip-mat-type :
      when "WLF" then lv-type-dscr = "Label".
      when "I" then lv-type-dscr = "Ink".
      when "GTS" then lv-type-dscr = "Joint Glue".
      when "V" then lv-type-dscr = "Coating".
