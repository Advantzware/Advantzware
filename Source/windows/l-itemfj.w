...
and (lookup(itemfg.i-no,ip-cur-val) > 0 ~
  or ip-cur-val = "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 86 BY 11.19
         BGCOLOR 8 .
...
DEFINE FRAME Dialog-Frame
     ...
     bt-ok AT ROW 14.1 COL 64
     bt-cancel AT ROW 14.1 COL 76
...
"itemfg.i-no" ? ? "character" ? ? ? ? ? ? no ? no no "30" yes no no "U" "" ""
     _FldNameList[2]   > ASI.itemfg.i-name
"itemfg.i-name" ? ? "character" ? ? ? ? ? ? no ? no no "50" yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
