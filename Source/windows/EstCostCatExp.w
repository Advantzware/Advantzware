...
ASSIGN 
    cTextListToSelect  = "Cost Category,Group,Group Level,ID" 

    cFieldListToSelect = "cost-cat,group,group-level,cost-id" .

ASSIGN 
    cTextListToDefault = "Cost Category,Group,Group Level,ID"  .
{sys/inc/ttRptSel.i}
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 RECT-8 begin_cost-cat ~
end_cost-cat begin_id end_id sl_avail Btn_Def sl_selected Btn_Add ~
Btn_Remove btn_Up btn_down fi_file tb_OpenCSV tbAutoClose btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_cost-cat end_cost-cat begin_id ~
end_id sl_avail sl_selected fi_file tb_OpenCSV tbAutoClose 
...
DEFINE VARIABLE begin_cost-cat AS CHARACTER FORMAT "x(50)" 
    LABEL "From Cost Category" 
    VIEW-AS FILL-IN 
    SIZE 20 BY 1.

DEFINE VARIABLE begin_id       AS CHARACTER FORMAT "x(50)" 
    LABEL "From Cost ID" 
    VIEW-AS FILL-IN 
    SIZE 20 BY 1.

DEFINE VARIABLE end_cost-cat   AS CHARACTER FORMAT "X(50)" INITIAL "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz" 
    LABEL "To Cost Category" 
    VIEW-AS FILL-IN 
    SIZE 21 BY 1.

DEFINE VARIABLE end_id         AS CHARACTER FORMAT "X(50)" INITIAL "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz" 
    LABEL "To Cost ID" 
    VIEW-AS FILL-IN 
    SIZE 21 BY 1.

DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\EstCostCategory.csv" 
    LABEL "Name" 
...
DEFINE FRAME rd-fgexp
    begin_cost-cat AT ROW 3.43 COL 25.2 COLON-ALIGNED HELP
    "Enter Beginning Vendor Number" WIDGET-ID 142
    end_cost-cat AT ROW 3.43 COL 65 COLON-ALIGNED HELP
    "Enter Cost Category" WIDGET-ID 144
    begin_id AT ROW 4.86 COL 25.2 COLON-ALIGNED HELP
    "Enter Beginning Vendor Number" WIDGET-ID 146
    end_id AT ROW 4.86 COL 65 COLON-ALIGNED HELP
    "Enter Cost ID" WIDGET-ID 148
...
ASSIGN 
    begin_cost-cat:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    begin_id:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    end_cost-cat:PRIVATE-DATA IN FRAME rd-fgexp = "parm".

ASSIGN 
    end_id:PRIVATE-DATA IN FRAME rd-fgexp = "parm".
...
&Scoped-define SELF-NAME begin_cost-cat
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_cost-cat rd-fgexp
ON LEAVE OF begin_cost-cat IN FRAME rd-fgexp /* From Cost Category */
    DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME begin_id
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_id rd-fgexp
ON LEAVE OF begin_id IN FRAME rd-fgexp /* From Cost ID */
    DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_cost-cat
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_cost-cat rd-fgexp
ON LEAVE OF end_cost-cat IN FRAME rd-fgexp /* To Cost Category */
    DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_id
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_id rd-fgexp
ON LEAVE OF end_id IN FRAME rd-fgexp /* To Cost ID */
    DO:
        ASSIGN {&self-name}.
    END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        APPLY "entry" TO begin_cost-cat.
        fi_file:SCREEN-VALUE = "c:\tmp\EstCostCategory.csv".
    END.
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_cost-cat end_cost-cat begin_id end_id sl_avail sl_selected 
        fi_file tb_OpenCSV tbAutoClose 
        WITH FRAME rd-fgexp.
    ENABLE RECT-6 RECT-7 RECT-8 begin_cost-cat end_cost-cat begin_id end_id 
...
PROCEDURE run-report :
    ...
    FOR EACH ttEstCostCategory NO-LOCK
        WHERE ttEstCostCategory.costCategoryLabel GE begin_cost-cat
        AND ttEstCostCategory.costCategoryLabel LE end_cost-cat
        AND ttEstCostCategory.estCostCategoryID GE begin_id
        AND ttEstCostCategory.estCostCategoryID LE end_id  , 
        ...
        FOR EACH ttRptSelected:    
            CASE ttRptSelected.FieldList:                                                               
                WHEN "cost-cat" THEN                                                                
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(ttEstCostCategory.costCategoryLabel).            
                WHEN "group" THEN                                                              
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(ttEstCostGroup.costGroupLabel).         
                WHEN "group-level" THEN                                                              
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(ttEstCostGroupLevel.estCostGroupLevelDesc).         
                WHEN "cost-id" THEN                                                             
                    v-excel-detail-lines = v-excel-detail-lines + appendXLLine(ttEstCostCategory.estCostCategoryID).        
