...
def input parameter ip-cust-no like oe-ord.cust-no no-undo.
def input parameter ip-ship-id like oe-relh.ship-id no-undo.
def input parameter ip-i-no like po-ordl.i-no no-undo.
...
&Scoped-define INTERNAL-TABLES oe-ord oe-ordm
...
oe-ord.ord-date oe-ord.sold-name oe-ordm.charge 
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1 
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH oe-ord WHERE ~{&KEY-PHRASE} ~
      AND oe-ord.company Eq ip-company ~
AND oe-ord.opened EQ YES ~
AND (oe-ord.cust-no EQ ip-cust-no ~
OR ip-cust-no EQ "") ~
AND LOOKUP(oe-ord.stat,lv-ord-ok) GT 0  NO-LOCK, ~
      FIRST oe-ordm OF oe-ord ~
      WHERE oe-ordm.charge = ip-i-no NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
AND LOOKUP(oe-ord.stat,lv-ord-ok) GT 0  NO-LOCK, ~
      ...
      WHERE oe-ordm.charge = ip-i-no NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 oe-ord oe-ordm
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 oe-ord
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 oe-ordm
...
DEFINE FRAME Dialog-Frame
     ...
     SPACE(54.79) SKIP(1.89)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "PO Information (Open Orders)".
...
/* Query rebuild information for BROWSE BROWSE-1
     _TblList          = "ASI.oe-ord,ASI.oe-ordm OF ASI.oe-ord"
     _Options          = "NO-LOCK KEY-PHRASE SORTBY-PHRASE"
     _TblOptList       = ", FIRST"
     _Where[1]         = "ASI.oe-ord.company Eq ip-company
AND oe-ord.opened EQ YES
AND (oe-ord.cust-no EQ ip-cust-no
OR ip-cust-no EQ """")
AND LOOKUP(oe-ord.stat,lv-ord-ok) GT 0 "
     _Where[2]         = "ASI.oe-ordm.charge = ip-i-no"
...
"oe-ord.est-no" "Estimate#" "x(8)" "character" ? ? ? ? ? ? no ? no no "14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[9]   = ASI.oe-ordm.charge
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* PO Information (Open Orders) */
DO:
