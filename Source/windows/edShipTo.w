...
&SCOPED-DEFINE h_Browse01 h_edshipto
...
DEFINE VARIABLE h_edshipto AS HANDLE NO-UNDO.
DEFINE VARIABLE h_edshipto-2 AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* EDI Shipto Codes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* EDI Shipto Codes */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/edshipto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edshipto ).
       RUN set-position IN h_edshipto ( 4.62 , 4.00 ) NO-ERROR.
       RUN set-size IN h_edshipto ( 19.52 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_edshipto. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_edshipto ).
       RUN add-link IN adm-broker-hdl ( h_edshipto , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edshipto ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/edshipto.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_edshipto-2 ).
       RUN set-position IN h_edshipto-2 ( 4.81 , 4.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_edshipto-2. */
       RUN add-link IN adm-broker-hdl ( h_edshipto , 'Record':U , h_edshipto-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_edshipto-2 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_edshipto-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_edshipto-2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_edshipto-2 , 'AFTER':U ).
