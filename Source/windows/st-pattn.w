...
&SCOPED-DEFINE h_Browse01 h_st-pattn
...
DEFINE VARIABLE h_st-pattn AS HANDLE NO-UNDO.
DEFINE VARIABLE h_stkimage AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Stack Pattern|Image' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updsav ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_st-pattn ,
             h_p-updsav , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/st-pattn.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_stkimage ).
       RUN set-position IN h_stkimage ( 4.81 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 15.52 , 146.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_stkimage. */
       RUN add-link IN adm-broker-hdl ( h_st-pattn , 'stackImage':U , h_stkimage ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_stkimage ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
