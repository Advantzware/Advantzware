...
&SCOPED-DEFINE h_Browse01 h_bank
...
DEFINE VARIABLE h_bank AS HANDLE NO-UNDO.
DEFINE VARIABLE h_bank-2 AS HANDLE NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Banks */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_f-add ).
       RUN set-position IN h_f-add ( 1.00 , 54.20 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Browse Banks|View Bank' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       ...
       /* Links to SmartNavBrowser h_bank. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_bank ).
       RUN add-link IN adm-broker-hdl ( h_bank , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bank ,
             FRAME message-frame:HANDLE , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/bank.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bank-2 ).
       ...
       /* Links to SmartViewer h_bank-2. */
       RUN add-link IN adm-broker-hdl ( h_bank , 'Record':U , h_bank-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updncp , 'TableIO':U , h_bank-2 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'add-item':U , h_bank-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bank-2 ,
             FRAME message-frame:HANDLE , 'AFTER':U ).
