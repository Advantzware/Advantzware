...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH rfqitem WHERE ~{&KEY-PHRASE} ~
      AND rfqitem.company = ip-company  ~
 NO-LOCK, ~
      EACH rfq OF rfqitem NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 rfq
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _TblOptList       = ","
     _Where[1]         = "rfq.rfqitem.company = ip-company 
"
     _JoinCode[2]      = "rfq.rfqitem.company = ip-company 
"
     _FldNameList[1]   = rfq.rfqitem.rfq-no
     _FldNameList[2]   = rfq.rfq.cust-no
     _FldNameList[3]   > rfq.rfqitem.stock-no
"rfqitem.stock-no" "FG Item#" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[4]   = rfq.rfqitem.i-name
     _FldNameList[5]   > rfq.rfqitem.part-no
"rfqitem.part-no" "Customer Part#" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
DO:
   op-char-val = string(rfqitem.rfq-no) + "," +
                 string(rfqitem.seq) 
                 .
