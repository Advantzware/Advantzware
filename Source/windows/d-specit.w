...
def input param ip-num-item as int no-undo.
def input param ip-item-list as cha no-undo.
def output param op-item-no as cha no-undo.
{custom/globdefs.i}
def temp-table tt-item field i-no like itemfg.i-no
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Items for Spec Note */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE build-browser Dialog-Frame 
PROCEDURE build-browser :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  ...
  do li-cnt = 1 to ip-num-item: 
     create tt-item.
     assign tt-item.i-no = entry(li-cnt,ip-item-list).

     find itemfg where itemfg.company = g_company and
                       itemfg.i-no = tt-item.i-no
                       no-lock no-error.
     tt-item.i-name = if avail itemfg then itemfg.i-name else ''.
