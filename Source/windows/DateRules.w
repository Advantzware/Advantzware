...
&SCOPED-DEFINE h_Browse01 h_DateRules
...
DEFINE VARIABLE h_DateRules AS HANDLE NO-UNDO.
DEFINE VARIABLE h_DateRules-2 AS HANDLE NO-UNDO.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_smartmsg ).
       RUN set-position IN h_smartmsg ( 1.48 , 4.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/DateRules.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_DateRules ).
       RUN set-position IN h_DateRules ( 3.14 , 2.00 ) NO-ERROR.
       RUN set-size IN h_DateRules ( 26.19 , 158.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_DateRules. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_DateRules ).
       RUN add-link IN adm-broker-hdl ( h_DateRules , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_DateRules ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/DateRules.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
       ...
       /* Links to SmartViewer h_DateRules-2. */
       RUN add-link IN adm-broker-hdl ( h_DateRules , 'Record':U , h_DateRules-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_DateRules-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_DateRules-2 ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_DateRules-2 , 'AFTER':U ).
