...
DEFINE VARIABLE h_venditemcost AS HANDLE NO-UNDO.
DEFINE VARIABLE h_venditemcost-2 AS HANDLE NO-UNDO.
...
IF THIS-PROCEDURE:INSTANTIATING-PROCEDURE:NAME NE "nosweat/persist.p" THEN 
DO:   
...
--------------------------------------------------- ---------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/vendcostmtx.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
       ...
       RUN add-link IN adm-broker-hdl ( h_venditemcost , 'Record':U , THIS-PROCEDURE ).
       RUN add-link IN adm-broker-hdl ( h_venditemcost , 'export-xl':U , h_export ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_venditemcost ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/vendcostmtx.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/bVendCostLevel.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
       ...
       RUN add-link IN adm-broker-hdl ( h_venditemcost , 'Record':U , h_venditemcost-2 ).
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_venditemcost-2 ).
...
PROCEDURE pCheckEditMode :
   ...
   DEFINE VARIABLE lQuoteitmExists AS LOGICAL NO-UNDO.    
   IF VALID-HANDLE(h_venditemcost-2) THEN DO:     
     RUN vendcost-newitem IN h_venditemcost-2 (OUTPUT lQuoteitmExists).
     IF lQuoteitmExists THEN DO:
