...
def input parameter ip-i-code like item.i-code no-undo.
...
and (lookup(item.mat-type,ip-mat-type) > 0 or ip-mat-type eq "") ~
and (item.i-code = ip-i-code or ip-i-code = "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
(item.industry = ip-industry or ip-industry = """")
and (lookup(item.mat-type,ip-mat-type) > 0 or ip-mat-type eq """")
and (item.i-code = ip-i-code or ip-i-code = """")"
     _FldNameList[1]   > ASI.item.i-no
"item.i-no" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[2]   > ASI.item.i-name
"item.i-name" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _FldNameList[3]   > ASI.item.i-dscr
"item.i-dscr" ? ? "character" ? ? 0 ? ? ? no ? no no ? yes no no "U" "" ""
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Raw Materials */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  &SCOPED-DEFINE key-phrase item.company = ip-company AND (item.industry = ip-industry or ip-industry = "") and (lookup(item.mat-type,ip-mat-type) > 0 or ip-mat-type eq "") and (item.i-code = ip-i-code or ip-i-code = "")
