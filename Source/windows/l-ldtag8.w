...
&Scoped-define INTERNAL-TABLES rm-bin loadtag 
...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH rm-bin WHERE ~
      rm-bin.company EQ ip-company ~
 NO-LOCK, ~
   EACH loadtag WHERE  ~{&KEY-PHRASE} ~
      AND loadtag.company = rm-bin.company ~
      AND loadtag.item-type = ip-itemtype ~
      AND loadtag.is-case-tag = ll-casetag ~
      AND loadtag.i-no EQ rm-bin.i-no ~
      AND loadtag.loc EQ rm-bin.loc ~
      AND loadtag.loc-bin EQ rm-bin.loc-bin ~
      AND loadtag.tag-no EQ rm-bin.tag ~
        NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY BROWSE-1  FOR EACH rm-bin WHERE ~
  rm-bin.company EQ ip-company ~
 NO-LOCK, ~
   EACH loadtag WHERE  ~{&KEY-PHRASE} ~
      AND loadtag.company = rm-bin.company ~
      ...
      AND loadtag.tag-no EQ rm-bin.tag ~
        NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-BROWSE-1 rm-bin loadtag 
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-1 rm-bin 
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-1 loadtag
...
loadtag.is-case-tag = ll-casetag"
     _JoinCode[2]      = "rm-bin.company EQ loadtag.company
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  END.
   ...
   /* If they didn't pass anything in, no need to open query twice */
   /* IF ip-cur-val GT "" THEN */
     RUN new-rd-sort.   
