...
DEFINE VARIABLE h_specnot3 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_specnot3-2 AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'browsers/specnot3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_specnot3-2 ).
       RUN set-position IN h_specnot3-2 ( 4.81 , 7.00 ) NO-ERROR.
       RUN set-size IN h_specnot3-2 ( 18.10 , 139.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_specnot3-2. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_specnot3-2 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnot3-2 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'viewers/specnot3.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_specnot3 ).
       RUN set-position IN h_specnot3 ( 4.86 , 12.00 ) NO-ERROR.
       ...
       /* Links to SmartViewer h_specnot3. */
       RUN add-link IN adm-broker-hdl ( h_p-updsav , 'TableIO':U , h_specnot3 ).
       RUN add-link IN adm-broker-hdl ( h_specnot3-2 , 'Record':U , h_specnot3 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_specnot3 ,
             h_folder , 'AFTER':U ).
