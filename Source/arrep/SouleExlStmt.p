...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tt-cust-excel:
    ...
    ASSIGN
      ...
      chWorkSheet:Range("F7"):value = "Terms: " + tt-cust-excel.terms.
    ...
    FOR EACH tt-inv WHERE
        ...
        chWorkSheet:Range("B" + STRING(row-count)):value = tt-inv.po-no
...
------------------------------------------------------------------------------*/
  ...
  RUN sys/ref/getFileFullPathName.p ("Template\SouleStatement.xlt", OUTPUT chFile).
  IF chFile = ? THEN  
