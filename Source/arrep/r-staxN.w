...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 begin_stax-gr end_stax-gr ~
sl_avail Btn_Def sl_selected Btn_Add Btn_Remove btn_Up btn_down rd-dest ~
fi_file tb_OpenCSV tbAutoClose btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_stax-gr end_stax-gr sl_avail ~
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\SalesTaxList.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    ...
    sl_selected AT ROW 5.19 COL 60.6 NO-LABELS WIDGET-ID 28
    Btn_Add AT ROW 6.19 COL 40.6 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 32
    Btn_Remove AT ROW 7.19 COL 40.6 HELP
    "Remove Selected Table from Tables to Audit" WIDGET-ID 34
    btn_Up AT ROW 8.24 COL 40.6 WIDGET-ID 40
    btn_down AT ROW 9.24 COL 40.6 WIDGET-ID 42
    lv-ornt AT ROW 11.24 COL 31 NO-LABELS
    lines-per-page AT ROW 11.24 COL 84 COLON-ALIGNED
    rd-dest AT ROW 11.38 COL 6 NO-LABELS
    lv-font-no AT ROW 12.19 COL 35 COLON-ALIGNED
    tb_excel AT ROW 12.19 COL 68 RIGHT-ALIGNED
    lv-font-name AT ROW 13.14 COL 29 COLON-ALIGNED NO-LABELS
    td-show-parm AT ROW 14.24 COL 28.6
    fi_file AT ROW 15.43 COL 26.6 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 15.52 COL 92.4 RIGHT-ALIGNED
    tbAutoClose AT ROW 17 COL 28.6 WIDGET-ID 78
    btn-ok AT ROW 18 COL 28.6
    btn-cancel AT ROW 18 COL 48.6
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 4.48 COL 60.4 WIDGET-ID 44
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21.2 BY .71 AT ROW 1.14 COL 5
    " Output Destination" VIEW-AS TEXT
    SIZE 19 BY .62 AT ROW 10.67 COL 5
    "Available Columns" VIEW-AS TEXT
    SIZE 29 BY .62 AT ROW 4.48 COL 4.2 WIDGET-ID 38
...
/* SETTINGS FOR FRAME FRAME-A
   FRAME-NAME                                                           */
ASSIGN 
    begin_stax-gr:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    end_stax-gr:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ON END-ERROR OF C-Win /* Sales Tax List */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Sales Tax List */
    DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "AR3" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_stax-gr end_stax-gr sl_avail sl_selected rd-dest fi_file 
        tb_OpenCSV tbAutoClose 
        WITH FRAME FRAME-A IN WINDOW C-Win.
    ENABLE RECT-6 RECT-7 begin_stax-gr end_stax-gr sl_avail Btn_Def sl_selected 
...
PROCEDURE run-report :
    /* --------------------------------------------------- sys/rep/stax.p 9/91 cd */
    /* stax list printout                                                         */
    /* -------------------------------------------------------------------------- */

    /*{sys/form/r-top.f}*/

    DEFINE VARIABLE ftax           AS CHARACTER NO-UNDO.
    DEFINE VARIABLE ttax           LIKE ftax NO-UNDO.
    DEFINE VARIABLE i              AS INTEGER   NO-UNDO.
    DEFINE VARIABLE l-codes-exist  AS LOGICAL   NO-UNDO.
    ...
    DEFINE VARIABLE v-tax-code     AS CHARACTER NO-UNDO.
    DEFINE VARIABLE v-tax-dscr     AS CHARACTER NO-UNDO.
    DEFINE VARIABLE v-tax-rate     AS DECIMAL   NO-UNDO.
    DEFINE VARIABLE v-tax-acc      AS CHARACTER NO-UNDO.
    ...
    FORM stax.tax-group   TO 10
        stax.tax-code1[i] TO 16
        stax.tax-dscr1[i] AT 19
        stax.tax-rate1[i] AT 46
        stax.tax-acc1[i]  AT 56 
        /*        stax.tax-code[2] to 16
                    stax.tax-dscr[2] at 19
                        stax.tax-rate[2] at 46
                            stax.tax-acc[2] at 56 skip
                stax.tax-code[3] to 16
        ...
        WITH FRAME stax WIDTH 80 OVERLAY NO-LABELS NO-UNDERLINE STREAM-IO DOWN.
    ...
    DO i = 1 TO EXTENT(stax.tax-code1):
        IF stax.tax-rate1[i] <> 0 THEN 
        DO:
            l-codes-exist = TRUE.
            /*    DISPLAY stax.tax-code1[i]
                        stax.tax-dscr1[i]
                        stax.tax-rate1[i]
                        stax.tax-acc1[i] WITH FRAME stax.
            ...
            ASSIGN
                v-tax-code = stax.tax-code1[i]                 
                v-tax-dscr = stax.tax-dscr1[i]                   
                v-tax-rate = stax.tax-rate1[i]                   
                v-tax-acc  = stax.tax-acc1[i] .  
            ...
            DO j = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField: 
                    WHEN "grp"    THEN 
                        cVarValue = IF i = 1 THEN STRING(stax.tax-group) ELSE "" .
                    WHEN "cod"    THEN 
                        cVarValue = STRING(stax.tax-code1[i]) .
                    WHEN "dscr"   THEN 
                        cVarValue = STRING(stax.tax-dscr1[i]) .
                    WHEN "rate"   THEN 
                        cVarValue = STRING(stax.tax-rate1[i],">>>9.99999") .
                    WHEN "act"    THEN 
                        cVarValue = STRING(stax.tax-acc1[i]) .
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\SalesTaxList.csv".    
    END.
