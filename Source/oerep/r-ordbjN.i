     ...
     DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
        cTmpField = entry(getEntryNumber(INPUT cTextListToSelect, INPUT ENTRY(i,cSelectedList)), cFieldListToSelect).
             CASE cTmpField: 
                  ...
                  WHEN "ord-no"       THEN cVarValue = STRING(v-ord-no) .
                  ...
                  WHEN "invrel-dt"    THEN cVarValue = IF v-dat NE ? THEN STRING(v-dat) ELSE "" .
                  WHEN "inv-no"       THEN cVarValue = IF tt-report.inv THEN STRING(tt-report.inv-no) ELSE "" .
                  WHEN "qty-pro"      THEN cVarValue = string(tt-report.prod-qty,"->>,>>>,>>>") .
                  WHEN "qty-to-pro"   THEN cVarValue = STRING(tt-report.qty-to-prod,"->,>>>,>>>,>>>") .
                  WHEN "qty-shp"      THEN cVarValue = STRING(tt-report.q-shp,"->>,>>>,>>>").
                  WHEN "rel-qty"      THEN cVarValue = IF AVAIL oe-rell AND NOT tt-report.inv THEN STRING(oe-rell.qty,"->>,>>>,>>>") ELSE IF AVAIL oe-rel AND NOT tt-report.inv THEN STRING(oe-rel.tot-qty,"->>,>>>,>>>") ELSE "".
                  WHEN "inv-amt"      THEN cVarValue =  STRING(v-field2).
                  WHEN "bal-due"      THEN cVarValue = STRING(v-bal-qty,"->>,>>>,>>>") .
                  WHEN "qty-on-hand"  THEN cVarValue = IF last-of(tt-report.row-id) THEN STRING(on-hand-qty,"->>,>>>,>>9") ELSE  "" .
                  
             END CASE.
             
             IF cTmpField = "ord-date" THEN cExcelVarValue = IF oe-ord.ord-date NE ? THEN DYNAMIC-FUNCTION("sfFormat_Date",oe-ord.ord-date) ELSE "" .
             ELSE IF cTmpField = "invrel-dt" THEN cExcelVarValue = IF v-dat NE ? THEN DYNAMIC-FUNCTION("sfFormat_Date",v-dat) ELSE "" .
