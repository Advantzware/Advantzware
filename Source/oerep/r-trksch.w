...
DEFINE VARIABLE v-qty              LIKE oe-rel.qty.
DEFINE VARIABLE v-ship-id          LIKE oe-rel.ship-id.
DEFINE VARIABLE v-ship-text        AS CHARACTER FORMAT "x(60)" NO-UNDO.
DEFINE VARIABLE v-del-zone         AS CHARACTER NO-UNDO.
DEFINE VARIABLE viRelNum           AS INTEGER   NO-UNDO.
DEFINE VARIABLE v-no-units         AS INTEGER   NO-UNDO.
DEFINE VARIABLE v-pallets          AS INTEGER   NO-UNDO.
DEFINE VARIABLE v-dec              AS DECIMAL   NO-UNDO.
DEFINE VARIABLE v-decu             AS DECIMAL   NO-UNDO.
DEFINE VARIABLE vcTruckCode        AS CHARACTER NO-UNDO.
DEFINE VARIABLE vcTruckDscr        AS CHARACTER NO-UNDO.
DEFINE VARIABLE viStop             AS INTEGER   NO-UNDO.
DEFINE VARIABLE vdShipDate         AS DATE      NO-UNDO.
DEFINE VARIABLE vcCity             AS CHARACTER NO-UNDO.
DEFINE VARIABLE vcState            AS CHARACTER NO-UNDO.
DEFINE VARIABLE vcZip              AS CHARACTER NO-UNDO.
DEFINE VARIABLE viBolNo            AS INTEGER   NO-UNDO.
DEFINE VARIABLE vcLoadNo           AS CHARACTER NO-UNDO.
DEFINE VARIABLE char-val           AS CHARACTER NO-UNDO.
DEFINE VARIABLE lv-unique-no       AS INTEGER   NO-UNDO.
DEFINE VARIABLE v-msf              AS DECIMAL   NO-UNDO.
DEFINE VARIABLE v-weight           AS DECIMAL   NO-UNDO.
DEFINE VARIABLE v-units-per-pallet AS INTEGER   NO-UNDO.

DEFINE TEMP-TABLE tt-report-bol NO-UNDO LIKE tt-report.

DEFINE BUFFER bf-oe-relh   FOR oe-relh.
DEFINE BUFFER bf-tt-report FOR tt-report.
...
end_ord-no begin_i-no end_i-no begin_date end_date begin_carr end_carr ~
tb_scheduled tb_actual tb_late tb_backordered tb_invoiceable tb_posted ~
begin_loc end_loc begin_delv end_delv tbAutoClose 
...
DEFINE FRAME FRAME-A
    begin_cust-no AT ROW 1.91 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Customer Number"
    end_cust-no AT ROW 1.91 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Customer Number"
    begin_ord-no AT ROW 2.95 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Order Number"
    end_ord-no AT ROW 2.95 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Order Number"
    begin_i-no AT ROW 3.95 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Order Number"
    end_i-no AT ROW 3.95 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Item Number"
    begin_date AT ROW 8.1 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Date"
    end_date AT ROW 8.1 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Date"
    begin_carr AT ROW 6 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Carrier Number"
    end_carr AT ROW 6 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Carrier Number"
    tb_scheduled AT ROW 10.67 COL 20.8
    tb_actual AT ROW 10.67 COL 45
    tb_late AT ROW 11.38 COL 20.8
    tb_backordered AT ROW 11.38 COL 45
    tb_invoiceable AT ROW 12.1 COL 20.8
    tb_posted AT ROW 10.67 COL 66
    btn-ok AT ROW 14.33 COL 31
    btn-cancel AT ROW 14.33 COL 51.2
    begin_loc AT ROW 4.95 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Warehouse"
    end_loc AT ROW 4.95 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Warehouse"
    begin_delv AT ROW 7.05 COL 29.2 COLON-ALIGNED HELP
    "Enter Beginning Delivery Zone"
    end_delv AT ROW 7.05 COL 75.4 COLON-ALIGNED HELP
    "Enter Ending Delivery Zone"
    tbAutoClose AT ROW 13.38 COL 31.4 WIDGET-ID 64
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.1 COL 4
    "Planned Releases" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 9.71 COL 20.8
    "Actual Releases" VIEW-AS TEXT
    SIZE 16 BY .62 AT ROW 9.71 COL 44.8
    "Posted Releases" VIEW-AS TEXT
    SIZE 17 BY .62 AT ROW 9.71 COL 65.8
    RECT-7 AT ROW 1.57 COL 3
    RECT-40 AT ROW 9.48 COL 3
...
ASSIGN 
    begin_delv:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    end_delv:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
PROCEDURE add-saved-recs :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE VARIABLE v-last-unique   AS INTEGER.
    DEFINE VARIABLE v-saved-pallets AS INTEGER.
    ...
    FOR EACH tt-report:
        FOR EACH truck-run-print WHERE truck-run-print.company EQ tt-report.company AND
            ...
            FIND FIRST bf-tt-report WHERE bf-tt-report.truck-print-key = truck-run-print.rec_key
                NO-LOCK NO-ERROR.
            IF AVAILABLE bf-tt-report THEN
                NEXT.
            ...
            ASSIGN 
                tt-report.carrier = truck-run-print.carrier
...
PROCEDURE assign-tt-report-fields :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE VARIABLE v-first-truck   AS ROWID.
    DEFINE VARIABLE v-last-unique   AS INTEGER.
    DEFINE VARIABLE v-truck-run-key AS CHARACTER.
    DEFINE VARIABLE v-saved-pallets AS INTEGER. 
    DEFINE BUFFER bf-tt-report FOR tt-report.
    v-first-truck = ?.
    v-truck-run-key = ?.
    ...
    END.
    /*OT1 release*/
    IF vcTruckCode EQ "" AND INDEX("AB",tt-report.key-06) GT 0 THEN 
    DO:
...
PROCEDURE enable_UI :
    ...
    ENABLE begin_cust-no end_cust-no begin_ord-no end_ord-no begin_i-no end_i-no 
        ...
        end_loc begin_delv end_delv RECT-7 RECT-40 tbAutoClose 
        WITH FRAME FRAME-A IN WINDOW C-Win.
...
PROCEDURE generate-data :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE VARIABLE v-cum-no-units AS INTEGER NO-UNDO.
    DEFINE VARIABLE v-cum-pallets  AS INTEGER NO-UNDO.
        ...
        DO:
            ...
            DO:
                ...
                IF v-units-per-pallet = 0 THEN
                    v-units-per-pallet =  IF AVAILABLE itemfg THEN itemfg.case-pal ELSE 0.
                IF v-units-per-pallet = 0 THEN
                    v-units-per-pallet = 1.
                IF oe-boll.qty-case NE 0 THEN
                    ASSIGN v-weight = oe-boll.weight
                        v-msf    = (oe-boll.qty * (IF AVAILABLE itemfg THEN itemfg.t-sqft ELSE 0) / 1000)
                ...
                ELSE
                    ASSIGN v-weight = 0
                        v-msf    = 0
                        v-dec    = 0.
                IF v-dec NE 0 AND v-units-per-pallet = 1 THEN 
                DO:            
        ...
        ELSE IF AVAILABLE oe-rell THEN
            DO:
                ...
                FOR EACH bf-oe-rell
                    ...
                    IF bf-oe-rell.qty-case NE 0 THEN
                        ASSIGN v-weight = ((IF AVAILABLE itemfg THEN itemfg.weight-100 ELSE 0) *  bf-oe-rell.qty / 100)
                            v-msf    = (bf-oe-rell.qty * (IF AVAILABLE itemfg THEN itemfg.t-sqft ELSE 0) / 1000)
                ...
                DO:
                   ...
                   */
                    IF oe-rell.qty-case NE 0 THEN
                        ASSIGN v-weight = ((IF AVAILABLE itemfg THEN itemfg.weight-100 ELSE 0) *  oe-rell.qty / 100)
                            v-msf    = (oe-rell.qty * (IF AVAILABLE itemfg THEN itemfg.t-sqft ELSE 0) / 1000)
            ...
            DO:
                ...
                IF oe-ordl.cas-cnt NE 0 THEN
                    ASSIGN v-weight = ((IF AVAILABLE itemfg THEN itemfg.weight-100 ELSE 0) *  v-qty / 100)
                        v-msf    = (v-qty * (IF AVAILABLE itemfg THEN itemfg.t-sqft ELSE 0) / 1000)
    ...
    DO:
        FOR EACH tt-report
            ...
            ASSIGN 
                v-cum-no-units = v-cum-no-units + tt-report.no-units
            ...
            DO:
                ...
                BUFFER-COPY tt-report EXCEPT no-units pallets TO tt-report-bol
                    ASSIGN 
                    tt-report-bol.no-units = v-cum-no-units
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE lv-no-units AS INTEGER   NO-UNDO.
    DEFINE VARIABLE v-dec       AS DECIMAL   NO-UNDO.
...
PROCEDURE tot-wgt-proc :
    ...
    FOR EACH tt-report WHERE
        ...
        FOR EACH bf-tt-report WHERE
            ...
            ASSIGN 
                tt-report.tot-msf    = tt-report.tot-msf + bf-tt-report.msf
