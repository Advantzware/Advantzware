...
DEFINE VARIABLE lv-attach-push        AS CHARACTER NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-8 RECT-9 RECT-7 rd_ack-ordmst ~
...
RS_whs-mths tb_sch-rel tb_inst tb_ship-to tb_billnotes tb_act-rel ~
tb_prt-revise tb_prt-bom tb_itempo tb_untcnt TG_print-due-cd tb_terms ~
TG_print-pen-notes tb_print-component tb_hide_sell tb_itm-tot tb_shpnot ~
spec-code lv-termFile dept-code rd-dest sel-attch TG_eml-push-att ~
TG_preview run_format tbAutoClose btn-ok btn-cancel 
...
tb_inst tb_ship-to tb_billnotes tb_act-rel tb_prt-revise tb_prt-bom ~
tb_itempo tb_untcnt TG_print-due-cd tb_terms TG_print-pen-notes ~
tb_print-component tb_hide_sell tb_itm-tot tb_shpnot spec-code lv-termFile ~
dept-code rd-dest sel-attch TG_eml-push-att TG_preview run_format ~
tbAutoClose 
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "OU4" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    DISPLAY rd_ack-ordmst begin_ord-no end_ord-no begin_cust-no end_cust-no 
        ...
        tb_billnotes tb_act-rel tb_prt-revise tb_prt-bom tb_itempo tb_untcnt 
        TG_print-due-cd tb_terms TG_print-pen-notes tb_print-component 
        tb_hide_sell tb_itm-tot tb_shpnot spec-code lv-termFile dept-code 
        rd-dest sel-attch TG_eml-push-att TG_preview run_format tbAutoClose 
        WITH FRAME FRAME-A IN WINDOW C-Win.
    ENABLE RECT-6 RECT-8 RECT-9 RECT-7 rd_ack-ordmst begin_ord-no end_ord-no 
        begin_cust-no end_cust-no begin_due-date end_due-date begin_relnum 
        end_relnum tb_reprint TG_cons_form TG_whs-mths RS_whs-mths tb_sch-rel 
        tb_inst tb_ship-to tb_billnotes tb_act-rel tb_prt-revise tb_prt-bom 
        tb_itempo tb_untcnt TG_print-due-cd tb_terms TG_print-pen-notes 
        tb_print-component tb_hide_sell tb_itm-tot tb_shpnot spec-code 
        lv-termFile dept-code rd-dest sel-attch TG_eml-push-att TG_preview 
...
PROCEDURE GenerateEmail :
    ...
    ASSIGN 
        lv-mailto      = IF ls-to-list NE "" THEN ('To:' + ls-to-list) ELSE ""
    ...
    CASE icPrintFormat:
        ...
        WHEN "ackhead 2" THEN 
            ASSIGN 
                ...
                lines-per-page = 65.
        WHEN "ackhead 10" OR 
        WHEN "ackhead 20" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackxprnt10.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Shamrock-Ack" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackshamrock.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "ackhead10-CAN" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackcan.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "Badger" THEN 
            ASSIGN 
        ...
        WHEN "PremierX" THEN 
            ASSIGN 
                ...
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "AckHead-Mex" THEN 
            ASSIGN 
                v-program      = "oe/rep/ackpremMex.p" 
                is-xprint-form = YES 
                lines-per-page = 65.
        WHEN "ACPI" THEN 
            ASSIGN 
