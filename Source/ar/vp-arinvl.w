...
DEFINE TEMP-TABLE tt-ar-invl NO-UNDO LIKE ar-invl.
...
&SCOPED-DEFINE ENABLED-OBJECTS Btn-View Btn-Save Btn-Add  ~
Btn-Delete  
...
DO:
  ...
  DO:
     FOR EACH bf-ar-invl NO-LOCK 
         WHERE bf-ar-invl.company EQ ar-inv.company
           AND bf-ar-invl.x-no EQ ar-inv.x-no
           AND bf-ar-invl.inv-no EQ ar-inv.inv-no
           AND bf-ar-invl.line NE 0
            BY bf-ar-invl.line:
         iL[1] = iL[1] + 1.
     END.

     RUN ar/d-arinvl.w (?, RECID(ar-inv), "add",OUTPUT lv-rowid).
...
DO:
    ...
    DO:
      IF ar-inv.posted EQ NO THEN DO:   
       EMPTY TEMP-TABLE tt-ar-invl.
      
       CREATE tt-ar-invl.
       BUFFER-COPY ar-invl TO tt-ar-invl.
      
       RUN ar/d-arinvl.w (RECID(ar-invl),RECID(ar-inv), "update", OUTPUT lv-rowid) . 
       
       BUFFER-COMPARE tt-ar-invl TO ar-invl SAVE RESULT IN ll.
...
DO:
  ...
  IF AVAILABLE ar-invl THEN
      RUN ar/d-arinvl.w (RECID(ar-invl), RECID(ar-inv), "view",OUTPUT lv-rowid). 
END.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}: 
    IF AVAIL ar-inv AND ar-inv.posted EQ YES THEN DO:
