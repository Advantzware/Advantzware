...
DEF BUFFER btt-inv FOR tt-inv.

DEF TEMP-TABLE tt-invl LIKE ar-invl.
...
&Scoped-define INTERNAL-TABLES tt-inv tt-invl

/* Definitions for BROWSE BROWSE-2                                      */
&Scoped-define FIELDS-IN-QUERY-BROWSE-2 tt-inv.inv-no tt-inv.inv-date tt-inv.net tt-inv.paid tt-inv.due tt-invl.bol-no tt-invl.actnum tt-invl.i-no tt-invl.part-no tt-invl.ord-no tt-invl.po-no tt-invl.est-no tt-invl.i-name   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-2 tt-inv.inv-no tt-inv.inv-date tt-inv.due ~
tt-invl.bol-no tt-invl.i-no tt-invl.part-no tt-invl.ord-no tt-invl.po-no ~
tt-invl.est-no tt-invl.i-name   
&Scoped-define ENABLED-TABLES-IN-QUERY-BROWSE-2 tt-inv tt-invl
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-BROWSE-2 tt-inv
&Scoped-define SECOND-ENABLED-TABLE-IN-QUERY-BROWSE-2 tt-invl
&Scoped-define SELF-NAME BROWSE-2
&Scoped-define QUERY-STRING-BROWSE-2 FOR  EACH tt-inv, ~
       EACH tt-invl OUTER-JOIN WHERE tt-invl.x-no = tt-inv.x-no
&Scoped-define OPEN-QUERY-BROWSE-2 OPEN QUERY {&SELF-NAME} FOR  EACH tt-inv, ~
       EACH tt-invl OUTER-JOIN WHERE tt-invl.x-no = tt-inv.x-no .
&Scoped-define TABLES-IN-QUERY-BROWSE-2 tt-inv tt-invl
&Scoped-define FIRST-TABLE-IN-QUERY-BROWSE-2 tt-inv
&Scoped-define SECOND-TABLE-IN-QUERY-BROWSE-2 tt-invl
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 129 BY 12.38
...
/* Query rebuild information for BROWSE BROWSE-2
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR  EACH tt-inv, EACH tt-invl OUTER-JOIN WHERE tt-invl.x-no = tt-inv.x-no .
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE BROWSE-2 */
&ANALYZE-RESUME
  ...
  IF {&browse-name}:NUM-SELECTED-ROWS GT 0 THEN DO:
    ...
    IF ll-ans OR (NOT ip-is-memo AND NOT ll-ans) THEN DO:
        ...
        FOR EACH btt-inv WHERE btt-inv.selekt,
            FIRST ar-inv WHERE ROWID(ar-inv) EQ btt-inv.row-id NO-LOCK
            BREAK BY ar-inv.due:

           ASSIGN
            op-cancel       = NO
            li-seq-no       = li-seq-no + 1
            btt-inv.seq-no   = li-seq-no
            btt-inv.amt-disc = 0.
           ...
           ASSIGN
            btt-inv.amt-paid = ar-inv.due
            ld-applied      = ld-applied + (btt-inv.amt-paid - btt-inv.amt-disc).
        END.   /* each tt-inv */
        ...
        FOR EACH btt-inv WHERE btt-inv.selekt,
           ...
           IF ld-applied LE ar-cash.check-amt THEN DO:
             ...
             v-amt-paid-inv-no = btt-inv.inv-no.
             LEAVE.
           END.
        END.
        ...
        FOR EACH btt-inv WHERE btt-inv.selekt:
           ...
           ASSIGN
            ar-cashl.inv-no = btt-inv.inv-no
            ar-cashl.inv-date = btt-inv.inv-date
            ar-cashl.amt-due = btt-inv.due
            ar-cashl.amt-paid = btt-inv.amt-paid
            ar-cashl.amt-disc = btt-inv.amt-disc.

           /*CM*/
           IF btt-inv.check-no >= 90000000 AND
              btt-inv.check-no <= 99999999 THEN
              ar-cashl.inv-no = v-amt-paid-inv-no.

           IF ip-is-memo OR
              (btt-inv.check-no >= 90000000 AND
              btt-inv.check-no <= 99999999) THEN
              ar-cashl.amt-paid = ar-cashl.amt-paid * -1.

           FIND b-cashl WHERE ROWID(b-cashl) EQ btt-inv.row-id NO-LOCK NO-ERROR.
...
/*on-account should be processed through AC5 (apply/reapply)*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE InitProc D-Dialog 
PROCEDURE InitProc :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  IF AVAIL TT-INV THEN
    ASSIGN 
       tt-inv.inv-no:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-inv.inv-date:read-only IN BROWSE {&browse-name} = YES
       tt-invl.bol-no:read-only IN BROWSE {&browse-name} = YES
       tt-inv.due:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-invl.i-no:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-invl.part-no:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-invl.ord-no:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-invl.po-no:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-invl.est-no:READ-ONLY IN BROWSE {&browse-name} = YES
       tt-invl.i-name:READ-ONLY IN BROWSE {&browse-name} = YES
      .
