/* ---------------------------------------------- ar/rep/invsthpkl.p   */
/* PRINT INVOICE   Xprint form for SOuth Pack logistics              */
/* -------------------------------------------------------------------------- */
...
ELSE lv-comp-color = "BLACK".
 ...
 END.
       ...
       for each ar-invl NO-LOCK where ar-invl.x-no  eq ar-inv.x-no  
        ...
        for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no
          ...
          assign v-case-line = ""
                 ...
                 v-i-no = ar-invl.i-no
                 v-i-dscr = IF NOT ar-invl.misc THEN ar-invl.part-dscr1 ELSE ar-invl.i-name
    ...
    PUT "<R58><C60><#8><FROM><R+6><C+20><RECT> " 
        ...
        "<=8><R+3> " v-bot-lab[2]
        "<=8><R+4> " v-bot-lab[3]
        "<=8><R+5> Grand Total:" ar-inv.net + v-t-tax[1] + v-t-tax[2] + v-t-tax[3] FORM "->>,>>9.99" .
