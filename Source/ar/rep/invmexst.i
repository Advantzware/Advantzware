...
ELSE IF company.company = '005' THEN
        PUT "<C1><#2>" /*<R+10><C+35><IMAGE#2=" ls-full-img2 SKIP  /* company image */ */
            ...
            SPACE(12) "Contacto: MCI PACKAGING GROUP LLC" SKIP
...
ELSE           
        PUT "<C1><#2>" /*<R+10><C+35><IMAGE#2=" ls-full-img2 SKIP  /* company image */ */
            ...
            "<FCourier New>"
            SPACE(4) "<P11><B>" cCompanyID FORMAT "x(30)" "</B><P10>" SKIP(1)
            SPACE(12) "Contacto: PREMIER PACKAGING" SKIP
...
PUT "<FArial><P12><=#3><R-2> <P10>" ip-copy-title FORM "x(20)" SKIP
    "<=#3>             Citar#                       " ar-inv.inv-no FORMAT ">>>>>>>9"
...
PUT "<FArial><=4> Fecha de envío    Flete a bordo           Embarcar vía                     Condiciones                           Vendedor                        BOL#" SKIP
...
PUT "<FArial><=5><C1.5>CUST PO#<C15>Cust Parte #<C62>P<C66>Precio" SKIP.
IF ip-print-s THEN
    PUT "<FArial><C1.5>Nuestra orden #<C18>Ít #<C35>Descripción<C53.5>INVOICED<C62>C<C66>(UOM)<C72.5>Cantidad" SKIP(1).
ELSE PUT "<FArial><C1.5>Nuestra orden #<C18>Ít #<C35>Descripción<C53.5>Enviado<C62>C<C66>(UOM)<C72.5>Cantidad" SKIP(1).
