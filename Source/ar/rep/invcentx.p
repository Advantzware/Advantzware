    ...
    for each report where report.term-id eq v-term-id no-lock,
     ...
     find FIRST carrier where carrier.company = ar-inv.company and
        ...
        for each ar-invl NO-LOCK where ar-invl.x-no  eq ar-inv.x-no  
           ...
           if ar-invl.sman[1] ne "" then do:
             create w-sman.
             assign w-sman.sman = ar-invl.sman[1].
             FIND FIRST sman WHERE sman.company = ar-invl.company
                            AND sman.sman = ar-invl.sman[1] NO-LOCK NO-ERROR.
           ...
           IF ar-invl.misc AND ar-invl.i-dscr <> "" THEN ln-cnt = ln-cnt + 1.
        ...
        for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no
          ...
          END.
            ...
            IF FIRST-OF(ar-invl.misc) THEN do: 
                IF ar-invl.misc = YES THEN do:
            ...
            PUT v-po-no
                ...
                STRING(ar-invl.ord-no) FORMAT "X(8)" SPACE(3)
            ...
            do v = 1 to 2:
              v-part-info = if v eq 1 then 
                                 IF ar-invl.misc AND ar-invl.i-dscr <> "" THEN ar-invl.i-dscr
                                 ELSE ar-invl.part-dscr1
                            else ar-invl.part-dscr2.
