...
/*                         ELSE IF ar-inv.freight <> 0 THEN ar-inv.freight */
    ...
    PUT "<R58><C54><#8><FROM><R+5><C+26><RECT> " 
        ...
        "<=8><R+2> Impuesto de venta  :" ar-inv.tax-amt FORM "->>,>>9.99"
        "<=8><R+3>" "" 
        "<=8><R+4> Total de la factura:" v-inv-total FORM "->>,>>9.99" . /* ar-inv.gross*/


    PUT "<FArial><R58><C1><#9><P12><B> Gracias. </B> <P9> " SKIP
            "<R60><C1><P12> Todas las monedas mostradas en " cCurCode FORMAT "x(3)" ". <P9> " SKIP
        "<=9><R-6>" v-notes[1]
        "<=9><R-5>" v-notes[2]
        "<=9><R-4>" v-notes[3]
        "<=9><R-3>" v-notes[4]
