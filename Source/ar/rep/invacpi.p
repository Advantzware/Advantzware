...
*/
          ...
          /*  assign v-line = v-line + 1
           ...
           if avail oe-ordl then
            ...
            assign v-inv-qty = ar-invl.inv-qty  /* task 04091404*/
        ...
        IF CAN-FIND(FIRST ar-invl NO-LOCK WHERE ar-invl.x-no  EQ ar-inv.x-no
                                            AND ar-invl.misc) THEN DO:
            PUT CHR(10)
        ...
        FOR EACH ar-invl NO-LOCK WHERE ar-invl.x-no  EQ ar-inv.x-no
           ...
           END.
          PUT 
              ar-invl.prep-charge FORMAT "x(15)"   AT 17 
              ar-invl.prep-dscr   FORMAT "x(30)"   AT 33
              ar-invl.prep-amt                     AT 83   
             SKIP.
           ASSIGN 
 ...
 */
       ...
       v-bot-lab[i] = if v-t-tax[i] ne 0 then
                    /*  ((if avail stax then string(stax.tax-dscr[i],"x(5)")
                       ...
                       string(v-t-tax[i],"->>>>>9.99")) else "".
                        ...
                        /*ar-inv.t-inv-freight*/.
    v-inv-total = /*
                   ar-inv.net + ar-inv.tax-amt + v-inv-freight
                   v-subtot-lines + v-t-tax[1] + v-t-tax[2] + v-t-tax[3]  + v-inv-freight
                  */
                  /* gdm - 10070908*/
                  v-subtot-lines + ar-inv.tax-amt  + v-inv-freight
                  .
    ...
    PUT "<R58><C58><#8><FROM><R+5><C+22><RECT> </B>" 
        ...
        "<=8><R+2> Sales Tax    :" ar-inv.tax-amt FORM "->>,>>9.99"
                  /*  v-bot-lab[2] */
        "<=8><R+3>" "" 
        "<=8><R+4><B> Total Invoice:" v-inv-total FORM "->>,>>9.99" "</B>". /* ar-inv.gross*/
