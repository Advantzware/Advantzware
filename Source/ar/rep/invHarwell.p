                     ...
                     FOR EACH notes WHERE
                         ...
                         IF notes.note_text <> "" THEN
                            DO i = 1 TO v-tmp-lines:
                               ...
                               PUT SKIP substring(NOTES.NOTE_TEXT,(1 + 80 * (i - 1)), 80) FORM "x(80)".              
                               v-printline = v-printline + 1.
                            END.
...
PUT "<R58><C1><#4><FROM><R61.5><C80><RECT><||3>"  /* Rectangle */
...
PUT 
    ...
    "<FArial><P8>"
    "<R59><C56>PAYEZ CE MONTANT"
    "<R60><C53.5>PLEASE PAY THIS AMOUNT"
    "<FCourierNew><P10>"
    "<R60><C29.2>" v-tot-sqft 
    "<R60><C41.2>" v-tot-wght
    "<R60><C71.2>" v-subtot-lines + v-t-tax[1] + v-t-tax[2] + v-t-tax[3] + v-inv-freight FORMAT "->>,>>9.99"
    "<R61><C71.2><FGCOLOR=RED>" lv-currency 
    "<FGCOLOR=BLACK>" 
    .
PUT      
    "<FArial><P6>"
