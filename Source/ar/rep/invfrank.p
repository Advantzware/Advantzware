/* ---------------------------------------------- ar/rep/invfrank.p  */
/* PRINT INVOICE   Xprint Standard Form  for Frankston           */
/* -------------------------------------------------------------------------- */
...
*/
  ...
  for each report where report.term-id eq v-term-id no-lock,
      ...
      FIND FIRST bf-ar-invl WHERE bf-ar-invl.x-no = ar-inv.x-no NO-LOCK NO-ERROR.
        IF AVAIL bf-ar-invl THEN
            lv-bol-no = bf-ar-invl.bol-no.
      ASSIGN v-line = 1
             v-printline = 0.
...
/*
        ...
        find first ar-invl where ar-invl.r-no = inv-head.r-no
                                  no-lock no-error.
            ...
            */
            ...
            IF v-printline > 50 THEN DO:
               PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") + 
                   " of [=@endPage" + string(ar-inv.inv-no) + "-@startPage" + string(ar-inv.inv-no) + "+1] "
                    FORM "x(120)" .
               PAGE.
               v-printline = 0.
               {ar/rep/invfrank.i}
...
*/         IF v-printline > 53 THEN DO:
              PUT "<R59><C65>Page " string(PAGE-NUM - lv-pg-num,">>9") +
                  " of [=@endPage" + string(ar-inv.inv-no) + "-@startPage" + string(ar-inv.inv-no) + "+1] " FORM "x(120)" .
              PAGE.
              v-printline = 0.
              {ar/rep/invfrank.i}
           END.
