...
FOR EACH est-flm NO-LOCK
    ...
    FIND CURRENT est-flm EXCLUSIVE-LOCK NO-ERROR. 
  ASSIGN
   est-flm.snum = {3}
   est-flm.bnum = {4}.
END.

FOR EACH xeb-form-ef NO-LOCK
    WHERE xeb-form-ef.company EQ {1}company
      AND xeb-form-ef.est-no  EQ {1}est-no
      AND xeb-form-ef.form-no EQ {1}form-no:
  FIND CURRENT xeb-form-ef EXCLUSIVE-LOCK NO-ERROR.           
  IF {4} NE 0 THEN DO:
...
FOR EACH est-inst NO-LOCK
  ...
  FIND CURRENT est-inst EXCLUSIVE-LOCK NO-ERROR.           
  est-inst.line = {3}.
END.
...
FOR EACH box-design-line NO-LOCK
  ...
  FIND CURRENT box-design-line EXCLUSIVE-LOCK NO-ERROR. 
  ASSIGN
...
FOR EACH box-design-hdr NO-LOCK
  ...
  FIND CURRENT box-design-hdr EXCLUSIVE-LOCK NO-ERROR. 
  ASSIGN
...
FOR EACH notes NO-LOCK
  ...
  FIND CURRENT notes EXCLUSIVE-LOCK NO-ERROR.         
...
FOR EACH ef-nsh NO-LOCK
  ...
  FIND CURRENT ef-nsh EXCLUSIVE-LOCK NO-ERROR.          
  ef-nsh.form-no = {3}.
END.
