...
ASSIGN 
    cTextListToSelect  = "MessageId,Table Name,Field Name,Frame Name,Auto Created,Review Status," +
                         "Show In Glossary,Usage Count,Updated By,Updated Date,Title,Help Contents"
                           
    cFieldListToSelect = "MSG-NUM,FIL-NAME,FLD-NAME,FRM-NAME,autoCreated,reviewStatus," +
                         "showInGlossary,usageCount,updatedBy,updatedDate,FRM-TITLE,help-txt" .
{sys/inc/ttRptSel.i}

ASSIGN 
    cTextListToDefault = "MessageId,Table Name,Field Name,Frame Name,Auto Created,Review Status," +
                         "Show In Glossary,Usage Count,Updated By,Updated Date,Title,Help Contents".
...
&Scoped-Define ENABLED-OBJECTS RECT-6 RECT-7 RECT-8 begin_message ~
end_message begin_table end_table begin_field end_field begin_frame ~
end_frame Btn_Def sl_avail sl_selected Btn_Add Btn_Remove btn_Up btn_down ~
fi_file tb_OpenCSV tbAutoClose btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS begin_message end_message begin_table ~
end_table begin_field end_field begin_frame end_frame sl_avail sl_selected ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getValue-help rd-utilexp 
FUNCTION getValue-help RETURNS CHARACTER
    ( BUFFER ipb-hlp-head FOR hlp-head, ipc-field AS CHARACTER )  FORWARD.
...
DEFINE VARIABLE begin_field AS CHARACTER FORMAT "x(20)" 
     LABEL "From Filed Name" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.

DEFINE VARIABLE begin_frame AS CHARACTER FORMAT "x(20)" 
     LABEL "From Frame Name" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.

DEFINE VARIABLE begin_message AS INTEGER FORMAT "->,>>>,>>9" INITIAL 0 
     LABEL "From Message#" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.

DEFINE VARIABLE begin_table AS CHARACTER FORMAT "x(20)" 
     LABEL "From Table Name" 
     VIEW-AS FILL-IN 
     SIZE 20 BY 1.

DEFINE VARIABLE end_field AS CHARACTER FORMAT "X(20)" INITIAL "zzzzzzzzzzzzzzz" 
     LABEL "To Field Name" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.

DEFINE VARIABLE end_frame AS CHARACTER FORMAT "X(20)" INITIAL "zzzzzzzzzzzzzzz" 
     LABEL "To Frame Name" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.

DEFINE VARIABLE end_message AS INTEGER FORMAT "->>>>>>9" INITIAL 99999999 
     LABEL "To Message#" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.

DEFINE VARIABLE end_table AS CHARACTER FORMAT "X(20)" INITIAL "zzzzzzzzzzzzzzz" 
     LABEL "To Table Name" 
     VIEW-AS FILL-IN 
     SIZE 21 BY 1.
...
DEFINE FRAME rd-utilexp
     begin_message AT ROW 2.71 COL 27 COLON-ALIGNED HELP
          "Enter Beginning Message" WIDGET-ID 142
     end_message AT ROW 2.71 COL 68 COLON-ALIGNED HELP
          "Enter Ending Name" WIDGET-ID 144
     begin_table AT ROW 3.67 COL 27 COLON-ALIGNED HELP
          "Enter Beginning Table Name" WIDGET-ID 150
     end_table AT ROW 3.67 COL 68 COLON-ALIGNED HELP
          "Enter Ending Table Name" WIDGET-ID 152
     begin_field AT ROW 4.62 COL 27 COLON-ALIGNED HELP
          "Enter Beginning Filed Name" WIDGET-ID 154
     end_field AT ROW 4.62 COL 68 COLON-ALIGNED HELP
          "Enter Ending Field Name" WIDGET-ID 156
     begin_frame AT ROW 5.57 COL 27 COLON-ALIGNED HELP
          "Enter Beginning Frame Name" WIDGET-ID 158
     end_frame AT ROW 5.57 COL 68 COLON-ALIGNED HELP
          "Enter Ending Frame Name" WIDGET-ID 160
     ...
     btn-ok AT ROW 19.95 COL 31.6 WIDGET-ID 14
     btn-cancel AT ROW 19.95 COL 51.6 WIDGET-ID 12
...
ASSIGN 
       begin_field:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       begin_frame:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       begin_message:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       begin_table:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       end_field:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       end_frame:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       end_message:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".

ASSIGN 
       end_table:PRIVATE-DATA IN FRAME rd-utilexp     = 
                "parm".
...
&Scoped-define SELF-NAME begin_field
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_field rd-utilexp
ON LEAVE OF begin_field IN FRAME rd-utilexp /* From Filed Name */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME begin_frame
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_frame rd-utilexp
ON LEAVE OF begin_frame IN FRAME rd-utilexp /* From Frame Name */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME begin_message
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_message rd-utilexp
ON LEAVE OF begin_message IN FRAME rd-utilexp /* From Message# */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME begin_table
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL begin_table rd-utilexp
ON LEAVE OF begin_table IN FRAME rd-utilexp /* From Table Name */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_field
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_field rd-utilexp
ON LEAVE OF end_field IN FRAME rd-utilexp /* To Field Name */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_frame
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_frame rd-utilexp
ON LEAVE OF end_frame IN FRAME rd-utilexp /* To Frame Name */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_message
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_message rd-utilexp
ON LEAVE OF end_message IN FRAME rd-utilexp /* To Message# */
DO:
        ASSIGN {&self-name}.
    END.
...
&Scoped-define SELF-NAME end_table
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL end_table rd-utilexp
ON LEAVE OF end_table IN FRAME rd-utilexp /* To Table Name */
DO:
        ASSIGN {&self-name}.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY begin_message end_message begin_table end_table begin_field end_field 
          begin_frame end_frame sl_avail sl_selected fi_file tb_OpenCSV 
          tbAutoClose 
      WITH FRAME rd-utilexp.
  ENABLE RECT-6 RECT-7 RECT-8 begin_message end_message begin_table end_table 
         begin_field end_field begin_frame end_frame Btn_Def sl_avail 
...
/*------------------------------------------------------------------------------
    ...
    DEFINE BUFFER b-hlp-head FOR hlp-head.
    ...
    FOR EACH b-hlp-head NO-LOCK
        WHERE  b-hlp-head.MSG-NUM GE  begin_message
        AND b-hlp-head.MSG-NUM LE  end_message
        AND b-hlp-head.FIL-NAME GE begin_table
        AND b-hlp-head.FIL-NAME LE end_table
        AND b-hlp-head.FLD-NAME GE begin_field
        AND b-hlp-head.FLD-NAME LE end_field
        AND b-hlp-head.FRM-NAME GE begin_frame
        AND b-hlp-head.FRM-NAME LE end_frame :   
        
        v-excel-detail-lines = "".

        FOR EACH ttRptSelected:
            v-excel-detail-lines = v-excel-detail-lines + 
                appendXLLine(getValue-help(BUFFER b-hlp-head,ttRptSelected.FieldList)).
        END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getValue-help rd-utilexp 
FUNCTION getValue-help RETURNS CHARACTER
    ( BUFFER ipb-hlp-head FOR hlp-head, ipc-field AS CHARACTER ) :
    /*------------------------------------------------------------------------------
    ...
    CASE ipc-field :
        ...
        DO:
            ...
            END.
            h-field = BUFFER ipb-hlp-head:BUFFER-FIELD(ipc-field).
