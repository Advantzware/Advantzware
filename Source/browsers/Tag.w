...
/*------------------------------------------------------------------------
  ...
  Description: Browse for Tag Viewer dialog screen
...
&Scoped-define QUERY-STRING-br_table FOR EACH tag WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH tag WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table tag
&Scoped-define FIRST-TABLE-IN-QUERY-br_table tag
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN NO-COLUMN-SCROLLING SEPARATORS SIZE 128 BY 14.52.
...
"description" ? "X(95)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ReopenQuery B-table-Win 
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "tag"}
