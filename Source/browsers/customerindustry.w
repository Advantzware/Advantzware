...
DEFINE VARIABLE lv-sort-by              AS CHAR INIT "ID" NO-UNDO.
DEFINE VARIABLE lv-sort-by-lab          AS CHAR INIT "AR Industry ID" NO-UNDO.
...
&SCOPED-DEFINE sortby-phrase-desc BY ({&sortby-log}) DESC ~
        {&sortby}
...
&Scoped-define INTERNAL-TABLES customerindustry
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH customerindustry WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY Browser-Table FOR EACH customerindustry WHERE ~{&KEY-PHRASE} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-Browser-Table customerindustry
&Scoped-define FIRST-TABLE-IN-QUERY-Browser-Table customerindustry
...
/*------------------------------------------------------------------------------
    ...
    APPLY 'ENTRY':U TO fi_industryId IN FRAME {&FRAME-NAME}.
...
/*------------------------------------------------------------------------------
    ...
    /* Code placed here will execute AFTER standard behavior.    */
    IF AVAILABLE customerindustry THEN APPLY "value-changed" TO BROWSE {&browse-name}.
...
/*------------------------------------------------------------------------------
    ...
    IF ROWID(customerindustry) EQ lvLastRowID THEN
        opNavType = 'L'.
      
    IF ROWID(customerindustry) EQ lvFirstRowID THEN
