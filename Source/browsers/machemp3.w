...
&Scoped-define FIELDS-IN-QUERY-Browser-Table machemp.employee Employee-Name(machtran.company,machemp.employee) @ employee-name machtran.machine machemp.start_date STRING(machemp.start_time,'HH:MM am') @ start-time machemp.end_date machemp.shift Time_String(machemp.end_time,yes) @ end-time machemp.ratetype machemp.posted Time_String(machemp.total_time,no) @ total-time   
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH machemp WHERE machemp.employee = employee.employee       AND machemp.posted = YES USE-INDEX postemp NO-LOCK, ~
             EACH machtran WHERE TRUE /* Join to machemp incomplete */       AND machtran.rec_key eq machemp.table_rec_key  AND machtran.company = gcompany NO-LOCK    ~{&SORTBY-PHRASE} /*     BY machemp.employee      */ /*      BY machemp.start_date   */ /*       BY machemp.start_time. */
&Scoped-define OPEN-QUERY-Browser-Table OPEN QUERY {&SELF-NAME} FOR EACH machemp WHERE machemp.employee = employee.employee       AND machemp.posted = YES USE-INDEX postemp NO-LOCK, ~
             EACH machtran WHERE TRUE /* Join to machemp incomplete */       AND machtran.rec_key eq machemp.table_rec_key  AND machtran.company = gcompany NO-LOCK    ~{&SORTBY-PHRASE} /*     BY machemp.employee      */ /*      BY machemp.start_date   */ /*       BY machemp.start_time. */.
...
DEFINE FRAME F-Main
     ...
     fi_sortby AT ROW 19.33 COL 36.2 COLON-ALIGNED NO-LABEL WIDGET-ID 2
...
/* Query rebuild information for BROWSE Browser-Table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH machemp WHERE machemp.employee = employee.employee
      AND machemp.posted = YES USE-INDEX postemp NO-LOCK,
...
DEF VAR ToEmpId AS CHAR INIT "" NO-UNDO.
...
IF AVAIL machemp AND machemp.employee NE "" THEN
    ...
    RUN addon/browsers/rd-mh3exp.w (FromEmpId,ToEmpId). 
...
------------------------------------------------------------------------------*/
  ...
  /*IF AVAIL employee THEN lv-rowid = ROWID(employee).
  RUN get-link-handle in adm-broker-hdl(THIS-PROCEDURE, "record-source", OUTPUT char-hdl).
  IF VALID-HANDLE(WIDGET-HANDLE(char-hdl)) THEN
    RUN get-rowid IN WIDGET-HANDLE(char-hdl) (OUTPUT lv-rowid).
  FIND employee WHERE ROWID(employee) EQ lv-rowid NO-LOCK NO-ERROR. */
