...
attach.est-no attach.i-no attach.creat-date 
...
(v-i-no <> "" and lookup(attach.i-no,v-i-no) > 0) and attach.i-no gt "") NO-LOCK ~
    ~{&SORTBY-PHRASE}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _STRUCTURED
  QUERY Browser-Table NO-LOCK DISPLAY
      ...
      attach.est-no FORMAT "x(8)":U
      attach.i-no COLUMN-LABEL "FG Item#" FORMAT "x(15)":U
...
/* Query rebuild information for BROWSE Browser-Table
 ...
 (LOOKUP(attach.rec_key,v-rec-key-list) gt 0 and
(v-est-no <> """" and trim(attach.est-no) = trim(v-est-no)) or 
(v-i-no <> """" and lookup(attach.i-no,v-i-no) > 0) and attach.i-no gt """")"
     _FldNameList[1]   = asi.attach.attach-file
     _FldNameList[2]   > asi.attach.est-no
"attach.est-no" ? "x(8)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > asi.attach.i-no
"attach.i-no" "FG Item#" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = asi.attach.creat-date
