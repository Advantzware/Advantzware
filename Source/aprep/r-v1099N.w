...
ASSIGN 
    cTextListToSelect  = "Vendor #,Tax ID#,Name,Address 1,Address 2,City,State,Zip,Amount," +
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Vendor #,Tax ID#,Name,Address 1,Address 2,City,State,Zip,Amount"   .
...
begin_date end_date tb_zero-ven sl_avail Btn_Def sl_selected Btn_Add ~
...
end_date tb_zero-ven sl_avail sl_selected rd-dest fi_file tb_OpenCSV ~
tbAutoClose 
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\Vendor1099.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    begin_vend AT ROW 2.67 COL 29.4 COLON-ALIGNED HELP
    "Enter Beginning Vendor Number"
    end_vend AT ROW 2.67 COL 68.4 COLON-ALIGNED HELP
    "Enter Ending Vendor Number"
    rs-date AT ROW 4 COL 31.2 NO-LABELS
    begin_date AT ROW 5.33 COL 29.4 COLON-ALIGNED HELP
    "Enter Beginning Date"
    end_date AT ROW 5.33 COL 68.4 COLON-ALIGNED HELP
    "Enter Ending Date"
    tb_zero-ven AT ROW 6.67 COL 31.6
    sl_avail AT ROW 9.1 COL 3 NO-LABELS WIDGET-ID 26
    Btn_Def AT ROW 9.1 COL 40.2 HELP
    "Add Selected Table to Tables to Audit" WIDGET-ID 56
    sl_selected AT ROW 9.1 COL 60.6 NO-LABELS WIDGET-ID 28
    ...
    lv-font-no AT ROW 14.86 COL 36 COLON-ALIGNED
    lv-ornt AT ROW 14.86 COL 45 NO-LABELS
    lines-per-page AT ROW 14.91 COL 87 COLON-ALIGNED
    rd-dest AT ROW 15.05 COL 5 NO-LABELS
    lv-font-name AT ROW 15.52 COL 27 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 16.33 COL 92 RIGHT-ALIGNED
    td-show-parm AT ROW 16.48 COL 28
    fi_file AT ROW 17.33 COL 26 COLON-ALIGNED HELP
    "Enter File Name"
    ...
    SIZE 29 BY .62 AT ROW 8.38 COL 3 WIDGET-ID 38
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 14.33 COL 4
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.24 COL 4
    "Selected Columns(In Display Order)" VIEW-AS TEXT
    SIZE 34 BY .62 AT ROW 8.38 COL 60.4 WIDGET-ID 44
    RECT-6 AT ROW 14.76 COL 3
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "VR10" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_vend end_vend rs-date begin_date end_date tb_zero-ven sl_avail 
    ...
    ENABLE RECT-6 RECT-7 begin_vend end_vend rs-date begin_date end_date 
        tb_zero-ven sl_avail Btn_Def sl_selected Btn_Add Btn_Remove btn_Up 
...
PROCEDURE run-report :
    ...
    DEFINE VARIABLE svend          LIKE vend.vend-no.
    DEFINE VARIABLE evend          LIKE vend.vend-no INIT "zzzzzzzz".
    DEFINE VARIABLE sdate          LIKE ap-inv.inv-date.
    DEFINE VARIABLE edate          LIKE ap-inv.inv-date.
    DEFINE VARIABLE v-zero         AS LOG       INITIAL NO NO-UNDO.

    DEFINE VARIABLE v-vend-tot     AS DECIMAL   FORMAT "->>,>>>,>>9.99" NO-UNDO.
    DEFINE VARIABLE v-grand-tot    AS DECIMAL   FORMAT "->>,>>>,>>9.99" NO-UNDO.
        ...
        FOR EACH vend FIELDS(company vend-no code-1099 NAME tax-id add1
            ...
            DO:
                ...
                DO:
                    ...
                    CASE vend.code-1099:
                        WHEN "Y" THEN 
                            cBoxDscr = "Box 3 Other Income".
                        OTHERWISE 
                        DO i = 1 TO NUM-ENTRIES(c1099Code) :
                            IF ENTRY(i,c1099Code) EQ vend.code-1099  THEN 
                            DO:
                    ...
                    DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                        ...
                        CASE cTmpField:             
                            WHEN "vend"    THEN 
                                cVarValue = STRING(vend.vend-no,"x(10)") .
                            WHEN "tax"   THEN 
                                cVarValue = STRING(vend.tax-id,"x(20)").
                            WHEN "name"   THEN 
                            ...
                            WHEN "city"  THEN 
                                cVarValue = STRING(vend.city,"x(15)") .
                            WHEN "stat"   THEN 
                                cVarValue = STRING(vend.state,"x(5)") .
                            WHEN "zip"  THEN 
                                cVarValue = STRING(vend.zip,"xxxxx-xxxx") .
                            WHEN "amt"  THEN 
                                cVarValue = STRING(v-vend-tot,"->>>,>>>,>>9.99") .
                            WHEN "box"  THEN 
                                cVarValue = STRING(vend.code-1099,"x") .
                            WHEN "box-dscr"  THEN 
                                cVarValue = STRING(cBoxDscr,"x(35)") .
            ...
            DO:
                ...
                DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                    ...
                    CASE cTmpField:             
                        ...
                        WHEN "zip"  THEN 
                            cVarValue = "" .
                        WHEN "amt"  THEN 
                            cVarValue = STRING(v-grand-tot,"->>>,>>>,>>9.99") .
                        WHEN "box"  THEN 
                            cVarValue = "" .
                        WHEN "box-dscr"  THEN 
                            cVarValue = "" .
                ...
                PUT str-line SKIP.
                PUT UNFORMATTED 
                    "  ======= GRAND TOTAL:" SUBSTRING(cDisplay,23,200) SKIP.
                IF tb_excel THEN 
                DO:
                    PUT STREAM excel UNFORMATTED 
                        ' GRAND TOTAL  ,'  
                        SUBSTRING(cExcelDisplay,4,200) SKIP.
                END.
            END.
        END.
    END.
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\Vendor1099.csv".    
    END.
