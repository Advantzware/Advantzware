...
ASSIGN 
    cTextListToSelect  = "Inv#,Account,Vendor,Name,Description,Date,Amount,Line Amount,Currency Code"
...
{sys/inc/ttRptSel.i}
ASSIGN 
    cTextListToDefault = "Inv#,Account,Vendor,Name,Description,Date,Amount,Line Amount".
...
end_date tb_by-invdt begin_account end_account sl_avail Btn_Def sl_selected ~
...
tb_by-invdt begin_account end_account sl_avail sl_selected rd-dest fi_file ~
...
DEFINE VARIABLE fi_file        AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\APInvoicesByVendor.csv" 
    LABEL "Name" 
...
DEFINE VARIABLE tb_by-invdt  AS LOGICAL   INITIAL NO 
...
DEFINE FRAME FRAME-A
    begin_vend AT ROW 2.38 COL 24 COLON-ALIGNED HELP
    "Enter Beginning Vendor Number"
    end_vend AT ROW 2.38 COL 66 COLON-ALIGNED HELP
    "Enter Ending Vendor Number"
    begin_date AT ROW 3.81 COL 24 COLON-ALIGNED HELP
    "Enter Beginning AP Invoice Date"
    end_date AT ROW 3.81 COL 66 COLON-ALIGNED HELP
    "Enter Ending AP Invoice Date"
    tb_by-invdt AT ROW 4.86 COL 26.2
    begin_account AT ROW 6.1 COL 24.2 COLON-ALIGNED HELP
    "Enter Beginning General Ledger Account Number" WIDGET-ID 4
    end_account AT ROW 6.1 COL 66 COLON-ALIGNED HELP
    ...
    sl_selected AT ROW 8.38 COL 60.8 NO-LABELS WIDGET-ID 28
    ...
    lv-font-name AT ROW 15.29 COL 29 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 16.48 COL 92 RIGHT-ALIGNED
    td-show-parm AT ROW 16.67 COL 28.4
    fi_file AT ROW 17.57 COL 26.2 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 17.67 COL 92.4 RIGHT-ALIGNED
    tbAutoClose AT ROW 18.86 COL 28.4 WIDGET-ID 78
    btn-ok AT ROW 19.81 COL 28.2
    btn-cancel AT ROW 19.81 COL 55
...
&Scoped-define SELF-NAME tb_by-invdt
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tb_by-invdt C-Win
ON VALUE-CHANGED OF tb_by-invdt IN FRAME FRAME-A /* Run By Post Date? */
    DO:
        ASSIGN {&self-name}.
    END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "VR12" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    ------------------------------------------------------------------------------*/
    DISPLAY begin_vend end_vend begin_date end_date tb_by-invdt begin_account 
        end_account sl_avail sl_selected rd-dest fi_file tb_OpenCSV 
        tbAutoClose 
        WITH FRAME FRAME-A IN WINDOW C-Win.
    ENABLE RECT-6 RECT-7 begin_vend end_vend begin_date end_date tb_by-invdt 
        begin_account end_account sl_avail Btn_Def sl_selected Btn_Add 
    ...
    FOR EACH tt-ap-inv NO-LOCK,
        ...
        DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
            ...
            CASE cTmpField:             
                ...
                WHEN "curr-code"                  THEN 
                    cVarValue = STRING(tt-ap-inv.curr-code[1]).
        ...
        IF LAST-OF(tt-ap-inv.vend-no) THEN 
        DO:
            ...
            DO i = 1 TO NUM-ENTRIES(cSelectedlist):                             
                ...
                CASE cTmpField:             
                    ...
                    WHEN "date"                   THEN 
                        cVarValue = "".
                    WHEN "amt"              THEN 
                        cVarValue =  STRING(lv-gtotal[1],"->>>,>>>,>>>,>>9.99") .
                    WHEN "line-amt"           THEN 
                        cVarValue = STRING(lv-total[1],"->>>,>>>,>>>,>>9.99") .
                    WHEN "curr-code"                  THEN 
                        cVarValue = "".
        ...
        IF LAST(tt-ap-inv.vend-no) /*AND ll-total[2]*/ THEN 
        DO:
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\APInvoicesByVendor.csv".    
    END.
