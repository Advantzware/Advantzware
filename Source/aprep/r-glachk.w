...
DEF TEMP-TABLE tt-report    LIKE report
    ...
    FIELD act-no            AS CHAR
    FIELD act-amt           AS DECIMAL 
    FIELD chk-label AS CHAR INIT "   Check Total" .
...
DEF VAR v-amt-acount LIKE ap-payl.amt-paid NO-UNDO.
DEF VAR v-sub-disc  LIKE ap-payl.amt-disc NO-UNDO.
DEF VAR v-sub-paid LIKE ap-payl.amt-paid NO-UNDO.
DEF VAR v-sub-amt LIKE ap-payl.amt-paid NO-UNDO.
DEF VAR v-sub-acount LIKE ap-payl.amt-paid NO-UNDO.
      ...
      IF vcount = 1 THEN do:
        ASSIGN
            ...
            tt-report.act-amt      = tt-report.act-amt + ap-payl.amt-paid * 
                        (ap-invl.amt / (ap-inv.net + ap-inv.freight)).
        IF FIRST-OF(ap-invl.inv-no) THEN do:
            ASSIGN
...
FOR EACH tt-report NO-LOCK WITH FRAME ap-chk
  ...
  IF FIRST-OF(tt-report.key-02) THEN do:
      ASSIGN v-sub-acount = tt-report.act-amt
  ...
  DISPLAY tt-report.check-no   WHEN FIRST-OF(tt-report.key-03)
          ...
          tt-report.vend-name  WHEN FIRST-OF(tt-report.key-03)
          tt-report.inv-no     WHEN FIRST-OF(tt-report.key-03)
          /*tt-report.due-date   WHEN FIRST-OF(tt-report.key-03)*/
          tt-report.gross-amt  WHEN FIRST-OF(tt-report.key-03)
          tt-report.amt-disc   WHEN FIRST-OF(tt-report.key-03)
          tt-report.amt-paid   WHEN FIRST-OF(tt-report.key-03)
          tt-report.act-no
          tt-report.act-amt   
  ...
  IF last-of(tt-report.key-02) AND NOT FIRST-OF(tt-report.key-02) THEN DO:
