...
DEFINE TEMP-TABLE tt-report NO-UNDO LIKE report
    ...
    FIELD check-no  AS CHARACTER FORMAT "x(13)"
    FIELD vend-no   AS CHARACTER FORMAT "X(30)"
    ...
    FIELD misc-chk  AS CHARACTER FORMAT "X(10)".
...
tb_dep tb_jrn rd_rec tb_clr tb_unclr tb_vend-sort rd-dest tbAutoClose ~
btn-ok btn-cancel 
&Scoped-Define DISPLAYED-OBJECTS bank_code begin_vend end_vend ~
begin_chk-date end_chk-date begin_chk-no end_chk-no begin_j-no end_j-no ~
tb_dep tb_jrn lbl_rec rd_rec tb_clr tb_unclr tb_vend-sort rd-dest ~
tbAutoClose 
...
DEFINE FRAME FRAME-A
    ...
    begin_vend AT ROW 3.62 COL 28 COLON-ALIGNED HELP
    "Enter Beginning Vendor"
    end_vend AT ROW 3.62 COL 71 COLON-ALIGNED HELP
    "Enter Ending Vendor"
    begin_chk-date AT ROW 4.81 COL 28 COLON-ALIGNED
    end_chk-date AT ROW 4.81 COL 71 COLON-ALIGNED HELP
    "Enter Ending Check Date"
    begin_chk-no AT ROW 6 COL 28 COLON-ALIGNED HELP
    "Enter Beginning Check Number"
    end_chk-no AT ROW 6 COL 71 COLON-ALIGNED HELP
    "Enter Ending Check Number"
    begin_j-no AT ROW 7.19 COL 28 COLON-ALIGNED HELP
    "Enter Beginning Journal Number"
    end_j-no AT ROW 7.19 COL 71 COLON-ALIGNED HELP
    "Enter Ending Customer Number"
    tb_dep AT ROW 8.62 COL 34
    tb_jrn AT ROW 9.81 COL 34
    lbl_rec AT ROW 11 COL 20 COLON-ALIGNED NO-LABELS
    rd_rec AT ROW 11 COL 29 NO-LABELS
    ...
    lv-ornt AT ROW 16 COL 43.6 NO-LABELS
    lines-per-page AT ROW 16 COL 87.4 COLON-ALIGNED
    rd-dest AT ROW 16.14 COL 5 NO-LABELS
    lv-font-name AT ROW 17.14 COL 29 COLON-ALIGNED NO-LABELS
    td-show-parm AT ROW 19.52 COL 30.8
    tbAutoClose AT ROW 21.1 COL 31.2 WIDGET-ID 64
    btn-ok AT ROW 22.05 COL 31
    btn-cancel AT ROW 22.05 COL 52
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.05 COL 4
    " Output Destination" VIEW-AS TEXT
    SIZE 19 BY .62 AT ROW 15.29 COL 4
...
/* SETTINGS FOR FRAME FRAME-A
   FRAME-NAME                                                           */
ASSIGN 
    bank_code:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    begin_chk-date:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    begin_chk-no:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
ASSIGN 
    end_chk-date:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    end_chk-no:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
/* SETTINGS FOR FILL-IN lbl_rec IN FRAME FRAME-A
   NO-ENABLE                                                            */
ASSIGN 
    lbl_rec:PRIVATE-DATA IN FRAME FRAME-A = "rd_rec".
...
ASSIGN 
    rd_rec:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_clr:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_dep:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_jrn:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_unclr:PRIVATE-DATA IN FRAME FRAME-A = "parm".

ASSIGN 
    tb_vend-sort:PRIVATE-DATA IN FRAME FRAME-A = "parm".
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "VT3" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    DISPLAY bank_code begin_vend end_vend begin_chk-date end_chk-date begin_chk-no 
        ...
        tb_unclr tb_vend-sort rd-dest tbAutoClose 
    ...
    ENABLE RECT-6 RECT-7 bank_code begin_vend end_vend begin_chk-date 
        ...
        rd_rec tb_clr tb_unclr tb_vend-sort rd-dest tbAutoClose btn-ok 
        btn-cancel 
        WITH FRAME FRAME-A IN WINDOW C-Win.
...
PROCEDURE run-report :
    ...
    DEFINE BUFFER ar-mcash-ref FOR reftable.
    DEFINE BUFFER b-ap-pay     FOR ap-pay.

    DEFINE VARIABLE v-chkno     LIKE ap-chk.check-no FORMAT ">>>>>9" INIT 0 NO-UNDO.
    DEFINE VARIABLE v-chkno2    LIKE v-chkno INIT 999999 NO-UNDO.
    DEFINE VARIABLE v-s-date    LIKE ap-chk.check-date FORMAT "99/99/9999" INIT "01/01/0001" NO-UNDO.
    DEFINE VARIABLE v-e-date    LIKE v-s-date INIT TODAY NO-UNDO.
    DEFINE VARIABLE v-bank-code LIKE bank.bank-code NO-UNDO.
    DEFINE VARIABLE v-rep-type  AS ch      FORMAT "x" NO-UNDO INIT "U".
    DEFINE VARIABLE v-head-1    AS ch      FORMAT 'x(20)' NO-UNDO.
    DEFINE VARIABLE v-bank-act  LIKE bank.actnum NO-UNDO.
    DEFINE VARIABLE v-bank-hld  LIKE bank.actnum NO-UNDO.
    DEFINE VARIABLE v-tot-amt   AS DECIMAL INIT 0 FORMAT "->,>>>,>>9.99" NO-UNDO.
    DEFINE VARIABLE save_id     AS RECID   NO-UNDO.
    DEFINE VARIABLE time_stamp  AS ch      NO-UNDO.
    DEFINE VARIABLE v-amt       AS DECIMAL NO-UNDO.
    DEFINE VARIABLE ll-skipped  AS LOG     NO-UNDO.
    ...
    FOR EACH ap-pay
        ...
        IF AVAILABLE ap-ledger               AND
            ap-ledger.tr-date GE v-s-date AND
            ap-ledger.tr-date LE v-e-date THEN 
        DO:
            CREATE tt-report.
        ...
        IF ap-pay.reconciled EQ ? AND NOT ll-skipped THEN 
        DO:
            FIND FIRST ap-ledger NO-LOCK
                WHERE ap-ledger.company EQ ap-pay.company
                AND ap-ledger.vend-no EQ ap-pay.vend-no
    ...
    FOR EACH tt-report /*,
      FIRST ap-pay WHERE RECID(ap-pay) EQ tt-report.rec-id NO-LOCK*/
        ...
        IF v-bank-hld NE tt-report.check-act THEN 
        DO:
