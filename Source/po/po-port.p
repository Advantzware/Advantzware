/* --------------------------------------------- po/po-port.p */
/* Purchase Order Print Program for S-8-POPRINT = Portugese                        */
/* -------------------------------------------------------------------------- */
...
/*      if po-ord.type eq "D" then  */
      ...
      if po-ord.stat eq "U" then
        v-change-ord = "(ALTERAR APENAS ORDEM)".
      ...
      if po-ord.frt-pay eq "P" then
        assign v-freight-dscr = "Pré-pago".
      else if po-ord.frt-pay eq "C" then
        assign v-freight-dscr = "Coletar".
      else
        assign v-freight-dscr = "Conta".
/*
...
{po/po-port.i}
      ...
      for each po-ordl 
       ...
       :
        ...
        if po-ordl.stat eq "A" THEN ASSIGN v-change-dscr = "Adicionada".
        else if po-ordl.stat eq "U" THEN assign v-change-dscr = "Atualizada".
        else if po-ordl.stat eq "O" THEN assign v-change-dscr = "Abrir".
        else if po-ordl.stat eq "P" then assign v-change-dscr = "Parcial".
        else if po-ordl.stat eq "C" then assign v-change-dscr = "Fechada".

        if po-ordl.deleted eq yes then   assign v-change-dscr = "Excluída".
...
***/
      ...
      /*  IF TRIM(v-lstloc) NE "" THEN DO:      /*Task# 10041302*/
        ...
        IF po-ordl.ord-no NE 0 THEN DO:
            put "<C15>N° da ordem:" string(po-ordl.ord-no,">>>>>>>9").
            PUT "<C35>PO da Cliente:" v-cut-po           /*Task# 10041302*/
                skip.
            lv-add-line = NO.
            v-line-number = v-line-number + 1.
            v-printline = v-printline + 1.
       ...
       v-tot-sqft = v-tot-sqft + (v-qty * 1000).
        ...
        IF AVAIL ITEM THEN ASSIGN lv-flute = "  flauta: " + ITEM.flute
                                  lv-reg-no = "Teste: " + ITEM.reg-no.
...
/*                      /*vend.tax-gr + "        :       " */ + STRING(po-ord.tax,">,>>>,>>9.99"). */
    ...
    "<=8><R+1> Subtotal  :" po-ord.t-cost - po-ord.tax FORM ">,>>>,>>9.99"
    "<=8><R+2> "  v-bot-lab[1] 
    "<=8><R+3> "  " " /*PST        :" inv-head.t-inv-tax FORM "->>,>>9.99"*/
                /*v-bot-lab[2] */
    "<=8><R+4> Total geral:" po-ord.t-cost FORM ">,>>>,>>9.99" .
/*Please acknowledge receipt and that pricing is correct on this P.O." */ 
IF v-print-terms THEN
PUT "<FArial><R58><C1><P12><B> Termos e Condições </B> <P9> " SKIP
     "Confirme este pedido verificando preço, condições de frete, quantidade e data de entrega." SKIP  /*Task# 10041302*/
     "As faturas serão pagas apenas para este pedido de compra. " SKIP
     " " SKIP(1)     
     "" SKIP  
