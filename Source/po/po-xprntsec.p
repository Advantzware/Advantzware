...
DEFINE VARIABLE cGrandTotSF AS DECIMAL NO-UNDO .
...
{po/po-xprntsec.i}
      ...
      for each po-ordl WHERE
       ...
       /* v-job-no = po-ordl.job-no + "-" + STRING(po-ordl.job-no2,">>").*/
        ...
        cGrandTotSF = cGrandTotSF + (v-wid * v-len * po-ordl.ord-qty).
        lPrintMsf = NO .
...
FOR EACH notes WHERE notes.rec_key = po-ord.rec_key NO-LOCK:
  ...
  IF lPrintGrandTotMsf THEN
      ...
      END.        
      PUT "Grand Total SF: " +
              TRIM(STRING(cGrandTotSF,"->>,>>>,>>9.9<<")) AT 50 FORMAT "x(30)"
              SKIP.
