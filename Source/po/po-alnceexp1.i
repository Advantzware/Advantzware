/* -------------------------------------------------- po/po-ccexp.p */
/*                                                                  */
/* AlliFlutes export PO (copy of hrms)                              */
...
IF v-rec-found THEN
    AlliFlutes-log = LOGICAL(v-rtn-char) NO-ERROR.

RUN sys/ref/nk1look.p (cocode, "AlliFlutes1", "DS", NO, NO, "", "", 
    OUTPUT v-rtn-char, OUTPUT v-rec-found).
    
IF v-rec-found THEN
    AlliFlutes-dir = v-rtn-char NO-ERROR.
    
RUN sys/ref/nk1look.p (cocode, "AlliFlutes1", "I", NO, NO, "", "", 
    OUTPUT v-rtn-char, OUTPUT v-rec-found).
...
IF AVAIL cust AND AlliFlutes-log AND AlliFlutes-dir NE "" THEN
print-po-blok:
FOR EACH report WHERE report.term-id EQ v-term-id NO-LOCK,
  ...
  IF OPSYS EQ "UNIX" AND substr(AlliFlutes-dir,1,1) NE v-slash THEN
    AlliFlutes-dir = v-slash + AlliFlutes-dir.

  IF substr(AlliFlutes-dir,LENGTH(AlliFlutes-dir),1) EQ v-slash THEN
    substr(AlliFlutes-dir,LENGTH(AlliFlutes-dir),1) = "".
  ...
  IF SEARCH(v-outfile[2]) NE ? THEN DO:
    ...
    MESSAGE "AlliFlutes file:" TRIM(v-outfile[3]) "has been created" 
            VIEW-AS ALERT-BOX.
  END.
