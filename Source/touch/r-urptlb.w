...
DEFINE TEMP-TABLE tt-emp-time NO-UNDO
    ...
    FIELD t-lgInTm   AS INTEGER 
    FIELD t-lgOutTm  AS INTEGER 
    FIELD t-HrWrk    AS INTEGER 
    FIELD t-OT       AS INTEGER
    FIELD t-OT2      AS INTEGER
    FIELD t-MlgInTm  AS INTEGER
    FIELD t-MlgOutTm AS INTEGER
    FIELD t-MHrWrk   AS INTEGER
    FIELD t-MOT      AS INTEGER
    FIELD t-MOT2     AS INTEGER
    .
...
scr-plant-clock rd-dest fi_file tb_OpenCSV tbAutoClose 
...
DEFINE VARIABLE fi_file          AS CHARACTER FORMAT "X(45)" INITIAL "c:~\tmp~\UnreportedLabor.csv" 
    LABEL "Name" 
...
DEFINE FRAME FRAME-A
    selected-company AT ROW 2.29 COL 28.6 COLON-ALIGNED
    begin_employee AT ROW 3.33 COL 28.6 COLON-ALIGNED HELP
    "Enter Beginning Employee"
    end_employee AT ROW 3.33 COL 67.6 COLON-ALIGNED HELP
    "Enter Ending Employee"
    begin_end_date AT ROW 4.38 COL 28.6 COLON-ALIGNED HELP
    "Enter Beginning Employee"
    end_end_date AT ROW 4.38 COL 67.6 COLON-ALIGNED HELP
    "Enter Ending Employee"
    lbl_select-shift AT ROW 6.48 COL 63.6 COLON-ALIGNED NO-LABELS WIDGET-ID 16
    select-shift AT ROW 7.62 COL 64.6 HELP
    "Enter description of this Material Type." NO-LABELS WIDGET-ID 18
    scr-plant-clock AT ROW 8.67 COL 15 NO-LABELS WIDGET-ID 4
    shifts AT ROW 10.76 COL 60 COLON-ALIGNED WIDGET-ID 20
    lines-per-page AT ROW 13.1 COL 87 COLON-ALIGNED
    rd-dest AT ROW 13.14 COL 4.8 NO-LABELS
    lv-font-no AT ROW 13.14 COL 35 COLON-ALIGNED
    lv-ornt AT ROW 13.14 COL 44 NO-LABELS
    lv-font-name AT ROW 14.1 COL 29 COLON-ALIGNED NO-LABELS
    tb_excel AT ROW 15.29 COL 71
    td-show-parm AT ROW 15.52 COL 28.8
    fi_file AT ROW 16.43 COL 26.8 COLON-ALIGNED HELP
    "Enter File Name"
    tb_OpenCSV AT ROW 16.48 COL 92.2 RIGHT-ALIGNED
    tbAutoClose AT ROW 17.67 COL 28.8 WIDGET-ID 64
    btn-ok AT ROW 18.62 COL 28.8
    btn-cancel AT ROW 18.62 COL 54
    "Time Calculation Basis:" VIEW-AS TEXT
    SIZE 22 BY .62 AT ROW 7.48 COL 7 WIDGET-ID 8
    " Output Destination" VIEW-AS TEXT
    SIZE 18 BY .62 AT ROW 12.43 COL 4
    " Selection Parameters" VIEW-AS TEXT
    SIZE 21 BY .71 AT ROW 1.29 COL 4
    RECT-2 AT ROW 1.71 COL 3
    RECT-6 AT ROW 12.86 COL 3
    RECT-3 AT ROW 6.24 COL 3 WIDGET-ID 10
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN 
        select-shift:LIST-ITEMS IN FRAME {&FRAME-NAME} = lv-shift-list.

    btn-ok:LOAD-IMAGE("Graphics/32x32/Ok.png").
    btn-cancel:LOAD-IMAGE("Graphics/32x32/cancel.png").
    RUN enable_UI.
    {sys/inc/reportsConfigNK1.i "TR13" }
    ASSIGN
...
PROCEDURE enable_UI :
    ...
    DISPLAY selected-company begin_employee end_employee begin_end_date 
        end_end_date lbl_select-shift select-shift scr-plant-clock rd-dest 
        fi_file tb_OpenCSV tbAutoClose 
        WITH FRAME FRAME-A IN WINDOW C-Win.
    ENABLE RECT-2 RECT-6 RECT-3 begin_employee end_employee begin_end_date 
        end_end_date select-shift scr-plant-clock rd-dest fi_file tb_OpenCSV 
...
PROCEDURE get-clock-time :
    ...
    DEFINE VARIABLE ll-first-emp     AS LOG     NO-UNDO. 
    ...
    FOR EACH emplogin NO-LOCK 
        ...
        DO:
            ...
            IF NOT AVAILABLE tt-emp-time THEN 
            DO:
                CREATE tt-emp-time.                           
                ASSIGN                                        
...
PROCEDURE get-mach-time :
    ...
    FOR EACH machemp NO-LOCK 
        ...
        /* ===== daily total display */
        IF LAST-OF(machemp.start_date) THEN 
        DO:
            ...
            IF AVAILABLE shifts AND 
                NOT shifts.lunch_paid THEN 
            DO:
...
PROCEDURE run-report :
    ...
    DO:
        ...
        ASSIGN 
            excelheader = "EmployeeId,Employee Name,Start Date,Shift,Login Time,Logout Time," +
    ...
    FOR EACH tt-emp-time 
        ...
        IF FIRST-OF(tt-emp-time.t-empid) THEN 
        DO:      
        ...
        IF LAST-OF(tt-emp-time.t-empid) THEN 
        DO:
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ELSE
            ASSIGN
                ...
                .
        ASSIGN 
            fi_file:SCREEN-VALUE = "c:\tmp\UnreportedLabor.csv".   
    END.
