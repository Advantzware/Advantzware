...
DEF TEMP-TABLE tt-sch FIELD rec-id AS RECID
                      ...
                      FIELD anchored LIKE job-mch.anchored LABEL "Lock" 
...
&Scoped-define QUERY-STRING-Browser-Table FOR EACH tt-sch, ~
               ...
               FIRST job-hdr WHERE job-hdr.company = job-mch.company                       AND job-hdr.job = job-mch.job NO-LOCK                       BY tt-sch.start-date BY tt-sch.seq-no
...
&Scoped-Define DISPLAYED-OBJECTS lv-mach lv-start-date lv-end-date ~
v-cust-no v-board v-die v-plate v-color 
...
DEFINE VARIABLE v-color AS CHARACTER FORMAT "X(15)":U 
     LABEL "Color" 
     VIEW-AS FILL-IN 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS Browser-Table B-table-Win _FREEFORM
  QUERY Browser-Table NO-LOCK DISPLAY
      ...
      tt-sch.start-date FORMAT "99/99/9999":U              LABEL-BGCOLOR 14
...
DEFINE FRAME F-Main
     ...
     lv-start-date AT ROW 1.24 COL 53 COLON-ALIGNED
     lv-end-date AT ROW 1.24 COL 81 COLON-ALIGNED
     v-cust-no AT ROW 2.43 COL 13 COLON-ALIGNED
     v-board AT ROW 2.43 COL 40 COLON-ALIGNED
     v-die AT ROW 2.43 COL 65 COLON-ALIGNED
     v-plate AT ROW 2.43 COL 92 COLON-ALIGNED
     v-color AT ROW 2.43 COL 117 COLON-ALIGNED
...
OPEN QUERY {&SELF-NAME}
    FOR EACH tt-sch,
        ...
        FIRST job-hdr WHERE job-hdr.company = job-mch.company
                      ...
                      BY tt-sch.start-date BY tt-sch.seq-no.
     _END_FREEFORM
...
ASSIGN lv-start-date = TODAY
       lv-end-date = TODAY
       lv-mach = IF AVAIL mach THEN mach.m-code ELSE "".
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH bf-job-mch WHERE bf-job-mch.company = ASI.mach.company
      ...
      FIRST bf-job WHERE bf-job.company = bf-job-hdr.company
                         ...
                         AND (bf-job-mch.start-date >= lv-start-date 
                              /*or lv-start-date = ?*/ )
                         AND bf-job-mch.start-date <= lv-end-date NO-LOCK      
      ...
      /*
      IF FIRST-OF(bf-job.start-date) THEN  ASSIGN lv-seq = 0
                                                  lv-start-time = 0.
      lv-seq = lv-seq + 1.
      */
      ...
      tt-sch.due-date =     IF AVAIL oe-ordl THEN oe-ordl.prom-date ELSE ?. /* oe-ordl.req-date */
  ...
  FOR EACH job-sch WHERE job-sch.company = mach.company 
                     AND job-sch.m-code = mach.m-code:
                     /*AND job-sch.m-date = job-sch.start-date:*/
...
------------------------------------------------------------------------------*/
  ...
  RUN jcsch/r-jobsch.w(mach.m-code,lv-start-date).
...
MESSAGE "Are you sure you want to RELEASE all jobs?" VIEW-AS ALERT-BOX WARNING
       BUTTON YES-NO UPDATE ll-ans AS LOG.
IF ll-ans THEN DO:
   SESSION:SET-WAIT-STATE("general").
   FOR EACH tt-sch:
...
------------------------------------------------------------------------------*/
  ...
  RUN jcsch/d-jobsch.w (mach.m-code,INPUT-OUTPUT lv-date-wkst, INPUT-OUTPUT lv-date-wkend, OUTPUT lv-time-wkst, OUTPUT lv-time-wkend,OUTPUT lv-mcode).
...
------------------------------------------------------------------------------*/
 ...
 MESSAGE "Are you sure you want to Re-Sort Sequence?" VIEW-AS ALERT-BOX WARNING
        BUTTON YES-NO UPDATE ll-ans AS LOG.
 IF ll-ans THEN do:
     /* assign values from screen */
 FOR EACH bt-sch,
     FIRST bf-job-mch WHERE RECID(bf-job-mch) = bt-sch.rec-id AND NOT bf-job-mch.anchored,
...
*/
      ...
      IF FIRST-OF(bt-sch.start-date) THEN ASSIGN lv-seq = 1
...
------------------------------------------------------------------------------*/
     ...
     DO WHILE VALID-HANDLE(lv-brs-col-hd):
        lv-brs-col-hd-list = IF lv-brs-col-hd-list <> "" THEN lv-brs-col-hd-list + ","
...
------------------------------------------------------------------------------*/
  ...
  IF ll-enabled THEN do:
      APPLY "entry" TO tt-sch.seq IN BROWSE {&BROWSE-name} .
  END.
...
------------------------------------------------------------------------------*/
  ...
  lv-mach = IF AVAIL mach THEN mach.m-code ELSE "".
  DISP lv-mach WITH FRAME {&FRAME-NAME}.
  ...
  IF lv-old-seq > tt-sch.seq THEN DO:
      FOR EACH bf-tsch WHERE bf-tsch.seq >= tt-sch.seq AND bf-tsch.seq <= lv-old-seq
          ...
          IF FIRST-OF(bf-tsch.start-date) AND NOT first(bf-tsch.start-date)
                         THEN ASSIGN lv-seq = 1.

          IF tt-sch.seq = 1 THEN DO:
             FIND FIRST mch-act WHERE mch-act.company = mach.company
                               AND mch-act.m-code = mach.m-code  NO-ERROR.
             lv-time-wkst = IF AVAIL mch-act THEN mch-act.START ELSE 0.      
          ...
          IF FIRST-OF(bf-tsch.start-date) THEN ASSIGN lv-seq = 1 
                                                     lv-start-time = lv-time-wkst
                                                      .
    ...
    FOR EACH tt-sch ,
     ...
     FOR EACH bf-tsch,
         ...
         FIRST bf-jhdr WHERE bf-jhdr.company = bf-mch.company
                      ...
                      {&sortby-phrase-asc2}
                      BY bf-tsch.start-date BY bf-tsch.seq-no
                      :
...
------------------------------------------------------------------------------*/
     ...
     li-num-of-col = NUM-ENTRIES(lv-brs-col-hd-list).
/*MESSAGE "set color entry" li-num-of-col
    "COLOR"  lv-new-bgcolor
    VIEW-AS ALERT-BOX.
*/
     DO i = 1 TO NUM-ENTRIES(lv-brs-col-hd-list):
        lv-new-col-hd = WIDGET-HANDLE(ENTRY(i,lv-brs-col-hd-list)).
        lv-new-col-hd:bgcolor = lv-new-bgcolor NO-ERROR.     
     END.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  ELSE IF tt-sch.due-date < tt-sch.start-date OR tt-sch.due-date < TODAY THEN lv-col-bgcolor = 12. /* RED*/ 
