...
DEF VAR v-cum-32 AS cha NO-UNDO.
...
/* PUT{1} 500. */
  ...
  /*===== 32th conversion =======*/                            
        ...
        v-len-32 = "".
        DO i = 1 TO LENGTH(v-lscore-c):
           v-tmp-len = SUBSTRING(v-lscore-c,i,1).
           ...
           END.
 /*          MESSAGE "loop:" i "," v-tmp-len "," v-tmp-dec v-got-dot SKIP
        ...
        v-tmp-dec = "".
        DO i = 1 TO LENGTH(v-lcum-score-c):
           v-tmp-len = SUBSTRING(v-lcum-score-c,i,1).
           IF v-tmp-len = " " THEN do:
              IF v-tmp-dec <> "" THEN DO:             
                 vd-tmp-dec = DEC("0." + v-tmp-dec) .  /* make to decimal*/
                 {sys/inc/k16bb.i vd-tmp-dec}      /*   convert to dec from 16th */
     ...
     */
     ...
     FOR EACH box-design-line OF box-design-hdr NO-LOCK:
        ...
        IF box-design-line.wscore <> "" THEN
         DO i = 1 TO LENGTH(box-design-line.wscore):
           v-tmp-len = SUBSTRING(box-design-line.wscore,i,1).
           IF v-tmp-len = " " THEN do:
              IF v-tmp-dec <> "" THEN DO:
                 vd-tmp-dec = DEC("0." + v-tmp-dec) . /* make to decimal*/
                 {sys/inc/k16bb.i vd-tmp-dec}         /* convert to dec from 16th */                 
                  vd-tmp-dec = dec(v-tmp-dec).
                 /*v-tmp-dec = SUBSTRING(string(vd-tmp-dec),INDEX(STRING(vd-tmp-dec),".") + 1). */
                 /* RUN sys/inc/decfrac2.p (vd-tmp-dec,32,OUTPUT v-tmp-dec). No "-" */
                 RUN sys/inc/dec-frac.p (vd-tmp-dec,32,OUTPUT v-tmp-dec).
              END.
        ...
        DO i = 1 TO LENGTH(box-design-line.wcum-score):
          v-tmp-len = SUBSTRING(box-design-line.wcum-score,i,1).
          IF v-tmp-len = " " THEN do:
             IF v-tmp-dec <> "" THEN DO:
                vd-tmp-dec = DEC("0." + v-tmp-dec) . /* make to decimal*/
                {sys/inc/k16bb.i vd-tmp-dec}         /* convert to dec from 16th */                
          ...
          ELSE do:
              ...
              ELSE IF v-got-dot THEN DO: /* decimal*/
                    v-tmp-dec = v-tmp-dec + v-tmp-len.
              END.
              ELSE v-cum-32 = v-cum-32 + v-tmp-len.
          END.        
       END.
       IF v-tmp-dec <> "" THEN do:
            vd-tmp-dec = DEC("0." + v-tmp-dec) .  /* make to decimal*/
