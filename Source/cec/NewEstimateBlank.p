...
DEFINE VARIABLE cls-part-no         AS CHARACTER NO-UNDO.
...
FIND FIRST bf-eb
    ...
    NO-LOCK NO-ERROR.
cls-part-no = IF AVAIL bf-eb THEN bf-eb.part-no ELSE "".
...
ASSIGN
    ...
    IF ip-industry <> "C" THEN 
        ASSIGN 
            eb.part-no  = cls-part-no + IF cls-part-no NE "" THEN ("-" + STRING(eb.form-no) + "-" + STRING(eb.blank-no)) ELSE ""
...
IF ip-industry = "C" AND ef.est-type EQ 6 AND cls-part-no NE "" THEN 
DO:
    li = 1.
    FOR EACH bf-eb
        WHERE bf-eb.company  EQ ef.company
    ...
    DO WHILE TRUE:
        IF NOT CAN-FIND(FIRST bf-eb
            WHERE bf-eb.company EQ ef.company
            AND bf-eb.est-no  EQ ef.est-no
            AND bf-eb.part-no EQ cls-part-no + "-" + STRING(li)
            AND ROWID(bf-eb)  NE ROWID(eb))
            THEN 
        DO:
            eb.part-no = cls-part-no + "-" + STRING(li).
            LEAVE.
        END.
        li = li + 1.
    END.
