/* ------------------------------------------- cec/quote/quoport-xl.i  06/05 YSK */
/* print quote items                                                           */
/* -------------------------------------------------------------------------- */
...
FOR EACH xqitm OF xquo NO-LOCK BREAK BY xqitm.part-no:
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */ :
    ...
    IF i EQ 1 THEN DO:
      ...
      ASSIGN v-cell = "R" + string(inrowcount + 1) + "C1".
      chExcelApplication:Goto(v-cell) NO-ERROR.
      ASSIGN chExcelApplication:ActiveCell:Value = xquo.q-no .
    ...
    IF i EQ 3 THEN DO:
      ...
      chExcelApplication:Goto(v-cell) NO-ERROR.
      ASSIGN chExcelApplication:ActiveCell:Value = "JUNTA: " + IF AVAIL eb THEN eb.adhesive ELSE ""
             v-cell = "R" + string(LvLineCnt) + "C3".
            
      chExcelApplication:Goto(v-cell) NO-ERROR.
      ASSIGN chExcelApplication:ActiveCell:Value = "N° DO MOLDE: " + IF AVAIL eb THEN eb.die-no ELSE "" .
    ...
    IF i EQ 4 THEN DO:
      ...
      ASSIGN chExcelApplication:ActiveCell:Value =  xqitm.i-coldscr 
       ...
       chExcelApplication:Goto(v-cell) NO-ERROR.
       ASSIGN chExcelApplication:ActiveCell:Value = "N° CAD: " + (IF AVAIL eb THEN eb.cad-no ELSE "") .
       ...
       do:*/
          /*ASSIGN v-cell = "R" + string(LvLineCnt) + "C2".
          chExcelApplication:Goto(v-cell) NO-ERROR.
          ASSIGN chExcelApplication:ActiveCell:Value = xqitm.i-dscr.*/ /* task 12121311  */
       /*end.*/
    END.
    ELSE
    IF i EQ 5 /* AND numfit GE 5 */ THEN DO:
      ...
      chExcelApplication:Goto(v-cell) NO-ERROR.
      ASSIGN chExcelApplication:ActiveCell:Value = "N° PRJ: " + (IF AVAIL eb THEN eb.spc-no ELSE "") .
             v-cell = "R" + string(LvLineCnt) + "C2".
    ...
    IF i EQ 6 THEN DO:
       ...
       IF AVAIL ef THEN
         ...
         v-cell = "R" + string(LvLineCnt) + "C3".
         chExcelApplication:Goto(v-cell) NO-ERROR.
         ASSIGN chExcelApplication:ActiveCell:Value = "N° ART: " + (IF AVAIL ef THEN ef.cad-image ELSE "") .
    END.
    ELSE
    IF i EQ 7 /* AND numfit GE 5 */ THEN DO:
       ...
       do :
        ...
        ASSIGN LvLineCnt = LvLineCnt + 1 
                v-cell = "R" + string(LvLineCnt) + "C3".
        chExcelApplication:Goto(v-cell) NO-ERROR.
        ASSIGN chExcelApplication:ActiveCell:Value = "N° FG: " + lv-fg# .
       end.
       ELSE do :
          ASSIGN
             LvLineCnt = LvLineCnt + 1 
          ...
          chExcelApplication:Goto(v-cell) NO-ERROR.
          chExcelApplication:ActiveCell:Value = "QUANT. POR PALETE: " + (IF AVAIL eb THEN STRING(eb.tr-cnt) ELSE "") .
       END.
    END.
    ELSE
    IF i EQ 8 THEN DO:
         ...
         v-cell = "R" + string(LvLineCnt) + "C2".
         chExcelApplication:Goto(v-cell) NO-ERROR.
         ASSIGN chExcelApplication:ActiveCell:Value = "QUANT. PACOTE: " + (IF AVAIL eb THEN STRING(eb.cas-no) ELSE "") .
    ...
    IF i EQ 9 THEN DO:
         ...
         v-cell = "R" + string(LvLineCnt) + "C2".
         chExcelApplication:Goto(v-cell) NO-ERROR.
         ASSIGN chExcelApplication:ActiveCell:Value = "PESO/M: " + STRING(dWeight) .
    ...
    IF i EQ 10 THEN DO:      
        ...
        v-cell = "R" + string(LvLineCnt) + "C2".
        chExcelApplication:Goto(v-cell) NO-ERROR.
        ASSIGN chExcelApplication:ActiveCell:Value = "Cores: " + STRING(eb.i-col + eb.i-coat) .
    END.
  ...
  IF s-print-comp THEN DO :      /* Print components of a set */
    ...
    FOR EACH ef WHERE ef.company EQ est.company
      ...
      IF FIRST(ef.form-no) THEN DO:
        ...
        chExcelApplication:Goto(v-cell) NO-ERROR.
        ASSIGN chExcelApplication:ActiveCell:Value = "componentes".
      END.
  ...
  FOR EACH xqqty OF xqitm NO-LOCK,
    ...
    IF FIRST-OF(xqchg.qty) THEN DO:
      ...
      ASSIGN inrowcount = inrowcount + 1
             v-cell = "R" + string(inrowcount) + "C2".
      chExcelApplication:Goto(v-cell) NO-ERROR.
      ASSIGN chExcelApplication:ActiveCell:Value = "PARA QTY.: " + TRIM(STRING(xqchg.qty,">>>>>>>")).
    END.
  ...
  /*Itemwise Department Notes Task# 11111308*/
     {custom/notespr2.i job v-inst2 EXTENT(v-inst2) "notes.rec_key = xquo.rec_key and notes.note_code >= fdept and notes.note_code <= tdept" }
      idx = 0.
      ...
      IF idx NE 0 THEN DO:
         ...
         chExcelApplication:Goto(v-cell) NO-ERROR.
         ASSIGN chExcelApplication:ActiveCell:Value = "Notas do Departamento: " . 
...
if (ch-multi /*and v-last*/ ) or (not ch-multi) then do:
  if ch-inst then do:
    ...
    for each est-inst
      ...
      if first-of(est-inst.est-no) then do:
        ...
        chExcelApplication:Goto(v-cell) NO-ERROR.
        ASSIGN chExcelApplication:ActiveCell:Value = "--Instruçães de Fabricação do Departamento--" . 
  ...
  do i = 1 to 5:
    ...
    IF i = 5 AND NOT v-last AND ch-multi THEN DO: /* task 12121311 */
        ASSIGN inrowcount = inrowcount + 1
             v-cell = "R" + string(inrowcount) + "C2".
      chExcelApplication:Goto(v-cell) NO-ERROR.
  ...
  if (ch-multi and v-last ) or (not ch-multi) then   /* task 12121311 */
  ...
  DO:
     ...
     ASSIGN chExcelApplication:ActiveCell:Value = "Obrigado por considerar a Container Service Corporation como um". 
     ...
     ASSIGN chExcelApplication:ActiveCell:Value = "fornecedor de suas necessidades de embalagem.". 
