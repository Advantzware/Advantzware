...
FOR EACH xqitm OF xquo NO-LOCK BREAK BY xqitm.part-no:
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */:
    ...
    IF i EQ 1 THEN DO:
      ...
      */
      ...
      PUT "<C8.5>" xqitm.part-no FORMAT "x(18)"
          "<C26>" TRIM(lv-part-dscr1) FORMAT "x(28)". 
      ELSE DO:
    ...
    IF i EQ 2 THEN DO:
      ...
      lv-part-dscr2 = IF ll-prt-dscr2 THEN xqitm.part-dscr2 ELSE chrX /*style-dscr*/ .
    ...
    IF i EQ 3 THEN DO:
       ...
       ELSE IF v-boardDescription EQ 'Quote' THEN DO:
         ...
         IF NOT logSetPrinting THEN
           chrX = xqitm.i-dscr.
       END.
      lv-i-coldscr = IF ll-prt-dscr2 THEN chrX /*style-dscr*/ ELSE "" /*xqitm.i-coldscr*/ .
  ...
  if ch-inst then do:
      ...
      IF idx NE 0 THEN DO:
         ...
         DO idx = 1 TO EXTENT(v-dept-inst):
           ...
           RUN printHeader (7,OUTPUT idummy).
         END. /* do idx */
      END. /* if idx */
  end.
  put skip(1).
