 ...
 */ 
  IF AVAIL cust THEN
     ASSIGN v-comp-add1 = cust.addr[1]
            ...
            v-comp-add4 = "Tele. : " + string(cust.area-code,"(999)") + string(cust.phone,"999-9999") 
...
if (not ch-multi) then do:
  ...
  lv-tot-pg = lv-tot-pg + TRUNC( ln-cnt / 22,0) .  /* 33 - 55 */
   ...
   {cec/quote/quo10can.i 1}
   v-quo-total = v-line-total + v-t-tax[1] + v-t-tax[2] + v-t-tax[3].
        
   PUT "<FArial><R58><C1><P12><B> Commentaires / Comments  </B> <P9> " .
   ...
   v-tmp-lines = 0.
    ...
    {cec/quote/quo10can.i 2}  
    v-quo-total = v-line-total + v-t-tax[1] + v-t-tax[2] + v-t-tax[3].
   
    IF LINE-COUNTER > PAGE-SIZE - 1 THEN DO:      
      v-printline = 0.
      page.  
      {cec/quote/quo10can.i}
   END.
   ...
   IF (ch-multi and (v-last OR s-sep-page)) then do:
    ...
    /*
      v-note-lines = 0.
      notes:
      FOR EACH notes WHERE notes.rec_key = lv-first-qreckey /*xquo.rec_key */ NO-LOCK:
          ...
          IF notes.note_text <> "" THEN DO i = 1 TO v-tmp-lines:
             ...
             v-note-lines = v-note-lines + 1.
             IF v-note-lines > 5 THEN LEAVE.
          END.
