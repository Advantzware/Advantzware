...
if (xquo.shipto[1] eq xquo.soldto[1] and
  ...
  RUN sys/ref/getFileFullPathName.p ("Template\MsPackaging.xlt", OUTPUT chFile).
  IF chFile = ? THEN  
...
if not ch-multi then do:
   ...
   {cec/quote/quomsp2-xl.i}    
   {cec/quote/quomsp-xl.i 1}
   ...
   chExcelApplication:activeSheet:PageSetup:CenterFooter = "P.O BOX 70 / RINGGOLD GEORGIA 30736 / 706-965-9337 / FAX: 706-965-9269".
   chExcelApplication:Goto("R19C1") NO-ERROR.

   os-delete value("c:\tmp\quote.xls").     
   	os-delete value("c:\tmp\asi.pdf").
   	os-delete value("c:\tmp\quote.pdf").
       ...
       chExcelApplication:ActiveSheet:SaveAs("c:\tmp\quote.xls") no-error. 	   
       NO-RETURN-VALUE chWorkbook:PrintOut(,,,,,False,). 
       chWorkbook:Close(no) no-error.   
       chExcelApplication:Quit() no-error.
        
       /*
       os-command silent value ( CurrDir + "\util\CZ-Xls2Pdf.exe " + 
                                 "c:\tmp\quote.xls " +
                                 "/d:c:\tmp /h " ) .
       ...
       */
       ...
       pause 3.
       OS-DELETE VALUE("c:\tmp\quote.xls").
       OS-RENAME c:\tmp\asi.pdf c:\tmp\quote.pdf.
       ASSIGN LvCtr = LvCtr + 1.
       CREATE tt-filelist.
       ASSIGN tt-FileCtr  = LvCtr
       	      tt-FileName = "c:\tmp\quote.pdf".
   	END.
...
os-delete c:\tmp\bol.pdf.

FOR EACH tt-filelist :
   assign CommandString = CommandString + " " + tt-FileName.
END.

assign CommandString = CommandString + " cat output c:\tmp\quote.pdf".
