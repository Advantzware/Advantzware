/* ------------------------------------------- cec/quote/quostclair.i 11/00 JLF */
...
FOR EACH xqitm OF xquo NO-LOCK BREAK BY xqitm.part-no:
  ...
  /*IF AVAIL eb THEN DO:
      IF v-print-item THEN 
          ASSIGN v-item-name = eb.part-dscr1.
  
      IF v-print-desc1 THEN
          ASSIGN v-item-dscr1 = eb.part-dscr2.
  END.*/
  END.
  ...
  FIND FIRST itemfg WHERE itemfg.company = xqitm.company
                      AND itemfg.i-no = xqitm.i-no NO-LOCK NO-ERROR.


  IF v-print-item THEN DO:
      IF AVAIL eb THEN 
          ASSIGN v-item-name = eb.part-dscr1.
      IF v-item-name EQ "" THEN
          IF AVAIL itemfg THEN
              ASSIGN v-item-name = itemfg.i-name.
  END.

  IF v-print-desc1 THEN DO:
      IF AVAIL eb THEN 
          ASSIGN v-item-dscr1 = eb.part-dscr2.
      IF  v-item-dscr1 EQ "" THEN
          IF AVAIL itemfg THEN
              ASSIGN  v-item-dscr1 = itemfg.part-dscr1.
  END.

  IF AVAIL itemfg THEN DO:
      IF v-fg-desc2 THEN
          ASSIGN v-fg-dscr2 = itemfg.part-dscr2.
      IF v-fg-desc3 THEN
          ASSIGN v-fg-dscr3 = itemfg.part-dscr3.
  END.
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */:
    ...
    IF i EQ 1 THEN DO:
      ...
      */
      PUT TRIM(lv-est-no) FORM "x(5)" AT 2
          xqitm.part-no AT 8 FORMAT "x(15)"

          /* gdm - 11040801 deducted 2 char from format, used to be 30 - now 28*/
           TRIM(lv-part-dscr1) AT 24 FORMAT "x(28)".
    ...
    IF i EQ 2 THEN DO:
      ...
      PUT  xquo.q-no /*trim-size*/ lv-fg# AT 8 FORM "x(15)" /*AT 1 FORM ">>>>9"*/
          /* gdm - 11170804 deducted 2 char from format, used to be 30 */
              /*xqitm.style*/ lv-part-dscr2 /* style-dscr*/ AT 24 FORM "x(28)" .
    END.
    ELSE
    IF i EQ 3 THEN DO:
    ...
    IF i EQ 4 THEN DO:
       ...
       */   
       ...
       IF ll-prt-dscr2 THEN DO:
         ...
         PUT xqitm.i-coldscr AT 24 FORM "x(28)".
       END.
    ...
    IF i EQ 5 /* AND numfit GE 5 */ THEN DO:
       ...
       */
       ...
       IF logSetPrinting THEN DO: 
       IF v-board NE "" THEN
         PUT /*"FG#: " + lv-fg# AT 8 FORM "x(21)"*/ v-board  AT 24 FORM "x(28)".
       END.
       ELSE DO:
       ...
       END.
       IF v-item-name NE " " THEN
           PUT v-item-name AT 24.

       IF v-item-dscr1  NE " " THEN
           PUT v-item-dscr1 AT 24 .

       IF v-fg-dscr2 NE " " THEN
           PUT v-fg-dscr2 AT 24.

       IF v-fg-dscr3 NE " " THEN
           PUT v-fg-dscr3 AT 24.
    ...
    IF AVAIL xqqty THEN DO:
       ASSIGN xxx    = IF xqqty.uom EQ "L" THEN xqqty.price    ELSE
    ...
    /*  IF i EQ 3 THEN
       ...
       */
       ...
       IF AVAIL xqqty THEN 
       IF xqqty.uom = "cs"  THEN DO:
       IF AVAIL eb THEN
           v-qty = (xqqty.qty / eb.cas-cnt).
       END.
