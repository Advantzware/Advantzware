...
/*           xqitm.part-no space(1) lv-part-dscr1. */
    ...
    IF i EQ 6 THEN DO:
       FIND FIRST item
            ...
            NO-LOCK NO-ERROR. 
        IF  AVAIL ITEM AND (item.est-dscr NE "" OR ef.brd-dscr NE "") THEN
        PUT "<C26>" IF item.est-dscr NE "" THEN item.est-dscr ELSE IF AVAIL ef THEN ef.brd-dscr ELSE ""   FORMAT "x(30)".
        ELSE do:
             numfit = numfit + 1 . 
             NEXT .
         END.
    ...
    IF AVAIL xqqty THEN DO:
       ...
       PUT "<C53>" xqqty.qty /* xqqty.rels*/ 
           "<C63>" xqqty.price FORM "->>,>>9.99" 
           "<C76>" xqqty.uom  .
    ...
    FOR EACH ef WHERE ef.company EQ est.company
      ...
      IF v-board EQ "" THEN DO:
        FIND FIRST item
            ...
            NO-LOCK NO-ERROR.
        IF AVAIL item THEN
          v-board =  IF item.est-dscr GT "" THEN item.est-dscr ELSE
                     IF item.i-name   GT "" THEN item.i-name   ELSE
                     item.i-dscr.
      END.
      IF LINE-COUNTER GT PAGE-SIZE - 2 THEN DO: 
            page.  
