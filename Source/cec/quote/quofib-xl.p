...
DEF VAR v-last-q-no AS LOG NO-UNDO.
...
DEF VAR v-cas-cnt LIKE oe-ordl.cas-cnt NO-UNDO.
DEF VAR v-blank AS DEC NO-UNDO.
...
if (xquo.shipto[1] eq xquo.soldto[1] and
  ...
  RUN sys/ref/getFileFullPathName.p ("Template\QuoteFib.xlt", OUTPUT chFile).
  IF chFile = ? THEN  
...
if not ch-multi then do:
   ...
   {cec/quote/quofib2-xl.i}    
   {cec/quote/quofib-xl.i 1}
   
   /*inrowcount = inrowcount + 4.*/

   RUN util/GetBottomRow.p(input 17, input 50, input 6, input inrowcount, output inrowcount ).

   ASSIGN v-cell = "R" + string(inrowcount) + "C2".
   chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = "Quote Prepared By: _____________________"
      inrowcount = inrowcount + 1
      v-cell = "R" + string(inrowcount) + "C2".

   chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = "Quote Valid for 30 Days"
      inrowcount = inrowcount + 2
      v-cell = "R" + string(inrowcount) + "C2".

   chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = FILL(" ",53) + "15250 Don Julian Road, City of Industry, CA 91745"
      inrowcount = inrowcount + 1
      v-cell = "R" + string(inrowcount) + "C2".
   chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = FILL(" ",60) + "Phone (626) 968-8503 Fax (626) 330-0870"
      inrowcount = inrowcount + 1
      v-cell = "R" + string(inrowcount) + "C2".

   chExcelApplication:Goto(v-cell) NO-ERROR.
   chExcelApplication:ActiveCell:Value = FILL(" ",72) + "www.fleetwood-fibre.com".
   ...
   release est.
    ...
    for each report where report.term-id eq v-term-id,
      ...
      ASSIGN
        ...
        v-last-q-no = LAST-OF(report.key-03)
        v-line-total = 0.
  
      {cec/quote/quofib-xl.i 2}  
      v-quo-total = v-line-total + v-t-tax[1] + v-t-tax[2] + v-t-tax[3].
