/* ------------------------------------------- cec/quote/quoport-xl.p - BPV 6/27/2012        */
/* -------------------------------------------------------------------------- */
...
DEFINE VARIABLE opcDateStringQuoDate AS CHARACTER NO-UNDO.
...
IF NOT VALID-HANDLE(chExcelApplication) THEN DO:
    MESSAGE 
      "É necessário o Microsoft Excel. Este relatório não pode ser executado."
...
if (xquo.shipto[1] eq xquo.soldto[1] and
  ...
  RUN sys/ref/getFileFullPathName.p ("Template\QuotePortWaterMark.xlt", OUTPUT chFile).
  ELSE RUN sys/ref/getFileFullPathName.p ("Template\QuotePort.xlt", OUTPUT chFile). 
  IF chFile = ? THEN  
...
if not ch-multi then do:
   ...
   {cec/quote/quoport2-xl.i}    
   {cec/quote/quoport-xl.i 1}
   ...
   chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = " Para itens que utilizam ferramentas de corte e/ou chapas de impressão, a Premier Packaging reserva-se o direito de descartar ferramentas"
    ...
    chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = " e chapas de impressão devido a inatividade por mais de 12 meses. Os clientes terão a opção de solicitar ferramentas antes do descarte."
      inrowcount = inrowcount + 3
   ...
   cPdfFileName = "N° da cotação " + STRING(xquo.q-no) + " Para orçamento " + TRIM(xquo.est-no) .
