...
DEFINE VARIABLE ls-full-img2    AS CHARACTER FORMAT "x(200)" NO-UNDO.
...
ASSIGN    
    ls-image1 = "CustFiles\images\GcTerms.pdf"
...
do:
    ...
    DO :      /* Print components of a set */
        ...
        FIND FIRST est WHERE est.company EQ xquo.company
            AND est.est-no  EQ xquo.est-no NO-LOCK NO-ERROR.
        IF AVAILABLE est AND (est.est-type EQ 6 OR est.est-type EQ 2) AND est.form-qty GT 1 THEN
