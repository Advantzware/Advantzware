/* ------------------------------------------- cec/quote/quoprnts.p 10/02 YSK */
...
if (not ch-multi) then do:
  ...
  PUT "<FArial><R54><C1><P14><B> Comments </B> <P12>" .
  ...
  do i = 1 to 5:
     if xquo.comment[i] ne "" THEN DO: 
        put "<C1><R" string(54.5 + li-cline,">9") + "><C6>" xquo.comment[i]  . 
        li-cline = li-cline + 1.
     END.
  end.
  ...
  END.
   ...
   IF (ch-multi and (v-last OR s-sep-page)) then do:
      PUT "<FArial><R54><C1><P14><B> Comments </B><P12>" SKIP(1) .
     ...
     do i = 1 to 5:      
        if bf-quo.comment[i] ne "" then DO:
            li-cline = li-cline + 1.
            put "<C1><R" string(54.5 + li-cline,">9") + "><C6>" bf-quo.comment[i] .            
        END.
...
/*MESSAGE "test " + STRING(LINE-COUNTER) + "  " STRING(PAGE-SIZE) VIEW-AS ALERT-BOX ERROR .*/
