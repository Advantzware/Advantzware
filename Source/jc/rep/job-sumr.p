...
DEFINE INPUT PARAMETER cocode AS CHARACTER NO-UNDO.
DEFINE INPUT PARAMETER locode AS CHARACTER NO-UNDO.
DEFINE INPUT PARAMETER tb_exclude_run_if_no_prod AS LOG  NO-UNDO.
DEFINE INPUT PARAMETER v-tspost-val AS CHARACTER NO-UNDO.
DEFINE INPUT PARAMETER tb_curr AS LOG NO-UNDO.
DEFINE INPUT PARAMETER tb_curr-crew AS LOG NO-UNDO.

{jc/rep/job-sum.i } /* with share vars */

/* end new parameters */
DEFINE VARIABLE ll-act-rate AS LOG     NO-UNDO.
DEFINE VARIABLE ld-tot-rate AS DECIMAL NO-UNDO.

DEFINE BUFFER b-work-mch FOR work-mch.
...
FOR EACH job-mch WHERE
    ...
    DO:
        FOR EACH mch-act 
            ...
            NO-LOCK.
            IF AVAILABLE mch-act AND AVAIL(job-code) AND mch-act.hours GT 0 THEN
                LEAVE.
        END.
        IF NOT AVAILABLE mch-act THEN 
        DO:       
            FOR EACH mch-act 
                WHERE mch-act.company = cocode 
                AND mch-act.job = job.job 
...
PROCEDURE merge-into-mch-rec:
    DEFINE VARIABLE v-hours-entered AS LOG.
    DEFINE BUFFER bf-work-mch FOR work-mch.
    IF lDebug THEN 
    DO:
    ...
    FOR EACH b-work-mch WHERE b-work-mch.frm GT 0
        ...
        IF FIRST-OF(b-work-mch.m-code) THEN 
        DO:
            ASSIGN 
                v-num-speeds = 0.
        ...
        END.
        DEFINE VARIABLE llDuplicate AS LOG.
        ...
        FOR EACH bf-work-mch WHERE bf-work-mch.frm EQ b-work-mch.frm
            ...
            IF AVAIL(bf-dept) 
                ...
                AND b-work-mch.mr-hr EQ 0 THEN 
            DO:
                llDuplicate = YES.
            END.
        ...
        IF NOT tb_exclude_run_if_no_prod  THEN 
        DO:
            IF b-work-mch.m-code = "BMA" AND lDebug THEN
                MESSAGE "mch" work-mch.m-code
                    ...
                    VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
            ASSIGN 
                work-mch.est-run-hr   = work-mch.est-run-hr    + b-work-mch.est-run-hr
        ...
        DO:
            ...
            IF b-work-mch.run-qty GT 0 THEN 
            DO:
                ...
                /* 7/30 test   AND b-work-mch.run-hr + b-work-mch.mr-hr GT 0) */ )) THEN 
                DO:
        ...
        IF LAST-OF(b-work-mch.m-code) THEN 
        DO:
            ...
            IF AVAILABLE mach AND mach.therm AND liNumForms EQ 1 THEN 
            DO:
...
PROCEDURE fix-missing-costs:
    ...
    DEFINE INPUT PARAMETER ipcJob LIKE job.job.
    ...
    FOR EACH job-mch 
        ...
        FOR EACH mch-act 
            ...
            IF AVAIL(work-mch) AND work-mch.est-run-hr EQ 0 AND work-mch.est-run-cost1 EQ 0
                AND work-mch.est-run-cost2 EQ 0 THEN 
            DO:
        
                v-run-hr = job-mch.run-hr.
                  
                IF job-mch.run-hr EQ 0 OR work-mch.est-run-hr = 0 THEN 
                DO:
                    IF AVAILABLE mch-act AND mch-act.hours GT 0 THEN
...
PROCEDURE add-in-actuals:
    DEFINE INPUT PARAMETER ipcJob LIKE job.job NO-UNDO.
    ...
    FOR EACH mch-act WHERE
        ...
        IF work-mch.m-code = "BMA" AND lDebug AND AVAILABLE job-code THEN
            MESSAGE "job-code" job-code.cat "if run, add in hours " mch-act.hours " times rate = " mach.run-rate ld-tot-rate
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
        IF AVAILABLE job-code AND job-code.cat EQ "RUN" OR job-code.cat EQ "DT" THEN 
        DO:
            ASSIGN
        ...
        DO:
            ...
            IF work-mch.est-run-cost1 EQ ?
                ...
                OR work-mch.est-run-cost3 EQ ? THEN 
            DO:
                IF work-mch.m-code = "BMA" AND lDebug THEN
