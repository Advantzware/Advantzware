...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 ttTable.tableName ttTable.indexName   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1   
&Scoped-define SELF-NAME BROWSE-1
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH ttTable WHERE (tableMatches EQ NO AND (ttTable.tableName BEGINS tableSearch OR ttTable.indexName BEGINS tableSearch)) OR (tableMatches EQ YES AND (ttTable.tableName MATCHES "*" + tableSearch + "*" OR ttTable.indexName MATCHES "*" + tableSearch + "*"))  ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY {&SELF-NAME} FOR EACH ttTable WHERE (tableMatches EQ NO AND (ttTable.tableName BEGINS tableSearch OR ttTable.indexName BEGINS tableSearch)) OR (tableMatches EQ YES AND (ttTable.tableName MATCHES "*" + tableSearch + "*" OR ttTable.indexName MATCHES "*" + tableSearch + "*"))  ~{&SORTBY-PHRASE}.
...
DEFINE FRAME Dialog-Frame
     btnOK AT ROW 27.43 COL 51 WIDGET-ID 6
     tableSearch AT ROW 1.48 COL 3 WIDGET-ID 62
     tableMatches AT ROW 1.48 COL 56 HELP
          "Select for Table Search Matches" WIDGET-ID 40
     BROWSE-1 AT ROW 2.67 COL 2 WIDGET-ID 200
     btnCancel AT ROW 27.43 COL 59 WIDGET-ID 4
     RECT-1 AT ROW 27.19 COL 50 WIDGET-ID 2
     RECT-TABLE AT ROW 1.24 COL 2 WIDGET-ID 46
     SPACE(0.00) SKIP(26.89)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         BGCOLOR 15 FGCOLOR 1 
         TITLE "Indexes" WIDGET-ID 100.
...
OR ttTable.indexName BEGINS tableSearch))
OR (tableMatches EQ YES
AND (ttTable.tableName MATCHES "*" + tableSearch + "*"
OR ttTable.indexName MATCHES "*" + tableSearch + "*"))
 ~{&SORTBY-PHRASE}.
     _END_FREEFORM
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Indexes */
DO:
...
DO:
    ASSIGN
        iphWidget:SCREEN-VALUE = "USE-INDEX " + ttTable.indexName
        ENTRY(2,iphWidget:PRIVATE-DATA,"|") = ttTable.tableName
        .
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  FIND FIRST ttTable
       WHERE ttTable.indexName EQ ENTRY(2,iphWidget:SCREEN-VALUE," ")
       NO-ERROR.
...
{methods/sortByProc.i "pByIndexName" "ttTable.indexName"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pGetIndexNames Dialog-Frame 
PROCEDURE pGetIndexNames :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    DO idx = 1 TO NUM-ENTRIES(ipcTableList):
        ...
        RUN nosweat/indxlist.p (cTableName, OUTPUT cIndexList).
        DO jdx = 1 TO NUM-ENTRIES(cIndexList):
            CREATE ttTable.
            ASSIGN
                ttTable.tableDB   = cTableDB
                ttTable.tableName = cTableName
                ttTable.indexName = ENTRY(jdx,cIndexList)
                .
        END. /* do jdx */
    END. /* do idx */
    {&OPEN-QUERY-{&BROWSE-NAME}}
