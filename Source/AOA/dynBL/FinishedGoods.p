...
PROCEDURE pBusinessLogic:     
    ...
    DEFINE VARIABLE cInks           AS CHARACTER NO-UNDO EXTENT 20.
    DEFINE VARIABLE cInksDesc       AS CHARACTER NO-UNDO EXTENT 20.
    DEFINE VARIABLE cCoat           AS CHARACTER NO-UNDO EXTENT 20.
    DEFINE VARIABLE iCoatCnt        AS INTEGER   NO-UNDO.
    DEFINE VARIABLE cSpecGroup      AS CHARACTER NO-UNDO EXTENT 7.
    DEFINE VARIABLE cSpecTitle      AS CHARACTER NO-UNDO EXTENT 7.
    DEFINE VARIABLE cSpecNote       AS CHARACTER NO-UNDO EXTENT 7.
    ...
    FOR EACH itemfg NO-LOCK
        WHERE itemfg.company EQ cCompany
          ...
          AND itemfg.style   GE cStartStyleNo
        ...
        IF AVAILABLE eb THEN DO:
            FIND FIRST ef NO-LOCK
                 WHERE ef.company EQ cCompany
                   AND ef.est-no  EQ itemfg.est-no 
                   AND ef.form-no EQ eb.form-no
                 NO-ERROR.        
            FIND FIRST style NO-LOCK
                 WHERE style.company EQ itemfg.company
                   AND style.style   EQ eb.style
                 NO-ERROR.     
            ASSIGN 
                ttFinishGoods.style = IF AVAILABLE style THEN style.dscr ELSE eb.style
                cTrNo               = eb.tr-no
                cCasNo              = eb.cas-no
            ...
            FIND FIRST item NO-LOCK
                 ...
                 NO-ERROR.
            IF AVAILABLE item THEN
            cCasName = item.i-NAME.
            FIND FIRST item NO-LOCK
                 WHERE item.company EQ eb.company
                   AND item.i-no    EQ cBoard
                 NO-ERROR.       
            IF AVAILABLE item THEN
            dClip = item.cal.     
            ASSIGN 
