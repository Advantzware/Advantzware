...
PROCEDURE pBusinessLogic:
    ...
    DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
       ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
        ...
        cTransactionTime = STRING(int(SUBSTRING(STRING(sys-ctrl.dec-fld),1,2)) * 60 * 60 + INT(SUBSTRING(STRING(sys-ctrl.dec-fld),3,4)) * 60 , "hh:mm:ss").
        ELSE cTransactionTime = STRING(TIME,"hh:mm:ss").
...
PROCEDURE pPostBols :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE INPUT PARAMETER fgreorder-log AS LOGICAL NO-UNDO.
