...
DEFINE TEMP-TABLE ttSchedShipvsQtyHand NO-UNDO
    ...
    FIELD variance AS INTEGER   FORMAT "->>>,>>>,>>>" LABEL "Variance"
    FIELD ordQty   AS INTEGER   FORMAT "->>>,>>>,>>>" LABEL "Ordered Qty"
    FIELD qtyHand  AS INTEGER   FORMAT "->>>,>>>,>>>" LABEL "On Hand Qty"
    FIELD netQty   AS INTEGER   FORMAT "->>>,>>>,>>>" LABEL "Net Qty"
    FIELD balShip  AS INTEGER   FORMAT "->>>,>>>,>>>" LABEL "Bal to Ship"
    FIELD openPo   AS INTEGER   FORMAT "->>>,>>>,>>>" LABEL "Open PO's"
...
PROCEDURE pBusinessLogic:
    DEFINE VARIABLE iBalShip  AS INTEGER FORMAT "->>>,>>>,>>>" INITIAL 0 NO-UNDO.
    DEFINE VARIABLE iNetQty   AS INTEGER FORMAT "->>>,>>>,>>>" INITIAL 0 NO-UNDO.
    DEFINE VARIABLE iQtyHand  AS INTEGER FORMAT "->>>,>>>,>>>" INITIAL 0 NO-UNDO. 
    DEFINE VARIABLE iQtyOrder AS INTEGER FORMAT "->>>,>>>,>>>" INITIAL 0 NO-UNDO.
    DEFINE VARIABLE iQtyPo    AS INTEGER FORMAT "->>>,>>>,>>>" INITIAL 0 NO-UNDO.
        ...
        FOR EACH po-ordl
            ...
            ELSE
                RUN sys/ref/convquom.p(po-ordl.cons-uom, "EA", 0, 0, 0, 0,
                    po-ordl.cons-qty, OUTPUT dHldQty).
