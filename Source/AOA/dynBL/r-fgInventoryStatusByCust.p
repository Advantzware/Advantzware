...
PROCEDURE pBusinessLogic:
    ...
    DEFINE VARIABLE lCustOwn       AS LOG       FORMAT "Y/N" INIT "N" NO-UNDO.
    DEFINE VARIABLE lZeroQtyOnHand AS LOG       FORMAT "Y/N" INIT "N" NO-UNDO.
    DEFINE VARIABLE type           AS CHARACTER FORMAT "!" INIT "A" NO-UNDO.
    
    ASSIGN
        TYPE     = SUBSTR(cItemCode2,1,1)
    ...
    FOR EACH cust NO-LOCK
        ...
        IF  cSlsRep    GE cStartSalesRep
        AND cSlsRep    LE cEndSalesRep 
        THEN DO:
...
FOR EACH tt-cust,
    FIRST cust
      ...
      IF FIRST-OF(cust.cust-no) THEN
        ...
        FOR EACH itemfg WHERE itemfg.company  EQ cCompany
                          AND itemfg.cust-no EQ cust.cust-no
                          AND itemfg.cust-po-no GE cStartCustPoNo
                          AND itemfg.cust-po-no LE cEndCustPoNo
               NO-LOCK 
        ...
        IF NOT (cSlsRep    GE cStartSalesRep
            AND cSlsRep    LE cEndSalesRep
            ) THEN 
            NEXT.
          ...
          DO:
            FOR EACH fg-bin NO-LOCK WHERE fg-bin.company = cCompany AND
              ...
              DO:
                IF (dQtyOnhand NE 0 ) OR (dQtyOnhand EQ 0 AND lZeroQtyOnHand) THEN
                DO:
                  ...
                  IF AVAILABLE uom THEN
                    dExt = (dQtyOnhand * itemfg.sell-price / uom.mult).
                  ELSE
                    dExt = (dQtyOnhand * itemfg.sell-price) / 1000.

                  IF itemfg.sell-uom = "L" THEN
                    dExt = itemfg.sell-price.
                   bin-block:
                  FOR EACH xbin WHERE xbin.company = cCompany AND
                   ...
                   DO:
                      FIND FIRST xbin2 WHERE xbin2.company = cCompany AND
                                            ...
                                            (xbin2.job-no  <> xbin.job-no OR
                                             ...
                                             xbin2.qty <> 0
                                             NO-LOCK NO-ERROR.
                      IF AVAILABLE xbin2 AND dQtyJob = 0 THEN
                         NEXT.
                      IF itemfg.sell-uom = "CS" AND
                         itemfg.case-count NE 0 THEN
                         dExtJob = (dQtyJob * itemfg.sell-price) /
                                      itemfg.case-count.
                      ELSE
                         FIND FIRST uom WHERE uom.uom = itemfg.sell-uom AND
                                              uom.mult NE 0 NO-LOCK NO-ERROR.
                      IF AVAILABLE uom THEN
                         dExtJob = (dQtyJob * itemfg.sell-price / uom.mult).
                      ELSE
                         dExtJob = (dQtyJob * itemfg.sell-price) / 1000.
                      ...
                      IF AVAIL xbin THEN
                         ...
                         IF AVAIL b-rcpth THEN dtTransDate = b-rcpth.trans-date.
                    ...
                    IF xbin.job-no = "" AND xbin.job-no2 = 0 THEN
                          cJobNo = "".
                       ELSE
                          cJobNo = xbin.job-no + "-" + string(xbin.job-no2,"999").
                   ...
                   IF AVAIL oe-ordl THEN
                     ...
                     IF AVAIL oe-ordl THEN 
                         ASSIGN dSellprice = (oe-ordl.t-price / oe-ordl.qty * 1000) .
                     ELSE 
                         ASSIGN dSellprice = itemfg.sell-price .
                    ...
                    ASSIGN
                        ...
                        ttFGInventoryStatus.cPo          = IF AVAIL oe-ordl THEN STRING(oe-ordl.po-no,"x(15)") ELSE STRING(itemfg.cust-po-no,"x(15)")
                        ...
                        ttFGInventoryStatus.cOrdDueDate  = IF AVAIL oe-ord AND oe-ord.due-date NE ? THEN STRING(oe-ord.due-date,"99/99/9999") ELSE ""
                        ttFGInventoryStatus.cJobDueDate  = IF AVAIL job AND job.due-date NE ? THEN STRING(job.due-date,"99/99/9999") ELSE ""
                        ttFGInventoryStatus.cCSR         = IF AVAIL oe-ord AND oe-ord.csrUser_id NE "" THEN STRING(oe-ord.csrUser_id,"x(8)") ELSE ""
                        iCount = iCount + 1
                        .
