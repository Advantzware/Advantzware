...
&Scoped-define ttTempTable ttInventoryValue
...
PROCEDURE pBusinessLogic:
    ...
    ASSIGN
        lProcessLastSale = CAN-FIND(FIRST dynValueColumn
                                    WHERE dynValueColumn.subjectID    EQ dynParamValue.subjectID
                                      ...
                                      AND dynValueColumn.colName      EQ "lastSale")
        lProcessRel      = CAN-FIND(FIRST dynValueColumn
                                    WHERE dynValueColumn.subjectID    EQ dynParamValue.subjectID
                                      AND dynValueColumn.user-id      EQ dynParamValue.user-id
                                      AND dynValueColumn.prgmName     EQ dynParamValue.prgmName
                                      AND dynValueColumn.paramValueID EQ dynParamValue.paramValueID
                                      AND (dynValueColumn.colName     EQ "relQty"
                                       OR dynValueColumn.colName      EQ "relPO"))
        .
    ...
    FOR EACH itemfg NO-LOCK
        WHERE itemfg.company EQ cCompany
          ...
          AND (itemfg.i-code EQ cItemCode2 OR cItemCode2 EQ "A")
          AND (itemfg.stat EQ "A" OR lIncludeInactiveItems)
          AND (lPrintSetandComponentsOnly EQ NO OR itemfg.isaset EQ YES
        ...
        RUN fg/rep/tt-fgbin.p (
            ...
            cEndLocBin,
            lIncludeZeroBalance,
            iOnlyShowQOHOlderThanDays,
            YES,
            lIncludeCustomerOwnedWarehouse
            ).
        
        FOR EACH tt-fg-bin
            WHERE tt-fg-bin.company EQ itemfg.company
              AND tt-fg-bin.i-no    EQ itemfg.i-no
              AND (lIncludeCustomerOwnedWarehouse OR lOnlyCustomerOwnedWarehouse
