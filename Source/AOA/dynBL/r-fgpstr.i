/* r-fgpstr.i - rstark - 6.27.2022 */

/* Temp-Table Definitions ---                                           */
{AOA/tempTable/ttFGPostHist.i}
...
PROCEDURE pBusinessLogic:
    ...
    FOR EACH tt-report
        ...
        ASSIGN
            ...
            ttFGPostHist.dmiID         = IF AVAILABLE mach THEN mach.spare-int-2 ELSE 0
            ttFGPostHist.m-code        = IF AVAILABLE job-mch THEN job-mch.m-code ELSE ""
            ttFGPostHist.jobNo         = fg-rcpth.job-no
            ttFGPostHist.jobNo2        = fg-rcpth.job-no2
            ttFGPostHist.frm           = IF AVAILABLE job-mch THEN job-mch.frm ELSE 0
            ...
            ttFGPostHist.weightPerEA   = itemfg.weightPerEA
            ttFGPostHist.qtyInLBS      = itemfg.weightPerEA * ttFGPostHist.v-fg-qty
            ttFGPostHist.qtyInTons     = ttFGPostHist.qtyInLBS / 2000
            ttFGPostHist.matCost       = ttFGPostHist.item-mat-cost
                                       / (IF ttFGPostHist.lv-cost-uom EQ "M" THEN 1000 ELSE 1)
                                       * ttFGPostHist.v-fg-qty
            ttFGPostHist.prodCost      = (ttFGPostHist.item-dl-cost
                                       +  ttFGPostHist.item-voh-cost
                                       +  ttFGPostHist.item-foh-cost) 
                                       / (IF ttFGPostHist.lv-cost-uom EQ "M" THEN 1000 ELSE 1)
                                       * ttFGPostHist.v-fg-qty
                                       .
    END.
    &IF {&subjectID} EQ 209 &THEN
    FOR EACH ttFGPostHist
        BREAK BY ttFGPostHist.dmiID
              BY ttFGPostHist.itemNo
        :
        IF FIRST-OF(ttFGPostHist.itemNo) THEN DO:
        CREATE ttCostByMachine.
        ASSIGN
            ttCostByMachine.dmiID       = ttFGPostHist.dmiID
            ttCostByMachine.m-code      = ttFGPostHist.m-code
            ttCostByMachine.itemNo      = ttFGPostHist.itemNo
            ttCostByMachine.itemName    = ttFGPostHist.itemName
            ttCostByMachine.uom         = ttFGPostHist.lv-cost-uom
            ttCostByMachine.weightPerEA = ttFGPostHist.weightPerEA
            .
        END. // first-of
        ASSIGN
            ttCostByMachine.qtyInLbs  = ttCostByMachine.qtyInLbs  + ttFGPostHist.qtyInLbs
            ttCostByMachine.qtyInTons = ttCostByMachine.qtyInTons + ttFGPostHist.qtyInTons
            ttCostByMachine.matCost   = ttCostByMachine.matCost   + ttFGPostHist.matCost
            ttCostByMachine.prodCost  = ttCostByMachine.prodCost  + ttFGPostHist.prodCost
            .
