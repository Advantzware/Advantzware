...
/*------------------------------------------------------------------------
  ...
  Description: Subject Tables in Query Design

  Input Parameters: Selected Query Object, Object Type
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-1 ttTable.tableDB ttTable.tableName ttTable.tableDscr   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-1   
&Scoped-define SELF-NAME BROWSE-1
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH ttTable WHERE (tableMatches EQ NO AND (ttTable.tableName BEGINS tableSearch OR ttTable.tableDscr BEGINS tableSearch)) OR (tableMatches EQ YES AND (ttTable.tableName MATCHES "*" + tableSearch + "*" OR ttTable.tableDscr MATCHES "*" + tableSearch + "*"))  ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-BROWSE-1 OPEN QUERY {&SELF-NAME} FOR EACH ttTable WHERE (tableMatches EQ NO AND (ttTable.tableName BEGINS tableSearch OR ttTable.tableDscr BEGINS tableSearch)) OR (tableMatches EQ YES AND (ttTable.tableName MATCHES "*" + tableSearch + "*" OR ttTable.tableDscr MATCHES "*" + tableSearch + "*"))  ~{&SORTBY-PHRASE}.
...
btnCancel findType 
&Scoped-Define DISPLAYED-OBJECTS tableSearch tableMatches findType 
...
DEFINE FRAME Dialog-Frame
     btnOK AT ROW 27.43 COL 82 WIDGET-ID 6
     tableSearch AT ROW 1.48 COL 3 WIDGET-ID 62
     tableMatches AT ROW 1.48 COL 86 HELP
          "Select for Table Search Matches" WIDGET-ID 40
     BROWSE-1 AT ROW 2.67 COL 2 WIDGET-ID 200
     btnCancel AT ROW 27.43 COL 90 WIDGET-ID 4
     findType AT ROW 27.91 COL 48 NO-LABEL WIDGET-ID 64
     RECT-1 AT ROW 27.19 COL 81 WIDGET-ID 2
     RECT-TABLE AT ROW 1.24 COL 2 WIDGET-ID 46
     SPACE(0.00) SKIP(26.90)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         BGCOLOR 15 FGCOLOR 1 
         TITLE "Tables" WIDGET-ID 100.
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Tables */
DO:
...
DO:
    IF ipcType EQ "OF" THEN
    findType = ipcType.
    ASSIGN
        ENTRY(2,iphWidget:PRIVATE-DATA,"|") = ttTable.tableDscr
        iphWidget:SCREEN-VALUE = findType + " "
                               + ttTable.tableDB + "."
                               + ttTable.tableName
                               .
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL findType Dialog-Frame
ON VALUE-CHANGED OF findType IN FRAME Dialog-Frame
DO:
    ASSIGN {&SELF-NAME}.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  ELSE
  ASSIGN
      findType:SCREEN-VALUE = ipcType
      findType
      .
  FIND FIRST ttTable
       WHERE ttTable.tableDB   EQ ENTRY(1,ENTRY(2,iphWidget:SCREEN-VALUE," "),".")
         AND ttTable.tableName EQ ENTRY(2,ENTRY(2,iphWidget:SCREEN-VALUE," "),".")
       NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY tableSearch tableMatches findType 
      WITH FRAME Dialog-Frame.
  ENABLE btnOK tableSearch tableMatches BROWSE-1 btnCancel findType 
      WITH FRAME Dialog-Frame.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pGetDBTables Dialog-Frame 
