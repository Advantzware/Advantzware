...
&SCOPED-DEFINE h_Browse01 h_b-reconc
...
DEFINE VARIABLE h_b-reconc AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Bank Reconciliation */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Bank Reconciliation */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_smartmsg ).
       RUN set-position IN h_smartmsg ( 1.00 , 81.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Bank Reconcl' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 3.14 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 22.67 , 154.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
        RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_export ).
       RUN set-position IN h_export ( 1.00 , 68.80 ) NO-ERROR.
       /* Size in UIB:  ( 1.81 , 7.80 ) */
        ...
        RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_movecol ).
       RUN set-position IN h_movecol ( 1.00 , 76.80 ) NO-ERROR.
       /* Size in UIB:  ( 1.81 , 7.80 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ap/b-reconc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-reconc ).
       RUN set-position IN h_b-reconc ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-reconc ( 19.05 , 145.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartObject h_export. */
       RUN add-link IN adm-broker-hdl ( h_b-reconc , 'export-xl':U , h_export ).

       /* Links to SmartViewer h_movecol. */
       RUN add-link IN adm-broker-hdl (h_b-reconc , 'move-columns':U , h_movecol ).

       /* Links to SmartNavBrowser h_b-reconc. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-reconc ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-reconc ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
