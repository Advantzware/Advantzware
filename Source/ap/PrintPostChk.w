...
DEFINE FRAME FRAME-A
     ...
     begin_vend-no AT ROW 6.43 COL 28.6 COLON-ALIGNED HELP
          "Enter Beginning Vendor  Number"
     end_vend-no AT ROW 6.43 COL 67.2 COLON-ALIGNED HELP
          "Enter Ending Vendor Number"
     begin_check AT ROW 7.91 COL 28.6 COLON-ALIGNED HELP
          "Enter Beginning Check  Number" WIDGET-ID 66
     end_check AT ROW 7.91 COL 67.2 COLON-ALIGNED HELP
          "Enter Ending Vendor Number" WIDGET-ID 68
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW C-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Reprint Posted Check/Remittance"
         HEIGHT             = 18.71
         WIDTH              = 95.2
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 273.2
...
ON END-ERROR OF C-Win /* Reprint Posted Check/Remittance */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Reprint Posted Check/Remittance */
DO:
       ...
       // stnum = START_check-no.
        ...
        DO:
            IF CAN-FIND(FIRST ap-pay
                ...
                AND ap-pay.vend-no   GE begin_vend-no
                AND ap-pay.vend-no   LE end_vend-no
                ...
                FOR EACH ap-pay
                    ...
                    AND ap-payl.vend-no   EQ b-ap-chk.vend-no
                    AND ap-payl.man-check EQ NO),
                    FIRST b-vend
                    WHERE b-vend.company EQ cocode
                    AND b-vend.vend-no EQ ap-pay.vend-no
                    BREAK BY ap-pay.company
                    BY ap-pay.vend-no:
                    ...
                    DO:
                        FIND FIRST sys-ctrl-shipto
                            ...
                            AND sys-ctrl-shipto.cust-vend-no = ap-pay.vend-no 
                        ...
                        RUN run-report(ap-pay.vend-no, TRUE).
                        RUN GenerateReport.
                    END.
                END. /* FOR EACH */
            ELSE
...
------------------------------------------------------------------------------*/
  DISPLAY check-date bank-code bank-name begin_vend-no end_vend-no begin_check 
          end_check rd-dest run_format td-show-parm tbAutoClose 
      WITH FRAME FRAME-A IN WINDOW C-Win.
  ENABLE RECT-6 RECT-7 RECT-9 check-date bank-code begin_vend-no end_vend-no 
...
DEFINE INPUT PARAMETER icVendNo AS CHARACTER NO-UNDO.
    ...
    DO ON ERROR UNDO outers, LEAVE outers :
        ...
        ASSIGN 
            num-of-chks = 0
            sel-per-chk = 0
            x-bank      = bank-code:SCREEN-VALUE IN FRAME {&FRAME-NAME}
            wdate       = date(check-date:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
        ...
        ASSIGN
            iBeginCheckNo = integer(begin_check:SCREEN-VALUE IN FRAME {&FRAME-NAME})
            iEndCheckNo = integer(end_check:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
        ...
        CASE ipcFormName:
            WHEN "ASI" THEN
                ASSIGN
                    ...
                    max-per-chk       = 40
                    next-program      = "ap/ApPrintAchASI.p".
            WHEN "Badger" THEN
                ASSIGN 
                    ll-is-xprint-form = YES
                    max-per-chk       = 40
                    next-program      = "ap/ApPrintAchBadger.p".
        END CASE.
    ...
    DO:
        ...
        CASE ipcFormName:
            WHEN "Brick" THEN
                ASSIGN
                    max-per-chk  = 20
                    next-program = "ap/ApPrintCkbrk.p".
            WHEN "AIHalper" THEN
                ASSIGN
                    max-per-chk  = 13
                    next-program = "ap/ApPrintCkaih.p".
            WHEN "P&P" THEN
                ASSIGN
                    max-per-chk  = 13
                    next-program = "ap/ApPrintCkpnp.p".
            WHEN "ContSrvc" THEN
                ASSIGN
                    max-per-chk  = 13
                    next-program = "ap/ApPrintCkcsc.p".
           /* WHEN "Pacific" THEN
                ASSIGN
                    max-per-chk  = 8
            ...
            WHEN "IndianaL" THEN
                ASSIGN
                    max-per-chk  = 10
                    next-program = "ap/ApPrintCkind.p". /* same as ap-ckasi.p */
            /*WHEN "Adaptls" THEN
                ASSIGN
                    max-per-chk  = 10
