...
&Scoped-define QUERY-STRING-BROWSE-1 FOR EACH ap-inv WHERE ~{&KEY-PHRASE} ~
      AND ap-inv.company = ip-company ~
and ap-inv.vend-no = ip-vend-no ~
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 114 BY 11.19
         BGCOLOR 8 FONT 0.
...
/* Query rebuild information for BROWSE BROWSE-1
     ...
     _Where[1]         = "ASI.ap-inv.company = ip-company
and ap-inv.vend-no = ip-vend-no
 AND ASI.ap-inv.posted eq yes
and ap-inv.due ne 0"
...
"ap-inv.due" "Due" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE BROWSE-1 */
&ANALYZE-RESUME
