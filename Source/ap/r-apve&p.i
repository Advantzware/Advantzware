      ...
      DO i = 1 TO extent(stax.tax-rate1):
        IF stax.tax-rate1[i] NE 0 THEN DO:
          ...
          ASSIGN
           ...
           tt-ap-tax.cDescription = "Vendor:" + vend.vend-no + " Invoice:" + string(ap-inv.inv-no).
    ...
    ELSE DO:
      ...
      ASSIGN
       ...
       tt-ap-tax.curr-amt = ap-inv.tax-amt *
                            (IF AVAIL currency THEN currency.ex-rate ELSE 1)
       tt-ap-tax.cDescription = (IF AVAIL vend THEN "Vendor:" + vend.vend-no ELSE "") + " Invoice:" + string(ap-inv.inv-no) .
