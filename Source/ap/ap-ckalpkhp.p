/* ap-ckallpkhp.p  Check print for All Packaging Laser HP 1536DN */
&ANALYZE-SUSPEND _VERSION-NUMBER AB_v10r12
&ANALYZE-RESUME
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CUSTOM _DEFINITIONS Procedure 
/*------------------------------------------------------------------------
    File        : ap/ap-ckalpkhp.p
...
DEF VAR viLn-Section-1  AS INT NO-UNDO INIT 4.  /* Line to start printing top section details. */
DEF VAR viLn-Check      AS INT NO-UNDO INIT 26. /* Line to start printing the check. */
DEF VAR viLn-Void       AS INT NO-UNDO INIT 26. /* Line to start printing VOID. */
DEF VAR viLn-Section-2  AS INT NO-UNDO INIT 43. /* Line to start printing bottom section details. */
DEF VAR viLn-Detail-Max AS INT NO-UNDO INIT 20. /* Max number of detail lines in a section. */
...
DEF VAR vlLeft                AS LOG NO-UNDO INIT YES. /*right or left detail tracking variable*/
...
/*         ap-chk.check-no   = stnum */
       ...
       FOR EACH ap-sel WHERE 
         ...
         IF FIRST(ap-sel.inv-no) THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    PUT 
/*         LineNum() FORMAT "99" AT 1 */
        ipCheckNo TO 6
        ap-chk.check-date TO 15 FORMAT "99/99/99"
...
&IF DEFINED(EXCLUDE-Print-Detail1) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Print-Detail1 Procedure 
PROCEDURE Print-Detail1 :
/*------------------------------------------------------------------------------
...
/*          wrk-chk.amt-paid              TO 56 FORMAT "->>>,>>9.99" */
         ...
         wrk-chk.disc-amt              TO 34 FORMAT "->>,>>9.99".
...
&IF DEFINED(EXCLUDE-Print-Detail2) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Print-Detail2 Procedure 
PROCEDURE Print-Detail2 :
/*------------------------------------------------------------------------------
  Purpose:     Print a detail line.
  Parameters:  <none>
  Notes:       Uncomment line number when testing line numbers.
...
------------------------------------------------------------------------------*/
     ...
     /* Print a detail line. */
     PUT /*LineNum() FORMAT "99" AT 1 */
         wrk-chk.inv-no                TO 59 FORMAT "x(12)"
/*          trim(string(wrk-chk.po-no,">>>>>>")) to 25 format "x(12)" */
/*          wrk-chk.inv-date              TO 34 FORMAT "99/99/99" */
/*          wrk-chk.inv-amt               TO 45 FORMAT "->>>,>>9.99" */
/*          wrk-chk.amt-paid              TO 56 FORMAT "->>>,>>9.99" */
         wrk-chk.line-amt              TO 70 FORMAT "->>>,>>9.99"
         wrk-chk.disc-amt              TO 80 FORMAT "->>,>>9.99"
         .
...
------------------------------------------------------------------------------*/
  DEFINE INPUT PARAMETER ipCheckNo LIKE ap-chk.check-no NO-UNDO.
  DEFINE VAR iSkipLines AS INT NO-UNDO.
  
  vlLeft = YES.
  iSkipLines = (viLn-Detail-Max / 2) + 1 - INT(ROUND((viNumDetailsPrinted) / 2, 0)).
  
  skipLines(iSkipLines).
