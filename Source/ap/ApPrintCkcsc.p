/***************************************************************************\
*****************************************************************************
**  Program: /u2/fold/all/test/asi/ap/ap
**       By: Chris Heins
** Descript: Check format for Container Service Corp.
**
*****************************************************************************
\***************************************************************************/

{sys/inc/var.i shared}
{sys/form/s-top.f}

{ap/ap-chk.i}

form skip(6)
     ap-pay.check-no   to 79
     skip(1)
     dol               at 10 skip
     skip(2)
     ap-pay.check-date to 61 FORM "99/99/99"
     ctot              to 80 format "*,***,**9.99"
     skip(1)
     vend.remit        at 10
     add1              at 10
     add2              at 10
     csz               at 10
     skip(2)
     "Memo:"           to 8
     vend.check-memo 
     skip(2)

     with frame b1 width 85 no-box no-labels stream-io no-attr-space.

if v-print-mode ne "align" then do:
  for each ap-pay NO-LOCK
      where ap-pay.company   eq cocode
        and ap-pay.vend-no   ge wvend-no
        and ap-pay.vend-no   le evend-no
        and ap-pay.check-no  GE iBeginCheckNo
        and ap-pay.check-no  LE iEndCheckNo
        AND ap-pay.check-date EQ wdate
        AND ap-pay.bank-code EQ x-bank
        and ap-pay.man-check eq no,
        
      first vend NO-LOCK
      where vend.company eq cocode
        and vend.vend-no eq ap-pay.vend-no,
        
      each ap-payl NO-LOCK
      where ap-payl.company   eq cocode
        and ap-payl.vend-no   eq ap-pay.vend-no
        and ap-payl.man-check eq no

      break by (if v-sort-name then vend.name else "")
            by ap-pay.vend-no
            by ap-payl.vend-no
            by ap-payl.inv-no:

    IF FIRST-OF(ap-pay.vend-no) THEN DO:
      ASSIGN
       add1   = vend.r-add1
       add2   = vend.r-add2
       csz    = vend.r-city + ", " + vend.r-state
       lv-zip = vend.r-zip.

      IF add1 EQ "" AND add2 EQ "" THEN    /*if no remit-to address*/
        ASSIGN
         add1   = vend.add1
         add2   = vend.add2
         csz    = vend.city + ", " + vend.state
         lv-zip = vend.zip.

      IF lv-zip BEGINS "00000" THEN lv-zip = "".

      csz = TRIM(csz) + "  " + SUBSTR(lv-zip,1,5) +
            (IF LENGTH(lv-zip) GT 5 THEN ("-" + SUBSTR(lv-zip,6,4)) ELSE "").

      IF TRIM(csz) EQ "," THEN csz = "".

      ASSIGN
       ll                = 0        
       v-vend-no         = vend.vend-no
       v-vend-name       = vend.name.

     IF add1 EQ "" THEN
       ASSIGN
        add1 = add2
        add2 = "".

     IF add2 EQ "" THEN
       ASSIGN
        add2 = csz
        csz  = "".
    END.

    /************* print check stub at top of form ***************/
    find first ap-inv NO-LOCK
        where ap-inv.company eq cocode
          and ap-inv.inv-no  eq ap-payl.inv-no
          and ap-inv.vend-no  eq ap-payl.vend-no
        use-index inv-no.

    if ll eq 0 then
      put skip(2)
          "Vendor No: " at 5
          v-vend-no
          " / "
          "Name:"
          v-vend-name
          skip(1)
          "Invoice"     at 4
          "Reference"   at 17
          "Inv date"    to 44
          "Inv Amt"     to 56
          "Discount"    to 68
          "Payment"     to 80.

    assign
     ll                = ll + 1     
     v-left            = yes
     ctot              = ctot + ap-payl.amt-paid
     cdis              = cdis + ap-payl.amt-disc
     ap-payl.check-date = wdate
     cgrossl           = cgrossl + (ap-payl.amt-paid + ap-payl.amt-disc)
     cgross            = cgross + cgrossl.

    put ap-payl.inv-no             at 4  format "x(12)"
        ap-payl.dscr               at 17 format "x(19)"
        ap-inv.inv-date           to 44 FORM "99/99/99"
        ap-inv.net                to 56 format "->>>,>>9.99"
        ap-payl.amt-disc           to 68 format "->>>,>>9.99"
        ap-payl.amt-paid           to 80 format "->>>,>>9.99".

    cgrossl = 0.

    if ll eq max-per-chk and v-left and not last-of(ap-payl.vend-no) then do:
      assign
       stnum = stnum + 1
       ap-pay.check-no = stnum
       ll = 0.

      display skip(8)
              "******"                                to 79
              skip(1)
              fill("*",70)                            at 10 format "x(70)"
              skip(2)
              fill("*",8)                             to 61 format "x(8)"
              "************"                          to 80
              skip(1)
              "**   **      ****      **     ***** "  at 10
              "**   **     **  **     **     **  **"  at 10
              " ** **      **  **     **     **  **"  at 10
              "  ***        ****      **     ***** "  at 10
              skip(5)

          with frame m no-box no-labels stream-io width 85 no-attr-space.
    end.

    /********************** print check ****************************/
    if last-of(ap-payl.vend-no) then do:
      checks-avail = yes.

      put skip(max-per-chk - ll)
          "--------------"  to 80 skip
          "Check Date = "   at 29
          ap-pay.check-date to 49 FORM "99/99/99"
          "Check Total = "  at 54
          ctot              to 80 skip.

      run ap/apchks.p (input ctot, input 70, output dol).

      display ap-pay.check-no
              ap-pay.check-date
              ctot
              caps(dol)         @ dol
              caps(vend.remit)  @ vend.remit
              caps(add1)        @ add1
              caps(add2)        @ add2
              caps(csz)         @ csz
              vend.check-memo

          with frame b1.

      assign
       stnum  = stnum + 1
       ctot   = 0
       cdis   = 0
       cgross = 0
       dol    = ""
       ll     = 0.
    end.
  end.

  if not(checks-avail) then put "No checks found ready to print!".
end.

