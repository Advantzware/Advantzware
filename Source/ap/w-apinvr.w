...
&SCOPED-DEFINE h_Browse01 h_b-apinvr
&SCOPED-DEFINE h_Object01 h_p-cashl
&SCOPED-DEFINE h_Object02 h_v-apload
...
DEFINE VARIABLE h_b-apinvr AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-apload AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v-apload AS HANDLE NO-UNDO.
...
ON END-ERROR OF W-Win /* Recurring AP Invoices */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Recurring AP Invoices */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Invoices|View Invoices|Load Invoices' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ap/b-apinvr.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-apinvr ).
       RUN set-position IN h_b-apinvr ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-apinvr ( 19.52 , 134.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2':U) NO-ERROR.

       /* Links to SmartNavBrowser h_b-apinvr. */
       RUN add-link IN adm-broker-hdl ( h_p-navico , 'Navigation':U , h_b-apinvr ).
       RUN add-link IN adm-broker-hdl ( h_b-apinvr , 'Record':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-apinvr ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navico ).
       RUN set-position IN h_p-navico ( 11.00 , 7.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-arinv ).
       RUN set-position IN h_p-arinv ( 11.00 , 65.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = One-Record':U ,
             OUTPUT h_p-cashl ).
       RUN set-position IN h_p-cashl ( 22.19 , 37.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-apinvr , 'Record':U , h_v-apinv ).
       ...
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'recurring':U , h_v-apinv ).
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ap/b-apload.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-apload ).
       RUN set-position IN h_b-apload ( 4.57 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-apload ( 17.62 , 134.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ap/v-apload.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-apload ).
       RUN set-position IN h_v-apload ( 22.19 , 54.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.38 , 44.00 ) */

       /* Links to SmartViewer h_v-apload. */
       RUN add-link IN adm-broker-hdl ( h_b-apload , 'load-recurring':U , h_v-apload ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-apload ,
             h_folder , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-apload ,
             h_b-apload , 'AFTER':U ).
    END. /* Page 3 */
