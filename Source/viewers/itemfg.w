...
DEFINE FRAME F-Main
     ...
     itemfg.spare-char-4 AT ROW 12.67 COL 117.8 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN 
          SIZE 13.2 BY 1
     itemfg.subZone AT ROW 12.67 COL 131 COLON-ALIGNED NO-LABEL FORMAT "x(8)"
          VIEW-AS FILL-IN 
          SIZE 11 BY 1
...
/* SETTINGS FOR FILL-IN itemfg.quantityPartial IN FRAME F-Main
   EXP-LABEL EXP-FORMAT EXP-HELP                                        */
/* SETTINGS FOR FILL-IN itemfg.receiveAsRMItemID IN FRAME F-Main
   EXP-LABEL                                                            */
...
/* SETTINGS FOR FILL-IN itemfg.weight-100 IN FRAME F-Main
   EXP-LABEL                                                            */
/* SETTINGS FOR FILL-IN itemfg.weightPerEA IN FRAME F-Main
   EXP-LABEL                                                            */
        ...
        CASE lw-focus:NAME :
           ...
           WHEN "spare-char-4" THEN 
            ...
            WHEN "receiveAsRMItemID" THEN DO:
                ...
                IF char-val <> "" AND char-val NE itemfg.receiveAsRMItemID:SCREEN-VALUE THEN
                    itemfg.receiveAsRMItemID:SCREEN-VALUE = char-val.                
            END.
        END.    
...
/*------------------------------------------------------------------------------
    ...
    for each venditemcost where venditemcost.company = cocode 
        ...
        FOR EACH vendItemCostLevel WHERE vendItemCostLevel.vendItemCostID = vendItemCost.vendItemCostID:
            ...
            buffer-copy vendItemCostLevel except vendItemCostLevel.vendItemCostID venditemcostlevel.rec_key vendItemCostLevel.vendItemCostLevelID to b-vendItemCostLevel.
            assign 
...
/*------------------------------------------------------------------------------
    ...
    IF itemfg.est-no NE "" THEN DO:
        FIND FIRST bf-eb EXCLUSIVE-LOCK
             ...
             NO-ERROR.
        IF AVAILABLE bf-eb THEN
            bf-eb.receiveAsRMItemID = itemfg.receiveAsRMItemID.
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF itemfg.receiveAsRMItemID:SCREEN-VALUE EQ "" THEN
        RETURN.
        
    IF NOT CAN-FIND(FIRST item
                    WHERE item.company  EQ cocode
                      AND item.i-no     EQ itemfg.receiveAsRMItemID:SCREEN-VALUE) THEN DO:
        MESSAGE "Invalid RM Item #, try help..." VIEW-AS ALERT-BOX ERROR.
        
        APPLY "ENTRY" TO itemfg.receiveAsRMItemID.
