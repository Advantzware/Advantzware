...
&SCOPED-DEFINE viewerHandle h_jobBrowse
...
DEFINE VARIABLE h_jobBrowse AS HANDLE NO-UNDO.

/* ************************  Frame Definitions  *********************** */

DEFINE FRAME detailFrame
     SPACE(47.41) SKIP(3.63)
...
ON END-ERROR OF W-Win /* Job Browse - Scheduler */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Job Browse - Scheduler */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-RESIZED OF W-Win /* Job Browse - Scheduler */
DO:
  RUN winReSize.
END.
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
           ...
           &ENDIF
             ...
             INPUT  '':U ,
             OUTPUT h_jobBrowse ).
       RUN set-position IN h_jobBrowse ( 1.00 , 1.00 ) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  jobBrowse = findProgram('{&viewers}/',ID,'/jobBrowse.w').
...
------------------------------------------------------------------------------*/
  ...
  RUN select-page (0).
  RUN passHandle IN {&viewerHandle} (ipHandle,ipBoard,ipResource).
