...
&SCOPED-DEFINE h_Browse01 h_b-arinq
...
DEFINE VARIABLE h_b-arinq AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "Customer Invoices Inquiry"
         HEIGHT             = 24
         WIDTH              = 151.2
         MAX-HEIGHT         = 320
         MAX-WIDTH          = 320
...
ON END-ERROR OF W-Win /* Customer Invoices Inquiry */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* Customer Invoices Inquiry */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Brws Invoices|View Invoice|Credit Status|Bill Notes|BOL' + ',
                     FOLDER-TAB-TYPE = 2':U ,
             OUTPUT h_folder ).
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_printInv ).
       RUN set-position IN h_printInv ( 1.00 , 77.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_optinote ).
       RUN set-position IN h_optinote ( 1.00 , 61.00 ) NO-ERROR.
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_v-arinq ).
       RUN set-position IN h_v-arinq ( 4.81 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_b-arinvl ).
       RUN set-position IN h_b-arinvl ( 13.62 , 4.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-nav2 ).
       RUN set-position IN h_v-nav2 ( 21.48 , 26.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-disabl ).
       RUN set-position IN h_p-disabl ( 21.48 , 92.00 ) NO-ERROR.
       RUN set-size IN h_p-disabl ( 2.38 , 31.00 ) NO-ERROR.

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-arinq. */
       RUN add-link IN adm-broker-hdl ( h_b-arinq , 'Record':U , h_v-arinq ).

       /* Links to SmartNavBrowser h_b-arinvl. */
       RUN add-link IN adm-broker-hdl ( h_b-arinq , 'inquiry':U , h_b-arinvl ).
       ...
       RUN add-link IN adm-broker-hdl ( h_b-arinq , 'nav-itm':U , h_v-nav2 ).
    ...
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v-arcusi ).
       RUN set-position IN h_v-arcusi ( 4.57 , 3.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-arcusi ).
       RUN set-position IN h_b-arcusi ( 11.95 , 26.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-arinq , 'Record':U , h_v-arcusi ).
    ...
    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ar/v-arbil.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-arbil ).
       RUN set-position IN h_v-arbil ( 6.48 , 19.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.48 , 109.60 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_v-arbil. */
       RUN add-link IN adm-broker-hdl ( h_b-arinq , 'Record':U , h_v-arbil ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v-arbil ,
             h_folder , 'AFTER':U ).
    END. /* Page 4 */
    WHEN 5 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b-oeboll ).
       RUN set-position IN h_b-oeboll ( 4.57 , 3.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_b-arinq , 'Record':U , h_b-oeboll ).
...
/*------------------------------------------------------------------------------
  Purpose:     NONE
  Parameters:  <none>
  Notes:       added here so other smartobject in container can function elsewhere
------------------------------------------------------------------------------*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE select_InvNote W-Win 
PROCEDURE select_InvNote :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
   DEF VAR Inv_rec_key_value AS cha NO-UNDO.
   DEF VAR Inv_Header_value AS cha NO-UNDO.

   RUN get-link-handle IN adm-broker-hdl (THIS-PROCEDURE,"record-source", OUTPUT char-hdl).
   RUN get-invNoteKey IN WIDGET-HANDLE(char-hdl) (OUTPUT Inv_rec_key_value, OUTPUT Inv_header_value).
   RUN windows/invnotes.w(Inv_rec_key_value,Inv_HEADER_value).
END PROCEDURE.
