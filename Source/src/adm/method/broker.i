...
&IF DEFINED(EXCLUDE-getlinktables) = 0 &THEN
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE state-changed ADM-Broker 
PROCEDURE getlinktable:
    /* -----------------------------------------------------------
      Purpose:     
      Parameters:  <none>
      Notes:       
    -------------------------------------------------------------*/
    DEFINE INPUT PARAMETER procedureuniqueID AS INTEGER.
    DEFINE OUTPUT PARAMETER objHandList AS CHARACTER .

    FOR EACH adm-link-table WHERE link-source-id = procedureuniqueID 
                               OR link-target-id = procedureuniqueID 
                            BREAK BY link-target :  
        IF FIRST-OF(link-target)THEN
            objHandList = objHandList + "," + string(link-target).
    END. 
    objHandList = TRIM(objHandList,",").
    FOR EACH adm-link-table WHERE link-source-id = procedureuniqueID 
                               OR link-target-id = procedureuniqueID 
                         BREAK BY link-source :    
        IF FIRST-OF(link-source) AND lookup(STRING(link-source),objHandList, ",") < 0 THEN
            objHandList = objHandList + "," + string(link-source).
    END.
    objHandList = TRIM(objHandList,",").
 &global-define EXCLUDE-getlinktables true
END PROCEDURE.
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
&ENDIF
