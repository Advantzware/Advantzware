/* cerep/jobcbox.p copied from cerep/jobdaytn.p              05/26/04 YSK     */
/*  factory ticket  for folding Century Box                                   */
/* -------------------------------------------------------------------------- */
...
DEFINE VARIABLE iSheetRequired AS INTEGER NO-UNDO.
...
ASSIGN
 ...
 v-spec-list = spec-list.
    ...
    {cerep/jobcbox.i}
        ...
        /*  if last-of(eb.form-no) then do: */
          ...
          IF LAST-OF(eb.form-no) THEN DO:
             ...
             PUT "<P10>" v-fill SKIP                       /*REQ'D*/                 
                 "<B>BOARD CODE            DUE DATE    SHEETS SHEET SIZE              DIE SIZE      BOARD PO# VENDOR#   DIE#                 CAD#</B>"
                 SKIP.
            /** PRINT SHEET **/
             x = 2.
             for each wrk-sheet break by wrk-sheet.form-no:
               ...
               display wrk-sheet.brd-dscr
                    v-po-duedate wrk-sheet.gsh-qty FORMAT ">>>>,>>9"
             ...
             IF s-prt-mstandard THEN DO:                
                IF s-run-speed THEN
                   PUT wrk-op.m-dscr   SPACE(5)
                       ...
                       wrk-op.num-sh[job-hdr.frm] FORM ">>,>>>,>>9" SPACE(2)
                       v-output FORM ">>,>>>,>>9"
               ...
               ELSE
                   PUT wrk-op.m-dscr   SPACE(5)
                       wrk-op.mr-waste[job-hdr.frm]   SPACE(5)
                       wrk-op.mr[job-hdr.frm]         SPACE(5)
                       wrk-op.run-hr[job-hdr.frm]  FORM ">,>>9.99"   SPACE(4)
                       /*wrk-op.spoil[job-hdr.frm]*/ v-spoil   FORM ">>,>>9"   SPACE(5)
                       wrk-op.num-sh[job-hdr.frm] FORM ">>,>>>,>>9" SPACE(2)
                       v-output FORM ">>,>>>,>>9"
                       /*v-mat-for-mach FORM "x(60)"    */
     ...
     END. /* lookup(cPrintSheetTicket,"S,B") GT 0*/
      ...
      IF LAST(job-hdr.job) AND lookup(cPrintSheetTicket,"S,B") GT 0 THEN DO:
       ...
       FOR EACH wrk-sheet NO-LOCK
           BREAK BY wrk-sheet.gsh-qty DESC
                 BY wrk-sheet.i-no :
           iSheetRequired = iSheetRequired + wrk-sheet.gsh-qty.
       END.
          IF cPrintSheetTicket EQ "B" THEN
          PAGE.

          PUT  "<OLANDSCAPE><P10><FArial>".
          PUT  "<C+25><#1>".
          PUT  "<=1>" SKIP.
          ...
          PUT "<FArial><=4><R5><C5><B> Sheets Required: </B>" iSheetRequired FORMAT ">>>>>>>9"   .
           ...
           IF FIRST-OF(wrk-sheet.i-no) THEN do:
               j = j + 1 .
                   ...
                   IF j EQ 1 THEN do:
                       ...
                       PUT "<FArial><R14><C5><B> Actual Sheets: "  SKIP .
                   ...
                   ELSE IF j EQ 2 THEN DO:
                       ...
                       PUT "<FArial><=6><R14><C52><B> Actual Sheets: "  SKIP .
                       PUT "<FArial><=6><R15><C52><B> Board PO: " STRING(v-board-po,">>>>>>9") FORMAT "x(10)" "</B>" .
                   ...
                   ELSE IF j EQ 3 THEN do:
                       ...
                       PUT "<FArial><R23><C5><B> Actual Sheets: "  "</B>" SKIP.
                   ...
                   ELSE IF j EQ 4 THEN DO:
                       ...
                       PUT "<FArial><R23><C52><B> Actual Sheets: " "</B>" .
                   ...
                   ELSE IF j EQ 5 THEN do:
                       ...
                       PUT "<FArial><R33><C5><B> Actual Sheets: "  "</B>" SKIP.
                   ...
                   ELSE IF j EQ 6 THEN do:
                       ...
                       PUT "<FArial><R33><C52><B> Actual Sheets: "  "</B>" .
