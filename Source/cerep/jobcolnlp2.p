...
DEFINE VARIABLE v-dept-inst      AS CHARACTER     FORMAT "x(80)" EXTENT 100 NO-UNDO.
...
/*                    v-cust-lot#[1] = IF AVAILABLE reftable THEN reftable.CODE ELSE "" .*/
        ...
        PUT "<B> Customer Name:</B>" v-cust-name FORM "x(25)" "<B>Acct Code:</B> " job-hdr.cust-no 
            ...
            v-shipto[4] AT 9 "Pharma Code:" AT 41 v-upc-no  /*v-ship-date[4] AT 61 v-due-qty[4] AT 71 v-po-no[4] FORM "x(15)" AT 85 v-cust-lot#[4]  AT 98 FORM "x(15)"*/ TRIM(job-hdr.est-no) AT 116 SKIP 
            v-fill SKIP.
        ...
        DO:
            ...
            /*break by wrk-sheet.form-no*/ :  
                ...
                DISPLAY
                    ...
                    ITEM.i-name  FORMAT "x(23)"  
                    ef.xgrain FORMAT "x(2)" SPACE(1)
/*                     v-lbs SPACE(2) */
                    FILL(" ",8 - LENGTH(STRING(wrk-sheet.sh-wid))) + STRING(wrk-sheet.sh-wid)
                    SPACE (1)
                    (IF AVAILABLE job-mat THEN job-mat.len  ELSE ef.gsh-len) FORMAT ">>9.9999"
                    SPACE(2)
              ...
              FIND FIRST job-mat NO-LOCK
                ...
                IF FIRST(wrk-film.leaf) THEN
                    PUT "<P10>" SKIP
                        ...
                        SKIP.
                DISPLAY 
                    SPACE(1) wrk-film.leaf FORMAT "x(15)"
                    (IF AVAIL ITEM THEN ITEM.i-name ELSE "") FORMAT "x(28)" SPACE(2)
...
DO:
    ...
    FOR EACH notes NO-LOCK WHERE notes.rec_key EQ job.rec_key
        ...
        DO:
            ...
            FOR EACH tt-formtext:
                i = i + 1.
                IF  i LE 100 THEN v-dept-inst[i] = tt-formtext.tt-text.      
            END.
            ...
            DO:
                ...
                DISPLAY v-fill SKIP
                    ...
                    WHEN v-fgitm[2] <> ""  AT 45
                     TRIM(STRING(DYNAMIC-FUNCTION('sfFormat_JobFormatWithHyphen', v-job-no, v-job-no2))) FORM "x(13)"
                    WHEN v-fgitm[2] <> "" 
                    "Job#:" 
