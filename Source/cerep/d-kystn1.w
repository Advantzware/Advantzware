...
&Scoped-Define ENABLED-OBJECTS RECT-40 RECT-41 v-aql v-style ~
v-style-2 Btn_OK Btn_Cancel 
&Scoped-Define DISPLAYED-OBJECTS v-i-no v-aql v-style v-style-2 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-1 v-i-no v-aql v-style v-style-2 v-newsize ~
v-tdesc v-tqty v-mb 
...
------------------------------------------------------------------------------*/
  DISPLAY v-i-no v-aql v-style v-style-2 
      WITH FRAME Dialog-Frame.
  ENABLE RECT-40 RECT-41 v-aql v-style v-style-2 Btn_OK Btn_Cancel 
      WITH FRAME Dialog-Frame.
...
------------------------------------------------------------------------------*/
  ...
  DO li = 1 TO 9:
    ...
    CASE li:
      ...
      WHEN 03 THEN IF ip-get THEN v-style       = reftable.dscr.
                             ELSE reftable.dscr = v-style.
      WHEN 04 THEN IF ip-get THEN v-style-2     = reftable.dscr.
                             ELSE reftable.dscr = v-style-2.
      WHEN 05 THEN IF ip-get THEN v-newsize     = reftable.dscr.
                             ELSE reftable.dscr = v-newsize.
      WHEN 06 THEN IF ip-get THEN v-tdesc       = reftable.dscr.
                             ELSE reftable.dscr = v-tdesc.
      WHEN 07 THEN IF ip-get THEN v-tqty        = INT(reftable.dscr).
                             ELSE reftable.dscr = STRING(v-tqty).
      WHEN 08 THEN IF ip-get THEN v-mb          = reftable.dscr.
