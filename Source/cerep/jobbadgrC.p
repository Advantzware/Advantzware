/* ---------------------------------------------------- cerep/jobbadgrC.p     */
/*  factory ticket  for  corrugated Badger                                    */
/* -------------------------------------------------------------------------- */
...
format header
       " JOB NUMBER:<B>" TRIM(STRING(DYNAMIC-FUNCTION('sfFormat_JobFormatWithHyphen', v-job-no, v-job-no2))) FORM "x(13)"  SPACE(1)  v-reprun  "</B>" 
...
for each job-hdr NO-LOCK
      ...
      if v-first then do:
       ...
       FIND first eb WHERE eb.company     EQ job-hdr.company
        ...
        PUT "<B> Customer Name:</B>" v-cust-name  "<B>Acct Code:</B> " job-hdr.cust-no 
            "<B> SHIP DATE:    QTY DUE:  PO#:     Estimate:        Print Date:" SKIP
            " Shipto:</B>" v-shipto[1] v-ship-date[1] AT 70 v-due-qty[1] AT 79  v-po-no[1] FORMAT "x(8)" AT 94 TRIM(job-hdr.est-no) FORMAT "x(8)" AT 103
            TODAY FORMAT "99/99/9999" AT 120 SKIP  
            v-shipto[2] AT 9 v-ship-date[2] AT 66 v-due-qty[2] AT 75 v-po-no[2] AT 90 SKIP  
            v-shipto[3] AT 9 "<B>QC/SPC#</B>:" AT 41 v-spc-no v-ship-date[3] AT 66 v-due-qty[3] AT 75 v-po-no[3] AT 90 SKIP
            v-shipto[4] AT 9  v-ship-date[4] AT 66 v-due-qty[4] AT 75  v-po-no[4] AT 90 SKIP 
            v-fill SKIP.
      ...
      for each ef
        ...
        for each eb
          ...
          IF LAST-OF(eb.form-no) THEN DO:
            ...
            PUT "<P10>" 
                "<B> STOCK CODE  DESCRIPTION         WEIGHT     WIDTH   CYLINDER  DIE SIZE              DIE#               PRINT QTY      PRINT FEET</B>" 
                SKIP.
      ...
      if last-of(job-hdr.frm) then do:
        ...
        FOR EACH notes WHERE notes.rec_key = job.rec_key
            ...
            IF FIRST-OF(notes.note_code) THEN DO:
               ...
               FOR EACH tt-formtext:
                   i = i + 1.
                   IF  i <= 20 THEN v-dept-inst[i] = tt-formtext.tt-text.      
               END.

               IF first(notes.note_code) THEN PUT "<B> DEPARTMENT   INSTRUCTION NOTES</B>" SKIP.
              /*MESSAGE "page  " + STRING(PAGE-SIZE) VIEW-AS ALERT-BOX ERROR.*/
                 DO i = 1 TO 20:
                     IF PAGE-SIZE - LINE-COUNTER < 8 THEN PAGE.
                     IF v-dept-inst[i] NE "" THEN
                  PUT " " v-dept-inst[i] FORM "x(128)" SKIP.
                 END.
...
ELSE  DO:
        ...
        FOR EACH tt-fgitm BY tt-fgitm.seq.
          ...
          ASSIGN i = i + 1
                 ...
                 j = j + 1.
          IF i >= 3 THEN DO:
              IF PAGE-SIZE - LINE-COUNTER < 20 THEN PAGE.
...
END. /* s-prt-label*/           
    ...
    PUT "<R48.5><C1>FORM -0046 9/01/09" .
