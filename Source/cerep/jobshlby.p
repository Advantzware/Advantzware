/* cerep/jobshlby.p   Xprint FC Factory  Ticket for Shelby */
/* -------------------------------------------------------------------------- */
...
DEF TEMP-TABLE tt-ink NO-UNDO FIELD i-code LIKE wrk-ink.i-code
...
format header
       "<TOP=10mm><B>  JOB NUMBER:<P18><R-0.3><P14>" TRIM(STRING(DYNAMIC-FUNCTION('sfFormat_JobFormatWithHyphen', v-job-no, v-job-no2))) FORM "x(13)" "</B><P10><R+0.3>"
       "<C32><B><P12>S H E L B Y   F A C T O R Y   T I C K E T<P11>"  "<C77>JOB START DATE:"  v-start-date /*"  PRINTED DATE:" TODAY*/  "</B><P10>" skip
       v-fill
...
for each job-hdr NO-LOCK
    ...
    if v-first then do:   
       ...
       ELSE PUT SKIP
            "<TOP=10mm><B>  JOB NUMBER:<P18><R-0.3>" TRIM(STRING(DYNAMIC-FUNCTION('sfFormat_JobFormatWithHyphen', v-job-no, v-job-no2))) FORM "x(13)" "</B><P10><R+0.3>"
            "<C32><B><P12>S H E L B Y  F A C T O R Y   T I C K E T<P11>"  "<C77>JOB START DATE:" v-start-date /*"  PRINTED DATE: " TODAY */ "</B><P10>" skip
            v-fill SKIP.
       view frame head.
       VIEW FRAME bott.
...
FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
    ...
    IF FIRST-OF(bf-jobhdr.ord-no) THEN DO:
       ...
       PUT "  <B>CUSTOMER NAME:<P18><R-0.3>" v-cust-name  "<P10><R+0.3>"
            "<B>DUE DATE:         ESTIMATE:         " AT 97 SKIP
            ...
            v-shipto[4] AT 10 "<B>OVERRUN:</B> " AT 75 v-over% "   <B>UNDERRUN:</B> "  v-under% SKIP
            v-fill SKIP.     
...
END. /* for each bf-jobhdr*/
     ...
     FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
        ...
        IF FIRST(bf-jobhdr.frm) THEN
           PUT "  <P10><B>F/B   FG ITEM/PO #    #UP    FORM/JOB QTY  NAME/DESCRIPTION          STYLE  CARTON SIZE             COUNT CASE CODE  DUE DATE " /*v-upc-lbl*/ "</B>" SKIP.
...
/*==== 
                ...
                if eb.i-code2[i] eq job-mat.i-no then do:
                   find first wrk-ink
                    ...
                    if not avail wrk-ink then do: 
                       ...
                       wrk-ink.i-dscr   = item.i-name /*eb.i-dscr2[i]*/
            ...
            find first wrk-ink where wrk-ink.i-code    eq job-mat.i-no
                /*and wrk-ink.form-no   eq job-mat.frm
                and (wrk-ink.blank-no eq job-mat.blank-no OR est.est-type     eq 4) */
                no-error.                
            if not avail wrk-ink then do:
              create wrk-ink.
              assign
     ...
     /*
        ...
        for each xjob-hdr
            where xjob-hdr.company eq cocode
              ...
              and xjob-hdr.i-no    eq bf-jobhdr.i-no
            no-lock:
      ...
      /*    if est.est-type eq 4 and eb.stock-no ne job-hdr.i-no then next ebloop. */
        ...
        /*  if last-of(eb.form-no) then do: */
            ...
            FOR EACH bf-fg-bin NO-LOCK where bf-fg-bin.company eq eb.company
                                      ...
                                      /* AND bf-fg-bin.job-no LE job-hdr.job-no
                                       AND bf-fg-bin.job-no2 LE job-hdr.job-no2*/
            ...
            display " " trim(string(eb.form-no,">>9")) + "-" +
                    ...
                    v-up FORM ">9"
                    v-ord-qty /*v-job-qty * v-fac - changed after task 05091201 & discussion with BJ */ format "->>,>>>,>>9" SPACE(7)
                    /*eb.part-no*/
                    v-item-name FORM "x(25)"
                    eb.style /*v-stypart */
                    v-size[1] FORM "x(23)" /* was 15*/
                    eb.cas-cnt 
                    oe-ordl.cas-cnt WHEN AVAIL oe-ordl AND oe-ordl.cas-cnt <> 0 @ eb.cas-cnt
                    eb.cas-no
                    v-cas-no WHEN v-cas-no <> "" @ eb.cas-no
                    v-case-due-date
                    skip
                    v-po-no FORM "x(15)" AT 9
                    v-job-qty /*v-est-qty - changed after task 05091201 & discussion with BJ*/ FORM  "->>,>>>,>>9"         AT 28 SPACE(7)
                    v-dsc[1] FORM "x(25)"  
                    "PREV:"  v-prev-job FORM "x(21)"
                    v-job-cnt FORM ">>>>9" AT 103
    ...
    END. /*tt-reftable*/
     ...
     PUT "<P10>" /*v-fill SKIP*/                       /*REQ'D*/
         "<B>F#  BOARD CODE                CAT  DUE DATE      SHEETS SHEET SIZE       HG  NET SHEET/DIE    VEN NAME    LAYOUT#     CAD # </B>" 
         SKIP.
     ...
     FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
            ...
            /** PRINT SHEET **/
             for each wrk-sheet WHERE wrk-sheet.form-no = tt-reftable.val[12] break by wrk-sheet.form-no:
                 ...
                 /*IF AVAIL po-ord THEN
                    ...
                    v-po-duedate = IF AVAIL po-ordl THEN po-ordl.due-date ELSE ?.
                 */  
                 FOR EACH po-ordl NO-LOCK WHERE po-ordl.company = job-hdr.company
                                            AND po-ordl.i-no = wrk-sheet.i-no
                                            AND po-ordl.opened 
               ...
               PUT UNFORMATTED  wrk-sheet.form-no FORM ">9" " "
                       " " wrk-sheet.brd-dscr FORM "x(25)" " "
                       v-cat FORM "x(5)"
                    v-po-duedate FORM "99/99/9999" " " 
                    wrk-sheet.gsh-qty FORM ">,>>>,>>9" " "
                  /*  string(wrk-sheet.sh-wid) + "x" + string(wrk-sheet.sh-len) decimal */
                    v-sht-size format "x(16)" " "
                    v-hg " "
                    v-net-size  FORM "x(16)"
                    /*v-board-po*/  " "
                    v-vend   FORM "x(10)" " "   
                   /* string(ef.nsh-wid) + "x" + string(ef.nsh-len) FORM "x(30)"
                    v-board-po  
                    v-vend  */
                    /*string(ef.trim-w) + "x" + string(ef.trim-l) FORM "x(20)" decimal*/
                    eb.die-no FORM "x(10)"  
                    eb.cad-no FORM "x(10)"           SKIP
      ...
      for each wrk-ink WHERE /*wrk-ink.form-no = eb.form-no*/
                ...
                IF first-OF(wrk-ink.i-code) /*OR v-ink-use-per-blank > 1 */ THEN DO: 
                   ...
                   IF AVAIL ITEM THEN DO:                          
                            IF item.min-lbs > v-i-qty THEN
                              ASSIGN v-i-qty = item.min-lbs.
                   END.
         
                   ASSIGN v-ink1[i] = string(wrk-ink.i-code,"X(15)") + " " + 
                                      string(wrk-ink.i-dscr,"x(29)") + "  " + /*trim(v-alloc) + " " + */
                                      STRING(v-i-qty,">>>,>>9")
                                            /*v-item[i]*/
      ...
      put v-fill at 1 skip.
      IF s-run-speed THEN
           PUT "<B>FORM MACHINE                MR WASTE   MR HRS  RUN SPEED   SPOIL%     SHEETS  " /* MATERIAL FOR MACHINE          SIZE   TOTAL REQUIRED</B>"  */
               /*"<C72> CAD#         COUNT  CASE SIZE                  VENDOR PART# */ "</B>"     
                SKIP.
      else PUT "<B>FORM MACHINE                MR WASTE   MR HRS   RUN HOUR    SPOIL%     SHEETS  " /* MATERIAL FOR MACHINE          SIZE   TOTAL REQUIRED</B>" */
               /*"<C72> CAD#         COUNT  CASE SIZE                  VENDOR PART#*/ "</B>"                    
               SKIP.
      FOR EACH tt-size:
          DELETE tt-size.
         ...
         FOR EACH wrk-op WHERE wrk-op.s-num = tt-reftable.val[12] BREAK by wrk-op.d-seq by wrk-op.b-num:
             ...
             ELSE PUT wrk-op.s-num "  " wrk-op.m-dscr   SPACE(5)
                     /* wrk-op.mr-waste[job-hdr.frm]   */ SPACE(10)
                      ...
                      /*wrk-op.speed[job-hdr.frm] >>>>9*/      SPACE(18)
                      /*wrk-op.spoil[job-hdr.frm]   >>9.99   SPACE(19)/* "  |    "*/*/
                    /*  v-mat-for-mach FORM "x(60)" */
                    wrk-op.num-sh[wrk-op.s-num] SPACE(3)
                      .
             i = i + 1.
             IF i MOD 2 <> 0 THEN PUT UNFORMATTED "_______________________________________________".
             PUT SKIP.       
             IF LINE-COUNTER > 47 THEN PAGE.
     ...
     /* PUT v-fill AT 1 SKIP. */
      ...
      FOR EACH bf-jobhdr NO-LOCK WHERE bf-jobhdr.company = job-hdr.company
       ...
       IF FIRST(bf-jobhdr.frm) THEN DO:
        ...
        FOR EACH notes WHERE notes.rec_key = job.rec_key and notes.note_code <> '' AND LOOKUP(notes.note_code,v-exc-depts) EQ 0 NO-LOCK,
            FIRST dept NO-LOCK WHERE dept.CODE = notes.note_code BREAK BY notes.note_form_no BY dept.fc:
            v-inst2 = "".
            IF v-prev-note-rec <> ? AND
               v-prev-note-rec <> RECID(notes) THEN v-prev-extent = /*v-prev-extent + k*/ lv-note-cnt.
            /*FIND FIRST dept WHERE dept.CODE = notes.note_code NO-LOCK NO-ERROR.*/
            lv-form-note = /*(IF AVAIL dept THEN "<B>" + caps(dept.dscr) + "</B>" ELSE caps(note.note_code)) +
            ...
            /* PRINT*/
            /*<ADJUST=LPI>*/
            PUT "<LPI=4.3><P16>" NOTEs.note_form_no "  " "<B>" + caps(trim(dept.dscr)) + "</B>" FORM "x(22)"
