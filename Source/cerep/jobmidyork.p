...
DEF VAR v-stock-item       AS CHAR NO-UNDO.
DEF VAR v-item-desc        AS CHAR NO-UNDO.
DEF VAR v-brd-dscr          AS CHAR FORMAT "x(17)" NO-UNDO.
DEF VAR v-lar-dep          AS CHAR  NO-UNDO.

{custom/notesdef.i}
{cerep/tt-wrk-ink.i "SHARED" }

FUNCTION ttrim RETURNS CHARACTER
  ( ip-dec AS DECIMAL ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
  def var lv-ret-value as CHAR no-undo.
lv-ret-value = trim(TRIM(trim(trim(string(ip-dec,">9.99999")),"0")), ".").
  

  RETURN lv-ret-value.   /* Function return value. */
...
FORMAT 
   ...
   "Salesman:" AT 68 sman.sNAME "Order#:" AT 113 oe-ord.ord-no
...
FOR EACH job-hdr NO-LOCK WHERE job-hdr.company               EQ cocode
   ...
   IF v-first THEN DO: 
      ...
      PUT 
         ...
         "<B>REQ DATE:   DUE DATE:   Estimate:" SPACE(3) 
         "<B>Sales Rep:            CSR:     PO#"   SKIP
         "Shipto:</B>" v-shipto[1] v-req-date AT 49 v-due-date AT 61 TRIM(job-hdr.est-no) FORMAT "x(8)" AT 74  /*oe-ord.sname[1]*/sman.sNAME FORMAT "x(20)" AT 85 job.USER-ID FORMAT "x(8)" AT 107 v-po-no AT 116 FORMAT "x(15)"
         SKIP
...
/*                END.                                                                            */
            ...
            IF FIRST-OF(eb.form-no) THEN do:
               ...
               PUT 
                  "<R-1><P9><B> Qty Ordered " AT 29 SKIP
                  "<P9><B>F/B   FG Item #    Qty Ordered  with Overage  Part#          Description           #UP    Last Job# Case#       Count  Case/Pal  #Cases " "</B>" 
               SKIP.
            END.
            ASSIGN
               v-job-qty = 0
            ...
            DISPLAY 
               ...
               eb.stock-no @ job-hdr.i-no FORMAT "x(15)"
               v-est-qty /** v-fac*/ /*AT 18 */ FORMAT "->>>>>>9"
               v-job-qty FORMAT "->>>>>>9"
               eb.part-no AT 46
               v-itemfg-i-name FORMAT "x(21)"  
               v-up AT 84
               v-last-job AT 94
               eb.cas-no  AT 102
               v-case-cnt FORMAT ">>>>9"
            ...
            /*IF v-itemfg-part-dscr1 <> "" THEN*/
               DISPLAY
                  v-itemfg-part-dscr1 AT 62
                  TRIM(string(eb.lp-len) + "x" + STRING(eb.lp-wid) + "x" + STRING(v-lar-dep)) AT 102 
                  eb.lp-up AT 111  
                  SKIP
            ...
            IF LAST-OF(eb.form-no) THEN DO:
               ...
               ASSIGN
                  ...
                  v-stock-item = IF AVAIL oe-ordl THEN oe-ordl.i-no ELSE " "
                  v-item-desc = IF AVAIL oe-ordl THEN oe-ordl.i-name ELSE " ". 
                  /*v-bord-po = IF AVAIL oe-ordl THEN oe-ordl.po-no-po ELSE 0 .*/
               ...
               PUT "<P10>" v-fill SKIP                       /*REQ'D*/
                  "<B>Board Po#   Board Description  SHEETS NET SHEET    Carton Size                DIE#        Type File#    TOTAL UP  CAD#</B>"
               SKIP.
               ...
               FOR EACH wrk-sheet BREAK BY wrk-sheet.form-no:
                  ...
                  FIND FIRST ITEM WHERE ITEM.i-no EQ ef.board
                                    AND ITEM.company EQ cocode NO-LOCK NO-ERROR.
                   ASSIGN v-brd-dscr = ITEM.i-name.
                  DISPLAY
                     ef.board FORMAT "x(10)"
                     v-brd-dscr AT 13 
                     /*v-stock-item FORMAT "x(15)"
                     v-item-desc AT 17 FORMAT "x(20)"
                     b-item.procat
                     b-ITEM.cal*/
                     wrk-sheet.gsh-qty FORM ">>>,>>9"
                     v-net-sheet FORM "x(12)"
                     ttrim(eb.len) + "x" + ttrim(eb.wid) + "x" + ttrim(eb.dep) 
                        FORMAT "x(26)" 
                     /*v-die-size  AT 70 FORM "x(20)"*/
                     eb.die-no AT 79 FORMAT "x(10)"
                     itemfg.prod-notes AT 91 FORMAT "x(15)"
                     v-up-cnt AT 108
                     eb.cad-no AT 114
...
/*          END.                                                             */
      ...
      {custom/notespr4.i job v-inst2 20 "notes.rec_key = job.rec_key AND notes.note_code <> '' AND notes.note_form_no = job-hdr.frm and LOOKUP(notes.note_code,v-exc-depts) EQ 0"}
