...
/*--------------------------------------------------------------------------
    ...
    Created     : 12/01/97
    Notes       : Modified by Consultingwerk to improve error handling, when
                  no AppServer connection can be made (structured error 
                  handling)
...
DEFINE VARIABLE oException  AS AppServerConnectionException             NO-UNDO.
...
/* Do the connection */
ASSIGN ret = conn-hdl:CONNECT(cConnString, usr-id, psswrd, app_server_info) NO-ERROR .
IF NOT ret THEN DO ON ERROR UNDO, THROW:
    
    oException = NEW AppServerConnectionException (partition_name) . 
    
    IF ERROR-STATUS:GET-MESSAGE (1) > "":U THEN 
        oException:AddMessage (ERROR-STATUS:GET-MESSAGE(1),
...
IF tt-file ne ? THEN DO ON ERROR UNDO, THROW:  /* We found the file */
  INPUT FROM VALUE(tt-file) NO-ECHO.
  /* Mike Fechner, Consultingwerk Ltd. 30.05.2013
     When there is an error loading a record from the appsrvtt.d file,
     throw error which will return error from the main block */
  REPEAT TRANSACTION ON ERROR UNDO, THROW:
    ...
    IF AppSrv-TT.Partition = "":U THEN DO:
        DELETE AppSrv-TT .
        NEXT .
    END .
...
FOR EACH AppSrv-TT NO-LOCK:
  ...
  IF AppSrv-TT.App-Service = "":U AND AppSrv-TT.Host = "":U AND
     AppSrv-TT.Service = "":U AND AppSrv-TT.Configuration = NO THEN
    DELETE AppSrv-TT.
  ELSE
