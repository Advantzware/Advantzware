...
DO ON ERROR UNDO, THROW:
    ...
    DEFINE VARIABLE cWinKitEmbedWindow AS CHARACTER NO-UNDO . 
    ...
    ASSIGN cWinKitEmbedWindow = SEARCH ("winkit/embedwindow.p":U) .
    
    IF cWinKitEmbedWindow = ? THEN 
        ASSIGN cWinKitEmbedWindow = "src/winkit/embedwindow.p":U .
       
    DO ON ERROR UNDO, THROW:
        RUN VALUE(cWinKitEmbedWindow) ({&WINDOW-NAME}:HANDLE, "{&WinKitFormType}":U, OUTPUT oForm) . 
