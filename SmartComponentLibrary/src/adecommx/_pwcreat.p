...
DEFINE OUTPUT PARAMETER poForm      AS Consultingwerk.Studio.ProcedureEditor.ProcedureEditorForm NO-UNDO . 
...
ASSIGN
  ...
  TRIGGERS:
    ...
    ON HELP           PERSISTENT RUN adecomm/_pwhelp.p ( INPUT "KEYWORD":U ).
...
/* Mike Fechner, Consultingwerk Ltd. 18.12.2009
   Create WindowContainer and assign WindowHandle */

&scoped-define WINDOW-NAME p_Window
&scoped-define WinKitFormType Consultingwerk.Studio.ProcedureEditor.ProcedureEditorForm
&scoped-define NOMENU
&scoped-define WinKitIgnoreCustomizations

{src/adecommx/embedwindow.i}

oForm:MakeMdiChild = TRUE . 

ASSIGN poForm = oFormControl . 

ASSIGN p_Window:SENSITIVE      = YES
       p_Window:VISIBLE        = YES .
    
ON WINDOW-CLOSE OF p_Window DO:
    ERROR-STATUS:ERROR = FALSE NO-ERROR . 
    
    RUN adecomm/_pwclose.p.
    
    IF ERROR-STATUS:ERROR THEN RETURN . 
    
    {src/adecommx/closewindow.i}
END.    
