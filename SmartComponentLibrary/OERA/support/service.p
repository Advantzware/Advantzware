 ...
 do:
   ...
   run fetchData in hEntity
                   (pcTables,
                    ...
                    no-error.
   if error-status:error then return error if return-value > '' then return-value else error-status:get-message(1) .
...
------------------------------------------------------------------------------*/
    ...
    END.
    ELSE DO:
        ASSIGN pcContext = SUBSTRING(pcContext, 1, iPos) + CHR(1) + cEntityContext.
    END.
...
----------------------------------------------------------------------------*/
 define input        parameter pcEntity  as character no-undo.
 define input-output parameter phDataset as handle. 
...
------------------------------------------------------------------------------*/
    ...
    IF iPos > 0 THEN
        DYNAMIC-FUNCTION ("setContext":U IN phEntity, SUBSTRING(pcContext, iPos + 1)) .  
    ELSE 
...
function getPhysicalName returns character private 
  ...
  if getTypePath(pcType) = '' then       
    return lc(pcType) + caps(substring(pcName,1,1)) 
                      + lc(substring(pcName,2)). 
  else 
    return lc(pcName).
