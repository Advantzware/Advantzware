...
DEFINE {&ACCESS} TEMP-TABLE ttColumnProperties NO-UNDO {&REFERENCE-ONLY} &IF DEFINED (NO-BEFORE) EQ 0 &THEN BEFORE-TABLE eTempTable1Before &ENDIF
    ...
    FIELD CalculatedColumn AS LOGICAL FORMAT "yes/no":U INITIAL "no":U
    FIELD Hidden AS LOGICAL FORMAT "yes/no":U INITIAL "no":U
    FIELD ExcludeFromColumnChooser AS LOGICAL FORMAT "yes/no":U INITIAL "no":U
    FIELD Width AS INTEGER FORMAT ">>>,>>>,>>9":U
    FIELD Caption AS CHARACTER FORMAT "X(50)":U
    FIELD ToolTipText AS CHARACTER FORMAT "X(50)":U
    FIELD AllowSorting AS LOGICAL FORMAT "yes/no":U INITIAL "yes":U
    FIELD AllowRowFiltering AS LOGICAL FORMAT "yes/no":U INITIAL "yes":U
    FIELD AllowRowSummaries AS CHARACTER FORMAT "x(20)":U
    FIELD AllowGroupBy AS LOGICAL FORMAT "yes/no":U INITIAL "yes":U
    FIELD GroupByMode AS CHARACTER FORMAT "X(40)":U
    FIELD HiddenWhenGroupBy AS LOGICAL FORMAT "yes/no":U INITIAL "no":U
    ...
    FIELD EnabledForAdd AS LOGICAL FORMAT "yes/no":U INITIAL "yes":U
    FIELD EnabledForUpdate AS LOGICAL FORMAT "yes/no":U INITIAL "yes":U
    FIELD HeaderFixed AS LOGICAL FORMAT "yes/no":U INITIAL "no":U
