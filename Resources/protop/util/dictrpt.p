 ...
 *
 * this program can be run either stand-alone or from within ProTop via the ctrl-d command
 *
 *
 * december 9, 1988 */
...
rptOut = substitute( "&1/&2.dict.rpt", pt_rptdir, max( pt_shortname, ldbname(1))).
...
for each _file no-lock where
      _file._file-num > 0 and
      _file._file-num < 10000
    by _file._file-name:
...
for each _sequence no-lock:
  display
    _seq-Name format  "x(40)"          label "Name"
    _seq-Num  format  ">>>9"           label "Num"
    _seq-Init format  "->>>>>>>>>>>>9" label "Init"
    _seq-Incr format  "->>>>>>9"       label "Incr"
    _seq-Min  format  "->>>>>>>>>>>>9" label "Min"
    _seq-Max                           label "Max"
    _cycle-Ok                          label "Cyc"
   with width 132
  .
...
for each _file no-lock where
  ...
  for each _field no-lock
      where _field._file-recid = recid( _file )
        and _data-type matches "*lob*":

    find first _storageobject no-lock where _object-type = 3 and _object-number = _fld-stlen.
    find first _area no-lock where _area._area-number = _storageObject._area-number.
