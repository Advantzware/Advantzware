...
DEFINE FRAME DEFAULT-FRAME
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 80.8 BY 15.43 WIDGET-ID 100.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE
   ON END-KEY UNDO MAIN-BLOCK, LEAVE:
    ...
    DISCONNECT audit NO-ERROR.
    
    ASSIGN 
        cCmdLine = cUpdCompressDir + "\7z.exe x " + cUpdProgramDir + "\embedded.7z -y -o" + cUpdatesDir.

    OS-COMMAND SILENT VALUE(cCmdLine).
    IF SEARCH (cUpdatesDir + "\startUpdate.bat") NE ? THEN DO:
        OS-COMMAND SILENT VALUE(cUpdatesDir + "\startUpdate.bat").
        ASSIGN 
            oplSuccess = TRUE.
    END.
    ELSE DO:
        MESSAGE 
            "The included patch files did not extract correctly." SKIP 
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN
        ...
        cUpdAdminDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdAdminDir
        cUpdCompressDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdCompressDir
        cUpdDataDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdDataDir
        cUpdDeskDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdDeskDir
        cUpdMenuDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdMenuDir
        cUpdProgramDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdProgramDir
        cUpdRelNotesDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdRelNotesDir
        cUpdStructureDir = cUpdatesDir + "\PATCH" + ipcToVer + "\" + cUpdStructureDir
