<%@ Page Language="C#" AutoEventWireup="true" Inherits="upc_lookup" Codebehind="upc_lookup.aspx.cs" %>
  ...
  <td id="tdSearch" runat="server" class="shade" vAlign="middle" align="center" width="800">&nbsp;                
                <B>Search for:&nbsp; </B>&nbsp;&nbsp;&nbsp;
                  <asp:dropdownlist id="ddlSearchField" runat="server">                  
                      <%--<asp:ListItem Value="upc-no">Upc No </asp:ListItem>  --%>
                      <asp:ListItem Value="upc">Upc No </asp:ListItem>                        
                  </asp:dropdownlist>&nbsp;&nbsp;
            ...
            <Columns>
                <asp:TemplateField>
                  <ItemStyle HorizontalAlign=Center Wrap="False" />
                   <ItemTemplate>       
		    <a href="#" onClick="javascript:top.opener.window.UPCLookup('<%#DataBinder.Eval(Container,"DataItem.vUpc")%>','<%#DataBinder.Eval(Container,"DataItem.vitemno")%>');window.close();">Select</a>             	    
                   </ItemTemplate>
                </asp:TemplateField> 
                <asp:BoundField DataField="vUpc" HeaderText="Upc No" SortExpression="vUpc" />
                <asp:BoundField DataField="vitemno" HeaderText="item" SortExpression="vitemno" />
                <asp:BoundField DataField="vitemname" HeaderText="Item-Name" SortExpression="vitemname" />
                
            </Columns>
        </asp:GridView>
        <input type="button" name="close" class="buttonM" id="close" value="Close" onclick="javascript:window.close()" />
        <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" OldValuesParameterFormatString="original_{0}"
            SelectMethod="SelectCadLook" TypeName="LookUp">
            <SelectParameters>               
