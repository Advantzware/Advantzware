<%@ Page Language="C#" AutoEventWireup="True" Debug="true" Codebehind="return_tag_lookup.aspx.cs" Inherits="retrntaglookup" %>
  ...
  <td id="tdSearch" runat="server" class="shade" vAlign="middle" align="center" width="800">&nbsp;                
    ...
    <div>
            ...
            <Columns>
             <asp:TemplateField>
                  <ItemStyle HorizontalAlign=Center Wrap="False" />
                   <ItemTemplate>       
		    <a href="#" onClick="javascript:top.opener.window.retrntaglookup('<%#DataBinder.Eval(Container,"DataItem.tag")%>','<%#DataBinder.Eval(Container,"DataItem.vLoc")%>','<%#DataBinder.Eval(Container,"DataItem.vLocBin")%>','<%#DataBinder.Eval(Container,"DataItem.vJobNo")%>','<%#DataBinder.Eval(Container,"DataItem.vJob2")%>','<%#DataBinder.Eval(Container,"DataItem.vINo")%>','<%#DataBinder.Eval(Container,"DataItem.vIName")%>','<%#DataBinder.Eval(Container,"DataItem.unit")%>','<%#DataBinder.Eval(Container,"DataItem.vunitCount")%>','<%#DataBinder.Eval(Container,"DataItem.vunitprplt")%>','<%#DataBinder.Eval(Container,"DataItem.cost")%>','<%#DataBinder.Eval(Container,"DataItem.vUom")%>','<%#DataBinder.Eval(Container,"DataItem.totqty")%>','<%#DataBinder.Eval(Container,"DataItem.extcost")%>');window.close();">Select</a>             	    
                   </ItemTemplate>
                </asp:TemplateField>     
                 <asp:BoundField DataField="tag" HeaderText="Tag#" SortExpression="tag" ItemStyle-Wrap="false" />
                  ...
                  <asp:BoundField DataField="plltcnt" HeaderText="Pallet/Count" SortExpression="plltcnt" ItemStyle-Wrap="false"/>
        ...
        <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" OldValuesParameterFormatString="original_{0}"
            SelectMethod="SelectretrntakLook" TypeName="browspo">
            <SelectParameters>               
                ...
                <asp:QueryStringParameter QueryStringField="binloc" Name="prmcurval" Type="String" />               
