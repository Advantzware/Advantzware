...
public partial class view_act_dbcr : System.Web.UI.Page
{
    string oldinv = "";
    string recid = "";
   protected void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            if (Session["User"] != null)
            {
                ...
                if (Convert.ToString(Session["custdebit_memo_add_button"]) == "add")
                {
                    FormView1.ChangeMode(FormViewMode.Insert);
                    Session["custdebit_memo_add_button"] = null;
                }
                if (Session["view_debit_memo_cust_reckey_index"] != null)
                {
                    try
                    {
                        GridView1.SelectedIndex = Convert.ToInt32(Session["view_debit_memo_cust_reckey_index"]);
                        Session["view_debit_memo_cust_reckey"] = ((Label)GridView1.SelectedRow.FindControl("reclabel")).Text;
                    }
                    catch { }
                }
                else
   ...
   protected void FormView1_OnDataBound(object sender, EventArgs e)
   {
       ...
       if (FormView1.CurrentMode == FormViewMode.Insert)
       {
          ...
          // {
               ...
               ds = con.ActWriteCrDbist("Addnew", "", UserLogin.UserName, "", "", 0, "", 0, "", "", 0, "", "");
               ...
               stat.Text = Convert.ToString(ds.Tables[0].Rows[0][5]);
               curr.Text = Convert.ToString(ds.Tables[0].Rows[0][6]);
               exch.Text = Convert.ToString(ds.Tables[0].Rows[0][7]);
               reckey.Text = Convert.ToString(ds.Tables[0].Rows[0][9]);
       ...
       {           
           ...
           { 
               ...
               Session["debit_memo_cust_reckey_rec"] = reckey.Text.Trim();
   ...
   protected void delete_Button_Click(object sender, EventArgs e)
   {
       ...
       try
       {
           ...
           Response.Write("<script>window.location.href='view_act_dbcr.aspx'</script>");
       }
       catch { }
   ...
   protected void Formview1_InsertCancelButtonClick(object sender, EventArgs e)
   {
       try
       {
           ...
           account con = new account();
           DataSet ds = new DataSet();           
           ds = con.ActWriteCrDbist("DataDelete", "", UserLogin.UserName, "", "", 0, "", 0, "", "", 0, "", reckey.Text.Trim());
   ...
   protected void InsertButton_Click(object sender, EventArgs e)
   {
    ...
    {
        Label stat = (Label)FormView1.FindControl("statLabel");
        UserClass UserLogin = (UserClass)Session["User"];
        ObjectDataSource1.SelectParameters["prmUser"].DefaultValue = UserLogin.UserName;
        ObjectDataSource1.SelectParameters["prmAction"].DefaultValue = "HoldRel";
        if (stat.Text == "RELEASED")
            ObjectDataSource1.SelectParameters["prmstat"].DefaultValue = "ON HOLD";
        else
            ObjectDataSource1.SelectParameters["prmstat"].DefaultValue = "Release";
    ...
    {
        Response.Redirect("act_dbcrlist.aspx");
    }
    ...
    {
        Session["view_debit_memo_cust_reckey_index"] = GridView1.SelectedIndex;
        Session["view_debit_memo_cust_reckey"] = ((Label)GridView1.SelectedRow.FindControl("reclabel")).Text;
    }
    ...
    protected void UpdateButton_Formview2_Click(object sender, EventArgs e)
    {
        ...
        TextBox mmodscr = (TextBox)FormView2.FindControl("memdscrTextBox"); 
        ...
        bool check = val.ValidActWriteDbCrView("ValidateUpdate", "", UserLogin.UserName, "", Convert.ToInt32(invno.Text.Trim()), invdate.Text.Trim(), Convert.ToDecimal(baldue.Text.Trim()), Convert.ToDecimal(disc.Text.Trim()), Convert.ToDecimal(totl.Text.Trim()), act.Text.Trim(), "", "", Convert.ToString(Session["debit_memo_cust_reckey_rec"]), Convert.ToString(Session["view_debit_memo_cust_reckey"]));
        ...
        if (value == "True")
        {
                ...
                ObjectDataSource3.SelectParameters["prmmemodscr"].DefaultValue = mmodscr.Text.Trim();
    ...
    protected void AddButton_Formview2_Click(object sender, EventArgs e)
    {
        ...
        bool check = val.ValidActWriteDbCrView("ValidateAdd", "", UserLogin.UserName, "", Convert.ToInt32(invno.Text.Trim()), invdate.Text.Trim(), Convert.ToDecimal(baldue.Text.Trim()), Convert.ToDecimal(disc.Text.Trim()), Convert.ToDecimal(totl.Text.Trim()), act.Text.Trim(), "", "", Convert.ToString(Session["debit_memo_cust_reckey_rec"]), Convert.ToString(Session["view_debit_memo_cust_reckey"]));

        string value = Convert.ToString(check);
        if (value == "True")
        {

                Session["view_debit_memo_cust_reckey"] = reckey.Text.Trim();
    ...
    protected void FormView2_OnDataBound(object sender, EventArgs e)
    {
        if (FormView2.CurrentMode == FormViewMode.Insert)
        {
            ...
            try
            {
                account val = new account();
                DataSet ds = new DataSet();
                ds = val.ActWriteDbCrView("AddNew", "", UserLogin.UserName, "", 0, "", 0, 0, 0, "", "", "", Convert.ToString(Session["debit_memo_cust_reckey_rec"]), "");
                    ...
                    totl.Text = Convert.ToString(ds.Tables[0].Rows[0][5]);
                    act.Text = Convert.ToString(ds.Tables[0].Rows[0][6]);
                    actdscr.Text = Convert.ToString(ds.Tables[0].Rows[0][7]);
                    mmodscr.Text = Convert.ToString(ds.Tables[0].Rows[0][8]);
    ...
    protected void CancelButton_FormView2_Delete(object sender, EventArgs e)
    {
        ...
        try
        {
            account con = new account();
            DataSet ds = new DataSet();
            ds = con.ActWriteDbCrView("AddDelete", "", UserLogin.UserName, "", 0, "", 0, 0, 0, "", "", "", "", reckey.Text.Trim());
        }
        catch { }
    ...
    protected void deleteButton_FormView2_Click(object sender, EventArgs e)
    {
        ...
        Label reckey = (Label)FormView2.FindControl("ReckeyLabel");
                ...
                ObjectDataSource3.SelectParameters["prmOut"].DefaultValue = Convert.ToString(Session["debit_memo_cust_reckey_rec"]);
                Response.Write("<script>window.location.href='view_act_dbcr.aspx'</script>");
    ...
    protected void Invoice_OnTextChange(object sender, EventArgs e)
    {
        ...
        TextBox act = (TextBox)FormView2.FindControl("actnumTextBox");
        TextBox actdscr = (TextBox)FormView2.FindControl("actdscrLabel");
        TextBox mmodscr = (TextBox)FormView2.FindControl("memdscrLabel");
