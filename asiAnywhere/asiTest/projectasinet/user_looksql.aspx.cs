...
public partial class user_looksql : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (Session["User"] != null)
            {
                ...
                txt_fldname.Text = Convert.ToString(Session["user_looksql_index_help_field"]);
                txt_title.Text = Convert.ToString(Session["user_looksql_index_help_title"]);
                try
                {
        ...
        dbGrid_help_main.SelectedIndex = Convert.ToInt32(Session["user_looksql_index_help"]);
        try
        {
            if (Session["user_looksql_index_help"] == null)
            {
                dbGrid_help_main.SelectedIndex = 0;
                Session["user_looksql_code_help"] = dbGrid_help_main.SelectedRow.Cells[1].Text;
            }
        }
        catch
        {
    ...
    private void BuildDataSource()
    {
        ...
        try
        {
            conn.Open();
            string cmd = "";
            cmd = "select Username as 'User#', name as 'Name' from user_master";
            if (txt_fldname.Text != "")
            {
                cmd = "select Username as 'User#', name as 'Name' from user_master where Username LIKE '" + txt_fldname.Text + "%'";

            }
            if (txt_title.Text != "")
            {
                cmd = "select Username as 'User#', name as 'Name' from user_master where name LIKE '" + txt_title.Text + "%'";

            }
            if (txt_fldname.Text != "" && txt_title.Text != "")
            {
                cmd = "select Username as 'User#', name as 'Name' from user_master where Username LIKE '" + txt_fldname.Text + "%' and name LIKE '" + txt_title.Text + "%'";
    ...
    protected void btnShowAll_Click(object sender, System.EventArgs e)
    {
        ...
        Session["user_looksql_index_help_field"] = null;
        Session["user_looksql_index_help_title"] = null;
        Session["user_looksql_index_help"] = null;

    }

    protected void btnSearch_Click(object sender, System.EventArgs e)
    {
        Session["user_looksql_index_help_field"] = txt_fldname.Text.Trim();
        Session["user_looksql_index_help_title"] = txt_title.Text.Trim();
        //SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Project1ConnectionString"].ToString());
        //try
        //{
        //    conn.Open();
    ...
    {
        Session["user_looksql_index_help"] = dbGrid_help_main.SelectedIndex;
        //Session["user_looksql_code"] = Session["help_main_list_code"];
        Session["user_looksql_code_help"] = dbGrid_help_main.SelectedRow.Cells[1].Text;
