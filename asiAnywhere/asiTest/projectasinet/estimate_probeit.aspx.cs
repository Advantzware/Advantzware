...
public partial class estimate_probeit : System.Web.UI.Page
{
    ...
    protected void updateItem(object sender, EventArgs e)
    {
        ...
        Int32 lineno = Convert.ToInt32(Session["corr_print_line"]);

        if (oldsellprc.Text != sellprice.Text)
        {
            Corrugated corrugated = new Corrugated();
            corrugated.SelectEstimateProbeit(UserLogin.UserName, "updateitem", estimate, lineno, Convert.ToDecimal(sellprice.Text), partno.Text);

            FormView1.DataBind();
            FormView1.ChangeMode(FormViewMode.ReadOnly);

            Response.Write("<script>window.location.href='estimate_probeit.aspx'</script>");
        }
    }
}
