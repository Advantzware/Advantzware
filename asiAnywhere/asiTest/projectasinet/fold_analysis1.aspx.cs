...
public partial class fold_analysis1 : System.Web.UI.Page
{
    string puge = "";
        ...
        if (Session["User"] != null)
        {
            ...
            if (aUsers == "external")
            {
                if (!Page.IsPostBack)
                {
                    ...
                    Session["fold_analysis_1_vendor_no"] = "";                   
    ...
    //{
    //    Response.Redirect("corr_print_gsm.aspx");
    //}
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        ds = corr.SelectFoldQtyTandem(UserLogin.UserName, "doCalc", do_gsa, do_mr, do_speed, estimate, formno, blankno, HiddenField1.Value.Trim(),HiddenField_Vendor.Value.Trim());
        ...
        try
        {
            if (do_gsa == "True")
            {
                Session["fold_analysis1_gsa_mat"] = ds.Tables[0].Rows[0][5].ToString();
                Session["fold_analysis1_gsa_lab"] = ds.Tables[0].Rows[0][6].ToString();
                Session["fold_analysis1_gsa_war"] = ds.Tables[0].Rows[0][7].ToString();               
                Session["fold_analysis1_gsa_month"] = ds.Tables[0].Rows[0][9].ToString();
                SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Project1ConnectionString"].ToString());
                conn.Open();
                int row = 0;
                ...
                conn.Close();
                Response.Write("<script>location.href='fold_gsa2.aspx?seq=" + seqno + "&qty=" + qty + "'</script>");                                
            }
            else
            {
    ...
    protected void GridView1_selectedindex_change(object sender, EventArgs e)
    {
        ...
        Session["fold_analysis_1_vendor_no"] = GridView2.SelectedRow.Cells[1].Text;
    }
    protected void ok_button_click(object sender, EventArgs e)
    {
        vendordiv.Visible = false;
    ...
    protected void Price_Button_Click(object sender, EventArgs e)
    {
        ...
        Session["fold_analysis_1_vendor_no"] = "bestvendor";
        vendordiv.Visible = false;
        FormView1.ChangeMode(FormViewMode.Insert);
        GridView1.Visible = true;
        Button1.Visible = true;
