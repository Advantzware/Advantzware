...
IF prmlvcopied NE ? THEN DO:
  FIND CURRENT eb EXCLUSIVE-LOCK NO-ERROR.
  FIND CURRENT ef EXCLUSIVE-LOCK NO-ERROR.

  /*FIND itemfg WHERE ROWID(itemfg) EQ prmlvcopied NO-LOCK NO-ERROR.*/
  FIND itemfg WHERE itemfg.rec_key EQ prmlvcopied NO-LOCK NO-ERROR.
  ...
  ELSE
  FIND bf-eb WHERE bf-eb.rec_key EQ prmlvcopied NO-LOCK NO-ERROR.
