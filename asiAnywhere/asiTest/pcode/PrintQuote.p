...
DEF NEW SHARED VAR v-print-desc1 AS LOG INIT YES NO-UNDO.
DEF NEW SHARED VAR v-fg-desc2 AS LOG INIT YES NO-UNDO.
DEF NEW SHARED VAR v-fg-desc3 AS LOG INIT YES NO-UNDO.
...
IF prmAction = "PrintQuote" THEN do:
   /* IF rd-dest = 5  THEN*/
    ...
    /*MESSAGE sys-ctrl.descrip
       ...
       IF INDEX("Pacific,Xprint,Hughes,SouthPak,ABox,Midwest,Axis,MWFIBRE,century,Concepts,oracle,Harwell,PremierX,Elite,Unipak,Ottpkg,Frankstn,Mirpkg,APC,FibreX,Boss,Protagon,Loylang,LoylangBSF,PPI,Packrite,Xprint30,StClair,AllWest,Soule,Sultana,SouleMed,Simkins,CCC,Peachtree,Accord,Perform",v-print-fmt) > 0
        ...
        ELSE IF v-print-fmt EQ "Hughes" THEN 
            ASSIGN v-program = "cec/quote/quohughes.p" 
            lines-per-page = 66.
        ...
        ELSE IF v-print-fmt EQ "Boss" THEN 
            ASSIGN v-program = "cec/quote/quoboss.p" lines-per-page = 66.

       ELSE IF v-print-fmt EQ "Protagon" THEN 
           ASSIGN v-program = "cec/quote/quoprotg.p" lines-per-page = 66.

       ELSE IF v-print-fmt EQ "Loylang" THEN
            ASSIGN v-program = "cec/quote/quolylng.p" lines-per-page = 66.

       ELSE IF v-print-fmt EQ "LoylangBSF" THEN 
           ASSIGN v-program = "cec/quote/quolylng.p" lines-per-page = 66.
        ...
        ELSE
            ...
            ELSE IF v-print-fmt EQ "Midwest" THEN
             ...
             ELSE IF v-print-fmt EQ "Axis" THEN ASSIGN v-program = "cec/quote/quoaxis.p" lines-per-page = 66.

             ELSE IF v-print-fmt EQ "MWFIBRE" THEN ASSIGN v-program = "cec/quote/quomwfib.p" lines-per-page = 66.
            ...
            ELSE IF v-print-fmt EQ "Accord" THEN 
                ASSIGN v-program = "cec/quote/quoaccd.p" lines-per-page = 66.
            ...
            ELSE IF v-print-fmt EQ "MSPACK-EXCEL" THEN
                ...
                lines-per-page = 66.
            ELSE IF v-print-fmt EQ "Premier-EXCEL" THEN
                ASSIGN v-program = "cec/quote/quoprm-xl.p"            /*excel*/
                lines-per-page = 66.
            /* gdm - 04200908*/
            ...
            ELSE IF v-print-fmt EQ "Sultana" THEN 
                ASSIGN v-program = "cec/quote/quosult.p" lines-per-page = 80.

          ELSE IF v-print-fmt EQ "SouleMed" THEN
               ASSIGN v-program = "cec/quote/quosoulemed.p" lines-per-page = 66.  
            ...
            ELSE IF v-print-fmt =  "Peachtree" THEN 
                ASSIGN v-program = "cec/quote/quoxptree.p"
                              lines-per-page = 66.   
            ...
            ELSE IF v-print-fmt = "Perform" THEN 
                 ASSIGN v-program = "cec/quote/quoprfrm.p" 
                 lines-per-page = 66. 
...
PROCEDURE BatchMail :
   ...
   /* IF not(v-print-fmt EQ "CSC-EXCEL" OR
    ...
    IF v-print-fmt = "CSC-EXCEL" OR v-print-fmt = "TRILAKE-EXCEL" OR v-print-fmt = "KNIGHT-EXCEL" OR 
        v-print-fmt = "MSPACK-EXCEL" OR v-print-fmt = "FIBRE-EXCEL" OR v-print-fmt = "CCC-EXCEL" OR v-print-fmt = "PREMIER-EXCEL" THEN DO:
        OS-DELETE VALUE("d:\webapps\PrintQuote.csv") .
        OUTPUT TO d:\webapps\PrintQuote.csv .
        EXPORT DELIMITER ","
                prmUser
                prmQuote
                "PrintQuote" .
       ...
       PAUSE 3.  
        ...
        ASSIGN ttPrintQuote1.QteFile = "Quoteccc" + string(prmQuote)  + ".xls" .
        ASSIGN ttoutputfile = "Quoteccc" + string(prmQuote)  + ".xls" .
    END.
...
ASSIGN
 ...
 v-boardDescription = "Est" 
 s-print-2nd-dscr  = YES .
...
/* Check for XL also */
IF v-print-fmt = "CSC-EXCEL" OR v-print-fmt = "TRILAKE-EXCEL" OR v-print-fmt = "MSPACK-EXCEL" OR
     v-print-fmt = "FIBRE-EXCEL" OR v-print-fmt = "CCC-EXCEL" OR v-print-fmt = "PREMIER-EXCEL" OR v-print-fmt = "KNIGHT-EXCEL" THEN.
ELSE
