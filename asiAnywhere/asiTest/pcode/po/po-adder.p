...
/*
  ...
  for each job-mat
    ...
    if avail e-item and avail e-item-vend then do:
      if po-ordl.pr-qty-uom eq e-item.std-uom then
        v-qty-comp = po-ordl.ord-qty.
      else
        run sys/ref/convquom.p(po-ordl.pr-qty-uom, e-item.std-uom,
    ...
    /*  if i eq 1 then v-setup = e-item-vend.setup. */
      ...
      /* This adds the Adder cost in */
      if e-item.std-uom ne "EA" then
        run sys/ref/convcuom.p(e-item.std-uom, "EA", job-mat.basis-w,
                               job-mat.len, job-mat.wid, item.s-dep,
                               v-cost, output v-cost).
    end.
    ...
    v-tot-cost = v-tot-cost + v-cost.
  end.

  if po-ordl.pr-uom eq "EA" then
    po-ordl.cost = v-tot-cost.
  else
    run sys/ref/convcuom.p("EA", po-ordl.pr-uom,
                           v-basis-w, v-len, v-wid, v-dep,
                           v-tot-cost, output po-ordl.cost).
