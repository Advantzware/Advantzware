...
DEF VAR v-lscore-c-line-1 AS CHAR NO-UNDO.
DEF VAR v-lscore-c-line-2 AS CHAR NO-UNDO.
DEF VAR v-lcum-score-c-line-1 AS CHAR NO-UNDO.
DEF VAR v-lcum-score-c-line-2 AS CHAR NO-UNDO.
...
for each ef
    where ef.company = est.company 
      ...
      /*and (v-ef-recid eq ? or
           v-ef-recid eq recid(ef))*/ no-lock,    
    each eb
  ...
  if avail box-design-hdr          and
    ...
    RUN cec\lscruni.p(INPUT v-lscore-c,
                      OUTPUT v-lscore-c-line-1,
                      OUTPUT v-lscore-c-line-2).

    RUN cec\lscruni.p(INPUT v-lcum-score-c,
                      OUTPUT v-lcum-score-c-line-1,
                      OUTPUT v-lcum-score-c-line-2).
  ...
  if v-triad THEN DO:
    put {1} space(6) v-lcum-score-c-line-1 space(2) "Totals" skip.
    IF v-lcum-score-c-line-2 <> "" THEN
      put {1} space(6) v-lcum-score-c-line-2 space(2) "Totals" skip.
  END.
  
  IF est.metric THEN DO:
    IF LENGTH(v-lscore-c) <= 90 THEN DO:
       if v-triad THEN DO:
         put {1} /*"Score"*/ "         "  v-lscore-c-line-1 SKIP.
         IF v-lscore-c-line-2 <> "" THEN
           PUT {1} "         "  v-lscore-c-line-2 SKIP.
       END.
       ELSE
         DO:
           put {1} space(7) "           "  v-lscore-c-line-1 FORM "x(100)"  skip.
           IF v-lscore-c-line-2 <> "" THEN
             PUT {1} space(7) "           "  v-lscore-c-line-2 FORM "x(100)"  skip.

           PUT {1} space(7) "           " v-lcum-score-c-line-1 FORM "x(100)"  skip.
           IF v-lcum-score-c-line-2 <> "" THEN
             PUT {1} space(7) "           " v-lcum-score-c-line-2 FORM "x(100)"  skip.
         END.
    END.
    ELSE DO:
       if v-triad THEN DO:
       ...
       END.
       ELSE DO:
         put {1} space(7) "           "  v-lscore-c-line-1  FORM "x(135)"  skip.
         IF v-lscore-c-line-2 <> "" THEN
           PUT {1} space(7) "           "  v-lscore-c-line-2  FORM "x(135)"  skip.
         PUT {1} space(7) "           " v-lcum-score-c-line-1 FORM "x(135)" skip.
         IF v-lcum-score-c-line-2 <> "" THEN
           PUT {1} space(7) "           " v-lcum-score-c-line-2 FORM "x(135)" skip.
       END.
    END.
  END.   /* metric */
  ELSE DO:
     IF LENGTH(v-lscore-c) <= 90 THEN DO:
       if v-triad THEN DO:
         put {1} /*space(13) "     " */ v-lscore-c-line-1 skip.
         IF v-lscore-c-line-2 <> "" THEN
           PUT {1} v-lscore-c-line-2 SKIP.
       END.
       ELSE DO:
         put {1} space(3) /* "     " */  v-lscore-c-line-1 FORM "x(90)"  skip.
         IF v-lscore-c-line-2 <> "" THEN
           PUT {1} space(3) v-lscore-c-line-2 FORM "x(90)"  skip.
         
         PUT space(3) /*"      " */ v-lcum-score-c-line-1 FORM "x(90)"  skip.
         IF v-lcum-score-c-line-2 <> "" THEN
           PUT space(3) /*"      " */ v-lcum-score-c-line-2 FORM "x(90)"  skip.
       END.
     END.
     ELSE DO:
        if v-triad THEN DO:
        ...
        ELSE DO: 
          ...
          put {1} space(3) v-lscore-c-line-1  FORM "x(145)"  skip.
          IF v-lscore-c-line-2 <> "" THEN
            PUT {1} space(3) v-lscore-c-line-2  FORM "x(145)"  skip.
                                   
          PUT space(3) /*"      " */ v-lcum-score-c-line-1 FORM "x(145)" skip.
          IF v-lcum-score-c-line-2 <> "" THEN
            PUT space(3) /*"      " */ v-lcum-score-c-line-2 FORM "x(145)" skip.
        END.
     END.
  END.
  ...
  ELSE DO:     
     ...
     */  
     ...
     to put text */
     li-cnt = li-cnt + 1.
     PUT unformatted "<C3><#" li-cnt "><R+11><C+80><IMAGE#" li-cnt "=" box-design-hdr.box-image ">" .
     PUT UNFORMATTED "<=" li-cnt ">" SKIP.
     /*PUT "<R+11>Partition Length:" string(eb.est-no)   	" Slots:" 	" Slot Sizes:"   SKIP 
        " Partition Width:"      	" Slots"  	" Slot Sizes"   SKIP(1)
     "Form#  Die Layout:     Die Size 	Inside Slot to Slot:  Dimension from Center " SKIP
       STRING(eb.form-no)   SKIP.*/
     ...
     PUT "<=" li-cnt "><C1><R+22><FROM><C108><LINE><||3>" SKIP.
  END. /* else box image */
  IF li-cnt MOD 2 EQ 0 THEN PAGE.
