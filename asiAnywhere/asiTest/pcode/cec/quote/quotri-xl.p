/* ------------------------------------------- cec/quote/quotri-xl.p          */
/* -------------------------------------------------------------------------- */

/* skB- 1/24/07 Output selection for the report */
DEFINE SHARED VARIABLE LvOutputSelection AS CHAR NO-UNDO.
DEFINE SHARED VARIABLE CallingParameter AS CHAR NO-UNDO.

DEFINE SHARED VARIABLE v-webrootpath AS CHARACTER NO-UNDO.
DEFINE SHARED VARIABLE ttoutputfile AS CHARACTER.
...
DEF VAR v-new-item AS LOG NO-UNDO.

ASSIGN
   ls-image1 = "images\TRILAKE.jpg"
...
DEFINE VARIABLE v-outputfile AS CHARACTER.

ASSIGN v-outputfile = v-webrootpath + "Quote_" 
...
if not ch-multi then do:
   ...
   {cec/quote/quotri2-xl.i}    
   {cec/quote/quotri-xl.i 1}
   
   RUN util/GetBottomRow.p(input 18, input 44, input 4, input inrowcount, output inrowcount ).
   
   ASSIGN v-cell = "R" + string(inrowcount) + "C2".
     chExcelApplication:Goto(v-cell) NO-ERROR.
   ASSIGN
      chExcelApplication:ActiveCell:Value = "Sincerely,"
   ...
   IF ls-full-img1 NE "" THEN
   DO:
      mypict = chExcelApplication:Range("A1"):Parent:Pictures:Insert(substr(ls-full-img1,1,(length(ls-full-img1) - 1 ))).
      mypict:TOP = chExcelApplication:Range("A1"):TOP.
      mypict:LEFT = chExcelApplication:Range("A1"):LEFT.
   END.

   chExcelApplication:activeSheet:PageSetup:CenterFooter = "".
   chExcelApplication:Goto("R19C1") NO-ERROR.
   os-delete value(v-dir ).     
   ...
   DO:
      ...
      chExcelApplication:ActiveSheet:SaveAs(v-dir ) no-error. 	   
      /*NO-RETURN-VALUE chWorkbook:PrintOut(,,,,,False,). 
      chWorkbook:Close(no) no-error. */  
   ...
   RELEASE OBJECT mypict NO-ERROR.
   ...
   RELEASE OBJECT chHyper NO-ERROR.
    ...
    for each report where report.term-id eq v-term-id,
      ...
      IF LAST-OF(report.key-01) THEN DO:
         ...
         ASSIGN v-cell = "R" + string(inrowcount) + "C2".
  		 chExcelApplication:Goto(v-cell) NO-ERROR.
  	     ASSIGN chExcelApplication:ActiveCell:Value = "Sincerely," 
                chExcelApplication:Selection:Font:Bold = TRUE
                inrowcount = inrowcount + 3
                v-cell = "R" + string(inrowcount) + "C2".
