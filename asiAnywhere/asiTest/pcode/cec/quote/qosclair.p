/* ------------------------------------------- cec/quote/qusclair.p 10/02 YSK */
...
DEF VAR v-phone-no AS CHA FORM "xxxxxxxxxxxx"  NO-UNDO.
DEF VAR v-ship-phone AS CHA FORM "(xxx)xxx-xxxx"  NO-UNDO.
...
DEF SHARED VAR v-print-item AS LOG NO-UNDO.
DEF SHARED VAR v-print-desc1 AS LOG NO-UNDO.
DEF SHARED VAR v-fg-desc2 AS LOG NO-UNDO.
DEF SHARED VAR v-fg-desc3 AS LOG NO-UNDO.
DEF VAR  v-qty AS DEC FORMAT ">>>>>>>" NO-UNDO.
DEF VAR v-item-name AS CHAR FORMAT "x(30)" NO-UNDO.
DEF VAR v-item-dscr1 AS CHAR FORMAT "x(30)" NO-UNDO.
DEF VAR v-fg-dscr2 AS CHAR FORMAT "x(30)" NO-UNDO.
DEF VAR v-fg-dscr3 AS CHAR FORMAT "x(30)" NO-UNDO.
...
if (not ch-multi) then do:
  ...
  IF AVAIL notes THEN
      ASSIGN v-phone-no = notes.note_text.
  ELSE v-phone-no = " ".

  FIND FIRST shipto WHERE shipto.ship-id = xquo.ship-id
      AND shipto.company = xquo.company  NO-LOCK NO-ERROR.
  IF AVAIL shipto THEN
      v-ship-phone = shipto.area-code + shipto.phone.
  ELSE v-ship-phone = " ".
  
  {cec/quote/qosclair.i}

  {cec/quote/quostclair.i 1}
