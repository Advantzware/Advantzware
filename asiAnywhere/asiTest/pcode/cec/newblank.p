...
FIND FIRST bb WHERE bb.company  EQ ef.company AND bb.est-no   EQ ef.est-no AND bb.form-no  EQ 0 AND bb.blank-no EQ 0 NO-LOCK NO-ERROR.
  ls-part-no = IF AVAIL bb THEN bb.part-no ELSE "".

FIND LAST bb NO-LOCK WHERE bb.company EQ ef.company AND bb.est-no  EQ ef.est-no AND bb.form-no NE 0 USE-INDEX est-qty NO-ERROR.
IF AVAIL bb THEN  
    ASSIGN
    prev-cust = bb.cust-no
    prev-ship = bb.ship-id.

FIND LAST bb NO-LOCK WHERE bb.company EQ ef.company AND bb.est-no  EQ ef.est-no AND bb.form-no EQ ef.form-no USE-INDEX est-qty NO-ERROR.
