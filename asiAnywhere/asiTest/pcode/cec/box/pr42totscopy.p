...
DEF SHARED VAR sh-gsa-mat       AS DECIMAL NO-UNDO.
DEF SHARED VAR sh-gsa-lab       AS DECIMAL NO-UNDO.
DEF SHARED VAR sh-gsa-war       AS DECIMAL NO-UNDO.
DEF SHARED VAR sh-gsa-fm        AS DECIMAL NO-UNDO.
DEF SHARED VAR sh-gsa-war-tot   AS DECIMAL NO-UNDO.
DEF SHARED VAR sh-gsa-brd       AS DECIMAL NO-UNDO.
DEFINE SHARED VAR getqty3 AS LOG NO-UNDO.
    ...
    IF NOT AVAIL carr-mtx THEN 
     ...
     if xeb.fr-out-c ne 0 then
      ...
      if avail carr-mtx then do:
        ...
        if yyy lt carr-mtx.min-rate then yyy = carr-mtx.min-rate / xest.form-qty.
        yyy = yyy + (carr-mtx.min-rate * (rels[vmcl] - 1) / xest.form-qty).
...
output close.
/*run cec/gsa.p (ip-rowid, qtty[vmcl], rels[vmcl]).*/
/*output to value(outfile1) append .*/
...
ASSIGN
    ...
    gsa-war     = sh-gsa-war    
    gsa-fm      = sh-gsa-fm     
    ctrl2[1]    = sh-gsa-war-tot
    .

DO TRANSACTION:
    FIND CURRENT calcpcts EXCLUSIVE-LOCK NO-ERROR.
    calcpcts.val[1] = sh-gsa-brd.
