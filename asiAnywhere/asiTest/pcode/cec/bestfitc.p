...
DEF INPUT PARAM ip-rmitem AS CHAR NO-UNDO.
DEF INPUT PARAM ip-seaitem AS CHAR NO-UNDO.
DEF INPUT PARAM ip-maxyield AS CHAR NO-UNDO.
...
ASSIGN
    v-code          = ip-rmitem 
    v-max-yld       = IF ip-maxyield = "Yes" THEN TRUE ELSE FALSE
    useiCodeOriName = ip-seaitem  .
...
for each mach
  ...
  for each item
    ...
    if style.type eq "F" and v-max-yld then do:
      ...
      assign
       /*v-dim[1] = (v-dim[1] * tt-eb.num-wid) + (tt-ef.nsh-wid - tt-ef.trim-w)
       v-dim[2] = (v-dim[2] * tt-eb.num-len) + (tt-ef.nsh-len - tt-ef.trim-l)
       v-dim[3] = v-dim[3] * tt-ef.n-out-d
       v-nsh[1] = tt-ef.nsh-wid
       v-nsh[2] = tt-ef.nsh-len
       v-nsh[3] = tt-ef.nsh-dep*/
       v-gsh[1] = item.s-wid
       v-gsh[2] = item.s-len
       v-gsh[3] = item.s-dep.
    ...
    IF /*((v-onh-sh GE v-net-sh AND v-show-all EQ "QOH>QEst") OR
        (v-avl-sh GT 0        AND v-show-all EQ "QAvail>0") OR
        v-show-all EQ "AllItems")                               AND*/
       v-n-out NE 0 AND v-n-outl NE 0 AND v-n-outd NE 0         THEN DO:
      assign
  ...
  END.
     ...
     cError =  "No board found with required dimensions. " .
  end.             
  leave.
end.
