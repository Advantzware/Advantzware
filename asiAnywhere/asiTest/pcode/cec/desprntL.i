...
for each ef
  ...
  then do:
      /*page.  not work page sige = 0 */
    /* === set to have same font as top section (for page header ) ===========*/
/* not any more in window    
    find asi.printer where recid(asi.printer) eq v-out1-id no-lock no-error.
    assign
     v-start-compress = ""
     v-end-compress = "".
    ...
    do i = 1 to num-entries(asi.printer.pr-cmd):
      if entry(i,asi.printer.pr-cmd) ne ? then
        v-start-compress = v-start-compress + chr(int(entry(i,asi.printer.pr-cmd))).
    end.
    if asi.printer.pitch ne "" then
    do i = 1 to num-entries(asi.printer.pitch):
      if entry(i,asi.printer.pitch) ne ? then
        v-end-compress = v-end-compress + chr(int(entry(i,asi.printer.pitch))).
    end.
    ...
    put control v-start-compress.
      ...
      view frame head.
      put {1} skip(1).
     /* ======== set font for box design ==========*/
      find asi.printer where recid(asi.printer) eq v-out2-id no-lock.
      assign
             v-start-compress = ""
             v-end-compress = "".
            if asi.printer.pr-cmd ne "" then
            ...
            do i = 1 to num-entries(asi.printer.pitch):
              if entry(i,asi.printer.pitch) ne ? then
                v-end-compress = v-end-compress +
                                 chr(int(entry(i,asi.printer.pitch))).
            end.
       put control v-start-compress.
      /* ===================================== */
      ===========*/
   ...
   */  
     ...
     to put text */
     PUT unformatted "<C3><#30><R+25><C+80><IMAGE#30=" box-design-hdr.box-image ">" .
