...
def var v-across as int.
def var v-up     as int.
def var v-bsqft as dec.
def var v-ssqft as dec.
def var v-len like ef.gsh-len.
def var v-wid like ef.gsh-wid.
def var v-dep like ef.gsh-dep.
def var v-nsh like ef.nsh-wid.
def var v-out like ef.n-out.
def var v-cut like ef.n-cuts.
def var v-hed like ef.n-cuts.
...
if avail est-op then do:
  if est-op.n-out ne 0 then v-out = est-op.n-out.
      ...
      find first {sys/inc/machposw.i reftable "est-op"}
    no-lock no-error.
