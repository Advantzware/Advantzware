  ...
  /*     RUN check-use NO-ERROR.
    ...
    */
    ...
    ELSE DO WITH FRAME {&FRAME-NAME}:
         ...
         IF AVAIL itemfg THEN itemfg.alloc = rd_alloc EQ "U".
...
/*
   ...
   END.
   IF i LE 1 AND bf-eb.yld-qty EQ 2 THEN DO:
     RUN check-use NO-ERROR.
     IF ERROR-STATUS:ERROR THEN RETURN.
     RUN upd-2box.     
   END.
   ...
   ASSIGN
    ld-msf   = 0
    rd_alloc = IF v-alloc THEN "U" ELSE "A".
   ...
   FOR EACH b-eb
     ...
     ASSIGN
      ld-yld   = IF b-eb.yld-qty LT 0 THEN -1 / b-eb.yld-qty ELSE b-eb.yld-qty
      ld-sqin  = est.est-qty[1] * ld-yld * b-eb.t-sqin
      ld-msf   = ld-msf + ((IF v-corr THEN (ld-sqin * .007) ELSE (ld-sqin / 144)) / 1000).

     IF FIRST(b-eb.est-no) AND LAST(b-eb.est-no) THEN rd_alloc = "U".
   END.
   fi_msf = ld-msf.
