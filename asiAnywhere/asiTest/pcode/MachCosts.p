...
DEFINE INPUT-OUTPUT PARAMETER DATASET FOR dsMachCosts .
...
FOR EACH oe-ordl where oe-ordl.company EQ prmComp AND
    ...
    FOR EACH job WHERE job.company = oe-ordl.company 
        ...
        FOR each x-mch break by x-mch.line:
            create MachCosts.
            ASSIGN
                MachCosts.c-line = x-mch.line
                MachCosts.c-form-no = x-mch.form-no
                MachCosts.c-blank-no = x-mch.blank-no
                MachCosts.c-m-code = x-mch.m-code
                MachCosts.c-i-no   = x-mch.i-no
                MachCosts.c-dept = x-mch.dept
                MachCosts.c-run-std = x-mch.run-std
                MachCosts.c-run-act = x-mch.run-act
                MachCosts.c-run-var = x-mch.run-var
                MachCosts.c-mr-std = x-mch.mr-std
                MachCosts.c-mr-act = x-mch.mr-act
                MachCosts.c-mr-var = x-mch.mr-var
                .
             
            if MachCosts.c-run-act > 0 and x-mch.est-speed <> 0 THEN do:
                if ct = 6 THEN MachCosts.c-run-std = x-mch.act-qty * (x-mch.wst-prct * .01).
                if ct = 5 THEN MachCosts.c-run-std = x-mch.run-hrs * x-mch.est-speed.
                if ct < 5 THEN MachCosts.c-run-std = (x-mch.run-std / x-mch.std-hrs) *
                    ((x-mch.act-qty + x-mch.wst-qty) / x-mch.est-speed).
            end.
            assign
                MachCosts.c-run-var  = MachCosts.c-run-act - MachCosts.c-run-std
                MachCosts.c-mr-var   = MachCosts.c-mr-act  - MachCosts.c-mr-std
                v-std-tot[1] = v-std-tot[1] + MachCosts.c-run-std
                v-act-tot[1] = v-act-tot[1] + MachCosts.c-run-act
                v-var-tot[1] = v-var-tot[1] + MachCosts.c-run-var
                v-std-tot[2] = v-std-tot[2] + MachCosts.c-mr-std
                v-act-tot[2] = v-act-tot[2] + MachCosts.c-mr-act
                v-var-tot[2] = v-var-tot[2] + MachCosts.c-mr-var.
        end. /*FOR each x-mch break by x-MachCosts.c-line:*/
        FIND FIRST MachCosts WHERE MachCosts.c-form-no  EQ 0
                         AND MachCosts.c-blank-no EQ 0
                         AND MachCosts.c-m-code   EQ "ALL"
                          NO-ERROR.
        IF AVAIL MachCosts THEN DELETE MachCosts.
        CREATE MachCosts.
        ASSIGN
            MachCosts.c-form-no  = 0
            MachCosts.c-blank-no = 0
            MachCosts.c-m-code   = "ALL"
            MachCosts.c-run-std  = v-std-tot[1]
            MachCosts.c-run-act  = v-act-tot[1]
            MachCosts.c-run-var  = v-var-tot[1]
            MachCosts.c-mr-std   = v-std-tot[2]
            MachCosts.c-mr-act   = v-act-tot[2]
            MachCosts.c-mr-var   = v-var-tot[2]
            .
