...
do on error undo:
  ...
  for each bf-eb where bf-eb.company = oe-ordl.company
      ...
      break by bf-eb.form-no
            by bf-eb.blank-no:
                 MESSAGE "bf-eb" ROWID(bf-eb) bf-eb.form-no bf-eb.est-no v-est-fg1 bf-eb.stock-no. 
    release itemfg.
    if bf-eb.stock-no eq "" then do:
      ...
      IF v-est-fg1 EQ "Fibre" THEN DO:
        ...
        SUBSTR(v-item-no,1,LENGTH(TRIM(oe-ordl.i-no)) - 1) =
          ...
          MESSAGE "bf-eb2" v-item-no bf-eb.form-no .
      END.
    END.
    else do:
      find first itemfg
    ...
    do with frame eb-part:
      ...
      if not avail itemfg then create itemfg.
      assign
