...
ASSIGN ls-image1 = "images\mwfiber.jpg"
       tmpstore = fill("-",130)
...
for each xxreport where xxreport.term-id eq v-term-id,
  ...
  /*IF oe-bolh.tot-pallets NE 0 THEN v-tot-palls = oe-bolh.tot-pallets.*/
     ASSIGN v-tot-palls = oe-bolh.tot-pallets.
 
  PUT "<R54><C50><#8><FROM><R+4><C+28><RECT> "
    "<=8><R+0.1> Total Unit          : " v-tot-cases
    "<=8><R+1> Total Pallets       : " v-tot-palls 
    "<=8><R+2> Total Sq-Feet       :   " v-tot-sqft 
  ...
  PUT "<FArial><R52.5><C1><P12><B>     Shipping Instructions: </B> <P9> " 
    ...
    "<R65><C7>" "Date _______________________________                           Date _______________________________________"     
    .
