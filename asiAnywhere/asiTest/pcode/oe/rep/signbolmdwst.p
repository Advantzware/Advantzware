...
format header
    ...
    with frame hd-top-comp no-box no-labels  stream-io width 80.
...
for each xreport  where xreport.term-id eq v-term-id,
  ...
  if last-of(oe-bolh.bol-no) then do:
    ...
    put skip(25 - v-printlines)
        "<R47>TOTAL UNITS"                               at 1
        "<R47> " + trim(string(v-tot-pkgs))                    to 20
        "<R47>TOTAL PALLETS"                             at 22
        "<R47> " + trim(string(v-tot-pal))                     to 43
        "<R47>WEIGHT"                                    at 45
        "<R47> " + trim(string(v-tot-wt))                      to 59
        "<R47>TOTAL SQFT"                                at 61
        "<R47> " + trim(string(v-tot-sqft,">>>>>>>>>"))        to 79
        skip(3)
...
PUT 
    ...
    "<R50><C1>" "<B>  Received by: </B>"                  "<C32> PALLETS EXCHANGED       " 
    "<R51><C11><#1><R+6><C+46><IMAGE#1=" ls-full-img3 
