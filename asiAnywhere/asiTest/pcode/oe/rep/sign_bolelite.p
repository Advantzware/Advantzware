...
/* === with xprint ====*/
DEF VAR lsfullimg3 AS CHAR FORMAT "x(250)"  NO-UNDO.
DEF VAR ls-image AS CHAR NO-UNDO.
ASSIGN
        ls-image   = vImage  /*"images\icc.jpg" */  .

    lsfullimg3 = SEARCH(vImage) + ">" .
...
for each xxreport where xxreport.term-id eq v-term-id,
  ...
  PUT "<FArial><R47><C1><P12><B>     Shipping Instructions: </B> <P9> " 
    ...
    "<R54><C1>" "<B>  Signature of Receipt </B>"                  "<C42> Carrier       " 
     "<R55><C13><#1><R+9><C+45><IMAGE#1=" lsfullimg3  
     
    "<R61><C1>" "Date __________________________________                       Date _________________________________" AT 20    
    .
