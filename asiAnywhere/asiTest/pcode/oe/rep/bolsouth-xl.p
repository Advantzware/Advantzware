...
for each xxreport where xxreport.term-id eq v-term-id,
  ...
  if last-of(oe-bolh.bol-no) then do:
    ...
    /*  end of getting total page per po */
     ...
     os-delete value("c:\tmp\" + string(oe-bolh.bol-no) + ".xls").     
     os-delete value("c:\tmp\" + string(oe-bolh.bol-no) + ".pdf").
     ...
     DO:
       WshNetwork:SetDefaultPrinter(AdobePrinter).
       chExcelApplication:ActiveSheet:SaveAs("c:\tmp\" + string(oe-bolh.bol-no) + ".xls")no-error. 	   
       chExcelApplication:ActiveWindow:SelectedSheets:PrintOut().
       chWorkbook:Close(no) no-error.   
       ...
       OS-RENAME c:\tmp\asi.pdf VALUE("c:\tmp\" + string(oe-bolh.bol-no) + ".pdf").
       
       ASSIGN LvCtr = LvCtr + 1.
       CREATE tt-filelist.
       ASSIGN tt-FileCtr  = LvCtr
              tt-FileName = "c:\tmp\" + string(oe-bolh.bol-no) + ".pdf"
       .
     END.
...
/* message "Please check the files in c:\tmp if they are created before clicking OK" view-as alert-box. */
...
END.
assign CommandString = CommandString + " cat output c:\tmp\bol.pdf".

os-command silent value(CommandString).

/* message "Please check if bol.pdf is created in c:\tmp before clicking OK" view-as alert-box. */
