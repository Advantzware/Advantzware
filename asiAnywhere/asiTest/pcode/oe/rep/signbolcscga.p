...
ASSIGN ls-image1 = "images\csc.bmp"
...
IF ll-display-comp THEN DO:
  ...
  IF AVAIL cust THEN
     ...
     ASSIGN v-comp-add3 = cust.city + ", " + cust.state + "  " + cust.zip
            v-comp-add4 = "Phone: 706-965-9337"  
            v-comp-add5 = "Fax    : 706-965-9269" 
...
for each xxreport where xxreport.term-id eq v-term-id,
         ...
         IF ll-consol-bolls THEN DO:
            /*IF FIRST-OF(tt-boll.LINE) THEN DO:*/
             IF FIRST-OF(tt-boll.i-no) THEN DO:
               ...
               FIND FIRST oe-rel WHERE oe-rel.company = cocode
                                 AND oe-rel.ord-no = oe-ordl.ord-no
                                 AND oe-rel.LINE = oe-ordl.LINE
                                 AND oe-rel.i-no = oe-ordl.i-no NO-LOCK NO-ERROR.
               ASSIGN vDate = oe-rel.rel-date.
  ...
  PUT
    ...
    "<R58><C1>" "<B>  Signature of Receipt </B>"                  "<C42> Carrier       " 
    "<R59><C14><#1><R+6><C+46><IMAGE#1=" ls-full-img3 
    "<R65><C7>" "Date _______________________________"            "<C42> Date _______________________________________"     
    .
