/* ---------------------------------------------- oe/rep/bolmet.p 04/04 YSK */
/* PRINT Metro                                                             */
/* -------------------------------------------------------------------------- */
DEFINE  SHARED VAR vImage AS CHAR NO-UNDO.
...
DEF VAR ls-full-img3 AS CHAR FORMAT "x(250)"  NO-UNDO.
DEF VAR ls-image3 AS CHAR NO-UNDO.
ASSIGN
        ls-image3   = vImage  /*"images\icc.jpg" */  .
ASSIGN
    ls-full-img3 = SEARCH(ls-image3) + ">" .
...
ASSIGN ls-image1 = "images\metrobol.jpg".
...
/* gdm - 10060907 end                                                        */
  ...
  for each oe-boll where oe-boll.company eq oe-bolh.company and oe-boll.b-no eq oe-bolh.b-no:
      IF v-frt-class = "" THEN do:
         FIND FIRST xitemfg WHERE xitemfg.company = oe-bolh.company
                              AND xitemfg.i-no = oe-boll.i-no NO-LOCK NO-ERROR.
         v-frt-class = (IF AVAIL xitemfg AND xitemfg.frt-class <> "" THEN xitemfg.frt-class 
                        ELSE "A").

         IF AVAIL xitemfg THEN
            FOR EACH fg-rcpth NO-LOCK
               WHERE fg-rcpth.company      EQ xitemfg.company
                 AND fg-rcpth.i-no         EQ xitemfg.i-no,
  ...
  if last-of(oe-bolh.bol-no) then do:
     ...
     for each report where report.term-id eq v-term-id,
         ...
         ASSIGN v-tot-plt = v-tot-plt + v-pal.
         ...
         ASSIGN v-tot-cwt = v-tot-cwt + 
                         ( (oe-boll.cases + if oe-boll.partial gt 0 then 1 else 0) * v-case-weight ).
  ...
  PUT "<R42><C40><#7><C-6>Shipper Signature"
      "<=7><C+10><FROM><R+2><C+20><RECT> " 
      "<R44><C50><#8><FROM><R+5><C+30><RECT> " 
  ...
  PUT "<FBook Antiqua><R42><C1><P12><B>     Shipping Instructions: </B> <P9> " SKIP(1)
      ...
      "<R50><C1>" "<B>  Signature of Receipt </B>"                  "<C42> Carrier       " 
      "<R51><C14><#1><R+7><C+46><IMAGE#1=" ls-full-img3 
      "<R56><C7>" "Date _____________________________________                           Date _______________________________________"   AT 20 SKIP   
