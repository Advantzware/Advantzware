...
for each report where report.term-id eq v-term-id no-lock,
  ...
  if LAST-OF(report.key-06) then do:
    ...
    ASSIGN
      ...
      chWorkSheet:Range("C16"):value = cust.NAME
      chWorkSheet:Range("C17"):value = cust.addr[1]
      chWorkSheet:Range("C18"):value = cust.addr[2]
      chWorkSheet:Range("C19"):value = v-cust-addr3.
  
    ASSIGN
       chWorkSheet:Range("C29"):value = cust.NAME
       chWorkSheet:Range("C31"):VALUE = oe-boll.po-no
       chWorkSheet:Range("C46"):VALUE = v-bol-qty.

    IF AVAIL oe-ordl THEN
    DO:
       ASSIGN
         chWorkSheet:Range("C33"):VALUE = oe-ordl.part-no
         chWorkSheet:Range("C42"):VALUE = oe-ordl.ord-no.
       ...
       IF AVAIL eb AND eb.part-dscr1 NE "" THEN
          chWorkSheet:Range("C35"):VALUE = eb.part-dscr1.
       ELSE
          chWorkSheet:Range("C35"):VALUE = oe-ordl.i-name.
    
       IF AVAIL eb AND eb.cad-no NE "" THEN
          chWorkSheet:Range("C37"):VALUE = eb.cad-no.
       ELSE
          chWorkSheet:Range("C37"):VALUE = itemfg.cad-no.
    ...
    IF SEARCH(FILE-INFO:FULL-PATHNAME) NE ? THEN DO:
       mypict = chExcelApplication:Range("B52"):Parent:Pictures:Insert(FILE-INFO:FULL-PATHNAME).
       mypict:TOP = chExcelApplication:Range("B52"):TOP.
       mypict:LEFT = chExcelApplication:Range("B52"):LEFT.
...
OS-DELETE value("c:\tmp\cofc.xls").     
OS-DELETE value("c:\tmp\asi.pdf").
OS-DELETE value("c:\tmp\cofc.pdf").
...
DO:
   ...
   chExcelApplication:ActiveSheet:SaveAs("c:\tmp\cofc.xls") no-error. 	   
   NO-RETURN-VALUE chWorkbook:PrintOut(,,,,,False,). 
   chWorkbook:Close(no) no-error.   
   chExcelApplication:Quit() no-error.
   pause 3.
   OS-DELETE VALUE("c:\tmp\cofc.xls").
   OS-RENAME c:\tmp\asi.pdf c:\tmp\cofc.pdf.
   LvCtr = LvCtr + 1.
   CREATE tt-filelist.
   ASSIGN tt-FileCtr  = LvCtr
          tt-FileName = "c:\tmp\cofc.pdf".
END.
...
PROCEDURE CleanUp:
  ...
  assign CommandString = CommandString + " cat output c:\tmp\cofc.pdf".
