/* ---------------------------------------------- oe/rep/bolcentx.i YSK     */
/* PRINT Century Box BOL                                                           */
/* -------------------------------------------------------------------------- */
...
for each report where report.term-id eq v-term-id,
  ...
  /*IF LAST-OF(report.key-01) THEN DO: */
    ...
    /*IF AVAIL oe-ord AND LOOKUP(oe-ord.TYPE,lv-ord-type-code) > 0 THEN DO:
       lv-prt-sts = entry(LOOKUP(oe-ord.TYPE,lv-ord-type-code),lv-ord-type-list).
    END. */
    ...
    DISPLAY  {1}
          ...
          /*oe-boll.i-no */ oe-boll.po-no 
          oe-ordl.i-name /* FORM "x(28)" */    FORM "x(27)"
          oe-boll.tag FORM "x(20)"
          oe-boll.i-no WHEN oe-boll.tag = "" @ oe-boll.tag
          lv-pal-tot FORM ">>>,>>9"
          /*oe-boll.cases*/ lv-cases-tot FORM "->>>>" AT 91 "@" 
      ...
      */
         ...
         lv-qty-tot FORM "->>>>>z"  /*WHEN lv-partial-tot = 0 */
        /*SPACE(6)   v-part-comp WHEN lv-partial-tot = 0  */        
    ...
    IF oe-ordl.part-dscr1 <> "" OR v-unit-qty <> "" THEN DO:
       PUT {1}
    ...
    END.
    /*
    IF lv-partial-tot > 0 THEN do: /*PUT {1} "====================" AT 77 SKIP  . */
       PUT {1}
        /*  lv-cases /*v-tot-pkgs*/ AT 79 FORM ">>>" " = "
          lv-qty-tot /*oe-boll.qty */ FORM "->>>>>z" SPACE(12) */
          v-part-comp  AT 111 SPACE(12)
         /* oe-boll.weight*/  SKIP .
       v-printline = v-printline + 1.
    END.
    ELSE*/ PUT {1} SKIP.
   ...
   /* display componets of set */
    ...
    ASSIGN /*v-tot-wt = v-tot-wt + oe-boll.weight*/
           v-tot-cases = v-tot-cases + lv-pal-tot /*v-tot-pkgs*/.
