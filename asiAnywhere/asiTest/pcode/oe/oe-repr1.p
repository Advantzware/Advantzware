...
find first cust where (cust.company = cocode) and cust.cust-no = xoe-ord.cust-no
	   use-index cust no-lock no-error.
MESSAGE "cust-rep" xoe-ord.cust-no .
...
for each bfx-ordl where bfx-ordl.ord-no = xoe-ord.ord-no and
  ...
  IF AVAIL oe-prmtx THEN DO:
	  ...
	  do:
             MESSAGE "oe-prmtx" oe-prmtx.price[i] .
  ...
  MESSAGE "totale" bfx-ordl.price bfx-ordl.t-price .
