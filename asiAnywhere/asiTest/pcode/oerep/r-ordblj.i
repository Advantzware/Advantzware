    ...
    FOR EACH tt-report
      ...
      /* wfk - question - can there be only one job per order ? */
      ASSIGN job-qty = 0 job-qty-rcvd = 0 open-job-qty = 0 produced-qty = 0
             order-qty = 0 open-job-qty = 0 on-hand-qty = 0 wip-qty = 0.
      /*
      MESSAGE "order numb" oe-ordl.ord-no "coc" cocode 
          "job" oe-ordl.job-no oe-ord.job-no oe-ord.job-no2 
          "seljob" v-job[1] v-job[2] VIEW-AS ALERT-BOX. */
      for each job-hdr
