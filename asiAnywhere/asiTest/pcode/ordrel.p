...
/*------------------------------------------------------------------------
    ...
    Description : Return a Dataset of all Order Release
...
DEFINE INPUT-OUTPUT PARAMETER DATASET FOR dsItemfg.
...
PROCEDURE build-qry:
    ...
    IF prmItem = "" THEN
        ASSIGN
            prm-query = "FOR EACH Itemfg NO-LOCK".
    ELSE
        ASSIGN
             prm-query = "FOR EACH Itemfg WHERE Itemfg.i-no = '" + prmItem + "' NO-LOCK".    
END PROCEDURE.
