...
do j = 1 to 2:
  do i = 1 to 3:
    ws_rate[j] = ws_rate[j] +
                 if (j eq 1 or stax.tax-frt[i]) then stax.tax-rate[i] else 0.
                 
    if stax.accum-tax and i gt 1 then do:
      v-rate = stax.tax-rate[i].
      
      do k = 1 to i - 1:
        v-rate = v-rate + (v-rate * (stax.tax-rate[k] / 100)).
      end.
      
      ws_rate[j] = ws_rate[j] + v-rate - stax.tax-rate[i].
    end.               
  end.
end.
