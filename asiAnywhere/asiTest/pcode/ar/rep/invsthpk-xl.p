 ...
 END.
    ...
    for each report where report.term-id eq v-term-id no-lock,
       ...
       for each ar-invl NO-LOCK where ar-invl.x-no  eq ar-inv.x-no  
        ...
        ASSIGN chExcelApplication:ActiveCell:Value = ar-inv.ord-no.
        ...
        for each ar-invl no-lock where ar-invl.x-no = ar-inv.x-no
          ...
          assign v-case-line = ""
            ...
            chExcelApplication:Goto(v-cell) NO-ERROR.
            ASSIGN chExcelApplication:ActiveCell:Value = ar-invl.po-no .


            /* Customer Item */
            ASSIGN v-cell = "R" + STRING(inrowcount) + "C24".
            chExcelApplication:Goto(v-cell) NO-ERROR.
            ASSIGN chExcelApplication:ActiveCell:Value = ar-invl.part-no.
    ...
    ASSIGN inrowcount = inrowcount + 1
           v-cell = "R" + STRING(inrowcount) + "C45".
    chExcelApplication:Goto(v-cell) NO-ERROR.
    ASSIGN chExcelApplication:ActiveCell:Value = ar-inv.net + v-t-tax[1] + v-t-tax[2] + v-t-tax[3] + v-inv-freight.

    chExcelApplication:Goto("R15C1") NO-ERROR.
    os-delete value("c:\tmp\" + STRING(ar-inv.inv-no) + ".xls").     
    os-delete value("c:\tmp\asi.pdf").
    os-delete value("c:\tmp\" + STRING(ar-inv.inv-no) + ".pdf").
    ...
    IF LvOutputSelection = "Email" THEN
    DO:
      chExcelApplication:ActiveSheet:SaveAs("c:\tmp\" + STRING(ar-inv.inv-no) + ".xls") no-error. 	   
      ...
      OS-RENAME c:\tmp\asi.pdf VALUE("c:\tmp\" + STRING(ar-inv.inv-no) + ".pdf").
       		
      ASSIGN LvCtr = LvCtr + 1.
      CREATE tt-filelist.
      ASSIGN tt-FileCtr  = LvCtr
          		tt-FileName = "c:\tmp\" + STRING(ar-inv.inv-no) + ".pdf"
       		.
    END.
    ...
    PUT "<FArial><R58><C1><P12><B> Comments </B> <P9> " SKIP
         "  The packaging professionals at South-Pak" SKIP
         "  sincerely appreciate your business and goodwill." SKIP
         "  We strive to offer consistent best pricing, quality " SKIP
         "  and timely service.  PLEASE PAY BY THIS INVOICE." SKIP.
...
os-delete c:\tmp\Invoice.pdf.

FOR EACH tt-filelist :
  assign CommandString = CommandString + " " + tt-FileName .
END.

assign CommandString = CommandString + " cat output c:\tmp\Invoice.pdf".
