...
{sys/inc/var.i new shared}
    ...
    DEFINE TEMP-TABLE ttSchedReleaseRep NO-UNDO
        FIELD abc AS CHAR
        FIELD vSchedRelease AS CHAR.
DEFINE DATASET dsSchedReleaseRep FOR ttSchedReleaseRep.
    DEFINE INPUT PARAMETER  prmUser         AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmSchedAct     AS CHAR NO-UNDO.
    ...
    DEFINE INPUT PARAMETER  prmBeLoc        AS CHAR NO-UNDO.
    ...
    DEFINE INPUT PARAMETER  prmBeCarrier    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmEndCarrier   AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmBeSpec       AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmEndSpec      AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmBeCat        AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmEndCat        AS CHAR NO-UNDO.

    DEFINE INPUT PARAMETER  prmSeheduled    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmLate         AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmLastShipDate AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmActual       AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmBackOrder    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmBillLading   AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmInvPosted    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmCompleted    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmSort         AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintOpt1    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintOpt2    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintOpt3    AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintDueAlert AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintPo      AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmSubTotalCust AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintStats   AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmPrintRelease AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER  prmSpecNote     AS CHAR NO-UNDO.
    DEFINE INPUT PARAMETER prmOut           AS CHAR NO-UNDO.
    DEFINE OUTPUT PARAMETER cError AS CHAR NO-UNDO.


 DEFINE INPUT-OUTPUT PARAMETER DATASET FOR dsSchedReleaseRep.

     IF prmUser           = ? THEN ASSIGN     prmUser           = "".   
     IF prmSchedAct       = ? THEN ASSIGN     prmSchedAct       = "". 
     ...
     IF prmBeLoc          = ? THEN ASSIGN     prmBeLoc          = "". 
     ...
     IF prmBeCarrier      = ? THEN ASSIGN     prmBeCarrier      = "". 
     IF prmEndCarrier     = ? THEN ASSIGN     prmEndCarrier     = "". 
     IF prmBeSpec         = ? THEN ASSIGN     prmBeSpec         = "". 
     IF prmEndSpec        = ? THEN ASSIGN     prmEndSpec        = "". 
     IF prmBeCat          = ? THEN ASSIGN     prmBeCat          = "". 
     IF prmEndCat         = ? THEN ASSIGN     prmEndCat         = "zzzzz". 
     IF prmSeheduled      = ? THEN ASSIGN     prmSeheduled      = "". 
     IF prmLate           = ? THEN ASSIGN     prmLate           = "". 
     IF prmLastShipDate   = ? THEN ASSIGN     prmLastShipDate   = "". 
     IF prmActual         = ? THEN ASSIGN     prmActual         = "".  
     IF prmBackOrder      = ? THEN ASSIGN     prmBackOrder      = "".  
     IF prmBillLading     = ? THEN ASSIGN     prmBillLading     = "". 
     IF prmInvPosted      = ? THEN ASSIGN     prmInvPosted      = "". 
     IF prmCompleted      = ? THEN ASSIGN     prmCompleted      = "". 
     IF prmSort           = ? THEN ASSIGN     prmSort           = "". 
     IF prmPrintOpt1      = ? THEN ASSIGN     prmPrintOpt1      = "". 
     IF prmPrintOpt2      = ? THEN ASSIGN     prmPrintOpt2      = "". 
     IF prmPrintOpt3      = ? THEN ASSIGN     prmPrintOpt3      = "". 
     IF prmPrintDueAlert  = ? THEN ASSIGN     prmPrintDueAlert  = "".
     IF prmPrintPo        = ? THEN ASSIGN     prmPrintPo        = "".
     IF prmSubTotalCust   = ? THEN ASSIGN     prmSubTotalCust   = "".
     IF prmPrintStats     = ? THEN ASSIGN     prmPrintStats     = "".
     IF prmPrintRelease   = ? THEN ASSIGN     prmPrintRelease   = "".
     IF prmSpecNote       = ? THEN ASSIGN     prmSpecNote       = "".
                                                              




DEFINE VARIABLE begin_carr AS CHARACTER FORMAT "X(5)" NO-UNDO.
DEFINE VARIABLE begin_cat AS CHARACTER FORMAT "X(5)" NO-UNDO.
DEFINE VARIABLE begin_cust-no AS CHARACTER FORMAT "X(8)" NO-UNDO.
DEFINE VARIABLE begin_date AS DATE FORMAT "99/99/9999" NO-UNDO.
...
DEFINE VARIABLE begin_spec AS CHARACTER FORMAT "X(3)":U NO-UNDO.
...
DEFINE VARIABLE end_loc AS CHARACTER FORMAT "X(5)" INITIAL "zzzzz" NO-UNDO.
...
DEFINE VARIABLE end_spec AS CHARACTER FORMAT "X(3)":U INITIAL "zzzzz" NO-UNDO.
DEFINE VARIABLE fi_file AS CHARACTER FORMAT "X(30)"  NO-UNDO.
DEFINE VARIABLE lines-per-page AS INTEGER FORMAT ">>":U INITIAL 99 NO-UNDO.
DEFINE VARIABLE rd_print AS CHARACTER INITIAL "Item Name" NO-UNDO.
...
DEFINE VARIABLE rs_qty AS CHARACTER INITIAL "Job#" NO-UNDO.
...
DEFINE VARIABLE tb_notes AS LOGICAL INITIAL no NO-UNDO.
...
DEFINE VARIABLE tb_stats AS LOGICAL INITIAL no  NO-UNDO.
...
DEF STREAM excel.
  ...
  IF prmSchedAct = "SehedRelease" THEN DO:
        ASSIGN
        v-today        = TODAY
        begin_carr     = prmBeCarrier
        begin_cat      = prmBeCat
        begin_cust-no  = prmBeginCust
        begin_date     = datetime(prmBeDate)
        begin_i-no     = prmBeItem
        begin_loc      = prmBeLoc
        begin_ord-no   = prmBeOrder
        begin_slsmn    = prmBeSales
        begin_spec     = prmBeSpec
        end_carr       = prmEndCarrier
        end_cat        = prmEndCat 
        ...
        rd_sort         = prmSort
        rd_print        = prmPrintOpt1  
        rs-item-option  = prmPrintOpt2  
        rs_qty          = prmPrintOpt3  
        rd_rel          = prmPrintRelease

        tb_scheduled    = IF prmSeheduled = "Yes" THEN TRUE ELSE FALSE
        tb_late         = IF prmLate = "Yes" THEN TRUE ELSE FALSE
        tb_invoiceable  = IF prmLastShipDate = "Yes" THEN TRUE ELSE FALSE
        tb_actual       = IF prmActual = "Yes" THEN TRUE ELSE FALSE
        tb_backordered  = IF prmBackOrder = "Yes" THEN TRUE ELSE FALSE
        tb_posted       = IF prmBillLading = "Yes" THEN TRUE ELSE FALSE
        tb_invoice      = IF prmInvPosted = "Yes" THEN TRUE ELSE FALSE
        tb_completed    = IF prmCompleted = "Yes" THEN TRUE ELSE FALSE
        
        tb_notes        = IF prmSpecNote = "Yes" THEN TRUE ELSE FALSE
        tb_po-no        = IF prmPrintPo = "Yes" THEN TRUE ELSE FALSE
        tb_stats        = IF prmPrintStats = "Yes" THEN TRUE ELSE FALSE
        tb_subt         = IF prmSubTotalCust  = "Yes" THEN TRUE ELSE FALSE  
        tg-print-due    = IF prmPrintDueAlert  = "Yes" THEN TRUE ELSE FALSE   .
    ...
    assign
      init-dir    =  v-webrootpath
        fi_file = init-dir + "schedrelease" +
            STRING(YEAR(v-today),"9999")
                   + STRING(MONTH(v-today),"99")
                   + STRING(DAY(v-today),"99") + STRING(TIME) + ".csv".  
        vPdfFile   = "schedrelease" +
            STRING(YEAR(v-today),"9999")
                   + STRING(MONTH(v-today),"99")
                   + STRING(DAY(v-today),"99") + STRING(TIME) + ".csv". 
        DEFINE VAR vTextFile AS CHAR NO-UNDO.
        vTextFile = "schedrelease" +
            STRING(YEAR(v-today),"9999")
                   + STRING(MONTH(v-today),"99")
 ...
 run run-report.
    ...
    CREATE ttSchedReleaseRep.
    IF tb_excel THEN
        ASSIGN ttSchedReleaseRep.vSchedRelease = vPdfFile.
    IF NOT tb_excel THEN
         ASSIGN ttSchedReleaseRep.vSchedRelease = vTextFile .
 ...
 {sys/inc/outprint.i value(lines-per-page)}
  ...
  FOR EACH oe-ordl
      ...
      WHERE cust.company EQ oe-ord.company
        AND cust.cust-no EQ oe-ord.cust-no AND LOOKUP(cust.cust-no, custcount) <> 0 
      NO-LOCK:
