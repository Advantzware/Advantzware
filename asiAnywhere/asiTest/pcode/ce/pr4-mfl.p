...
do  WITH STREAM-IO no-box no-labels frame med1:
   ...
   if xef.n-out-l = 0 THEN
      ASSIGN
      med-len = xef.nsh-len
      med-wid = (IF item.i-code EQ "R" THEN ITEM.r-wid ELSE xef.nsh-wid) / (1 - (item.shrink / 100)). /* proper length */
   ELSE
      ASSIGN
      med-len = xef.nsh-len / (1 - (item.shrink / 100)) /* proper length */
      med-wid = (IF item.i-code EQ "R" THEN ITEM.r-wid ELSE xef.nsh-wid).
   
   ASSIGN
      med-qty = (( med-wid * med-len) * mqty) / 144000 /*now msf*/
      fg-wt = fg-wt + ((fg-qty / (1 - (item.shrink / 100))) * item.basis-w).
