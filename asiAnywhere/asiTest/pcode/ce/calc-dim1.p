...
DEF VAR ld-tons AS DEC INIT -1 NO-UNDO.
      ...
      find style where (style.company eq cocode) and
      ...
      find first mach where (mach.company = cocode and
       mach.loc     = locode) and mach.m-code = XEF.m-code
      no-lock no-error.
      ...
      ELSE do:
         ...
         do i = 1 to 4:
            ...
            do:
               find first item where (item.company = cocode) and item.i-no eq xef.leaf[i]
               no-lock no-error.
