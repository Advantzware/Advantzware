...
for each cas where cas.typ = 1 by cas.snum by cas.bnum with no-labels no-box:
   ...
   find first item where (item.company = cocode) and item.i-no = cas.ino
   no-lock no-error.
...
for each xeb where xeb.company = xest.company
   ...
   if xeb.tr-no ne "" then do with frame ac3 no-box no-labels stream-io:
      ...
      if not available cas then do:
         find first item where (item.company = cocode) and item.i-no = xeb.tr-no
         no-lock no-error.
...
PROCEDURE do-cas-no:
  ...
  if xeb.cas-no ne "" then do with frame ac2 no-box no-labels stream-io:
    find first item where (item.company = cocode) and item.i-no = xeb.cas-no
    no-lock no-error.
    find first e-item of item no-lock no-error.
    find first cas
        where cas.typ  eq 1
  ...
  if xeb.cas-no ne "" and xeb.layer-pad ne "" and xeb.lp-up ne 0 then do:
    find first item where (item.company = cocode) and item.i-no = xeb.layer-pad
    no-lock no-error.
    find first e-item of item no-lock no-error.
    find first cas
        where cas.typ  eq 5
  ...
  if xeb.cas-no ne "" and xeb.divider ne "" and xeb.div-up ne 0 then do:
    find first item where (item.company = cocode) and item.i-no = xeb.divider
    no-lock no-error.
    find first e-item of item no-lock no-error.
    find first cas
        where cas.typ  eq 6
