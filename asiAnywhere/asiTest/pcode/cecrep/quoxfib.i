...
FOR EACH xqitm OF xquo NO-LOCK BREAK BY xqitm.part-no:
  ...
  IF AVAIL eb THEN DO:
    ...
    IF AVAIL ef AND ef.adder[2] <> "" AND
       ((AVAIL est AND est.est-type GT 4 and est.est-type NE 6) OR NOT AVAIL est) THEN numfit = numfit + 1.
  ...
  DO i = 1 TO numfit /*WITH FRAME item-10p */:
    ...
    END.
    ELSE
    IF i EQ 5 AND NOT (AVAIL est AND (est.est-type = 2 OR est.est-type = 6)) THEN DO:
       v-board = "".
    ...
    IF i > 5 THEN do:
       v-board = "".
       IF AVAIL ef AND ((AVAIL est AND est.est-type GT 4 and est.est-type NE 6) OR NOT AVAIL est) AND ef.adder[i - 4] <> "" THEN DO:
          FIND FIRST ITEM WHERE ITEM.company = xquo.company
    ...
    IF AVAIL xqqty THEN DO:
       xxx    = IF xqqty.uom EQ "L" THEN xqqty.price    ELSE
        ...
        PUT xqqty.qty           TO 65   FORMAT ">>>>>>9"
            xqqty.rels          TO 68   FORMAT ">>9"   
            xqqty.price         TO 83   FORMAT "->>,>>9.99" 
            xqqty.uom           TO 92.
