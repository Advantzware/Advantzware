...
FOR FIRST b-eb WHERE ROWID(b-eb) EQ ip-rowid NO-LOCK,
  ...
  FOR EACH itemfg
      WHERE itemfg.company          EQ eb.company
        ...
        AND SUBSTR(itemfg.i-no,8,4) GE "0000"
        AND SUBSTR(itemfg.i-no,8,4) LE "9999"
      NO-LOCK
      BY SUBSTR(itemfg.i-no,1,11) DESC:
    li = INT(SUBSTR(itemfg.i-no,8,4)) NO-ERROR. 
  ...
  FOR EACH oe-ordl
      WHERE oe-ordl.company          EQ eb.company
        ...
        AND SUBSTR(oe-ordl.i-no,8,4) GT STRING(li,"9999")
        AND SUBSTR(oe-ordl.i-no,8,4) LE "9999"
      NO-LOCK
      BY SUBSTR(oe-ordl.i-no,1,8 + 3) DESC:
    li1 = INT(SUBSTR(oe-ordl.i-no,8,4)) NO-ERROR. 
