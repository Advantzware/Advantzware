  ...
  for each w-fg-rctd
    ...
    if w-fg-rctd.po-no ne " " then
      find po-ord where po-ord.po-no = int(w-fg-rctd.po-no) NO-LOCK.
    else
      release po-ord.
    ...
    IF tb_excel  THEN DO:
      IF rd-ItmPo EQ 1 THEN DO:
          PUT STREAM excel UNFORMATTED
           (IF first-of(w-fg-rctd.i-no) THEN w-fg-rctd.rct-date ELSE ?) ",,"
           (IF first-of(w-fg-rctd.i-no) THEN v-time ELSE ?) ",,"
           (IF first-of(w-fg-rctd.i-no) THEN  v-cstprt    ELSE "") ","
            w-fg-rctd.i-name   ","
