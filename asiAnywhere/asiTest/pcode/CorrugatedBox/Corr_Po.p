/*------------------------------------------------------------------------
    ...
    Description : Return a Dataset of all jobstds
...
/* ***************************  Definitions  ************************** */
DEFINE TEMP-TABLE ttCorrdopono NO-UNDO 
    FIELD  vForm       AS INT 
    FIELD  vBlank      AS INT
    FIELD  vRMItem     AS CHAR
    FIELD  vName       AS CHAR     
    FIELD  vLength     AS DECIMAL
    FIELD  vWidth      AS DECIMAL
    FIELD  vDepth      AS DECIMAL
    FIELD  vUp         AS DECIMAL

    FIELD  vVander#    AS CHAR
    FIELD  vVaName     AS CHARACTER
    FIELD  vTons       AS CHAR
    FIELD  vMsf        AS CHAR
    FIELD  vCost       AS DECIMAL
    FIELD  vLead       AS DECIMAL
    FIELD  vExtCost    AS DECIMAL 
    FIELD  vRecid      AS CHAR  .


DEFINE DATASET dsCorrdopono FOR ttCorrdopono.
...
DEFINE INPUT-OUTPUT PARAMETER DATASET FOR dsCorrdopono.
...
DEFINE VARIABLE fi_dep AS DECIMAL FORMAT ">,>>9.99<<<<" INITIAL 0 NO-UNDO.
...
DEF TEMP-TABLE w-job-mat LIKE job-mat
...
DEF TEMP-TABLE tt-eiv NO-UNDO LIKE e-item-vend FIELD rec-id AS RECID.
 ...
 IF AVAIL est THEN
  FOR EACH job-hdr
      ...
      where job.company EQ job-hdr.company
        and job.job     EQ job-hdr.job  /*2449*/
...
IF AVAIL job AND ((v-format NE "Manual" AND v-po-best EQ NO) OR v-po-best) THEN
FOR EACH job-mat
...
IF AVAIL job AND oeautoprep-log AND
...
FOR EACH w-job-mat
      ...
      FOR EACH e-item-vend OF e-item NO-LOCK:
        ...
        IF NOT CAN-FIND(FIRST tt-eiv
                        WHERE tt-eiv.company   EQ e-item-vend.company
                          AND tt-eiv.i-no      EQ e-item-vend.i-no AND (tt-eiv.i-no  = prmItem OR prmItem = "")
                          AND tt-eiv.vend-no   EQ e-item-vend.vend-no) THEN DO:
            
          CREATE tt-eiv.
          BUFFER-COPY e-item-vend TO tt-eiv
          ASSIGN
      ...
      IF w-job-mat.est-no NE "" THEN DO:
        ...
        FOR EACH e-itemfg-vend NO-LOCK
          ...
          IF NOT CAN-FIND(FIRST tt-eiv
            ...
            CREATE tt-eiv.
            BUFFER-COPY e-itemfg-vend TO tt-eiv
            ASSIGN
  ...
  if not avail b-orderpo then do:
    ...
    IF AVAIL bf-ordl AND bf-ordl.vend-no NE "" AND bf-ordl.po-no-po NE 0 THEN DO:
      find first po-ordl
          ...
          no-lock no-error.
      if avail po-ordl then b-orderpo.val[1] = po-ordl.po-no.
    end.
  ...
  choice = NO.
  IF v-vend-no EQ "" THEN DO ON ENDKEY UNDO, LEAVE:
  ...
  IF AVAIL tt-ei THEN DO:
    ...
    FOR EACH tt-eiv
        WHERE tt-eiv.company    EQ cocode
          AND tt-eiv.i-no       EQ tt-ei.i-no AND (tt-eiv.i-no = prmItem OR prmItem = "")
    ...
    IF choice THEN DO:
         IF NOT w-job-mat.this-is-a-rm  THEN DO:
             RUN caljobmat .
         END.
         find job-mat where recid(job-mat) = w-job-mat.w-recid  no-lock no-error.
         if avail job-mat THEN DO :
            ...
            IF prmItem = "" THEN DO:
             CREATE ttCorrdopono .
                ASSIGN
                    ttCorrdopono.vForm    = job-mat.frm
                    ttCorrdopono.vBlank   = job-mat.blank-no
                    ttCorrdopono.vRMItem  = job-mat.i-no
                    ttCorrdopono.vName    = item.i-name
                    ttCorrdopono.vLength  = job-mat.len
                    ttCorrdopono.vWidth   = job-mat.wid
                    ttCorrdopono.vDepth   = 0
                    ttCorrdopono.vUp      = job-mat.n-up
                    ttCorrdopono.vRecid   = STRING(w-job-mat.this-is-a-rm) .
     ...
     ELSE
         DO:
         find job-prep where recid(job-prep) = w-job-mat.w-recid no-lock no-error.
  ...
  /*END. /* choice  b-orderpo.val[1] (po-no) eq 0 */*/
  ...
  IF choice AND po-ord.opened THEN DO:
  ...
  if not avail po-ordl and v-format eq "AutoRM" and w-job-mat.this-is-a-rm then do:
    find first po-ordl
        where po-ordl.company eq cocode
          and po-ordl.job-no  eq w-job-mat.job-no
          and po-ordl.job-no2 eq w-job-mat.job-no2
  ...
  IF po-ordl.item-type THEN DO:
    if avail tt-ei then do:
      ...
      IF AVAIL bf-ordl AND po-ordl.item-type then do:
        ...
        if v-format eq "AutoRM" and not v-new-avail then do:
          fil_id = recid(b-item).
  ...
  if avail job-mat                        and 
     ...
     b-item.i-code   eq "E"               then run po/po-adder.p (recid(po-ordl), recid(job-mat)).
  ...
  IF AVAIL tt-ref1 OR AVAIL tt-ref2 THEN DO:
    ...
    ll = NO.
    IF AVAIL b-ref1 AND AVAIL tt-ref1 THEN DO li = 1 TO EXTENT(b-ref1.val):
      IF b-ref1.val[li] NE tt-ref1.val[li] THEN ll = YES.
      IF ll THEN LEAVE.
    END.
    IF NOT ll                         AND
       AVAIL b-ref2 AND AVAIL tt-ref2 THEN DO li = 1 TO EXTENT(b-ref2.val):
...
------------------------------------------------------------------------------*/
  ...
  for each report where report.term-id eq v-term NO-LOCK:
     ...
     CREATE ttCorrdopono .
        ...
        ASSIGN
            ttCorrdopono.vVander#  = report.key-03
            ttCorrdopono.vVaName   = if avail vend then vend.name else "" 
            ttCorrdopono.vTons     = report.key-04
            ttCorrdopono.vMsf      = report.key-02
            ttCorrdopono.vCost     = dec(report.key-01)
            ttCorrdopono.vLead     = if avail vend then vend.disc-days else 0 
            ttCorrdopono.vExtCost  = dec(report.key-02) * ttCorrdopono.vCost
            /*ttCorrdopono.vRecid    = string(report.rec-id)*/    
            ttCorrdopono.vRecid   = STRING(w-job-mat.this-is-a-rm)  
                                       .
            END.
...
PROCEDURE caljobmat:
    FIND FIRST report where report.term-id eq v-term NO-LOCK NO-ERROR.
        /*  RELEASE e-item-vend.
          RELEASE e-itemfg-vend.*/
    ...
    IF prmItem = "" THEN DO:
             CREATE ttCorrdopono .
                ASSIGN
                    ttCorrdopono.vRMItem  = e-itemfg-vend.i-no 
                    ttCorrdopono.vRecid   = STRING(w-job-mat.this-is-a-rm) .
