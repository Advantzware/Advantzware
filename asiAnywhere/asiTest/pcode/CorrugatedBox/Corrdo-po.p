...
DEFINE SHARED VARIABLE vLogicError AS CHARACTER NO-UNDO.
...
FOR EACH w-job-mat
  ...
  IF v-vend-no EQ "" THEN DO ON ENDKEY UNDO, LEAVE:
      ASSIGN  
          vLogicError = "Do you wish to create a PO line for " +
            (IF w-job-mat.this-is-a-rm
                          THEN ("Job/Form: " + TRIM(v-job) )
                          ELSE ("Order/FG#: " +
  ...
  IF choice AND po-ord.opened THEN DO:
    ASSIGN
        vLogicError = "Do you wish to update PO #" +
            TRIM(STRING(po-ord.po-no,">>>>>>>>")) + " for " +
            (IF AVAIL job THEN ("Job: " + TRIM(v-job) )
                          ELSE ("Order/FG#: " +
                                TRIM(STRING(v-ord-no,">>>>>>>>>>")))) +
            "  "  + "?" .
