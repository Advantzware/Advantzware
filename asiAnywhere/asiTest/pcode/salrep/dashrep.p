...
&GLOBAL-DEFINE salesrep 1
&GLOBAL-DEFINE raw-salesmen 2
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN v-outputfile = v-webrootpath + "salesrep_high_rpt_" + cocode
...
------------------------------------------------------------------------------*/
   ...
   ASSIGN
      chWorkSheet:Range("C22"):VALUE = ROUND(mtd-amt-total,2)
      chWorkSheet:Range("D22"):VALUE = ROUND(mtd-msf-total,2)
      chWorkSheet:Range("E22"):VALUE = IF mtd-msf-total NE 0 THEN
                                          ROUND(ROUND(mtd-amt-total,2) /
                                                ROUND(mtd-msf-total,2),2) ELSE 0
      
      chWorkSheet:Range("C43"):VALUE = ROUND(ytd-amt-total,2)
      chWorkSheet:Range("D43"):VALUE = ROUND(ytd-msf-total,2)
      chWorkSheet:Range("E43"):VALUE = IF ytd-msf-total NE 0 THEN
                                          ROUND(ROUND(ytd-amt-total,2) /
                                                ROUND(ytd-msf-total,2),2) ELSE 0

      chWorkSheet:Range("AM65"):VALUE = ROUND(ytd-amt-total,2)
      chWorkSheet:Range("AN65"):VALUE = ROUND(ytd-msf-total,2)
      chWorkSheet:Range("AO65"):VALUE = IF ytd-msf-total NE 0 THEN
                                           ROUND(ROUND(ytd-amt-total,2) /
                                                 ROUND(ytd-msf-total,2),2) ELSE 0
      row-count = 27.
