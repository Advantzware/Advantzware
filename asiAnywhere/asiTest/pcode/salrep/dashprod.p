...
/*------------------------------------------------------------------------
    ...
    MOD-001 DSG 11/13/2009 Save excel file to webroot and close excel                                                                      
  ----------------------------------------------------------------------*/
...
DEFINE SHARED VAR v-webrootpath AS CHAR NO-UNDO.
...
------------------------------------------------------------------------------*/
 ...
 ASSIGN v-outputfile = v-webrootpath + "prod_high_rpt_" + cocode 
     + STRING(YEAR(TODAY), "9999") + STRING(MONTH(TODAY), "99")
     + STRING(DAY(TODAY), "99") + ".xlsm"  .

 
 
 /* pickup file name*/ 
  ttoutputfile =  "prod_high_rpt_" + cocode 
     + STRING(YEAR(TODAY), "9999") + STRING(MONTH(TODAY), "99") 
     + STRING(DAY(TODAY), "99")  + ".xlsm".

 /* delete eventually existing file*/
 os-command silent del value(v-outputfile).
 /*OR*/

  /*SaveAs*/ 
 chExcelApplication:DisplayAlerts = FALSE.
 chWorkbook:SaveAs(v-outputfile,52,,,,,).
