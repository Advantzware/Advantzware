...
FIND FIRST fg-bin WHERE fg-bin.company = itemfg.company
                    AND fg-bin.i-no = itemfg.i-no NO-LOCK NO-ERROR.
IF AVAIL fg-bin THEN DO:
   FOR EACH fg-rcpth WHERE fg-rcpth.company = itemfg.company
       ...
       IF AVAIL fg-rdtlh THEN ASSIGN v-case-count = fg-rdtlh.qty-case
                                     /*v-pallet-count = fg-rdtlh.unit-pallet */
...
PUT unformatted
    ...
    "<P10><C26>" /*940 West 94th Street Chicago, IL 60620"*/ v-comp-add1 SKIP
    "<C25>" /*Phone (773)445-5030 - Fax (773)445-1636"*/ v-comp-add2 SKIP(1).
PUT
...
PUT
    ...
    "<=2><R+38><C12>SHELF LIFE:" "<C27>" v-spec-note FORM "x(40)"  /*AT 87*/ SKIP
