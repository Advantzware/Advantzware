...
DEFINE INPUT-OUTPUT PARAMETER DATASET FOR dsInvoLook.
...
if prmAction <> "search"  then do:
    FOR EACH ar-invl WHERE ar-invl.ord-no = INT(prmOrderNum) AND ar-invl.company = prmComp NO-LOCK:
        create ttInvoLook.
            assign                                         
                ttInvoLook.Invo     = ar-invl.inv-no
                ttInvoLook.InvoItem = ar-invl.i-no
                ttInvoLook.InvoName = ar-invl.i-name 
                ttInvoLook.Invocust-no = ar-invl.cust-no
            ...
            IF users.internal-user = NO THEN DO:
                FOR EACH ttInvoLook:
                    FIND FIRST usercust WHERE usercust.user_id = prmUser 
                        AND usercust.cust-no = ttInvoLook.Invocust-no
                        AND usercust.company EQ prmComp
                        NO-LOCK NO-ERROR.
                    IF NOT AVAILABLE usercust THEN DELETE ttInvoLook.
                END. /*FOR EACH ttInvoLook*/
...
IF prmAction = "search" then do:
    IF prmField = "ANY" then do:
        IF prmCondition = "EQUAL" then do:
            FOR EACH ar-invl WHERE
                ...
                ar-invl.company = prmComp
                no-lock:

                IF ar-invl.inv-no = int(prmText) OR
                   ar-invl.i-no = prmText or
                   ar-invl.i-name = prmText THEN
                DO:
                   create ttInvoLook.
                   assign
                       ttInvoLook.Invo     = ar-invl.inv-no
        ...
        IF prmCondition = "BEGIN" then do:
            FOR EACH ar-invl WHERE
                ...
                IF ar-invl.inv-no = int(prmText) OR
                   ar-invl.i-no begins prmText or
                   ar-invl.i-name begins prmText THEN
                DO:
                   create ttInvoLook.
                   assign                 
                       ttInvoLook.Invo     = ar-invl.inv-no
    ...
    if prmField = "inv-no"  then do:
        if prmCondition = "EQUAL" then do:
            FOR EACH ar-invl WHERE ar-invl.ord-no = INT(prmOrderNum) AND ar-invl.company = prmComp AND  ar-invl.inv-no = int(prmText) no-lock:
                create ttInvoLook.
                assign                                         
                    ttInvoLook.Invo     = ar-invl.inv-no
                    ttInvoLook.InvoItem = ar-invl.i-no
    ...
    IF prmField = "i-no" then do:
        if prmCondition = "EQUAL" then do:
            FOR EACH ar-invl WHERE ar-invl.ord-no = INT(prmOrderNum) AND ar-invl.company = prmComp AND ar-invl.i-no = prmText no-lock:
                create ttInvoLook.
                assign
                    ttInvoLook.Invo     = ar-invl.inv-no
                    ttInvoLook.InvoItem = ar-invl.i-no
        ...
        END. /*if prmCondition = EQUAL*/
        IF prmCondition = "BEGIN" then do:
            FOR EACH ar-invl  WHERE ar-invl.ord-no = INT(prmOrderNum) AND ar-invl.company = prmComp AND ar-invl.i-no begins prmText no-lock:
                create ttInvoLook.
                assign                                                      
                    ttInvoLook.Invo     = ar-invl.inv-no
                    ttInvoLook.InvoItem = ar-invl.i-no
