...
/*------------------------------------------------------------------------
    File        : UserComp.p
    Purpose     : UserComp Maintenance

    Syntax      :

    Description : Return a Dataset of all UserComp
...
CASE prmAction:
    WHEN "select" THEN DO:
        ...
        q-UserCompQuery:QUERY-PREPARE(v-qry-string).        
        DATASET dsUserComp:FILL().

        
    END.
    WHEN "delete" THEN DO:
        OUTPUT STREAM s1 TO catalog.txt.
        FOR EACH beforeUserComp TRANSACTION:
            ASSIGN v-return-value = BUFFER beforeUserComp:SAVE-ROW-CHANGES("usercomp") NO-ERROR.
        END.
...
PROCEDURE build-qry:
    ...
    IF  vComp <> "" THEN do:
        ...
        assign prm-query = prm-query +  " , EACH usercomp OF users where usercomp.loc NE '' and usercomp.company = '" + vComp + "' no-lock".
   ...
   ELSE do:
        ...
        ASSIGN prm-query = " PRESELECT EACH users NO-LOCK WHERE users.user_id = '" + prmId + "'  ".
        assign prm-query = prm-query +  " , EACH usercomp OF users where usercomp.loc = '' no-lock".
