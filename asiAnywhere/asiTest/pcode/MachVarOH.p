...
DEFINE INPUT-OUTPUT PARAMETER DATASET FOR dsMachVarOH .
...
DEF VAR ct AS INT INIT 3 NO-UNDO.
...
FOR EACH oe-ordl where oe-ordl.company EQ prmComp AND
    ...
    FOR EACH job WHERE job.company = oe-ordl.company 
        ...
        FOR each x-mch break by x-mch.line:
            create VarOH.
            ASSIGN
                VarOH.V-line = x-mch.line
                VarOH.V-form-no = x-mch.form-no
                VarOH.V-blank-no = x-mch.blank-no
                VarOH.V-m-code = x-mch.m-code
                VarOH.V-i-no   = x-mch.i-no
                VarOH.V-dept = x-mch.dept
                VarOH.V-run-std = x-mch.run-std
                VarOH.V-run-act = x-mch.run-act
                VarOH.V-run-var = x-mch.run-var
                VarOH.V-mr-std = x-mch.mr-std
                VarOH.V-mr-act = x-mch.mr-act
                VarOH.V-mr-var = x-mch.mr-var
                .
             
            if VarOH.V-run-act > 0 and x-mch.est-speed <> 0 THEN do:
                if ct = 6 THEN VarOH.V-run-std = x-mch.act-qty * (x-mch.wst-prct * .01).
                if ct = 5 THEN VarOH.V-run-std = x-mch.run-hrs * x-mch.est-speed.
                if ct < 5 THEN VarOH.V-run-std = (x-mch.run-std / x-mch.std-hrs) *
                    ((x-mch.act-qty + x-mch.wst-qty) / x-mch.est-speed).
            end.
            assign
                VarOH.V-run-var  = VarOH.V-run-act - VarOH.V-run-std
                VarOH.V-mr-var   = VarOH.V-mr-act  - VarOH.V-mr-std
                v-std-tot[1] = v-std-tot[1] + VarOH.V-run-std
                v-act-tot[1] = v-act-tot[1] + VarOH.V-run-act
                v-var-tot[1] = v-var-tot[1] + VarOH.V-run-var
                v-std-tot[2] = v-std-tot[2] + VarOH.V-mr-std
                v-act-tot[2] = v-act-tot[2] + VarOH.V-mr-act
                v-var-tot[2] = v-var-tot[2] + VarOH.V-mr-var.
        end. /*FOR each x-mch break by x-VarOH.V-line:*/
        FIND FIRST VarOH WHERE VarOH.V-form-no  EQ 0
                         AND VarOH.V-blank-no EQ 0
                         AND VarOH.V-m-code   EQ "ALL"
                          NO-ERROR.
        IF AVAIL VarOH THEN DELETE VarOH.
        CREATE VarOH.
        ASSIGN
            VarOH.V-form-no  = 0
            VarOH.V-blank-no = 0
            VarOH.V-m-code   = "ALL"
            VarOH.V-run-std  = v-std-tot[1]
            VarOH.V-run-act  = v-act-tot[1]
            VarOH.V-run-var  = v-var-tot[1]
            VarOH.V-mr-std   = v-std-tot[2]
            VarOH.V-mr-act   = v-act-tot[2]
            VarOH.V-mr-var   = v-var-tot[2]
            .
