...
/*------------------------------------------------------------------------
    ...
    Author(s)   : 
    Created     : Thu Dec 27 10:27:42 EST 2018
    Notes       :
  ----------------------------------------------------------------------*/
...
DEF  SHARED    VAR      fwd-embedded-mode AS LOG           NO-UNDO INIT FALSE.

DEFINE  SHARED VARIABLE g_lookup-var      AS CHARACTER     NO-UNDO.
DEFINE  SHARED VARIABLE g_track_usage     AS LOGICAL       NO-UNDO.
DEFINE  SHARED VARIABLE g_header_line     AS CHARACTER     NO-UNDO.
DEFINE  SHARED VARIABLE g_groups          AS CHARACTER     NO-UNDO.
DEFINE  SHARED VARIABLE init_menu         AS LOGICAL       NO-UNDO.
DEFINE  SHARED VARIABLE g_developer       AS CHARACTER     NO-UNDO.
DEFINE  SHARED VARIABLE g_version         AS CHARACTER     NO-UNDO.
DEFINE  SHARED VARIABLE g_rec_key         AS CHARACTER     NO-UNDO.
DEFINE  SHARED VARIABLE g_pageno          AS INTEGER       NO-UNDO.
DEFINE  SHARED VARIABLE g_mainmenu        AS WIDGET-HANDLE NO-UNDO.
DEFINE  SHARED VARIABLE g-sharpshooter    AS LOG           NO-UNDO.
DEFINE  SHARED VARIABLE cIniLoc           AS CHAR          NO-UNDO.
DEFINE  SHARED VARIABLE cUsrLoc           AS CHAR          NO-UNDO.
...
DEFINE SHARED        VARIABLE g_init            AS LOGICAL       NO-UNDO.
DEFINE SHARED        VARIABLE g_batch           AS LOGICAL       NO-UNDO.
DEFINE SHARED        VARIABLE g_batch-rowid     AS ROWID         NO-UNDO.
DEFINE SHARED        VARIABLE miscflds_reckey   AS CHARACTER.
DEFINE SHARED        VARIABLE table_reckey      AS CHARACTER.
DEFINE  SHARED VARIABLE persistent-handle AS HANDLE.
DEFINE SHARED        VARIABLE ListLogic-Handle  AS HANDLE.
DEFINE SHARED        VARIABLE igsSessionID      AS INTEGER.
...
ASSIGN 
    cIniVarList = "# Setup Variables,siteName,hostname,drive,dbDrive,topDir,mapDir,DLCDir,currVer,verDate,connectAudit,makeBackup,lockoutTries," +
...
PROCEDURE ipAutoLogin :
    /*------------------------------------------------------------------------------
      ...
      Notes:       
    ------------------------------------------------------------------------------*/
    DEFINE OUTPUT PARAMETER opcRunPgm AS CHARACTER NO-UNDO.
    ASSIGN 
    ...
    RUN ipClickOK (OUTPUT opcRunPgm).
...
PROCEDURE ipChangeEnvironment :
    ...
    CASE cbEnvironment:
        WHEN "Prod" THEN 
            DO:
                DO iCtr = 1 TO NUM-ENTRIES(cDatabaseList):
                    ...
                    DO:
                        ASSIGN
                            cbDatabase = ENTRY(iCtr,cDatabaseList).
                        LEAVE.
                    END.
                END.
            END.
        ...
        OTHERWISE 
        DO:
            IF iEnvLevel LT 16060000 THEN 
            DO iCtr = 1 TO NUM-ENTRIES(cDatabaseList):
                ASSIGN
                    iDbLevel = intVer(ENTRY(iCtr,cDbVerList)).
                IF iDbLevel GE 16050000
                    AND iDbLevel LT 16060000 AND cSessionParam EQ "" THEN 
                DO:
                    ASSIGN
                        cbDatabase = ENTRY(iCtr,cDatabaseList).
                    LEAVE.
                END.
            END.
            ELSE IF iEnvLevel LT 16070000 THEN 
                DO iCtr = 1 TO NUM-ENTRIES(cDatabaseList):
                    ASSIGN
                        iDbLevel = intVer(ENTRY(iCtr,cDbVerList)).
                    IF iDbLevel GE 16060000
                        AND iDbLevel LT 16070000 AND cSessionParam EQ "" THEN 
                    DO:
                        ASSIGN
                            cbDatabase = ENTRY(iCtr,cDatabaseList).
                        LEAVE.
                    END.
                END.
                ELSE IF iEnvLevel LT 16080000 THEN 
                    DO iCtr = 1 TO NUM-ENTRIES(cDatabaseList):
                        ASSIGN
                            iDbLevel = intVer(ENTRY(iCtr,cDbVerList)).
                        IF iDbLevel GE 16070000
                            AND iDbLevel LT 16080000 AND cSessionParam EQ "" THEN 
                        DO:
                            ASSIGN
                                cbDatabase = ENTRY(iCtr,cDatabaseList).
                            LEAVE.
                        END.
                    END.
                    ELSE IF iEnvLevel LT 16090000 THEN 
                        DO iCtr = 1 TO NUM-ENTRIES(cDatabaseList):
                            ASSIGN
                                iDbLevel = intVer(ENTRY(iCtr,cDbVerList)).
                            IF iDbLevel GE 16080000
                                AND iDbLevel LT 16090000 AND cSessionParam EQ "" THEN 
                            DO:
                                ASSIGN
                                    cbDatabase = ENTRY(iCtr,cDatabaseList).
                                LEAVE.
                            END.
                        END.
                        ELSE IF iEnvLevel LT 16100000 THEN 
                            DO iCtr = 1 TO NUM-ENTRIES(cDatabaseList):
                                ASSIGN
                                    iDbLevel = intVer(ENTRY(iCtr,cDbVerList)).
                                IF iDbLevel GE 16090000
                                    AND iDbLevel LT 16100000 AND cSessionParam EQ "" THEN 
                                DO:
                                    ASSIGN
                                        cbDatabase = ENTRY(iCtr,cDatabaseList).
                                    LEAVE.
...
PROCEDURE ipGetLists:
DEFINE OUTPUT PARAMETER opcEnvList          AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcModeList         AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcDbList           AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcModeScrList      AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcEnvironmentList  AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcEnvVerList       AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcDatabaseList     AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcDbVerList        AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcValidDBs         AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcValidEnvs        AS CHARACTER NO-UNDO.
DEFINE OUTPUT PARAMETER opcValidModes       AS CHARACTER NO-UNDO.



ASSIGN
  opcEnvList        = cEnvList
  opcModeList       = cModeList
  opcDbList         = cDbList
  opcModeScrList    = cMOdeScrList
  opcEnvironmentList = cEnvironmentList
  opcEnvVerList     = cEnvVerList
  opcDatabaseList   = cDatabaseList
  opcDbVerList      = cDbVerList
  opcValidDBs       = cValidDBs
  opcValidEnvs      = cValidEnvs
  opcValidModes     = cValidModes
  .
END PROCEDURE.

PROCEDURE ipGetSV:

  DEFINE OUTPUT PARAMETER opcDatabase         AS CHARACTER NO-UNDO.
  opcDatabase = cbDatabase.
...
DEFINE INPUT PARAMETER ipcPassword AS CHARACTER.
DEFINE INPUT PARAMETER ipcEnvironment AS CHARACTER.
DEFINE INPUT PARAMETER ipcMode AS CHARACTER.
DEFINE INPUT PARAMETER ipcDatabase AS CHARACTER.
DEFINE INPUT PARAMETER ipcEnvList AS CHARACTER.
DEFINE INPUT PARAMETER ipcDbList AS CHARACTER.
DEFINE INPUT PARAMETER ipcModeScr AS CHARACTER.
ASSIGN fiUserID = ipcUserID
       fiPassword = ipcPassword
       cbEnvironment = ipcEnvironment
       cbMode        = ipcMode
       cbDatabase    = ipcDatabase
       cEnvironmentList = ipcEnvList
       cDatabaseList = ipcDbList
       cModeScrList  = ipcModeScr
       .
