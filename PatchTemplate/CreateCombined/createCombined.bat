...
SET dbConnASI=-H !hostname! -S 2800 -N tcp
SET dbConnNOS=-H !hostname! -S 2802 -N tcp
SET dbConnEMP=-H !hostname! -S 2803 -N tcp
SET dbConnJOB=-H !hostname! -S 2804 -N tcp
SET dbConnRFQ=-H !hostname! -S 2806 -N tcp
SET dbConnHLP=-H !hostname! -S 2801 -N tcp
SET dbConnASN=-H !hostname! -S 2805 -N tcp -ld asinos
SET startTime=%time%
:ENDSETDEFAULTS
...
SET /P hostname="Enter the hostname of this machine (%hostname%): "
...
SET /P addonDir="Enter the directory where addon databases are located (%addonDir%): "
...
ECHO Addon DB Directory  : !topDir!!dbDir!!addonDir!
...
IF NOT EXIST "!TopDir!!dbDir!!splitDir!!addonDir!\emptrack.db" (
    IF NOT EXIST "!TopDir!!dbDir!!splitDir!\addon\emptrack.db" (
        ECHO Database emptrack not in expected location
        SET isError=Y
        )
    )
IF NOT EXIST "!TopDir!!dbDir!!splitDir!!addonDir!\jobs.db" (
    IF NOT EXIST "!TopDir!!dbDir!!splitDir!\addon\jobs.db" (
        ECHO Database jobs not in expected location
        SET isError=Y
        )
    )
IF NOT EXIST "!TopDir!!dbDir!!splitDir!!addonDir!\rfq.db" (
    IF NOT EXIST "!TopDir!!dbDir!!splitDir!\addon\rfq.db" (
        ECHO Database rfq not in expected location
        SET isError=Y
        )
    )
IF NOT EXIST "!TopDir!!dbDir!!splitDir!!addonDir!\nosweat.db" (
    ECHO Database asinos not in expected location
    SET isError=Y
    )
:ENDERRCHECK
IF "!isError!" == "Y" (
...
IF EXIST "!combDbName!_8.d2" SET pctComp=100
IF EXIST "!combDbName!_8.d3" SET pctComp=80
IF EXIST "!combDbName!_8.d4" SET pctComp=100
...
CALL !TopDir!!dbDir!!adminDir!!dataDir!\asinos\dumpasinos.bat
CD !TopDir!!DbDir!
:ENDDUMPDATA
...
CALL !TopDir!!dbDir!!adminDir!!dataDir!\asinos\loadasinos.bat
